...
/**
 * Provides the content of the ISO639-3 standard for language codes. Description of what
 * this standard is (taken from http://www.sil.org/iso639-3/default.asp):
 *
 * "ISO 639-3 is a code that aims to define three-letter identifiers for all known human
 * languages. At the core of ISO 639-3 are the individual languages already accounted for
 * in ISO 639-2. The large number of living languages in the initial inventory of ISO
 * 639-3 beyond those already included in ISO 639-2 was derived primarily from Ethnologue
 * (15th edition). Additional extinct, ancient, historic, and constructed languages have
 * been obtained from Linguist List."
 *
 * Source of the code tables in here: http://www.sil.org/iso639-3/iso-639-3_20100707.tab
 *
...
public final class ISO639_3 {
	 ...
	 *
	 * The quoted texts on the JavaDoc of the member variables are the original comments
	 * from the SQL-table-definition.
	 *
	 * All members are final, therefore objects of this class do not need to be cloned by
	 * clients of ISO639_3.
	 */
	public static final class LanguageCode implements Comparable<LanguageCode> {
		...
		/**
		 * "Equivalent 639-2 identifier of the bibliographic applications code set, if
		 * there is one", 3 characters, may be null.
		 *
		 */
		public final String part2B;

		/**
		 * "Equivalent 639-2 identifier of the terminology applications code set, if there
		 * is one", 3 characters, may be null.
		 */
	...
	private static Hashtable<String, LanguageCode> loadFromTabFile() {
		...
		try {
			...
			in = ISO639_3.class.getClassLoader().getResourceAsStream("/l10n/iso-639-3_20100707.tab");
...

			{
				...
				if (!headerTokens[0].equals("﻿Id") || !headerTokens[1].equals("Part2B")
						|| !headerTokens[2].equals("Part2T") || !headerTokens[3].equals("Part1")
						|| !headerTokens[4].equals("Scope") || !headerTokens[5].equals("Language_Type")
						|| !headerTokens[6].equals("Ref_Name") || !headerTokens[7].equals("Comment"))
			...
			for (String line = br.readLine(); line != null; line = br.readLine()) {
				...
				final LanguageCode newCode = new LanguageCode(tokens[0].toCharArray(), tokens[1].toCharArray(),
						tokens[2].toCharArray(), tokens[3].toCharArray(), LanguageCode.Scope.fromTabFile(tokens[4]),
						LanguageCode.Type.fromTabFile(tokens[5]), tokens[6], tokens.length == 8 ? tokens[7] : null);
	...
	/**
	 * Constructs a new ISO639_3 and loads the list of languages from the .tab file in the
	 * classpath. The list is cached for the lifetime of this ISO639_3 object, make sure
	 * not to keep the ISO639_3 object alive if you only use a small part of all
	 * languages.
	 * @throws RuntimeException If the .tab file is not present in the classpath or if
	 * parsing fails.
	 */
	public ISO639_3() {
	 ...
	 * @return Returns the map of all ISO639-3 language codes. The key in the returned
	 * list is the ID of the language code, which is the 3-letter code of ISO639-3. The
	 * given map is unmodifiable since it is used for the cache.
	 */
	...
	/**
	 * Gets a filtered list of languages. The list is cached for the lifetime of this
	 * ISO639_3 object.
	 * @param scope Must not be null.
	 * @param type Must not be null.
	 * @return Gets a {@link Hashtable} of language codes with the given scope and type.
	 * The key in the returned list is the ID of the language code, which is the 3-letter
	 * code of ISO639-3. The given Hashtable is free for modification.
	 */
	public final Hashtable<String, LanguageCode> getLanguagesByScopeAndType(LanguageCode.Scope scope,
		...
		for (final LanguageCode c : all.values()) {
			if (c.scope.equals(scope) && c.type.equals(type))
				result.put(c.id, c); // We do not clone the code because all its fields
										// are final.
		}
	...
	/**
	 * @return The special symbolic language code which is supposed to be a category for
	 * multiple languages.
	 */
