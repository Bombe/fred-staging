...
import freenet.bucket.PrependLengthOutputStream;
...
public abstract class ChecksumChecker {
	...
	/**
	 * Get an OutputStream that will write to a temporary Bucket, append a checksum and
	 * prepend a length.
	 * @param dos The underlying stream, which will not be closed.
	 * @param bf Used to allocate temporary storage.
	 * @throws IOException
	 */
	public PrependLengthOutputStream checksumWriterWithLength(final OutputStream dos, BucketFactory bf)
			throws IOException {
	...
	/**
	 * Read from disk and verify the checksum that follows the data. If it throws, the
	 * buffer will be zero'ed out.
	 */
