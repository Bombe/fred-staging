...
/**
 * Random access files with a limited number of open files, using a pool. LOCKING
 * OPTIMISATION: Contention on DEFAULT_FDTRACKER likely here. It's not clear how to avoid
 * that, FIXME. However, this is doing disk I/O (even if cached, system calls), so maybe
 * it's not a big deal ...
 *
...
public class PooledFileRandomAccessBuffer implements LockableRandomAccessBuffer, Serializable {
	...
	/**
	 * >0 means locked. We will wait until we get the lock if necessary, this is always
	 * accurate. LOCKING: Synchronized on fds.
	 */
	private int lockLevel;

	/**
	 * The actual RAF. Non-null only if open. LOCKING: Synchronized on (this). LOCKING:
	 * Always take (this) last, i.e. after fds.
	 */
	...
	/**
	 * -1 = not persistent-temp. Otherwise the ID. We need the ID so we can move files if
	 * the prefix changes.
	 */
	 ...
	 * @throws IOException
	 */
	public PooledFileRandomAccessBuffer(File file, boolean readOnly, long forceLength, Random seedRandom,
			long persistentTempID, boolean deleteOnFree) throws IOException {
		...
		try {
			...
			if (forceLength >= 0 && forceLength != currentLength) {
				...
				try (WrapperKeepalive wrapperKeepalive = new WrapperKeepalive()) {
					...
					// reflection
					Fallocate.forChannel(raf.getChannel(), raf.getFD(), forceLength).fromOffset(currentLength)
							.execute();
				}
	...
	public PooledFileRandomAccessBuffer(File file, String mode, byte[] initialContents, int offset, int size,
			long persistentTempID, boolean deleteOnFree, boolean readOnly) throws IOException {
		this.file = file;
		this.readOnly = readOnly;
		this.length = size;
		this.persistentTempID = persistentTempID;
	 ...
	 */
	public PooledFileRandomAccessBuffer(DataInputStream dis, BucketFilenameGenerator fg,
	...
	/**
	 * Must reimplement equals() as two PooledRAFWrapper's could well be the same storage
	 * object i.e. file on disk. This is particularly important during resuming a
	 * splitfile insert.
	 */
	@Override
