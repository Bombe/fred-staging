...
public final class EncryptedRandomAccessBuffer implements LockableRandomAccessBuffer, Serializable {
	...
	/**
	 * Creates an instance of EncryptedRandomAccessBuffer wrapping underlyingBuffer. Keys
	 * for key encryption and MAC generation are derived from the MasterSecret. If this is
	 * a new ERAT then keys are generated and the footer is written to the end of the
	 * underlying RAT. Otherwise the footer is read from the underlying RAT.
	 * @param type The algorithms to be used for the ERAT
	 * @param underlying The underlying RAT that will be storing the data. Must be larger
	 * than the footer size specified in type.
	 * @param masterKey The MasterSecret that will be used to derive various keys.
	 * @param newFile If true, treat it as a new file, and writer a header. If false, the
	 * ERAT must already have been initialised.
	 * @throws IOException
	 * @throws GeneralSecurityException
	 */
	public EncryptedRandomAccessBuffer(EncryptedRandomAccessBufferType type, LockableRandomAccessBuffer underlying,
			MasterSecret masterKey, boolean newFile) throws IOException, GeneralSecurityException {
		this.type = type;
	...
	private void setup(MasterSecret masterKey, boolean newFile) throws IOException, GeneralSecurityException {
		...
		try {
			KeyParameter cipherKey = new KeyParameter(KeyGenUtils
					.deriveSecretKey(unencryptedBaseKey, getClass(), kdfInput.underlyingKey.input, type.encryptKey)
					.getEncoded());
			tempPram = new ParametersWithIV(cipherKey, KeyGenUtils
					.deriveIvParameterSpec(unencryptedBaseKey, getClass(), kdfInput.underlyingIV.input, type.encryptKey)
					.getIV());
		}
	...
	/**
	 * Reads the specified section of the underlying RAT and decrypts it. Decryption is
	 * thread-safe.
	 */
	@Override
	...
	/**
	 * Encrypts the given data and writes it to the underlying RAT. Encryption is
	 * thread-safe.
	 */
	@Override
	...
	private void writeHeader() throws IOException, GeneralSecurityException {
		...
		try {
			...
			throw new GeneralSecurityException("Something went wrong with key generation. please " + "report",
					e.fillInStackTrace());
		}
	...
	/**
	 * Reads the iv, the encrypted key and the MAC from the footer. Then decrypts they key
	...
	private boolean verifyHeader() throws IOException, InvalidKeyException {
		...
		try {
			CryptByteBuffer crypt = new CryptByteBuffer(type.encryptType, headerEncKey, headerEncIV);
			unencryptedBaseKey = KeyGenUtils.getSecretKey(type.encryptKey, crypt.decryptCopy(encryptedKey));
		}
		catch (InvalidKeyException e) {
	...
	public static LockableRandomAccessBuffer create(DataInputStream dis, BucketFilenameGenerator fg,
