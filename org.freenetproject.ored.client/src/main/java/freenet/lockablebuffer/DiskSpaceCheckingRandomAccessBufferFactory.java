...
public class DiskSpaceCheckingRandomAccessBufferFactory
	...
	/**
	 * LOCKING: We synchronize throughout the whole operation to prevent fragmentation and
	 * to have an accurate free space estimate. FIXME ideally this would be
	 * per-filesystem. It might be possible to get that information from Java (1.7) via
	 * java.nio.file.
	 */
	private static final Lock lock = new ReentrantLock(true);

	public DiskSpaceCheckingRandomAccessBufferFactory(LockableRandomAccessBufferFactory underlying, File dir,
			long minDiskSpace) {
	...
	/**
	 * Create a new RAF for a specified file, which must exist but be 0 bytes long. Will
	 * delete the file if an RAF cannot be created.
