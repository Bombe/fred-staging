...
public class NewLZMACompressor extends AbstractCompressor {
	...
	// Dictionary size 1MB, this is equivalent to lzma -4, it uses 16MB to compress and
	// 2MB to decompress.
				...
				public void SetProgress(long processedInSize, long processedOutSize) {
					if (compressionEffectShouldBeChecked && processedInSize > amountOfDataToCheckCompressionRatio) {
						try {
							...
							throw new RuntimeException(e); // need to escape from foreign
															// API :-(
						}
	...
	public Bucket decompress(Bucket data, BucketFactory bf, long maxLength, long maxCheckSizeLength, Bucket preferred)
			throws IOException, CompressionOutputSizeException {
		Bucket output;
		if (preferred != null)
			output = preferred;
