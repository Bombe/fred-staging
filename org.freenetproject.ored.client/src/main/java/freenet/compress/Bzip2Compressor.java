 ...
 **
 ** Due to historical reasons (we used to use the ant-tools bz2 libraries, rather than
 * commons-compress) the compressed streams **DO NOT** have the standard "BZ" header.
 */
public class Bzip2Compressor extends AbstractCompressor {
	...
	public Bucket compress(Bucket data, BucketFactory bf, long maxReadLength, long maxWriteLength)
		...
		try (InputStream is = data.getInputStream(); OutputStream os = output.getOutputStream()) {
