...
public class RealCompressor {
					...
					public void run() {
						...
						try {
							try {
								...
								j.onFailure(new InsertException(InsertExceptionMode.INTERNAL_ERROR, t, null), null,
										context);
							}
	...
	private static int getMaxRunningCompressionThreads() {
		...
		if (!osName.contains("Windows") && (osName.toLowerCase().indexOf("mac os x") > 0)
			...
			// OS/X niceness is really weak, so we don't want any more background CPU load
			// than necessary
