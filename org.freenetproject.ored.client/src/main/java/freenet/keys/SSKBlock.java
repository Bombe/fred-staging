...
/**
 * SSKBlock. Contains a full fetched key. Can do a node-level verification. Can decode
 * original data when fed a ClientSSK.
 */
public class SSKBlock implements KeyBlock {
	...
	/*
	 * HEADERS FORMAT: 2 bytes - hash ID 2 bytes - symmetric cipher ID 32 bytes -
	 * E(H(docname)) ENCRYPTED WITH E(H(docname)) AS IV: 32 bytes - H(decrypted data), =
	 * data decryption key 2 bytes - data length + metadata flag 2 bytes - data
	 * compression algorithm or -1 IMPLICIT - hash of data IMPLICIT - hash of remaining
	 * fields, including the implicit hash of data
	 *
	 * SIGNATURE ON THE ABOVE HASH: 32 bytes - signature: R (unsigned bytes) 32 bytes -
	 * signature: S (unsigned bytes)
	 *
	 * PLUS THE PUBKEY: Pubkey Group
	 */
	...
	static public final short TOTAL_HEADERS_LENGTH = 2 + SIG_R_LENGTH + SIG_S_LENGTH + 2 + E_H_DOCNAME_LENGTH
			+ ClientSSKBlock.DATA_DECRYPT_KEY_LENGTH + 2 + 2;
	...
	/**
	 * Initialize, and verify data, headers against key. Provided key must have a pubkey,
	 * or we throw.
	 */
	public SSKBlock(byte[] data, byte[] headers, NodeSSK nodeKey, boolean dontVerify) throws SSKVerifyException {
		...
		if (x + SIG_R_LENGTH + SIG_S_LENGTH > headers.length)
			throw new SSKVerifyException(
					"Headers too short: " + headers.length + " should be at least " + x + SIG_R_LENGTH + SIG_S_LENGTH);
