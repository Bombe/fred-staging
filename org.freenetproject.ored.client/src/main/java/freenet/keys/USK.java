 ...
 *
 * Contains: - Enough information to produce a real SSK. - Site name. - Site edition
 * number.
 *
...
public class USK extends BaseClientKey implements Comparable<USK>, Serializable {
	...
	/*
	 * The character to separate the site name from the edition number in its SSK form. I
	 * chose "-", because it makes it ludicrously easy to go from the USK form to the SSK
	 * form, and we don't need to go vice versa.
	 */
	...
	// Extra must be verified on creation, and is fixed for now. FIXME if it becomes
	// changeable, need to keep values here.
	...
	public USK(byte[] pubKeyHash, byte[] cryptoKey, byte[] extra, String siteName, long suggestedEdition)
		...
		// Verify extra bytes, get cryptoAlgorithm - FIXME this should be a static method
		// or something?
		...
		if (pubKeyHash.length != NodeSSK.PUBKEY_HASH_SIZE)
			throw new MalformedURLException(
					"Pubkey hash wrong length: " + pubKeyHash.length + " should be " + NodeSSK.PUBKEY_HASH_SIZE);
		if (cryptoKey.length != ClientSSK.CRYPTO_KEY_LENGTH)
			throw new MalformedURLException(
					"Decryption key wrong length: " + cryptoKey.length + " should be " + ClientSSK.CRYPTO_KEY_LENGTH);
		hashCode = Fields.hashCode(pubKeyHash) ^ Fields.hashCode(cryptoKey) ^ siteName.hashCode()
				^ (int) suggestedEdition ^ (int) (suggestedEdition >> 32);
	}
	...
	// FIXME: Be careful with this constructor! There must not be an edition in the
	// ClientSSK!
	public USK(ClientSSK ssk, long myARKNumber) {
		...
		if (badDocNamePattern.matcher(siteName).matches()) // not error -- just "possible"
															// bug
	...
	public USK(USK usk) {
		...
		// If we can guarantee that neither USK nor anything getting it without copying
		// will change it?
		// db4o treats byte[] as individual byte members, so there are no issues with
		// deactivation.
	...
	public final USK copy() {
		...
		// FIXME when we are sure we don't need to copy the byte[]'s we might be able to
	...
	public FreenetURI turnMySSKIntoUSK(FreenetURI uri) {
		if (uri.getKeyType().equals("SSK") && Arrays.equals(uri.getRoutingKey(), pubKeyHash)
				&& Arrays.equals(uri.getCryptoKey(), cryptoKey)
				&& Arrays.equals(uri.getExtra(), ClientSSK.getExtraBytes(cryptoAlgorithm)) && uri.getDocName() != null
