...
public abstract class Key implements WritableToDataOutputStream, Comparable<Key> {
	...
	public static KeyBlock createBlock(short keyType, byte[] keyBytes, byte[] headersBytes, byte[] dataBytes,
			byte[] pubkeyBytes) throws KeyVerifyException {
	...
	/**
	 * Convert the key to a double between 0.0 and 1.0. Normally we will hash the key
	 * first, in order to make chosen-key attacks harder.
	 */
	 ...
	 * Get key type
	 * <ul>
	 * <li>High 8 bit (<tt>(type >> 8) & 0xFF</tt>) is the base type
	 * ({@link NodeCHK#BASE_TYPE} or {@link NodeSSK#BASE_TYPE}).
	...
	static Bucket decompress(boolean isCompressed, byte[] input, int inputLength, BucketFactory bf, long maxLength,
			short compressionAlgorithm, boolean shortLength) throws CHKDecodeException, IOException {
		if (maxLength < 0)
	 ...
	 */
	public static Compressed compress(Bucket sourceData, boolean dontCompress, short alreadyCompressedCodec,
			long sourceLength, long MAX_LENGTH_BEFORE_COMPRESSION, int MAX_COMPRESSED_DATA_LENGTH, boolean shortLength,
			...
			if (alreadyCompressedCodec >= 0) {
				...
				if (sourceData.size() > maxCompressedDataLength) {
					...
					for (COMPRESSOR_TYPE comp : comps) {
						...
						try {
							compressedData = (ArrayBucket) comp.compress(sourceData, new ArrayBucketFactory(),
									Long.MAX_VALUE, maxCompressedDataLength);
						}
						catch (CompressionOutputSizeException e) {
							continue;
						}
	...
	// Not just the routing key, enough data to reconstruct the key (excluding any pubkey
	// needed)
	...
	/**
	 * Get the full key, including any crypto type bytes, everything needed to construct a
	 * Key object
	 */
	 ...
	 * in-memory storage. E.g. for SSKs, strips the DSAPublicKey. Copies it whether or not
	 * we need to copy it because the original might pick up a pubkey after this call. And
	 * the returned key will not accidentally pick up extra data.
	 */
	public abstract Key archivalCopy();

	public static boolean isValidCryptoAlgorithm(byte cryptoAlgorithm) {
		return cryptoAlgorithm == ALGO_AES_PCFB_256_SHA256 || cryptoAlgorithm == ALGO_AES_CTR_256_SHA256;
	}
