...
public class ClientCHKBlock implements ClientKeyBlock {
	...
	/**
	 * Construct from data retrieved, and a key. Do not do full decode. Verify what can be
	 * verified without doing a full decode.
	 ...
	 * @throws IOException If there is a bucket error.
	 */
	@SuppressWarnings("deprecation") // FIXME Back compatibility, using dubious ciphers;
										// remove eventually.
	 ...
	 * Note that we assume that HMAC SHA256 is available; the problem is AES is limited to
	 * 128 bits.
	 * @return the original data
	 * @throws IOException If there is a bucket error.
	 */
	 ...
	 * @param cryptoKey The encryption key. Can be null in which case this is equivalent
	 * to a normal block encode.
	 */
	static public ClientCHKBlock encodeSplitfileBlock(byte[] data, byte[] cryptoKey, byte cryptoAlgorithm)
		...
		if (Rijndael.AesCtrProvider == null) {
			return encodeNewNoJCA(data, CHKBlock.DATA_LENGTH, md256, cryptoKey, false, (short) -1, cryptoAlgorithm,
	 ...
	 * @param alreadyCompressedCodec If !dontCompress, and this is >=0, then the data is
	 * already compressed, and this is the algorithm.
	 ...
	 */
	static public ClientCHKBlock encode(Bucket sourceData, boolean asMetadata, boolean dontCompress,
			short alreadyCompressedCodec, long sourceLength, String compressorDescriptor, byte[] cryptoKey,
	...
	// forceNoJCA for unit tests.
	static ClientCHKBlock encode(Bucket sourceData, boolean asMetadata, boolean dontCompress,
			short alreadyCompressedCodec, long sourceLength, String compressorDescriptor, byte[] cryptoKey,
			byte cryptoAlgorithm, boolean forceNoJCA) throws CHKEncodeException, IOException {
		byte[] finalData = null;
		byte[] data;
	...
	/**
	 * Format: [0-1]: Block hash algorithm [2-34]: HMAC (with cryptokey) of data + length
	 * bytes. [35-36]: Length bytes. Encryption: CTR with IV = 1st 16 bytes of the hash.
	 * (It has to be deterministic as this is a CHK and we need to be able to reinsert
	 * them easily): - Data - Length bytes.
	 ...
	 * @return
	 */
	public static ClientCHKBlock encodeNew(byte[] data, int dataLength, MessageDigest md256, byte[] encKey,
		...
		try {
			...
			// It's okay that this is the same for 2 blocks with the same key and the same
			// content.
	 ...
	 * @throws CHKEncodeException
	 */
	public static ClientCHKBlock encodeNewNoJCA(byte[] data, int dataLength, MessageDigest md256, byte[] encKey,
			boolean asMetadata, short compressionAlgorithm, byte cryptoAlgorithm, int blockHashAlgorithm)
			throws CHKEncodeException {
		if (cryptoAlgorithm != Key.ALGO_AES_CTR_256_SHA256)
			throw new IllegalArgumentException("Unsupported crypto algorithm " + cryptoAlgorithm);
	...
	@SuppressWarnings("deprecation") // FIXME Back compatibility, using dubious ciphers;
										// remove eventually.
	public static ClientCHKBlock innerEncode(byte[] data, int dataLength, MessageDigest md256, byte[] encKey,
			boolean asMetadata, short compressionAlgorithm, byte cryptoAlgorithm) {
		data = data.clone(); // Will overwrite otherwise. Callers expect data not to be
								// clobbered.
		if (cryptoAlgorithm != Key.ALGO_AES_PCFB_256_SHA256)
			throw new IllegalArgumentException("Unsupported crypto algorithm " + cryptoAlgorithm);
		byte[] header;
		...
		// For splitfiles we reuse the same decryption key for multiple blocks; it is
		// derived from the overall hash,
		...
		// Ouch!
		// Those bytes being 2 bytes for the length, followed by the first 30 bytes of the
		// data.
	 ...
	 */
	static public ClientCHKBlock encode(byte[] sourceData, boolean asMetadata, boolean dontCompress,
			short alreadyCompressedCodec, int sourceLength, String compressorDescriptor)
		...
		try {
			return encode(new ArrayBucket(sourceData), asMetadata, dontCompress, alreadyCompressedCodec, sourceLength,
					compressorDescriptor, null, Key.ALGO_AES_CTR_256_SHA256);
		}
		catch (IOException e) {
			// Can't happen
			throw new Error(e);
