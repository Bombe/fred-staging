...
/**
 * Client level CHK. Can be converted into a FreenetURI, can be used to decrypt a
 * CHKBlock, can be produced by a CHKBlock.
 */
public class ClientCHK extends ClientKey implements Serializable {
	...
	/*
	 * We use EXTRA_LENGTH above for consistency, rather than dis.read etc. Some code
	 * depends on this being accurate. Change those uses if you like.
	 */
	...
	/**
	 * @param routingKey The routing key. This is the overall hash of the header and
	 * content of the key.
	 * @param encKey The decryption key. This is not passed to other nodes and is
	 * extracted from the URI.
	 * @param isControlDocument True if the document is a Control Document. These carry
	 * metadata, whereas ordinary keys carry data, and have no type.
	 * @param algo The encryption algorithm's identifier. See ALGO_* for values.
	 */
	public ClientCHK(byte[] routingKey, byte[] encKey, boolean isControlDocument, byte algo,
	...
	/**
	 * Create from a raw binary CHK. This expresses the key information in as few bytes as
	 * possible.
	 * @throws IOException
	 */
	...
	static {
		for (byte cryptoAlgorithm = Key.ALGO_AES_PCFB_256_SHA256; cryptoAlgorithm <= Key.ALGO_AES_CTR_256_SHA256; cryptoAlgorithm++) {
			for (short compressionAlgorithm = -1; compressionAlgorithm <= (short) (COMPRESSOR_TYPE
					.countCompressors()); compressionAlgorithm++) {
	...
	public String toString() {
		return super.toString() + ':' + Base64.encode(routingKey) + ',' + Base64.encode(cryptoKey) + ','
				+ compressionAlgorithm + ',' + controlDocument + ',' + cryptoAlgorithm;
	}
	...
	/**
	 * Read a raw binary CHK. This is an ultra-compact representation, for splitfile
	 * metadata etc.
	 */
