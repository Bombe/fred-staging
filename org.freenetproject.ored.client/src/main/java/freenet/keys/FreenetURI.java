...
/**
 * Note that the metadata pairs below are not presently supported. They are supported by
 * the old (0.5) code however.
 *
 * FreenetURI handles parsing and creation of the Freenet URI format, defined as follows:
 * <p>
 * <code>freenet:[KeyType@]RoutingKey,CryptoKey[,n1=v1,n2=v2,...][/docname][/metastring]</code>
 * </p>
 * <p>
 * where KeyType is the TLA of the key (currently USK, SSK, KSK, or CHK). If omitted,
 * KeyType defaults to KSK. BUT: CHKs don't support or require a docname. KSKs and SSKs
 * do. Therefore CHKs go straight into metastrings.
 * </p>
 * <p>
 * For KSKs, the string keyword (docname) takes the RoutingKey position and the remainder
 * of the fields are inapplicable (except metastring). Examples: <coe>freenet:KSK@foo/bar
 * freenet:KSK@test.html freenet:test.html</code>.
 * </p>
 * <p>
 * RoutingKey is the modified Base64 encoded key value. CryptoKey is the modified Base64
 ...
 * </p>
 * <p>
 * The docname is only meaningful for SSKs, and is hashed with the PK fingerprint to get
 * the key value.
 * </p>
 * <p>
 * The metastring is meant to be passed to the metadata processing systems that act on the
 * retrieved document.
 * </p>
 * <p>
 * When constructing a FreenetURI with a String argument, it is now legal for CHK keys to
 * have a '.extension' tail, eg 'CHK@blahblahblah.html'. The constructor will simply chop
 * off at the first dot.
 * </p>
 * REDFLAG: Old code has a FieldSet, and the ability to put arbitrary metadata in through
...
public class FreenetURI implements Cloneable, Comparable<FreenetURI>, Serializable {
	...
	/**
	 * The meta-strings, in the order they are given. Typically we will construct the base
	 * key from the key type, routing key, extra, and document name
	 * (SSK@blah,blah,blah/filename, CHK@blah,blah,blah, KSK@filename or
	 * USK@blah,blah,blah/filename/20), fetch it, discover that it is a manifest, and look
	 * up the first meta-string. If this is the final data, we use that (and complain if
	 * there are meta-strings left), else we look up the next meta-string in the manifest,
	 * and so on. This is executed by SingleFileFetcher.
	 */
	private final String[] metaStr;

	/*
	 * for SSKs, routingKey is actually the pkHash. the actual routing key is calculated
	 * in NodeSSK and is a function of pkHash and the docName
	 */
	 ...
	 * @return False if there is no routing key or no crypto key (CHKs, SSKs, USKs have
	 * them, KSKs don't), or if the keys don't have the same crypto key and routing key.
	 */
	...
	public FreenetURI(String keyType, String docName, String metaStr, byte[] routingKey, byte[] cryptoKey) {
		this(keyType, docName, (metaStr == null ? (String[]) null : new String[] { metaStr }), routingKey, cryptoKey,
				null);
	}
	...
	public FreenetURI(String keyType, String docName, String[] metaStr, byte[] routingKey, byte[] cryptoKey,
			byte[] extra2, long suggestedEdition) {
		// this.uniqueHashCode = super.hashCode();
		this.keyType = keyType.trim().toUpperCase().intern();
		this.docName = docName;
		this.metaStr = metaStr;
		...
		if ((isSSK || isUSK || isKSK) && !sv.isEmpty()) {
			...
			if (isUSK) {
				...
				try {
					...
					throw (MalformedURLException) new MalformedURLException("Invalid suggested edition: " + e)
							.initCause(e);
				}
			}
			else
	...
	/**
	 * Get the document name. For a KSK this is everything from the @ to the first slash
	 * or the end of the key. For an SSK this is everything from the slash to the next
	 * slash or the end of the key. CHKs don't have a doc name, they only have meta
	 * strings.
	 */
	...
	/**
	 * Get the first meta-string. This is just after the main part of the key and the doc
	 * name. Meta-strings are directory (manifest) lookups delimited by /'es after the
	 * main key and the doc name if any.
	 */
	...
	/**
	 * Get the last meta string. Meta-strings are directory (manifest) lookups after the
	 * main key and the doc name if any. So the last meta string, if there is one, is from
	 * the last / to the end of the uri i.e. usually the filename.
	 */
	...
	/**
	 * Get all the meta strings. Meta strings are directory (manifest) lookups after the
	 * main key and the doc name if any. Examples:
	 *
	 * CHK@blah,blah,blah/filename
	 *
	 * This has a routing key, a crypto key, extra bytes, no document name, and one meta
	 * string "filename"
	 *
	 * SSK@blah,blah,blah/docname/dir/subdir/filename
	 *
	 * This has a routing key, a crypto key, extra bytes, a document name, and three meta
	 * strings "dir", "subdir" and "filename". The SSK including the docname is turned
	 * into a low level Freenet key, which we fetch. This will produce a metadata document
	 * containing a manifest, within which we look up "dir". This either gives us another
	 * metadata document directly, or a redirect if the dir is inserted separately. And so
	 * on. If it's a container, the files will be stored, with the metadata, in the
	 * container (tar.bz2 or whatever); the metadata fetched by SSK@blah,blah,blah/docname
	 * will say that there is a container and explain how to fetch it.
	 *
	 * KSK@gpl.txt
	 *
	 * This has no routing key, no crypto key, and no meta strings (but KSKs *can* have
	...
	/**
	 * Get the routing key. This is the first part of the key after the @ for CHKs, SSKs
	 * and USKs. For purposes of FreenetURI, KSKs do not have a routing key. For CHKs,
	 * this is ultimately derived from the hash of the encrypted data; for SSKs it is the
	...
	/**
	 * Get the crypto key. This is the second part of the key after the @ for CHKs, SSKs
	 * and USKs. For purposes of FreenetURI, KSKs do not have a crypto key. For CHKs, this
	 * is derived from the hash of the *original* plaintext data; for SSKs it is a
	 * separate key for decryption. The crypto key is kept on the requesting node and is
	 * not sent over the network - but of course many freesites and other documents on the
	 * network include URIs which do include crypto keys.
	 */
	 ...
	 * @return A new FreenetURI with the specified number of meta-strings removed from the
	 * end.
	 */
	...
	public FreenetURI setMetaString(String[] newMetaStr) {
		return new FreenetURI(keyType, docName, newMetaStr, routingKey, cryptoKey, extra, suggestedEdition);
	}
	...
	public String toString() {
		if (toStringCache == null)
			toStringCache = toString(false,
					false)/* + "#"+super.toString()+"#"+uniqueHashCode */;
	...
	/**
	 * Get the FreenetURI as a pure ASCII string, any non-english characters as well as
	 * any dangerous characters are encoded.
	 * @return
	 */
	 ...
	 * @param pureAscii If true, encode any non-english characters. If false, only encode
	 * dangerous characters (slashes e.g.).
	 */
	...
	/**
	 * Encode to a user-friendly, incomplete string with ... replacing some of the base64.
	 * Allow spaces, foreign chars etc.
	 */
	...
	/**
	 * Get the extra bytes. SSKs and CHKs have extra bytes, these come after the second
	 * comma, and specify encryption and hashing algorithms etc.
	 */
	...
	/**
	 * Create a FreenetURI from the binary form of the key, read from a stream, with no
	 * length.
	...
	/**
	 * Write a binary representation of this URI, with a short length, so it can be passed
	 * over if necessary.
	 * @param dos The stream to write to.
	 * @throws MalformedURLException If the key could not be written because of
	 * inconsistencies or other problems in the key itself.
	...
	/**
	 * Generate a suggested filename for the URI. This may be constructed from more than
	 * one part of the URI e.g. SSK@blah,blah,blah/sitename/ might return sitename. The
	 * returned string will already have been through FileUtil.sanitize().
	 */
	...
	/**
	 * Returns a <b>new</b> FreenetURI with a new suggested edition number. Note that the
	 * suggested edition number is only valid for USKs.
	 */
	...
	public FreenetURI setKeyType(String newKeyType) {
		return new FreenetURI(newKeyType, docName, metaStr, routingKey, cryptoKey, extra, suggestedEdition);
	}

	/**
	 * Returns a <b>new</b> FreenetURI with a new routing key. KSKs do not have a routing
	 * key.
	 */
	public FreenetURI setRoutingKey(byte[] newRoutingKey) {
		return new FreenetURI(keyType, docName, metaStr, newRoutingKey, cryptoKey, extra, suggestedEdition);
	}

	/**
	 * Throw an InsertException if we have any meta-strings. They are not valid for
	 * inserts, you must insert a directory to create a directory structure.
	 */
	...
	/**
	 * Throw an InsertException if the argument has any meta-strings. They are not valid
	 * for inserts, you must insert a directory to create a directory structure.
	 */
	...
	public URI toRelativeURI() throws URISyntaxException {
		...
		// Hence we can have slashes, question marks etc in the path, but they are
		// encoded.
	...
	/**
	 * Convert a USK into an SSK by appending "-" and the suggested edition to the
	 * document name and changing the key type.
	 */
	...
	public long getEdition() {
		if (keyType.equalsIgnoreCase("USK"))
			...
			Matcher matcher = docNameWithEditionPattern.matcher(docName);
			if (!matcher.matches()) /*
									 * Taken from uskForSSK, also modify there if
									 * necessary; TODO just use isSSKForUSK() here?!
									 */
				throw new IllegalStateException();
	...
	@Override
	/**
	 * This looks expensive, but 99% of the time it will quit out pretty early on: Either
	 * a different key type or a different routing key. The worst case cost is relatively
	 * bad though. Unfortunately we can't use a HashMap if an attacker might be able to
	 * influence the keys and create a hash collision DoS, so we *do* need this.
	 */
	...
	/**
	 * If this object is a USK/SSK insert URI, this function computes the request URI
	 * which belongs to it. If it is a CHK/KSK, the original URI is returned as CHK/KSK do
	 * not have a private insert URI, they are their own "insert URI".
	 *
	 * If you want to give people access to content at an URI, you should always publish
	 * only the request URI. Never give away the insert URI, this allows anyone to insert
	 * under your URI!
	 * @return The request URI which belongs to this insert URI.
	 * @throws MalformedURLException If this object is a USK/SSK request URI already. NOT
	 * thrown for CHK/KSK URIs!
	 */
