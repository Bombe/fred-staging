...
/**
 * Client-level SSK, i.e. a low level SSK with the decryption key needed to decrypt the
 * data once it is fetched. Note that you can only use this to *REQUEST* keys, not to
 * *INSERT* them, because it only has the public key and not the private key, @see
 * InsertableClientSSK.
 */
public class ClientSSK extends ClientKey {
	...
	private ClientSSK(ClientSSK key) {
		...
		hashCode = Fields.hashCode(pubKeyHash) ^ Fields.hashCode(cryptoKey) ^ Fields.hashCode(ehDocname)
				^ docName.hashCode();
	}

	public ClientSSK(String docName, byte[] pubKeyHash, byte[] extras, DSAPublicKey pubKey, byte[] cryptoKey)
	...
	public synchronized void setPublicKey(DSAPublicKey pubKey) {
		...
		if (!Arrays.equals(newKeyHash, pubKeyHash))
			throw new IllegalArgumentException("New pubKey hash does not match pubKeyHash: "
					+ HexUtil.bytesToHex(newKeyHash) + " ( " + HexUtil.bytesToHex(pubKey.asBytesHash()) + " != "
