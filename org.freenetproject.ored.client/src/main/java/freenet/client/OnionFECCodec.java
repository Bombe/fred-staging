...
public class OnionFECCodec extends FECCodec {
	...
	/**
	 * Cache of PureCode by {k,n}. The memory usage is relatively small so we account for
	 * it in the FEC jobs, see maxMemoryOverheadDecode() etc.
	 */
	...
	public long maxMemoryOverheadDecode(int dataBlocks, int checkBlocks) {
		...
		return matrixSize * 3; // Very approximately, the last one absorbing some columns
								// and fixed overhead.
	}
	...
	public int getCheckBlocks(int dataBlocks, CompatibilityMode compatibilityMode) {
		 ...
		 * redundancy by simply duplicating blocks, for obvious reasons (see e.g. Wuala).
		 * But we have to have some redundancy at the duplicating blocks level because we
		 * do use some keys directly etc: we store an insert in 3 nodes. We also cache it
		 * on 20 nodes, but generally the key will fall out of the caches within days. So
		 * long term, it's 3. Multiplied by 2 here, makes 6. Used to be 1.5 * 3 = 4.5.
		 * Wuala uses 5, but that's all FEC.
		 */
		int checkBlocks = dataBlocks * ClientConstants.SPLITFILE_CHECK_BLOCKS_PER_SEGMENT
				/ ClientConstants.SPLITFILE_SCALING_BLOCKS_PER_SEGMENT;
		if (dataBlocks >= ClientConstants.SPLITFILE_CHECK_BLOCKS_PER_SEGMENT)
			checkBlocks = ClientConstants.SPLITFILE_CHECK_BLOCKS_PER_SEGMENT;
		...
		if (compatibilityMode == InsertContext.CompatibilityMode.COMPAT_1250
			...
			// Builds of that period using the native FEC (ext #26) will segfault
			// sometimes on >100% redundancy.
