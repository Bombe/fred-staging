...
/**
 * Essentially a map of integer to incrementible integer. FIXME maybe move this to
 * support, give it a better name?
 *
...
public class FailureCodeTracker implements Cloneable, Serializable {
	 ...
	 * @param fs The SimpleFieldSet containing the FieldSet (non-verbose) form of the
	 * tracker.
	 */
	...
	/**
	 * Copy the FailureCodeTracker. We implement Cloneable to shut up findbugs, but
	 * Object.clone() won't work because it's a shallow copy, so we implement it with
	 * merge().
	 */
	@Override
	...
	public static int getFixedLength(boolean insert) {
		int upperLimit = insert ? InsertException.UPPER_LIMIT_ERROR_CODE : FetchException.UPPER_LIMIT_ERROR_CODE;
