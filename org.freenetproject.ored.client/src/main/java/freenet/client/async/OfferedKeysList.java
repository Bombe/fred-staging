...
/**
 * All the keys at a given priority which we have received key offers from other nodes
 * for.
 *
 * This list needs to be kept up to date when: - A request is removed. - A request's
 * priority changes. - A key is found. - A node disconnects or restarts (through the
 * BlockOffer objects on the FailureTable).
 *
...
public class OfferedKeysList extends BaseSendableGet implements RequestClient {
	...
	private final ArrayList<Key> keysList; // O(1) remove random element the way we use
											// it, see chooseKey().
	...
	OfferedKeysList(NodeClientRequest core, RandomSource random, short priorityClass, boolean isSSK,
	...
	public SendableRequestSender getSender(ClientContext context) {
		return new SendableRequestSender() {
			...
			public boolean send(NodeClientRequest core, final RequestScheduler sched, ClientContext context,
				...
				// We check the datastore, take up offers if any (on a short timeout), and
				// then quit if we still haven't fetched the data.
				// Obviously this may have a marginal impact on load but it should only be
				// marginal.
				core.asyncGet(key, true, new RequestCompletionListener() {

					@Override
					public void onSucceeded() {
						// We don't use ChosenBlockImpl so have to remove the keys from
						// the fetching set ourselves.
					...
					public void onFailed(LowLevelGetException e) {
						...
						// the fetching set ourselves.
						sched.removeFetchingKey(key);
						// Something might be waiting for a request to complete (e.g. if
						// we have two requests for the same key),
