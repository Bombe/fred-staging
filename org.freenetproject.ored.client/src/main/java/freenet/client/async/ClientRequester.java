...
/**
 * A high level request or insert. This may create any number of low-level requests of
 * inserts, for example a request may follow redirects, download splitfiles and unpack
 * containers, while an insert (for a file or a freesite) may also have to insert many
 * blocks. A high-level request is created by a client, has a FetchContext or
 * InsertContext for configuration. Compare to
 *
 * @see SendableRequest for a low-level request (which may still be multiple actual
 * requests or inserts). WARNING: Changing non-transient members on classes that are
 * Serializable can result in restarting downloads or losing uploads.
 */
public abstract class ClientRequester implements Serializable, ClientRequestSchedulerGroup {
	...
	/**
	 * The RequestClient, used to determine whether this request is persistent, and also
	 * we round-robin between different RequestClient's in scheduling within a given
	 * priority class and retry count.
	 */
	...
	protected ClientRequester(short priorityClass, RequestClient requestClient) {
		...
		hashCode = super.hashCode(); // the old object id will do fine, as long as we
	...
	/**
	 * Cancel the request. Inner method, subclasses should actually tell the
	 * ClientGetState or whatever to cancel itself: this does not do anything apart from
	 * set a flag!
	...
	/**
	 * Cancel the request. Subclasses must implement to actually tell the ClientGetState's
	 * or ClientPutState's to cancel.
	 * @param context The ClientContext object including essential but non-persistent
	 * objects such as the schedulers.
	 */
	...
	/**
	 * Get the URI for the request or insert. For a request this is set at creation, but
	 * for an insert, it is set when we know what the final URI will be.
	 */
	public abstract FreenetURI getURI();

	/**
	 * Is the request or insert completed (succeeded, failed, or cancelled, which is a
	 * kind of failure)?
	 */
	 ...
	 * @see #getLatestSuccess() Explanation of the content and especially the default
	 * value.
	 */
	...
	/**
	 * Has at least one block been scheduled to be sent to the network? Requests can be
	 * satisfied entirely from the datastore sometimes.
	 */
	 ...
	 * Initialized to current time for usability purposes: This allows the user to sort
	 * downloads by last success in the user interface to determine which ones are
	 * stalling - those will be the ones with the oldest last success date. If we
	 * initialized it to "null" only, that would not be possible: The user couldn't
	 * distinguish very old stalling downloads from downloads which merely had no success
	 * yet because they were added a short time ago.<br>
	 */
	public Date getLatestSuccess() {
		// clone() because Date is mutable.
		// Null-check for backwards compatibility: Old serialized versions of objects of
		// this
	...
	/**
	 * UTC Date of latest increase of {@link #failedBlocks} or
	 * {@link #fatallyFailedBlocks}.<br>
	...
	/**
	 * The set of blocks has been finalised, total will not change any more. Notify
	 * clients.
	 * @param context The ClientContext object including essential but non-persistent
	 * objects such as the schedulers.
	 */
	...
	public void completedBlock(boolean dontNotify, ClientContext context) {
		if (logMINOR)
			Logger.minor(this,
					"Completed block (" + dontNotify + "): total=" + totalBlocks + " success=" + successfulBlocks
							+ " failed=" + failedBlocks + " fatally=" + fatallyFailedBlocks + " finalised="
	 ...
	 * addMustSucceedBlocks to avoid confusing consequences when addMustSucceedBlocks does
	 * other things.
	 */
	 ...
	 * Called off-thread. Please do not change SimpleEventProducer to always produce
	 * events off-thread, it is better to deal with that here, because events could be
	 * re-ordered, which matters for some events notably SimpleProgressEvent.
	 */
	protected abstract void innerNotifyClients(ClientContext context);

	/**
	 * Called when we first send a request to the network. Ensures that it really is the
	 * first time and passes on to innerToNetwork().
	 */
	...
	/**
	 * Notify clients that a request has gone to the network, for the first time, i.e. we
	 * have finished checking the datastore for at least one part of the request.
	 */
	 ...
	 * @param ctx The ClientContext, contains essential transient objects such as the
	 * schedulers.
	 */
	...
	/**
	 * @return A byte[] representing the original client, to be written to the file
	 * storing a persistent download. E.g. for FCP, this will include the Identifier,
	 * whether it is on the global queue and the client name.
	 * @param checker Used to checksum and isolate large components where we can recover
	 * if they fail.
	 * @throws IOException
	 */
	...
	protected static byte[] getClientDetail(PersistentClientCallback callback, ChecksumChecker checker)
			throws IOException {
	...
	/**
	 * Called for a persistent request after startup. Should call notifyClients() at the
	 * end, after the callback has been registered etc.
	...
	/**
	 * Called by onResume() once and only once after restarting. Must be overridden, and
	 * must call super.innerOnResume().
	 * @throws ResumeFailedException
	 */
	...
	/**
	 * Get the group the request belongs to. For single requests (the default) this is the
	 * request itself; for those in a group, such as a site insert, it is a common value
	 * between them.
	 */
