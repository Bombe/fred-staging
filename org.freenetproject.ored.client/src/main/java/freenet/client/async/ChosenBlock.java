...
/**
 * A single selected request, including everything needed to execute it. Most important
 * functions are the callbacks, which run off-thread, call the upstream callbacks on the
 * SendableGet etc, and remove the fetching keys from the KeysFetchingLocally.
 *
...
public abstract class ChosenBlock {
	...
	/**
	 * The token indicating the key within the request to be fetched/inserted. Meaning is
	 * entirely defined by the request.
	 */
	...
	/**
	 * The actual data delivery goes through CRS.tripPendingKey(). This is just a
	 * notification for book-keeping purposes. We call the scheduler to tell it that the
	 * request succeeded, so that it can be rescheduled soon for more requests.
	...
	public boolean send(NodeClientRequest core, RequestScheduler sched) {
