 ...
 *
 * This stage: Attempt to compress the file. Off-thread if it will take a while. Then hand
 * it off to some combination of SingleBlockInserters and SplitFileInserters, possibly
...
class SingleFileInserter implements ClientPutState, Serializable {
	...
	/**
	 * If true, we are not the top level request, and should not update our parent to
	 * point to us as current put-stage.
	 */
	...
	/**
	 * When positive, means we will return metadata rather than a URI, once the metadata
	 * is under this length. If it is too short it is still possible to return a URI, but
	 * we won't return both.
	 */
	 ...
	 */
	SingleFileInserter(BaseClientPutter parent, PutCompletionCallback cb, InsertBlock block, boolean metadata,
			InsertContext ctx, boolean realTimeFlag, boolean dontCompress, boolean reportMetadataOnly, Object token,
			ARCHIVE_TYPE archiveType, boolean freeData, String targetFilename, boolean forSplitfile, boolean persistent,
	...
	void onCompressed(CompressionOutput output, ClientContext context) {
		...
		try {
			...
			cb.onFailure(new InsertException(InsertExceptionMode.INTERNAL_ERROR, t, null), SingleFileInserter.this,
					context);
		}
	}
	...
	void onCompressedInner(CompressionOutput output, ClientContext context) throws InsertException {
		...
		if (bestCodec != null) {
			if (logMINOR)
				Logger.minor(this,
						"The best compression algorithm is " + bestCodec + " we have gained"
								+ (100 - (bestCompressedDataSize * 100 / origSize)) + "% ! (" + origSize + '/'
								+ bestCompressedDataSize + ')');
			shouldFreeData = true; // must be freed regardless of whether the original
									// data was to be freed
			if (freeData) {
				block.getData().free();
			}
			block.nullData();
		...
		// We do NOT need to switch threads here: the actual compression is done by
		// InsertCompressor on the RealCompressor thread,
		// which then switches either to the database thread or to a new executable to run
		// this method.

		if (parent == cb) {
			short codecID = bestCodec == null ? -1 : bestCodec.metadataID;
			ctx.eventProducer.produceEvent(new FinishedCompressionEvent(codecID, origSize, bestCompressedDataSize),
					context);
			if (logMINOR)
		...
		if ((noMetadata || metadata) && archiveType == null) {
			if (fitsInOneBlockAsIs) {
				...
				// Just insert it
				ClientPutState bi = createInserter(parent, data, codecNumber, ctx, cb, metadata, (int) origSize, -1,
						true, context, shouldFreeData, forSplitfile);
				if (logMINOR)
		...
		if (fitsInOneCHK) {
			...
			if (reportMetadataOnly) {
				SingleBlockInserter dataPutter = new SingleBlockInserter(parent, data, codecNumber,
						FreenetURI.EMPTY_CHK_URI, ctx, realTimeFlag, cb, metadata, (int) origSize, -1, true, true,
						token, context, persistent, shouldFreeData,
						forSplitfile ? ctx.extraInsertsSplitfileHeaderBlock : ctx.extraInsertsSingleBlock,
						cryptoAlgorithm, forceCryptoKey);
				if (logMINOR)
				...
				MultiPutCompletionCallback mcb = new MultiPutCompletionCallback(cb, parent, token, persistent, false,
						ctx.earlyEncode);
				SingleBlockInserter dataPutter = new SingleBlockInserter(parent, data, codecNumber,
						FreenetURI.EMPTY_CHK_URI, ctx, realTimeFlag, mcb, metadata, (int) origSize, -1, true, false,
						token, context, persistent, shouldFreeData,
						forSplitfile ? ctx.extraInsertsSplitfileHeaderBlock : ctx.extraInsertsSingleBlock,
						cryptoAlgorithm, forceCryptoKey);
				if (logMINOR)
		...
		if (reportMetadataOnly) {
			SplitFileInserter sfi = new SplitFileInserter(persistent, parent, cb, dataRAF, shouldFreeData, ctx, context,
					origSize, bestCodec, block.clientMetadata, metadata, archiveType, cryptoAlgorithm, forceCryptoKey,
					hashThisLayerOnly, hashes, ctx.dontCompress, parent.getMinSuccessFetchBlocks(),
					parent.getTotalBlocks(), origDataLength, origCompressedDataLength, realTimeFlag, token);
			if (logMINOR)
			...
			SplitFileInserter sfi = new SplitFileInserter(persistent, parent, sh, dataRAF, shouldFreeData, ctx, context,
					origSize, bestCodec, block.clientMetadata, metadata, archiveType, cryptoAlgorithm, forceCryptoKey,
					hashThisLayerOnly, hashes, ctx.dontCompress, parent.getMinSuccessFetchBlocks(),
					parent.getTotalBlocks(), origDataLength, origCompressedDataLength, realTimeFlag, token);
			sh.sfi = sfi;
	...
	private RandomAccessBucket fixNotPersistent(RandomAccessBucket data, ClientContext context) throws InsertException {
		...
		// Note that SegmentedBCB *does* support splitting, so we don't need to do
	...
	private Metadata makeMetadata(ARCHIVE_TYPE archiveType, FreenetURI uri, HashResult[] hashes) {
		...
		if (archiveType != null)
			meta = new Metadata(DocumentType.ARCHIVE_MANIFEST, archiveType, null, uri, block.clientMetadata, data,
					compressed, req, total, topDontCompress, topCompatibilityMode, hashes);
		else // redirect
			meta = new Metadata(DocumentType.SIMPLE_REDIRECT, archiveType, null, uri, block.clientMetadata, data,
					compressed, req, total, topDontCompress, topCompatibilityMode, hashes);
	 ...
	 * @param forSplitfile Whether this insert is above a splitfile. This affects whether
	 * we do multiple inserts of the same block.
	 */
	private ClientPutState createInserter(BaseClientPutter parent, Bucket data, short compressionCodec,
		...
		if (uri.getKeyType().equals("USK")) {
			...
			SingleBlockInserter sbi = new SingleBlockInserter(parent, data, compressionCodec, uri, ctx, realTimeFlag,
					cb, isMetadata, sourceLength, token, addToParent, false, this.token, context, persistent, freeData,
	...
	/**
	 * When we get the metadata, start inserting it to our target key. When we have
	 * inserted both the metadata and the splitfile, call the master callback.
	 */
	public class SplitHandler implements PutCompletionCallback, ClientPutState, Serializable {
			...
			synchronized (this) {
				...
				if (state == sfi) {
					...
					if (!metaInsertSuccess && !metaInsertStarted) {
						...
						sfi = null;
						if (logMINOR)
							Logger.minor(this, "Metadata already started for " + this + " : success="
									+ metaInsertSuccess + " started=" + metaInsertStarted);
					}
				}
		...
		public void onFailure(InsertException e, ClientPutState state, ClientContext context) {
			...
			synchronized (this) {
				if (logMINOR)
					Logger.minor(this, "onFailure(): " + e + " on " + state + " on " + this + " sfi = " + sfi
							+ " metadataPutter = " + metadataPutter);
				if (state == sfi) {
					sfi = null;
				}
				else if (state == metadataPutter) {
			...
			// When it does, it will come back here, and we won't call fail(), because
			// fail() has already set finished = true.
			if (toFail)
				fail(e, context);
		}
		...
		public void onMetadata(Metadata meta, ClientPutState state, ClientContext context) {
			...
			synchronized (this) {
				...
				if (reportMetadataOnly) {
					...
					Logger.error(this,
							"Got metadata from unknown state " + state + " sfi=" + sfi + " metadataPutter="
									+ metadataPutter + " on " + this + " persistent=" + persistent,
							new Exception("debug"));
					e = new InsertException(InsertExceptionMode.INTERNAL_ERROR, "Got metadata from unknown state",
							null);
				}
				else {
					// Already started metadata putter ? (in which case we've got the
					// metadata twice)
			...
			if (metadataThreshold > 0 && metaBytes.length < metadataThreshold) {
				...
				// It's probably already included in the splitfile, but need to check
				// that, and test it.
				synchronized (this) {
					metaInsertSuccess = true;
				}
			...
			synchronized (this) {
				// Only the bottom layer in a multi-level splitfile pyramid has randomised
				// keys. The rest are unpredictable anyway, and this ensures we only need
				// to supply one key when reinserting.
				metadataPutter = new SingleFileInserter(parent, this, newBlock, true, ctx, realTimeFlag, false, false,
						token, archiveType, true, metaPutterTargetFilename, true, persistent, origDataLength,
			...
			synchronized (this) {
				...
				if (state == metadataPutter) {
					...
					if (metadataPutter != null) {
						...
						// Okay, return it.
						metaInsertSuccess = true; // Not going to start it now, so
													// effectively it has succeeded.
					}
				}
