...
/**
 * A thread which does postprocessing of decompressed data, in particular, writing it to
 * its final destination. This thread also handles hashing and filtering. If these are not
 * required, <code>null</code> may be passed through the relevant constructor arguments.
 */
public class ClientGetWorkerThread extends Thread {
	 ...
	 */
	@Deprecated // use @GetClientWorkerThread with schemeHostAndPort instead, pass null if
				// needed.
	public ClientGetWorkerThread(InputStream input, OutputStream output, FreenetURI uri, String mimeType,
			HashResult[] hashes, boolean filterData, String charset, FoundURICallback prefetchHook,
			TagReplacerCallback tagReplacer, LinkFilterExceptionProvider linkFilterExceptionProvider)
			throws URISyntaxException {
		this(input, output, uri, mimeType, null, hashes, filterData, charset, prefetchHook, tagReplacer,
	 ...
	 * @param charset Charset to be passed to the ContentFilter. Only needed if filterData
	 * is true.
	 ...
	 */
	public ClientGetWorkerThread(InputStream input, OutputStream output, FreenetURI uri, String mimeType,
			String schemeHostAndPort, HashResult[] hashes, boolean filterData, String charset,
	...
	public void run() {
		...
		try {
			...
			if (filterData) {
				...
				FilterStatus filterStatus = ContentFilter.filter(input, output, mimeType, uri, schemeHostAndPort,
						prefetchHook, tagReplacer, charset, linkFilterExceptionProvider);

				String detectedMIMEType = filterStatus.mimeType
						.concat(filterStatus.charset == null ? "" : "; charset=" + filterStatus.charset);
				synchronized (this) {
			...
			try {
				while (true) {
					...
					// Note this is only necessary because we might have an
					// AEADInputStream?
	...
	/**
	 * Blocks until all threads have finished executing and cleaning up. This method also
	 * passes an exception which occurred back to the parent thread.
