...
/**
 * Fetch a single block file. Used directly for very simple fetches, but also base class
 * for SingleFileFetcher.
 *
...
public class SimpleSingleFileFetcher extends BaseSingleFileFetcher implements ClientGetState, Serializable {
	...
	@Override
	public void onFailure(LowLevelException e, SendableRequestItem reqTokenIgnored, ClientContext context) {
		onFailure(translateException((LowLevelGetException) e), false, context);
	}
	...
	protected void onSuccess(FetchResult data, ClientContext context) {
		...
		rcb.onSuccess(new SingleFileStreamGenerator(data.asBucket(), persistent), data.getMetadata(), null, this,
				context);
	}
	...
	public void onSuccess(ClientKeyBlock block, boolean fromStore, Object reqTokenIgnored, ClientContext context) {
		...
		if (!block.isMetadata()) {
			...
			onFailure(new FetchException(FetchExceptionMode.INVALID_METADATA, "Metadata where expected data"), false,
					context);
		}
	}
