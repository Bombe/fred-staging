...
/**
 * Similar to SplitFileFetcherStorage. The status of a splitfile insert, including the
 * encoded check and cross-check blocks, is stored in a RandomAccessBuffer separate to
 * that containing the data itself. This class is only concerned with encoding, storage
 * etc, and should have as few dependencies on the runtime as possible, partly so that it
 * can be tested in isolation. Actual details of inserting the blocks will be kept on
 * SplitFileInserter.
 *
 * CONTENTS OF FILE: (for persistent case, transient leaves out most of the below) *
 * Magic, version etc. * Basic settings. * Settings for each segment. * Settings for each
 * cross-segment (including block lists). * Padded last block (if necessary) * Global
 * status * (Padding to a 4096 byte boundary) * Cross-check blocks (by cross-segment). *
 * Check blocks * Segment status * Segment key list (each key separately stored and
 * checksummed)
 *
 * Intentionally not as robust (against disk corruption in particular) as
 * SplitFileFetcherStorage: We encode the block CHKs when we encode the check blocks, and
 * if the block data has changed by the time we insert the data, we fail; there's no way
 * to obtain correct data. Similarly, we fail if the keys are corrupted. However, for most
 * of the duration of the insert neither the data, the check blocks or the keys will be
 * written, so it's still reasonable assuming you only get errrors when writing, and it
 * isn't really possible to improve on this much anyway...
 *
 * @author toad
 *
 */
public class SplitFileInserterStorage {
	...
	/**
	 * Length in bytes of the data being uploaded, i.e. the original file, ignoring
	 * padding, check blocks etc.
	 */
	...
	/**
	 * Compression codec that should be used to decompress the data. We do not do the
	 * compression here but we need it to generate the Metadata.
	 */
	...
	/**
	 * Number of segments which have 1 fewer block than segmentSize. Not necessarily valid
	 * for very old compatibility modes.
	 */
	...
	/**
	 * If true, the splitfile crypto key must be included in the metadata. If false, it
	 * was auto-generated so can be left implicit.
	 */
	...
	/**
	 * True if the size of the file is not exactly divisible by one block. If so, we have
	 * the last block, after padding, stored in raf. (This means we can change the padding
	 * algorithm slightly more easily)
	 */
	private final boolean hasPaddedLastBlock;

	/**
	 * Status. Generally depends on the status of the individual segments... Not
	 * persisted: Can be deduced from the state of the segments, except for the last 3
	 * states, which are only used during completion (we don't keep the storage around
	 * once we're finished).
	 */
	enum Status {
	 ...
	 * @param rafFactory A factory (persistent or not as appropriate) used to create the
	 * temporary RAF storing check blocks etc.
	 * @param bf A temporary Bucket factory used for temporarily storing the cross-segment
	 * settings.
	 * @param topRequiredBlocks The minimum number of blocks needed to fetch the content,
	 * so far. We will add to this for the final metadata.
	 * @throws IOException
	 * @throws InsertException
	 */
	public SplitFileInserterStorage(LockableRandomAccessBuffer originalData, long decompressedLength,
			SplitFileInserterStorageCallback callback, COMPRESSOR_TYPE compressionCodec, ClientMetadata meta,
			boolean isMetadata, ARCHIVE_TYPE archiveType, LockableRandomAccessBufferFactory rafFactory,
			boolean persistent, InsertContext ctx, byte splitfileCryptoAlgorithm, byte[] splitfileCryptoKey,
			byte[] hashThisLayerOnly, HashResult[] hashes, BucketFactory bf, ChecksumChecker checker, Random random,
			MemoryLimitedJobRunner memoryLimitedJobRunner, PersistentJobRunner jobRunner, Ticker ticker,
			KeysFetchingLocally keysFetching, boolean topDontCompress, int topRequiredBlocks, int topTotalBlocks,
		...
		if (cmode == CompatibilityMode.COMPAT_1250_EXACT) {
			...
			if (ctx.splitfileSegmentDataBlocks < segSize) {
				segs = (totalDataBlocks + ctx.splitfileSegmentDataBlocks - 1) / ctx.splitfileSegmentDataBlocks;
		...
		if (segs >= 20 && (cmode == CompatibilityMode.COMPAT_CURRENT
				|| cmode.ordinal() >= CompatibilityMode.COMPAT_1255.ordinal())) {
		...
		segments = makeSegments(segmentSize, segs, totalDataBlocks, crossCheckBlocks, deductBlocksFromSegments,
				persistent, cmode, random, keysFetching, consecutiveRNFsCountAsSuccess);
		...
		if (crossCheckBlocks != 0) {
			...
			for (int i = 0; i < crossSegments.length; i++) {
				...
				SplitFileInserterCrossSegmentStorage seg = new SplitFileInserterCrossSegmentStorage(this, i, persistent,
						segLen, crossCheckBlocks);
				crossSegments[i] = seg;
		...
		if (persistent) {
			...
			try {
				crossSegmentSettings = encodeCrossSegmentSettings(bf); // Checksummed with
																		// length
			}
			catch (IOException e) {
				throw new InsertException(InsertExceptionMode.BUCKET_ERROR,
						"Failed to write to temporary storage while creating splitfile inserter", null);
			}
		}
		...
		// Encrypted RAFs are not initialised with 0's, so we need to clear explicitly (we
		// do store keys even for transient inserts).
		for (SplitFileInserterSegmentStorage segment : segments) {
			segment.clearKeys();
		}
		...
		// Include the cross check blocks in the required blocks. The actual number needed
		// may be
		// slightly less, but this is consistent with fetching, and also with pre-1468
		// metadata.
	 ...
	 * @param raf The file the insert was stored to. Caller must resume it before calling
	 * constructor.
	 * @param originalData The original data to be inserted. Caller must resume it before
	 * calling constructor.
	 ...
	 */
	public SplitFileInserterStorage(LockableRandomAccessBuffer raf, LockableRandomAccessBuffer originalData,
			SplitFileInserterStorageCallback callback, Random random, MemoryLimitedJobRunner memoryLimitedJobRunner,
			PersistentJobRunner jobRunner, Ticker ticker, KeysFetchingLocally keysFetching,
			BucketFilenameGenerator persistentFG, PersistentFileTracker persistentFileTracker, MasterSecret masterKey)
		...
		LockableRandomAccessBuffer rafOrig = BucketTools.restoreRAFFrom(dis, persistentFG, persistentFileTracker,
				masterKey);
		...
		long blocks = 0;
		for (int i = 0; i < segmentCount; i++) {
			segments[i] = new SplitFileInserterSegmentStorage(this, dis, i, keyLength, splitfileCryptoAlgorithm,
					splitfileCryptoKey, random, maxRetries, consecutiveRNFsCountAsSuccess, keysFetching);
	...
	/**
	 * This one could actually be rather large, since it includes the listing of which
	 * blocks go in which cross-segments ...
	 */
	...
	private void allocateCrossCheckBlock(SplitFileInserterCrossSegmentStorage segment, Random xsRandom) {
		...
		for (int i = 0; i < 10; i++) {
			x = xsRandom.nextInt(segments.length);
			SplitFileInserterSegmentStorage seg = segments[x];
			int blockNum = seg.allocateCrossCheckBlock(segment, xsRandom, segment.getAllocatedCrossCheckBlocks());
			if (blockNum >= 0) {
	...
	private SplitFileInserterSegmentStorage[] makeSegments(int segmentSize, int segCount, int dataBlocks,
			int crossCheckBlocks, int deductBlocksFromSegments, boolean persistent, CompatibilityMode cmode,
			Random random, KeysFetchingLocally keysFetching, int consecutiveRNFsCountAsSuccess) {
		...
		if (segCount == 1) {
			...
			segments[0] = new SplitFileInserterSegmentStorage(this, 0, persistent, dataBlocks, checkBlocks,
					crossCheckBlocks, keyLength, splitfileCryptoAlgorithm, splitfileCryptoKey, random, maxRetries,
			...
			for (int i = segmentSize;;) {
				...
				j = i;
				segments[segNo] = new SplitFileInserterSegmentStorage(this, segNo, persistent, data, check,
						crossCheckBlocks, keyLength, splitfileCryptoAlgorithm, splitfileCryptoKey, random, maxRetries,
						consecutiveRNFsCountAsSuccess, keysFetching);

				if (deductBlocksFromSegments != 0)
					if (logMINOR)
						Logger.minor(this, "INSERTING: Segment " + segNo + " of " + segCount + " : " + data
								+ " data blocks " + check + " check blocks");
	...
	public void start() {
		...
		synchronized (this) {
			...
			if (status == Status.ENCODED)
				return;
			if (status == Status.FAILED || status == Status.GENERATING_METADATA || status == Status.SUCCEEDED)
				return;
		}
		for (SplitFileInserterSegmentStorage segment : segments)
			segment.checkKeys();
	...
	/**
	 * Called when a cross-segment finishes encoding blocks. Can be called inside locks as
	 * it runs off-thread.
	 * @param completed
	 */
	...
	/**
	 * Called when a segment finishes encoding blocks. Can be called inside locks as it
	 * runs off-thread.
	 * @param completed
	 */
	...
	/**
	 * Lock the main RAF open to avoid the pooled fd being closed when we are doing a
	 * major I/O operation involving many reads/writes.
	 */
	...
	/**
	 * Lock the originalData RAF open to avoid the pooled fd being closed when we are
	...
	/**
	 * Encode the Metadata. The caller must ensure that all segments have encoded keys
	 * first.
	 * @throws MissingKeyException This indicates disk corruption or a bug (e.g. not all
	 * segments had encoded keys). Since we don't checksum the blocks, there isn't much
	 * point in trying to recover from losing a key; but at least we can detect that there
	 * was a problem.
	 *
	...
	Metadata encodeMetadata() throws IOException, MissingKeyException {
		...
		return new Metadata(splitfileType, dataKeys, checkKeys, segmentSize, checkSegmentSize, deductBlocksFromSegments,
				clientMetadata, dataLength, archiveType, compressionCodec, decompressedLength, isMetadata, hashes,
				hashThisLayerOnly, origDataSize, origCompressedDataSize, topRequiredBlocks, topTotalBlocks,
				topDontCompress, cmode, splitfileCryptoAlgorithm, splitfileCryptoKey, specifySplitfileKeyInMetadata,
				crossCheckBlocks);
	}
	...
	void innerWriteSegmentKey(int segNo, int blockNo, byte[] buf) throws IOException {
		...
		if (logDEBUG)
			Logger.debug(this,
					"Writing key for block " + blockNo + " for segment " + segNo + " of " + this + " to " + fileOffset);
	...
	byte[] innerReadSegmentKey(int segNo, int blockNo) throws IOException {
		...
		if (logDEBUG)
			Logger.debug(this,
					"Reading key for block " + blockNo + " for segment " + segNo + " of " + this + " to " + fileOffset);
			...
			public boolean run(ClientContext context) {
				...
				if (allSegmentsSucceeded()) {
					...
					try {
						...
						// Fail here too. If we're getting disk corruption on keys, we're
						// probably
						// getting it on the original data too.
						InsertException e1 = new InsertException(InsertExceptionMode.BUCKET_ERROR, "Missing keys",
								null);
						synchronized (this) {
							failing = e1;
							status = Status.FAILED;
	...
	void fail(final InsertException e) {
		synchronized (this) {
			if (this.status == Status.SUCCEEDED || this.status == Status.FAILED
				...
				// Not serious but often indicates a problem e.g. we are sending requests
				// after completing.
	...
	/**
	 * Choose a block to insert. FIXME make SplitFileInserterSender per-segment, eliminate
	...
	public BlockInsert chooseBlock() {
		...
		// lowest-retry-count from
		// each segment?
		// Less important for inserts than for requests though...
		synchronized (cooldownLock) {
			synchronized (this) {
				if (status == Status.FAILED || status == Status.SUCCEEDED || status == Status.GENERATING_METADATA
						|| failing != null) {
					return null;
				}
			}
			// Generally segments are fairly well balanced, so we can usually pick a
			// random segment
	...
	/**
	 * @return -1 if the insert has finished, 0 if has blocks to send, otherwise
	 * Long.MAX_VALUE.
	 */
