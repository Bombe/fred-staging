...
/**
 * Represents a single segment, in memory and on disk. Handles storage and decoding. Note
 * that the on-disk data, and therefore the read-in metadata, may be inaccurate; we check
 * everything opportunistically. Hence we are very robust (but not completely immune) to
 * disk corruption.
 *
...
public class SplitFileFetcherSegmentStorage {
	...
	// Set this to false to turn off checking the CHKs on blocks decoded (and encoded) via
	// FEC.
	// Generally it is a good idea to have consistent behaviour regardless of what order
	// we fetched
	// the blocks in, whether binary blobs are enabled etc ... and this has caught nasty
	// bugs in
	...
	/**
	 * Offset to the segment's cross-check block data. This may be kept separately if we
	 * are going to complete the fetch via truncation, otherwise it's at the end of the
	 * segment data.
	 */
	...
	/**
	 * Length of the segment status for purposes of locating it on disk, may be larger
	 * than segmentStatusLength.
	 */
	...
	/**
	 * Count of cross-segment check blocks. These occur only in larger splitfiles and
	 * count as data blocks for segment-level FEC, but they also count as check blocks for
	 * cross-segment level FEC. Generally between 0 and 3. Numbered dataBlocks ..
	 * dataBlocks+crossSegmentCheckBlocks-1
	 */
	public final int crossSegmentCheckBlocks;

	/**
	 * Count of check blocks (generated by FEC). Numbered
	 * dataBlocks+crossSegmentCheckBlocks ..
	 * dataBlocks+crossSegmentCheckBlocks+checkBlocks-1
	 */
	public final int checkBlocks;

	/**
	 * Keeps track of how many times we've tried each block, which blocks we have
	 * downloaded etc.
	 */
	private final SplitFileFetcherSegmentBlockChooser blockChooser;

	/**
	 * What is the order of the blocks on disk? Should be kept consistent with
	 * blocksFound! Is read from disk on startup and may be inaccurate, checked on FEC
	 * decode. Elements: -1 = not fetched yet.
	 */
	private final int[] blocksFetched;

	/**
	 * True if we have downloaded and decoded all the data blocks and cross-segment check
	 * blocks, and written them to their final location in the parent storage file.
	 */
	private boolean succeeded;

	/**
	 * True if we have not only downloaded and decoded, but also finished with encoding
	 * and queueing healing blocks.
	 */
	private boolean finished;

	/**
	 * True if the segment has been cancelled, has failed due to an internal error, etc.
	 * In which case it is not interested in further blocks. Not true if it has run out of
	 * retries, in which case (for cross-segment) we may still be interested in blocks.
	 */
	...
	/**
	 * The cross segments for each data or cross-segment check block. This allows us to
	 * tell the cross-segments when we may have data to decode. The array is null if there
	 * are no cross-segments, and the elements are null if there is no associated
	 * cross-segment.
	 */
	 ...
	 * @param segmentCrossCheckDataOffset -1 to mean store the cross-check blocks just
	 * after the data and check blocks for this segment. Otherwise the offset.
	...
	public SplitFileFetcherSegmentStorage(SplitFileFetcherStorage parent, int segNumber,
			...
			long segmentDataOffset, long segmentCrossCheckDataOffset, long segmentKeysOffset, long segmentStatusOffset,
		...
		this.blockChooser = new SplitFileFetcherSegmentBlockChooser(total, parent.random, parent.maxRetries,
				parent.cooldownTries, parent.cooldownLength, this, keysFetching, ignoreLastBlock ? dataBlocks - 1 : -1);
		...
		for (int i = 0; i < blocksFetched.length; i++)
			blocksFetched[i] = -1;
		segmentStatusPaddedLength = paddedStoredSegmentStatusLength(dataBlocks, checkBlocks, crossCheckBlocks,
				writeRetries, parent.checksumLength, parent.persistent);
		segmentKeyListLength = storedKeysLength(blocksForDecode(), checkBlocks, parent.splitfileSingleCryptoKey != null,
	...
	/**
	 * Construct from a saved file. Uses the DataInputStream to read static settings, i.e.
	 * number of blocks, does not use the RAF to read block status etc; caller must call
	 * readMetadata and readKeys separately for that.
	 * @param parent
	 * @param dis DataInputStream to which the static settings have been saved. Anything
	 * else we will need to read separately from the RandomAccessBuffer.
	 ...
	 */
	public SplitFileFetcherSegmentStorage(SplitFileFetcherStorage parent, DataInputStream dis, int segNo,
			boolean writeRetries, long segmentDataOffset, long segmentCrossCheckDataOffset, long segmentKeysOffset,
			long segmentStatusOffset, KeysFetchingLocally keysFetching) throws IOException, StorageFormatException {
		this.segNo = segNo;
		this.parent = parent;
		...
		boolean ignoreLastBlock = (segNo == parent.segments.length - 1 && parent.lastBlockMightNotBePadded());
		blockChooser = new SplitFileFetcherSegmentBlockChooser(total, parent.random, parent.maxRetries,
				parent.cooldownTries, parent.cooldownLength, this, keysFetching, ignoreLastBlock ? dataBlocks - 1 : -1);
		int minFetched = blocksForDecode();
		...
		for (int i = 0; i < blocksFetched.length; i++)
			blocksFetched[i] = -1;
		segmentStatusPaddedLength = paddedStoredSegmentStatusLength(dataBlocks, checkBlocks, crossSegmentCheckBlocks,
				writeRetries, parent.checksumLength, true);
		segmentKeyListLength = storedKeysLength(blocksForDecode(), checkBlocks, parent.splitfileSingleCryptoKey != null,
				parent.checksumLength);
	...
	/**
	 * Read all the blocks, encode them according to their supposed keys and check that
	 * they are in fact the blocks that they should be. If the metadata is inaccurate,
	 * update it and writeMetadata(). If we have enough blocks to decode, and we don't
	 * have all the blocks, then schedule a decode on the FEC thread.
	 * @return True if we scheduled a decode or are already finished. False if we do not
	 * have enough blocks to decode and need to fetch more blocks.
	 */
			...
			public boolean start(MemoryLimitedChunk chunk) {
				...
				try {
					...
					try {
						...
						// In particular, if we failed, we may need to complete
						// cancellation, and we
						// can't do that until both tryDecode=false and parent gets the
						// callback.
						if (!shutdown)
	...
	private void innerDecode(MemoryLimitedChunk chunk) throws IOException {
		...
		// Even if we fail, once we set tryDecode=true, we need to notify the parent when
		// we're done.
		boolean fail;
		synchronized (this) {
			if (finished)
		...
		synchronized (this) {
			...
			if (fetchedCount < blocksForDecode()) {
				...
				if (blockChooser.successCount() != oldBlocksFetchedCount) {
					Logger.warning(this, "Corrected block count to " + blockChooser.successCount() + " from "
							+ oldBlocksFetchedCount);
				}
			}
		}
		if (fetchedCount < blocksForDecode()) {
			// We *DO* want to write the metadata immediately here, because we've just
			// gone over
		...
		for (MyBlock test : maybeBlocks) {
			...
			try {
				ClientCHKBlock block = ClientCHKBlock.encodeSplitfileBlock(buf, decodeKey.getCryptoKey(),
		...
		// Report success if we are not verifying decoded keys, but if we *are*, we need
		// to wait
		...
		if (checkDecodedKeys) {
			if (!checkEncodedDataBlocks(checkBlocks, checkBlocksPresent, keys, capturingBinaryBlob)) {
				...
				// That means the splitfile is broken as inserted, or there's some wierd
	...
	private void checkDecodedDataBlocks(byte[][] dataBlocks, boolean[] dataBlocksPresent, SplitFileSegmentKeys keys,
		...
		for (int i = 0; i < dataBlocks.length; i++) {
			...
			try {
				...
				// Impossible!
				parent.fail(
						new FetchException(FetchExceptionMode.INTERNAL_ERROR, "Decoded block could not be encoded"));
	...
	private boolean checkEncodedDataBlocks(byte[][] checkBlocks, boolean[] checkBlocksPresent,
		...
		for (int i = 0; i < checkBlocks.length; i++) {
			...
			try {
				...
				ClientCHK actualKey = block.getClientKey();
				if (!actualKey.equals(decodeKey)) {
					Logger.error(this, "Splitfile check block " + i + " does not encode to expected key for " + this
							+ " for " + parent);
					return false;
				}
				if (capturingBinaryBlob)
	...
	/**
	 * A block has been fetched which the caller believes is one of ours. Check whether it
	 * is in fact ours, and that we don't have it already. Find the key and decode it, and
	 * add it to our collection. If any cross-segments are waiting for this block, tell
	 * them. If we can decode, do so. Can be quite involved, should be called off-thread.
	 * @param key
	 * @param block
	 * @throws IOException If we were unable to write the block to disk.
	 * @return True if we successfully decoded a block, in which case the function will be
	 * called again. False if there was no match, if we have already fetched that block,
	 * or if various errors occurred.
	 */
	public boolean onGotKey(NodeCHK key, CHKBlock block) throws IOException {
		...
		synchronized (this) {
			...
			if (blockChooser.hasSucceeded(blockNumber))
				return false; // Even if this is inaccurate, it will be corrected on a FEC
								// attempt.
			if (tryDecode)
				return false;
	 ...
	 * @param blockNumber A block number for the data. (But there may be several block
	 * numbers with identical keys).
	 * @param decodedData The block's payload data.
	 * @return True if we decoded the block, or
	 * @throws IOException
	 */
	boolean innerOnGotKey(NodeCHK key, ClientCHKBlock block, SplitFileSegmentKeys keys, int blockNumber,
			byte[] decodedData) throws IOException {
		if (decodedData.length != CHKBlock.DATA_LENGTH) {
			if (blockNumber == dataBlocks - 1 && this.segNo == parent.segments.length - 1
				...
				// FIXME We can use it if we have all the other data blocks, but it's not
				// worth
				// checking, and might have non-obvious complications if we e.g. have data
				// loss in
				// FEC decoding.
		...
		do {
			...
			int slotNumber;
			// LOCKING We have to do the write inside the lock to prevent parallel decodes
			// messing up etc.
			synchronized (this) {
				if (succeeded || failed || finished) {
					if (logMINOR)
				...
				if (blockChooser.successCount() >= blocksForDecode()) {
					...
					// Don't remove it from the filter. We haven't written it, so it could
					// be
					// removed twice. And if we decode successfully, the filter will be
					// ignored.
					return saved;
				}
			...
			lazyWriteMetadata();
			if (logMINOR)
				Logger.minor(this, "Got block " + blockNumber + " (" + key + ") for " + this + " for " + parent
						+ " written to " + slotNumber);
	...
	private synchronized void writeDownloadedBlock(int slotNumber, byte[] data) throws IOException {
		// FIXME Do we need to pad here for really old splitfiles, or does the FEC code do
		// it?
	 ...
	 * synchronized(this). Metadata is fairly sparse on disk, we are expected to deduce it
	 * (and check it) when constructing.
	 * @throws IOException
	 */
	...
	/**
	 * Only called during construction. Reads the variable metadata from the
	 * RandomAccessBuffer.
	...
	public static int paddedStoredSegmentStatusLength(int dataBlocks, int checkBlocks, int crossCheckBlocks,
		...
		if (!persistent)
			return 0;
		return storedSegmentStatusLength(dataBlocks, checkBlocks, crossCheckBlocks, trackRetries) + checksumLength;
	}
	...
	void writeToInner(OutputStream os) throws IOException {
		// FIXME if we use readAllBlocks() we'll need to run on the memory limited
		// queue???
	...
	public void onNonFatalFailure(int blockNumber) {
		...
		synchronized (this) {
			...
			if (blockChooser.onNonFatalFailure(blockNumber)) {
				...
				if (logMINOR)
					Logger.minor(this, "Block " + blockNumber + " on " + this + " : "
							+ blockChooser.getRetries(blockNumber) + "/" + blockChooser.maxRetries);
		...
		if (kill) {
			if (crossSegmentsByBlock == null) {
				...
				// Similar to DSOnly... finishedEncoding will fail eventually when all
				// segments
	...
	/**
	 * The metadata has been updated. We should write it ... at some point. CALLER MUST
	 * SET metadataDirty!
	 */
	 ...
	 * @param random PRNG seeded from the splitfile metadata, which determines which
	 * blocks to allocate in a deterministic manner.
	 * @return The block number allocated (between dataBlocks and
	 * dataBlocks+crossSegmentCheckBlocks).
	 */
	...
	/**
	 * Write minimal fixed metadata for the segment. This should include lengths rather
	 * than offsets. Does not write cross-segment block assignments; these are handled by
	 * the cross-segments.
	 * @throws IOException
	 */
	...
	public int chooseRandomKey() {
		...
		synchronized (this) {
			...
			if (corruptMetadata)
				return -1; // Will be fetchable after we've found out what blocks we
							// actually have.
	...
	public void cancel() {
		...
		synchronized (this) {
			...
			finished = true;
			decoding = tryDecode;
			// If already decoding, must wait for decoder to check in before completing
			// shutdown.
		}
		if (!decoding)
			parent.finishedEncoding(this);
	...
	public synchronized byte[] checkAndGetBlockData(int blockNum) throws IOException {
		...
		for (int i = 0; i < blocksFetched.length; i++) {
			if (blocksFetched[i] == blockNum) {
				byte[] buf = readBlock(i);
				try {
					ClientCHKBlock block = ClientCHKBlock.encodeSplitfileBlock(buf, key.getCryptoKey(),
							key.getCryptoAlgorithm());
