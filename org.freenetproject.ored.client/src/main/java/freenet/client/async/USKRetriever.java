...
public class USKRetriever extends BaseClientGetter implements USKCallback {
	...
	// In wierd
	/**
	 * The USKCallback that is actually subscribed. This is used when we may be going
	 * through a USKSparseProxyCallback.
	 */
	private USKCallback proxy;

	/**
	 * Alternatively, we may be driving a USKFetcher directly. This happens when the
	 * client subscribes with a custom FetchContext.
	 */
	...
	public USKRetriever(FetchContext fctx, short prio, final RequestClient client, USKRetrieverCallback cb,
			USK origUSK) {
		super(prio, client);
	...
	@Override
	public void onFoundEdition(long l, USK key, ClientContext context, boolean metadata, short codec, byte[] data,
		...
		if (l < origUSK.suggestedEdition) {
			Logger.warning(this,
					"Found edition prior to that specified by the client: " + l + " < " + origUSK.suggestedEdition);
			return;
		}
		if (logMINOR)
		...
		// Fetch it. If it fails, ignore it, if it succeeds, return the data with the
		// edition # to the client.
		FreenetURI uri = key.getSSK(l).getURI();
		try {
			SingleFileFetcher getter = (SingleFileFetcher) SingleFileFetcher.create(this, this, uri, ctx,
					new ArchiveContext(ctx.maxTempLength, ctx.maxArchiveLevels), ctx.maxNonSplitfileRetries, 0, true, l,
					true, false, context, realTimeFlag, false);
	...
	public void onSuccess(StreamGenerator streamGenerator, ClientMetadata clientMetadata,
		...
		try {
			...
			if (decompressors != null) {
				...
				ClientGetWorkerThread worker = new ClientGetWorkerThread(new BufferedInputStream(pipeIn), output, null,
						null, ctx.getSchemeHostAndPort(), null, false, null, null, null,
	...
	/**
	 * Get the original USK URI which was passed when creating the retriever - not the
	 * latest known URI!
	 */
	...
	public FreenetURI getURI() {
		// FIXME: Toad: Why did getURI() return null? Does it break anything that I made
		// it return the URI?
	...
	/**
	 * Called when we subscribe() in USKManager, if we don't directly subscribe the
	 * USKRetriever. Usually this happens when we put a proxy between them, e.g.
	 * USKProxyCompletionCallback, which hides updates for efficiency.
	 ...
	 * USKManager.subscribeContentCustom(). FIXME this is a special case hack, For a
	 * generic solution see https://bugs.freenetproject.org/view.php?id=4984
	 * @param time The new cooldown time. At least 30 minutes or we throw.
	 * @param tries The new number of tries after each cooldown. Greater than 0 and less
	 * than 3 or we throw.
	 */
