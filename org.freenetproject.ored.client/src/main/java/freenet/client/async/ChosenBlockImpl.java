...
public class ChosenBlockImpl extends ChosenBlock {
	...
	public ChosenBlockImpl(SendableRequest req, SendableRequestItem token, Key key, ClientKey ckey,
		...
		if (logDEBUG)
			Logger.minor(this, "Created " + this + " for " + (persistent ? "persistent" : "transient") + " block "
					+ token + " for key " + key, new Exception("debug"));
	}
		...
		context.getJobRunner(persistent).queueNormalOrDrop(new PersistentJob() {
			...
			public boolean run(ClientContext context) {
				try {
					...
					sched.removeRunningInsert(request, token.getKey());
					// Something might be waiting for a request to complete (e.g. if we
					// have two requests for the same key),
	...
	public void onInsertSuccess(final ClientKey key, ClientContext context) {
		context.getJobRunner(persistent).queueNormalOrDrop(new PersistentJob() {
			...
			public boolean run(ClientContext context) {
				...
				// Something might be waiting for a request to complete (e.g. if we have
	...
	public void onFetchSuccess(ClientContext context) {
		context.getJobRunner(persistent).queueNormalOrDrop(new PersistentJob() {
			...
			public boolean run(ClientContext context) {
				try {
					sched.succeeded((BaseSendableGet) request, false);
				}
				finally {
					sched.removeFetchingKey(key);
				}
