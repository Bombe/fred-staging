...
public class ClientPutter extends BaseClientPutter implements PutCompletionCallback {
	...
	/**
	 * Target filename. If specified, we create manifest metadata so that the file can be
	 * accessed at [ final key ] / [ target filename ].
	 */
	 ...
	 * @param data The data to insert. This will be freed when the insert has completed,
	 * whether it succeeds or not, so wrap it in a @link freenet.support.io.NoFreeBucket
	 * if you don't want it to be freed.
	 * @param targetURI
	 * @param cm
	 * @param ctx
	 * @param priorityClass
	 * @param isMetadata
	 * @param targetFilename If set, create a one-file manifest containing this filename
	 ...
	 */
	public ClientPutter(ClientPutCallback client, RandomAccessBucket data, FreenetURI targetURI, ClientMetadata cm,
			InsertContext ctx, short priorityClass, boolean isMetadata, String targetFilename, boolean binaryBlob,
			ClientContext context, byte[] overrideSplitfileCrypto, long metadataThreshold) {
	...
	public boolean start(boolean restart, ClientContext context) throws InsertException {
		...
		try {
			...
			// If the top level key is an SSK, all CHK blocks and particularly splitfiles
			// below it should have
			// randomised keys. This substantially improves security by making it
			// impossible to identify blocks
			// even if you know the content. In the user interface, we will offer the
			// option of inserting as a
			...
			synchronized (this) {
				if (restart) {
					...
					if (currentState != null && !finished) {
						if (logMINOR)
							Logger.minor(this,
									"Can't restart, not finished and currentState != null : " + currentState);
						return false;
					}
					if (finished)
				...
				if (currentState != null) {
					if (logMINOR)
						Logger.minor(this, "Can't " + (restart ? "restart" : "start") + " : currentState != null : "
								+ currentState);
					return false;
				}
				...
				if (!cancel) {
					if (!binaryBlob) {
						...
						currentState = new SingleFileInserter(this, this, new InsertBlock(data, meta, targetURI),
								isMetadata, ctx, realTimeFlag, false, false, null, null, false, targetFilename, false,
								persistent(), 0, 0, null, cryptoAlgorithm, cryptoKey, metadataThreshold);
					}
					else
						currentState = new BinaryBlobInserter(data, this, getClient(), false, priorityClass, ctx,
								context);
				}
			}
			if (cancel) {
	...
	public static boolean randomiseSplitfileKeys(FreenetURI targetURI, InsertContext ctx, boolean persistent) {
		// If the top level key is an SSK, all CHK blocks and particularly splitfiles
		// below it should have
		// randomised keys. This substantially improves security by making it impossible
		// to identify blocks
		// even if you know the content. In the user interface, we will offer the option
		// of inserting as a
		// random SSK to take advantage of this.
	...
	public void onSuccess(ClientPutState state, ClientContext context) {
		...
		if (super.failedBlocks > 0 || super.fatallyFailedBlocks > 0 || super.successfulBlocks < super.totalBlocks) {
			...
			// If only generating the key, splitfile may not have reported the blocks as
			// inserted.
			if (!uri.isUSK() && !ctx.getCHKOnly)
				Logger.error(this,
						"Failed blocks: " + failedBlocks + ", Fatally failed blocks: " + fatallyFailedBlocks
								+ ", Successful blocks: " + successfulBlocks + ", Total blocks: " + totalBlocks
								+ " but success?! on " + this + " from " + state,
						new Exception("debug"));
		}
		...
		synchronized (this) {
			...
			if (this.uri != null) {
				if (!this.uri.equals(u)) {
					Logger.error(this,
							"onEncode() called twice with different URIs: " + this.uri + " -> " + u + " for " + this,
							new Exception("error"));
				}
				return;
			}
	...
	/**
	 * Called when metadataThreshold was specified and metadata is being returned instead
	 * of a URI.
	 */
	@Override
	...
	/**
	 * Cancel the insert. Will call onFailure() if it is not already cancelled, so the
	 * callback will normally be called.
	 */
	@Override
	...
	/**
	 * Called when a ClientPutState transitions to a new state. If this is the current
	 * state, then we update it, but it might also be a subsidiary state, in which case we
	 * ignore it.
	 */
	@Override
	...
	/**
	 * Called when we have generated metadata for the insert. This should not happen,
	 * because we should insert the metadata!
	 */
	@Override
	public void onMetadata(Metadata m, ClientPutState state, ClientContext context) {
		Logger.error(this,
				"Got metadata on " + this + " from " + state + " (this means the metadata won't be inserted)");
	}
	...
	/**
	 * Called (sometimes) when enough of the data has been inserted that the file can now
	 * be fetched. Not very useful unless earlyEncode was enabled.
	 */
	@Override
