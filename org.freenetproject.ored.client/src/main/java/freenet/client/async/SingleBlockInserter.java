...
public class SingleBlockInserter extends SendableInsert implements ClientPutState, Serializable {
	...
	static class MySendableRequestSender implements SendableRequestSender {
		...
		@Override
		public boolean send(NodeClientRequest core, RequestScheduler sched, final ClientContext context, final ChosenBlock req) {
			...
			try {
				...
				if(req.localRequestOnly)
					try {
						...
						KeyBlock collided = core.getKeyBlockStore().fetch(k.getNodeKey(), true, req.canWriteClientCache, false, false, null);
						if(collided == null) {
					...
					core.realPut(b, req.canWriteClientCache, req.forkOnCacheable, NodeConstants.PREFER_INSERT_DEFAULT, NodeConstants.IGNORE_LOW_BACKOFF_DEFAULT, req.realTimeFlag);
