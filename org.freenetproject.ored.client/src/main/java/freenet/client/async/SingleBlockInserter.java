...
public class SingleBlockInserter extends SendableInsert implements ClientPutState, Serializable {
	...
	final FreenetURI uri; // uses essentially no RAM in the common case of a CHK because
							// we use FreenetURI.EMPTY_CHK_URI
	 ...
	 * @param cryptoKey
	 */
	public SingleBlockInserter(BaseClientPutter parent, Bucket data, short compressionCodec, FreenetURI uri,
			InsertContext ctx, boolean realTimeFlag, PutCompletionCallback cb, boolean isMetadata, int sourceLength,
			int token, boolean addToParent, boolean dontSendEncoded, Object tokenObject, ClientContext context,
			boolean persistent, boolean freeData, int extraInserts, byte cryptoAlgorithm, byte[] cryptoKey) {
	...
	protected ClientKeyBlock innerEncode(RandomSource random) throws InsertException {
		try {
			return innerEncode(random, uri, sourceData, isMetadata, compressionCodec, sourceLength,
					ctx.compressorDescriptor, cryptoAlgorithm, cryptoKey);
		}
	...
	protected static ClientKeyBlock innerEncode(RandomSource random, FreenetURI uri, Bucket sourceData,
			boolean isMetadata, short compressionCodec, int sourceLength, String compressorDescriptor,
			byte cryptoAlgorithm, byte[] cryptoKey) throws InsertException, CHKEncodeException, IOException,
			SSKEncodeException, MalformedURLException, InvalidCompressionCodecException {
		...
		if (uriType.equals("CHK")) {
			return ClientCHKBlock.encode(sourceData, isMetadata, compressionCodec == -1, compressionCodec, sourceLength,
					compressorDescriptor, cryptoKey, cryptoAlgorithm);
		}
	...
	public void onFailure(LowLevelException e, SendableRequestItem keyNum, ClientContext context) {
		...
		LowLevelPutException pe = (LowLevelPutException) e;
		switch (pe.code) {
		...
		case LowLevelPutException.ROUTE_REALLY_NOT_FOUND:
			...
			break;
		default:
			Logger.error(this, "Unknown LowLevelPutException code: " + pe.code);
			errors.inc(InsertExceptionMode.INTERNAL_ERROR);
		}
		if (pe.code == LowLevelPutException.ROUTE_NOT_FOUND || pe.code == LowLevelPutException.ROUTE_REALLY_NOT_FOUND) {
			consecutiveRNFs++;
			if (logMINOR)
	...
	public void onSuccess(SendableRequestItem keyNum, ClientKey key, ClientContext context) {
		...
		synchronized (this) {
			if (extraInserts > 0 && !ctx.getCHKOnly) {
				if (++completedInserts <= extraInserts) {
					if (logMINOR)
						Logger.minor(this, "Completed inserts " + completedInserts + " of extra inserts " + extraInserts
								+ " on " + this);
					return; // Let it repeat until we've done enough inserts. It hasn't
		...
		if (shouldSendKey)
			cb.onEncode(key, this, context); // In case of race conditions etc, especially
												// for LocalRequestOnly.
	...
	static class MySendableRequestSender implements SendableRequestSender {
		...
		@Override
		public boolean send(NodeClientRequest core, RequestScheduler sched, final ClientContext context,
			...
			try {
				try {
					encodedBlock = innerEncode(context.random, block.uri, block.copyBucket, block.isMetadata,
							block.compressionCodec, block.sourceLength, compressorDescriptor, block.cryptoAlgorithm,
							block.cryptoKey);
					b = encodedBlock.getBlock();
				}
				catch (CHKEncodeException | SSKEncodeException | InsertException | IOException
				...
				if (req.localRequestOnly)
					try {
						core.getKeyBlockStore().store(b, false, req.canWriteClientCache, true, false);
					}
					catch (KeyCollisionException e) {
						LowLevelPutException failed = new LowLevelPutException(LowLevelPutException.COLLISION);
						KeyBlock collided = core.getKeyBlockStore().fetch(k.getNodeKey(), true, req.canWriteClientCache,
								false, false, null);
						if (collided == null) {
					...
					core.realPut(b, req.canWriteClientCache, req.forkOnCacheable, NodeConstants.PREFER_INSERT_DEFAULT,
							NodeConstants.IGNORE_LOW_BACKOFF_DEFAULT, req.realTimeFlag);
			}
				...
				if (e.code == LowLevelPutException.COLLISION) {
					// Collision
					try {
						ClientSSKBlock collided = ClientSSKBlock.construct(((SSKBlock) e.getCollidedBlock()),
								(ClientSSK) k);
						...
						if (collided.isMetadata() == block.isMetadata
							...
							if (SingleBlockInserter.logMINOR)
								Logger.minor(this, "Apparently real collision: collided.isMetadata="
										+ collided.isMetadata() + " block.isMetadata=" + block.isMetadata
										+ " collided.codec=" + collided.getCompressionCodec() + " block.codec="
										+ block.compressionCodec + " collided.datalength=" + data.length
										+ " block.datalength=" + inserting.length + " H(collided)="
	...
	public void tryEncode(ClientContext context) {
		...
		try {
			...
			// Not necessary to do so (we'll ask again when we need it), and it'll
	...
	private BlockItem getBlockItem(BlockItemKey key, ClientContext context) throws InsertException {
		try {
			...
			return new BlockItem(key, data, isMetadata, compressionCodec, sourceLength, u, persistent, cryptoAlgorithm,
					cryptoKey);
		}
		catch (IOException e) {
			throw new InsertException(InsertExceptionMode.BUCKET_ERROR, e, null);
	...
	/**
	 * Everything needed to actually run a request, without access to the
	 * SingleBlockInserter (this is why we copy the Bucket).
	 */
	private static class BlockItem implements SendableRequestItem {
		...
		BlockItem(BlockItemKey key, Bucket bucket, boolean meta, short codec, int srclen, FreenetURI u,
				boolean persistent, byte cryptoAlgorithm, byte[] cryptoKey) {
			this.key = key;
