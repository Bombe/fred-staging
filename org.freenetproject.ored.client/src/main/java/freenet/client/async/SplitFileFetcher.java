...
/**
 * Splitfile fetcher based on keeping as much state as possible, and in particular the
 * downloaded blocks, in a single file.
 *
 * The main goals here are: 1) Minimising disk seeks. E.g. in the older versions we
 * abstracted out block storage, this caused a lot of unnecessary seeking and copying.
 * It's better to keep the downloaded data close together on disk. 2) Robustness. This
 * should be robust even against moderate levels of on-disk data corruption. And it's
 * separate for each splitfile. And it has fixed disk usage, until it completes. Some of
 * this robustness violates layering e.g. checking blocks against the CHKs they are
 * supposed to represent. This actually simplifies matters e.g. when decoding a segment,
 * and allows us to not only recover from almost any error (at least in the parts that
 * change, which are most likely to be corrupted), but also to do so efficiently.
 *
 * The SplitFileFetcher*Storage classes manage storing the data and FEC decoding. This
 * class deals with everything else: The interface to the rest of the client layer,
 * selection of keys to fetch, listening for blocks, etc.
 *
 * PERSISTENCE ROADMAP: Now: Currently this class does not support persistence.
 *
 * Near future goal: Support persistence within the database. Stay in memory, do not
 * deactivate. Store this class in the database but not its *Storage classes. Load the
 * SplitFileFetcherStorage on loading Freenet, resuming existing downloads. => Significant
 * improvement in reliability and disk I/O. In principle we could resume downloads
 * separately from the database e.g. if it breaks. Too complicated in practice because of
 * other data structures. - Require: Add "persistence", hashCode, force no deactivation,
 * activate where necessary (when dealing with other stuff). Fill in context and load
 * storage when activated on startup.
 *
 * Longer term goal: Implement similar code for inserts. Eliminate db4o and persist
 * everything that remains with simple checkpointed serialisation.
 *
 * LOCKING: (this) should be taken last, because it is used by e.g.
...
public class SplitFileFetcher implements ClientGetState, SplitFileFetcherStorageCallback, Serializable {
	...
	/**
	 * If non-null, this is the temporary file we have allocated for completion via
	 * truncation. The download will be stored in this file until it is complete, at which
	 * point the storage will truncate it and we will feed it to the callback.
	 */
	...
	/**
	 * Does the actual requests. Created in onResume() or in the constructor, so must be
	 * volatile.
	 */
	...
	public SplitFileFetcher(Metadata metadata, GetCompletionCallback rcb, ClientRequester parent,
			...
			ClientMetadata clientMetadata, long token, boolean topDontCompress, short topCompatibilityMode,
			boolean persistent, FreenetURI thisKey, boolean isFinalFetch, ClientContext context)
		...
		try {
			...
			if (isFinalFetch && cb instanceof FileGetCompletionCallback
					&& (decompressors == null || decompressors.size() == 0) && !fetchContext.filterData) {
				...
				if (targetFile != null) {
					...
					fileCompleteViaTruncation = File.createTempFile(targetFile.getName(), ".freenet-tmp",
			...
			storage = new SplitFileFetcherStorage(metadata, this, decompressors, clientMetadata, topDontCompress,
					topCompatibilityMode, fetchContext, realTimeFlag, getSalter(), thisKey, parent.getURI(),
					isFinalFetch, parent.getClientDetail(checker), context.random, context.tempBucketFactory,
					persistent ? context.persistentRAFFactory : context.tempRAFFactory,
					persistent ? context.jobRunner : context.dummyJobRunner, context.ticker,
					context.memoryLimitedJobRunner, checker, persistent, fileCompleteViaTruncation,
		...
		raf = storage.getRAF();
		if (logMINOR)
			Logger.minor(this, "Created " + (persistent ? "persistent" : "transient") + " download for " + thisKey
					+ " on " + raf + " for " + this);
	...
	/**
	 * Fail the whole splitfile request when we get an IOException on writing to or
	 * reading from the on-disk storage. Can be called asynchronously by
	 * SplitFileFetcher*Storage if an off-thread job (e.g. FEC decoding) breaks, or may be
	 * called when SplitFileFetcher*Storage throws.
	 ...
	 * can't read the keys. FIXME ROBUSTNESS in some cases this could actually be
	 * recovered by restarting from the metadata or the original URI.
	 */
	@Override
	 ...
	 * GetCompletionCallback. See bug #6063 for a better way that probably is too much
	...
	public void onSuccess() {
		...
		if (this.callbackCompleteViaTruncation != null) {
			...
			this.callbackCompleteViaTruncation.onSuccess(fileCompleteViaTruncation, finalLength, storage.clientMetadata,
					this, context);
			// Don't need to call storage.finishedFetcher().
		}
		else {
			cb.onSuccess(storage.streamGenerator(), storage.clientMetadata, storage.decompressors, this, context);
			storage.finishedFetcher();
		}
	}
	...
	@Override
	public void onSplitfileCompatibilityMode(CompatibilityMode min, CompatibilityMode max, byte[] customSplitfileKey,
			boolean compressed, boolean bottomLayer, boolean definitiveAnyway) {
		cb.onSplitfileCompatibilityMode(min, max, customSplitfileKey, compressed, bottomLayer, definitiveAnyway,
				context);
	}
	...
	public void queueHeal(byte[] data, byte[] cryptoKey, byte cryptoAlgorithm) {
		try {
			context.healingQueue.queue(BucketTools.makeImmutableBucket(context.tempBucketFactory, data), cryptoKey,
					cryptoAlgorithm, context);
		}
		catch (IOException e) {
	...
	public void onResume(ClientContext context) throws FetchException {
		...
		try {
			...
			this.storage = new SplitFileFetcherStorage(raf, realTimeFlag, this, blockFetchContext, context.random,
					context.jobRunner, context.getChkFetchScheduler(realTimeFlag).fetchingKeys(), context.ticker,
					context.memoryLimitedJobRunner, new CRCChecksumChecker(), context.jobRunner.newSalt(), salter,
					resumed, callbackCompleteViaTruncation != null);
		}
