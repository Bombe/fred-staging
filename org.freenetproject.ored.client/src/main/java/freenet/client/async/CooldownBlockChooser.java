...
public class CooldownBlockChooser extends SimpleBlockChooser {
	...
	/**
	 * Time at which the whole block chooser will next become fetchable. 0 to mean it is
	 * fetchable now. Equal to the earliest valid cooldown time for any individual block.
	 * INVARIANT: This can safely be too early (small) but not too late (large).
	 */
	...
	protected synchronized int innerOnNonFatalFailure(int blockNo) {
		...
		if (ret % cooldownTries == 0) {
			...
			overallCooldownTime = Math.min(blockCooldownTimes[blockNo], overallCooldownTime); // Must
																								// not
																								// be
																								// left
																								// at
