...
public class SplitFileFetcherKeyListener implements KeyListener {
	...
	/**
	 * Salt used in the secondary Bloom filters if the primary matches. The primary Bloom
	 * filters use the already-salted saltedKey.
	 */
	...
	/**
	 * Default mainBloomElementsPerKey. False positives is approx 0.6185^[this number], so
	 * 19 gives us 0.01% false positives, which should be acceptable even if there are
	 * thousands of splitfiles on the queue.
	 */
	...
	/**
	 * What proportion of false positives is acceptable for the per-segment Bloom filters?
	 * This is divided by the number of segments, so it is (roughly) an overall
	 * probability of any false positive given that we reach the per-segment filters. IMHO
	 * 1 in 100 is adequate.
	 */
	static final double ACCEPTABLE_BLOOM_FALSE_POSITIVES_ALL_SEGMENTS = 0.01;

	/**
	 * Size of per-segment bloom filter in bytes. This is calculated from the above
	 * constant and the number of segments, and rounded up.
	 */
	...
	/**
	 * The overall bloom filter, containing all the keys, salted with the global hash.
	 * When a key is found, it is removed from this.
	 */
	private final CountingBloomFilter filter;

	/**
	 * The per-segment bloom filters, containing the keys for each segment. These are not
	 * changed.
	 */
	...
	public SplitFileFetcherKeyListener(SplitFileFetcherStorageCallback fetcher, SplitFileFetcherStorage storage,
			boolean persistent, byte[] localSalt, int origSize, int segBlocks, int segments) throws FetchException {
		if (origSize <= 0) {
		...
		if (elementsLong > Integer.MAX_VALUE)
			throw new FetchException(FetchExceptionMode.TOO_BIG, "Cannot fetch splitfiles with more than "
	...
	public SplitFileFetcherKeyListener(SplitFileFetcherStorage storage, SplitFileFetcherStorageCallback callback,
			DataInputStream dis, boolean persistent, boolean newSalt) throws IOException, StorageFormatException {
		this.storage = storage;
	...
	/**
	 * The segment bloom filters should only need to be written ONCE, and can all be
	 * written at once. Include a checksum.
	 */
