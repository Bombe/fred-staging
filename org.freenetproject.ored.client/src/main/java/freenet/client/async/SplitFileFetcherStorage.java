...
/**
 * <p>
 * Stores the state for a SplitFileFetcher, persisted to a LockableRandomAccessBuffer
 * (i.e. a single random access file), but with most of the metadata in memory. The data,
 * and the larger metadata such as the full keys, are read from disk when needed, and
 * persisted to disk.
 * </p>
 *
 * <p>
 ...
 * </p>
 * <ol>
 * <li>Maximise robustness.</li>
 * <li>Minimise seeks.</li>
 * <li>Minimise disk usage.</li>
 * <li>Be as simple as realistically possible.</li>
 * </ol>
 *
 * <p>
 * Overall on-disk structure: BLOCK STORAGE: Decoded data, one segment at a time (the last
 * segment's size is rounded up to a whole block). Within each segment, the number of
 * blocks is equal to the number of data blocks (plus the number of cross-check blocks if
 * there are cross-check blocks), but they are not necessarily actually data blocks (they
 * may be check blocks), and they may not be in the correct order. When we FEC decode, we
 * read in the blocks, construct the CHKs to see what keys they belong to, check that we
 * still have enough valid keys (update the metadata if the counts were wrong), do the
 * decode, and write the data blocks back in the correct order; the segment is finished.
 * When all the segments are finished, we generate a stream as usual, i.e. we still need
 * to copy the file. It may be possible in future to simply truncate the file but in many
 * cases we need to decompress or filter, and there are significant issues with code
 * complexity and seeks during FEC decodes, see bug #6063.
 *
 * KEY LIST: The original key list. Not changed when a block is fetched. - Fixed and
 * checksummed (each segment has a checksum).
 *
 * SEGMENT STATUS: The status of each segment, including the status of each block,
 * including flags and where it is in the block storage within the segment. - Checksummed
 * per segment. So it needs to be written as a whole segment. Can be regenerated from the
 * block store and key list, which happens routinely when FEC decoding.
 *
 ...
 *
 * BASIC SETTINGS: Type of splitfile, length of file, overall decryption key, number of
 * blocks and check blocks per segment, etc. - Fixed and checksummed. Read as a block so
 * we can check the checksum.
 *
 * FOOTER: Length of basic settings. (So we can seek back to get them) Version number.
 ...
 *
 * CONCURRENCY: Callbacks into fetcher should be run off-thread, as they will usually be
 * inside a MemoryLimitedJob.
 *
 * LOCKING: Trivial or taken last. Hence can be called inside e.g. RGA calls to
 * getCooldownTime etc.
 *
...
public class SplitFileFetcherStorage {
	...
	/**
	 * If true we will complete the download by truncating the file. The file was passed
	 * in at construction and we are not responsible for freeing it. Once all segments
	 * have decoded and encoded we call onSuccess(), and we don't free the data. Also, if
	 * this is true, cross-check blocks will be kept on disk *AFTER* all the main data and
	 * check blocks for the whole file.
	 */
	...
	/**
	 * Final length of the downloaded data. *BEFORE* decompression, filtering, etc. I.e.
	 * this is the length of the data on disk, which will be written by the
	 * StreamGenerator.
	 */
	final long finalLength;

	/**
	 * Final length of the downloaded data, after decompression. (May change if the data
	 * is filtered).
	 */
	...
	/**
	 * False = Transient: We are using the RAF as scratch space, we only need to write the
	 * blocks, and the keys (if we don't keep them in memory). True = Persistent: It must
	 * be possible to resume after a node restart. Ideally we'd like to be able to recover
	 * the download in its entirety without needing any additional information, but at a
	 * minimum we want to be able to continue it while passing in the usual external
	 * arguments (FetchContext, parent, etc).
	 */
	...
	/**
	 * Every cooldownTries attempts, a key will enter cooldown, and won't be re-tried for
	 * a period.
	 */
	...
	/**
	 * Offset to start of the original details in bytes. "Original details" includes the
	 * URI to this download (if available), the original URI for the whole download (if
	 * available), whether this is the final fetch (it might be a metadata or container
	 * fetch), and data from the ultimate client, e.g. the Identifier, whether it is on
	 * the Global queue, the client name if it isn't etc.
	 */
	...
	/**
	 * List of segments we need to tryStartDecode() on because their metadata was
	 * corrupted on startup.
	 */
	private List<SplitFileFetcherSegmentStorage> segmentsToTryDecode;

	/**
	 * Construct a new SplitFileFetcherStorage from metadata. Creates the
	 * RandomAccessBuffer and writes the initial data to it. There is another constructor
	 ...
	 * @param checker
	 * @param persistent
	 * @param storageFile If non-null, we will use this file to store the data in. It must
	 * already exist, and must be 0 bytes long. We will use it, and then when complete,
	 * truncate the file so it only contains the final data before calling onSuccess().
	 * Also, in this case, rafFactory must be a
	 * DiskSpaceCheckingRandomAccessBufferFactory.
	 * @param diskSpaceCheckingRAFFactory
	 * @param keysFetching Must be passed in at this point as we will need it later.
	 * However, none of this is persisted directly, so this is not a problem.
	 * @throws FetchException If we failed to set up the download due to a problem with
	 * the metadata.
	...
	public SplitFileFetcherStorage(Metadata metadata, SplitFileFetcherStorageCallback fetcher,
			List<COMPRESSOR_TYPE> decompressors, ClientMetadata clientMetadata, boolean topDontCompress,
			short topCompatibilityMode, FetchContext origFetchContext, boolean realTime, KeySalter salt,
			FreenetURI thisKey, FreenetURI origKey, boolean isFinalFetch, byte[] clientDetails, RandomSource random,
			BucketFactory tempBucketFactory, LockableRandomAccessBufferFactory rafFactory, PersistentJobRunner exec,
			...
			KeysFetchingLocally keysFetching) throws FetchException, MetadataParseException, IOException {
		...
		if (decompressors.size() > 1) {
			Logger.error(this, "Multiple decompressors: " + decompressors.size() + " - this is almost certainly a bug",
					new Exception("debug"));
		}
		this.clientMetadata = clientMetadata == null ? new ClientMetadata() : clientMetadata.clone(); // copy
																										// it
																										// as
																										// in
																										// SingleFileFetcher
		...
		/**
		 * Only non-zero if the cross-check blocks are stored separately i.e. if
		...
		for (SplitFileSegmentKeys keys : segmentKeys) {
			...
			storedKeysLength += SplitFileFetcherSegmentStorage.storedKeysLength(dataBlocks, checkBlocks,
					splitfileSingleCryptoKey != null, checksumLength);
			storedSegmentStatusLength += SplitFileFetcherSegmentStorage.paddedStoredSegmentStatusLength(
					dataBlocks - crossCheckBlocks, checkBlocks, crossCheckBlocks, maxRetries != -1, checksumLength,
					persistent);
		}
		...
		if (splitfileType == SplitfileAlgorithm.NONREDUNDANT) {
			...
			if (topCompatibilityMode != 0) {
				// If we have top compatibility mode, then we can give a definitive answer
				// immediately, with the splitfile key, with dontcompress, etc etc.
			...
			// If the top-block stats are passed in then we can safely say the report is
			// definitive.
			...
			if ((blocksPerSegment > origFetchContext.maxDataBlocksPerSegment)
				...
				throw new FetchException(FetchExceptionMode.TOO_MANY_BLOCKS_PER_SEGMENT, "Too many blocks per segment: "
						+ blocksPerSegment + " data, " + checkBlocksPerSegment + " check");
		...
		if (logMINOR)
			Logger.minor(this,
					"Algorithm: " + splitfileType + ", blocks per segment: " + blocksPerSegment
		...
		segments = new SplitFileFetcherSegmentStorage[segmentCount]; // initially null on
																		// all entries
		...
		keyListener = new SplitFileFetcherKeyListener(fetcher, this, false, localSalt,
				splitfileDataBlocks + totalCrossCheckBlocks + splitfileCheckBlocks,
				blocksPerSegment + checkBlocksPerSegment, segmentCount);
		...
		if (persistent) {
			...
			this.offsetOriginalMetadata = offsetSegmentBloomFilters + keyListener.totalSegmentBloomFiltersSize();
		}
		else {
		...
		long dataOffset = 0;
		long crossCheckBlocksOffset = storedBlocksLength; // Only used if
															// completeViaTruncation
		...
		for (int i = 0; i < segments.length; i++) {
			...
			if ((dataBlocks > origFetchContext.maxDataBlocksPerSegment)
				...
				throw new FetchException(FetchExceptionMode.TOO_MANY_BLOCKS_PER_SEGMENT, "Too many blocks per segment: "
						+ blocksPerSegment + " data, " + checkBlocksPerSegment + " check");
			segments[i] = new SplitFileFetcherSegmentStorage(this, i, splitfileType, dataBlocks, checkBlocks,
					crossCheckBlocks, dataOffset, completeViaTruncation ? crossCheckBlocksOffset : -1, // Put
																										// at
																										// end
																										// if
																										// truncating.
					segmentKeysOffset, segmentStatusOffset, maxRetries != -1, keys, keysFetching);
			...
			segmentKeysOffset += SplitFileFetcherSegmentStorage.storedKeysLength(dataBlocks + crossCheckBlocks,
					checkBlocks, splitfileSingleCryptoKey != null, checksumLength);
			segmentStatusOffset += SplitFileFetcherSegmentStorage.paddedStoredSegmentStatusLength(dataBlocks,
					checkBlocks, crossCheckBlocks, maxRetries != -1, checksumLength, persistent);
			...
			if (logDEBUG)
				Logger.debug(this,
						"Segment " + i + ": data blocks offset " + segments[i].segmentBlockDataOffset
								+ " cross-check blocks offset " + segments[i].segmentCrossCheckBlockDataOffset
								+ " for segment " + i + " of " + this);
		}
		...
		/*
		 * Lie about the required number of blocks. For a cross-segment splitfile, the
		 * actual number of blocks needed is somewhere between splitfileDataBlocks and
		 * splitfileDataBlocks + totalCrossCheckBlocks depending on what order we fetch
		 * them in. Progress over 100% is apparently more annoying than finishing at
		 * 98%...
		 */
		...
		if (crossCheckBlocks != 0) {
			Random crossSegmentRandom = new MersenneTwister(
			...
			for (int i = 0; i < crossSegments.length; i++) {
				...
				SplitFileFetcherCrossSegmentStorage seg = new SplitFileFetcherCrossSegmentStorage(i, segLen,
						crossCheckBlocks, this, fecCodec);
				crossSegments[i] = seg;
		...
		if (persistent) {
			...
			encodedBasicSettings = encodeBasicSettings(splitfileDataBlocks, splitfileCheckBlocks,
					crossCheckBlocks * segments.length);
			totalLength = offsetBasicSettings + // rest of file
		...
		try {
			...
			if (persistent) {
				...
				// Pointers.
				raf.pwrite(offsetBasicSettings + encodedBasicSettings.length, bufToWrite, 0, bufToWrite.length);
				// Checksum.
				raf.pwrite(offsetBasicSettings + encodedBasicSettings.length + bufToWrite.length, checksum, 0,
						checksum.length);
				// Version.
				raf.pwrite(offsetBasicSettings + encodedBasicSettings.length + bufToWrite.length + checksum.length,
	 ...
	 * appropriate local settings passed in. Ideally this would work with only basic
	 * system utilities such as those on ClientContext, i.e. we'd be able to restore the
	 * splitfile download without knowing anything about it.
	 * @param newSalt True if the global salt has changed.
	 * @param salt The global salter. Should be passed in even if the global salt hasn't
	 * changed, as we may not have completed regenerating bloom filters.
	 * @throws IOException If the restore failed because of a failure to read from disk.
	 * @throws StorageFormatException
	 * @throws FetchException If the request has already failed (but it wasn't processed
	 * before restarting).
	 */
	public SplitFileFetcherStorage(LockableRandomAccessBuffer raf, boolean realTime,
			SplitFileFetcherStorageCallback callback, FetchContext origContext, RandomSource random,
			PersistentJobRunner exec, KeysFetchingLocally keysFetching, Ticker ticker,
			MemoryLimitedJobRunner memoryLimitedJobRunner, ChecksumChecker checker, boolean newSalt, KeySalter salt,
			boolean resumed, boolean completeViaTruncation) throws IOException, StorageFormatException, FetchException {
		...
		// FIXME this is hideous! Rewrite the writing/parsing code here in a less ugly
		// way. However, it works...
		rafLength = raf.size();
		...
		// 4 bytes basic settings length and a checksum, which includes both the settings
		// length and the version.
		buf = new byte[14];
		...
		try {
			...
			for (int i = 0; i < segments.length; i++) {
				segments[i] = new SplitFileFetcherSegmentStorage(this, dis, i, maxRetries != -1, dataOffset,
						completeViaTruncation ? crossCheckBlocksOffset : -1, segmentKeysOffset, segmentStatusOffset,
						keysFetching);
				...
				segmentKeysOffset += SplitFileFetcherSegmentStorage.storedKeysLength(dataBlocks + crossCheckBlocks,
						checkBlocks, splitfileSingleCryptoKey != null, checksumLength);
				segmentStatusOffset += SplitFileFetcherSegmentStorage.paddedStoredSegmentStatusLength(dataBlocks,
						checkBlocks, crossCheckBlocks, maxRetries != -1, checksumLength, true);
			...
			if (countCrossCheckBlocks != totalCrossCheckBlocks)
				throw new StorageFormatException(
						"Total cross-check blocks " + countCrossCheckBlocks + " but expected " + totalCrossCheckBlocks);
	 ...
	 * @param resume True only if we are restarting without having serialized, i.e. from
	 * the file only. In this case we will need to tell the parent how many blocks have
	 * been fetched.
	 * @return True if it should be scheduled immediately. If false, the storage layer
	 * will callback into the fetcher later.
	 */
	...
	/**
	 * Write details needed to restart the download from scratch, and to identify whether
	 * it is useful to do so.
	 */
	private byte[] encodeAndChecksumOriginalDetails(FreenetURI thisKey, FreenetURI origKey, byte[] clientDetails,
			boolean isFinalFetch) throws IOException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		DataOutputStream dos = new DataOutputStream(baos);
	...
	public void lazyWriteMetadata() {
		...
		if (LAZY_WRITE_METADATA_DELAY != 0) {
			...
			ticker.queueTimedJob(wrapLazyWriteMetadata, "Write metadata for splitfile", LAZY_WRITE_METADATA_DELAY,
					false, true);
		}
	...
	private void closeOffThread() {
		jobRunner.queueNormalOrDrop(new PersistentJob() {
			...
			public boolean run(ClientContext context) {
				...
				// completion.
				// So after restart, even if the checkpoint failed, we will be in a valid
				// state.
	 ...
	 * this is not called on a MemoryLimitedJob thread.
	 */
	void close() {
		if (logMINOR)
	...
	/**
	 * Called when a segment has finished encoding. It is possible that it has simply
	 * restarted; it is not guaranteed to have encoded all blocks etc. But we still need
	 * the callback in case e.g. we are in the process of failing, and can't proceed until
	 * all the encode jobs have finished.
	 */
	...
	/**
	 * Called when a cross-segment has finished decoding. It doesn't necessarily have a
	 * "finished" state, except if it was cancelled.
	 */
	...
	private boolean allFinished() {
		// First, are any of the segments still working, that is, are they able to send
		// requests,
	...
	/**
	 * Fail the request, off-thread. The callback will call cancel etc, so it won't
	 * immediately shut down the storage.
	 * @param e
	 */
	...
	/**
	 * A segment ran out of retries. We have given up on that segment and therefore on the
	 * whole splitfile.
	 ...
	 * persistent data; it's okay to update caches and other stuff that isn't stored to
	 * disk. If we fail etc we should do it off-thread.
	 *
	 * FIXME make SplitFileFetcherGet per-segment, eliminate all this unnecessary
	 * complexity!
	 * @return The block number to be fetched, as an integer.
	 */
	public MyKey chooseRandomKey() {
		// FIXME this should probably use SimpleBlockChooser and hence use
		// lowest-retry-count from each segment?
		synchronized (this) {
			if (finishedFetcher)
				return null;
		}
		// Generally segments are fairly well balanced, so we can usually pick a random
		// segment
		// then a random key from it.
		// FIXME OPT SCALABILITY A simpler option might be just to have one
		// SplitFileFetcherGet per
	...
	/**
	 * Local only is true and we've finished checking the datastore. If all segments are
	 * not already finished/decoding, we need to fail with DNF. If the segments fail to
	 * decode due to data corruption, we will retry as usual.
	 */
	...
	/**
	 * Separate lock for cooldown operations, which must be serialized. Must be taken
	 * *BEFORE* segment locks.
	 */
	...
	public long getCooldownWakeupTime(long now) {
		...
		// It is safe to use both here (on the request selection thread), one after the
		// other.
		if (hasFinished())
			return -1;
		synchronized (cooldownLock) {
	 ...
	 * close(), it checks that the length is as expected, computes the checksum, and
	 * writes the data to the specified position in the file.
	...
	byte[] readBlock(SplitFileFetcherSegmentStorage segment, int slotNumber) throws IOException {
		...
		if (logDEBUG)
			Logger.minor(this, "Reading block " + slotNumber + " for " + segment.segNo + "/" + segments.length
					+ " from " + offset + " RAF length is " + raf.size());
