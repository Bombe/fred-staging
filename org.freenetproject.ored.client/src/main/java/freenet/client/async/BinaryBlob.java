...
public abstract class BinaryBlob {
	...
	static void writeBlobHeader(DataOutputStream binaryBlobStream, short type, short version, int length)
			throws IOException {
		...
		while (true) {
			...
			if (blobType == BinaryBlob.BLOB_END) {
				...
				if (blobVer != BinaryBlob.BLOB_BLOCK_VERSION)
					// Even if tolerant, if we can't read a blob there probably isn't much
					// we can do.
