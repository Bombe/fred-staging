...
public class DatastoreChecker implements PrioRunnable {
	...
	private static class QueueItem {
		...
		/**
		 * Request which we will call finishRegister() for when we have checked the keys
		 * lists. Deactivated (if persistent).
		 */
	...
	private final KeyBlockStore keyBlockStore;
	...
	public DatastoreChecker(KeyBlockStore keyBlockStore, boolean lazyStart, Executor executor, String threadName) {
		this.keyBlockStore = keyBlockStore;
		this.lazy = lazyStart;
		this.executor = executor;
		this.threadName = threadName;
		int priorities = PriorityClasses.NUMBER_OF_PRIORITY_CLASSES;
	...
	public void queueRequest(SendableGet getter, BlockSet blocks) {
		...
		if (logMINOR)
			Logger.minor(this,
					"Queueing transient request " + getter + " priority " + prio + " keys " + checkKeys.length);
		...
		synchronized (this) {
			...
			QueueItem queueItem = new QueueItem(finalKeysToCheck.toArray(new Key[finalKeysToCheck.size()]), getter,
					blocks);
	...
	private boolean realRun() {
			...
			while (true) {
				for (short prio = 0; prio < queue.length; prio++) {
					...
					if ((trans = queue[prio].pollFirst()) != null) {
						...
						if (logMINOR)
							Logger.minor(this, "Checking transient request " + getter + " prio " + prio + " of "
									+ queue[prio].size());
						break;
					}
				}
		...
		for (Key key : keys) {
			...
			if (blocks != null)
				...
				block = keyBlockStore.fetch(key, true, true, false, false, null);
			if (block != null) {
				if (logMINOR)
		...
		if (getter.persistent()) {
			...
			try {
				context.jobRunner.queue(new PersistentJob() {
					...
					public boolean run(ClientContext context) {
						try {
							scheduler.finishRegister(new BaseSendableGet[] { get }, true, valid);
						}
						catch (Throwable t) {
							Logger.error(this, "Failed to register " + get + ": " + t, t);
							try {
								get.onFailure(new LowLevelGetException(LowLevelGetException.INTERNAL_ERROR,
										"Internal error: " + t, t), null, context);
							}
							catch (Throwable t1) {
			...
			sched.finishRegister(new BaseSendableGet[] { getter }, false, anyValid);
		}
		return false;
	}
