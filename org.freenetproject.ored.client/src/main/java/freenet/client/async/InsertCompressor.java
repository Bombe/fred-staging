...
import freenet.compress.CompressJob;
...
/**
 * Compress a file in order to insert it. This class acts as a tag in the database to
 * ensure that inserts are not forgotten about, and also can be run on a non-database
 * thread from an executor.
 *
 * @author toad
 */
public class InsertCompressor implements CompressJob {
	...
	/**
	 * The SingleFileInserter we report to. We were created by it and when we have
	 * compressed our data we will call a method to process it and schedule the data.
	 */
	...
	public void tryCompress(final ClientContext context) throws InsertException {
		...
		try {
			...
			boolean first = true;
			long amountOfDataToCheckCompressionRatio = config.get("node")
					.getLong("amountOfDataToCheckCompressionRatio");
			...
			for (final COMPRESSOR_TYPE comp : comps) {
				...
				try {
					...
					try {
						...
						try {
							comp.compress(is, os, origSize, bestCompressedDataSize, amountOfDataToCheckCompressionRatio,
					...
					if (resultNumberOfBlocks < bestNumberOfBlocks) {
						if (logMINOR)
							Logger.minor(this,
									"New size " + resultSize + " (" + resultNumberOfBlocks
											+ " blocks) better than old best " + bestCompressedDataSize + " ("
											+ bestNumberOfBlocks + " blocks)");
						if (bestCompressedData != null && bestCompressedData != origData)
							bestCompressedData.free();
						bestCompressedData = result;
						bestCompressedDataSize = resultSize;
				...
				// if one iteration of compression took a lot of time, then we will not
			...
			if (persistent) {
				...
				context.jobRunner.queue(new PersistentJob() {
					...
					// This can wait until after the next checkpoint, because it's still
					// in the
					// persistentInsertCompressors list, so will be restarted if
					// necessary.
					@Override
					public boolean run(ClientContext context) {
	...
	public static InsertCompressor start(ClientContext ctx, SingleFileInserter inserter, RandomAccessBucket origData,
			...
			final Config config) {
		InsertCompressor compressor = new InsertCompressor(inserter, origData, minSize, bf, persistent, generateHashes,
				pre1254, config);
		compressor.init(ctx);
		return compressor;
	}
