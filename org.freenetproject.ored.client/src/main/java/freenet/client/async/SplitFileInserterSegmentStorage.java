...
public class SplitFileInserterSegmentStorage {
	...
	// These are also in parent but we need them here for easy access, especially as we
	// don't want to
	...
	public SplitFileInserterSegmentStorage(SplitFileInserterStorage parent, int segNo, boolean persistent,
			int dataBlocks, int checkBlocks, int crossCheckBlocks, int keyLength, byte splitfileCryptoAlgorithm,
			byte[] splitfileCryptoKey, Random random, int maxRetries, int consecutiveRNFsCountAsSuccess,
		...
		blockChooser = new SplitFileInserterSegmentBlockChooser(this, totalBlockCount, random, maxRetries, keysFetching,
	 ...
	 */
	public SplitFileInserterSegmentStorage(SplitFileInserterStorage parent, DataInputStream dis, int segNo,
			int keyLength, byte splitfileCryptoAlgorithm, byte[] splitfileCryptoKey, Random random, int maxRetries,
	...
	/**
	 * Allocate a cross-segment data block. Note that this algorithm must be reproduced
	 * exactly for splitfile compatibility; the Random seed is actually determined by the
	 * splitfile metadata.
	 * @param seg The cross-segment to allocate a block for.
	 * @param random PRNG seeded from the splitfile metadata, which determines which
	 * blocks to allocate in a deterministic manner.
	...
	/**
	 * Allocate a cross-segment check block. **Note that this algorithm must be reproduced
	 * exactly for splitfile compatibility**; the Random seed is actually determined by
	 * the splitfile metadata.
	 * @param seg The cross-segment to allocate a block for.
	 * @param random PRNG seeded from the splitfile metadata, which determines which
	 * blocks to allocate in a deterministic manner.
	...
	private static byte[] encodeKey(int segNo, int blockNumber, ClientCHK key, boolean hasSplitfileKey,
			ChecksumChecker checker, SplitFileInserterStorage parent) {
	 ...
	 * @param blockNumber The block number. Can be a data block, cross segment check block
	 * or check block, in that numerical order.
	...
	/**
	 * Set a flag indicating that we have a key. Call parent.onHasKeys if we have all of
	 * them. Note that this structure is not persisted!
	 */
	...
	/**
	 * Called on startup to check which keys we actually have. Does nothing unless the
	 * segment claims to have been encoded already. FIXME consider calling this later on
	 * for robustness, but we would then need to re-encode ...
	 */
	...
	public synchronized void startEncode(final short prio) {
		...
		if (logMINOR)
			Logger.minor(this, "Scheduling encode on " + this + " at priority " + prio + " blocks " + totalBlockCount
					+ " memory limit " + limit);
		parent.memoryLimitedJobRunner.queueJob(new MemoryLimitedJob(limit) {
			...
			public boolean start(MemoryLimitedChunk chunk) {
				...
				try {
					...
					try {
						if (!shutdown) {
							// We do want to call the callback even if we threw something,
							// because we
							// may be waiting to cancel. However we DON'T call it if we
							// are shutting down.
						...
						// Callback is part of the persistent job, unlock *after* calling
						// it.
						if (lock != null)
	...
	private byte[] readCrossCheckBlock(int blockNo) throws IOException {
		return crossSegmentBlockSegments[blockNo].readCheckBlock(crossSegmentBlockNumbers[blockNo], segNo,
				blockNo + dataBlockCount);
	}
	...
	/**
	 * For unit tests. Generally for concurrency purposes we want something that won't
	 * change back, hence e.g. isFinishedEncoding().
	 */
	...
	ClientCHKBlock encodeBlock(byte[] buf) {
		...
		try {
			block = ClientCHKBlock.encodeSplitfileBlock(buf, splitfileCryptoKey, splitfileCryptoAlgorithm);
		}
	...
	private ClientCHK innerReadKey(DataInputStream dis) throws IOException {
		if (splitfileCryptoKey != null) {
			...
			return new ClientCHK(routingKey, splitfileCryptoKey, false, splitfileCryptoAlgorithm, (short) -1);
		}
		else {
	...
	/**
	 * Has the segment completed all inserts? Should not change once we reach this state,
	 * but might be possible in case of disk errors causing losing keys etc.
	 */
	...
	public void onFailure(int blockNo, InsertException e) {
		...
		if (e.isFatal()) {
			...
			if (e.mode == InsertExceptionMode.ROUTE_NOT_FOUND && blockChooser.consecutiveRNFsCountAsSuccess > 0) {
				try {
					readKey(blockNo);
	 ...
	 * segments.
	 * @return True if the segment has completed cancellation. False if it is waiting for
	 * an encode, in which case a callback to parent will be made when the encode
	 * finishes.
	 */
	...
	/**
	 * Set the cross-segment associated with a cross-check block, which tells us how to
	 ...
	 * @param crossSegmentBlockNumber The cross-check block number (usually between 0 and
	 * 2 inclusive).
	 */
	void setCrossCheckBlock(SplitFileInserterCrossSegmentStorage crossSegment, int segmentBlockNumber,
