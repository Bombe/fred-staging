...
public class SplitFileInserterSegmentBlockChooser extends SimpleBlockChooser {
	...
	public SplitFileInserterSegmentBlockChooser(SplitFileInserterSegmentStorage segment, int blocks, Random random,
			int maxRetries, KeysFetchingLocally keysFetching, int consecutiveRNFsCountAsSuccess) {
	...
	protected int getMaxBlockNumber() {
		// Ignore cross-segment: We either send all blocks, if the segment has been
		// encoded, or
		// only the data blocks, if it hasn't (even if the cross-segment blocks have been
		// encoded).
	...
	/**
	 * Handle an RNF if the n-consecutive-RNFs-count-as-success hack is enabled. Must only
	 * be called if consecutiveRNFsCountAsSuccess > 0
	 */
	...
	/**
	 * Count the previous RNFs towards the retry limit, when the following error wasn't an
	 * RNF.
	 */
