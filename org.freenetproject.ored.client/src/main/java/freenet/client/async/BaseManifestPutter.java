...
/**
 * <P>
 * Base class for site insertion. This class contains all the insert logic, but not any
 * 'pack logic'. The pack logic have to be implement in a subclass in makePutHandlers.
 * <P>
 * Internal container redirect URIs: The internal container URIs should be always
 * redirects to CHKs, not just include the metadata into manifest only. The (assumed)
 * default behavior is the reuse of containers between editions, also ArchiveManger want
 * to have a URI given, not Metadata. This rule also makes site update code/logic much
 * more easier.
 * <P>
 * <DL>
 * <DT>container mode:
...
public abstract class BaseManifestPutter extends ManifestPutter {
	 ...
	 *
	 * Archives are not part of the site structure, they are used to group files that not
	 * fit into a container (for example a directory with brazilion files it) Archives are
	 * always inserted as CHK, references to items in it are normal redirects to
	...
	private final class ArchivePutHandler extends PutHandler {
		...
		private ArchivePutHandler(BaseManifestPutter bmp, PutHandler parent, String name, HashMap<String, Object> data,
				FreenetURI insertURI) {
			super(bmp, parent, name, null, containerPutHandlers);
			this.origSFI = new ContainerInserter(this, this, data, insertURI, ctx, false, false, null, ARCHIVE_TYPE.TAR,
					false, forceCryptoKey, cryptoAlgorithm, realTimeFlag);
		}
	 ...
	 *
	 * Containers are an integral part of the site structure, they are inserted as CHK,
	 * the root container is inserted at targetURI. references to items in it are
	...
	private final class ExternPutHandler extends PutHandler {
		...
		private ExternPutHandler(BaseManifestPutter bmp, PutHandler parent, String name, RandomAccessBucket data,
				ClientMetadata cm2) {
			super(bmp, parent, name, cm2, runningPutHandlers);
			InsertBlock block = new InsertBlock(data, cm, FreenetURI.EMPTY_CHK_URI);
			this.origSFI = new SingleFileInserter(this, this, block, false, ctx, realTimeFlag, false, true, null, null,
					false, null, false, persistent(), 0, 0, null, cryptoAlgorithm, forceCryptoKey, -1);
		}
		...
		public void onMetadata(Metadata m, ClientPutState state, ClientContext context) {
			...
			if (logMINOR)
				Logger.minor(this, "Assigning metadata: " + m + " for '" + this.itemName + "' " + this + " from "
	...
	private final class MetaPutHandler extends PutHandler {
		...
		// Metadata is not put with a cryptokey. It is derived from other stuff that is
		// already encrypted with random keys.
		...
		private MetaPutHandler(BaseManifestPutter smp, PutHandler parent, InsertBlock insertBlock) {
			...
			this.origSFI = new SingleFileInserter(this, this, insertBlock, true, ctx, realTimeFlag, false, false, null,
					null, true, null, true, persistent(), 0, 0, null, cryptoAlgorithm, null, -1);
			if (logMINOR)
				Logger.minor(this, "Inserting root metadata: " + origSFI);
		}

		// resolver
		private MetaPutHandler(BaseManifestPutter smp, PutHandler parent, Metadata toResolve, BucketFactory bf)
			...
			this.origSFI = new SingleFileInserter(this, this, ib, true, ctx, realTimeFlag, false, false, toResolve,
					null, true, null, true, persistent(), 0, 0, null, cryptoAlgorithm, null, -1);
			if (logMINOR)
		...
		public void onSuccess(ClientPutState state, ClientContext context) {
			...
			synchronized (BaseManifestPutter.this) {
				if (rootMetaPutHandler == this) {
					// if (containerPutHandlers.contains(this)) throw new
					// IllegalStateException("was in containerPutHandlers");
	...
	private final class JokerPutHandler extends PutHandler {
		...
		public JokerPutHandler(BaseManifestPutter bmp, PutHandler parent, String name, ClientMetadata cm2) {
			...
			// we dont know the final uri, so preconstructing the metadata does not help
			// here Metadata m = new Metadata(Metadata.SIMPLE_REDIRECT, null, null,
			// FreenetURI.EMPTY_CHK_URI, cm2);
		}
	...
	abstract class PutHandler extends BaseClientPutter implements PutCompletionCallback {
		...
		private PutHandler(final BaseManifestPutter bmp, PutHandler parent, String name, String nameInArchive,
				Metadata md, ClientMetadata cm) {
			super(bmp.priorityClass, bmp.cb.getRequestClient());
			this.persistent = bmp.persistent();
			this.cm = cm;
			this.itemName = name;
		...
		public void onTransition(ClientPutState oldState, ClientPutState newState, ClientContext context) {
			...
			// onTransition is *not* responsible for removing the old state, the caller
			// is.
			synchronized (this) {
				if (currentState == oldState) {
					currentState = newState;
					if (logMINOR)
						Logger.minor(this, "onTransition: cur=" + currentState + ", old=" + oldState + ", new="
								+ newState + " for " + this);
					return;
				}
		...
		/**
		 * The number of blocks that will be needed to fetch the data. We put this in the
		 * top block metadata.
		 */
	...
	private final static String[] defaultDefaultNames = new String[] { "index.html", "index.htm", "default.html",
			"default.htm" };
	...
	/**
	 * PutHandler: the *PutHandler HashMap<String, Object>: the 'metadata dir' that
	 * contains the item inserted by PutHandler the *PutHandler fills in its result here
	 * (Metadata)
	 */
	...
	private boolean hasResolvedBase; // if this is true, the final block is ready for
										// insert
	...
	public BaseManifestPutter(ClientPutCallback cb, HashMap<String, Object> manifestElements, short prioClass,
			FreenetURI target, String defaultName, InsertContext ctx, boolean randomiseCryptoKeys,
	 ...
	 * @param manifestElements A map from String to either ManifestElement or another
	 * String. This is the site structure, which will be split into containers and/or
	 * external inserts by the method.
	...
	/**
	 * Called when we have metadata for all the PutHandler's. This does *not* necessarily
	 * mean we can immediately insert the final metadata, since if these metadata's are
	 * too big, they will need to be inserted separately. See resolveAndStartBase().
	 * @param context
	 */
	...
	/**
	 * Attempt to insert the base metadata and the container. If the base metadata cannot
	 * be resolved, try to resolve it: start inserts for each part that cannot be
	 * resolved, and wait for them to generate URIs that can be incorporated into the
	 * metadata. This method will then be called again, and will complete, or do more
		...
		while (true) {
			try {
				...
				try {
					...
					// Eventually it will generate a URI and call onEncode(), which will
					// call back here.
					if (logMINOR)
	...
	/**
	 * Start inserts for unresolved (too big) Metadata's. Eventually these will call back
	 * with an onEncode(), meaning they have the CHK, and we can progress to
	 * resolveAndStartBase().
	 * @param e
	 * @param context
	 * @return
	...
	private void complete(ClientContext context) {
		// FIXME we could remove the put handlers after inserting all files but not having
		// finished the insert of the manifest
	...
	/**
	 * The number of blocks that will be needed to fetch the data. We put this in the top
	 * block metadata.
	 */
	protected int minSuccessFetchBlocks;
	...
	/**
	 * Add one or more blocks to the number of requires blocks, and don't notify the
	 * clients.
	 */
	@Override
	...
	/**
	 * Add one or more blocks to the number of requires blocks, and don't notify the
	 * clients. These blocks are added to the minSuccessFetchBlocks for the insert, but
	 * not to the counter for what the requestor must fetch.
	 */
	@Override
	...
	public void innerNotifyClients(ClientContext context) {
		...
		synchronized (this) {
			e = new SplitfileProgressEvent(this.totalBlocks, this.successfulBlocks, this.latestSuccess,
					this.failedBlocks, this.fatallyFailedBlocks, this.latestFailure, this.minSuccessBlocks,
					this.minSuccessFetchBlocks, this.blockSetFinalized);
		}
	...
	private void tryStartParentContainer(PutHandler containerHandle2, ClientContext context) throws InsertException {
		...
		if (perContainerPutHandlersWaitingForMetadata.get(containerHandle2).isEmpty()) {
			...
			// F:"+perContainerPutHandlersWaitingForFetchable.get(containerHandle2).size()
			// + " for "+containerHandle2);
			if (logMINOR)
				Logger.minor(this,
						"(spc) waiting m:" + perContainerPutHandlersWaitingForMetadata.get(containerHandle2).size()
								+ " for " + containerHandle2);
		}
	}
	...
	protected final ClientMetadata guessMime(String name, ManifestElementNew me) {
	...
	protected abstract class ManifestBuilder implements Serializable {
		...
		/**
		 * Map from name to either a Metadata (to be included as-is), a ManifestElement
		 * (either a redirect or a file), or another HashMap. Eventually processed by e.g.
		 ...
		 * @param isDefaultDoc
		 */
		public final void addElement(String name, ManifestElementNew element, boolean isDefaultDoc) {
		...
		/**
		 * Add a file as an external. It will be inserted separately and we will add a
		 * redirect to the metadata.
		 * @param name The name of the file (short name within the original folder, it's
		 * not in a container).
	...
	protected final class FreeFormBuilder extends ManifestBuilder {
		...
		public void addExternal(String name, RandomAccessBucket data, ClientMetadata cm, boolean isDefaultDoc) {
			...
			if (logMINOR)
				Logger.minor(this,
						"Inserting separately as PutHandler: " + name + " : " + ph + " persistent=" + ph.persistent());
			numberOfFiles++;
			totalSize += data.size();
	...
	protected final class ContainerBuilder extends ManifestBuilder {
		...
		/**
		 * Tree containing the status of the insert. Can have ManifestElement's (original
		 * files to insert or bundle inside a container), HashMap's (more subdirs),
		 * Metadata (to be put into a container as metadata for e.g. an external file), a
		 * ContainerPutHandler or an ArchivePutHandler (for containers that are part of
		 * the structure, and external containers for overflow, respectively).
		 */
		...
		private ContainerBuilder(boolean isRoot, PutHandler parent, String name, boolean isArchive) {
			...
			if (isArchive)
				selfHandle = new ArchivePutHandler(BaseManifestPutter.this, parent, name, _rootDir,
						(isRoot ? BaseManifestPutter.this.targetURI : FreenetURI.EMPTY_CHK_URI));
			else
				selfHandle = new ContainerPutHandler(BaseManifestPutter.this, parent, name, _rootDir,
		 ...
		 * testing/index.html).
		 * @param element The ManifestElement specifying the data, redirect, etc. Note
		 * that redirects are still included in containers, both for structural reasons
		 * and because the metadata can be large enough that we need to split it.
		 * @param isDefaultDoc If true, add a link from "" to this element, making it the
		 * default document in this container.
		 */
		public void addItem(String name, String nameInArchive, ManifestElementNew element, boolean isDefaultDoc) {
			ManifestElementNew me = new ManifestElementNew(element, name, nameInArchive);
			addItem(name, me, isDefaultDoc);
		}

		public void addItem(String name, ManifestElementNew element, boolean isDefaultDoc) {
		...
		/**
		 * FIXME what is going on here? Why do we need to add a JokerPutHandler, when a
		 * lot of code just calls addItem()?
		 */
		public void addArchiveItem(ContainerBuilder archive, String name, ManifestElementNew element,
				boolean isDefaultDoc) {
			assert (element.getData() != null);
			archive.addItem(name, new ManifestElementNew(element, name, name), false);
			PutHandler ph = new JokerPutHandler(BaseManifestPutter.this, selfHandle, name,
					guessMime(name, element.mimeOverride));
			putHandlersTransformMap.put(ph, currentDir);
			perContainerPutHandlersWaitingForMetadata.get(selfHandle).add(ph);
	...
	public static HashMap<String, Object> bucketsByNameToManifestEntries(HashMap<String, Object> bucketsByName) {
		...
		for (Map.Entry<String, Object> entry : bucketsByName.entrySet()) {
			...
			if (o instanceof ManifestElementNew) {
				...
				manifestEntries.put(name, new ManifestElementNew(name, data, null, data.size()));
			}
	...
	public static ManifestElementNew[] flatten(HashMap<String, Object> manifestElements) {
		List<ManifestElementNew> v = new ArrayList<ManifestElementNew>();
		flatten(manifestElements, v, "");
		return v.toArray(new ManifestElementNew[v.size()]);
	}

	public static void flatten(HashMap<String, Object> manifestElements, List<ManifestElementNew> v, String prefix) {
		for (Map.Entry<String, Object> entry : manifestElements.entrySet()) {
			String name = entry.getKey();
			...
			if (o instanceof HashMap) {
				...
				v.add(new ManifestElementNew(me, fullName));
			}
			else
