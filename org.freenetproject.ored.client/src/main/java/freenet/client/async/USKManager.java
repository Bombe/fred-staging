...
public class USKManager {
	...
	public void hintUpdate(USK usk, long edition, ClientContext context) {
		...
		final ClientGetter get = new ClientGetter(new NullClientCallback(rcBulk), uri, new FetchContext(backgroundFetchContext, FetchContext.IDENTICAL_MASK), PriorityClasses.UPDATE_PRIORITY_CLASS, new NullBucket(), null, null);
		try {
			get.start(context);
