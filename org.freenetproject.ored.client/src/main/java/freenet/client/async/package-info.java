 ...
 * an insert of a file etc. E.g. @see ClientGetter</li>
 * <li>@see ClientGetState The current stage in a request. E.g. fetch a block and follow
 * the metadata and redirects if necessary (@see SingleFileFetcher), or download a
 * splitfile (@see SplitFileFetcher).</li>
 * <li>@see ClientPutState The current stage in an insert. E.g. insert a single block
 * (@see SingleBlockInserter), or a splitfile (@see SplitFileInserter).</li>
 ...
 * ClientRequestSchedulerNonPersistent for transient requests.</li>
 * <li>The cooldown queue: @see CooldownTracker. This is used to ensure that we don't keep
 * on selecting the same request repeatedly, while choosing requests efficiently.
 ...
 * </p>
 * <ul>
 * <li>@see freenet.client.HighLevelSimpleClient (a lightweight API used both internally
 * and by some plugins)</li>
 * <li>@see freenet.clients.fcp The interface to external FCP clients</li>
 * <li>Some code and plugins use the classes here directly.</li>
 * </ul>
 * <p>
 * Code which the client layer uses:
 * </p>
 * <ul>
 * <li>@see freenet.client Support classes for MIME type handling, container handling,
 * etc</li>
 * <li>@see freenet.node.SendableRequest The actual implementation of sending
 * requests</li>
 ...
 * <li>@see freenet.client.filter Content filtering code. Content filtering is implemented
 * in the client layer for various reasons.</li>
 * </ul>
 */
