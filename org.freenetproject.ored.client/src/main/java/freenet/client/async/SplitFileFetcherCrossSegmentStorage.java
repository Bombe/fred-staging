...
/**
 * Cross-segments are "in parallel" with the main segments, an interlaced Reed-Solomon
 * scheme similar to that used on CD's, allowing us to fill in blocks from other segments.
 * There are 3 "cross-check blocks" in each segment, and therefore 3 in each
 * cross-segment; the rest are data blocks.
 */
public class SplitFileFetcherCrossSegmentStorage {
	...
	/**
	 * Whether each block in the cross-segment has been found. Kept up to date when blocks
	 * are found in the other segments. However, as in a normal segment, these may not be
	 * 100% accurate!
	 */
	...
	/**
	 * If true, the segment has completed. Once a segment decode starts, finished must not
	 * be set until it exits.
	 */
	...
	public void onFetchedRelevantBlock(SplitFileFetcherSegmentStorage segment, int blockNo) {
		...
		synchronized (this) {
			...
			if (totalFound < dataBlockCount) {
				if (logMINOR)
					Logger.minor(this, "Not decoding " + this + " : found " + totalFound + " blocks of "
							+ dataBlockCount + " (total " + segments.length + ")");
				return;
			}
	...
	/**
	 * Attempt FEC decoding. Check blocks before decoding in case there is disk
	 * corruption. Check the new decoded blocks afterwards to ensure reproducible
	 * behaviour.
	 */
	...
	private void reportBlockToSegmentOffThread(final int blockNo, final ClientCHK key, final ClientCHKBlock block,
			final byte[] data) {
		parent.jobRunner.queueNormalOrDrop(new PersistentJob() {
			...
			public boolean run(ClientContext context) {
				try {
					// FIXME CPU USAGE Add another API to the segment to avoid
					// re-decoding.
	...
	/**
	 * Read all blocks from all segments, checking the contents of the block against each
	 * key.
	 * @param If false, read data blocks, if true, read check blocks. (The FEC code takes
	 * separate arrays).
	 * @return An array of blocks, in the correct order. Each element is either a valid
	 * block or null if the block is invalid or hasn't been fetched yet. Will tell the
	 * ordinary segment if the block is bogus. Will also update our blocksFound.
	 */
	...
	public SplitFileFetcherCrossSegmentStorage(SplitFileFetcherStorage parent, int segNo, DataInputStream dis)
	 ...
	 * construction. But we must have read metadata on the regular segments first, which
	 * won't be true in the constructor.
	 */
