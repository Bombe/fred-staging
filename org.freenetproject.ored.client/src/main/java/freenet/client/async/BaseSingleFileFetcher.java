...
public abstract class BaseSingleFileFetcher extends SendableGet implements HasKeyListener {
	...
	protected boolean retry(ClientContext context) {
		if (isEmpty()) {
			...
			return false; // Cannot retry e.g. because we got the block and it failed to
							// decode - that's a fatal error.
		}
		// We want 0, 1, ... maxRetries i.e. maxRetries+1 attempts (maxRetries=0 => try
		// once, no retries, maxRetries=1 = original try + 1 retry)
		int r;
		r = ++retryCount;
		if (logMINOR)
		...
		if ((r <= maxRetries) || (maxRetries == -1)) {
			...
			if (cachedCooldownTries == 0 || r % cachedCooldownTries == 0) {
				...
				if (cooldownWakeupTime > now) {
					Logger.error(this,
							"Already on the cooldown queue for " + this + " until "
					...
					if (logMINOR)
						Logger.minor(this, "Added single file fetcher into cooldown until "
	...
	private void checkCachedCooldownData() {
		// 0/0 is illegal, and it's also the default, so use it to indicate we haven't
		// fetched them.
	...
	/**
	 * Remove the pendingKeys item and then remove from the queue as well. Call
	 * unregister(container) if you only want to remove from the queue.
	 */
	...
	public void onGotKey(Key key, KeyBlock block, ClientContext context) {
		...
		unregister(context, getPriorityClass()); // Key has already been removed from
													// pendingKeys
	...
	public KeyListener makeKeyListener(ClientContext context, boolean onStartup) {
		...
		if (parent == null) {
			Logger.error(this,
					"Parent is null on " + this + " persistent=" + persistent + " key=" + key + " ctx=" + ctx);
			return null;
		}
	...
	public synchronized long getWakeupTime(ClientContext context, long now) {
		...
		if (wakeTime <= 0 && fetching.hasKey(getNodeKey(null), this)) {
			...
			// tracker.cooldownWakeupTime is only set for a real cooldown period, NOT when
			// we go into hierarchical cooldown because the request is already running.
		}
		if (wakeTime == 0)
			return 0;
		return wakeTime;
	...
	/**
	 * Reread the cached cooldown values (and anything else) from the FetchContext after
	 * it changes. FIXME: Ideally this should be a generic mechanism, but that looks too
	 * complex without significant changes to data structures. For now it's just a hack to
	 * make changing the polling interval in USKs work.
