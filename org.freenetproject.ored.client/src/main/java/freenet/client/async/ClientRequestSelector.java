...
/**
 * The global request queue. Both transient and persistent requests are kept on this
 * in-RAM structure, which supports choosing a request to run. See KeyListenerTracker for
 * the code that matches up a fetched block with whoever was waiting for it, which needs
 * to be separate for various reasons. This class is not persistent.
 *
 * COOLDOWN TRACKER AND WAKEUP TIMES: Each node in the tree (priority, client, request)
 * keeps a "wakeup time". This indicates that until the time given there is no point
 * checking the requests below that node. This happens either because all the keys are
 * being fetched (in which case the wakeup time is Long.MAX_VALUE) or because a key has
 * been fetched repeatedly and has entered a cooldown period, meaning it will be fetchable
 * in 30 minutes.
 *
 * LOCKING: Consequently we need to lock the entire tree whenever we access either the
 * tree or the cooldown tracker (which really should be part of the tree, TODO!): When a
 * request completes, we start at the request itself and go up the tree until we stop
 * updating the wakeup times. However when we choose a request to send, we start at the
 * top and go down (and update the cooldown times when backtracking back up the tree if we
 * don't find anything).
 *
 * **We lock on ClientRequestSelector** when using the tree, including the cooldown times.
 *
 * REDFLAG LOCKING: Actually in the completion case we could find the top and then lock
 * the whole tree, and then update the cooldowns; and/or we could avoid updating the
 * cooldowns during request selection, e.g. by making sure that each structure always does
 * a bottom-up update when something changes, although that would not help with locking...
 *
 * FIXME: More seriously, we should really combine the cooldown tracker and the RGAs. The
 * RGAs and SRGAs should contain their own wakeup times. This could significantly simplify
 * the code. CooldownTracker is left over from the DB4O era.
 */
public class ClientRequestSelector implements KeysFetchingLocally {
	...
	// Layer 1: SRGA of RGAs (for a RequestClient), descend by
	...
	ClientRequestSelector(boolean isInsertScheduler, boolean isSSKScheduler, boolean isRTScheduler,
		...
		priorities = new RequestClientRGANode[PriorityClasses.NUMBER_OF_PRIORITY_CLASSES];
	}
	 ...
	 * complications with HTL, and also has the benefit that we can see stuff that's been
	 * scheduled on a SenderThread but that thread hasn't started yet. FIXME: Both issues
	 * can be avoided: first we'd get rid of the SenderThread and start the requests
	 * directly and asynchronously, secondly we'd move this to node but only track keys we
	 * are fetching at max HTL. LOCKING: Always lock this LAST.
	 */
	 ...
	 * @return The priority chosen or the time at which a priority will have requests to
	 * send. LOCKING: Synchronized because we may create new priorities. Both the cooldown
	 * queue and the RGA hierarchy, rooted at the priorities, use ClientRequestSelector
	 * lock.
	 */
	private synchronized long choosePriority(int fuzz, RandomSource random, ClientContext context, long now) {
		...
		while (iteration++ < PriorityClasses.NUMBER_OF_PRIORITY_CLASSES + 1) {
			...
			if (priority > PriorityClasses.MINIMUM_FETCHABLE_PRIORITY_CLASS) {
				fuzz++;
				continue; // Don't return because first round may be higher with soft
							// scheduling
			}
	 ...
	 * chooseRequestInner() may not be runnable (it may return null if the request is
	 * already running), so we may need to try repeatedly. FIXME this is only necessary
	 * because many classes only update their cooldown status when choosing a block to
	 * send, e.g. SplitFileInserter.
	 */
	ChosenBlock chooseRequest(int fuzz, RandomSource random, OfferedKeysList offeredKeys,
			RandomGrabArrayItemExclusionList starter, boolean realTime, ClientContext context) {
	...
	public ChosenBlock maybeMakeChosenRequest(SendableRequest req, ClientContext context, long now) {
		...
		if (req.getWakeupTime(context, now) != 0) {
			// Race condition. We don't need to add a wake-up job. FIXME this shouldn't
			// happen
			// because we only consider local requests of the same type?! Add logging and
			// debug!
			if (logMINOR)
		...
		if (token == null) {
			...
			if (req instanceof SendableGet) {
				...
				if (req instanceof SendableInsert) {
					...
					forkOnCacheable = NodeConstants.FORK_ON_CACHEABLE_DEFAULT;
					localRequestOnly = false;
					realTimeFlag = false;
				}
				ignoreStore = false;
	...
	/**
	 * Choose a request to run. Does not check whether the SendableRequest is actually
	 * runnable at the moment. The cooldown mechanism on the RGAs and SRGAs should ensure
	 * that it is usable most of the time.
	 * @return Either a chosen request or the time at which we should try again if all
	 * priorities are waiting for requests to finish / cooldown periods to expire.
	 */
	SelectorReturn chooseRequestInner(int fuzz, RandomSource random, OfferedKeysList offeredKeys,
			RandomGrabArrayItemExclusionList starter, boolean realTime, ClientContext context, long now) {
		...
		long wakeupTime = Long.MAX_VALUE;
		outer: for (; choosenPriorityClass <= PriorityClasses.MINIMUM_FETCHABLE_PRIORITY_CLASS; choosenPriorityClass++) {
			if (logMINOR)
			...
			while (true) {
				...
				synchronized (this) {
					...
					// This is important for keeping the cooldown tracker consistent
					// amongst other
					// things: We can get a race condition between thread A reading the
					// tree,
					// finding nothing and setCachedWakeup(), and thread B waking up a
					// request,
				...
				if (req.getPriorityClass() != choosenPriorityClass) {
					...
					if (clientGrabber != null) {
						...
						if (baseRGA != null) {
							// Must synchronize to avoid nasty race conditions with
							// cooldown.
							synchronized (this) {
				...
				/**
				 * Choose a recently succeeded request. 50% chance of using a recently
				 * succeeded request, if there is one. We keep a list of recently
				 * succeeded BaseSendableGet's, because transient requests are chosen
				 * individually.
				 */
				if (!isInsertScheduler) {
					...
					if (altReq != null && altReq != req) {
						...
						if (prio <= choosenPriorityClass) {
							...
							if (logMINOR)
								Logger.minor(this,
										"Recently succeeded (transient) req " + altReq + " (prio="
												+ altReq.getPriorityClass() + ") is better than " + req + " (prio="
												+ req.getPriorityClass() + "), using that");
							// Don't need to reregister, because removeRandom doesn't
							// actually remove!
							req = altReq;
						}
						else {
				...
				if (logMINOR)
					Logger.minor(this, "removeFirst() returning " + req + " (prio " + req.getPriorityClass()
							+ ", client " + req.getClient() + ", client-req " + req.getClientRequest() + ')');
				if (logMINOR)
	...
	private static final short[] tweakedPrioritySelector = { PriorityClasses.MAXIMUM_PRIORITY_CLASS,
			PriorityClasses.MAXIMUM_PRIORITY_CLASS, PriorityClasses.MAXIMUM_PRIORITY_CLASS,
			PriorityClasses.MAXIMUM_PRIORITY_CLASS, PriorityClasses.MAXIMUM_PRIORITY_CLASS,
			PriorityClasses.MAXIMUM_PRIORITY_CLASS, PriorityClasses.MAXIMUM_PRIORITY_CLASS,

			PriorityClasses.INTERACTIVE_PRIORITY_CLASS, PriorityClasses.INTERACTIVE_PRIORITY_CLASS,
			PriorityClasses.INTERACTIVE_PRIORITY_CLASS, PriorityClasses.INTERACTIVE_PRIORITY_CLASS,
			PriorityClasses.INTERACTIVE_PRIORITY_CLASS, PriorityClasses.INTERACTIVE_PRIORITY_CLASS,

			PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS, PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS,
			PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS, PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS,
			PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS,

			PriorityClasses.UPDATE_PRIORITY_CLASS, PriorityClasses.UPDATE_PRIORITY_CLASS,
			PriorityClasses.UPDATE_PRIORITY_CLASS, PriorityClasses.UPDATE_PRIORITY_CLASS,

			PriorityClasses.BULK_SPLITFILE_PRIORITY_CLASS, PriorityClasses.BULK_SPLITFILE_PRIORITY_CLASS,
			PriorityClasses.BULK_SPLITFILE_PRIORITY_CLASS,

			PriorityClasses.PREFETCH_PRIORITY_CLASS, PriorityClasses.PREFETCH_PRIORITY_CLASS };

	private static final short[] prioritySelector = { PriorityClasses.MAXIMUM_PRIORITY_CLASS,
			PriorityClasses.INTERACTIVE_PRIORITY_CLASS, PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS,
			PriorityClasses.UPDATE_PRIORITY_CLASS, PriorityClasses.BULK_SPLITFILE_PRIORITY_CLASS,
			PriorityClasses.PREFETCH_PRIORITY_CLASS };
	...
	public boolean hasKey(Key key, BaseSendableGet getterWaiting) {
		...
		synchronized (keysFetching) {
			...
			if (!ret)
				return ret;
			// It is being fetched. Add the BaseSendableGet to the wait list so it gets
			// woken up when the request finishes.
			if (getterWaiting != null) {
				...
				if (waiting == null) {
					transientRequestsWaitingForKeysFetching.put(key,
							(WeakReference<BaseSendableGet>[]) new WeakReference<?>[] {
									new WeakReference<BaseSendableGet>(getterWaiting) });
				}
				else {
	...
	public boolean addRunningInsert(SendableRequestItemKey token) {
		synchronized (runningInserts) {
			...
			if (!retval) {
				// This shouldn't happen often, because the chooseBlock()'s should check
				// for it...
	...
	public long checkRecentlyFailed(Key key, boolean realTime) {
		RecentlyFailedChecker checker = sched.getFailedChecker();
		return checker.checkRecentlyFailed(key, realTime);
	}

	/**
	 * Add a request (or insert) to the request selection tree.
	 * @param priorityClass The priority of the request.
	 * @param client Label object indicating which larger group of requests this request
	 * belongs to (e.g. the global queue, or an FCP client), and whether it is persistent.
	 * @param cr The high-level request that this single block request is part of. E.g. a
	 * fetch for a single key may download many blocks in a splitfile; an insert for a
	 * large freesite is considered a single @see ClientRequester.
	 * @param req A single SendableRequest object which is one or more low-level requests.
	 * E.g. it can be an insert of a single block, or it can be a request or insert for a
	 * single segment within a splitfile.
	 * @param context The client context object, which contains links to all the important
	 * objects that are not persisted in the database, e.g. executors, temporary filename
	 * generator, etc.
	 */
	void addToGrabArray(short priorityClass, RequestClient client, ClientRequestSchedulerGroup cr, SendableRequest req,
			ClientContext context) {
		if ((priorityClass > PriorityClasses.PAUSED_PRIORITY_CLASS)
				|| (priorityClass < PriorityClasses.MAXIMUM_PRIORITY_CLASS))
			throw new IllegalStateException(
					"Invalid priority: " + priorityClass + " - range is " + PriorityClasses.MAXIMUM_PRIORITY_CLASS
							+ " (most important) to " + PriorityClasses.PAUSED_PRIORITY_CLASS + " (least important)");
		// Client
		synchronized (this) {
	...
	private ClientRequestRGANode makeSRGAForClient(short priorityClass, RequestClient client, ClientContext context) {
		...
		if (requestGrabber == null) {
			...
			if (logMINOR)
				Logger.minor(this, "Creating new grabber: " + requestGrabber + " for " + client + " from "
						+ clientGrabber + " : prio=" + priorityClass);
	...
	public void reregisterAll(ClientRequester request, RequestScheduler lock, ClientContext context, short oldPrio) {
		...
		synchronized (this) {
			...
			if (clientGrabber == null) {
				// Normal as most of the schedulers aren't relevant to any given
				// insert/request.
				if (logMINOR)
	 ...
	 * @param req
	 * @param context
	 * @param maybeActive Array of requests, can be null, which are being registered in
	 * this group. These will be ignored for purposes of checking whether stuff is
	 * activated when it shouldn't be. It is perfectly okay to have req be a member of
	 * maybeActive.
	 *
	...
	void innerRegister(SendableRequest req, ClientContext context, SendableRequest[] maybeActive) {
		...
		if (isInsertScheduler != req.isInsert())
			throw new IllegalArgumentException(
					"Request isInsert=" + req.isInsert() + " but my isInsertScheduler=" + isInsertScheduler + "!!");
		short prio = req.getPriorityClass();
		if (logMINOR)
			Logger.minor(this, "Still registering " + req + " at prio " + prio + " for " + req.getClientRequest()
					+ " ssk=" + this.isSSKScheduler + " insert=" + this.isInsertScheduler);
