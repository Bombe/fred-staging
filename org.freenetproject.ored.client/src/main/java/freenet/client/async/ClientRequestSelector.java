...
public class ClientRequestSelector implements KeysFetchingLocally {
	 ...
	 * SplitFileInserter. */
	ChosenBlock chooseRequest(int fuzz, RandomSource random, OfferedKeysList offeredKeys, RandomGrabArrayItemExclusionList starter, boolean realTime, ClientContext context) {
	...
	SelectorReturn chooseRequestInner(int fuzz, RandomSource random, OfferedKeysList offeredKeys, RandomGrabArrayItemExclusionList starter, boolean realTime, ClientContext context, long now) {
...
outer:	for(;choosenPriorityClass <= PriorityClasses.MINIMUM_FETCHABLE_PRIORITY_CLASS;choosenPriorityClass++) {
	...
	private static final short[] tweakedPrioritySelector = {
			...
			PriorityClasses.UPDATE_PRIORITY_CLASS,
			PriorityClasses.UPDATE_PRIORITY_CLASS,
			PriorityClasses.UPDATE_PRIORITY_CLASS,
			PriorityClasses.UPDATE_PRIORITY_CLASS,
    ...
    void addToGrabArray(short priorityClass, RequestClient client, ClientRequestSchedulerGroup cr, SendableRequest req, ClientContext context) {
        if((priorityClass > PriorityClasses.PAUSED_PRIORITY_CLASS) || (priorityClass < PriorityClasses.MAXIMUM_PRIORITY_CLASS))
            throw new IllegalStateException("Invalid priority: "+priorityClass+" - range is "+PriorityClasses.MAXIMUM_PRIORITY_CLASS+" (most important) to "+PriorityClasses.PAUSED_PRIORITY_CLASS+" (least important)");
        // Client
        synchronized(this) {
