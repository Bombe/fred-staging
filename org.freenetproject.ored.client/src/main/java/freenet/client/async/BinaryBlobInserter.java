...
public class BinaryBlobInserter implements ClientPutState {
	...
	BinaryBlobInserter(Bucket blob, ClientPutter parent, RequestClient clientContext, boolean tolerant, short prioClass,
			InsertContext ctx, ClientContext context) throws IOException, BinaryBlobFormatException {
		...
		for (Key key : blocks.keys()) {
			...
			MySendableInsert inserter = new MySendableInsert(x++, block, prioClass, getScheduler(block, context),
					clientContext);
	...
	class MySendableInsert extends SimpleSendableInsert {
		...
		public void onFailure(LowLevelException e, SendableRequestItem keyNum, ClientContext context) {
			...
			if (pe.code == LowLevelPutException.ROUTE_NOT_FOUND) {
				consecutiveRNFs++;
				if (logMINOR)
	...
	public void maybeFinish(ClientContext context) {
		...
		if (success) {
			...
			parent.onFailure(new InsertException(InsertExceptionMode.FATAL_ERRORS_IN_BLOCKS, errors, null), this,
					context);
		else
			parent.onFailure(new InsertException(InsertExceptionMode.TOO_MANY_RETRIES_IN_BLOCKS, errors, null), this,
					context);
	}
