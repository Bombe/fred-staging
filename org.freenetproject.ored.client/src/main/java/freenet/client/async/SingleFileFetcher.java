...
public class SingleFileFetcher extends SimpleSingleFileFetcher {
	...
	/**
	 * Number of metaStrings which were added by redirects etc. They are added to the
	 * start, so this is decremented when we consume one.
	 */
	...
	/**
	 * If true, success/failure is immediately reported to the client, and therefore we
	 * can check TOO_MANY_PATH_COMPONENTS.
	 */
	...
	/**
	 * Create a new SingleFileFetcher and register self. Called when following a redirect,
	 * or direct from ClientGet. FIXME: Many times where this is called internally we
	 * might be better off using a copy constructor?
	 * @param topCompatibilityMode
	 * @param topDontCompress
	 * @param hasInitialMetadata
	 */
	public SingleFileFetcher(ClientRequester parent, GetCompletionCallback cb, ClientMetadata metadata, ClientKey key,
			List<String> metaStrings, FreenetURI origURI, int addedMetaStrings, FetchContext ctx,
			boolean deleteFetchContext, boolean realTimeFlag, ArchiveContext actx, ArchiveHandler ah,
			Metadata archiveMetadata, int maxRetries, int recursionLevel, boolean dontTellClientGet, long l,
			boolean isEssential, boolean isFinal, boolean topDontCompress, short topCompatibilityMode,
			ClientContext context, boolean hasInitialMetadata) throws FetchException {
	...
	/**
	 * Copy constructor, modifies a few given fields, don't call schedule(). Used for
	 * things like slave fetchers for MultiLevelMetadata, therefore does not remember
	 * returnBucket, metaStrings etc.
	 */
	public SingleFileFetcher(SingleFileFetcher fetcher, boolean persistent, boolean deleteFetchContext,
			Metadata newMeta, GetCompletionCallback callback, FetchContext ctx2, ClientContext context)
			throws FetchException {
		// Don't add a block, we have already fetched the data, we are just handling the
		// metadata in a different fetcher.
		super(persistent ? fetcher.key.cloneKey() : fetcher.key, fetcher.maxRetries, ctx2, fetcher.parent, callback,
				false, true, fetcher.token, context, deleteFetchContext, fetcher.realTimeFlag);
		if (logMINOR)
			Logger.minor(this,
					"Creating SingleFileFetcher for " + fetcher.key + " meta=" + fetcher.metaStrings.toString(),
					new Exception("debug"));
	...
	/**
	 * Handle the current metadata. I.e. do something with it: transition to a splitfile,
	 * look up a manifest, etc. LOCKING: Synchronized as it changes so many variables; if
	 * we want to write the structure to disk, we don't want this running at the same
	 * time. LOCKING: Therefore it should not directly call e.g. onFailed,
	 * innerWrapHandleMetadata, other stuff that might cause lots of stuff to happen on
	 * other objects, eventually ClientRequestScheduler gets locked -> deadlock. This is
		...
		while (true) {
			...
			if (metaStrings.size() == 0) {
				if (metadata.hasTopData()) {
					if ((metadata.topSize > ctx.maxOutputLength) || (metadata.topCompressedSize > ctx.maxTempLength)) {
						// Just in case...
			...
			if (metadata.isSimpleManifest()) {
				...
				// Then parse it. Then we may need to fetch something from inside the
				// archive.
				// It's more efficient to keep the existing ah if we can, and it is vital
				// in
				...
				if (dataBucket != null) {
					...
					fetchArchive(true, archiveMetadata, filename, new ArchiveExtractCallback() {
						...
						public void gotBucket(Bucket data, ClientContext context) {
							if (logMINOR)
								Logger.minor(this, "Returning data");
							// Because this will be processed immediately, and because the
							// callback uses a StreamGenerator,
							// we can simply pass in the output bucket, even if it is not
							// persistent.
							// If we ever change it so a StreamGenerator can be saved,
							// we'll have to copy here.
				...
				// Clear our own metadata so it can be garbage collected, it will be
				// replaced by whatever is fetched.
				// The new fetcher has our metadata so we don't need to removeMetadata().
				this.metadata = null;
				// We must transition to the sub-fetcher so that if the request is
				...
				// Move any new meta strings to beginning of our list of remaining meta
				// strings
				...
				if ((redirectedKey instanceof ClientCHK) && !((ClientCHK) redirectedKey).isMetadata()) {
					...
					rcb.onSplitfileCompatibilityMode(metadata.getMinCompatMode(), metadata.getMaxCompatMode(),
							redirectedCryptoKey, !((ClientCHK) redirectedKey).isCompressed(), true, true, context);
				}
				...
				if (isFinal && !ctx.ignoreTooManyPathComponents) {
					if (!metaStrings.isEmpty()) {
						...
						if (addedMetaStrings > 0) {
							// Should this be an error?
							// It would be useful to be able to fetch the data ...
							// On the other hand such inserts could cause unpredictable
							// results?
							// Would be useful to make a redirect to the key we actually
							// fetched.
							rcb.onFailure(
									new FetchException(FetchExceptionMode.INVALID_METADATA,
											"Invalid metadata: too many path components in redirects", thisKey),
									this, context);
						}
						else {
							// TOO_MANY_PATH_COMPONENTS
							...
							rcb.onFailure(new FetchException(FetchExceptionMode.TOO_MANY_PATH_COMPONENTS,
									metadata.uncompressedDataLength(), (rcb == parent), clientMetadata.getMIMEType(),
									tryURI), this, context);
						}
						// Just in case...
						return;
				...
				if ((uncompressedLen > ctx.maxOutputLength) || (len > ctx.maxTempLength)) {
					...
					throw new FetchException(FetchExceptionMode.TOO_BIG, uncompressedLen,
							isFinal && decompressors.size() <= (compressed ? 1 : 0), clientMetadata.getMIMEType());
				}

				ClientGetState sf;
				boolean reallyFinal = isFinal;
				if (isFinal && !parent.isCurrentState(this)) {
					Logger.error(this, "isFinal but not the current state for " + this, new Exception("error"));
					reallyFinal = false;
				}
				sf = new SplitFileFetcher(metadata, rcb, parent, ctx, realTimeFlag, decompressors, clientMetadata,
						token, topDontCompress, topCompatibilityMode, persistent, thisKey, reallyFinal, context);
				this.deleteFetchContext = false;
				parent.onTransition(this, sf, context);
				sf.schedule(context);
				rcb.onBlockSetFinished(this, context);
				// Clear our own metadata, we won't need it any more.
				// Note that SplitFileFetcher() above will have used the keys from the
				// metadata,
	...
	private void fetchArchive(boolean forData, Metadata meta, String element, ArchiveExtractCallback callback,
		...
		f = new SingleFileFetcher(this, persistent, true, newMeta,
				new ArchiveFetcherCallback(forData, element, callback),
				new FetchContext(ctx, FetchContext.SET_RETURN_ARCHIVES, true, null), context);
		if (logMINOR)
			Logger.minor(this, "fetchArchive(): " + f);
		// Fetch the archive. The archive fetcher callback will unpack it, and either call
		// the element
		// callback, or just go back around handleMetadata() on this, which will see that
		// the data is now
		// available.

		// We need to transition here, so that everything gets deleted if we are cancelled
		// during the archive fetch phase.
		parent.onTransition(this, f, context);
	...
	class ArchiveFetcherCallback implements GetCompletionCallback, Serializable {
		...
		/**
		 * For activation we need to know whether we are persistent even though the parent
		...
		public void onSuccess(StreamGenerator streamGenerator, ClientMetadata clientMetadata,
			...
			Bucket data = null;
			// FIXME not strictly correct and unnecessary - archive size already checked
			// against ctx.max*Length inside SingleFileFetcher
			...
			if (key instanceof ClientSSK) {
				// Fetching the container is essentially a full success, we should update
				...
				if (hashes != null) {
					...
					try {
						...
						if (!HashResult.strictEquals(results, hashes)) {
							onFailure(new FetchException(FetchExceptionMode.CONTENT_HASH_FAILED),
									SingleFileFetcher.this, context);
							return;
						}
					}
					catch (InsufficientDiskSpaceException e) {
						...
						onFailure(new FetchException(FetchExceptionMode.BUCKET_ERROR, e), SingleFileFetcher.this,
								context);
						return;
					}
					finally {
		...
		public void onSplitfileCompatibilityMode(CompatibilityMode min, CompatibilityMode max, byte[] splitfileKey,
				boolean dontCompress, boolean bottomLayer, boolean definitiveAnyway, ClientContext context) {
			// This is fetching an archive, which may or may not contain the file we are
			// looking for (it includes metadata).
	...
	class MultiLevelMetadataCallback implements GetCompletionCallback, Serializable {
		...
		public void onSuccess(StreamGenerator streamGenerator, ClientMetadata clientMetadata,
			...
			// To be effective should try guess minimal possible size earlier by the
			// number of segments
			long maxLen = Math.min(ctx.maxTempLength, ctx.maxOutputLength);
			try {
			...
			try {
				...
				SingleFileFetcher.this.onFailure(new FetchException(FetchExceptionMode.NOT_ENOUGH_DISK_SPACE), false,
						context);
				return;
			}
			catch (IOException e) {
		...
		public void onFailure(FetchException e, ClientGetState state, ClientContext context) {
			parent.onTransition(state, SingleFileFetcher.this, context);
			// Pass it on; fetcher is assumed to have retried as appropriate already, so
			// this is fatal.
			SingleFileFetcher.this.onFailure(e, true, context);
		}
		...
		public void onSplitfileCompatibilityMode(CompatibilityMode min, CompatibilityMode max, byte[] splitfileKey,
			...
			// Hence we can get them straight away rather than waiting for the bottom
			// layer.
	 ...
	 */
	public static ClientGetState create(ClientRequester requester, GetCompletionCallback cb, FreenetURI uri,
			FetchContext ctx, ArchiveContext actx, int maxRetries, int recursionLevel, boolean dontTellClientGet,
			long l, boolean isEssential, boolean isFinal, ClientContext context, boolean realTimeFlag,
		...
		if ((!uri.hasMetaStrings()) && ctx.allowSplitfiles == false && ctx.followRedirects == false
		...
		if (key instanceof ClientKey || hasInitialMetadata)
			...
			return uskCreate(requester, realTimeFlag, cb, (USK) key, new ArrayList<String>(uri.listMetaStrings()), ctx,
					actx, maxRetries, recursionLevel, dontTellClientGet, l, isEssential, isFinal, context);
		}
	}

	private static ClientGetState uskCreate(ClientRequester requester, boolean realTimeFlag, GetCompletionCallback cb,
			USK usk, ArrayList<String> metaStrings, FetchContext ctx, ArchiveContext actx, int maxRetries,
			int recursionLevel, boolean dontTellClientGet, long l, boolean isEssential, boolean isFinal,
		...
		if (usk.suggestedEdition >= 0) {
			...
			if (edition <= usk.suggestedEdition) {
				...
				if (edition > usk.suggestedEdition) {
				...
				else if (edition == -1 && context.uskManager.lookupLatestSlot(usk) == -1) { // We
																							// do
																							// not
																							// want
																							// to
					...
					USKFetcherTag tag = context.uskManager.getFetcher(usk.copy(usk.suggestedEdition), ctx, false,
							requester.persistent(), realTimeFlag,
							new MyUSKFetcherCallback(requester, cb, usk, metaStrings, ctx, actx, realTimeFlag,
									maxRetries, recursionLevel, dontTellClientGet, l, requester.persistent(), true),
							false, context, true);
					if (isEssential)
					...
					GetCompletionCallback myCB = new USKProxyCompletionCallback(usk, cb, requester.persistent());
					// Want to update the latest known good iff the fetch succeeds.
					SingleFileFetcher sf = new SingleFileFetcher(requester, myCB, null, usk.getSSK(), metaStrings,
							usk.getURI().addMetaStrings(metaStrings), 0, ctx, false, realTimeFlag, actx, null, null,
							maxRetries, recursionLevel, dontTellClientGet, l, isEssential, isFinal, false, (short) 0,
							context, false);
					return sf;
				}
			}
			...
			USKFetcherTag tag = context.uskManager.getFetcher(usk.copy(-usk.suggestedEdition), ctx, false,
					requester.persistent(), realTimeFlag,
					new MyUSKFetcherCallback(requester, cb, usk, metaStrings, ctx, actx, realTimeFlag, maxRetries,
							recursionLevel, dontTellClientGet, l, requester.persistent(), false),
					false, context, false);
			if (isEssential)
				requester.addMustSucceedBlocks(1);
			return tag;
	...
	public static class MyUSKFetcherCallback implements USKFetcherTagCallback, Serializable {
		...
		public MyUSKFetcherCallback(ClientRequester requester, GetCompletionCallback cb, USK usk,
			...
			if (logMINOR)
				Logger.minor(this,
						"Created " + this + " for " + usk + " and " + cb + " datastore only = " + datastoreOnly);
		}
			...
			try {
				if (l == usk.suggestedEdition) {
					SingleFileFetcher sf = new SingleFileFetcher(parent, cb, null, key, metaStrings,
							key.getURI().addMetaStrings(metaStrings), 0, ctx, false, realTimeFlag, actx, null, null,
							maxRetries, recursionLevel + 1, dontTellClientGet, token, false, true, false, (short) 0,
							context, false);
					if (tag != null) {
		...
		public void onFailure(ClientContext context) {
			...
			if (cb == null)
				throw new NullPointerException(
						"Callback is null in " + this + " for usk " + usk + " with datastoreOnly=" + datastoreOnly);
			cb.onFailure(e, null, context);
		}
