...
/**
 * Tracks which blocks have been completed, how many attempts have been made for which
 * blocks, allows choosing a random block, failing a block etc.
 *
 * @author toad
 */
public class SimpleBlockChooser {
	 ...
	 * @return The total number of attempts for the block so far. Some callers (e.g.
	 * inserter) may fail after a single terminal failure, others after some number of
	 * failures (e.g. getter), so we leave this to the caller.
	 */
	 ...
	 * is no longer available due to disk corruption.
	 * @param blockNo
	 */
	 ...
	 * Useful for inserts where we will be able to insert all the blocks until after
	...
	/**
	 * Mass replace of success/failure. Used by fetchers when we try to decode and fail,
	 * possibly because of disk corruption.
	...
	/**
	 * Write the retry counts only, and only if maxRetries != -1. Used if the caller will
	 * manage persistence for the actual list of blocks fetched, as in
