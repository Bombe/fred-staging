...
/**
 * A high level data request. Follows redirects, downloads splitfiles, etc. Similar to
 * what you get from FCP, and is used internally to implement FCP. Also used by fproxy,
 * and plugins, and so on. The current state of the request is stored in currentState. The
 * ClientGetState's do most of the work. SingleFileFetcher for example fetches a key,
 * parses the metadata, and if necessary creates other states to e.g. fetch splitfiles.
 */
public class ClientGetter extends BaseClientGetter
	...
	/**
	 * The current state of the request. SingleFileFetcher when processing metadata for
	 * fetching a simple key, SplitFileFetcher when fetching a splitfile, etc.
	 */
	...
	/**
	 * If not null, Bucket to return the data in, otherwise we create one. If non-null, it
	 * is the responsibility of the callback to create and resume this bucket.
	 */
	...
	/**
	 * If set, and filtering is enabled, the MIME type we filter with must be compatible
	 * with this extension.
	 */
	...
	public ClientGetter(ClientGetCallback client, FreenetURI uri, FetchContext ctx, short priorityClass,
			Bucket returnBucket, BinaryBlobWriter binaryBlobWriter, boolean dontFinalizeBlobWriter,
			Bucket initialMetadata) {
		this(client, uri, ctx, priorityClass, returnBucket, binaryBlobWriter, dontFinalizeBlobWriter, initialMetadata,
				null);
	}
	 ...
	 * ClientGetter must either write the data directly to the bucket, or copy it and free
	 * the original temporary bucket. Preferably the former, obviously!
	 * @param binaryBlobWriter If non-null, we will write all the keys accessed (or that
	 * could have been accessed in the case of redundant structures such as splitfiles) to
	 * this binary blob writer.
	 * @param dontFinalizeBlobWriter If true, the caller is responsible for BlobWriter
	 * finalization
	 */
	public ClientGetter(ClientGetCallback client, FreenetURI uri, FetchContext ctx, short priorityClass,
			Bucket returnBucket, BinaryBlobWriter binaryBlobWriter, boolean dontFinalizeBlobWriter,
			Bucket initialMetadata, String forceCompatibleExtension) {
	 ...
	 * @param overrideURI If non-null, change the URI we are fetching (usually when
	 * restarting).
	 * @param context The client context, contains important mostly non-persistent global
	 * objects.
	...
	/**
	 * Called when the request fails. Retrying will have already been attempted by the
	 * calling state, if appropriate; we have tried to get the data, and given up.
	 ...
	 * @param force If true, finished may already have been set. This is usually set when
	...
	/**
	 * Cancel the request. This must result in onFailure() being called in order to send
	 * the client a cancel FetchException, and to removeFrom() the state.
	 */
	@Override
	...
	/**
	 * Notify clients listening to our ClientEventProducer of the current progress, in the
	 * form of a SplitfileProgressEvent.
	 */
	@Override
	...
	/**
	 * Notify clients that some part of the request has been sent to the network i.e. we
	 * have finished checking the datastore for at least some part of the request. Sent
	 * once only for any given request.
	 */
	@Override
	...
	/**
	 * Called when no more blocks will be added to the total, and therefore we can
	 * confidently display a percentage for the overall progress. Will notify clients with
	 * a SplitfileProgressEvent.
	 */
	@Override
	...
	/**
	 * Called when the current state creates a new state and we switch to that. For
	 * example, a SingleFileFetcher might switch to a SplitFileFetcher. Sometimes this
	 * will be called with oldState not equal to our currentState; this means that a
	 * subsidiary request has changed state, so we ignore it.
	 */
	@Override
	 ...
	 * @param filterData
	 * @param context The database. We must be on the database thread! See ClientContext
	...
	/**
	 * Set a callback to snoop on metadata during fetches. Call this before starting the
	 * request.
	 */
	...
	/**
	 * Set a callback to snoop on buckets (all intermediary data - metadata, containers)
