...
public class SplitFileInserterSender extends SendableInsert {
	...
	public SplitFileInserterSender(SplitFileInserter parent, SplitFileInserterStorage storage) {
		super(parent.persistent, parent.realTime); // Persistence should be from parent so
													// that e.g. callbacks get run on the
													// right jobRunner.
		this.parent = parent;
		...
		public boolean send(NodeClientRequest core, final RequestScheduler sched, ClientContext context,
			...
			try {
				...
				if (request.localRequestOnly) {
					...
					core.realPut(block, request.canWriteClientCache, request.forkOnCacheable,
							NodeConstants.PREFER_INSERT_DEFAULT, NodeConstants.IGNORE_LOW_BACKOFF_DEFAULT,
				...
				context.getJobRunner(request.isPersistent()).queueNormalOrDrop(new PersistentJob() {
					...
					public boolean run(ClientContext context) {
						try {
							...
							request.onFailure(
									new LowLevelPutException(LowLevelPutException.INTERNAL_ERROR, "Disk error", e),
									context);
						}
						return true;
					}
