...
public abstract class PersistentJobRunnerImpl implements PersistentJobRunner {
	...
	private class JobRunnable implements Runnable {
		...
		public void run() {
			...
			try {
				...
				if (logDEBUG)
					Logger.debug(this, "Completed " + job + " with mustCheckpoint=" + mustCheckpoint
							+ " enableCheckpointing=" + enableCheckpointing + " runningJobs=" + runningJobs);
	...
	public void handleCompletion(boolean ret, int threadPriority) {
		synchronized (sync) {
			runningJobs--;
			if (runningJobs == 0)
				// Even if not going to checkpoint indirectly, somebody might be waiting,
				// need to notify.
				sync.notifyAll();
	...
	/**
	 * Typically called after shutdown() to wait for current jobs to complete. Does not
	 * check killed for this reason.
	 */
	...
	/**
	 * Wait until a checkpoint has been completed, or if the job runner becomes idle, do
	 * it here.
