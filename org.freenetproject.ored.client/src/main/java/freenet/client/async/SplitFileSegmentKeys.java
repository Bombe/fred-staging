 ...
 *
 * We will auto-upgrade SplitFileFetcherSegment's to use SplitFileSegmentKeys in the
 * caller.
 *
 * @author toad
 */
public class SplitFileSegmentKeys implements Cloneable, Serializable {
	...
	public SplitFileSegmentKeys(int blocksPerSegment, int checkBlocksPerSegment, byte[] splitfileSingleCryptoKey,
	...
	public static int storedKeysLength(int dataBlocks, int checkBlocks, boolean commonDecryptKey) {
		// FIXME URGENT Implement a unit test for storedKeysLength() vs writeKeys() vs
		// readKeys().
	...
	// Not often used, not very efficient, but overriding equals() requires overriding
	// hashCode().
	@Override
	public int hashCode() {
		final int prime = 31;
