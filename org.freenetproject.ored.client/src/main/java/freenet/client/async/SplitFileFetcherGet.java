...
public class SplitFileFetcherGet extends SendableGet implements HasKeyListener {
	...
	public void onFailure(LowLevelException e, SendableRequestItem token, ClientContext context) {
		FetchException fe = translateException((LowLevelGetException) e);
		if (fe.isDefinitelyFatal()) {
			// If the error is definitely-fatal it means there is either a serious local
			// problem
			// or the inserted data was corrupt. So we fail the entire splitfile
			// immediately.
	 ...
	 * Schedule the fetch.
	 * @param context
	 * @param ignoreStore If true, don't check the datastore before re-registering the
	 * requests to run. Should be true when rescheduling after a normal cooldown, false
	 * after recovering from data corruption (the blocks may still be in the store), false
	 * otherwise.
	 */
