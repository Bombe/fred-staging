...
/**
 * Every X seconds, the RequestSender calls the ClientRequestScheduler to ask for a
 * request to start. A request is then started, in its own thread. It is removed at that
 * point.
 */
public class ClientRequestScheduler implements RequestScheduler {
	...
	/**
	 * Offered keys list. Only one, not split by priority, to prevent various attacks
	 * relating to offering specific keys and timing how long it takes for the node to
	 * request the key. Non-persistent.
	 */
	...
	private final RecentlyFailedChecker failedChecker;
	...
	public ClientRequestScheduler(boolean forInserts, boolean forSSKs, boolean forRT, RandomSource random,
			RequestStarter starter, RecentlyFailedChecker failedChecker, NodeClientRequest core,
			DatastoreChecker storeChecker, String name, ClientContext context) {
		this.isInsertScheduler = forInserts;
		this.isSSKScheduler = forSSKs;
		this.isRTScheduler = forRT;
		schedTransient = new KeyListenerTracker(forInserts, forSSKs, forRT, random, this, null, false);
		this.datastoreChecker = storeChecker;
	...
	/**
	 * Register a group of requests (not inserts): a GotKeyListener and/or one or more
	 * SendableGet's.
	 * @param hasListener Listens for specific keys. Can be null if the listener is
	 * already registered i.e. on retrying.
	 * @param getters The actual requests to register to the request sender queue.
	 * @param persistent True if the request is persistent. NOTE:
	 * delayedStoreCheck/probablyNotInStore is unnecessary because we only register the
	 * listener once.
	 * @throws FetchException
	 */
	public void register(final HasKeyListener hasListener, final SendableGet[] getters, final boolean persistent,
			final BlockSet blocks, final boolean noCheckStore) {
		if (logMINOR)
	...
	void finishRegister(final BaseSendableGet[] getters, boolean persistent, final boolean anyValid) {
		if (logMINOR)
	...
	/**
	 * All the persistent SendableRequest's currently running (either actually in flight,
	 * just chosen, awaiting the callbacks being executed etc). We MUST compare by
	 * pointer, as this is accessed on threads other than the database thread, so we don't
	 * know whether they are active (and in fact that may change under us!). So it can't
	 * be a HashSet.
	 */
	...
	public void removeRunningRequest(SendableRequest request) {
		synchronized (runningPersistentRequests) {
			if (runningPersistentRequests.remove(request)) {
				if (logMINOR)
					Logger.minor(this,
							"Removed running request " + request + " size now " + runningPersistentRequests.size());
			}
		}
		// We *DO* need to call clearCooldown here because it only becomes runnable for
		// persistent requests after it has been removed from starterQueue.
	...
	/**
	 * Called by RequestWaker to find a request to run.
	 */
	@Override
	...
	public void tripPendingKey(final KeyBlock block) {
		...
		if (schedCore.anyProbablyWantKey(key, clientContext)) {
			try {
				...
				// It can wait until after the next checkpoint if necessary. So use
				// queue().
	...
	public void removeFetchingKey(Key key) {
		// Don't need to call clearCooldown(), because selector will do it for each
		// request blocked on the key.
		selector.removeFetchingKey(key);
	}

	@Override
	public void removeRunningInsert(SendableRequest insert, SendableRequestItemKey token) {
		selector.removeRunningInsert(token);
		// Must remove here, because blocks selection and therefore creates cooldown cache
		// entries.
	...
	@Override
	public boolean addRunningInsert(SendableRequest insert, SendableRequestItemKey token) {
	...
	/**
	 * Only used in rare special cases e.g. ClientRequestSelector. FIXME add some
	 * interfaces to get rid of this gross layer violation.
	 */
	RecentlyFailedChecker getFailedChecker() {
