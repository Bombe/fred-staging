 ...
 * recreated here on resume, like in splitfile fetches. The actual status is stored in a
 * RAF.
 *
 * @author toad
 */
public class SplitFileInserter implements ClientPutState, Serializable, SplitFileInserterStorageCallback {
	...
	/**
	 * Whether to free the data when the insert completes/fails. E.g. this is true if the
	 * data is the result of compression.
	 */
	...
	/**
	 * Stores the state of the insert and does most of the work. Created in onResume() or
	 * in the constructor, so must be volatile.
	 */
	private transient volatile SplitFileInserterStorage storage;

	/**
	 * Actually does the insert. Created in onResume() or in the constructor, so must be
	 * volatile.
	 */
	...
	SplitFileInserter(boolean persistent, BaseClientPutter parent, PutCompletionCallback cb,
			LockableRandomAccessBuffer originalData, boolean freeData, InsertContext ctx, ClientContext context,
			long decompressedLength, COMPRESSOR_TYPE compressionCodec, ClientMetadata meta, boolean isMetadata,
			ARCHIVE_TYPE archiveType, byte splitfileCryptoAlgorithm, byte[] splitfileCryptoKey,
			byte[] hashThisLayerOnly, HashResult[] hashes, boolean topDontCompress, int topRequiredBlocks,
			int topTotalBlocks, long origDataSize, long origCompressedDataSize, boolean realTime, Object token)
		...
		try {
			storage = new SplitFileInserterStorage(originalData, decompressedLength, this, compressionCodec, meta,
					isMetadata, archiveType, context.getRandomAccessBufferFactory(persistent), persistent, ctx,
					splitfileCryptoAlgorithm, splitfileCryptoKey, hashThisLayerOnly, hashes,
					context.tempBucketFactory /*
												 * only used for temporaries within
												 * constructor
												 */, new CRCChecksumChecker(), context.fastWeakRandom,
					context.memoryLimitedJobRunner, context.getJobRunner(persistent), context.ticker,
					context.getChkInsertScheduler(realTime).fetchingKeys(), topDontCompress, topRequiredBlocks,
					topTotalBlocks, origDataSize, origCompressedDataSize);
	...
	protected void reportMetadata(Metadata metadata) {
		// Splitfile insert is always reportMetadataOnly, i.e. it always passes the
		// metadata back
		// to the parent SingleFileInserter, which will write it to a Bucket and probably
		// insert it.
