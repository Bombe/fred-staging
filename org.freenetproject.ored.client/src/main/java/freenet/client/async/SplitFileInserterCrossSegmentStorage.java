...
public class SplitFileInserterCrossSegmentStorage {
	...
	// Set to true to encode block keys during *cross-segment* encoding, and thus detect
	// e.g. storage bugs.
	...
	public SplitFileInserterCrossSegmentStorage(SplitFileInserterStorage parent, int segNo, boolean persistent,
			int segLen, int crossCheckBlocks) {
		this.parent = parent;
		this.segNo = segNo;
	...
	void addBlock(SplitFileInserterSegmentStorage seg, int blockNum) {
		...
		if (logMINOR)
			Logger.minor(this, "Allocated cross-segment block " + counter + " to block " + blockNum + " on " + seg
					+ " for " + this);
		counter++;
	}
	...
	SplitFileInserterCrossSegmentStorage(SplitFileInserterStorage parent, DataInputStream dis, int segNo)
		...
		for (int i = 0; i < totalBlocks; i++) {
			...
			if (readBlockNumber < 0 || (readBlockNumber >= segment.dataBlockCount + segment.crossCheckBlockCount)
	 ...
	 * @throws IOException
	 */
	byte[] readCheckBlock(int slotNumberWithinCrossSegment, int segmentNumber, int blockNoWithinSegment)
			throws IOException {
	...
	/**
	 * Cancel the encode.
	 * @return True if we can complete cancelling now, false if we are encoding, in which
	 * case parent will get the usual callback when it is done.
	 */
