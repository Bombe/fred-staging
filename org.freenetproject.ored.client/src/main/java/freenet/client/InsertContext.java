...
public class InsertContext implements Cloneable, Serializable {
	...
	/**
	 * Maximum number of retries (after the initial attempt) for each block inserted. -1 =
	 * retry forever or until it succeeds (subject to consecutiveRNFsCountAsSuccess) or
	 * until a fatal error.
	 */
	public int maxInsertRetries;

	/**
	 * On a very small network, any insert will RNF. Therefore we allow some number of
	 * RNFs to equal success.
	 */
	...
	/**
	 * Maximum number of check blocks per segment for splitfiles. Will be reduced
	 * proportionally if there are fewer data blocks.
	 */
	...
	/**
	 * Can this insert write to the client-cache? We don't store all requests in the
	 * client cache, in particular big stuff usually isn't written to it, to maximise its
	 * effectiveness. Plus, local inserts are not written to the client-cache by default
	 * for privacy reasons.
	 */
	 ...
	 * @see freenet.compress.Compressor.COMPRESSOR_TYPE#getCompressorsArray(String)
	 */
	...
	/**
	 * Compatibility mode. This determines exactly how we insert data, so that we can
	 * produce the same CHK when reinserting a key even if it is with a later version of
	 * Freenet. It is also important for e.g. auto-update to be able to insert keys
	 * compatible with older nodes, but CompatibilityMode's are sometimes backwards
	 * compatible, there are separate versioning systems for keys and Metadata, which will
	 * be set as appropriate for an insert depending on the CompatibilityMode.
	 */
	public static enum CompatibilityMode {
		...
		/**
		 * No compatibility issues, use the most efficient metadata possible. Used only in
		 * the front-end: MUST NOT be stored: Code should convert this to a specific mode
		 * as early as possible, or inserts will break when a new mode is added.
		 * InsertContext does this.
		 */
		COMPAT_CURRENT((short) 1),
		// The below *are* used in Metadata compatibility mode detection. And they are
		...
		COMPAT_1250((short) 3),
		/**
		 * 1251/2/3: Basic even splitting, 1 extra check block if data blocks < 128, max
		 * 131 data blocks.
		 */
		COMPAT_1251((short) 4),
		/**
		 * 1255: Second stage of even splitting, a whole bunch of segments lose one block
		 * rather than the last segment losing lots of blocks. And hashes too!
		 */
		COMPAT_1255((short) 5),
		/** 1416: New CHK encryption */
		COMPAT_1416((short) 6),
		/**
		 * 1468: Fill in topDontCompress and topCompatibilityMode on splitfiles. Same
		 * blocks, but slightly different metadata.
		 */
		COMPAT_1468((short) 7);
		...
		// Inserts should be converted to a specific compatibility mode as soon as
		// possible, to avoid
		// problems when an insert is restarted on a newer build with a newer default
		// compat mode.
		...
		/**
		 * Must be called whenever we accept a CompatibilityMode as e.g. a config option.
		 * Converts the pseudo-
		 */
		 ...
		 * Usually this will be COMPAT_CURRENT (it will get converted into a specific mode
		 * later), but when a new compatibility mode is deployed we may want to keep this
		 * at an earlier version to avoid a period when data inserted with the new/testing
		 * builds can't be fetched with earlier versions.
		 */
	 ...
	 * layers as soon as we can, rather than waiting for the lower layers. The default
	 * behaviour is safer, because an attacker can usually only identify the datastream
	 * once he has the top block, or once you have announced the key.
	 */
	...
	public InsertContext(int maxRetries, int rnfsToSuccess, int splitfileSegmentDataBlocks,
			int splitfileSegmentCheckBlocks, ClientEventProducer eventProducer, boolean canWriteClientCache,
			boolean forkOnCacheable, boolean localRequestOnly, String compressorDescriptor, int extraInsertsSingleBlock,
	...
	public int hashCode() {
		...
		result = prime * result + ((compressorDescriptor == null) ? 0 : compressorDescriptor.hashCode());
