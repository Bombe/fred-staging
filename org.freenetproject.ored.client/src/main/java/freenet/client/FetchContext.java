...
/**
 * Context for a Fetcher. Contains all the settings a Fetcher needs to know about. FIXME
 * these should be final or private, with getters/setters and checking for valid values
 * e.g. maxRecursionLevel >= 1.
 */
public class FetchContext implements Cloneable, Serializable {
	...
	/**
	 * Maximum length of data fetched in order to obtain the final data - metadata,
	 * containers, etc.
	 */
	public long maxTempLength;

	/**
	 * 1 = only fetch a single block. 2 = allow one redirect, e.g. metadata block pointing
	 * to actual data block. Etc. 0 may work sometimes but is not recommended.
	 */
	...
	/**
	 * Maximum number of manifest lookups during a request. A manifest lookup is looking
	 * up a part of a pathname in a "manifest", which is essentially a directory (folder).
	 * Usually manifest lookups are inside containers (archives), which are usually tar
	 * files, which may or may not be compressed (compression occurs transparently on a
	 * different level). This is not necessarily the same as the number of slashes in the
	 * key after the part for the key itself, since keys can redirect to other keys. If
	 * you are fetching user-uploaded keys, e.g. in fproxy, especially freesites, you will
	 * want this to be non-zero. However if you are using keys only internally, and never
	 * upload freesites, you should set this to 0.
	...
	/**
	 * Maximum number of retries (after the original attempt) for a splitfile block. -1 =
	 * try forever or until success or a fatal error. A fatal error is either an internal
	 * error (problem with the node) or something resulting from the original data being
	 * corrupt as inserted. So with retries = -1 we will not report Data not found, Route
	 * not found, All data not found, etc, because these are nonfatal errors and we will
	 * retry. Note that after every 3 attempts the request is put on the cooldown queue
	 * for 30 minutes, so the cost of retries = -1 is really not that high.
	 */
	public int maxSplitfileBlockRetries;

	/**
	 * Maximum number of retries (after the original attempt) for a non-splitfile block.
	 * -1 = try forever or until success or a fatal error.. -1 = try forever or until
	 * success or a fatal error. A fatal error is either an internal error (problem with
	 * the node) or something resulting from the original data being corrupt as inserted.
	 * So with retries = -1 we will not report Data not found, Route not found, All data
	 * not found, etc, because these are nonfatal errors and we will retry. Note that
	 * after every 3 attempts the request is put on the cooldown queue for 30 minutes, so
	 * the cost of retries = -1 is really not that high.
	 */
	...
	/**
	 * If true, only read from the datastore and caches, do not send the request to the
	 * network
	 */
	public boolean localRequestOnly;

	/**
	 * If true, send the request to the network without checking whether the data is in
	 * the local store
	 */
	...
	/**
	 * If true, and we get a ZIP manifest, and we have no meta-strings left, then return
	 * the manifest contents as data.
	 */
	...
	/**
	 * If non-null, the request will be stopped if it has a MIME type that is not one of
	 * these, or has no MIME type.
	 */
	...
	/**
	 * Can this request write to the client-cache? We don't store all requests in the
	 * client cache, in particular big stuff usually isn't written to it, to maximise its
	 * effectiveness.
	 */
	 ...
	 * RequestScheduler.COOLDOWN_PERIOD, because ULPRs will ensure rapid success with that
	 * interval or less.
	 */
	...
	public FetchContext(long curMaxLength, long curMaxTempLength, int maxMetadataSize, int maxRecursionLevel,
			int maxArchiveRestarts, int maxArchiveLevels, boolean dontEnterImplicitArchives,
			int maxSplitfileBlockRetries, int maxNonSplitfileRetries, int maxUSKRetries, boolean allowSplitfiles,
			boolean followRedirects, boolean localRequestOnly, boolean filterData, int maxDataBlocksPerSegment,
			int maxCheckBlocksPerSegment, BucketFactory bucketFactory, ClientEventProducer producer,
	 ...
	 * @param ctx The old FetchContext to copy.
	 * @param maskID Mask mode for the copy operation e.g. SPLITFILE_DEFAULT_BLOCK_MASK.
	 * @param keepProducer If true, keep the existing EventProducer. Must be false if we
	 * are creating a new request. Can be true if we are masking the FetchContext within a
	 * single request, e.g. to download a container. This is important so that we see the
	 * progress updates for the request and not for other requests sharing the
	 * FetchContext, but also it could break serialization.
	...
	public void setCooldownTime(long cooldownTime, boolean force) {
		...
		if (cooldownTime < RequestScheduler.COOLDOWN_PERIOD && !force)
			throw new IllegalArgumentException("Invalid COOLDOWN_PERIOD: Must be >= " + RequestScheduler.COOLDOWN_PERIOD
	...
	/**
	 * Create from a saved form, e.g. for restarting a request from scratch. Will create
	 * its own SimpleEventProducer.
	 * @param dis
	...
	public FetchContext(DataInputStream dis) throws StorageFormatException, IOException {
		...
		try {
			...
			// input stream reached EOF, so it must have been and old version without
			// scehmeHostAndPort.
			s = "";
		}
		if (s.equals("")) {
	...
	public int hashCode() {
		...
		// Don't include blocks. It doesn't implement content-based hashCode() and
		// equals(). FIXME
	...
	/**
	 * Are two InsertContext's equal? Ignores the EventProducer, compares only the actual
	 * config values.
	 */
	@Override
	public boolean equals(Object obj) {
		...
		if (allowedMIMETypes == null) {
			...
			return false;
		// We *DO* compare on blocks, which means that two FetchContext's can be non-equal
		// even
