...
public class Metadata implements Cloneable, Serializable {
	...
	public enum DocumentType {
		...
		SIMPLE_REDIRECT((byte) 0), MULTI_LEVEL_METADATA((byte) 1), SIMPLE_MANIFEST((byte) 2), ARCHIVE_MANIFEST(
				(byte) 3), ARCHIVE_INTERNAL_REDIRECT(
						(byte) 4), ARCHIVE_METADATA_REDIRECT((byte) 5), SYMBOLIC_SHORTLINK((byte) 6);
	...
	// static final short FLAGS_SPLIT_USE_LENGTHS = 64; FIXME not supported, reassign to
	// something else if we need a new flag
	static final short FLAGS_COMPRESSED = 128;
	static final short FLAGS_TOP_SIZE = 256;
	static final short FLAGS_HASHES = 512;

	// If parsed version = 1 and splitfile is set and hashes exist, we create the
	// splitfile key from the hashes.
	// This flag overrides this behaviour and reads a key anyway.
	static final short FLAGS_SPECIFY_SPLITFILE_KEY = 1024;

	// We can specify a hash just for this layer as well as hashes for the final content
	// in a multi-layer splitfile.
	...
	/**
	 * The compressed MIME type - lookup index for the MIME types table. Must be between 0
	 * and 32767.
	 */
	...
	/**
	 * Metadata is sometimes used as a key in hashtables. Therefore it needs a persistent
	 * hashCode.
	 */
	...
	public enum SplitfileAlgorithm {
		...
		NONREDUNDANT((short) 0), ONION_STANDARD((short) 1);
	...
	// If false, the splitfile key can be computed from the hashes. If true, it must be
	// specified.
	...
	/**
	 * Parse a block of bytes into a Metadata structure. Constructor method because of
	 * need to catch impossible exceptions.
	...
	public Metadata(DataInputStream dis, long length) throws IOException, MetadataParseException {
		...
		if (hasTopBlocks) {
			...
			if (CompatibilityMode.hasCode(code) && code != CompatibilityMode.COMPAT_CURRENT.code) { // COMPAT_UNKNOWN
																									// is
																									// OK
																									// but
																									// COMPAT_CURRENT
		...
		if ((!splitfile) && ((documentType == DocumentType.SIMPLE_REDIRECT)
			...
			if (splitfileCheckBlocks > MAX_SPLITFILE_BLOCKS)
				throw new MetadataParseException(
						"Too many splitfile check-blocks (soft limit to prevent memory DoS): " + splitfileCheckBlocks);
			...
			if (splitfileAlgorithm == SplitfileAlgorithm.NONREDUNDANT) {
				...
				if (getParsedVersion() == 0) {
					...
					if (countDataBlocks == countCheckBlocks) {
						...
						if (blocksPerSegment == 128) {
							// Is the last segment small enough that we can't have used
							// even splitting?
				...
				if (topCompatibilityMode != CompatibilityMode.COMPAT_UNKNOWN) {
					// If we have top compatibility mode, then we can give a definitive
					// answer immediately, with the splitfile key, with dontcompress, etc
					// etc.
				...
				// FIXME remove this eventually. Will break compat with a few files
				// inserted between 1135 and 1136.
				// Work around a bug around build 1135.
				// We were splitting as (128,255), but we were then setting the
				// checkBlocksPerSegment to 64.
				// Detect this.
			...
			if (segmentCount <= 0) {
				...
				// so copy them
				segments[0] = new SplitFileSegmentKeys(splitfileBlocks, splitfileCheckBlocks, splitfileSingleCryptoKey,
				...
				for (int i = 0; i < segments.length; i++) {
					...
					if (i == segments.length - 1) {
						// Always accept the remainder as the last segment, but do basic
						// sanity checking.
						// In practice this can be affected by various things: 1) On old
						// splitfiles before full even
						// segment splitting with deductBlocksFromSegments (i.e.
						// pre-1255), the last segment could be
						// significantly smaller than the rest; 2) On 1251-1253, with
						// partial even segment splitting,
						// up to 131 data blocks per segment, cutting the check blocks if
						// necessary, and with an extra
						// check block if possible, the last segment could have *more*
						// check blocks than the rest.
						...
						if (copyCheckBlocks <= 0 || copyDataBlocks <= 0)
							throw new MetadataParseException("Last segment has bogus block count: total data blocks "
									+ splitfileBlocks + " total check blocks " + splitfileCheckBlocks + " segment size "
									+ blocksPerSegment + " data " + checkBlocksPerSegment + " check " + crossCheckBlocks
									+ " cross check blocks, deduct " + deductBlocksFromSegments + ", segments "
									+ segments.length);
					}
					else if (segments.length - i <= deductBlocksFromSegments) {
						// Deduct one data block from each of the last
						// deductBlocksFromSegments segments.
						// This ensures no segment is more than 1 block larger than any
						// other.
					...
					segments[i] = new SplitFileSegmentKeys(copyDataBlocks, copyCheckBlocks, splitfileSingleCryptoKey,
							splitfileSingleCryptoAlgorithm);
					if (logMINOR)
						Logger.minor(this, "REQUESTING: Segment " + i + " of " + segments.length + " : "
								+ copyDataBlocks + " data blocks " + copyCheckBlocks + " check blocks");
		...
		if ((documentType == DocumentType.ARCHIVE_INTERNAL_REDIRECT)
			...
			while (true) {
				if (targetName.isEmpty())
					throw new MetadataParseException(
							"Invalid target name is empty: \"" + new String(buf, "UTF-8") + "\"");
	...
	public static byte[] getCryptoKey(byte[] hash) {
		// This is exactly the same algorithm used by e.g. JFK for generating multiple
		// session keys from a single generated value.
		// The only difference is we use a constant of more than one byte's length here,
		// to avoid having to keep a registry.
	 ...
	 * @param dir A map of names (string) to either files (same string) or directories
	 * (more HashMap's)
	...
	/**
	 * Create a Metadata object and add manifest entries from the given map. The map can
	 * contain either string -> Metadata, or string -> map, the latter indicating subdirs.
	 */
	...
	private void addRedirectionManifestWithMetadata(HashMap<String, Object> dir) {
		...
		for (Map.Entry<String, Object> entry : dir.entrySet()) {
			...
			if (key.indexOf('/') != -1)
				throw new IllegalArgumentException(
						"Slashes in simple redirect manifest filenames! (slashes denote sub-manifests): " + key);
			Object o = entry.getValue();
	 ...
	 */
	public Metadata(DocumentType docType, ARCHIVE_TYPE archiveType, COMPRESSOR_TYPE compressionCodec, String arg,
			ClientMetadata cm) {
		hashCode = super.hashCode();
	...
	/**
	 * Create a Metadata redircet object that points to resolved metadata inside
	 * container. docType = ARCHIVE_METADATA_REDIRECT
	...
	public Metadata(DocumentType docType, ARCHIVE_TYPE archiveType, COMPRESSOR_TYPE compressionCodec, FreenetURI uri,
			ClientMetadata cm) {
		this(docType, archiveType, compressionCodec, uri, cm, 0, 0, 0, 0, false, CompatibilityMode.COMPAT_UNKNOWN,
				null);
	}
	 ...
	 */
	public Metadata(DocumentType docType, ARCHIVE_TYPE archiveType, COMPRESSOR_TYPE compressionCodec, FreenetURI uri,
			ClientMetadata cm, long origDataLength, long origCompressedDataLength, int reqBlocks, int totalBlocks,
			boolean topDontCompress, CompatibilityMode topCompatibilityMode, HashResult[] hashes) {
	 ...
	 * @param segmentSize The number of data blocks in a typical segment. Does not include
	 * cross-check blocks.
	 * @param checkSegmentSize The number of check blocks in a typical segment. Does not
	 * include cross-check blocks.
	 * @param deductBlocksFromSegments If this is set, the last few segments will lose a
	 * data block, so that all the segments are the same size to within 1 block. In older
	 * splitfiles, the last segment could be significantly smaller, and this impacted on
	 * retrievability.
	 * @param cm The client metadata i.e. MIME type.
	 * @param dataLength The size of the data that this specific splitfile encodes (as
	 * opposed to the final data), after compression if necessary.
	 ...
	 * @param decompressedLength The length of this specific splitfile's data after it has
	 * been decompressed.
	 * @param isMetadata If true, the splitfile is multi-level metadata i.e. it encodes a
	 * bucket full of metadata. This usually happens for really big splitfiles, which can
	 * be a pyramid of one block with metadata for a splitfile full of metadata, that
	 * metadata then encodes another splitfile full of metadata, etc. Hence we can support
	 * very large files.
	 * @param hashes Various hashes of <b>the final data</b>. There should always be at
	 * least an SHA256 hash, unless we are inserting with an old compatibility mode.
	 * @param hashThisLayerOnly Hash of the data in this layer (before compression).
	 * Separate from hashes of the final data. Not currently verified.
	 * @param origDataSize The size of the final/original data.
	 * @param origCompressedDataSize The size of the final/original data after it was
	 * compressed.
	 * @param requiredBlocks The number of blocks required on fetch to reconstruct the
	 * final data. Hence as soon as we have the top splitfile metadata (i.e. hopefully in
	 * the top block), we can show an accurate progress bar.
	 * @param totalBlocks The total number of blocks inserted during the whole insert for
	 * the final/original data.
	 * @param topDontCompress Whether dontCompress was enabled. This allows us to figure
	 * out reinsert settings more quickly.
	 * @param topCompatibilityMode The compatibility mode applying to the insert. This
	 * allows us to figure out reinsert settings more quickly.
	 * @param splitfileCryptoAlgorithm The block level crypto algorithm for all the blocks
	 * in the splitfile.
	 * @param splitfileCryptoKey The single encryption key used by all blocks in the
	 * splitfile. Older splitfiles don't have this so have to specify the full keys; newer
	 * splitfiles just specify the 32 byte routing key for each data or check key.
	 * @param specifySplitfileKey If false, the splitfile crypto key has been
	 * automatically computed from the final or this-layer data hash. If true, it has been
	 * specified explicitly, either because it is randomly generated (this significantly
	 * improves security against mobile attacker source tracing and is the default for
	 * splitfiles under SSKs), or because a file is being reinserted.
	 * @param crossSegmentBlocks The number of cross-check blocks. If this is specified,
	 * we are using cross-segment redundancy. This greatly improves reliability on files
	 * over 80MB, see bug #3370.
	 */
	public Metadata(SplitfileAlgorithm algo, ClientCHK[] dataURIs, ClientCHK[] checkURIs, int segmentSize,
			int checkSegmentSize, int deductBlocksFromSegments, ClientMetadata cm, long dataLength,
			ARCHIVE_TYPE archiveType, COMPRESSOR_TYPE compressionCodec, long decompressedLength, boolean isMetadata,
			HashResult[] hashes, byte[] hashThisLayerOnly, long origDataSize, long origCompressedDataSize,
			int requiredBlocks, int totalBlocks, boolean topDontCompress, CompatibilityMode topCompatibilityMode,
		...
		if (parsedVersion == 0) {
			...
			// The format of the Metadata object is different for creating one from
			// scratch for
			// inserting vs constructing it from a serialized metadata document.
			if (mode == SPLITFILE_PARAMS_CROSS_SEGMENT)
				b = Fields.intsToBytes(
						new int[] { segmentSize, checkSegmentSize, deductBlocksFromSegments, crossSegmentBlocks });
	 ...
	 * @throws IOException
	 * @throws MalformedURLException If the key could not be read due to an error in
	 * parsing the key. REDFLAG: May want to recover from these in future, hence the short
	 * length.
	 */
	 ...
	 * @throws IOException
	 * @throws MalformedURLException If an error in the key itself prevented it from being
	 * written.
	 */
	...
	/**
	 * Return and remove a specific document. Used in persistent requests so that when
	 * removeFrom() is called, the default document won't be removed, since it is being
	 * processed.
	 */
	...
	/**
	 * Return and remove the default document. Used in persistent requests so that when
	 * removeFrom() is called, the default document won't be removed, since it is being
	 * processed.
	 */
	...
	/**
	 * Return the name of the document referred to in the archive, if this is a archive
	 * internal redirect.
	 */
	...
	/**
	 * Return the name of the document referred to in the dir, if this is a symbolic short
	 * link.
	 */
	...
	/**
	 * Change the document type to a simple redirect. Used by the archive code to fetch
	...
	public void writeTo(DataOutputStream dos) throws IOException, MetadataUnresolvedException {
		...
		boolean hasTopBlocks = topBlocksRequired != 0 || topBlocksTotal != 0 || topSize != 0 || topCompressedSize != 0
				|| topCompatibilityMode != CompatibilityMode.COMPAT_UNKNOWN;
		if (haveFlags()) {
			short flags = 0;
			if (splitfile)
	 ...
	 * It is a replacement for mkRedirectionManifestWithMetadata, used in
	 * BaseManifestPutter <PRE>
	...
	/**
	 * If there is a custom (not computed from hashes) splitfile key, return it. Else
	 * return null.
	 */
	...
	/**
	 * Return a best-guess compatibility mode, guaranteed not to be COMPAT_UNKNOWN or
	 * COMPAT_CURRENT.
	 */
