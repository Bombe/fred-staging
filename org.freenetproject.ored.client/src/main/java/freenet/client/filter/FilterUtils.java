...
public class FilterUtils {
	...
	private final static int MAX_NTH = 999999; // Limit range of numbers allowed in isNth,
												// due to incorrect behavior found in
	...
	public static boolean isPercentage(String value) {
		if (value.length() >= 2 && value.charAt(value.length() - 1) == '%') // Valid
																			// percentage
																			// X%
		{
			// Percentages are <number>%
			// That means they can be positive, negative, zero, >100%, and they can
	...
	public static boolean isLength(String value, boolean isSVG) // SVG lengths allow %
																// values
	{
	...
	static {
		cssMedia.addAll(Arrays.asList("all", "aural", "braille", "embossed", "handheld", "print", "projection",
	...
	public static boolean isIntegerInRange(String strValue, int min, int max) {
		try {
			// Strip any leading '+' character, because Integer.parseInt handles it
			// differently between Java 6 (fails) and 7 (succeeds).
	...
	public static boolean isNth(String value) {
		if (value.equals("odd") || value.equals("even") || isIntegerInRange(value, -MAX_NTH, MAX_NTH)) {
			...
			// Check if value has the form "an+b" - where a and b can be any in range
			// integer.
