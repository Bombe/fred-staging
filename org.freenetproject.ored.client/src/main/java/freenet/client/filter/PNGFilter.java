...
public class PNGFilter implements ContentDataFilter {
	...
	static final String[] HARMLESS_CHUNK_TYPES = {
			...
			"tRNS", "cHRM", "gAMA", "iCCP", // FIXME Embedded ICC profile: could this
											// conceivably cause a web lookup?
			...
			"acTL", "fcTL", "fdAT"
			// MNG isn't supported by Firefox and IE because of lack of market demand. Big
			// surprise
			// given nobody supports it! It is supported by Konqueror though. Complex
			// standard,
		...
		try {
			...
			while (!hasSeenIEND) {
				...
				if (length > 0) {
					...
					if (logMINOR)
						if (logDEBUG)
							Logger.minor(this, "data (offset=0x" + Long.toHexString(offset) + ") "
				...
				if (checkCRCs) {
					...
					if (readCRC != computedCRC) {
						...
						if (logMINOR)
							Logger.minor(this,
									"CRC of the chunk " + chunkTypeString + " doesn't match ("
											+ Long.toHexString(readCRC) + " but should be "
				...
				if (!skip && "IHDR".equals(chunkTypeString)) { // http://www.w3.org/TR/PNG/#11IHDR
					...
					if (logMINOR)
						Logger.minor(this,
								"Info from IHDR: width=" + width + "px height=" + height + "px bitDepth=" + bitDepth
										+ " colourType=" + colourType + " compressionMethod=" + compressionMethod
				...
				if (!validChunkType) {
					...
					if (logMINOR)
						Logger.minor(this,
								"Writing " + chunkTypeString + " (" + baos.size() + ") to the output bucket");
					baos.writeTo(output);
					baos.flush();
				}
	...
	public static void main(String arg[]) throws Throwable {
		...
		try {
			...
			ContentFilter.filter(inputStream, outputStream, "image/png", new URI("http://127.0.0.1:8888/"), null, null,
					null, null);
		}
		finally {
