...
/**
 * Freenet content filter. This doesn't actually do any filtering, it organizes everything
...
public class ContentFilter {
	...
	public static final String[] HTML_MIME_TYPES = new String[] { "text/html", "application/xhtml+xml",
			"text/xml+xhtml", "text/xhtml", "application/xhtml" };
	...
	public static void init() {
		...
		// Plain text
		register(new FilterMIMEType("text/plain", "txt", new String[0], new String[] { "text", "pot" }, true, true,
				null, false, false, false, false, false, false, l10n("textPlainReadAdvice"), true, "US-ASCII", null,
				false));

		// GIF - has a filter
		register(new FilterMIMEType("image/gif", "gif", new String[0], new String[0], true, false, new GIFFilter(),
				false, false, false, false, false, false, l10n("imageGifReadAdvice"), false, null, null, false));

		// JPEG - has a filter
		register(new FilterMIMEType("image/jpeg", "jpeg", new String[0], new String[] { "jpg" }, true, false,
				new JPEGFilter(true, true), false, false, false, false, false, false, l10n("imageJpegReadAdvice"),
				false, null, null, false));

		// PNG - has a filter
		register(new FilterMIMEType("image/png", "png", new String[] { "image/x-png" }, new String[0], true, false,
				new PNGFilter(true, true, true), false, false, false, false, true, false, l10n("imagePngReadAdvice"),
				false, null, null, false));

		// BMP - has a filter
		// Reference: http://filext.com/file-extension/BMP
		register(new FilterMIMEType("image/bmp", "bmp",
				new String[] { "image/x-bmp", "image/x-bitmap", "image/x-xbitmap", "image/x-win-bitmap",
						"image/x-windows-bmp", "image/ms-bmp", "image/x-ms-bmp", "application/bmp", "application/x-bmp",
						"application/x-win-bitmap" },
				new String[0], true, false, new BMPFilter(), false, false, false, false, true, false,
				l10n("imageBMPReadAdvice"), false, null, null, false));

		/*
		 * Ogg - has a filter Xiph's container format. Contains one or more logical
		 * bitstreams. Each type of bitstream will likely require additional processing,
		 * on top of that needed for the Ogg container itself. Reference:
		 * http://xiph.org/ogg/doc/rfc3533.txt
		 */
		register(new FilterMIMEType("application/ogg", "ogx", new String[] { "video/ogg", "audio/ogg" },
				new String[] { "ogg", "oga", "ogv" }, true, false, new OggFilter(), true, true, false, true, false,
				false, l10n("containerOggReadAdvice"), false, null, null, false));

		/*
		 * FLAC - Needs filter Lossless audio format. This data is sometimes encapsulated
		 * inside of ogg containers. It is, however, not currently supported, and is very
		 * dangerous, as it may specify URLs from which album art will be dwonloaded from
		 */
		register(new FilterMIMEType("audio/flac", "flac", new String[] { "application/x-flac" }, new String[0], true,
				true, new FlacFilter(), true, true, false, true, false, false, l10n("audioFLACReadAdvice"), false, null,
				null, false));

		// M3U - strict filter
		register(new FilterMIMEType("audio/mpegurl", "m3u",
				new String[] { "application/vnd.apple.mpegurl", "application/mpegurl", "application/x-mpegurl",
						"audio/x-mpegurl" },
				new String[] { "m3u8" }, false, false, new M3UFilter(), false, false, false, false, false, false,
				l10n("audioM3UReadAdvice"), false, "utf-8", null, false));
		 ...
		 */
		register(new FilterMIMEType("audio/mpeg", "mp3",
				new String[] { "audio/mp3", "audio/x-mp3", "audio/x-mpeg", "audio/mpeg3", "audio/x-mpeg3", "audio/mpg",
						"audio/x-mpg", "audio/mpegaudio" },
		...
		// PDF - very dangerous - FIXME ideally we would have a filter, this is such a
		// common format...
		register(new FilterMIMEType("application/pdf", "pdf", new String[] { "application/x-pdf" }, new String[0],
				false, false, null, true, true, true, false, true, true, l10n("applicationPdfReadAdvice"), false, null,
				null, false));

		// HTML - dangerous if not filtered
		register(new FilterMIMEType(HTML_MIME_TYPES[0], "html",
				Arrays.asList(HTML_MIME_TYPES).subList(1, HTML_MIME_TYPES.length)
						.toArray(new String[HTML_MIME_TYPES.length - 1]),
				new String[] { "htm" }, false, false /* maybe? */, new HTMLFilter(), true, true, true, true, true, true,
				l10n("textHtmlReadAdvice"), true, "iso-8859-1", new HTMLFilter(), false));

		// CSS - danagerous if not filtered, not sure about the filter
		register(new FilterMIMEType("text/css", "css", new String[0], new String[0], false,
				false /* unknown */, new CSSReadFilter(), true, true, true, true, true, false,
				l10n("textCssReadAdvice"), true, "utf-8", new CSSReadFilter(), true));
	...
	public static FilterStatus filter(InputStream input, OutputStream output, String typeName, URI baseURI,
			FoundURICallback cb, TagReplacerCallback trc, String maybeCharset)
	 ...
	 * @throws UnsafeContentTypeException If the MIME type is declared unsafe (e.g. pdf
	 * files)
	 * @throws IllegalStateException If data is invalid (e.g. corrupted file) and the
	 * filter have no way to recover.
	 */
	public static FilterStatus filter(InputStream input, OutputStream output, String typeName, URI baseURI,
			String schemeHostAndPort, FoundURICallback cb, TagReplacerCallback trc, String maybeCharset)
			throws UnsafeContentTypeException, IOException {
	 ...
	 */
	@Deprecated // please move to filter with schemeHostAndPort, called from this method.
	public static FilterStatus filter(InputStream input, OutputStream output, String typeName, String maybeCharset,
	 ...
	 */
	public static FilterStatus filter(InputStream input, OutputStream output, String typeName, String maybeCharset,
			String schemeHostAndPort, FilterCallback filterCallback) throws UnsafeContentTypeException, IOException {
		if (logMINOR)
		...
		if (idx != -1) {
			...
			// Parse options
			// Format: <type>/<subtype>[ optional white space ];[ optional white space
	...
	public static String detectCharset(byte[] input, int length, FilterMIMEType handler, String maybeCharset)
			throws IOException {
		// Detect charset
		...
		if ((charset == null) && (handler.charsetExtractor != null)) {
			...
			if (bom != null) {
				charset = bom.charset;
				if (charset != null) {
					// These detections are not firm, and can detect a family e.g. ASCII,
					// EBCDIC,
			...
			if (handler.defaultCharset != null) {
				try {
					if ((charset = handler.charsetExtractor.getCharset(input, length,
							handler.defaultCharset)) != null) {
						if (logMINOR)
							Logger.minor(ContentFilter.class, "Returning charset: " + charset);
						return charset;
					}
	...
	/**
	 * Detect a Byte Order Mark, a sequence of bytes which identifies a document as
	 * encoded with a specific charset.
	 * @throws IOException
	 */
	...
	// Byte Order Mark's - from Wikipedia. We keep all of them because a rare encoding
	// might
	// be deliberately used by an attacker to confuse the filter, because at present a
	// charset
	// is not mandatory, and because some browsers may pick these up anyway even if one is
	// present.
	...
	// These BOMs are invalid. That is, we do not support them, they will produce an
	// unrecoverable error, since we cannot decode them, but the browser might be able to,
	// as e.g. the CSS spec refers to them.
	...
	/**
	 * Check whether we can safely handle a specific MIME type. Usually called when we
	 * haven't downloaded the data yet so can't filter it, so we can know whether there
