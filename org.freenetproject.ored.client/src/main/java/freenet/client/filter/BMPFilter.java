...
/**
 * @author kurmiashish This class would verify whether the BMP header is valid or not
 * Reference: http://www.fastgraph.com/help/bmp_header_format.html
...
public class BMPFilter implements ContentDataFilter {
	...
	public void readFilter(InputStream input, OutputStream output, String charset, Map<String, String> otherParams,
		...
		if ((!Arrays.equals(StartWord, bmpHeaderwindows)) && (!Arrays.equals(StartWord, bmpHeaderos2bArray))
				&& (!Arrays.equals(StartWord, bmpHeaderos2cIcon)) && (!Arrays.equals(StartWord, bmpHeaderos2cPointer))
				&& (!Arrays.equals(StartWord, bmpHeaderos2Icon)) && (!Arrays.equals(StartWord, bmpHeaderos2Pointer))) { // Checking
																														// the
																														// first
																														// word
		...
		if (compression_type == 0) {
			// Verifying the file size w.r.t. image dimensions(width and height), bitDepth
			// with imagedatasize(including padding).
