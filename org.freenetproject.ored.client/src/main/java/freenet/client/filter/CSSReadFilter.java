...
public class CSSReadFilter implements ContentDataFilter, CharsetExtractor {
	...
	// We do check for ascii, even though it's the first one to check for anyway, because
	// of the "as specified" rule: if it starts with @charset in ascii, it MUST have a
	// valid charset, or we ignore the whole sheet, as per the spec.
	...
	static final int maxBOMLength = Math.max(utf16be.length,
			Math.max(utf16le.length, Math.max(utf32_le.length, Math.max(utf32_be.length, Math.max(ebcdic.length,
					Math.max(ibm1026.length, Math.max(utf32_2143.length, Math.max(utf32_3412.length, gsm.length))))))));
