...
public interface CharsetExtractor {
	 ...
	 * Don't try too hard, if we don't find anything we will call getCharset() with some
	 * specific charset families to try.
	 * @param input The data.
	 * @return The BOM-detected charset family, this is essentially a guess which will
	 * have to be fed to getCharset(). (A true BOM would give an exact match, but the
	 * caller will have already tested for true BOMs by this point; we are looking for
	...
	public class BOMDetection {
		...
		/**
		 * If this is true, getCharset() must return a charset, if it does not, we ignore
		 * the whole stylesheet. See CSS 2.1 section 4.4, at the end, "as specified" rule.
		 */
