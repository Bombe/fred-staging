...
public class TheoraPacketFilter implements CodecPacketFilter {
	...
	private void verifyTypeAndHeader(String headerName, BitInputStream input, int expectedHeaderType)
		...
		if (!Arrays.equals(magicNumber, magicHeader)) {
			throw new UnknownContentTypeException("Packet magicHeader: " + Arrays.toString(magicHeader) + "; expected: "
					+ Arrays.toString(magicNumber));
		}
	}

	private int checkHeaderField(String headerName, String fieldName, BitInputStream input, int sizeInBits,
			Predicate<Integer> validator) throws IOException {
	...
	private CodecPacket constructCommentHeaderWithEmptyVendorStringAndComments() {
		// headerType - magicNumber - vendorStringLength (4 bytes, value 0) -
		// userCommentsNumber (4 bytes, value 0)
	...
	private int[] readHuffmanTable(int HBITSLength, int[] HTS, BitInputStream input) throws IOException {
		...
		if (ISLEAF == 1) {
			if (HTS.length == 32) {
				throw new UnknownContentTypeException(
						"HTS[hti] = " + Arrays.toString(HTS) + "; HTS[hti] is already 32 - The stream is undecodable.");
			}
