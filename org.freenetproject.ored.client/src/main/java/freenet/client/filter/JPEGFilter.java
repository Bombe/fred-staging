 ...
 *
 * http://www.obrador.com/essentialjpeg/headerinfo.htm Also the JFIF spec. Also
...
public class JPEGFilter implements ContentDataFilter {
	...
	static final byte[] soi = new byte[] { (byte) 0xFF, (byte) 0xD8 // Start of Image
	};
	static final byte[] identifier = new byte[] { (byte) 'J', (byte) 'F', (byte) 'I', (byte) 'F', 0 };
	static final byte[] extensionIdentifier = new byte[] { (byte) 'J', (byte) 'F', (byte) 'X', (byte) 'X', 0 };
	...
	public void readFilter(InputStream input, OutputStream output, String charset, Map<String, String> otherParams,
			FilterCallback cb, boolean deleteComments, boolean deleteExif) throws DataFilterException, IOException {
		...
		while (!finished) {
			...
			if (forceMarkerType != -1) {
				...
				if (markerStart != 0xFF) {
					throwError("Invalid marker", "The file includes an invalid marker start "
							+ Integer.toHexString(markerStart) + " and cannot be parsed further.");
				}
				if (baos != null)
					baos.write(0xFF);
			...
			if (markerType == 0xDA) {
				...
				// Copy marker
				if (blockLength < 2)
					throwError("Invalid frame length",
				...
				while (true) {
					...
					if (prevChar == 0xFF && x != 0 && !(x >= MARKER_RST0 && x <= MARKER_RST7)) { // reset
																									// markers
																									// can
																									// occur
																									// in
						...
						if (logMINOR)
							Logger.minor(this,
									"Moved scan at " + cis.count() + ", found a marker type " + Integer.toHexString(x));
						if (output != null)
							output.write(x);
				...
				switch (markerType) {
				// descriptions from http://svn.xiph.org/experimental/giles/jpegdump.c
				// (GPL)
				...
				case 0xc7: // differential lossless, huffman
					// DELETE 0xc8 - "reserved for JPEG extension" - likely to be used for
					// Bad Things
				...
				case 0xdf: // expand reference components
					// DELETE APP0 - APP15 - application data sections, likely to be
					// troublesome.
					// DELETE extension data sections JPG0-6,SOF48,LSE,JPG9-JPG13, JCOM
					// (comment!!), TEM ("temporary private use for arithmetic coding")
					// DELETE 0x02 - 0xbf reserved sections.
					// Do not support JPEG2000 at the moment. Probably has different
					// headers. FIXME.
					valid = true;
				}
				if (valid) {
					...
					if (markerType >= 0xE0 && markerType <= 0xEF) {
						...
						if (logMINOR)
							Logger.minor(this, "Dropping application marker type " + Integer.toHexString(markerType)
									+ " length " + blockLength);
					}
					else {
						if (logMINOR)
			...
			if (cis.count() != countAtStart + blockLength)
				throwError("Invalid frame",
						"The length of the frame is incorrect (read " + (cis.count() - countAtStart)
								+ " bytes, frame length " + blockLength + " for type " + Integer.toHexString(markerType)
								+ ").");
			// Write frame
			baos.writeTo(output);
		}
	...
	private void skipRest(int blockLength, long countAtStart, CountedInputStream cis, DataInputStream dis,
			DataOutputStream dos, String thing) throws IOException {
