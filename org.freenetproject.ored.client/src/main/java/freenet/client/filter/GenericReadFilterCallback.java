...
public class GenericReadFilterCallback implements FilterCallback, URIProcessor {
	...
	public String processURI(String u, String overrideType, boolean forBaseHref, boolean inline)
		...
		try {
			...
			if (u.startsWith("/") || u.startsWith("%2f"))
				...
				// Don't allow encoded /'s, they're just too confusing (here they would
				// get decoded and then coalesced with other slashes).
				noRelative = true;
			if (!noRelative)
	...
	private String finishProcess(HTTPRequest req, String overrideType, String path, URI u, boolean noRelative) {
		...
		// At the moment, ?type= and ?force= are the only options supported by FProxy
		// anyway.

		try {
			// URI encoding issues: FreenetURI.toString() does URLEncode'ing of critical
			// components.
			// So if we just pass it in to the component-wise constructor, we end up
			// encoding twice,
			// so get %2520 for a space.

			// However, we want to support encoded slashes or @'s in the path, so we don't
			// want to
			// just decode before feeding it to the constructor. It looks like the best
			// option is
			// to construct it ourselves and then re-parse it. This is doing unnecessary
			// work, it
			// would be much easier if we had a component-wise constructor for URI that
			// didn't
	...
	/**
	 * Process a form. Current strategy: - Both POST and GET forms are allowed to /
	 * Anything that is hazardous should be protected through formPassword.
	 * @throws CommentException If the form element could not be parsed and the user
	 * should be told.
	 */
	@Override
	public String processForm(String method, String action) throws CommentException {
		...
		// Allow access to Library for searching, form passwords are used for actions such
		// as adding bookmarks
