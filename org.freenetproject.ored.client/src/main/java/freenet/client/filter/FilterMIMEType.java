...
/**
 * A MIME type, for purposes of the filter. There is one instance of this class for every
 * MIME type that we have a filter for or that we have some knowledge of. Hence we can
 * either filter the content or generate a localised warning page with specific
 * information such as the name of the format and what the likely threats are.
 *
 * @see freenet.client.MediaType MediaType represents an individual parsed MIME type
 * string, e.g. "text/plain; charset=ISO-8859-1".
 */
public class FilterMIMEType {
	...
	/**
	 * If true, if we cannot detect the charset from a definite declaration or BOM, we
	 * will use the charset passed in from the referring document. So far this is only
	 * used by CSS.
	 */
	public final boolean useMaybeCharset;

	FilterMIMEType(String type, String ext, String[] extraTypes, String[] extraExts, boolean safeToRead,
			boolean safeToWrite, ContentDataFilter readFilter, boolean dangerousLinks, boolean dangerousInlines,
			boolean dangerousScripting, boolean dangerousReadMetadata, boolean dangerousWriteMetadata,
			boolean dangerousToWriteEvenWithFilter, String readDescription, boolean takesACharset,
