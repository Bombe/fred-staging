...
public interface FilterCallback {
	...
	/**
	 * Process a URI. If it cannot be turned into something sufficiently safe, then return
	 * null.
	...
	/**
	 * Process a URI forcing the host. If it cannot be turned into something sufficiently
	 * safe, then return null.
	 * @param overrideType Force the return type.
	 * @param inline inline URIs may be prefetched while filtering
	 * @throws CommentException If the URI is nvalid or unacceptable in some way.
	 */
	...
	/**
	 * Process plain-text. Notification only; can't modify. Type can be null, or can
	 * correspond, for example to HTML tag name around text (for example: "title").
	 *
	 * Note that the string will have been fed through the relevant decoder if necessary
	 * (e.g. HTMLDecoder). It must be re-encoded if it is sent out as text to a browser.
	 */
	...
	/**
	 * Process a tag. If it needs changing, then return the changed HTML, if not, then
	 * return null;
