...
/**
 * Stores the metadata that the client might actually be interested in. Currently this is
 * just the MIME type, but in future it might be more than that. Size is not stored here,
 * but maybe things like dublin core or whatever.
 *
...
public class ClientMetadata implements Cloneable, Serializable {
	...
	/**
	 * Factory method to keep the API cleaner, avoid ambiguity; this won't be used as
	 * often as the String constructor.
	 */
	...
	/**
	 * Get the document MIME type. Will always be a valid MIME type, unless there has been
	 * an error; if it is unknown, will return application/octet-stream.
	 */
	...
	/**
	 * Return the MIME type minus any type parameters (e.g. charset, see the RFCs defining
	 * the MIME type for details).
	 */
