...
/**
 * Event handeling for clients. SimpleEventProducer is a simple ClientEventProducer
 * implementation that can be used for others.
 *
 * @author oskar
 **/
public class SimpleEventProducer implements ClientEventProducer, Serializable {
	 ...
	 *
	 * Please do not change SimpleEventProducer to always produce events off-thread, it is
	 * better to run the client layer method that produces the event off-thread, because
	 * events could be re-ordered, which matters for some events notably
	 * SimpleProgressEvent. See e.g. ClientGetter.innerNotifyClients()),
	 **/
	@Override
