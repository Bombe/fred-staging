...
abstract class ArchiveStoreItem {
	...
	/**
	 * Delete any stored data on disk etc. Override in subtypes for specific cleanup. Will
	 * be called with locks held, so should only do low level operations such as deletes..
	 */
	void innerClose() {
	...
	/**
	 * Return the amount of cache space used by the item. May be called inside locks so
	 * should not take any nontrivial locks or take long.
	 */
	abstract long spaceUsed();

	/**
	 * Get the data as a Bucket, and guarantee that it won't be freed until the returned
	 * object is either finalized or freed.
	 */
