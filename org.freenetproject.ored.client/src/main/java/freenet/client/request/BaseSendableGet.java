...
public abstract class BaseSendableGet extends SendableRequest {
	 ...
	 * Some gets may want to cancel here, some may want to send an event to FCP.
	 * @param toNetwork If true, we are actually going to send requests (unless we cancel
	 * in this callback). If false, we completed all the work assigned.
	 * @return True to cancel the request at this stage i.e. not go to network, in which
	 * case *the BSG must handle the failure itself*.
	 */
