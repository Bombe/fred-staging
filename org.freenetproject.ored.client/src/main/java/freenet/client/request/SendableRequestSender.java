...
public interface SendableRequestSender {
	 ...
	 * callbacks on the ChosenBlock, which will remove the key from KeysFetchingLocally,
	 * or do it directly (in obscure cases such as OfferedKeysList).
	 * @param sched The scheduler this request has just been grabbed from.
	 * @param request The ChosenBlock containing the key, the SendableRequestItem, and the
	 * methods to call to indicate success/failure.
	 * @return True if a request was sent, false otherwise (in which case the request will
	 * be removed if it hasn't already been).
	 */
	public abstract boolean send(NodeClientRequest node, RequestScheduler sched, ClientContext context,
			ChosenBlock request);
