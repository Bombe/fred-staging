 ...
 * deeper explanation.
 *
 * A lot of this is over-complicated and over-expensive because of db4o. A lot of it is
 * O(n). This is all kept in RAM now so we can change it at will, plus there is only one
 * object queued per splitfile now, so memory pressure is much less of an issue. FIXME
...
public class SectoredRandomGrabArray<T, C extends RemoveRandomWithObject<T>>
	...
	private RemoveRandomReturn removeRandomExhaustive(RandomGrabArrayItemExclusionList excluding, ClientContext context,
		...
		synchronized (root) {
			...
			for (int i = 0; i < grabArrays.length; i++) {
				...
				if (logMINOR)
					Logger.minor(this, "RGA has picked " + x + "/" + grabArrays.length + ": " + item + " rga.isEmpty="
							+ rga.isEmpty());
				if (item != null) {
					return new RemoveRandomReturn(item);
				}
	...
	private RandomGrabArrayItem removeRandomLimited(RandomGrabArrayItemExclusionList excluding, ClientContext context,
		...
		synchronized (root) {
			/**
			 * Count of arrays that have items but didn't return anything because of
			 * exclusions
			 */
			...
			while (true) {
				...
				if (rga == null) {
					...
					if (excluded > MAX_EXCLUDED) {
						Logger.normal(this, "Too many sub-arrays are entirely excluded on " + this + " length = "
				...
				if (logMINOR)
					Logger.minor(this, "RGA has picked " + x + "/" + grabArrays.length + ": " + item + " rga.isEmpty="
							+ rga.isEmpty());
				// If it is not empty but returns null we exclude it, and count the
				// exclusion.
	...
	private RemoveRandomReturn removeRandomTwoOnly(RandomGrabArrayItemExclusionList excluding, ClientContext context,
			long now) {
		synchronized (root) {
			long wakeupTime = Long.MAX_VALUE;
			...
			if (item != null) {
				...
				if (firstRGA != null && firstRGA.isEmpty() && rga != null && rga.isEmpty()) {
					if (logMINOR)
						Logger.minor(this,
								"Removing both on " + this + " : " + firstRGA + " and " + rga + " are empty");
					grabArrays = newGrabberArray(0);
					grabClients = newClientArray(0);
				}
					...
					grabArrays = asGrabberArray(grabArrays[x]); // don't use RGA, it may
																// be nulled out
	...
	private RemoveRandomReturn removeRandomOneOnly(RandomGrabArrayItemExclusionList excluding, ClientContext context,
			long now) {
		synchronized (root) {
			long wakeupTime = Long.MAX_VALUE;
