 ...
 *
 * This is *NOT* persistent. The request selection structures are reconstructed on
 * restart. However it used to be, and probably has a lot of cruft and inefficiency as a
 * result.
 *
 * LOCKING: There is a single lock for the entire tree, the ClientRequestSelector. This
 * must be taken before calling any methods on RGA or SRGA. See the javadocs there for
 * deeper explanation.
 *
 * FIXME Simplify and improve performance. A lot of this is O(n), and this should probably
 * be fixed. Memory usage was an issue but probably isn't now given that the individual
 * items are now quite large (entire splitfiles or at least entire segments).
 */
public class RandomGrabArray implements RemoveRandom, RequestSelectionTreeNode {
	...
	/**
	 * Array of items. Non-null's followed by null's. We used to have a Set so we could
	 * check whether something is in the set quickly. We got rid of this because for
	 * persistent requests it is vastly faster to just loop the loop and check ==, and for
	 * non-persistent requests it doesn't matter much.
	 */
	...
	@Override
	public RemoveRandomReturn removeRandom(RandomGrabArrayItemExclusionList excluding, ClientContext context,
			long now) {
		if (logMINOR)
	...
	private RandomGrabArrayItem removeRandomLimited(RandomGrabArrayItemExclusionList excluding, ClientContext context,
			long now) {
		int excluded = 0;
		while (true) {
			...
			do {
				...
				// Check for nulls, but don't check for cancelled, since we'd have to
				// activate.
			}
	...
	private RemoveRandomReturn removeRandomExhaustiveSearch(RandomGrabArrayItemExclusionList excluding,
		...
		while (true) {
			...
			for (int i = 0; i < index; i++) {
				...
				if (item == null) {
					if (logMINOR)
						Logger.minor(this, "Found null item at offset " + offset + " i=" + i + " block = "
								+ blockNumReading + " on " + this);
					continue;
				}
				...
				if (itemWakeTime > 0) {
					...
					// We are doing compaction here. We don't need to swap with the end;
					// we write valid ones to the target location.
			...
			// We reach this point if 1) the random number we picked last round is invalid
			// because an item became cancelled or excluded
			...
			if (valid == 0 && exclude == 0) {
				...
				if (logMINOR)
					Logger.minor(this,
							"Looping to choose valid item " + random + " of " + valid + " (excluded " + exclude + ")");
				// Loop
			}
		}
	}
	...
	/**
	 * Set the wakeup time, and update parents recursively if it is reduced. If it is
	 * increased we don't need to bother parents as they will recompute the next time they
	 * need to. Only called by removeRandomExhaustive() i.e. after checking <b>all</b> our
