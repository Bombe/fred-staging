...
public interface RandomGrabArrayItem extends RequestSelectionTreeNode {
	...
	@Override
	/**
	 * @return -1 if the item is no longer needed and should be removed, because it is
	 * cancelled, is completing with the blocks it has already, etc. 0 if there are
	 ...
	 * Long.MAX_VALUE. In both the latter cases, will add itself to the cooldown cache via
	 * the RandomGrabArrayExclusionList. LOCKING: Should hold as few locks as reasonably
	 * possible, called inside RGA lock.
