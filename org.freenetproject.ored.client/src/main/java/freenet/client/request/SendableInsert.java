...
/**
 * Callback interface for a low level insert, which is immediately sendable. These should
 * be registered on the ClientRequestScheduler when we want to send them. It will then,
 * when it is time to send, create a thread, send the request, and call the callback
 * below.
 */
public abstract class SendableInsert extends SendableRequest implements LowLevelFailureHandler {
	...
	public abstract void onFailure(LowLevelException e, SendableRequestItem keyNum, ClientContext context);
	...
	public void internalError(Throwable t, RequestScheduler sched, ClientContext context, boolean persistent) {
		...
		sched.callFailure(this, new LowLevelPutException(LowLevelPutException.INTERNAL_ERROR, t.getMessage(), t),
