 ...
 * ClientRequestScheduler. LOCKING: Because some subclasses may do wierd things like
 * locking on an external object (see e.g. SplitFileFetcherSubSegment), if we do take the
 * lock we need to do it last i.e. not call any subclass methods inside it.
 *
 * WARNING: Changing non-transient members on classes that are Serializable can result in
 * restarting downloads or losing uploads. Not all subclasses of this class are actually
 * persisted.
 */
public abstract class SendableRequest implements RandomGrabArrayItem, Serializable {
	...
	/**
	 * Since we put these into Set's etc, hashCode must be persistent. Guaranteed not to
	 * be 0 unless this is a persistent object that is deactivated.
	 */
	...
	/**
	 * Choose a key to fetch. Must not modify any persisted structures, but will update
	 * cooldowns etc to avoid it being chosen again soon. There is a separate callback for
	 * when a fetch fails, succeeds, etc. Hence it is safe to call these outside of the
	 * PersistentJobRunner.
	...
	/**
	 * All key identifiers. Including those not currently eligible to be sent because they
	 * are on a cooldown queue, requests for them are in progress, etc.
	 */
	public abstract long countAllKeys(ClientContext context);

	/**
	 * All key identifiers currently eligible to be sent. Does not include those currently
	 * running, on the cooldown queue etc.
	 */
	 ...
	 * be registered any more. isEmpty() on the other hand means there are no queued
	 * blocks.
	 */
	 ...
	 * @param context
	 * @param oldPrio If we are changing priorities it can matter what the old priority
	 * is. However the parent method, SendableRequest, ignores this. In any case,
