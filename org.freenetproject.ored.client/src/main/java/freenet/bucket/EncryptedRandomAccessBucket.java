...
/**
 * A Bucket encrypted using the same format as an EncryptedRandomAccessBuffer, which can
 * therefore be converted easily when needed.
 *
 * @author toad
 */
public class EncryptedRandomAccessBucket implements RandomAccessBucket, Serializable {
	...
	public EncryptedRandomAccessBucket(EncryptedRandomAccessBufferType type, RandomAccessBucket underlying,
	...
	private SkippingStreamCipher setup(InputStream is) throws IOException, GeneralSecurityException {
		...
		byte[] header = Arrays.copyOfRange(fullHeader, fullHeader.length - VERSION_AND_MAGIC_LENGTH, fullHeader.length);
		int offset = 0;
	...
	private void setupKeys() {
		...
		try {
			KeyParameter cipherKey = new KeyParameter(KeyGenUtils.deriveSecretKey(unencryptedBaseKey,
					EncryptedRandomAccessBuffer.class, kdfInput.underlyingKey.input, type.encryptKey).getEncoded());
			tempPram = new ParametersWithIV(cipherKey, KeyGenUtils.deriveIvParameterSpec(unencryptedBaseKey,
					EncryptedRandomAccessBuffer.class, kdfInput.underlyingIV.input, type.encryptKey).getIV());
		}
		catch (InvalidKeyException e) {
	...
	public EncryptedRandomAccessBucket(DataInputStream dis, BucketFilenameGenerator fg,
			PersistentFileTracker persistentFileTracker, MasterSecret masterKey2)
