...
/**
 * Write to a temporary Bucket. On close, if not abort()'ed, write length and then copy
 * the data. Does not close the underlying Bucket.
 */
public class PrependLengthOutputStream extends FilterOutputStream {
	...
	/**
	 * Create a stream which writes to temporary space and then on a non-aborted close()
	 * will write the length (minus the offset) followed by the data.
	 */
