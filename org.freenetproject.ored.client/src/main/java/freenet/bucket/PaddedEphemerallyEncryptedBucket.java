...
/**
 * A proxy Bucket which adds: - Encryption with the supplied cipher, and a random,
 * ephemeral key. - Padding to the next PO2 size.
 *
 * CRYPTO WARNING: This uses PCFB with no IV. That means it is only safe if the key is
 * unique!
 */
public class PaddedEphemerallyEncryptedBucket implements Bucket, Serializable {
	 ...
	 * @param weakPRNG a week prng we will padd from. Serialization: Note that it is not
	 * our responsibility to free the random number generators, but we WILL free the
	 * underlying bucket.
	...
	public PCFBMode getPCFB() {
		...
		if (iv != null)
			...
			// FIXME CRYPTO We should probably migrate all old buckets automatically so we
			// can get rid of this?
			// Since the key is unique it is actually almost safe to use all zeros IV, but
			// it's better to use a real IV.
	...
	protected PaddedEphemerallyEncryptedBucket(DataInputStream dis, BucketFilenameGenerator fg,
