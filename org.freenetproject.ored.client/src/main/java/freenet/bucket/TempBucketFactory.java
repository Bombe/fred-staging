 ...
 *
 * Buckets created by this factory can be either: - ArrayBuckets OR - FileBuckets
 *
 * ArrayBuckets are used if and only if: 1) there is enough room remaining on the pool
 * (@see maxRamUsed and @see bytesInUse) 2) the initial size is smaller than
 * (@maxRAMBucketSize)
 *
 * Depending on how they are used they might switch from one type to another
 * transparently.
 *
 * Currently they are two factors considered for a migration: - if they are long-lived or
 * not (@see RAMBUCKET_MAX_AGE) - if their size is over
 * RAMBUCKET_CONVERSION_FACTOR*maxRAMBucketSize
 */
public class TempBucketFactory implements BucketFactory, LockableRandomAccessBufferFactory {
	...
	private final BucketFilenameGenerator filenameGenerator;
	...
	public class TempBucket implements Bucket, Migratable, RandomAccessBucket {
		...
		/**
		 * We have to account the size of the underlying bucket ourself in order to be
		 * able to access it fast
		 */
		...
		/**
		 * A timestamp used to evaluate the age of the bucket and maybe consider it for a
		 * migration
		 */
		...
		public synchronized OutputStream getOutputStreamUnbuffered() throws IOException {
			...
			// Hence we don't need to reset currentSize / _hasTaken() if a bucket is
			// reused.
		...
		private class TempBucketOutputStream extends OutputStream {
				...
				if (isRAMBucket()) {
					...
					if (shouldMigrate) {
						if (logMINOR) {
							if (isOversized)
								Logger.minor(this,
										"The bucket " + TempBucket.this + " is over "
			...
			public final void write(int b) throws IOException {
				synchronized (TempBucket.this) {
					...
					if (isRAMBucket()) // We need to re-check because it might have
										// changed!
						_hasTaken(1);
				}
			}
		...
		private class TempBucketInputStream extends InputStream {
			...
			/**
			 * Keep a counter to know where we are on the stream (useful when we have to
			 * reset and skip)
			 */
		...
		protected void finalize() throws Throwable {
			...
			if (!hasBeenFreed) {
				if (TRACE_BUCKET_LEAKS)
					Logger.error(this,
							"TempBucket not freed, size=" + size() + ", isRAMBucket=" + isRAMBucket() + " : " + this,
							tracer);
				else
					Logger.error(this,
							"TempBucket not freed, size=" + size() + ", isRAMBucket=" + isRAMBucket() + " : " + this);
	...
	public TempBucketFactory(Executor executor, BucketFilenameGenerator filenameGenerator, long maxBucketSizeKeptInRam,
			long maxRamUsed, Random weakPRNG, boolean reallyEncrypt, long minDiskSpace, MasterSecret masterSecret) {
	 ...
	 * @exception IOException If it is not possible to create a temp bucket due to an I/O
	 * error
	 */
	public TempBucket makeBucket(long size, float factor, long increment) throws IOException {
		...
		if (useRAMBucket) { // No need to consider them for migration if they can't be
							// migrated
			synchronized (ramBucketQueue) {
	 ...
	 * @param force If true, migrate one bucket which isn't necessarily long lived, just
	 * to free up space. Otherwise we will migrate all long-lived buckets but not any
	 * others.
	 * @return True if we migrated any buckets.
	 * @throws InsufficientDiskSpaceException If there is not enough space to migrate
	 * buckets to disk.
	 */
	...
	/**
	 * Unlike a TempBucket, the size is fixed, so migrate only happens on the migration
	 * thread.
	 */
	class TempRandomAccessBuffer extends SwitchableProxyRandomAccessBuffer implements Migratable {
		 ...
		 * TempRandomAccessBuffer *and* the TempBucket are no longer reachable, in which
		 * case we will free from the TempBucket. If this is null, then the TempRAB can
		 * free in finalizer.
		 */
		...
		public TempRandomAccessBuffer(byte[] initialContents, int offset, int size, long time, boolean readOnly)
				throws IOException {
		...
		public TempRandomAccessBuffer(LockableRandomAccessBuffer underlying, long creationTime, boolean migrated,
				TempBucket tempBucket) throws IOException {
