 ...
 * temporary files in the directory specified for the PersistentFileTracker (which
 * supports changing the directory, i.e. moving the files).
 *
 * These temporary files are encrypted using an ephemeral key (unless the node is
 * configured not to encrypt temporary files as happens with physical security level LOW).
 * FIXME NO CRYPTO AT THE MOMENT.
 *
 * Note that the files are only deleted *after* the transaction containing their deletion
 * reaches disk - so we should not leak temporary files, or forget that we deleted a
 * bucket and try to reuse it, if there is an unclean shutdown.
 *
 * PERSISTENCE: This class is involved in persistence but is not itself Serializable; it
 * is recreated on every startup, and persistent Bucket's register themselves with it.
 */
public class PersistentTempBucketFactory implements BucketFactory, PersistentFileTracker {

	/**
	 * Original contents of directory. This used to be used to delete any files that we
	 * can't account for. However at the moment we do not support garbage collection for
	 * non-blob persistent temp files. When we implement it it will probably not use this
	 * structure... FIXME!
	 */
	private HashSet<File> originalFiles;

	/**
	 * Filename generator. Tracks the directory and the prefix for temp files, can move
	 * them if these change, generates filenames.
	 */
	public final BucketFilenameGenerator fg;
	 ...
	 * @param strongPRNG Cryptographically strong random number generator, for making keys
	 * etc.
	 ...
	 */
	public PersistentTempBucketFactory(File dir, final String prefix, RandomSource strongPRNG, Random weakPRNG,
			boolean encrypt) throws IOException {
		this.strongPRNG = strongPRNG;
		this.weakPRNG = weakPRNG;
		this.encrypt = encrypt;
		this.fg = new BucketFilenameGenerator(weakPRNG, false, dir, prefix);
		if (!dir.exists()) {
			dir.mkdir();
			if (!dir.exists()) {
	 ...
	 * FIXME this is not currently used. @see #completedInit()
	 */
	@Override
	 ...
	 * DelayedFreeBucket so that they will not be deleted until after the transaction
	 * deleting them in the database commits.
	 */
	@Override
	public RandomAccessBucket makeBucket(long size) throws IOException {
		...
		synchronized (encryptLock) {
			if (encrypt) {
				...
				rawBucket = new EncryptedRandomAccessBucket(TempBucketFactory.CRYPT_TYPE, rawBucket, secret);
			}
		}
		if (mustWrap)
	...
	/**
	 * Returns a list of buckets to free. The caller should write the buckets to the
	 * checkpoint, and free them after the checkpoint has written successfully, by calling
	 * postCommit().
	 */
	...
	/**
	 * Set whether to encrypt new persistent temp buckets. Note that we do not
	 * encrypt/decrypt old ones when this changes.
	 */
