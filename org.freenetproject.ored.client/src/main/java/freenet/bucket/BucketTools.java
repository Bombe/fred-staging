...
public class BucketTools {
	...
	/**
	 * Read the entire bucket in as a byte array. Not a good idea unless it is very small!
	 * Don't call if concurrent writes may be happening.
	 * @throws IOException If there was an error reading from the bucket.
	 * @throws OutOfMemoryError If it was not possible to allocate enough memory to
	 * contain the entire bucket.
	 */
	...
	public static RandomAccessBucket makeImmutableBucket(BucketFactory bucketFactory, byte[] data, int length)
			throws IOException {
		return makeImmutableBucket(bucketFactory, data, 0, length);
	}

	public static RandomAccessBucket makeImmutableBucket(BucketFactory bucketFactory, byte[] data, int offset,
	 ...
	 * @throws IOException If there was an error reading from the bucket or writing to the
	 * stream.
	 */
	 ...
	 *
	 * If the passed-in Bucket is a FileBucket, will be efficiently split into
	 * ReadOnlyFileSliceBuckets, otherwise new buckets are created and the data written to
	 * them.
	 *
	 * Note that this method will allocate a buffer of size splitSize.
	 * @param bf
	 * @param freeData
	 * @param persistent If true, the data is persistent. This method is responsible for
	 * ensuring that the returned buckets HAVE ALREADY BEEN STORED TO THE DATABASE, using
	 * the provided handle. The point? SegmentedBCB's buckets have already been stored!!
	 * @throws IOException If there is an error creating buckets, reading from the
	 * provided bucket, or writing to created buckets.
	 */
	public static Bucket[] split(Bucket origData, int splitSize, BucketFactory bf, boolean freeData, boolean persistent)
			throws IOException {
	 ...
	 * @param truncateLength The maximum number of bytes to transfer, or -1 to copy the
	 * whole bucket.
	 * @return The number of bytes moved.
	 * @throws IOException If something breaks while copying the data.
	 */
	public static long copyTo(Bucket bucket, RandomAccessBuffer raf, long fileOffset, long truncateLength)
			throws IOException {
		if (truncateLength == 0)
			return 0;
		if (truncateLength < 0)
	 ...
	 */
	public static Bucket restoreFrom(DataInputStream dis, BucketFilenameGenerator fg,
	 ...
	 * FIXME Maybe we should just pass the ClientContext?
	 */
	public static LockableRandomAccessBuffer restoreRAFFrom(DataInputStream dis, BucketFilenameGenerator fg,
