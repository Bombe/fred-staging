 ...
 * Warning: Avoid using Closer.close() on InputStream's opened on this Bucket. The MAC is
 * only checked when the end of the bucket is reached, which may be in read() or may be in
 * close().
 *
 * @author toad
 */
public class AEADCryptBucket implements Bucket, Serializable {
	...
	public OutputStream getOutputStreamUnbuffered() throws IOException {
		...
		return AEADOutputStream.createAES(os, key, GlobalSecureRandom.getGlobalSecureRandom());
	}
	...
	public AEADCryptBucket(DataInputStream dis, BucketFilenameGenerator fg, PersistentFileTracker persistentFileTracker,
		...
		if (keyLength < 0 || !(keyLength == 16 || keyLength == 24 || keyLength == 32))
			throw new StorageFormatException("Unknown key length " + keyLength); // FIXME
																					// validate
																					// this
																					// in
																					// a
