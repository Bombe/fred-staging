...
public class AEADStreamsTest extends TestCase {
	...
	public void checkSuccessfulRoundTrip(int keysize, Random random, Bucket input, Bucket output, Bucket decoded)
			throws IOException {
	...
	public void checkFailedCorruptedRoundTrip(int keysize, Random random, Bucket input, Bucket output, Bucket decoded)
			throws IOException {
		byte[] key = new byte[keysize];
		random.nextBytes(key);
		OutputStream os = output.getOutputStream();
	...
	public void checkSuccessfulRoundTripRandomSplits(int keysize, Random random, Bucket input, Bucket output,
	...
	/**
	 * If we close the stream early but there is garbage after that point, it should throw
	 * on close().
	 * @throws IOException
	 */
