...
public class CSSParserTest extends TestCase {
	...
	static {
		...
		// If there was scripting it would not be safe, although datastore probing is
		...
		// CONFORMANCE: :first-line can only be attached to block-level, we don't enforce
		// this, it is not dangerous.
		...
		// CONFORMANCE: We combine pseudo-classes and pseudo-elements, so we allow
		// pseudo-elements on earlier selectors. This is against the spec, CSS2 section
		// 5.10.
	}
	...
	static {
		...
		// FIXME one day we should escape the ] to make this both valid and easy to parse,
		// rather than dropping it.
		CSS2_BAD_SELECTOR.add("h1[foo=\"bar]\"] {}");
		CSS2_BAD_SELECTOR.add("h1[foo=bar\\]] {}");
		// Closing an escape with \r\n. This is supported by verifying and splitting
		// logic, but not by the tokeniser.
		// FIXME fix this.
	...
	static {
		...
		propertyTests.put(
				"td { background: scroll transparent url(\"something.png\") right top repeat url(\"something-else.png\") }\n",
				"td { }\n");
		...
		propertyTests.put("td { background-color: rgb(100%, 0%, 50%)}", "td { background-color: rgb(100%, 0%, 50%)}");
		// Values outside the standard RGB device gamut are allowed by the spec, they will
		// be clipped and may be representible on some devices.
		...
		// Url with a space
		propertyTests.put(
				"h3 { background-image: url(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test page\") }",
				"h3 { background-image: url(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test%20page\") }");
		...
		propertyTests.put(
				...
				"h3 { BACKGROUND: URL(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test%20page\") }");
		propertyTests.put(
				"h3 { BACKGROUND: URL(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test page\") }",
				"h3 { BACKGROUND: url(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test%20page\") }");
		...
		// From Activelink Index. This is invalid but browsers will probably change sans
		// to sans-serif; and we will allow it as it might be a generic font family.
		...
		// Auto-close of style sheet. NOT IMPLEMENTED! The first test tests that we handle
		// unclosed sheet sanely, the second is commented out but would test closing it
		// and parsing it.
		propertyTests.put("@media screen {\n  p:before { content: 'Hello", "@media screen {\n  p:before {}} ");
		// propertyTests.put("@media screen {\n p:before { content: 'Hello", "@media
		...
		propertyTests.put(
				...
				"table { border: outset 10pt; border-collapse: separate; border-spacing: 15pt } td { border: inset 5pt } td.special { border: inset 10pt }");
		// Checking properties against elements is invalid because of inheritance.
		// empty-cells applies to td, th only, but if set on table it is inherited, this
		// example taken from CSS2.1 spec.
	...
	public void testCSS1Selector() throws IOException, URISyntaxException {
		...
		assertTrue(
				...
				CSS_DELETE_INVALID_SELECTORC.equals(filter(CSS_DELETE_INVALID_SELECTOR)));
		assertTrue("key=\"" + CSS_INVALID_MEDIA_CASCADE + "\" value=\"" + filter(CSS_INVALID_MEDIA_CASCADE) + "\"",
				"".equals(filter(CSS_INVALID_MEDIA_CASCADE)));
	}
	...
	public void testNewlines() throws IOException, URISyntaxException {
		assertTrue("key=\"" + CSS_STRING_NEWLINES + "\" value=\"" + filter(CSS_STRING_NEWLINES) + "\" should be: \""
				+ CSS_STRING_NEWLINESC + "\"", CSS_STRING_NEWLINESC.equals(filter(CSS_STRING_NEWLINES)));
	}

	public void testBackgroundURL() throws IOException, URISyntaxException {
		assertTrue("key=" + CSS_BACKGROUND_URL + " value=\"" + filter(CSS_BACKGROUND_URL) + "\" should be \""
				+ CSS_BACKGROUND_URLC + "\"", CSS_BACKGROUND_URLC.equals(filter(CSS_BACKGROUND_URL)));

		assertTrue("key=" + CSS_LCASE_BACKGROUND_URL + " value=\"" + filter(CSS_LCASE_BACKGROUND_URL) + "\"",
				CSS_LCASE_BACKGROUND_URLC.equals(filter(CSS_LCASE_BACKGROUND_URL)));
	}

	public void testImports() throws IOException, URISyntaxException {
		assertTrue("key=" + CSS_IMPORT + " value=\"" + filter(CSS_IMPORT) + "\"",
				CSS_IMPORTC.equals(filter(CSS_IMPORT)));
		assertTrue("key=" + CSS_IMPORT2 + " value=\"" + filter(CSS_IMPORT2) + "\"",
				CSS_IMPORT2C.equals(filter(CSS_IMPORT2)));
		assertTrue("key=" + CSS_IMPORT_MULTI_MEDIA + " value=\"" + filter(CSS_IMPORT_MULTI_MEDIA) + "\"",
				CSS_IMPORT_MULTI_MEDIAC.equals(filter(CSS_IMPORT_MULTI_MEDIA)));
		assertTrue("key=" + CSS_IMPORT_MULTI_MEDIA_BOGUS + " value=\"" + filter(CSS_IMPORT_MULTI_MEDIA_BOGUS) + "\"",
				CSS_IMPORT_MULTI_MEDIA_BOGUSC.equals(filter(CSS_IMPORT_MULTI_MEDIA_BOGUS)));
		assertTrue("key=" + CSS_IMPORT_MULTI_MEDIA_ALL + " value=\"" + filter(CSS_IMPORT_MULTI_MEDIA_ALL) + "\"",
				CSS_IMPORT_MULTI_MEDIA_ALLC.equals(filter(CSS_IMPORT_MULTI_MEDIA_ALL)));
		assertTrue("key=" + CSS_IMPORT_TYPE + " value=\"" + filter(CSS_IMPORT_TYPE) + "\"",
				CSS_IMPORT_TYPEC.equals(filter(CSS_IMPORT_TYPE)));
		assertTrue("key=" + CSS_IMPORT_SPACE_IN_STRING + " value=\"" + filter(CSS_IMPORT_SPACE_IN_STRING) + "\"",
				CSS_IMPORT_SPACE_IN_STRINGC.equals(filter(CSS_IMPORT_SPACE_IN_STRING)));
		assertTrue("key=" + CSS_IMPORT_QUOTED_STUFF + " value=\"" + filter(CSS_IMPORT_QUOTED_STUFF) + "\"",
				CSS_IMPORT_QUOTED_STUFFC.equals(filter(CSS_IMPORT_QUOTED_STUFF)));
		assertTrue("key=" + CSS_IMPORT_QUOTED_STUFF2 + " value=\"" + filter(CSS_IMPORT_QUOTED_STUFF2) + "\"",
				CSS_IMPORT_QUOTED_STUFF2C.equals(filter(CSS_IMPORT_QUOTED_STUFF2)));
		assertTrue("key=" + CSS_IMPORT_NOURL_TWOMEDIAS + " value=\"" + filter(CSS_IMPORT_NOURL_TWOMEDIAS) + "\"",
		...
		assertTrue("key=" + CSS_IMPORT_NOURL + " value=\"" + filter(CSS_IMPORT_NOURL) + "\"",
				CSS_IMPORT_NOURLC.equals(filter(CSS_IMPORT_NOURL)));
		assertTrue("key=" + CSS_IMPORT_BRACKET + " value=\"" + filter(CSS_IMPORT_BRACKET) + "\"",
				CSS_IMPORT_BRACKETC.equals(filter(CSS_IMPORT_BRACKET)));
		assertTrue("key=" + CSS_LATE_IMPORT + " value=\"" + filter(CSS_LATE_IMPORT) + "\"",
				CSS_LATE_IMPORTC.equals(filter(CSS_LATE_IMPORT)));
		assertTrue("key=" + CSS_LATE_IMPORT2 + " value=\"" + filter(CSS_LATE_IMPORT2) + "\"",
				CSS_LATE_IMPORT2C.equals(filter(CSS_LATE_IMPORT2)));
		assertTrue("key=" + CSS_LATE_IMPORT3 + " value=\"" + filter(CSS_LATE_IMPORT3) + "\"",
				CSS_LATE_IMPORT3C.equals(filter(CSS_LATE_IMPORT3)));
		assertTrue("key=" + CSS_BOGUS_AT_RULE + " value=\"" + filter(CSS_BOGUS_AT_RULE) + "\"",
				CSS_BOGUS_AT_RULEC.equals(filter(CSS_BOGUS_AT_RULE)));
		assertTrue("key=" + PRESERVE_CDO_CDC + " value=\"" + filter(PRESERVE_CDO_CDC) + "\"",
				PRESERVE_CDO_CDCC.equals(filter(PRESERVE_CDO_CDC)));
		assertTrue("key=" + BROKEN_BEFORE_IMPORT + " value=\"" + filter(BROKEN_BEFORE_IMPORT) + "\"",
				BROKEN_BEFORE_IMPORTC.equals(filter(BROKEN_BEFORE_IMPORT)));
		assertTrue("key=" + BROKEN_BEFORE_MEDIA + " value=\"" + filter(BROKEN_BEFORE_MEDIA) + "\"",
				BROKEN_BEFORE_MEDIAC.equals(filter(BROKEN_BEFORE_MEDIA)));
	}

	public void testEscape() throws IOException, URISyntaxException {
		assertTrue("key=" + CSS_ESCAPED_LINK + " value=\"" + filter(CSS_ESCAPED_LINK) + "\"",
				CSS_ESCAPED_LINKC.equals(filter(CSS_ESCAPED_LINK)));
		assertTrue("key=" + CSS_ESCAPED_LINK2 + " value=\"" + filter(CSS_ESCAPED_LINK2) + "\"",
				CSS_ESCAPED_LINK2C.equals(filter(CSS_ESCAPED_LINK2)));
	}
	...
	public void testProperties() throws IOException, URISyntaxException {
		for (Entry<String, String> entry : propertyTests.entrySet()) {
			...
			assertTrue("key=\"" + key + "\" encoded=\"" + filter(key) + "\" should be \"" + value + "\"",
	...
	private String filter(String css) throws IOException, URISyntaxException {
		...
		GenericReadFilterCallback cb = new GenericReadFilterCallback(new URI(
	...
	public void testCharset() throws IOException, URISyntaxException {
		...
		// IBM 1141-1144, 1147, 1149 do not use the same EBCDIC codes for the basic
		// english alphabet.
		...
		assertTrue("key=" + LATE_CHARSET + " value=\"" + filter(LATE_CHARSET) + "\"",
				LATE_CHARSETC.equals(filter(LATE_CHARSET)));
		try {
			String output = filter(WRONG_CHARSET);
			assertFalse(
					"Should complain that detected charset differs from real charset, but returned \"" + output + "\"",
					true);
		}
		catch (IOException e) {
			// Ok.
	...
	private void getCharsetTest(String charset, String family)
			throws DataFilterException, IOException, URISyntaxException {
		...
		assertTrue(
				"Charset detected \"" + detectedCharset + "\" should be \"" + charset + "\" or \"" + family
						+ "\" from getCharsetByBOM",
				detectedCharset == null || charset.equalsIgnoreCase(detectedCharset)
		...
		assertTrue(
				"Charset detected \"" + detectedCharset + "\" should be \"" + charset
						+ "\" from ContentFilter.detectCharset bom=\"" + bomCharset + "\"",
				charset.equalsIgnoreCase(detectedCharset));
		FilterStatus filterStatus = ContentFilter.filter(inputStream, outputStream, "text/css", new URI(
		...
		assertTrue("ContentFilter.filter() returns \"" + filtered + "\" not original \"" + original
				+ "\" for charset \"" + charset + "\"", original.equals(filtered));
	}
	...
	private void charsetTestUnsupported(String charset) throws DataFilterException, IOException, URISyntaxException {
		...
		BOMDetection bom = filter.getCharsetByBOM(bytes, bytes.length);
		String bomCharset = detectedCharset = bom == null ? null : bom.charset;
		assertTrue("Charset detected \"" + detectedCharset + "\" should be unknown testing unsupported charset \""
				+ charset + "\" from getCharsetByBOM", detectedCharset == null);
		detectedCharset = ContentFilter.detectCharset(bytes, bytes.length, cssMIMEType, null);
		assertTrue(
				"Charset detected \"" + detectedCharset + "\" should be unknown testing unsupported charset \""
						+ charset + "\" from ContentFilter.detectCharset bom=\"" + bomCharset + "\"",
				charset == null || "utf-8".equalsIgnoreCase(detectedCharset));
		try {
			FilterStatus filterStatus = ContentFilter.filter(inputStream, outputStream, "text/css", new URI(
					"/CHK@OR904t6ylZOwoobMJRmSn7HsPGefHSP7zAjoLyenSPw,x2EzszO4Kqot8akqmKYXJbkD-fSj6noOVGB-K2YisZ4,AAIC--8/1-works.html"),
					null, null, null, null);
			// It is safe to return utf-8, as long as we clobber the actual content; utf-8
			// is the default, but other stuff decoded to it is unlikely to be coherent...
			assertTrue(
					"ContentFilter.filter() returned charset \"" + filterStatus.charset
							+ "\" should be unknown testing  unsupported charset \"" + charset + "\"",
					filterStatus.charset.equalsIgnoreCase(charset) || filterStatus.charset.equalsIgnoreCase("utf-8"));// If
																														// we
																														// switch
																														// to
																														// JUnit
			...
			assertEquals("text/css", filterStatus.mimeType);
			String filtered = new String(BucketTools.toByteArray(outputBucket), charset);
			assertTrue("ContentFilter.filter() returns something: \"" + filtered
					+ "\" should be empty as unsupported charset, original: \"" + original + "\" for charset \""
					+ charset + "\"", filtered.equals(""));
		}
		catch (UnsupportedCharsetInFilterException e) {
			// Ok.
		}
	...
	private void testUseMaybeCharset(String charset)
		...
		String filtered = new String(BucketTools.toByteArray(outputBucket), charset);
		assertTrue("ContentFilter.filter() returns \"" + filtered + "\" not original \"" + original
				+ "\" with maybeCharset \"" + charset + "\"", original.equals(filtered));
	}
	...
	public void testWhitespace() throws IOException, URISyntaxException {
		assertTrue("value=\"" + filter(CSS_COMMA_WHITESPACE) + "\"",
				CSS_COMMA_WHITESPACE.equals(filter(CSS_COMMA_WHITESPACE)));
	}
