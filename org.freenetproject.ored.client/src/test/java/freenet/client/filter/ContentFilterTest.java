...
public class ContentFilterTest extends TestCase {
	...
	private static final String ANCHOR_TEST_SPECIAL = "<a href=\"#!$()*+,;=:@ABC0123-._~xyz%3f\" />"; // RFC3986
																										// /
																										// RFC
																										// 2396
	...
	private static final String ANCHOR_FALSE_POS1 = "<a href=\"/KSK@test/path/test.html#%23\">"; // yes,
																									// this
																									// is
																									// valid

	private static final String ANCHOR_FALSE_POS2 = "<a href=\"/KSK@test/path/%23.html#2\">"; // yes,
																								// this
																								// is
																								// valid
																								// too
	...
	// @see bug #2921
	private static final String PREVENT_EXTERNAL_ACCESS_CSS_SIMPLE = "<style>div { background: url(" + BASE_URI
			+ ") }</style>";

	private static final String PREVENT_EXTERNAL_ACCESS_CSS_CASE = "<style>div { background: uRl(" + BASE_URI
			+ ") }</style>";

	private static final String PREVENT_EXTERNAL_ACCESS_CSS_ESCAPE = "<style>div { background: \\u\\r\\l(" + BASE_URI
			+ ") }</style>";
	...
	public void testHTMLFilter() throws Exception {
		...
		assertTrue(HTMLFilter(EXTERNAL_LINK_CHECK2).contains(Toadlet.ExternalLink.PATH));
		...
		// bug #2297
		assertTrue(HTMLFilter(PREVENT_FPROXY_ACCESS).contains(Toadlet.ExternalLink.PATH));
		// bug #2921
	...
	public void testMetaRefresh() throws Exception {
		...
		assertEquals(META_TIME_ONLY_BADNUM_OUT + HTMLFilter.m3uPlayerScriptTagContent(),
				headFilter(META_TIME_ONLY_BADNUM1));
		assertEquals(META_TIME_ONLY_BADNUM_OUT + HTMLFilter.m3uPlayerScriptTagContent(),
				headFilter(META_TIME_ONLY_BADNUM2));
		assertEquals(META_VALID_REDIRECT + HTMLFilter.m3uPlayerScriptTagContent(), headFilter(META_VALID_REDIRECT));
		assertEquals(META_VALID_REDIRECT + HTMLFilter.m3uPlayerScriptTagContent(),
				headFilter(META_VALID_REDIRECT_NOSPACE));
		assertEquals(META_BOGUS_REDIRECT1_OUT + HTMLFilter.m3uPlayerScriptTagContent(),
				headFilter(META_BOGUS_REDIRECT1));
		assertEquals(META_BOGUS_REDIRECT2_OUT + HTMLFilter.m3uPlayerScriptTagContent(),
				headFilter(META_BOGUS_REDIRECT2));
		assertEquals(META_BOGUS_REDIRECT3_OUT + HTMLFilter.m3uPlayerScriptTagContent(),
				headFilter(META_BOGUS_REDIRECT3));
		assertEquals(META_BOGUS_REDIRECT4_OUT + HTMLFilter.m3uPlayerScriptTagContent(),
				headFilter(META_BOGUS_REDIRECT4));
		assertEquals(META_BOGUS_REDIRECT5_OUT + HTMLFilter.m3uPlayerScriptTagContent(),
				headFilter(META_BOGUS_REDIRECT5));
		assertEquals(META_BOGUS_REDIRECT_NO_URL + HTMLFilter.m3uPlayerScriptTagContent(),
				headFilter(META_BOGUS_REDIRECT6));
	...
	public void testEvilCharset() throws IOException {
		...
		try {
			ArrayBucket out = new ArrayBucket();
			FilterStatus fo = ContentFilter.filter(new ArrayBucket(total).getInputStream(), out.getOutputStream(),
					"text/html", null, null, null);
			fos = new FileOutputStream("output.filtered");
			fos.write(out.toByteArray());
			fos.close();
			failed = true;
			assertFalse("Filter accepted dangerous UTF8 text with BOM as UTF16! (ContentFilter) - Detected charset: "
					+ fo.charset, true);
		}
		catch (DataFilterException e) {
			System.out.println("Failure: " + e);
	...
	static public class TagVerifierTest extends TestCase {
		...
		public void testHTMLTagWithInvalidNS() throws DataFilterException {
			...
			assertEquals("HTML tag containing an invalid xmlns", HTML_INVALID_XMLNS,
		...
		public void testMetaTagHTMLContentType() throws DataFilterException {
			...
			assertEquals("Meta tag describing HTML content-type", HTMLTag.toString(),
					verifier.sanitize(HTMLTag, pc).toString());
		}
