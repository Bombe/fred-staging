...
public class GIFFilterTest extends TestCase {
	...
	private static final String RESOURCE_PATH = "/filter/gif/";
	...
	/**
	 * Asserts that the test file in the first argument, after passing through the content
	 * filter, is equal to the reference file in the second argument.
	...
	private static void assertEqualAfterFilter(String fileUnfiltered, String fileExpected) throws IOException {
		...
		assertTrue("Filtered and expected output are not identical. " + "Input = " + fileUnfiltered + ", expected = "
				+ fileExpected, equalBuckets(filtered, expected));
	}
	...
	private static Bucket filterGIF(Bucket input) throws IOException {
		...
		try (InputStream inStream = input.getInputStream(); OutputStream outStream = output.getOutputStream()) {
