...
public abstract class RandomAccessBufferTestBase extends TestCase {
	...
	/**
	 * Test that we can create and free a RandomAccessBuffer of various sizes, and it
	...
	/**
	 * Write using a given formula in random small writes, then check using random small
	 * reads.
	 */
	...
	private void innerTestWriteOverLimit(long sz, int choppedBytes) throws IOException {
		...
		if (sz != 0 && choppedBytes < sz) {
			if (startAt >= 0)
				readWriteMustSucceed(raf, startAt, buf, 0, buf.length); // Read, write up
																		// to the end
																		// work.
			else
				try {
					readWriteMustSucceed(raf, startAt, buf, 0, buf.length); // Read, write
																			// up to the
																			// end work.
		...
		if (startAt + 1 >= 0)
			readWriteMustFail(raf, startAt + 1, buf, 0, buf.length); // Read, write over
																		// the end fail.
		else
			try {
				readWriteMustSucceed(raf, startAt + 1, buf, 0, buf.length); // Read, write
																			// up to the
																			// end work.
				fail("Should fail to read at negative index");
			}
	...
	private void readWriteMustSucceed(RandomAccessBuffer raf, long startAt, byte[] buf, int offset, int length)
			throws IOException {
		raf.pread(startAt, buf, 0, buf.length); // Should work
		raf.pwrite(startAt, buf, 0, buf.length); // Should work
	}

	private void readWriteMustFail(RandomAccessBuffer raf, long startAt, byte[] buf, int offset, int length)
			throws IOException {
		if (length == 0)
			return; // NOP.
		try {
	...
	public void testClose() throws IOException {
		...
		// Implementations should add their own tests according to known thresholds (white
		// box).
		for (long size : fullSizeList)
			innerTestClose(size);
	}
	...
	/**
	 * Check that the array section equals the read data, then write it and repeat the
	 * check.
	 */
	public static void checkArraySectionEqualsReadData(byte[] buf, RandomAccessBuffer raf, int start, int end,
