...
public class PrioritizedTicker implements Ticker, Runnable {
	 ...
	 * @param runner The job to run. FastRunnable's get run directly on the PacketSender
	 * thread.
	 * @param name The name of the job, the thread running it will temporarily take this
	 * name, assuming it is run on a separate thread.
	 * @param offset The time at which to run the job in milliseconds after
	 * System.currentTimeMillis().
	 * @param runOnTickerAnyway If false, run jobs with offset <=0 on the ticker, to
	 * preserve their thread priorities; if true, jobs to run immediately through the
	 * executor (which normally will also preserve thread priorities, but may need to call
	 * back via runOnTickerAnyway=true if it needs to increase the thread priority).
	 * @param noDupes Don't run this job if it is already scheduled. Relatively expensive,
	 * O(n) with queued jobs. Necessary for Announcer to ensure that we don't get
	 * exponentially increasing numbers of announcement check jobs queued, while ensuring
	 * that we do always have one queued within the given period.
	 */
	@Override
	...
	/**
	 * Queue a job at a specific time (absolute time in milliseconds). If the time given
	 * has passed already, then run the job ASAP.
	 * @param time The time at which to run the job. @see System.currentTimeMillis()
	 */
	@Override
	public void queueTimedJobAbsolute(Runnable runner, String name, long time, boolean runOnTickerAnyway,
			boolean noDupes) {
		long now = System.currentTimeMillis();
	 ...
	 * System.currentTimeMillis()).
	 */
	private void queueTimedJobInner(Runnable runner, String name, long runJobAt, long offset, boolean runOnTickerAnyway,
			boolean noDupes) {
		if (noDupes)
	 ...
	 * Remove a queued job.
	 *
	 * @param runnable The job to remove. If this is currently queued, it will be removed.
	 * The Ticker should not throw if the job is not queued.
	 */
	 ...
	 * caller must check that it is present in timedJobsQueued, remove from that
	 * structure, and call this method, all inside the timedJobsByTime lock.
