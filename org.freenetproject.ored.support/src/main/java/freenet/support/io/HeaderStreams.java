...
final public class HeaderStreams {
	...
	public static InputStream augInput(final byte[] hd, InputStream s) throws IOException {
		return new FilterInputStream(s) {
			...
			public int read(byte[] buf, int off, int len) throws IOException {
				...
				// System.out.println("" + System.identityHashCode(this) + " || augInput
				// read: " + Arrays.toString(buf) + " off " + off + " len " + len);
			}
	...
	/**
	 ** Create an {@link OutputStream} which transparently swallows the expected header
	 * written to the underlying stream.
	 **
	 ** The {@code write} methods will throw {@link IOException} if bytes different from
	 * the header are written.
	 */
