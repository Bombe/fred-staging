 ...
 *
 * @see <a href=
 * "https://emu.freenetproject.org/pipermail/devl/2008-February/028357.html">Devl Mailing
 * List</a>
...
public class NativeThread extends Thread {
	...
	static {
		...
		if (maybeLoadNative) {
			...
			System.out
					.println("Using the NativeThread implementation (base nice level is " + NATIVE_PRIORITY_BASE + ')');
	...
	private boolean setNativePriority(int prio) {
		...
		if (NATIVE_PRIORITY_BASE != realPrio && !dontCheckRenice) {
			/*
			 * The user has reniced freenet or we didn't use the PacketSender to create
			 * the thread either ways it's bad for us.
			 *
		...
		final int linuxPriority = NATIVE_PRIORITY_BASE + NATIVE_PRIORITY_RANGE
		...
		Logger.minor(this,
				"Setting native priority to " + linuxPriority + " (base=" + NATIVE_PRIORITY_BASE + ") for " + this);
