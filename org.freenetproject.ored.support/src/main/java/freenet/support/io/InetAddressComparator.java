...
/**
 * Fast non-lexical Comparator for IP addresses for cases where an attacker might forge IP
 * addresses to try to exhaust a hashtable, so we need to use a TreeMap, but we don't
 * actually care whether similar IPs are close together.
 *
 * @author toad
 */
public class InetAddressComparator implements Comparator<InetAddress> {
	...
	public int compare(InetAddress arg0, InetAddress arg1) {
		...
		// Hostnames in InetAddress are merely cached, equals() only operates on the
		// byte[].
	}
