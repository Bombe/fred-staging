...
public final class Fallocate {
	...
	public void execute() throws IOException {
		...
		// if (IS_LINUX) {
		// final int result = FallocateHolder.fallocate(fd, mode, offset,
		// final_filesize-offset);
		// errno = result == 0 ? 0 : Native.getLastError();
		// } else if (IS_POSIX) {
		// errno = FallocateHolderPOSIX.posix_fallocate(fd, offset,
		// final_filesize-offset);
		// } else {
		// isUnsupported = true;
		// }
	...
	private static int getDescriptor(FileChannel channel) {
		...
		// throw new UnsupportedOperationException("unsupported FileChannel
		// implementation", e);
		// }
	}
	...
	private static int getDescriptor(FileDescriptor descriptor) {
		...
		// final Field field = descriptor.getClass().getDeclaredField(IS_ANDROID ?
		// "descriptor" : "fd");
		// field.setAccessible(true);
		// return (int) field.get(descriptor);
		// } catch (final Exception e) {
		// throw new UnsupportedOperationException("unsupported FileDescriptor
		// implementation", e);
		// }
	}
