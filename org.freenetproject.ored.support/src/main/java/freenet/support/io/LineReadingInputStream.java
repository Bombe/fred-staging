...
public class LineReadingInputStream extends FilterInputStream implements LineReader {
	 ...
	 * @param maxLength The maximum length of a line. If a line is longer than this, we
	 * throw IOException rather than keeping on reading it forever.
	...
	public String readLine(int maxLength, int bufferSize, boolean utf) throws IOException {
		...
		if (maxLength <= bufferSize)
			bufferSize = maxLength + 1; // Buffer too big, shrink it (add 1 for the
										// optional \r)
		...
		while (true) {
			...
			// REDFLAG this is definitely safe with the above charsets, it may not be safe
			// with some wierd ones.
			int end = ctr + x;
			for (; ctr < end; ctr++) {
				if (buf[ctr] == '\n') {
					String toReturn = "";
