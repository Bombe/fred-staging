...
/**
 * A class to control the global priority of the current process. Microsoft suggests
 * flagging daemon/server processes with the BELOW_NORMAL_PRIORITY_CLASS priority class so
 * that they don't interfere with the responsiveness of the rest of the system. This is
 * especially important when freenet is started at system startup. We use JNA to call the
 * OS libraries directly without needing JNI wrappers. Its usage is really simple: just
 * call ProcessPriority.enterBackgroundMode(). If the OS doesn't support it or if the
 * process doesn't have the appropriate permissions, the above call is simply a no-op.
 *
 */
