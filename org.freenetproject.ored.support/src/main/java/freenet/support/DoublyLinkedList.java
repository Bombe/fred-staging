...
/**
 * Framework for managing a doubly linked list. The purpose of DoublyLinkedList is simply
 * and solely so that we can override the entries with our own classes. This makes removal
 * for example extremely fast: O(1) not O(n). In any other case we can use LinkedList.
 *
 * @author tavin
 */
public interface DoublyLinkedList<T extends DoublyLinkedList.Item<?>> extends Iterable<T> {
	...
	public interface Item<T extends DoublyLinkedList.Item<?>> {
		...
		/**
		 * Get next {@link Item}. May or may not return <code>null</code> if this is the
		 * last <code>Item</code>.
		 *
		...
		/**
		 * Get previous {@link Item}. May or may not return <code>null</code> if this is
		 * the first <code>Item</code>.
		 *
		 * @see DoublyLinkedList#hasNext()
		 */
		T getPrev();
	...
	/**
	 * Check if this list is empty. @return <code>true</code> if this list is empty,
	...
	/**
	 * Returns true if the list contains an item i where <code>i.equals(item)</code> is
	 * true.
	 */
	...
	/**
	 * @return <code>true</code> if <code>i</code> has next item. (ie. not the last item);
	 * <code>false</code> otherwise
	 */
	boolean hasNext(T i);

	/**
	 * @return <code>true</code> if <code>i</code> has previous item. (ie. not the first
	...
	/**
	 * @return previous item of <code>i</code>. If this is the first element, return
	 * <code>null</code>
	 */
	T prev(T i);
