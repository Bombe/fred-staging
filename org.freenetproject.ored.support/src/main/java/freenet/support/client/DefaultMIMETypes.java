...
public class DefaultMIMETypes {
	...
	protected static synchronized void addMIMEType(short number, String type, String[] extensions,
		...
		if (extensions != null) {
			for (String ext : extensions) {
				...
				if (s != null) {
					// No big deal
					Logger.normal(DefaultMIMETypes.class, "Extension " + ext + " assigned to "
							+ byNumber(s.shortValue()) + " in preference to " + number + ':' + type);
				}
				else {
	...
	/**
	 * Add a MIME type, with extensions separated by spaces. This is more or less the
	 * format in /etc/mime-types.
	 */
	protected static synchronized void addMIMEType(short number, String type, String extensions) {
	...
	/**
	 * Get the number of a MIME type, or -1 if it is not in the table of known MIME types,
	 * in which case it will have to be sent uncompressed.
	 */
	...
	/*
	 * From toad's /etc/mime.types cat /etc/mime.types | sed "/^$/d;/#/d" | tr --squeeze
	 * '\t' ' ' | (y=0; while read x; do echo "$x" | sed -n
	 * "s/^\([^ ]*\)$/addMIMEType\($y, \"\1\"\);/p;s/^\([^ (),]\+\) \(.*\)$/addMIMEType\($y, \"\1\", \"\2\"\);/p;"
	 * ; y=$((y+1)); done)
	 */
