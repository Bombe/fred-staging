 ...
 * @author amphibian
 *
 * For the first <tt>maxReports</tt> reports, this is equivalent to a simple running
 * average. After that it is a decaying running average with a <tt>decayFactor</tt> of
 * <tt>1 / maxReports</tt>. We accomplish this by having
 * <tt>decayFactor = 1/(Math.min(#reports, maxReports))</tt>. We can therefore:
 * <ul>
...
public final class BootstrappingDecayingRunningAverage implements RunningAverage, Cloneable {
	 ...
	 * @param maxReports number of reports before bootstrapping period ends and decay
	 * begins
	 * @param fs {@link SimpleFieldSet} parameter for this object. Will override other
	 * parameters.
	 */
	public BootstrappingDecayingRunningAverage(double defaultValue, double min, double max, int maxReports,
			SimpleFieldSet fs) {
		this.min = min;
		this.max = max;
		reports = 0;
	 ...
	 *
	 * Used by {@link DecayingKeyspaceAverage} to normalize the stored averages. Calling
	 * this function may (purposefully) destroy the utility of the average being kept.
	 * @param d
	 * @return
