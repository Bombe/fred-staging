...
public final class SimpleRunningAverage implements RunningAverage, Cloneable {
	...
	public synchronized String toString() {
		return super.toString() + ": curLen=" + curLen + ", ptr=" + nextSlotPtr + ", total=" + total + ", average="
				+ total / curLen;
	}
	...
	public synchronized double minReportForValue(double targetValue) {
		if (curLen < refs.length) {
			/**
			 * Don't need to remove any values before reporting, so is slightly simpler.
			 * (total + report) / (curLen + 1) >= targetValue => report / (curLen + 1) >=
			 * targetValue - total/(curLen+1) => report >= (targetValue - total/(curLen +
			 * 1)) * (curLen+1) => report >= targetValue * (curLen + 1) - total EXAMPLE:
			 * Mean (5, 5, 5, 5, 5, X) = 10 X = 10 * 6 - 25 = 35 => Mean = (25 + 35) / 6 =
			 * 60/6 = 10
			 */
			...
			/**
			 * Essentially the same, but: 1) Length will be curLen, not curLen+1, because
			 * is full. 2) Take off the value that will be taken off first.
			 */
