...
/**
 * An LRU map from K to V. That is, when a mapping is added, it is pushed to the top of
 * the queue, even if it was already present, and pop/peek operate from the bottom of the
 * queue i.e. the least recently pushed. The caller must implement any size limit needed.
 * FIXME most callers should be switched to LinkedHashMap. Does not support null keys.
 *
...
public class LRUMap<K, V> {
	...
	/**
	 * We use our own DoublyLinkedList implementation because it improves performance to
	 * be able to inherit from and refer to QItem's directly.
	 */
	...
	/**
	 * Create a LRUMap that is safe to use with keys that can be controlled by an
	 * attacker. Meaning one based on a TreeMap, not a HashMap (think hash collision
	 * DoS's).
	 */
	...
	/**
	 * push()ing an object that is already in the queue moves that object to the most
	 * recently used position, but doesn't add a duplicate entry in the queue.
	 */
	...
	/**
	 * Note that this does not automatically promote the key. You have to do that by hand
	 * with push(key, value).
	 */
	...
	/**
	 * Note that unlike the java.util versions, this will not reallocate (hence it doesn't
	 * return), so pass in an appropriately big array, and make sure you hold the lock!
	 * @param entries
	 * @return
	 */
