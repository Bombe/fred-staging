...
public class AddressIdentifier {
	...
	static {
		...
		String ipv6AddressRegex = wordRegex + "?:" + wordRegex + ':' + wordRegex + ':' + wordRegex + ':' + wordRegex
	 ...
	 *
	 * REDFLAG: IPv6 percent scope ID's could cause problems with URI's. Should not be
	 * exploitable as we don't do anything important with URI's with hosts in anyway. In
	 * particular we MUST NOT do anything with hosts from URI's from untrusted sources
	 * e.g. content filter. But then that would be completely stupid, so we don't.
	 * @param address The address to determine the type of
	 * @return {@link AddressType#OTHER} if <code>address</code> is a hostname,
	 * {@link AddressType#IPv4} or {@link AddressType#IPv6} otherwise
	 */
	 ...
	 * Tries to detemine the address type of the given address.
	 * @param address The address to determine the type of
	 * @param allowIPv6PercentScopeID If true, match %<scope-id> suffixed IPv6 IP
	 * addresses
	 * @return {@link AddressType#OTHER} if <code>address</code> is a hostname,
	 * {@link AddressType#IPv4} or {@link AddressType#IPv6} otherwise
	 */
