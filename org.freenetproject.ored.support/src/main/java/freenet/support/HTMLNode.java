...
public class HTMLNode implements XMLCharacterClasses, Cloneable {
	...
	/**
	 * Text to be inserted between tags, or possibly raw HTML. Only non-null if name is
	 * "#" (= text) or "%" (= raw HTML). Otherwise the constructor will allocate a
	...
	/**
	 * Tests an HTML element name to determine if it is one of the elements permitted to
	 * be empty in the XHTML spec ( http://www.w3.org/TR/xhtml1/ )
	 ...
	 * for readability. All tags with newlines after the opening tag also get newlines
	...
	public HTMLNode clone() {
		...
		// FIXME is clearing read only an abuse of the clone() API? Should we rename the
		// method?
	...
	protected boolean checkNamePattern(String str) {
		...
		// Oddly these problems were seen where the answer is almost certainly in the
		// first matcher, because the tag name was "html"...
	...
	public HTMLNode(String name, String[] attributeNames, String[] attributeValues, String content) {
		...
		if ((attributeNames != null) && (attributeValues != null)) {
			...
			for (int attributeIndex = 0,
					attributeCount = attributeNames.length; attributeIndex < attributeCount; attributeIndex++) {
	 ...
	 * @return The added node. You can add more tags inside it with addChild(), or add
	 * attributes with addAttribute() etc. If you render the parent tag with generate(),
	 * it will include this tag in its output.
	 */
	 ...
	 * @return The name of the first "real" tag, or <code>null</code> if no "real" tag
	 * could be found
	 */
	...
	public StringBuilder generate(StringBuilder tagBuffer, int indentDepth) {
		...
		// Perhaps this should be something else, but since I don't know if '#' was not
		// just arbitrary chosen, I'll just pick '%'
	...
	/**
	 * Special HTML node for the DOCTYPE declaration. This node differs from a normal HTML
	 * node in that it's child (and it should only have exactly one child, the "html"
	 * node) is rendered <em>after</em> this node.
	 *
	...
	public static HTMLNode linkInNewWindow(String path) {
		return new HTMLNode("a", new String[] { "href", "target", "rel" },
