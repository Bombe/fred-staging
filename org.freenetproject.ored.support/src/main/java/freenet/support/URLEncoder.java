 ...
 * java.net.URLEncoder, which encodes strings to application/x-www-urlencoded. This is
 * much closer to what java.net.URI does. We don't turn spaces into +'s, and we allow
 * through non-ascii characters unless told not to.
 */
public class URLEncoder {
	 ...
	 * @param force List of characters (in the form of a string) which must be encoded as
	 * well as the built-in.
	 * @param ascii If true, encode all foreign letters, if false, leave them as is. Set
	 * to true if you are passing to something that needs ASCII (e.g. HTTP headers), set
	 * to false if you are using in an HTML page.
	...
	public static String encode(String URL, String force, boolean ascii, String extraSafeChars) {
		...
		for (int i = 0; i < URL.length(); ++i) {
			char c = URL.charAt(i);
			if ((safeURLCharacters.indexOf(c) >= 0 || (!ascii && c >= 128 && Character.isDefined(c)
					&& !Character.isISOControl(c) && !Character.isSpaceChar(c)) || extraSafeChars.indexOf(c) >= 0)
