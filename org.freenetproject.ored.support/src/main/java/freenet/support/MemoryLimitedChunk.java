...
/**
 * Represents a chunk of some limited resource, usually an estimate of bytes of memory in
 * use, which has been allocated to a MemoryLimitedJob. Can be released but not added to.
 *
 * @author toad
 */
public final class MemoryLimitedChunk {
	 ...
	 * MemoryLimitedJob has stopped using a large temporary buffer, and has made it
	 * GC'able, that is, there are no more (non-weak) pointers to it.
	 */
	public long release() {
		long released = 0;
		synchronized (this) {
	...
	/**
	 * Should be called when the caller is now using a smaller amount of the resource,
	 * e.g. we go from a big buffer to a small buffer. Note that this is irreversible.
	 */
