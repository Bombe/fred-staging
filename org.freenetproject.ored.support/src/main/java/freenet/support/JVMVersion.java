...
public class JVMVersion {
	...
	/**
	 * Java version before which to display an End-of-Life warning. Subsequent releases of
	 * Freenet will function with them, but that may soon not be the case.
	 */
	...
	/**
	 * Pre-9 is formatted as: major.feature[.maintenance[_update]]-ident Post-9 is
	 * formatted as: major[.minor[.security[. ...]]]-ident For comparison of
	 * compatibility, information beyond the major, feature/minor, maintenance/security
	 * and pre-9 update fields should not be of interest. We find a common denominator in
	 * major(.a(.b([._]c)?)?)?, skipping any additional postfix. The regex omits leading
	 * zeroes.
	 */
	private static final Pattern VERSION_PATTERN = Pattern
			.compile("^0*(\\d+)(?:\\.0*(\\d+)(?:\\.0*(\\d+)(?:[_.]0*(\\d+))?)?)?.*$");

	public static boolean isEOL() {
		return !isAndroid() // on android the version checks are done on the App level, so
	...
	/**
	 * Decomposes a version string into major, feature, and optional maintenance and
	 * update components. Missing optional components are set to 0, failed parses return
	 * all zeroes.
	 */
	...
	/**
	 * Compares two version strings, ignoring optional identifiers. Version strings that
	 * cannot be parsed are treated as version 0.0.0_0.
	 * @return A value < 0 if version1 is less than version2, 0 if they are equal, > 0
	 * otherwise.
	 */
