...
/**
 * This class provides encoding of byte arrays into Base64-encoded strings, and decoding
 * the other way.
 *
 * <P>
 * NOTE! This is modified Base64 with slightly different characters than usual, so it
 * won't require escaping when used in URLs.
 *
 * <P>
 * NOTE! This class only does the padding that's normal in Base64 if the 'true' flag is
 * given to the encode() method. This is because Base64 requires that the length of the
 * encoded text be a multiple of four characters, padded with '='. Without the 'true'
 * flag, we don't add these '=' characters.
 *
...
public class Base64 {
	...
	private static char[] base64Alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789~-"
			.toCharArray();

	private static char[] base64StandardAlphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
			.toCharArray();
	...
	/*
	 * FIXME: Figure out where this function is used and maybe remove it if its not used.
	 * Its old javadoc which has been here for a while fools the user into believing that
	 * the format is standard compliant
	 */
	 ...
	 *
	 * Please use this to decode into a string, rather than trying to decode the string
	 * yourself using new String(bytes[]) which is not deterministic.
	 */
	...
	/**
	 * Handles the standards-compliant (padded with '=' signs) as well as our shortened
	 * form.
	 */
