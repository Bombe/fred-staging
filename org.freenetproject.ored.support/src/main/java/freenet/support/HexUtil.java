 ...
 *
 * <p>
 * Unless otherwise stated, the conventions follow the rules outlined in the Java Language
 * Specification.
 * </p>
 *
 * @author syoung
...
public class HexUtil {
	...
	public static byte[] bitsToBytes(BitSet ba, int size) {
		...
		if (logDEBUG)
			sb = new StringBuilder(8 * bytesAlloc); // TODO: Should it be 2*8*bytesAlloc
													// here?
		...
		if (logDEBUG)
			Logger.debug(HexUtil.class, "bytes: " + bytesAlloc + " returned from bitsToBytes(" + ba + ',' + size + "): "
					+ bytesToHex(b) + " for " + sb.toString());
		return b;
	}
	...
	/**
	 * Turn a BigInteger into a hex string. BigInteger.toString(16) NPEs on Sun/Oracle JDK
	 * 1.4.2_05. :< The bugs in their Big* are getting seriously irritating...
	 */
