 ...
 * initialising it with the results of a method, eg <code>T identity(T o)
 * { T o; }</code>; however the "constant" behaviour might be required in some cases. A
 * more flexible solution is to add a method that returns the field, eg
 * {@link #publicVersion()}, and choose between the method and the field as necessary.
 *
 * <pre>$ grep -R "\WVersion\.\w*[^(a-zA-Z]" src</pre>
 *
 * can be used to find the places in the source code whose logic needs to be checked for
 * this.
 *
 */
public class Version {
	 ...
	 * computations, so cannot be safely changed!!! Hence publicVersion ...
	 */
	...
	/**
	 * Use this method when you want the value returned to be the run-time version, not
	 * the build-time version.
	 *
	 * For example, if you compile your class, then modify this class and compile it (the
	 * build script does this automatically), but don't re-compile your original class,
	 * then that will still have the old version compiled into it, since it is a static
	 * constant.
	...
	/**
	 * @return The lowest build number with which the node will connect and exchange data
	 * normally.
	 */
	...
	/**
	 * @return true if requests should be accepted from nodes brandishing this protocol
	 * version string
	 */
	...
	/**
	 * @return true if requests should be accepted from nodes brandishing this version
	 * string
	 */
	...
	/**
	 * @return true if requests should be accepted from nodes brandishing this version
	 * string, given an arbitrary lastGoodVersion
	 */
	...
	/**
	 * Update static variable highestSeenBuild anytime we encounter a new node with a
	 * higher version than we've seen before
	 */
	...
	/**
	 * @return true if the string describes the same node version as ours. Note that the
	...
	public static void main(String[] args) throws Throwable {
		System.out.println("Freenet: " + nodeName + ' ' + nodeVersion + " (protocol " + protocolVersion + ") build "
				+ buildNumber + " (last good build: " + lastGoodBuild() + ") " + cvsRevision);
	}
