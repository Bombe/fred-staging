...
public class FileLoggerHook extends LoggerHook implements Closeable {
	...
	/** Verbosity types */
	public static final int DATE = 1, CLASS = 2, HASHCODE = 3, THREAD = 4, PRIORITY = 5, MESSAGE = 6, UNAME = 7;
	...
	/**
	 * Something weird happens when the disk gets full, also we don't want to block So run
	 * the actual write on another thread
	 *
	 * Unfortunately, we can't use ConcurrentBlockingQueue because we need to dump stuff
	 * when the queue gets too big.
	 *
	 * FIXME PERFORMANCE: Using an ArrayBlockingQueue avoids some unnecessary memory
	 * allocations, but it means we have to take two locks. Seriously consider reverting
	 * 88268b99856919df0d42c2787d9ea3674a9f6f0d..e359b4005ef728a159fdee988c483de8ce8f3f6b
	...
	/**
	 * The extra parameter int digit is to be used for creating a logfile name when a log
	 * exists already with the same date.
	 * @param c
	...
	private long flushTime = 1000; // Default is 1 second. Will be set by
									// setMaxBacklogNotBusy().
				...
				try {
					...
					synchronized (list) {
						...
						while (o == null) {
							...
							try {
								if (thisTime < maxWait) {
									// Wait no more than 500ms since the CloserThread
									// might be waiting for closedFinished.
									list.wait(Math.min(500L, maxWait - thisTime));
									thisTime = System.currentTimeMillis();
									if (listBytes < LIST_WRITE_THRESHOLD) {
										// Don't write at all until the lower bytes
										// threshold is exceeded, or the time threshold
										// is.
							...
							if (o == null) {
								...
								if (thisTime >= maxWait) {
									timeoutFlush = true;
									timeWaitingForSync = -1; // We have stuff to write, we
	...
	/**
	 * Create a Logger to append to the given file. If the file does not exist it will be
	 * created.
	 ...
	 */
	public FileLoggerHook(String filename, String fmt, String dfmt, String logRotateInterval, LogLevel threshold,
			boolean assumeWorking, boolean logOverwrite, long maxOldLogfilesDiskUsage, int maxListSize)
			throws IOException, IntervalParseException {
		this(false, filename, fmt, dfmt, logRotateInterval, threshold, assumeWorking, logOverwrite,
				maxOldLogfilesDiskUsage, maxListSize);
	}
	...
	public void trimOldLogFiles() {
		synchronized (trimOldLogFilesLock) {
			while (oldLogFilesDiskSpaceUsage > maxOldLogfilesDiskUsage) {
				OldLogFile olf;
				// TODO: creates a double lock situation, but only here. I think this is
				// okay because the inner lock is only used for trivial things.
				synchronized (logFiles) {
					if (logFiles.isEmpty()) {
				...
				if (logMINOR)
					Logger.minor(this, "Deleting " + olf.filename + " - saving " + olf.size + " bytes, disk usage now: "
	...
	public FileLoggerHook(String filename, String fmt, String dfmt, String threshold, String logRotateInterval,
			boolean assumeWorking, boolean logOverwrite, long maxOldLogFilesDiskUsage, int maxListSize)
			throws IOException, InvalidThresholdException, IntervalParseException {
		this(filename, fmt, dfmt, logRotateInterval, LogLevel.valueOf(threshold.toUpperCase()), assumeWorking,
				logOverwrite, maxOldLogFilesDiskUsage, maxListSize);
	}
	...
	public FileLoggerHook(OutputStream os, String fmt, String dfmt, LogLevel threshold) throws IntervalParseException {
	 ...
	 * @throws IntervalParseException
	 */
	public FileLoggerHook(OutputStream stream, String fmt, String dfmt, LogLevel threshold, boolean overwrite)
			throws IntervalParseException {
	...
	public void start() {
		if (redirectStdOut) {
			try {
				System.setOut(new PrintStream(new OutputStreamLogger(LogLevel.NORMAL, "Stdout: ", ENCODING), false,
						ENCODING));
				if (redirectStdErr)
					System.setErr(new PrintStream(new OutputStreamLogger(LogLevel.ERROR, "Stderr: ", ENCODING), false,
							ENCODING));
			}
			catch (UnsupportedEncodingException e) {
				throw new Error(e);
	...
	public FileLoggerHook(boolean rotate, String baseFilename, String fmt, String dfmt, String logRotateInterval,
			LogLevel threshold, boolean assumeWorking, boolean logOverwrite, long maxOldLogfilesDiskUsage,
			int maxListSize) throws IOException, IntervalParseException {
	...
	public FileLoggerHook(boolean rotate, String baseFilename, String fmt, String dfmt, String threshold,
			String logRotateInterval, boolean assumeWorking, boolean logOverwrite, long maxOldLogFilesDiskUsage,
			int maxListSize) throws IOException, InvalidThresholdException, IntervalParseException {
	...
	private FileLoggerHook(String fmt, String dfmt, LogLevel threshold, String logRotateInterval, boolean overwrite,
			long maxOldLogfilesDiskUsage, int maxListSize) throws IntervalParseException {
		super(threshold);
	...
	public void sendLogByContainedDate(long time, OutputStream os) throws IOException {
		...
		synchronized (logFiles) {
			for (OldLogFile olf : logFiles) {
				if (logMINOR)
					Logger.minor(this, "Checking " + time + " against " + olf.filename + " : start=" + olf.start
							+ ", end=" + olf.end);
		...
		while (written < size) {
			...
			try {
				...
				Logger.error(this,
						"Could not read bytes " + written + " to " + (written + toRead) + " from file "
								+ toReturn.filename + " which is supposed to be " + size + " bytes ("
	...
	/**
	 * Set the maximum size of old (gzipped) log files to keep. Will start to prune old
	 * files immediately, but this will likely not be completed by the time the function
	 * returns as it is run off-thread.
	 */
