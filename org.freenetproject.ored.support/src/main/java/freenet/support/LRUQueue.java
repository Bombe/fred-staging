...
public class LRUQueue<T> {
	...
	/*
	 * I've just converted this to using the DLList and Hashtable this makes it Hashtable
	 * time instead of O(N) for push and remove, and Hashtable time instead of O(1) for
	 * pop. Since push is by far the most done operation, this should be an overall
	 * improvement.
	 */
	...
	/**
	 * Return the objects in the queue as an array in an arbitrary and meaningless order.
	 * @param array The array to fill in. If it is too small a new array of the same type
	 * will be allocated.
	 */
	...
	/**
	 * Return the objects in the queue as an array. The <strong>least</strong> recently
	 * used object is in <tt>[0]</tt>, the <strong>most</strong> recently used object is
	 * in <tt>[array.length-1]</tt>.
	 */
