 ...
 *
 * This iterator iterates over the underlying array, yielding its elements in an
 * (optionally) randomized order. Instances can be reused by {@link #reset(Random)
 * resetting} them to their original position, optionally yielding a new random
 * permutation of the array elements.
 *
 * Storage requirements are linear in the size of the underlying array, all instance
 * operations finish in constant time.
 *
...
public class RandomArrayIterator<E> implements Iterator<E> {
	...
	/**
	 * Permutation state. This array contains a permutation of indices into
	 * {@link #array}.
	 */
	...
	/**
	 * Creates a new iterator for the given array. Initially, the array is iterated in the
	 * original ordering, {@link #reset(Random)} the iterator to get a new random
	 * iteration ordering.
	 * @param array The underlying array
	 */
	 ...
	 * {@link #next()} will iterate over the array in a new random order. If it is
	 * {@code null}, the iteration order remains unaltered.
	 * @param random Random source for the next run, or {@code null} to repeat the
	 * previous run
	 */
