...
/**
 * Encodes any character mentioned with a substitute in the HTML spec. This includes
 * nulls, <>, quotes, but not control characters. It should be safe to put the output of
 * this function into a web page; if it is not then we have big problems. Because we
 * encode quotes it should also be safe to include it inside attributes. I am not certain
 * where the list in HTMLEntities came from, but the list of potentially
 * markup-significant characters in [X]HTML is *really* small.
 *
...
public class HTMLEncoder {
	...
	public final static CharTable charTable = new CharTable(HTMLEntities.encodeMap);
	 ...
	 *
	 * HTMLEncode.encode() use some HTML-specific entities (e.g. &amp;) hence not suitable
	 * for generic XML.
	 */
	...
	private final static class CharTable {
		...
		public CharTable(HashMap<Character, String> map) {
			...
			modulo = map.size();
			int[] collisionTable = new int[max + 1]; // using integers instead of booleans
														// (no cleanup)
			boolean ok = false;
			while (!ok) {
				...
				for (int i = 0; ok && i < keys.length; ++i) {
					...
					if (collisionTable[keyIndex] == modulo) { // is this value already
																// used
						ok = false;
					}
					else {
