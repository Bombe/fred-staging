...
public class TimeUtil {
	 ...
	 * @param maxTerms the terms number to display (e.g. 2 means "h" and "m" if the time
	 * could be expressed in hour, 3 means "h","m","s" in the same example). The maximum
	 * terms number available is 6
	...
	// FIXME: For me it returns a parsed time with 2 hours difference, so it seems to
	// parse localtime. WHY?
	...
	/**
	 * @return Returns the passed date with the same year/month/day but with the time set
	 * to 00:00:00.000
	 */
	public static Date setTimeToZero(final Date date) {
		// We need to cut off the hour/minutes/seconds
		final GregorianCalendar calendar = new GregorianCalendar(TimeZone.getTimeZone("UTC"));
		calendar.setTimeInMillis(date.getTime()); // We must not use setTime(date) in case
													// the date is not UTC.
