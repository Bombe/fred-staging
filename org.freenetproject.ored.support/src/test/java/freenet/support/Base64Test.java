...
public class Base64Test extends TestCase {
	 ...
	 * http://en.wikipedia.org/wiki/Base_64 as reference) to verify if it encode works
	 * correctly.
	 */
	 ...
	 * http://en.wikipedia.org/wiki/Base_64 as reference) to verify if it decode an
	 * already encoded string correctly.
	 */
	...
	/**
	 * Test encode(byte[] in) and decode(String inStr) methods, to verify if they work
	 * correctly together. It compares the string before encoding and with the one after
	 * decoding.
	 */
	...
	/**
	 * Test the encode(String,boolean) method to verify if the padding character '=' is
	 * correctly placed.
	 */
	...
	/**
	 * Test if the decode(String) method raise correctly an exception when providing a
	 * string with non-Base64 characters.
	 */
	...
	/**
	 * Test if the decode(String) method raise correctly an exception when providing a
	 * string with a wrong Base64 length. (as we can consider not-padded strings too, the
	 * only wrong lengths are the ones where -> number MOD 4 = 1).
	 */
	...
	public void testRandom() throws IllegalBase64Exception {
		...
		for (iter = 0; iter < 1000; iter++) {
			...
			for (int i = 0; i < b.length; i++)
				assertEquals(
						"data mismatch: index " + i + " of " + b.length + " should be 0x"
								+ Integer.toHexString(b[i] & 0xFF) + " was 0x" + Integer.toHexString(decoded[i] & 0xFF),
						b[i], decoded[i]);
		}
	}
