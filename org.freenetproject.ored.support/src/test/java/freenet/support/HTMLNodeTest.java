...
public class HTMLNodeTest extends TestCase {
	...
	/**
	 * Tests HTMLNode(String,String[],String[],String) constructor verifying if all
	 * attributes are correctly inserted
	 */
	public void testHTMLNode_AttributesArray() {
		...
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, methodAttributesName,
				methodAttributesValue, SAMPLE_NODE_CONTENT);
		// checks presence
		for (int i = 0; i < size; i++)
	...
	/**
	 * Tests addAttribute(String,String) method adding the same attribute many times and
	 * verifying it keeps only one reference to it.
	 */
	...
	/**
	 * Tests addChild(HTMLNode) method adding the Node itself as its child. The method
	...
	/**
	 * Tests addChildren(HTMLNode[]) method adding the Node itself as its child. The
	 * method should rise an exception
	 */
	public void testAddChildrenUsingTheNodeItselfAsChild() {
		HTMLNode[] methodHTMLNodesArray = { new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY), exampleNodeNonEmpty,
	...
	/**
	 * Tests addChild(String) method using the same name every time and verifying that a
	 * real new HTML is always added.
	 */
	...
	/**
	 * Tests addChild(HTMLNode) method verifying the behavior when adding the same
	 * HTMLNode instance two times. It should raise an IllegalArgument exception.
	 */
	...
	/**
	 * Tests addChildren(HTMLNode[]) method verifying the behavior when adding the same
	 * HTMLNode instance two times.
	 */
	...
	/**
	 * Tests addChildren(String,String,String) method verifying if the child is correctly
	 * added and if it generates good output using generate() method.
	 */
	public void testAddChild_StringStringString() {
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_EMPTY);
		methodHTMLNode.addChild(SAMPLE_OKAY_NODE_NAME_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME, SAMPLE_ATTRIBUTE_VALUE);
		List<HTMLNode> childrenList = methodHTMLNode.children;
		assertEquals(1, childrenList.size());
		assertEquals(generateNoContentNodeOutput(SAMPLE_OKAY_NODE_NAME_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
				SAMPLE_ATTRIBUTE_VALUE), childrenList.get(0).generate());
	}

	/**
	 * Tests addChildren(String,String,String,String) method verifying if the child is
	 * correctly added and if it generates good output using generate() method.
	 */
	public void testAddChild_StringStringStringString() {
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY);
		methodHTMLNode.addChild(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME, SAMPLE_ATTRIBUTE_VALUE,
				SAMPLE_NODE_CONTENT);
		List<HTMLNode> childrenList = methodHTMLNode.children;
		assertEquals(1, childrenList.size());
		assertEquals(generateFullNodeOutput(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
				SAMPLE_ATTRIBUTE_VALUE, SAMPLE_NODE_CONTENT), childrenList.get(0).generate());
	}

	/**
	 * Tests addChildren(String,String[],String[]) method verifying if the child is
	 * correctly added and the child attributes are corrects.
	 */
	public void testAddChild_StringArrayArray() {
		...
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY);
		methodHTMLNode.addChild(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, methodAttributesNamesArray,
	...
	public void testAddChild_StringArrayArrayString() {
		...
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY);
		methodHTMLNode.addChild(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, methodAttributesNamesArray,
				methodAttributesValuesArray, SAMPLE_NODE_CONTENT);
		testSingleChildAttributes(methodHTMLNode, methodAttributesNamesArray, methodAttributesValuesArray);
	}
	...
	public void testGetContent() {
		...
		assertEquals(SAMPLE_NODE_CONTENT, methodHTMLNode.children.get(0).getContent());
		assertNull(methodHTMLNode.getContent());
	...
	/**
	 * Tests getAttributes() and setAttribute(String,String) methods verifying if
	 * attributes are correctly inserted and fetched.
	 */
	...
	/**
	 * Tests addAttribute(String,String) method trying to insert an attribute with a null
	 * as name value. It should rise an IllegalArgument exception
	 */
	...
	/**
	 * Tests addAttribute(String,String) method trying to insert an attribute with a null
	 * as attribute value. It should rise an IllegalArgument exception
	 */
	...
	/**
	 * Tests HTMLNode(String,String,String,String) and HTMLNode(String,String,String)
	 * constructors trying to create a node that has attribute name null. It should raise
	 * an IllegalArgument exception
	 */
	public void testHTMLNode_nullAttributeName() {
		try {
			new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, null, SAMPLE_ATTRIBUTE_VALUE, SAMPLE_NODE_CONTENT);
			fail("Expected Exception Error Not Thrown!");
		}
		catch (IllegalArgumentException anException) {
			assertNotNull(anException);
	...
	/**
	 * Tests HTMLNode(String,String,String,String) and HTMLNode(String,String,String)
	 * constructors trying to create a node that has attribute value null. It should raise
	 * an IllegalArgument exception
	 */
	public void testHTMLNode_nullAttributeValue() {
		try {
			new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, SAMPLE_WRONG_ATTRIBUTE_NAME, null, SAMPLE_NODE_CONTENT);
			fail("Expected Exception Error Not Thrown!");
		}
		catch (IllegalArgumentException anException) {
			assertNotNull(anException);
	...
	/**
	 * Tests HTMLNode(String,String[],String[],String) constructor trying to create a node
	 * that has attributes name null. It should raise an IllegalArgument exception
	 */
	public void testHTMLNodeArray_nullAttributeName() {
		String[] methodAttributesNameArray = { "first", null, "after" };
		String[] methodAttributesValueArray = { SAMPLE_ATTRIBUTE_VALUE, SAMPLE_ATTRIBUTE_VALUE,
				SAMPLE_ATTRIBUTE_VALUE };
	...
	/**
	 * Tests HTMLNode(String,String[],String[],String) constructor trying to create a node
	 * that has attributes value null. It should raise an IllegalArgument exception
	 */
	...
	/**
	 * Tests HTMLNode(String,String[],String[],String) constructor trying to create a node
	 * that has different length for attributes names array and attributes values array.
	...
	/**
	 * Tests if the passed arrays raise an IllegalArgumentException using them to create a
	 * new HTMLNode (i.e. one of the name or value must be null)
	...
	private void testHTMLNodeArray_null(String[] attributesNames, String[] attributesValues) {
		try {
			new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, attributesNames, attributesValues, SAMPLE_NODE_CONTENT);
			fail("Expected Exception Error Not Thrown!");
		}
		catch (IllegalArgumentException anException) {
			assertNotNull(anException);
	...
	/**
	 * Tests generate() method with a HTMLNode that has "textarea","div","a" as node name,
	 * since they generates a different output from all other names.
	 */
	public void testGenerate_fromHTMLNode_textareaDivA() {
		...
		for (int i = 0; i < nodeNamesArray.length; i++) {
			...
			methodHTMLNode = new HTMLNode(nodeNamesArray[i], SAMPLE_OKAY_ATTRIBUTE_NAME, SAMPLE_ATTRIBUTE_VALUE);
			assertEquals(generateFullNodeOutput(nodeNamesArray[i], SAMPLE_OKAY_ATTRIBUTE_NAME, SAMPLE_ATTRIBUTE_VALUE,
					"", newlines), methodHTMLNode.generate());
		}
	}
	...
	public void testGenerate_fromHTMLNodeWithChild_SpecialNames() {
		...
		String[] nodeNamesArray = { "div", "form", // input is an empty element!
				"script", "table", "tr", "td" };
		HTMLNode methodChildNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
		...
		for (int i = 0; i < nodeNamesArray.length; i++) {
			...
			assertEquals(("<" + nodeNamesArray[i] + " ").toLowerCase() + SAMPLE_OKAY_ATTRIBUTE_NAME + "=" + "\""
					+ SAMPLE_ATTRIBUTE_VALUE + "\">\n" +
					// FIXME why is this using 2 tabs? I don't understand ...
					"\t\t" + SAMPLE_NODE_CONTENT +

					// child
					generateFullNodeOutput(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
							SAMPLE_ATTRIBUTE_VALUE, SAMPLE_NODE_CONTENT)
					+ "\n\t" + ("</" + nodeNamesArray[i] + ">\n").toLowerCase() + "\t",
	...
	/**
	 * Tests generate() method with a HTMLNode with only the name. The resulting string
	 * should be in the form: <node_name />
	 */
	public void testGenerate_fromHTMLNode_String() {
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_EMPTY);
		assertEquals(("<" + SAMPLE_OKAY_NODE_NAME_EMPTY + " />").toLowerCase(), methodHTMLNode.generate());
	}

	/**
	 * Tests generate() method with a HTMLNode with the name and content. The resulting
	 * string should be in the form: <node_name>Node_Content</node_name>
	 */
	public void testGenerate_fromHTMLNode_StringString() {
		...
		assertEquals(("<" + SAMPLE_OKAY_NODE_NAME_NON_EMPTY + ">").toLowerCase() + SAMPLE_NODE_CONTENT
				+ ("</" + SAMPLE_OKAY_NODE_NAME_NON_EMPTY + ">").toLowerCase(), methodHTMLNode.generate());
	}

	/**
	 * Tests generate() method with a HTMLNode with the name, an attribute and its value.
	...
	public void testGenerate_fromHTMLNode_StringStringString() {
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
				SAMPLE_ATTRIBUTE_VALUE);
		assertEquals(generateNoContentNodeOutput(SAMPLE_OKAY_NODE_NAME_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
				SAMPLE_ATTRIBUTE_VALUE), methodHTMLNode.generate());
	}
	...
	public void testGenerate_fromHTMLNode_StringStringStringString() {
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
				SAMPLE_ATTRIBUTE_VALUE, SAMPLE_NODE_CONTENT);
		assertEquals(generateFullNodeOutput(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
				SAMPLE_ATTRIBUTE_VALUE, SAMPLE_NODE_CONTENT), methodHTMLNode.generate());
	}

	/**
	 * Generates the correct output for the HTMLNode.generate() method when called from a
	 * single node having only a name and an attribute name and value
	...
	private String generateNoContentNodeOutput(String aName, String aAttributeName, String aAttributeValue) {
		return ("<" + aName + " ").toLowerCase() + aAttributeName + "=" + "\"" + aAttributeValue + "\"" + " />";
	}
	...
	/**
	 * Generates the correct output for the HTMLNode.generate() method when called from a
	 * single node having the specified parameters
	 * @param aName the HTMLNode name
	 * @param aAttributeName the HTMLNode attribute name
	 * @param aAttributeValue the HTMLNode attribute value
	...
	/**
	 * Tests generate() method with a HTMLNode that has a child.
	 * <node_name Attribute_Name="Attribute_Value">Node_Content
	 * <child_node_name child_Attribute_Name=
	 * "child_Attribute_Value">child_Node_Content</child_node_name> </node_name>
	 */
	public void testGenerate_HTMLNode_withChild() {
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
				SAMPLE_ATTRIBUTE_VALUE, SAMPLE_NODE_CONTENT);
		HTMLNode methodHTMLNodeChild = new HTMLNode(SAMPLE_OKAY_NODE_NAME_NON_EMPTY, SAMPLE_OKAY_ATTRIBUTE_NAME,
				SAMPLE_ATTRIBUTE_VALUE, SAMPLE_NODE_CONTENT);

		methodHTMLNode.addChild(methodHTMLNodeChild);

		assertEquals(("<" + SAMPLE_OKAY_NODE_NAME_NON_EMPTY + " ").toLowerCase() + SAMPLE_OKAY_ATTRIBUTE_NAME + "="
				+ "\"" + SAMPLE_ATTRIBUTE_VALUE + "\">" + SAMPLE_NODE_CONTENT +
	...
	/**
	 * Tests generate() method with a HTMLNode that has "%" as name. The expected output
	 * is just the HTMLNode content
	 */
	public void testGenerate_fromHTMLNode_percentName() {
		HTMLNode methodHTMLNode = new HTMLNode("%", SAMPLE_OKAY_ATTRIBUTE_NAME, SAMPLE_ATTRIBUTE_VALUE,
				SAMPLE_NODE_CONTENT);
		assertEquals(SAMPLE_NODE_CONTENT, methodHTMLNode.generate());
	}

	/**
	 * Tests HTMLDoctype.generate() method comparing the result with the expected String.
	 * It is useful for regression tests.
	 */
