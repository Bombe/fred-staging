...
public class SimpleFieldSetTest extends TestCase {
	...
	/*
	 * A double string array used across all tests it must not be changed in order to
	...
	/**
	 * Tests putSingle(String,String) method trying to store a key with two paired
	...
	/**
	 * Tests putAppend(String,String) method trying to store a key with two paired
	 * multi_level_chars (i.e. "..").
	 */
	...
	/**
	 * Tests put() and get() methods using a normal Map behaviour and without
	 * MULTI_LEVEL_CHARs
	 */
	public void testSimpleFieldSetPutAndGet_NoMultiLevel() {
		String[][] methodPairsArray = { { "A", "a" }, { "B", "b" }, { "C", "c" }, { "D", "d" }, { "E", "e" },
				{ "F", "f" } };
		assertTrue(checkPutAndGetPairs(methodPairsArray));
	}

	/**
	 * Tests put() and get() methods using a normal Map behaviour and with
	 * MULTI_LEVEL_CHARs
	 */
	public void testSimpleFieldSetPutAndGet_MultiLevel() {
		String[][] methodPairsArray_DoubleLevel = { { "A.A", "aa" }, { "A.B", "ab" }, { "A.C", "ac" }, { "A.D", "ad" },
				{ "A.E", "ae" }, { "A.F", "af" } };
		String[][] methodPairsArray_MultiLevel = { { "A.A.A.A", "aa" }, { "A.B.A", "ab" }, { "A.C.Cc", "ac" },
				{ "A.D.F", "ad" }, { "A.E.G", "ae" }, { "A.F.J.II.UI.BOO", "af" } };
		assertTrue(checkPutAndGetPairs(methodPairsArray_DoubleLevel));
		assertTrue(checkPutAndGetPairs(methodPairsArray_MultiLevel));
	}

	/**
	 * It puts key-value pairs in a SimpleFieldSet and verify if it can do the
	 * correspondant get correctly.
	 * @param aPairsArray
	...
	/**
	 * Tests subset(String) method putting two levels keys and fetching it through
	 * subset() method on the first level and then get() on the second
	 */
	public void testSimpleFieldSetSubset_String() {
		SimpleFieldSet methodSFS = new SimpleFieldSet(true);
		String[][] methodPairsArray_MultiLevel = { { "A", "A", "aa" }, { "A", "B", "ab" }, { "A", "C", "ac" },
		...
		for (int i = 0; i < methodPairsArray_MultiLevel.length; i++)
			assertEquals(methodSFS.subset(methodPairsArray_MultiLevel[i][0]).get(methodPairsArray_MultiLevel[i][1]),
					methodPairsArray_MultiLevel[i][2]);
		assertTrue(checkSimpleFieldSetSize(methodSFS, methodPairsArray_MultiLevel.length));
	}

	/**
	 * Tests putAllOverwrite(SimpleFieldSet) method trying to overwrite a whole
	 * SimpleFieldSet with another with same keys but different values
	 */
	public void testPutAllOverwrite() {
		...
		for (int i = 0; i < SAMPLE_STRING_PAIRS.length; i++)
			assertEquals(nullSFS.get(SAMPLE_STRING_PAIRS[i][0]), SAMPLE_STRING_PAIRS[i][1] + methodAppendedString);
	}
	...
	/**
	 * Tests put(String,SimpleFieldSet) and tput(String,SimpleFieldSet) trying to add
	...
	/**
	 * It creates a SFS from the SAMPLE_STRING_PAIRS and putting a suffix after every
	 * value
	...
	/**
	 * Tests put(String,boolean) and getBoolean(String,boolean) methods consistency. The
	 * default value (returned if the key is not found) is set to "false" and the real
	 * value is always set to "true", so we are sure if it finds the right value or not
	...
	/**
	 * Tests put(String,int) and [getInt(String),getInt(String,int)] methods consistency.
	 * The default value (returned if the key is not found) is set to a not present int
	 * value, so we are sure if it finds the right value or not (and does not use the
	 * default).
	 */
	...
	/**
	 * Tests put(String,long) and [getLong(String),getLong(String,long)] methods
	 * consistency. The default value (returned if the key is not found) is set to a not
	 * present long value, so we are sure if it finds the right value or not (and does not
	 * use the default).
	 */
	...
	/**
	 * Tests put(String,char) and [getChar(String),getChar(String,char)] methods
	 * consistency. The default value (returned if the key is not found) is set to a not
	 * present char value, so we are sure if it finds the right value or not (and does not
	 * use the default).
	 */
	...
	/**
	 * Tests put(String,short) and [getShort(String)|getShort(String,short)] methods
	 * consistency. The default value (returned if the key is not found) is set to a not
	 * present short value, so we are sure if it finds the right value or not (and does
	 * not use the default).
	 */
	...
	/**
	 * Tests put(String,double) and [getDouble(String)|getDouble(String,double)] methods
	 * consistency. The default value (returned if the key is not found) is set to a not
	 * present double value, so we are sure if it finds the right value or not (and does
	 * not use the default).
	 */
	public void testPut_StringDouble() {
		...
		for (int i = 0; i < methodPairsArray.length; i++) {
			try {
				// there is no assertEquals(Double,Double) so we are obliged to do this
				// way -_-
				assertEquals(Double.compare((methodSFS.getDouble(Double.toString(methodPairsArray[i][0]))),
						methodPairsArray[i][1]), 0);
				assertEquals(Double.compare(methodSFS.getDouble(Double.toString(methodPairsArray[i][0]), 5),
						methodPairsArray[i][1]), 0);
			}
			catch (FSParseException aException) {
				fail("Not expected exception thrown : " + aException.getMessage());
	...
	/**
	 * Tests SimpleFieldSet(String,boolean,boolean) constructor, with simple and border
	...
	/**
	 * Tests SimpleFieldSet(BufferedReader,boolean,boolean) constructor, with simple and
	 * border cases of the canonical form.
	 */
	public void testSimpleFieldSet_BufferedReaderBooleanBoolean() {
		String[][] methodStringPairs = SAMPLE_STRING_PAIRS;
		BufferedReader methodBufferedReader = new BufferedReader(new StringReader(sfsReadyString(methodStringPairs)));
		try {
	...
	/**
	 * Tests SimpleFieldSet(SimpleFieldSet) constructor, with simple and border cases of
	 * the canonical form.
	 */
	...
	/**
	 * Tests directSubsetNameIterator() method. It uses SAMPLE_STRING_PAIRS and for this
	 * reason the expected subset is "foo".
	 */
	...
	public void testPutAppend() {
		String methodKey = "foo.bar";
		String[] methodValues = { "boo", "bar", "zoo" };
		String expectedResult = "boo" + SimpleFieldSet.MULTI_VALUE_CHAR + "bar" + SimpleFieldSet.MULTI_VALUE_CHAR
				+ "zoo";
		SimpleFieldSet methodSFS = new SimpleFieldSet(true);
		for (int i = 0; i < methodValues.length; i++)
	...
	/**
	 * Verifies if all keys in a String[][] (We consider that keys are stored in
	 * String[x][0]) are the same that the Iterator provides. In this way both hasNext()
	 * and next() methods are tested.
	 * @param aStringPairsArray
	 * @param aPrefix that could be put before found key
	 * @param aIterator
	...
	/**
	 * Tests the Iterator given for the SimpleFieldSet class. It tests hasNext() and
	 * next() methods.
	 */
	...
	/**
	 * Tests the toplevelIterator given for the SimpleFieldSet class. It tests hasNext()
	 * and next() methods.
	 *
	...
	public void testSplit() {
		...
		assertTrue(Arrays.equals(SimpleFieldSet.split(";blah;1;2;;"), new String[] { "", "blah", "1", "2", "", "" }));
		assertTrue(
				Arrays.equals(SimpleFieldSet.split(";;blah;1;2;;"), new String[] { "", "", "blah", "1", "2", "", "" }));
