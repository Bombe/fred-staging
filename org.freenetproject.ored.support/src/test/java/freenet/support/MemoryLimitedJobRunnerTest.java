...
public class MemoryLimitedJobRunnerTest extends TestCase {
	...
	private void innerTestQueueingSmallDelayed(int JOB_SIZE, int JOB_COUNT, int JOB_LIMIT, boolean startLive)
		...
		// If it all fits, run all the jobs at once. If some are going to be queued, use a
		// small thread limit.
	...
	private void innerTestAsyncQueueingSmallDelayed(int JOB_SIZE, int JOB_COUNT, int JOB_LIMIT, boolean startLive)
			throws InterruptedException {
		SynchronousJob[] jobs = new SynchronousJob[JOB_COUNT];
		final Object completion = new Object();
		for (int i = 0; i < jobs.length; i++)
