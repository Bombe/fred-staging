...
public class WrapperConfig {
	...
	public static synchronized boolean setWrapperProperty(String name, String value) {
		// Some of this copied from UpdateDeployContext, hence no GPL header on this file
		// as none there.
		...
		if (!newConfig.renameTo(oldConfig)) {
			...
			if (oldOldConfig.exists() && !oldOldConfig.delete())
				try {
					...
					String error = "Unable to create temporary file and unable to copy wrapper.conf to wrapper.conf.old. Could not update wrapper.conf trying to set property "
							+ name;
			...
			if (!oldConfig.renameTo(oldOldConfig)) {
				String error = "Unable to change property " + name + ": Could not move old config file " + oldConfig
						+ ", so could not rename new config file " + newConfig
				...
				if (!oldOldConfig.renameTo(oldConfig)) {
					System.err.println(
							"CATASTROPHIC UPDATE ERROR: Unable to rename backup copy of config file over the current config file, after failing to update config file! The node will not boot until you get a new wrapper.conf!\n"
									+ "The old config file is saved in " + oldOldConfig
