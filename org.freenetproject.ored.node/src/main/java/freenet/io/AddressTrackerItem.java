...
/**
 * Tracks communication to/from a specific address. That address can be a specific
 * IP:port, a specific IP, or some completely different type of address, so we don't store
 * it in this class; subclasses will do.
 *
 * @author toad
 */
public class AddressTrackerItem {
	...
	/**
	 * The earliest time, before timeFirstReceivedPacket, at which we know for certain
	 * that there was no packet received. This is typically the startup time of the server
	 * socket. It may be later if the cache has to be flushed.
	 */
	private long timeDefinitelyNoPacketsReceived;

	/**
	 * The earliest time, before timeFirstSentPacket, at which we know for certain that
	 * there was no packet sent. This is typically the startup time of the node. It may be
	 * later if the cache has to be flushed.
	 */
