 ...
 * If created with a name, then the name is primary, and the IP address can change. Most
 * code ripped from Peer.
 *
 * Propagates the IP address on equals() but not the hostname. This does not change
 * hashCode() because it only happens if hostname is set, and in that case, hashCode() is
 * based on the hostname and not on the IP address. So it is safe to put
 * FreenetInetAddress's into hashtables: neither equals() nor getAddress() will change its
 * hashCode.
 *
 * BUT a FreenetInetAddress with IP 1.2.3.4 and no hostname is *NOT* equal to one with the
 * IP address and no name. So if you want to match on only the IP address, you need to
 * either call dropHostname() first (after which neither propagation nor getAddress() will
 * change the hashcode), or just use InetAddress's.
 *
...
public class FreenetInetAddress {
	...
	/**
	 * Create from an InetAddress. The IP address is primary i.e. fixed. The hostname
	 * either doesn't exist, or is looked up.
	 */
	...
	public FreenetInetAddress(String host, boolean allowUnknown) throws UnknownHostException {
		...
		// debugging log messages because AddressIdentifier doesn't appear to handle all
		// IPv6 literals correctly, such as "fe80::204:1234:dead:beef"
		...
		if (addressType != AddressIdentifier.AddressType.OTHER) {
			...
			// Don't catch UnknownHostException here, if it happens there's a bug in
			// AddressIdentifier.
			if (logDEBUG)
				Logger.debug(this,
	...
	/**
	 * Get the IP address. Look it up if necessary, but return the last value if it has
	 * ever been looked up before; will not trigger a new lookup if it has been looked up
	 * before.
	 */
	 ...
	 * has ever been looked up before; will not trigger a new lookup if it has been looked
	 * up before.
	 */
	...
	/**
	 * Get the IP address, looking up the hostname if the hostname is primary, even if it
	 * has been looked up before. Typically called on a reconnect attempt, when the dyndns
	...
	public InetAddress getHandshakeAddress() {
		...
		if ((_address != null) && (hostname == null)) {
			...
			/*
			 * Peers are constructed from an address once a handshake has been completed,
			 * so this lookup will only be performed during a handshake (this method
			 * should normally only be called from PeerNode.getHandshakeIPs() and once
			 * each connection from this.getAddress() otherwise) - it doesn't mean we
			 * perform a DNS lookup with every packet we send.
			 */
			try {
				...
				if (addr != null) {
					/*
					 * cache the answer since getHandshakeAddress() doesn't use the cached
					 * value, thus getHandshakeIPs() should always get the latest value
					 * from DNS (minus Java's caching)
					 */
	...
	public int hashCode() {
		if (hostname != null) {
			return hostname.hashCode(); // Was set at creation, so it can safely be used
										// here.
		}
		else {
			return _address.hashCode(); // Can be null, but if so, hostname will be
										// non-null.
		}
	}
	...
	/**
	 * Return the hostname or the IP address of the given InetAddress. Does not attempt to
	 * do a reverse lookup; if the hostname is known, return it, otherwise return the
	 * textual IP address.
	 */
	 ...
	 * @return a new <code>FreenetInetAddress</code> with host name removed; or
	 * {@code null} if no known ip address is associated with this object. You may want to
	 * do a <code>getAddress(true)</code> before calling this.
	 */
