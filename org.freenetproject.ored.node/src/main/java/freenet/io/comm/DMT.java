...
public final class DMT {
	 ...
	 * time to send it by not accepting an infeasible number of requests; starvation will
	 * cause bwlimitDelayTime to go high and requests to be rejected. That's the ultimate
	 * limiter if even output bandwidth liability fails.
	 */
	...
	public static int packetTransmitSize(int size, int _packets) {
		return size + 8 /* uid */ + 4 /* packet# */ + BitArray.serializedLength(_packets)
				+ 4 /* Message header */;
	}
	...
	// This is of priority BULK_DATA to cut down on suprious resend requests, it will be
	...
	// CHKs have a single DataInsert (followed by a transfer), whereas SSKs have 2-3
	// messages,
	...
	// FIXME consider using this again now we have new packet format which can handle big
	// messages on any connection.
	// FIXME be careful if we do - make sure boost priority if realtime, and note timeout
	// issues associated with sending a request at BULK.
	...
	public static Message createFNPSSKInsertRequest(long uid, short htl, NodeSSK myKey, byte[] headers, byte[] data,
			byte[] pubKeyHash, boolean realTime) {
	...
	/**
	 * Sent when a request to an opennet node is completed, but the data source does not
	...
	public static Message createFNPOpennetConnectDestinationNew(long uid, long transferUID, int noderefLength,
			int paddedLength) {
	...
	/**
	 * Path folding response. Sent when the requestor wants to path fold and has received
	...
	/**
	 * Announcement reply. Noderef is attached, will be transferred, both nodes add the
	 * other. A single request will result in many reply's. When the announcement is done,
	...
	/**
	 * Creates a probe response which indicates that the endpoint opted not to respond
	 ...
	 * @param uid Probe identifier.
	 * @param uptimePercent Percent of the requested period (48 hours or 7 days) which the
	 * endpoint was online.
	 * @return Message with requested attributes.
	 */
	...
	// More permanent secondary messages (should perhaps be replaced by new main messages
	// when stable)
	...
	/**
	 * Maximum transfers out, hard limit based on congestion control; we will be rejected
	 * if our usage is over this.
	 */
	public static final String MAX_TRANSFERS_OUT = "maxTransfersOut";

	/**
	 * Maximum transfers out, peer limit. If total is over the lower limit and our usage
	 * is over the peer limit, we will be rejected.
	 */
	public static final String MAX_TRANSFERS_OUT_PEER_LIMIT = "maxTransfersOutPeerLimit";

	/**
	 * Maximum transfers out, lower limit. If total is over the lower limit and our usage
	 * is over the peer limit, we will be rejected.
	 */
	public static final String MAX_TRANSFERS_OUT_LOWER_LIMIT = "maxTransfersOutLowerLimit";

	/**
	 * Maximum transfers out, upper limit. If total is over the upper limit, everything is
	 * rejected.
	 */
