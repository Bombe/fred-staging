...
public class MessageCore {
	...
	private static final long MAX_UNCLAIMED_FIFO_ITEM_LIFETIME = MINUTES.toMillis(10); // maybe
																						// this
																						// should
																						// be
																						// per
	...
	// FIXME may not work well for newly added filters with timeouts close to the minimum,
	// or filters with timeouts close to the minimum in general.
	...
	long removeTimedOutFilters(long nextTimeout) {
		...
		// Avoids exhaustive and unsuccessful search in waitFor() removal of a timed out
		// filter.
		if (logMINOR)
		...
		synchronized (_filters) {
			for (ListIterator<MessageFilter> i = _filters.listIterator(); i.hasNext();) {
					...
					if (logMINOR) {
						for (ListIterator<Message> it = _unclaimed.listIterator(); it.hasNext();) {
							...
							if (status == MATCHED.MATCHED) {
								// Don't match it, we timed out; two-level timeouts etc
								// may want it for the next filter.
								Logger.error(this,
										"Timed out but should have matched in _unclaimed: " + m + " for " + f);
								break;
							}
						}
					}
				...
				// Do not break after finding a non-timed-out filter because some filters
				// may
				// be timed out because their client callbacks say they should be.
				// Also simplifies the logic significantly, we've had some major bugs
				// here.
	...
	/**
	 * Dispatch a message to a waiting filter, or feed it to the Dispatcher if none are
	 * found.
	...
	public void checkFilters(Message m, PacketSocketHandler from) {
		...
		if (logMINOR && !(m.getSpec().equals(DMT.packetTransmit))) {
			Logger.minor(this,
					"" + (System.currentTimeMillis() % 60000) + ' ' + from + " <- " + m.getSource() + " : " + m);
		}
		...
		synchronized (_filters) {
			for (ListIterator<MessageFilter> i = _filters.listIterator(); i.hasNext();) {
				...
				if (status == MATCHED.TIMED_OUT || status == MATCHED.TIMED_OUT_AND_MATCHED) {
					...
					i.remove();
					match = f;
					// We must setMessage() inside the lock to ensure that waitFor() sees
					// it even if it times out.
					f.setMessage(m);
					if (logMINOR)
		...
		if (timedOut != null)
			timedOut.clear();
		// Keep the last few _unclaimed messages around in case the intended receiver
		// isn't receiving yet
		if (!matched) {
			if (logMINOR)
				Logger.minor(this, "Unclaimed: " + m);
			/**
			 * Check filters and then add to _unmatched is ATOMIC It has to be atomic,
			 * because otherwise we can get a race condition that results in timeouts on
			 * MFs.
			 *
			 * Specifically: - Thread A receives packet - Thread A checks filters. It
			 * doesn't match any. - Thread A feeds to Dispatcher. - Thread B creates
			 * filter. - Thread B checks _unmatched. - Thread B adds filter. - Thread B
			 * sleeps. - Thread A returns from Dispatcher. Which didn't match. - Thread A
			 * adds to _unmatched.
			 *
			 * OOPS! The only way to fix this is to have checking the filters and
			 * unmatched be a single atomic operation. Another race is possible if we
			 * merely recheck the filters after we return from dispatcher, for example.
			 */
			synchronized (_filters) {
				if (logMINOR)
				...
				if (!matched) {
					while (_unclaimed.size() > MAX_UNMATCHED_FIFO_SIZE) {
						...
						if ((removed.getSource()) instanceof PeerNode) {
							...
							Logger.normal(this, "Dropping unclaimed, lived "
									+ TimeUtil.formatTime(messageLifeTime, 2, true) + " (quantity)" + ": " + removed);
						}
					}
	...
	public void onDisconnect(PeerContext ctx) {
		ArrayList<MessageFilter> droppedFilters = null; // rare operation, we can waste
	...
	public void addAsyncFilter(MessageFilter filter, AsyncMessageFilterCallback callback, ByteCounter ctr)
		...
		// Drop any _unclaimed messages that the filter doesn't match that are also older
		// than MAX_UNCLAIMED_FIFO_ITEM_LIFETIME
		long now = System.currentTimeMillis();
		long messageDropTime = now - MAX_UNCLAIMED_FIFO_ITEM_LIFETIME;
		long messageLifeTime = 0;
		long timeout = filter.getTimeout();
		synchronized (_filters) {
			// Once in the list, it is up to the callback system to trigger the
			// disconnection, however, we may
			// have disconnected between check above and locking, so we *must* check
			// again.
			if (filter.anyConnectionsDropped()) {
				throw new DisconnectedException();
				// or... filter.onDroppedConnection(filter.droppedConnection());
			...
			for (ListIterator<Message> i = _unclaimed.listIterator(); i.hasNext();) {
				Message m = i.next();
				// These messages have already arrived, so we can match against them even
				// if we are timed out.
				...
				if (status == MATCHED.MATCHED) {
					...
					if ((m.getSource()) instanceof PeerNode) {
						...
						Logger.normal(this, "Dropping unclaimed, lived " + TimeUtil.formatTime(messageLifeTime, 2, true)
								+ " (age)" + ": " + m);
					}
				}
			}
	...
	/**
	 * Wait for a filter to trigger, or timeout. Blocks until either the trigger is
	 * activated, or it times out, or the peer is disconnected.
	...
	public Message waitFor(MessageFilter filter, ByteCounter ctr) throws DisconnectedException {
		...
		if (ret == null) {
			...
			synchronized (filter) {
				try {
					// Precaution against filter getting matched between being added to
					// _filters and
					// here - bug discovered by Mason
					// Check reallyTimedOut() too a) for paranoia, b) for filters with a
					// callback (we could conceivably waitFor() them).
		...
		synchronized (_filters) {
			...
			// Hence we need to be absolutely sure that when we remove it it hasn't been
			// matched.
			// Note also that the locking does work here - the filter lock is taken last,
			// and
			...
			// A filter being waitFor()'ed cannot have any callbacks, so we don't need to
			// call onMatched().
		}
