...
public class IOStatisticCollector {
	...
	private void _addInfo(InetAddress addr, int port, int inbytes, int outbytes, boolean isLocal) {
		...
		if (!isLocal) {
			synchronized (this) {
				...
				if (logDEBUG)
					Logger.debug(IOStatisticCollector.class, "Add(" + addr + ":" + port + ',' + inbytes + ',' + outbytes
	...
	/*
	 * to thead each update.... heavy stuff private class StatisticUpdater implements
	 * Runnable { private IOStatisticCollector sc; private String key; private int
	 * inbytes; private int outbytes;
	 *
	 * public StatisticUpdater(IOStatisticCollector sc, String key, int inbytes, int
	 * outbytes) { this.sc = sc; this.key = key; this.inbytes = inbytes; this.outbytes =
	 * outbytes; new Thread(this, "IOStatisticCollector$StatisticUpdater").run(); }
	 *
	 * public void run() {
	 *
	 * } }
