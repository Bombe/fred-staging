...
public final class MessageFilter {
	...
	/**
	 * If true, timeouts are relative to the start of waiting, if false, they are relative
	 * to the time of calling setTimeout()
	 */
	...
	void onStartWaiting(boolean waitFor) {
		synchronized (this) {
			/*
			 * We cannot wait on a MessageFilter with a callback, because onMatched()
			 * calls clearMatched() if we have a callback. The solution would be to: - Set
			 * a flag indicating we are waitFor()ing a filter here. - On matching a
			 * message (setMessage), call the callback immediately if not waitFor()ing. -
			 * If we are waitFor()ing, call the callback when we exit waitFor()
			 * (onStopWaiting()???).
			 */
	 ...
	 * waitFor().
	 * @param b If true, the timeout is relative to the time at which setTimeout() was
	 * called, if false, it's relative to the start of waitFor().
	 */
	...
	/**
	 * This filter will expire after the specificed amount of time. Note also that where
	 * two or more filters match the same message, the one with the nearer expiry time
	 * will get priority
	...
	/**
	 * Modifies the filter so that it returns true if either it or the filter in the
	 * argument returns true. Multiple combinations must be nested: such as
	...
	public MessageFilter or(MessageFilter or) {
		...
		if (or._initialTimeout != _initialTimeout) {
			Logger.error(this, "Message filters being or()ed have different timeouts! This is very dangerous! This is "
					+ this + " or is " + or);
	...
	enum MATCHED {
		...
		MATCHED, TIMED_OUT, TIMED_OUT_AND_MATCHED, NONE
	...
	public MATCHED match(Message m, boolean noTimeout, long now) {
		...
		if ((_type != null && !_type.equals(m.getSpec())) || (_source != null && !_source.equals(m.getSource()))) {
			// Timeout immediately, but don't check the callback, so we still need the
			// periodic check.
	 ...
	 * @return True if the filter has timed out, or if it has been matched already. Caller
	 * will remove the filter from _filters if we return true.
	 */
	...
	public synchronized void setMessage(Message message) {
		...
		_message = message;
		// Avoid race conditions where it is removed from the filter list because of a
		// timeout but not woken up.
		_matched = true;
		notifyAll();
	}
	...
	/**
	 * Notify because of a dropped connection. Caller must verify
	 * _matchesDroppedConnection and _source.
	 * @param ctx
	 */
	...
	/**
	 * Notify because of a restarted connection. Caller must verify
	 * _matchesDroppedConnection and _source.
	 * @param ctx
	 */
	...
	/**
	 * Notify waiters that we have been matched. Hopefully no locks will be held at this
	 * point by the caller.
	 */
