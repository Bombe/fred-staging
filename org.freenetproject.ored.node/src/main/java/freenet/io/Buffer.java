...
public class Buffer implements WritableToDataOutputStream {
	...
	/**
	 * Create a Buffer by reading a DataInputStream. Note that this a) expects that the
	 * first 4 bytes to be a length indicator of the rest of the byte stream and b) these
	 * first 4 bytes are removed from the byte stream before storing the rest
	...
	/**
	 * Retrieve a byte array containing the data in this buffer. Will be copied if the
	 * data does not fill the whole array, so please don't rely on it being the internal
	 * buffer.
