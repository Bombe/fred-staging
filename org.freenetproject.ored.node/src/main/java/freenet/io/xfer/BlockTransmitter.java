 ...
 * @author ian
 *
 * Given a PartiallyReceivedBlock retransmit to another node (to be received by
 * BlockReceiver). Since a PRB can be concurrently transmitted to many peers NOWHERE in
 * this class is prb.abort() to be called.
 *
...
public class BlockTransmitter {
	...
	class BlockSenderJob implements PrioRunnable {
					...
					synchronized (_senderThread) {
						...
						if (_unsent.size() == 0) {
							...
							if (_sentPackets.bitAt(packetNo)) {
								Logger.error(
										this, "Already sent packet in run(): " + packetNo + " for " + this
												+ " unsent is " + _unsent + " sent is " + _sentPackets,
										new Exception("error"));
								continue;
							}
						}
		...
		public void schedule() {
			if (_failed || _receivedSendCompletion || _completed) {
				if (logMINOR)
					Logger.minor(this,
							"Not scheduling for " + _uid + " to " + _destination + " :" + (_failed ? "(failed) " : "")
			...
			timeoutJob = new PrioRunnable() {
				...
				public void run() {
					...
					synchronized (_senderThread) {
						...
						// transmitted, terminate the send.
						if (_failed) {
							// Already failed, we were just waiting for the
							// acknowledgement sendAborted.
							...
							Logger.warning(this,
									"Terminating send " + _uid + " to " + _destination + " from "
											+ _destination.getSocketHandler() + " as we haven't heard from receiver in "
											+ timeString + '.');
	 ...
	 * called with _senderThread held. Caller must call the callback then call cleanup()
	 * outside the lock if this returns true.
	 */
	public boolean maybeComplete() {
		if (!_receivedSendCompletion) {
			if (logMINOR)
				Logger.minor(this, "maybeComplete() not completing because send not completed on " + this);
			// All the block sends have completed, wait for the other side to acknowledge
			// or timeout.
	 ...
	 * that we don't need to (e.g. on disconnection), the caller must call
	 * prepareSendAborted afterwards, and if that returns true, send the sendAborted via
	 * innerSendAborted. LOCKING: Must be called inside the _senderThread lock.
	...
	public Future maybeFail(final int reason, final String description) {
		...
		_failed = true;
		if (!_receivedSendCompletion) {
			// Don't actually timeout until after we have an acknowledgement of the
			// transfer cancel.
			// This is important for keeping track of how many transfers are actually
			// running, which will be important for load management later on.
			// The caller will immediately call prepareSendAbort() then
			// innerSendAborted().
			if (logMINOR)
		...
		if (blockSendsPending != 0) {
			...
			if (_sentSendAborted)
				...
				_sentSendAborted = true;
				// They have sent us a cancel, but we still need to send them an ack or
				// they will do a fatal timeout.
				return new Future() {
	...
	private AsyncMessageFilterCallback cbAllReceived = new SlowAsyncMessageFilterCallback() {
		...
		public void onMatched(Message m) {
			if (logMINOR) {
				...
				synchronized (avgTimeTaken) {
					...
					Logger.minor(this,
							"Block send took " + transferTime + " : " + avgTimeTaken + " on " + BlockTransmitter.this);
				}
			}
			synchronized (_senderThread) {
				_receivedSendCompletion = true;
		...
		public boolean shouldTimeout() {
			synchronized (_senderThread) {
				// We are waiting for the send completion, which is set on timeout as well
				// as on receiving a message.
				// In some corner cases we might want to get the allReceived after setting
				// _failed, so don't timeout on _failed.
				// We do want to timeout on _completed because that means everything is
				// finished - it is only set in maybeComplete() and maybeFail().
	...
	private AsyncMessageFilterCallback cbSendAborted = new SlowAsyncMessageFilterCallback() {
		...
		public boolean shouldTimeout() {
			synchronized (_senderThread) {
				// We are waiting for the send completion, which is set on timeout as well
				// as on receiving a message.
				// We don't want to timeout on _failed because we can set _failed, send
				// sendAborted, and then wait for the acknowledging sendAborted.
				// We do want to timeout on _completed because that means everything is
				// finished - it is only set in maybeComplete() and maybeFail().
				if (_receivedSendCompletion || _completed)
					return true;
	...
	public void sendAsync() {
		...
		try {
					...
					public void packetReceived(int packetNo) {
						synchronized (_senderThread) {
							...
							if (_unsent.contains(packetNo)) {
								Logger.error(this,
										"Already in unsent: " + packetNo + " for " + this + " unsent is " + _unsent,
										new Exception("error"));
								return;
							}
							if (_sentPackets.bitAt(packetNo)) {
								Logger.error(this,
										"Already sent packet in packetReceived: " + packetNo + " for " + this
												+ " unsent is " + _unsent + " sent is " + _sentPackets,
										new Exception("error"));
								return;
							}
			...
			MessageFilter mfAllReceived = MessageFilter.create().setType(DMT.allReceived).setField(DMT.UID, _uid)
					.setSource(_destination).setNoTimeout();
	...
	private void cleanup() {
		...
		// shouldTimeout() should deal with them adequately, maybe we don't need to
		...
		private void complete(boolean failed) {
			...
			synchronized (_senderThread) {
				...
				if (lastSentPacket > 0) {
					...
					if (delta > threshold)
						Logger.warning(this, "Time between packets on " + BlockTransmitter.this + " : "
								+ TimeUtil.formatTime(delta, 2, true) + " ( " + delta + "ms) realtime=" + realTime);
					else if (delta > threshold / 5)
						Logger.normal(this, "Time between packets on " + BlockTransmitter.this + " : "
								+ TimeUtil.formatTime(delta, 2, true) + " ( " + delta + "ms) realtime=" + realTime);
					else if (logMINOR)
						Logger.minor(this, "Time between packets on " + BlockTransmitter.this + " : "
								+ TimeUtil.formatTime(delta, 2, true) + " ( " + delta + "ms) realtime=" + realTime);
				}
				lastSentPacket = now;
				blockSendsPending--;
