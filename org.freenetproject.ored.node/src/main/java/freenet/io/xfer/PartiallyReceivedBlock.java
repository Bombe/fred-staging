...
public class PartiallyReceivedBlock {
		...
		synchronized (this) {
			...
			if (packet.getLength() != _packetSize) {
				throw new RuntimeException(
						"New packet size " + packet.getLength() + " but expecting packet of size " + _packetSize);
			}
	 ...
	 * @param reason
	 * @param description
	 * @param cancelledLocally If true, the transfer was deliberately aborted by *this
	 * node*, not as the result of a timeout.
	...
	public byte[] abort(int reason, String description, boolean cancelledLocally) {
		...
		synchronized (this) {
			...
			Logger.normal(this, "Aborting PRB: " + reason + " : " + description + " on " + this,
					new Exception("debug"));
			_aborted = true;
