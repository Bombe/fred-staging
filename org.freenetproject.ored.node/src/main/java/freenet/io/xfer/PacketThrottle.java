...
public class PacketThrottle {
	...
	protected static final double PACKET_TRANSMIT_INCREMENT = (4
	...
	/**
	 * The size of the window, in packets. Window size must not drop below 1.0. Partly
	 * this is because we need to be able to send one packet, so it is a logical lower
	 * bound. But mostly it is because of the non-slow-start division by _windowSize!
	 */
	...
	/**
	 * Notify the throttle that a packet was transmitted successfully. We will increase
	 * the window size.
	 * @param maxWindowSize The maximum window size. This should be at least twice the
	 * largest window size actually seen in flight at any time so far. We will ensure that
	 * the throttle's window size does not get bigger than this. This works even for new
	 * packet format, and solves some of the problems that RFC 2861 does.
	 */
	public synchronized void notifyOfPacketAcknowledged(double maxWindowSize) {
		...
		if (slowStart) {
			...
			if (_windowSize > maxWindowSize)
				slowStart = false;
			// Window size must not drop below 1.0. Partly this is because we need to be
			// able to send one packet, so it is a logical lower bound.
		...
		// Ensure that we the window size does not grow dramatically larger than the
		// largest window
	...
	public synchronized String toString() {
		return Double.toString(getBandwidth()) + " k/sec, (w: " + _windowSize + ", r:" + _roundTripTime + ", d:"
	...
	/**
	 * returns the number of bytes-per-second in the transmition link (?). FIXME: Will not
	 * return more than 1M/s due to MIN_DELAY in getDelay().
	 */
