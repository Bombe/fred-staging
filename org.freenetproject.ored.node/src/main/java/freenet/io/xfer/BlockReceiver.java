...
public class BlockReceiver implements AsyncMessageFilterCallback {
	...
	public interface BlockReceiverTimeoutHandler {
		 ...
		 * timeout.
		 *
		 * Simple requests will need to implement this and transfer ownership of the
		 * request to this node, because the source node will end the request as soon as
		 * it sees the transfer cancel resulting from the PRB being cancelled; assigning
		 * the UID to ourselves keeps it consistent, and thus avoids severe load
	...
	/*
	 * RECEIPT_TIMEOUT must be less than 60 seconds because BlockTransmitter times out
	 * after not hearing from us in 60 seconds. Without contact from the transmitter, we
	 * will try sending at most MAX_CONSECUTIVE_MISSING_PACKET_REPORTS every
	 * RECEIPT_TIMEOUT to recover.
	 */
	...
	/**
	 * sendAborted is not sent at the realtime/bulk priority. Most of the two stage
	 * timeout stuff uses 60 seconds, it's a good number.
	 */
	 ...
	 * @param completeAfterAckedAllReceived If true, we need to call completion only after
	 * we have received an ack to the allReceived message. Generally, handlers want to
	 * complete early (=false), so the slot is freed up and can be reused by the other
	 * side; senders want to complete late, so they don't end up reusing the slot before
	 * the handler has completed (=true).
	 */
	public BlockReceiver(MessageCore usm, PeerContext sender, long uid, PartiallyReceivedBlock prb, ByteCounter ctr,
			Ticker ticker, boolean doTooLong, boolean realTime, BlockReceiverTimeoutHandler timeoutHandler,
	...
	// Turn off if e.g. we know that the PRB is already partially received when we start
	// the transfer.
	// This prevents malicious or broken nodes from trickling transfers forever by sending
	// the same packets over and over.
	...
	private AsyncMessageFilterCallback notificationWaiter = new SlowAsyncMessageFilterCallback() {
		...
		public void onMatched(Message m1) {
			...
			if ((m1 != null) && (m1.getSpec().equals(DMT.packetTransmit))) {
				...
				try {
					...
					if (CHECK_DUPES && _prb.isReceived(packetNo)) {
						...
						Logger.error(this, "Already received the packet - DoS??? on " + this + " uid " + _uid + " from "
								+ _sender);
						...
						if (logMINOR) {
							synchronized (BlockReceiver.this) {
								...
								Logger.minor(this, "Packet interval: " + interval + " = "
										+ TimeUtil.formatTime(interval, 2, true) + " from " + _sender);
							}
						}
			...
			try {
				// Even if timeout <= 0, we still add the filter, because we want to
				// receive any messages that are already buffered before we timeout.
		...
		public void onTimeout() {
			...
			try {
				...
				// It is important for load management that the two sides agree on the
				// number of transfers happening.
				// Therefore we need to not complete until the other side has acknowledged
				// that the transfer has been cancelled.
				MessageFilter mfSendAborted = MessageFilter.create().setTimeout(ACK_TRANSFER_FAILED_TIMEOUT)
						.setType(DMT.sendAborted).setField(DMT.UID, _uid).setSource(_sender);
				try {
	...
	private void complete(int reason, String description) {
		...
		if (logMINOR)
			Logger.minor(this, "Transfer failed: (" + (_realTime ? "realtime" : "bulk") + ") " + reason + " : "
					+ description + " on " + _uid + " from " + _sender);
	...
	private MessageFilter relevantMessages(long timeout) {
		MessageFilter mfPacketTransmit = MessageFilter.create().setTimeout(timeout).setType(DMT.packetTransmit)
				.setField(DMT.UID, _uid).setSource(_sender);
		MessageFilter mfAllSent = MessageFilter.create().setTimeout(timeout).setType(DMT.allSent)
				.setField(DMT.UID, _uid).setSource(_sender);
		MessageFilter mfSendAborted = MessageFilter.create().setTimeout(timeout).setType(DMT.sendAborted)
				.setField(DMT.UID, _uid).setSource(_sender);
	 ...
	 * allSent, is quite common, as the receive() routine usually quits immeadiately on
	 * receiving all packets. packetTransmit is less common, when receive() requested what
	 * it thought was a missing packet, only reordered.
	 */
	@Override
	public void onMatched(Message m) {
		if (logMINOR)
