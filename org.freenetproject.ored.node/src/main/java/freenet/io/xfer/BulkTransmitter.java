...
/**
 * Bulk data transfer (not block). Bulk transfer is designed for files which may be much
 * bigger than a key block, and where we have the whole file at the outset. Do not persist
...
public class BulkTransmitter {
	...
	/**
	 * If no packets sent in this period, and no completion acknowledgement /
	 * cancellation, assume failure.
	 */
	...
	/**
	 * Blocks we have but haven't sent yet. 0 = block sent or not present, 1 = block
	 * present but not sent
	 */
	...
	public BulkTransmitter(PartiallyReceivedBulk prb, PeerContext peer, long uid, boolean noWait, ByteCounter ctr,
			boolean realTime) throws DisconnectedException {
		this(prb, peer, uid, noWait, ctr, realTime, null);
	}

	/**
	 * Create a bulk data transmitter.
	 * @param prb The PartiallyReceivedBulk containing the file we want to send, or the
	 * part of it that we have so far.
	 * @param peer The peer we want to send it to.
	 * @param uid The unique identifier for this data transfer
	 * @param noWait If true, don't wait for an FNPBulkReceivedAll, return as soon as
	 * we've sent everything.
	 * @throws DisconnectedException If the peer we are trying to send to becomes
	 * disconnected.
	 */
	public BulkTransmitter(PartiallyReceivedBulk prb, PeerContext peer, long uid, boolean noWait, ByteCounter ctr,
			boolean realTime, AllSentCallback cb) throws DisconnectedException {
		this.prb = prb;
		this.peer = peer;
		this.uid = uid;
		this.noWait = noWait;
		...
		try {
			prb.usm.addAsyncFilter(MessageFilter.create().setNoTimeout().setSource(peer)
					.setType(DMT.FNPBulkReceiveAborted).setField(DMT.UID, uid), new AsyncMessageFilterCallback() {
						@Override
			...
			prb.usm.addAsyncFilter(MessageFilter.create().setNoTimeout().setSource(peer).setType(DMT.FNPBulkReceivedAll)
					.setField(DMT.UID, uid), new AsyncMessageFilterCallback() {
						@Override
						public void onMatched(Message m) {
	...
	/**
	 * Received a block. Set the relevant bit to 1 to indicate that we have the block but
	 * haven't sent it yet. **Only called by PartiallyReceivedBulk.**
	...
	public void cancel(String reason) {
		...
		// If there are packets still waiting, it will be called after they are sent or
		// failed.
		setAllQueued();
	}

	/**
	 * Like cancel(), but without the negative overtones: The client says it's got
	 * everything, we believe them (even if we haven't sent everything; maybe they had a
	 * partial).
	 */
	...
	public boolean send() throws DisconnectedException {
		...
		outer: while (true) {
			...
			// FIXME Need to introduce the global limiter of [code]max[/code] for memory
			// management instead of hard-code for each, no?
