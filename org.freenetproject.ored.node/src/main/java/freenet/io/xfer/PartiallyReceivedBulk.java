...
/**
 * Equivalent of PartiallyReceivedBlock, for large(ish) file transfers. As presently
 * implemented, we keep a bitmap in RAM of blocks received, so it should be adequate for
 * fairly large files (128kB for a 1GB file e.g.). We can compress this structure later on
 * if need be.
 *
 * @author toad
 */
public class PartiallyReceivedBulk {
	 ...
	 * @param initialState If true, assume all blocks have been received. If false, assume
	 * no blocks have been received.
	 */
	...
	/**
	 * Clone the blocksReceived BitArray. Used by BulkTransmitter to find what blocks are
	 * available on creation. BulkTransmitter will have already taken the lock and will
	...
	/**
	 * Add a BulkTransmitter to the list of BulkTransmitters. When a block comes in, we
	 * will tell each BulkTransmitter about it.
	...
	/**
	 * Called when a block has been received. Will copy the data from the provided buffer
	 * and store it.
	...
	void received(int blockNum, byte[] data, int offset, int length) {
		...
		synchronized (this) {
			if (blocksReceived.bitAt(blockNum))
				return; // ignore
			blocksReceived.setBit(blockNum, true); // assume the rest of the function
													// succeeds
			blocksReceivedCount++;
