...
/**
 * Bulk (not block) data transfer - receiver class. Bulk transfer is designed for largish
 * files, much larger than blocks, where we have the whole file at the outset.
 *
 * @author toad
 */
public class BulkReceiver {
	...
	public boolean receive() {
		while (true) {
			MessageFilter mfSendKilled = MessageFilter.create().setSource(peer).setType(DMT.FNPBulkSendAborted)
					.setField(DMT.UID, uid).setTimeout(TIMEOUT);
			MessageFilter mfPacket = MessageFilter.create().setSource(peer).setType(DMT.FNPBulkPacketSend)
					.setField(DMT.UID, uid).setTimeout(TIMEOUT);
			if (prb.hasWholeFile()) {
				try {
