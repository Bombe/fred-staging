 ...
 *
 * Indicates: - Whether there is no UDP connectivity at all. - Whether there is full
 * inbound IP connectivity. - A list of detected public IPs.
 */
public class DetectedIP {
	...
	/**
	 * Full cone NAT. Once we have sent a packet out on a port, any node anywhere can send
	 * us a packet on that port. The nicest option, but very rare unfortunately.
	 */
	public static final short FULL_CONE_NAT = 3;

	/**
	 * Restricted cone NAT. Once we have sent a packet out to a specific IP, it can send
	 * us packets on the port we just used.
	 */
	public static final short RESTRICTED_CONE_NAT = 4;

	/**
	 * Port restricted cone NAT. Once we have sent a packet to a specific IP+Port, that
	 * IP+Port can send us packets on the port we just used.
	 */
	public static final short PORT_RESTRICTED_NAT = 5;

	/**
	 * Symmetric NAT. Uses a separate port number for each IP+port ! Not much hope for
