...
public class PluginInfoWrapper implements Comparable<PluginInfoWrapper> {
	...
	public PluginInfoWrapper(Node node, FredPlugin plug, String filename, boolean isOfficial) throws IOException {
		...
		// TODO: Code quality: Do we really need to cache these values? I don't care about
		// the
		// memory overhead, but it's the pointless clutter it causes in the member
		// variables, while
		// the information is right there and always accessible in the runtime type of
		// plug.
		...
		if (isConfigurablePlugin) {
			config = FilePersistentConfig.constructFilePersistentConfig(
	...
	/**
	 * Tell the plugin to quit. Interrupt it if it's a thread-based plugin which might be
	 * sleeping. Then call removePlugin() on it on the manager - either now, if it's
	 * threadless, or after it terminates, if it's thread based.
	 * @param manager The plugin manager object.
	 * @param maxWaitTime If a plugin is thread-based, we can wait for it to terminate.
	 * Set to -1 if you don't want to wait at all, 0 to wait forever or else a value in
	 * milliseconds.
	 **/
	 ...
	 * {@link FredPluginFCPMessageHandler.ServerSideFCPMessageHandler} of the plugin.<br>
	 * <br>
	 *
	 * TODO: Code quality: Currently, all the other is...() functions are used by
	 * PluginManager just to then manually cast the plugin main object to the desired
	 * type, i.e. it manually does what the body of this function does. This restricts the
	 * API to require the plugin main class to implement all the various interfaces.
	 * Instead, please add equivalents of this function for all the other is...(), and use
	 * those new functions everywhere in PluginManager. This will a preparation for
	 * allowing plugins to implement the various interfaces in DIFFERENT classes than
	 * their plugin main class - which will be a good idea for keeping plugin main classes
	 * short.
	 */
