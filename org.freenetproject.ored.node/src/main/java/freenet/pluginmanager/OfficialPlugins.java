...
public class OfficialPlugins {
	...
	public OfficialPlugins() {
		try {
			...
			addPlugin("WebOfTrustTesting").inGroup("communication").advanced().experimental().usesXml()
					.alwaysFetchLatestVersion().minimumVersion(17) // When changing this
																	// also update edition
																	// of USK below!
					.loadedFrom(
			...
			addPlugin("FlogHelper").inGroup("communication").minimumVersion(36).usesXml().loadedFrom(
					"CHK@zAvWPxjh2rJ7j2cAiXcm2wnDYlURL9BFDGfLBaVtjqU,Ak90k0Qw1b6CWoWZ1AMdvrUj~evz4p9vl75mg7BsEp0,AAMC--8/FlogHelper.jar");
			addPlugin("Sharesite").inGroup("communication").recommendedVersion(5).minimumVersion(2).loadedFrom(
					"CHK@MDQlAV6EcyIvkomR~~YUx7YOS7BbicvAC9hpedmUlUw,rf3V7plsMOf650hNXOWqmF35dGs1LXRf3TiFOwdghyU,AAMC--8/Sharesite-0-4-8.jar");
		}
	...
	private class OfficialPluginBuilder {
		...
		/**
		 * ATTENTION: Please read {@link OfficialPluginDescription#uri} before deciding
		 * whether to use USK or CHK!
		 */
		...
		private OfficialPluginDescription createOfficialPluginDescription() {
			return new OfficialPluginDescription(name, group, essential, minimumVersion, recommendedVersion,
					alwaysFetchLatestVersion, usesXml, uri, deprecated, experimental, advanced, unsupported);
		}
	...
	public static class OfficialPluginDescription {
		...
		/**
		 * The group of the plugin. The group is a technical name that needs to be
		 * translated before it is shown to the user.
		 */
		public final String group;

		/**
		 * If true, we will download it, blocking, over HTTP, during startup (unless
		 * explicitly forbidden to use HTTP). If not, we will download it on a separate
		 * thread after startup. Both are assuming we don't have it in a file.
		 */
		public final boolean essential;

		/**
		 * Minimum getRealVersion(). If the plugin is older than this, we will fail the
		 * load.
		 */
		 ...
		 * download the new version in the background, and either use it on restart, or
		 * offer the user the option to reload it. This is in fact identical to what
		 * happens on a USK-based update...
		 */
		public final long recommendedVersion;

		/**
		 * If true, if during startup we already have a copy of the plugin JAR on disk,
		 * the {@link PluginManager} will ignore it and redownload the JAR instead so the
		 * user gets a recent version if there is one.<br>
		 * <br>
		 *
		 ...
		 * the main USK, this *can* be an USK, but updating when a new version is inserted
		 * to the USK will only happen at certain points in time:<br>
		 * - if the plugin is manually unloaded and loaded again.<br>
		 * - at restart of Freenet if {@link #alwaysFetchLatestVersion} is true. If it is
		 * false, the cached local JAR file on disk will prevent updating!<br>
		 * So to make updating work using USK, set {@link #alwaysFetchLatestVersion} so we
		 * check for updates when the node is restarted.<br>
		 * <br>
		 *
		 * NOTICE the conclusion of the above: It is NOT RECOMMENDED to use USKs here:
		 * Updates will only be delivered at restarts of the node, while the main Freenet
		...
		/**
		 * If true, the plugin is geeky - it should not be shown except in advanced mode
		 * even though it's not deprecated nor is it experimental.
		 */
		public final boolean advanced;

		/**
		 * If true, the plugin used to be official, but is no longer supported. These are
		 * not shown even in advanced mode.
		 */
		public final boolean unsupported;

		OfficialPluginDescription(String name, String group, boolean essential, long minVer, long recVer,
				boolean alwaysFetchLatestVersion, boolean usesXML, FreenetURI uri, boolean deprecated,
				boolean experimental, boolean advanced, boolean unsupported) {
