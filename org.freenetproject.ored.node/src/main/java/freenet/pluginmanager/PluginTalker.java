...
public class PluginTalker {
	...
	public PluginTalker(FredPluginTalker fpt, Node node2, String pluginname2, String clientSideIdentifier)
		...
		// Normally, the clientIdentifier passed to the PluginReplySenderDirect() shall be
		// an identifier of the particular network connection to the client.
		// But this PluginTalker constructor typically gets called by
		// PluginRespirator.getPluginTalker() which is called directly by client plugins.
		// So there is no real FCP network connection, the client plugin runs in the same
		// node as the server plugin.
		// As we have no network connection to pull an ID from, we assume a new client for
		// each call of this constructor by computing a random clientIdentifier.
	...
	public PluginTalker(Node node2, FCPConnectionHandler handler, String pluginname2, String clientSideIdentifier,
			boolean access2) throws PluginNotFoundException {
		node = node2;
		pluginName = pluginname2;
		pluginRef = findPlugin(pluginname2);
		access = access2 ? FredPluginFCP.ACCESS_FCP_FULL : FredPluginFCP.ACCESS_FCP_RESTRICTED;

		// FCPConnectionHandler.connectionIdentifier is unique for each network connection
		// of a client, which is exactly what the PluginReplySenderFCP() wants.
	...
	public void sendSyncInternalOnly(final SimpleFieldSet plugparams, final Bucket data2) {
		try {
			...
			if (plug == null) {
				...
				// throw new PluginNotFoundException(How to get this out to surrounding
				// send(..)?);
