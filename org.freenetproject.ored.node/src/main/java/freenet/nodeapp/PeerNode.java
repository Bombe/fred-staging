...
import freenet.nodeapp.PeerManager.PeerStatusChangeListener;
import freenet.nodeapp.math.TimeDecayingRunningAverage;
	...
	void startARKFetcher() {
		...
		synchronized(arkFetcherSync) {
			...
			if(arkFetcher == null) {
				...
				arkFetcher = node.clientCore.uskManager.subscribeContent(myARK, this, true, node.arkFetcherContext, PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS, node.nonPersistentClientRT);
			}
		}
	}
	...
	*/
	...
	class LoadSender {
		...
		Message makeLoadStats(long now, int transfersPerInsert, boolean noRemember) {
			NodeStats.PeerLoadStats stats = node.nodeStats.createPeerLoadStats(PeerNode.this, transfersPerInsert, realTimeFlag);
			synchronized(this) {
	...
	class OutputLoadTracker {
		...
		public IncomingLoadSummaryStats getIncomingLoadStats() {
			...
			NodeStats.RunningRequestsSnapshot runningRequests = node.nodeStats.getRunningRequestsTo(PeerNode.this, loadStats.averageTransfersOutPerInsert, realTime);
		...
		private final EnumMap<NodeStats.RequestType,SlotWaiterList> slotWaiters = new EnumMap<NodeStats.RequestType,SlotWaiterList>(NodeStats.RequestType.class);
