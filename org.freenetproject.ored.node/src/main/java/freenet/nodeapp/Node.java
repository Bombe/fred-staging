...
import freenet.keys.*;
import freenet.nodeapp.math.TimeSkewDetectorCallback;
import freenet.nodeapp.useralerts.*;
import freenet.nodeapp.diagnostics.DefaultNodeDiagnostics;
import freenet.nodeapp.diagnostics.NodeDiagnostics;
import freenet.nodeapp.probe.Listener;
import freenet.nodeapp.probe.Probe;
import freenet.nodeapp.probe.Type;
import freenet.nodeapp.stats.DataStoreKeyType;
import freenet.nodeapp.stats.DataStoreType;
import freenet.nodeapp.stats.NotAvailNodeStoreStats;
...
import freenet.nodeapp.stats.DataStoreInstanceType;
import freenet.nodeapp.stats.DataStoreStats;
import freenet.nodeapp.stats.StoreCallbackStats;
...
public class Node implements TimeSkewDetectorCallback, KeyBlockStore, PersistentStatsChecker {
	...
	private SlashdotStore<StoreDSAPublicKey> pubKeySlashdotcacheStore;
	 ...
	 */
	 Node(PersistentConfig config, RandomSource r, RandomSource weakRandom, LoggingConfigHandler lc, NodeStarter ns, Executor executor) throws NodeInitException {
					...
					synchronized(Node.this) {
						if(opennet == null) {
							try {
								...
								clientCore.alerts.register(new SimpleUserAlert(false, l10n("enableOpennetFailedTitle"), l10n("enableOpennetFailed", "message", e.getLocalizedMessage()), l10n("enableOpennetFailed", "message", e.getLocalizedMessage()), FCPUserAlert.ERROR));
							}
						}
					}
					if(o != null) {
			...
			public void onChange(PHYSICAL_THREAT_LEVEL oldLevel, PHYSICAL_THREAT_LEVEL newLevel) {
					if(newLevel == PHYSICAL_THREAT_LEVEL.MAXIMUM) {
						...
						try {
							...
							clientCore.alerts.register(new SimpleUserAlert(true, NodeL10n.getBase().getString("SecurityLevels.cantDeletePasswordFileTitle"), NodeL10n.getBase().getString("SecurityLevels.cantDeletePasswordFile"), NodeL10n.getBase().getString("SecurityLevels.cantDeletePasswordFileTitle"), FCPUserAlert.CRITICAL_ERROR));
						}
					}
		...
		pubKeySlashdotcacheStore = new SlashdotStore<StoreDSAPublicKey>(pubKeySlashdotcache, maxSlashdotCacheKeys, slashdotCacheLifetime, PURGE_INTERVAL, ticker, this.clientCore.tempBucketFactory);
	...
	private void peersOffersFrefFilesConfiguration(SubConfig nodeConfig, int configOptionSortOrder) {
		...
		nodeConfig.register("peersOffersDismissed", false, configOptionSortOrder, true, true,
				"Node.peersOffersDismissed", "Node.peersOffersDismissedLong", new BooleanCallback() {
					...
					public void set(Boolean val) {
						if (val) {
							for (FCPUserAlert alert : clientCore.alerts.getAlerts())
	...
	*/
	...
	private void initRAMClientCacheFS() {
		...
		new RAMFreenetStore<StoreDSAPublicKey>(pubKeyClientcache, (int) Math.min(Integer.MAX_VALUE, maxClientCacheKeys));
	...
	private void initNoClientCacheFS() {
		...
		pubKeyClientcache = new PubkeyStore();
		new NullFreenetStore<StoreDSAPublicKey>(pubKeyClientcache);
		sskClientcache = new SSKStore(getPubKey);
	...
	private void finishInitSaltHashFS(final String suffix, NodeClientCore clientCore) {
		...
		chkDatastore.getStore().setUserAlertRegister(clientCore.alerts);
		chkDatacache.getStore().setUserAlertRegister(clientCore.alerts);
		pubKeyDatastore.getStore().setUserAlertRegister(clientCore.alerts);
		pubKeyDatacache.getStore().setUserAlertRegister(clientCore.alerts);
		sskDatastore.getStore().setUserAlertRegister(clientCore.alerts);
		sskDatacache.getStore().setUserAlertRegister(clientCore.alerts);
	}
	...
	private void initRAMFS() {
		...
		new RAMFreenetStore<StoreDSAPublicKey>(pubKeyDatastore, (int) Math.min(Integer.MAX_VALUE, maxStoreKeys));
		pubKeyDatacache = new PubkeyStore();
		getPubKey.setDataStore(pubKeyDatastore, pubKeyDatacache);
		new RAMFreenetStore<StoreDSAPublicKey>(pubKeyDatacache, (int) Math.min(Integer.MAX_VALUE, maxCacheKeys));
	...
	private void initSaltHashFS(final String suffix, boolean dontResizeOnStart, byte[] masterKey) throws NodeInitException {
		try {
			...
			final FreenetStore<StoreDSAPublicKey> pubkeyDataFS = makeStore("PUBKEY", true, pubKeyDatastore, dontResizeOnStart, masterKey);
			final PubkeyStore pubKeyDatacache = new PubkeyStore();
			final FreenetStore<StoreDSAPublicKey> pubkeyCacheFS = makeStore("PUBKEY", false, pubKeyDatacache, dontResizeOnStart, masterKey);
			((SaltedHashFreenetStore<StoreDSAPublicKey>) pubkeyCacheFS.getUnderlyingStore()).setAltStore(((SaltedHashFreenetStore<StoreDSAPublicKey>) pubkeyDataFS.getUnderlyingStore()));
	...
	private void initSaltHashClientCacheFS(final String suffix, boolean dontResizeOnStart, byte[] clientCacheMasterKey) throws NodeInitException {
		...
		try {
			...
			final FreenetStore<StoreDSAPublicKey> pubkeyDataFS = makeClientcache("PUBKEY", true, pubKeyClientcache, dontResizeOnStart, clientCacheMasterKey);
	...
	private void checkForEvilJVMBugs() {
		...
		if((!isOpenJDK) && (jvmVendor.startsWith("Sun ") || jvmVendor.startsWith("Oracle ")) || (jvmVendor.startsWith("The FreeBSD Foundation") && (jvmSpecVendor.startsWith("Sun ") || jvmSpecVendor.startsWith("Oracle "))) || (jvmVendor.startsWith("Apple "))) {
			...
			if(jvmVendor.startsWith("Free Software Foundation")) {
				...
				clientCore.alerts.register(new SimpleUserAlert(true, l10n("usingGCJTitle"), l10n("usingGCJ"), l10n("usingGCJTitle"), FCPUserAlert.WARNING));
			}
		}
...
//		}
	...
	public Map<DataStoreInstanceType, DataStoreStats> getDataStoreStats() {
		...
		map.put(new DataStoreInstanceType(DataStoreKeyType.CHK, DataStoreType.STORE), new StoreCallbackStats(chkDatastore, nodeStats.chkStoreStats()));
		map.put(new DataStoreInstanceType(DataStoreKeyType.CHK, DataStoreType.CACHE), new StoreCallbackStats(chkDatacache, nodeStats.chkCacheStats()));
		map.put(new DataStoreInstanceType(DataStoreKeyType.CHK, DataStoreType.SLASHDOT), new StoreCallbackStats(chkSlashdotcache,nodeStats.chkSlashDotCacheStats()));
		map.put(new DataStoreInstanceType(DataStoreKeyType.CHK, DataStoreType.CLIENT), new StoreCallbackStats(chkClientcache, nodeStats.chkClientCacheStats()));

		map.put(new DataStoreInstanceType(DataStoreKeyType.SSK, DataStoreType.STORE), new StoreCallbackStats(sskDatastore, nodeStats.sskStoreStats()));
		map.put(new DataStoreInstanceType(DataStoreKeyType.SSK, DataStoreType.CACHE), new StoreCallbackStats(sskDatacache, nodeStats.sskCacheStats()));
		map.put(new DataStoreInstanceType(DataStoreKeyType.SSK, DataStoreType.SLASHDOT), new StoreCallbackStats(sskSlashdotcache, nodeStats.sskSlashDotCacheStats()));
		map.put(new DataStoreInstanceType(DataStoreKeyType.SSK, DataStoreType.CLIENT), new StoreCallbackStats(sskClientcache, nodeStats.sskClientCacheStats()));

		map.put(new DataStoreInstanceType(DataStoreKeyType.PUB_KEY, DataStoreType.STORE), new StoreCallbackStats(pubKeyDatastore, new NotAvailNodeStoreStats()));
		map.put(new DataStoreInstanceType(DataStoreKeyType.PUB_KEY, DataStoreType.CACHE), new StoreCallbackStats(pubKeyDatacache, new NotAvailNodeStoreStats()));
		map.put(new DataStoreInstanceType(DataStoreKeyType.PUB_KEY, DataStoreType.SLASHDOT), new StoreCallbackStats(pubKeySlashdotcache, new NotAvailNodeStoreStats()));
		map.put(new DataStoreInstanceType(DataStoreKeyType.PUB_KEY, DataStoreType.CLIENT), new StoreCallbackStats(pubKeyClientcache, new NotAvailNodeStoreStats()));
