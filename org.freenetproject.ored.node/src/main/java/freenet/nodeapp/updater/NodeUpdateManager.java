...
import freenet.nodeapp.useralerts.RevocationKeyFoundUserAlert;
import freenet.nodeapp.useralerts.SimpleUserAlert;
import freenet.nodeapp.useralerts.UpdatedVersionAvailableUserAlert;
...
public class NodeUpdateManager {
	...
	public void start() throws InvalidConfigValueException {
		...
		if (updateInstallers) {
			...
			installerGetter.start(PriorityClasses.UPDATE_PRIORITY_CLASS,
					MAX_JAVA_INSTALLER_LENGTH);
			wininstallerGetter.start(PriorityClasses.UPDATE_PRIORITY_CLASS,
		...
		if (updateIPToCountry) {
			...
			ip4Getter.start(PriorityClasses.UPDATE_PRIORITY_CLASS,
	...
	private void deployUpdate() {
		...
		try {
			...
			node.clientCore.alerts.register(new SimpleUserAlert(false,
					error, error, error, FCPUserAlert.CRITICAL_ERROR));
		} finally {
			if(started && !success) {
				Bucket toFree = null;
				synchronized (this) {
