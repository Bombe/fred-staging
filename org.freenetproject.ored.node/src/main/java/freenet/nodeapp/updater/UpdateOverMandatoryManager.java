...
public class UpdateOverMandatoryManager implements RequestClient {
		...
		ClientGetCallback myCallback = new ClientGetCallback() {
			...
			public void onFailure(FetchException e, ClientGetter state) {
				...
				// Probably best to keep files around for now.
					...
					insertBlob(updateManager.revocationChecker.getBlobBucket(), "revocation", PriorityClasses.INTERACTIVE_PRIORITY_CLASS);
				} else {
	...
	protected void insertBlob(final RandomAccessBucket bucket, final String type, short priority) {
		...
		InsertContext ctx = updateManager.node.clientCore.makeClient(PriorityClasses.INTERACTIVE_PRIORITY_CLASS, false, false).getInsertContext(true);
