...
import freenet.nodeapp.useralerts.ProxyUserAlert;
	...
	public class PortForwardAlert extends BaseNodeUserAlert {
		...
		public short innerGetPriorityClass() {
			...
			if(portsNotForwarded != null) {
				for(int portNotForwarded: portsNotForwarded)
					if(portNotForwarded < 0) return FCPUserAlert.ERROR;
			}
		...
		public String getShortText() {
			String prefix = innerGetPriorityClass() == FCPUserAlert.ERROR ?
					...
					synchronized(this) {
						if(noConnectivityAlert == null)
							noConnectivityAlert = toRegister =
								new SimpleUserAlert(false, l10n("noConnectivityTitle"), l10n("noConnectivity"), l10n("noConnectivityShort"), FCPUserAlert.ERROR);
					}
					if(toRegister != null)
