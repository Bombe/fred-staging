...
public class Announcer {
	...
	private void connectSomeSeednodes() {
		...
		synchronized (this) {
			...
			if (logMINOR)
				Logger.minor(this, "count = " + count + " announced = " + announcedToIdentities.size() + " running = "
						+ runningAnnouncements + " still connecting " + stillConnecting);
	...
	private SimpleUserAlert announcementDisabledAlert = new SimpleUserAlert(false, l10n("announceDisabledTooOldTitle"),
			l10n("announceDisabledTooOld"), l10n("announceDisabledTooOldShort"), FCPUserAlert.CRITICAL_ERROR) {
		...
		public String getText() {
			...
			sb.append(" ");
			if (!node.nodeUpdater.isEnabled()) {
				sb.append(l10n("announceDisabledTooOldUpdateDisabled", new String[] { "config", "/config" },
						new String[] { "", "" }));
			}
			return sb.toString();
		}
	...
	boolean enoughPeers() {
		...
		if ((!node.nodeUpdater.isEnabled()) || (node.nodeUpdater.canUpdateNow() && !node.nodeUpdater.isArmed())) {
		...
		if (killAnnouncement) {
			...
			if (node.nodeUpdater.isEnabled() && node.nodeUpdater.isArmed() && node.nodeUpdater.uom.fetchingFromTwo()
	...
	protected boolean sendAnnouncement(final SeedServerPeerNode seed) {
		...
		AnnounceSender sender = new AnnounceSender(node.getLocation(), om, node, new AnnouncementCallback() {
			...
			public void completed() {
				...
				// If it takes more than COOLING_OFF_PERIOD to disconnect, we might not be
				// able to reannounce to this
				// node. However, we can't reannounce to it anyway until announcedTo is
				...
				if (acceptedSomewhere)
					System.out.println("Announcement to " + seed.userToString() + " completed (" + totalAdded
							+ " added, " + totalNotWanted + " not wanted, " + shallow + " shallow)");
				else
			...
			public void nodeNotWanted() {
				...
				Logger.normal(this,
						"Announcement to " + seed.userToString() + " returned node not wanted for a total of "
								+ announcementNotWantedNodes + " (" + totalNotWanted + " from this announcement)");
			}
