...
/**
 * A queued byte[], maybe including a Message, and a callback, which may be null. Note
 * that we always create the byte[] on construction, as almost everywhere which uses a
 * MessageItem needs to know its length immediately.
 */
public class MessageItem {
	...
	/**
	 * If true, the buffer may contain several messages, and is formatted for sending as a
	 * single packet.
	 */
	...
	public MessageItem(Message msg2, AsyncMessageCallback[] cb2, ByteCounter ctr, short overridePriority) {
		...
		if (buf.length > NewPacketFormat.MAX_MESSAGE_SIZE) {
			// This is bad because fairness between UID's happens at the level of message
			// queueing,
			// and the window size is frequently very small, so if we have really big
			// messages they
			// could cause big problems e.g. starvation of other messages, resulting in
			// timeouts
	...
	public MessageItem(byte[] data, AsyncMessageCallback[] cb2, boolean formatted, ByteCounter ctr, short priority,
			boolean sendLoadRT, boolean sendLoadBulk) {
		this.cb = cb2;
		this.msg = null;
		this.buf = data;
	...
	/**
	 * @param length The actual number of bytes sent to send this message, including our
	 * share of the packet overheads, *and including alreadyReportedBytes*, which is only
	 * used when deciding how many bytes to report to the throttle.
	 */
	public void onSent(int length) {
		// NB: The fact that the bytes are counted before callback notifications is
	...
	/**
	 * Set the deadline for this message. Called when a message is unqueued, when we start
	 * to send it. Used if the message does not entirely fit in the packet, and also if it
	 * is retransmitted.
