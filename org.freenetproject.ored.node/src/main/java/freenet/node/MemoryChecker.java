...
public class MemoryChecker implements Runnable {
	...
	public void run() {
		...
		if (totalMemory == maxMemory || maxMemory == Long.MAX_VALUE) {
			...
			if (avgFreeMemory.countReports() >= 3 && avgFreeMemory.currentValue() < 4 * 1024 * 1024) {// average
																										// free
																										// memory
																										// <
																										// 4
				...
				try {
					Thread.sleep(10); // Force a context switch, finalization need a CS to
										// complete
				}
		...
		if (aggressiveGCModificator > 0) {
			...
			if (logMINOR) {
				...
				Logger.minor(this,
						"GC completed after " + (afterGCTime - beforeGCTime) + "ms and \"recovered\" "
								+ (beforeGCUsedMemory - afterGCUsedMemory) + " bytes, leaving " + afterGCUsedMemory
								+ " bytes used");
			}
		}
