...
class NPFPacket {
	...
	/**
	 * Messages that are specific to a single packet and can be happily lost if it is
	 * lost. They must be processed before the rest of the messages. With early versions,
	 * these might be bogus, so be careful parsing them.
	 */
	...
	public static NPFPacket create(byte[] plaintext, BasePeerNode pn) {
		...
		packet.sequenceNumber = ((plaintext[offset] & 0xFF) << 24) | ((plaintext[offset + 1] & 0xFF) << 16)
				| ((plaintext[offset + 2] & 0xFF) << 8) | (plaintext[offset + 3] & 0xFF);
		offset += 4;
		...
		if (numAckRanges > 0) {
			try {
				...
				for (int i = 0; i < numAckRanges; i++) {
					if (i == 0) {
						ack = ((plaintext[offset] & 0xFF) << 24) | ((plaintext[offset + 1] & 0xFF) << 16)
								| ((plaintext[offset + 2] & 0xFF) << 8) | (plaintext[offset + 3] & 0xFF);
						offset += 4;
					}
					else {
		...
		while (offset < plaintext.length) {
			...
			if ((plaintext[offset] & 0x10) != 0) {
				...
				messageID = ((plaintext[offset] & 0x0F) << 24) | ((plaintext[offset + 1] & 0xFF) << 16)
						| ((plaintext[offset + 2] & 0xFF) << 8) | (plaintext[offset + 3] & 0xFF);
				offset += 4;
			}
			else {
				...
				messageID = prevFragmentID + (((plaintext[offset] & 0x0F) << 8) | (plaintext[offset + 1] & 0xFF));
				offset += 2;
			}
			...
			if ((offset + fragmentLength) > plaintext.length) {
				Logger.error(NPFPacket.class,
						"Fragment doesn't fit in the received packet: offset is " + offset + " fragment length is "
								+ fragmentLength + " plaintext length is " + plaintext.length + " message length "
								+ messageLength + " message ID " + messageID
			...
			packet.fragments.add(new MessageFragment(shortMessage, isFragmented, firstFragment, messageID,
					fragmentLength, messageLength, fragmentOffset, fragmentData, null));
		}
	...
	public int toBytes(byte[] buf, int offset, Random paddingGen) {
		...
		if (acksIterator.hasNext()) {
			...
			if (nextAck != endRange) { // Edge-case when the last ack does not fit into
										// previous range
				assert (nextAck - endRange >= 0);
		...
		for (MessageFragment fragment : fragments) {
			...
			if (fragment.isFragmented) {
				// If firstFragment is true, add total message length. Else, add fragment
				// offset
		...
		if (offset < buf.length) {
			...
			byte b = (byte) (buf[offset] & 0x9F); // Make sure firstFragment and
													// isFragmented isn't set
			if (b == 0x1F)
				b = (byte) 0x9F; // Make sure it doesn't match the pattern for lossy
									// messages
			buf[offset] = b;
		}
	...
	/**
	 * Get the list of lossy messages. Note that for early versions these may be bogus, so
	 * be careful when parsing them. Note also that these must be processed before the
	...
	public String toString() {
		return "Packet " + sequenceNumber + ": " + length + " bytes, " + acks.size() + " acks, " + fragments.size()
				+ " fragments";
	}
