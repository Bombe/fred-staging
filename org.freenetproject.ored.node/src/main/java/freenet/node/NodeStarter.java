...
public class NodeStarter implements WrapperListener {
	...
	/*
	 * (File.separatorChar == '\\') &&
	 * (System.getProperty("os.arch").toLowerCase().matches("(i?[x0-9]86_64|amd64)")) ? 6
	 * : 2;
	 */
	 ...
	 * wrapper code that it can start its application. This method call is expected to
	 * return, so a new thread should be launched if necessary.
	 * @param args List of arguments used to initialize the application.
	 * @return Any error code if the application should exit on completion of the start
	 * method. If there were no problems then this method should return null.
	 */
	@Override
	public Integer start(String[] args) {
		...
		// Prevent timeouts for a while. The DiffieHellman init for example could take
	...
	/**
	 * Called when the application is shutting down. The Wrapper assumes that this method
	 * will return fairly quickly. If the shutdown code code could potentially take a long
	 * time, then WrapperManager.signalStopping() should be called to extend the timeout
	 * period. If for some reason, the stop method can not return, then it must call
	 * WrapperManager.stopped() to avoid warning messages from the Wrapper.
	 * @param exitCode The suggested exit code that will be returned to the OS when the
	 * JVM exits.
	 * @return The exit code to actually return to the OS. In most cases, this should just
	 * be the value of exitCode, however the user code has the option of changing the exit
	 * code if there are any problems during shutdown.
	 */
	@Override
	...
	/**
	 * Called whenever the native wrapper code traps a system control signal against the
	 * Java process. It is up to the callback to take any actions necessary. Possible
	 * values are: WrapperManager.WRAPPER_CTRL_C_EVENT, WRAPPER_CTRL_CLOSE_EVENT,
	...
	public static RandomSource globalTestInit(String testName, boolean enablePlug, LogLevel logThreshold,
			String details, boolean noDNS) throws InvalidThresholdException {
	 ...
	 * @param baseDirectory The directory in which the test data will be placed. Will be
	 * created automatically if it does not exist. You should use the same one in
	 ...
	 * The function will NOT fail if the directory exists already. You should make sure on
	 * your own to delete this before and after tests to ensure a clean state. Notice that
	 * JUnit provides a mechanism for automatic creation and deletion of test directories
	 * (TemporaryFolder).
	 * @param randomSource The random number generator of the Node. Null for the default
	 * of {@link Yarrow}. <br>
	 ...
	 * @return If you passed a {@link RandomSource}, the same one is returned. Otherwise,
	 * a new {@link Yarrow} is returned.
	 */
	public static RandomSource globalTestInit(File baseDirectory, boolean enablePlug, LogLevel logThreshold,
	...
	public static Node createTestNode(int port, int opennetPort, String testName, boolean disableProbabilisticHTLs,
			short maxHTL, int dropProb, RandomSource random, Executor executor, int threadLimit, long storeSize,
	 ...
	 * @throws NodeInitException If the node cannot start up for some reason, most likely
	 * a config problem.
	 * @deprecated Use {@link #createTestNode(TestNodeParameters)} instead
	 */
	@Deprecated
	...
	public static Node createTestNode(TestNodeParameters params) throws NodeInitException {
		...
		SimpleFieldSet configFS = new SimpleFieldSet(false); // only happens once in
																// entire simulation
		...
		// All testing environments connect the nodes as they want, even if the old setup
		// is restored, it is not desired.
	...
	public static synchronized SecureRandom getGlobalSecureRandom() {
		if (globalSecureRandom == null) {
			...
			globalSecureRandom.nextBytes(new byte[16]); // Force it to seed itself so it
