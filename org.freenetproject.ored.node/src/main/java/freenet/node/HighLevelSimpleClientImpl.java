...
public class HighLevelSimpleClientImpl implements HighLevelSimpleClient, RequestClient, Cloneable {
	...
	/**
	 * If set, only check the local datastore, don't send an actual request out. Don't
	...
	public HighLevelSimpleClientImpl(NodeClientCore node, BucketFactory bf, RandomSource r, short priorityClass,
	...
	public FetchResult fetchFromMetadata(Bucket initialMetadata) throws FetchException {
		...
		FetchContext context = getFetchContext();
		FetchWaiter fw = new FetchWaiter(this);
		ClientGetter get = new ClientGetter(fw, FreenetURI.EMPTY_CHK_URI, context, priorityClass, null, null,
				initialMetadata);
		try {
			core.clientContext.start(get);
		}
	...
	@Override
	public ClientGetter fetch(FreenetURI uri, long maxSize, ClientGetCallback callback, FetchContext fctx, short prio)
			throws FetchException {
	...
	public FreenetURI insert(InsertBlock insert, boolean getCHKOnly, String filenameHint, boolean isMetadata,
	...
	public FreenetURI insert(InsertBlock insert, String filenameHint, boolean isMetadata, short priority,
			InsertContext ctx) throws InsertException {
		return insert(insert, filenameHint, isMetadata, priority, ctx, null);
	}

	public FreenetURI insert(InsertBlock insert, String filenameHint, boolean isMetadata, short priority,
			InsertContext ctx, byte[] forceCryptoKey) throws InsertException {
		PutWaiter pw = new PutWaiter(this);
		ClientPutter put = new ClientPutter(pw, insert.getData(), insert.desiredURI, insert.clientMetadata, ctx,
				priority, isMetadata, filenameHint, false, core.clientContext, forceCryptoKey, -1);
		try {
	...
	public ClientPutter insert(InsertBlock insert, String filenameHint, boolean isMetadata, InsertContext ctx,
		...
		ClientPutter put = new ClientPutter(cb, insert.getData(), insert.desiredURI, insert.clientMetadata, ctx,
				priority, isMetadata, filenameHint, false, core.clientContext, null, -1);
		try {
			core.clientContext.start(put);
		}
		catch (PersistenceDisabledException e) {
	...
	public FreenetURI insertManifest(FreenetURI insertURI, HashMap<String, Object> bucketsByName, String defaultName,
		...
		try {
			putter = new DefaultManifestPutter(pw, BaseManifestPutter.bucketsByNameToManifestEntries(bucketsByName),
					priorityClass, insertURI, defaultName, getInsertContext(true), false, forceCryptoKey,
					core.clientContext);
		}
	...
	public FetchContext getFetchContext(long overrideMaxSize, String schemeHostAndPort) {
		...
		return new FetchContext(maxLength, maxTempLength, curMaxMetadataLength, MAX_RECURSION, MAX_ARCHIVE_RESTARTS,
				MAX_ARCHIVE_LEVELS, DONT_ENTER_IMPLICIT_ARCHIVES, SPLITFILE_BLOCK_RETRIES, NON_SPLITFILE_RETRIES,
				USK_RETRIES, FETCH_SPLITFILES, FOLLOW_REDIRECTS, LOCAL_REQUESTS_ONLY, FILTER_DATA,
				MAX_SPLITFILE_BLOCKS_PER_SEGMENT, MAX_SPLITFILE_CHECK_BLOCKS_PER_SEGMENT, bucketFactory, eventProducer,
				false, CAN_WRITE_CLIENT_CACHE, null, null, schemeHostAndPort);
	}

	public static FetchContext makeDefaultFetchContext(long maxLength, long maxTempLength, BucketFactory bucketFactory,
			SimpleEventProducer eventProducer) {
		return new FetchContext(maxLength, maxTempLength, 1024 * 1024, MAX_RECURSION, MAX_ARCHIVE_RESTARTS,
				MAX_ARCHIVE_LEVELS, DONT_ENTER_IMPLICIT_ARCHIVES, SPLITFILE_BLOCK_RETRIES, NON_SPLITFILE_RETRIES,
				USK_RETRIES, FETCH_SPLITFILES, FOLLOW_REDIRECTS, LOCAL_REQUESTS_ONLY, FILTER_DATA,
				MAX_SPLITFILE_BLOCKS_PER_SEGMENT, MAX_SPLITFILE_CHECK_BLOCKS_PER_SEGMENT, bucketFactory, eventProducer,
	...
	public InsertContext getInsertContext(boolean forceNonPersistent) {
		return new InsertContext(INSERT_RETRIES, CONSECUTIVE_RNFS_ASSUME_SUCCESS,
				ClientConstants.SPLITFILE_BLOCKS_PER_SEGMENT, ClientConstants.SPLITFILE_CHECK_BLOCKS_PER_SEGMENT,
				eventProducer, CAN_WRITE_CLIENT_CACHE_INSERTS, Node.FORK_ON_CACHEABLE_DEFAULT, false,
				Compressor.DEFAULT_COMPRESSORDESCRIPTOR, ClientConstants.EXTRA_INSERTS_SINGLE_BLOCK,
				ClientConstants.EXTRA_INSERTS_SPLITFILE_HEADER, InsertContext.CompatibilityMode.COMPAT_DEFAULT);
	}
	...
	public void prefetch(FreenetURI uri, long timeout, long maxSize, Set<String> allowedTypes) {
		prefetch(uri, timeout, maxSize, allowedTypes, PriorityClasses.PREFETCH_PRIORITY_CLASS);
	}
