...
public class RequestTag extends UIDTag {
	...
	public synchronized void setSender(RequestSender rs, boolean coalesced) {
		// If it's because of transfer coalescing, we won't get anything from the
		// RequestSender, so we should not wait for it.
		if (!coalesced)
			sent = true;
	...
	/**
	 * The handler threw a Throwable, i.e. failed due to a serious bug. Unlock the
	 * handler, and record the throwable in case of future problems.
