 ...
 * @author amphibian
 *
 * Sends a request out onto the network, and deals with the consequences. Other half of
 * the request functionality is provided by RequestHandler.
 *
 * Must put self onto node's list of senders on creation, and remove self from it on
 * destruction. Must put self onto node's list of transferring senders when starts
 * transferring, and remove from it when finishes transferring.
 */
public final class RequestSender extends BaseSender implements PrioRunnable {
	...
	// After a get offered key fails, wait this long for two stage timeout. Probably we
	// will
	...
	/**
	 * One in this many successful requests is randomly reinserted. This is probably a
	 * good idea anyway but with the split store it's essential.
	 */
	 ...
	 * @param key The key to request. Its public key should have been looked up already;
	 * RequestSender will not look it up.
	 * @param realTimeFlag If enabled,
	 */
	public RequestSender(Key key, DSAPublicKey pubKey, short htl, long uid, RequestTag tag, Node n, PeerNode source,
			boolean offersOnly, boolean canWriteClientCache, boolean canWriteDatastore, boolean realTimeFlag) {
	...
	public void run() {
		node.getTicker().queueTimedJob(new Runnable() {
			...
			public void run() {
				...
				// it is possible for us to exceed the timeout. In which case the
				// downstream
				// node will get impatient. So we need to reassign to self when this
				// happens,
				...
				// We are still routing, yet we have exceeded the per-peer timeout,
				// probably due to routing to multiple nodes e.g. RNFs and accepted
				// timeouts.
		...
		try {
			...
			// LOCKING: Normally receivingAsync is set by this thread, so there is no need
			// to synchronize.
			// If it is set by another thread it will only be after it was set by this
			// thread.
	...
	private void realRun() {
		...
		final FailureTable.OfferList offers = node.failureTable.getOffers(key);
	...
	private void startRequests() {
		...
		routeAttempts = 0;
		starting = true;
		// While in no-cache mode, we don't decrement HTL on a RejectedLoop or similar,
		// but we only allow a limited number of such failures before RNFing.
	...
	/**
	 * Route requests. Method is responsible for its own completion, e.g. finish or
	 * chaining to MainLoopCallback, i.e. the caller isn't going to do more stuff relevant
	 * to the request afterwards.
	 */
		...
		while (true) {
			...
			if (dontDecrementHTLThisTime) {
				...
				if ((!starting) && (!canWriteStorePrev)) {
					...
					// we do not want to decrement the HTL for trivial rejections (e.g.
					// RejectedLoop),
					...
					/*
					 * If we haven't routed to any node yet, decrement according to the
					 * source. If we have, decrement according to the node which just
					 * failed. Because: 1) If we always decrement according to source then
					 * we can be at max or min HTL for a long time while we visit *every*
					 * peer node. This is BAD! 2) The node which just failed can be seen
					 * as the requestor for our purposes.
					 */
					// Decrement at this point so we can DNF immediately on reaching HTL
					// 0.
			...
			if (recentlyFailed > now) {
				...
				if (rfAnyway) {
					...
					// But we set a timeout of 0 because we're not generating one based on
					// where we've routed the key to.
					// Returning the time we were passed minus some value will give the
					// next node an inaccurate high timeout.
					// Rerouting (even assuming we change FNPRecentlyFailed to include a
					// hop count) would also cause problems because nothing would be
					// quenched until we have visited every node on the network.
					// That leaves forwarding a RecentlyFailed which won't create further
					// RecentlyFailed's.
					// However the peer will still avoid sending us the same key for 10
					// minutes due to per-node failure tables. This is fine, we probably
			...
			if (next == null) {
				if (logMINOR && rejectOverloads > 0)
					Logger.minor(this,
							"no more peers, but overloads (" + rejectOverloads + "/" + routeAttempts + " overloaded)");
				// Backtrack
				finish(ROUTE_NOT_FOUND, null, false);
				node.failureTable.onFinalFailure(key, null, htl, origHTL, -1, -1, source);
				return;
	...
	private class MainLoopCallback implements SlowAsyncMessageFilterCallback {
		...
		// Needs to be a separate class so it can check whether the main loop has moved on
		// to another peer.
		// If it has
		...
		public void onTimeout() {
			...
			Logger.warning(this,
					"Timed out after waiting " + searchTimeout + " on " + uid + " from " + waitingFor + " ("
							+ gotMessages + " messages; last=" + lastMessage + ") for " + uid + " noReroute="
							+ noReroute);
			if (noReroute) {
			...
			while (true) {
				...
				try {
					...
					Logger.normal(this, "Disconnected from " + waitingFor + " while waiting for reply on " + this);
	...
	enum OFFER_STATUS {
		...
		TWO_STAGE_TIMEOUT, // Waiting asynchronously for two stage timeout; remove the
							// offer, but don't unlock the tag.
	 ...
	 */
	private void tryOffers(final FailureTable.OfferList offers, PeerNode pn, OFFER_STATUS status) {
		while (true) {
			if (pn == null) {
				// Fetches valid offers, then expired ones. Expired offers don't count
				// towards failures,
	...
	private OFFER_STATUS tryOffer(final FailureTable.BlockOffer offer, final PeerNode pn,
			final FailureTable.OfferList offers) {
		if (pn == null)
			...
			node.usm.addAsyncFilter(getOfferedKeyReplyFilter(pn, getOfferedTimeout),
					new SlowAsyncMessageFilterCallback() {
						...
						public void onMatched(Message m) {
							OFFER_STATUS status = isSSK ? handleSSKOfferReply(m, pn, offer)
	...
	private MessageFilter getOfferedKeyReplyFilter(final PeerNode pn, long timeout) {
		MessageFilter mfRO = MessageFilter.create().setSource(pn).setField(DMT.UID, uid).setTimeout(timeout)
				.setType(DMT.FNPRejectedOverload);
		MessageFilter mfGetInvalid = MessageFilter.create().setSource(pn).setField(DMT.UID, uid).setTimeout(timeout)
				.setType(DMT.FNPGetOfferedKeyInvalid);
		if (isSSK) {
			MessageFilter mfAltDF = MessageFilter.create().setSource(pn).setField(DMT.UID, uid).setTimeout(timeout)
					.setType(DMT.FNPSSKDataFoundHeaders);
			return mfAltDF.or(mfRO.or(mfGetInvalid));
		}
		else {
			MessageFilter mfDF = MessageFilter.create().setSource(pn).setField(DMT.UID, uid).setTimeout(timeout)
					.setType(DMT.FNPCHKDataFound);
			return mfDF.or(mfRO.or(mfGetInvalid));
		}
	}

	private OFFER_STATUS handleOfferTimeout(final FailureTable.BlockOffer offer, final PeerNode pn,
							...
							if (logMINOR)
								Logger.minor(this,
										"Forked get offered key due to two stage timeout completed with status "
												+ status + " from message " + m + " for " + RequestSender.this + " to "
												+ pn);
						}
	...
	private OFFER_STATUS handleSSKOfferReply(Message reply, PeerNode pn, FailureTable.BlockOffer offer) {
		if (reply.getSpec() == DMT.FNPRejectedOverload) {
			...
			if (logMINOR)
				Logger.minor(this,
						"Node " + pn + " rejected FNPGetOfferedKey for " + key + " (expired=" + offer.isExpired());
			return OFFER_STATUS.KEEP;
		}
		else if (reply.getSpec() == DMT.FNPGetOfferedKeyInvalid) {
			// Fatal, delete it.
			if (logMINOR)
				Logger.minor(this, "Node " + pn + " rejected FNPGetOfferedKey as invalid with reason "
			...
			// Wait for the data
			MessageFilter mfData = MessageFilter.create().setSource(pn).setField(DMT.UID, uid)
					.setTimeout(getOfferedTimeout).setType(DMT.FNPSSKDataFoundData);
			Message dataMessage;
			try {
			...
			if (pubKey == null) {
				MessageFilter mfPK = MessageFilter.create().setSource(pn).setField(DMT.UID, uid)
						.setTimeout(getOfferedTimeout).setType(DMT.FNPSSKPubKey);
				Message pk;
				try {
	 ...
	 * to receive a block transfer asynchronously (in which case receivingAsync will be
	 * set, and if it fails the whole request will fail). False if we should try the next
	 * offer and/or normal fetches.
	 ...
	 */
	private OFFER_STATUS handleCHKOfferReply(Message reply, final PeerNode pn, final FailureTable.BlockOffer offer,
			final FailureTable.OfferList offers) {
		if (reply.getSpec() == DMT.FNPRejectedOverload) {
			// Non-fatal, keep it.
			if (logMINOR)
			...
			try {
				...
				// FIXME kill the transfer if off-thread (two stage timeout, offers ==
				// null) and it's already completed successfully?
				...
				br.receive(new BlockReceiverCompletion() {
					...
					public void blockReceived(byte[] data) {
						...
						try {
							...
							// This is only necessary here because we don't always call
							// finish().
							pn.noLongerRoutingTo(origTag, true);
						}
					}
					...
					public void blockReceiveFailed(RetrievalException e) {
						...
						try {
							if (e.getReason() == RetrievalException.SENDER_DISCONNECTED)
								...
								// A certain number of these are normal, it's better to
								// track them through statistics than call attention to
								// them in the logs.
								Logger.normal(this, "Transfer for offer failed (" + e.getReason() + "/"
										+ RetrievalException.getErrString(e.getReason()) + "): " + e + " from " + pn,
										e);
							if (offers != null) {
	...
	protected MessageFilter makeAcceptedRejectedFilter(PeerNode next, long acceptedTimeout, UIDTag tag) {
		assert (tag == origTag);
		/**
		 * What are we waiting for? FNPAccepted - continue FNPRejectedLoop - go to another
		 * node FNPRejectedOverload - propagate back to source, go to another node if
		 * local
		 */

		MessageFilter mfAccepted = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(acceptedTimeout).setType(DMT.FNPAccepted);
		MessageFilter mfRejectedLoop = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(acceptedTimeout).setType(DMT.FNPRejectedLoop);
		MessageFilter mfRejectedOverload = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(acceptedTimeout).setType(DMT.FNPRejectedOverload);
	...
	private MessageFilter createMessageFilter(int timeout, PeerNode next) {
		MessageFilter mfDNF = MessageFilter.create().setSource(next).setField(DMT.UID, uid).setTimeout(timeout)
				.setType(DMT.FNPDataNotFound);
		MessageFilter mfRF = MessageFilter.create().setSource(next).setField(DMT.UID, uid).setTimeout(timeout)
				.setType(DMT.FNPRecentlyFailed);
		MessageFilter mfRouteNotFound = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(timeout).setType(DMT.FNPRouteNotFound);
		MessageFilter mfRejectedOverload = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(timeout).setType(DMT.FNPRejectedOverload);

		MessageFilter mf = mfDNF.or(mfRF.or(mfRouteNotFound.or(mfRejectedOverload)));
		if (!isSSK) {
			MessageFilter mfRealDFCHK = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
					.setTimeout(timeout).setType(DMT.FNPCHKDataFound);
			mf = mfRealDFCHK.or(mf);
		}
		else {
			MessageFilter mfPubKey = MessageFilter.create().setSource(next).setField(DMT.UID, uid).setTimeout(timeout)
					.setType(DMT.FNPSSKPubKey);
			MessageFilter mfDFSSKHeaders = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
					.setTimeout(timeout).setType(DMT.FNPSSKDataFoundHeaders);
			MessageFilter mfDFSSKData = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
					.setTimeout(timeout).setType(DMT.FNPSSKDataFoundData);
	...
	private DO handleMessage(Message msg, boolean wasFork, PeerNode source, MainLoopCallback waiter) {
		// For debugging purposes, remember the number of responses AFTER the insert, and
		// the last message type we received.
		gotMessages++;
		...
		br.receive(new BlockReceiverCompletion() {
			...
			public void blockReceived(byte[] data) {
				try {
					...
					if (haveSetPRB) // It was a fork, so we didn't immediately send the
									// data.
						fireCHKTransferBegins();
			...
			public void blockReceiveFailed(RetrievalException e) {
				try {
					...
					if (e.getReason() == RetrievalException.SENDER_DISCONNECTED)
						...
						// A certain number of these are normal, it's better to track them
						// through statistics than call attention to them in the logs.
						Logger.normal(this,
								"Transfer failed (" + e.getReason() + "/"
										+ RetrievalException.getErrString(e.getReason()) + "): " + e + " from " + next,
								e);
					if (RequestSender.this.source == null)
						Logger.normal(this,
								"Local transfer failed: " + e.getReason() + " : "
										+ RetrievalException.getErrString(e.getReason()) + "): " + e + " from " + next,
								e);
					...
					boolean timeout = (!br.senderAborted()) && (reason == RetrievalException.SENDER_DIED
							|| reason == RetrievalException.RECEIVER_DIED || reason == RetrievalException.TIMED_OUT
							|| reason == RetrievalException.UNABLE_TO_SEND_BLOCK_WITHIN_TIMEOUT);
					// But we only do a transfer backoff (which is separate, and starts at
					// a higher threshold) if we timed out.
					if (timeout) {
						...
						// Quick failure (in that we didn't have to timeout). Don't
						// backoff.
						// Treat as a DNF.
						node.failureTable.onFinalFailure(key, next, htl, origHTL, FailureTable.RECENTLY_FAILED_TIME,
								FailureTable.REJECT_TIME, source);
	...
	private boolean handleRejectedOverload(Message msg, boolean wasFork, PeerNode next) {
		...
		if (msg.getBoolean(DMT.IS_LOCAL)) {
			// NB: IS_LOCAL means it's terminal. not(IS_LOCAL) implies that the rejection
			// message was forwarded from a downstream node.
			// "Local" from our peers perspective, this has nothing to do with local
			...
			// a) The node no longer has the resources to handle the request, even though
			// it did initially.
		...
		// so long as the node does not send a (IS_LOCAL) message. Interestingly messages
		// can often timeout having only received this message.
		return true;
	}
	...
	private void handleRecentlyFailed(Message msg, boolean wasFork, PeerNode next) {
		next.successNotOverload(realTimeFlag);
		/*
		 * Must set a correct recentlyFailedTimeLeft before calling this finish(), because
		 * it will be passed to the handler.
		 *
		 * It is *VITAL* that the TIME_LEFT we pass on is not larger than it should be. It
		 * is somewhat less important that it is not too much smaller than it should be.
		 *
		 * Why? Because: 1) We have to use FNPRecentlyFailed to create failure table
		 * entries. Because otherwise, the failure table is of little value: A request is
		 * routed through a node, which gets a DNF, and adds a failure table entry. Other
		 * requests then go through that node via other paths. They are rejected with
		 * FNPRecentlyFailed - not with DataNotFound. If this does not create failure
		 * table entries, more requests will be pointlessly routed through that chain.
		 *
		 * 2) If we use a fixed timeout on receiving FNPRecentlyFailed, they can be
		 * self-seeding. What this means is A sends a request to B, which DNFs. This
		 * creates a failure table entry which lasts for 10 minutes. 5 minutes later, A
		 * sends another request to B, which is killed with FNPRecentlyFailed because of
		 * the failure table entry. B's failure table lasts for another 5 minutes, but A's
		 * lasts for the full 10 minutes i.e. until 5 minutes after B's. After B's failure
		 * table entry has expired, but before A's expires, B sends a request to A. A
		 * replies with FNPRecentlyFailed. Repeat ad infinitum: A reinforces B's blocks,
		 * and B reinforces A's blocks!
		 *
		 * 3) This can still happen even if we check where the request is coming from. A
		 * loop could very easily form: A - B - C - A. A requests from B, DNFs (assume the
		 * request comes in from outside, there are more nodes. C requests from A, sets up
		 * a block. B's block expires, C's is still active. A requests from B which
		 * requests from C ... and it goes round again.
		 *
		 * 4) It is exactly the same if we specify a timeout, unless the timeout can be
		 * guaranteed to not increase the expiry time.
		 */

		// First take the original TIME_LEFT. This will start at 10 minutes if we get
		// rejected in
		...
		// This is in theory relative to when the request was received by the node. Lets
		// make it relative
		...
		// We don't store the recently failed time because we will either generate our
		// own, based on which
		// peers we have routed the key to (including the timeout we got here, which we DO
		// store in the
		// FTE), or we will send a RecentlyFailed with timeout 0, which won't cause RF's
		// on the downstream
		// peer. The point is, forwarding it as-is is inaccurate: it creates a timeout
		// which is not
	...
	/**
	 * Finish fetching an SSK. We must have received the data, the headers and the pubkey
	 * by this point.
	...
	private void finishSSK(PeerNode next, boolean wasFork, byte[] headers, byte[] sskData) {
		try {
			...
			block = node.fetch((NodeSSK) key, false, canWriteClientCache, canWriteClientCache, canWriteDatastore, false,
					null);
			if (block != null) {
	...
	/**
	 * Wait until either the transfer has started, we receive a RejectedOverload, or we
	 * get a terminal status code. Must not return until we are finished - cannot timeout,
	 * because the caller will unlock the UID!
	 * @param mask Bitmask indicating what NOT to wait for i.e. the situation when this
	 * function exited last time (see WAIT_ constants above). Bits can also be set true
	 * even though they were not valid, to indicate that the caller doesn't care about
	 * that bit. If zero, function will throw an IllegalArgumentException.
	 * @return Bitmask indicating present situation. Can be fed back to this function, if
	 * nonzero.
	 */
	public synchronized short waitUntilStatusChange(short mask) {
		...
		while (true) {
			...
			while (true) {
				short current = mask; // If any bits are set already, we ignore those
										// states.
				...
				try {
					...
					if (logMINOR)
						Logger.minor(this, "Waiting for status change on " + this + " current is " + current
								+ " status is " + status);
					wait(deadline - now);
					now = System.currentTimeMillis(); // Is used in the next iteration so
														// needed even without the logging

					if (now >= deadline) {
						Logger.error(this, "Waited more than 5 minutes for status change on " + this + " current = "
								+ current + ", maybe nobody called notify()");
						// Normally we would break; here, but we give the function a
						// change to succeed
						// in the next iteration and break in the above if(now >=
						// deadline) if it
						// did not succeed. This makes the function work if notify() is
						// not called.
					}
				}
	...
	private void finish(int code, PeerNode next, boolean fromOfferedKey) {
		...
		synchronized (this) {
			if (status != NOT_FINISHED) {
				if (logMINOR)
					Logger.minor(this, "Status already set to " + status + " - returning on " + this
							+ " would be setting " + code + " from " + next);
				if (next != null)
		...
		if (status == SUCCESS) {
			if ((!isSSK) && transferTime > 0 && logMINOR) {
				...
				synchronized (avgTimeTaken) {
					...
					if (logMINOR)
						Logger.minor(this,
								"Successful CHK request transfer " + transferTime + " average " + avgTimeTakenTransfer);
					if (logMINOR)
						Logger.minor(this, "Search phase: median "
								+ (avgTimeTaken.currentValue() - avgTimeTakenTransfer.currentValue()) + "ms, mean "
								+ (avgTimeTaken.meanValue() - avgTimeTakenTransfer.meanValue()) + "ms");
				}
			}
			if (next != null) {
	...
	/**
	 * Acknowledge the opennet path folding attempt without sending a reference. Once the
	 * send completes (asynchronously), unlock everything.
	 */
	...
	/**
	 * Do path folding, maybe. Wait for either a CompletedAck or a ConnectDestination. If
	 * the former, exit. If we want a connection, reply with a ConnectReply, otherwise
	 * send a ConnectRejected and exit. Add the peer.
	...
	private boolean finishOpennet(final PeerNode next) {
		...
		try {
			...
			if (node.addNewOpennetNode(ref, OpennetManager.ConnectionType.PATH_FOLDING) == null) {
				if (logMINOR)
				...
				// opennetNoderef = null i.e. we want the noderef so we won't pass it
				// further down.
			...
			origTag.timedOutToHandlerButContinued(); // Since we will tell downstream that
														// we are finished.
	...
	public void addListener(RequestSenderListener l) {
		...
		// If it has already been sent when we add the new listener, we need to send it
		// here.
		// Otherwise we don't, it will be called by the thread processing that event, even
		...
		if (sentFinished) {
			// At the time when we added the listener, we had sent the status to the
			// others.
	...
	// However at the moment NodeClientCore's realGetCHK and realGetSSK (the blocking
	// fetches)
	// do not register a RequestSenderListener. Eventually they will be replaced with
	// something that does.

	// Also we should consider whether a local RequestSenderListener added *after* the
	// request starts should
	// impact on the decision or whether that leaks too much information. It's probably
	// safe
	// given the amount leaked anyway! (Note that if we start the request locally we will
	// want
	// to finish it even if incoming RequestHandler's are coalesced with it and they fail
	// their
	// onward transfers).
	...
	protected void timedOutWhileWaiting(double load) {
		...
		// They happen when the network is very small, or when there is a capacity
		// bottleneck.
	...
	protected void handleAcceptedRejectedTimeout(final PeerNode next, final UIDTag origTag) {
		...
		try {
			node.usm.addAsyncFilter(mf, new SlowAsyncMessageFilterCallback() {
				...
				public void onTimeout() {
					Logger.error(this,
							"Fatal timeout waiting for Accepted/Rejected from " + next + " on " + RequestSender.this);
					next.fatalTimeout(origTag, false);
				}
