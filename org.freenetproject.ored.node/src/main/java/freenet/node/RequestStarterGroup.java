...
import freenet.client.async.RequestStarterSchedulerGroup;
...
public class RequestStarterGroup implements RequestStarterSchedulerGroup {
	...
	final NodeRequestStarter chkRequestStarterBulk;

	final MyRequestThrottle chkInsertThrottleBulk;

	final NodeRequestStarter chkInsertStarterBulk;

	final MyRequestThrottle sskRequestThrottleBulk;

	final NodeRequestStarter sskRequestStarterBulk;

	final MyRequestThrottle sskInsertThrottleBulk;

	final NodeRequestStarter sskInsertStarterBulk;

	final MyRequestThrottle chkRequestThrottleRT;

	final NodeRequestStarter chkRequestStarterRT;

	final MyRequestThrottle chkInsertThrottleRT;

	final NodeRequestStarter chkInsertStarterRT;

	final MyRequestThrottle sskRequestThrottleRT;

	final NodeRequestStarter sskRequestStarterRT;

	final MyRequestThrottle sskInsertThrottleRT;

	final NodeRequestStarter sskInsertStarterRT;
	...
	@Override
	public ClientRequestScheduler getChkFetchSchedulerBulk() {
		return chkFetchSchedulerBulk;
	}

	@Override
	public ClientRequestScheduler getChkPutSchedulerBulk() {
		return chkPutSchedulerBulk;
	}

	@Override
	public ClientRequestScheduler getSskFetchSchedulerBulk() {
		return sskFetchSchedulerBulk;
	}

	@Override
	public ClientRequestScheduler getSskPutSchedulerBulk() {
		return sskPutSchedulerBulk;
	}

	@Override
	public ClientRequestScheduler getChkFetchSchedulerRT() {
		return chkFetchSchedulerRT;
	}

	@Override
	public ClientRequestScheduler getChkPutSchedulerRT() {
		return chkPutSchedulerRT;
	}

	@Override
	public ClientRequestScheduler getSskFetchSchedulerRT() {
		return sskFetchSchedulerRT;
	}

	@Override
	public ClientRequestScheduler getSskPutSchedulerRT() {
		return sskPutSchedulerRT;
	}

	private final NodeStats stats;

	RequestStarterGroup(Node node, NodeClientCore core, int portNumber, RandomSource random, Config config,
			SimpleFieldSet fs, ClientContext ctx) throws InvalidConfigValueException {
		...
		throttleWindowInsert = new ThrottleWindowManager(2.0, fs == null ? null : fs.subset("ThrottleWindowInsert"),
				node);
		throttleWindowRequest = new ThrottleWindowManager(2.0, fs == null ? null : fs.subset("ThrottleWindowRequest"),
				node);
		...
		chkRequestStarterBulk = new NodeRequestStarter(core, chkRequestThrottleBulk,
		...
		chkRequestStarterRT = new NodeRequestStarter(core, chkRequestThrottleRT,
				"CHK Request starter (" + portNumber + ')', stats.localChkFetchBytesSentAverage,
				stats.localChkFetchBytesReceivedAverage, false, false, true);
		chkFetchSchedulerBulk = new ClientRequestScheduler(false, false, false, random, chkRequestStarterBulk, core,
				core, core.storeChecker, "CHKrequester", ctx);
		chkFetchSchedulerRT = new ClientRequestScheduler(false, false, true, random, chkRequestStarterRT, core, core,
				core.storeChecker, "CHKrequester", ctx);
		chkRequestStarterBulk.setScheduler(chkFetchSchedulerBulk);
		chkRequestStarterRT.setScheduler(chkFetchSchedulerRT);

		registerSchedulerConfig(schedulerConfig, "CHKrequester", chkFetchSchedulerBulk, chkFetchSchedulerRT, false,
				false);
		...
		chkInsertStarterBulk = new NodeRequestStarter(core, chkInsertThrottleBulk,
		...
		chkInsertStarterRT = new NodeRequestStarter(core, chkInsertThrottleRT,
				"CHK Insert starter (" + portNumber + ')', stats.localChkInsertBytesSentAverage,
				stats.localChkInsertBytesReceivedAverage, true, false, true);
		chkPutSchedulerBulk = new ClientRequestScheduler(true, false, false, random, chkInsertStarterBulk, core, core,
				core.storeChecker, "CHKinserter", ctx);
		chkPutSchedulerRT = new ClientRequestScheduler(true, false, true, random, chkInsertStarterRT, core, core,
				core.storeChecker, "CHKinserter", ctx);
		...
		sskRequestStarterBulk = new NodeRequestStarter(core, sskRequestThrottleBulk,
		...
		sskRequestStarterRT = new NodeRequestStarter(core, sskRequestThrottleRT,
				"SSK Request starter (" + portNumber + ')', stats.localSskFetchBytesSentAverage,
				stats.localSskFetchBytesReceivedAverage, false, true, true);
		sskFetchSchedulerBulk = new ClientRequestScheduler(false, true, false, random, sskRequestStarterBulk, core,
				core, core.storeChecker, "SSKrequester", ctx);
		sskFetchSchedulerRT = new ClientRequestScheduler(false, true, true, random, sskRequestStarterRT, core, core,
				core.storeChecker, "SSKrequester", ctx);
		sskRequestStarterBulk.setScheduler(sskFetchSchedulerBulk);
		sskRequestStarterRT.setScheduler(sskFetchSchedulerRT);

		registerSchedulerConfig(schedulerConfig, "SSKrequester", sskFetchSchedulerBulk, sskFetchSchedulerRT, true,
				false);
		...
		sskInsertStarterBulk = new NodeRequestStarter(core, sskInsertThrottleBulk,
		...
		sskInsertStarterRT = new NodeRequestStarter(core, sskInsertThrottleRT,
				"SSK Insert starter (" + portNumber + ')', stats.localSskInsertBytesSentAverage,
				stats.localSskFetchBytesReceivedAverage, true, true, true);
		sskPutSchedulerBulk = new ClientRequestScheduler(true, true, false, random, sskInsertStarterBulk, core, core,
				core.storeChecker, "SSKinserter", ctx);
		sskPutSchedulerRT = new ClientRequestScheduler(true, true, true, random, sskInsertStarterRT, core, core,
				core.storeChecker, "SSKinserter", ctx);
	...
	private void registerSchedulerConfig(SubConfig schedulerConfig, String name, ClientRequestScheduler csBulk,
		...
		schedulerConfig.register(name + "_priority_policy", ClientRequestScheduler.PRIORITY_SOFT, name.hashCode(), true,
				false,
				"RequestStarterGroup.scheduler" + (forSSKs ? "SSK" : "CHK") + (forInserts ? "Inserts" : "Requests"),
				"RequestStarterGroup.schedulerLong", callback);
	...
	public class MyRequestThrottle implements BaseRequestThrottle {
		...
		public String toString() {
			return "rtt: " + roundTripTime.currentValue() + " _s=" + getThrottleWindow().currentValue(realTime) + " RT="
					+ realTime;
		}
	...
	public static class PrioritySchedulerCallback extends StringCallback implements EnumerableOptionCallback {
		...
		private final String[] possibleValues = new String[] { ClientRequestScheduler.PRIORITY_HARD,
	...
	public long countQueuedRequests() {
		return chkFetchSchedulerBulk.countQueuedRequests() + sskFetchSchedulerBulk.countQueuedRequests()
				+ chkPutSchedulerBulk.countQueuedRequests() + sskPutSchedulerBulk.countQueuedRequests()
				+ chkFetchSchedulerRT.countQueuedRequests() + sskFetchSchedulerRT.countQueuedRequests()
				+ chkPutSchedulerRT.countQueuedRequests() + sskPutSchedulerRT.countQueuedRequests();
	}
