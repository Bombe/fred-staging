...
import freenet.clients.fcp.FCPClientRequest;
...
import freenet.lockablebuffer.DiskSpaceCheckingRandomAccessBufferFactory;
import freenet.lockablebuffer.MaybeEncryptedRandomAccessBufferFactory;
import freenet.lockablebuffer.PooledFileRandomAccessBufferFactory;
 ...
 */
public class NodeClientCore implements Persistable, RecentlyFailedChecker, NodeClientRequest, ClientRequestsFetcher {
	...
	/**
	 * <p>
	 * Must be included as a hidden field in order for any dangerous HTTP operation to
	 * complete successfully.
	 * </p>
	 * <p>
	 * The name of the variable is badly chosen: formPassword is an
	 * <a href="https://www.owasp.org/index.php/Cross-Site_Request_Forgery_%28CSRF%29">
	 * anti-CSRF token</a>. As for when to use one, two rules:
	 * </p>
	 * <p>
	 * 1) if you're changing server side state, you need a POST request
	 * </p>
	 * <p>
	 * 2) all POST requests need an anti-CSRF token (the exception being a login page,
	 * where credentials -that are unpredictable to an attacker- are exchanged).
	 * </p>
	 * <p>
	 * In practice this means that you must use POST whenever the request can change
	 * anything such as your database contents. Other words for this would be requests
	 * which change your database or "write" requests. Read-only requests can be GET. When
	 * processing the POST-request, you MUST validate that the received password matches
	 * this variable. If it does not, you must NOT process the request. In particular, you
	 * must NOT modify anything.
	 * </p>
	 * <p>
	 * To produce a form which already contains the password, use
	 * {@link PluginRespirator#addFormChild(freenet.support.HTMLNode, String, String)}.
	 * </p>
	 * <p>
	 * To validate that the right password was received, use
	 * {@link WebInterfaceToadlet#isFormPassword(HTTPRequest)}.
	 * </p>
	 */
	...
	public final BucketFilenameGenerator tempFilenameGenerator;

	public final BucketFilenameGenerator persistentFilenameGenerator;
	...
	/**
	 * How much disk space must be free when starting a long-term, unpredictable duration
	 * job such as a big download?
	 */
	...
	private static int maxBackgroundUSKFetchers; // Client stuff that needs to be
													// configged - FIXME
	static final int MAX_ARCHIVE_HANDLERS = 200; // don't take up much RAM... FIXME
	static final long MAX_CACHED_ARCHIVE_DATA = 32 * 1024 * 1024; // make a fixed fraction
																	// of the store by
																	// default? FIXME
	static final long MAX_ARCHIVED_FILE_SIZE = 1024 * 1024; // arbitrary... FIXME
	static final int MAX_CACHED_ELEMENTS = 256 * 1024; // equally arbitrary! FIXME
														// hopefully we can cache many of
														// these though

	private FCPUserAlert startingUpAlert;
	...
	NodeClientCore(Node node, Config config, SubConfig nodeConfig, SubConfig installConfig, int portNumber,
			int sortOrder, SimpleFieldSet oldConfig, SubConfig fproxyConfig, SimpleToadletServer toadlets,
			DatabaseKey databaseKey, MasterSecret persistentSecret) throws NodeInitException {
		this.node = node;
		...
		nodeConfig.register("lazyStartDatastoreChecker", false, sortOrder++, true, false,
				"NodeClientCore.lazyStartDatastoreChecker", "NodeClientCore.lazyStartDatastoreCheckerLong",
				new BooleanCallback() {
		...
		persister = new ConfigurablePersister(this, nodeConfig, "clientThrottleFile", "client-throttle.dat",
				sortOrder++, true, false, "NodeClientCore.fileForClientStats", "NodeClientCore.fileForClientStatsLong",
		...
		try {
			tempFilenameGenerator = new BucketFilenameGenerator(random, true, getTempDir(), "temp-");
		}
		catch (IOException e) {
		...
		uskManager = new NodeUSKManager(this);

		// Persistent temp files
		nodeConfig.register("encryptPersistentTempBuckets", true, sortOrder++, true, false,
				"NodeClientCore.encryptPersistentTempBuckets", "NodeClientCore.encryptPersistentTempBucketsLong",
				new BooleanCallback() {

					@Override
					public Boolean get() {
						return (persistentTempBucketFactory == null ? true
		...
		this.persistentTempDir = node.setupProgramDir(installConfig, "persistentTempDir",
				node.userDir().file("persistent-temp").toString(), "NodeClientCore.persistentTempDir",
				"NodeClientCore.persistentTempDirLong", nodeConfig);

		fcpPersistentRoot = new PersistentRequestRoot();
		try {
			this.persistentTempBucketFactory = new PersistentTempBucketFactory(persistentTempDir.dir(), "freenet-temp-",
					node.random, node.fastWeakRandom, nodeConfig.getBoolean("encryptPersistentTempBuckets"));
		...
		// Max bucket size 5% of the total, minimum 32KB (one block, vast majority of
		// buckets)
		long maxBucketSize = Math.max(32768, (defaultRamBucketPoolSize * 1024 * 1024) / 20);

		nodeConfig.register("maxRAMBucketSize", SizeUtil.formatSizeWithoutSpace(maxBucketSize), sortOrder++, true,
				false, "NodeClientCore.maxRAMBucketSize", "NodeClientCore.maxRAMBucketSizeLong", new LongCallback() {
		...
		nodeConfig.register("RAMBucketPoolSize", defaultRamBucketPoolSize + "MiB", sortOrder++, true, false,
				"NodeClientCore.ramBucketPoolSize", "NodeClientCore.ramBucketPoolSizeLong", new LongCallback() {
		...
		nodeConfig.register("encryptTempBuckets", true, sortOrder++, true, false, "NodeClientCore.encryptTempBuckets",
		...
		tempBucketFactory = new TempBucketFactory(node.executor, tempFilenameGenerator,
				nodeConfig.getLong("maxRAMBucketSize"), nodeConfig.getLong("RAMBucketPoolSize"), node.fastWeakRandom,
				nodeConfig.getBoolean("encryptTempBuckets"), minDiskFreeShortTerm, cryptoSecretTransient);

		bandwidthStatsPutter = new PersistentStatsPutter();

		clientLayerPersister = new ClientLayerPersister(node.executor, node.ticker, node, this,
				persistentTempBucketFactory, tempBucketFactory, bandwidthStatsPutter);
		...
		archiveManager = new ArchiveManager(MAX_ARCHIVE_HANDLERS, MAX_CACHED_ARCHIVE_DATA, MAX_ARCHIVED_FILE_SIZE,
				MAX_CACHED_ELEMENTS, tempBucketFactory);

		healingQueue = new SimpleHealingQueue(
				new InsertContext(0, 2, 0, 0, new SimpleEventProducer(), false, Node.FORK_ON_CACHEABLE_DEFAULT, false,
						Compressor.DEFAULT_COMPRESSORDESCRIPTOR, 0, 0, InsertContext.CompatibilityMode.COMPAT_DEFAULT),
				PriorityClasses.PREFETCH_PRIORITY_CLASS, 512 /* FIXME make configurable */);

		PooledFileRandomAccessBufferFactory raff = new PooledFileRandomAccessBufferFactory(persistentFilenameGenerator,
				node.fastWeakRandom);
		persistentDiskChecker = new DiskSpaceCheckingRandomAccessBufferFactory(raff, persistentTempDir.dir(),
				minDiskFreeLongTerm + tempBucketFactory.getMaxRamUsed());
		persistentRAFFactory = new MaybeEncryptedRandomAccessBufferFactory(persistentDiskChecker,
		...
		int maxMemoryLimitedJobThreads = Runtime.getRuntime().availableProcessors() / 2; // Some
																							// disk
																							// I/O
																							// ...
																							// tunable
		...
		// FIXME review thread limits. This isn't just memory, it's CPU and disk as well,
		// so we don't want it too big??
		// FIXME l10n the errors?
		nodeConfig.register("memoryLimitedJobThreadLimit", maxMemoryLimitedJobThreads, sortOrder++, true, false,
				"NodeClientCore.memoryLimitedJobThreadLimit", "NodeClientCore.memoryLimitedJobThreadLimitLong",
				new IntCallback() {
		...
		nodeConfig.register("memoryLimitedJobMemoryLimit", defaultMemoryLimitedJobMemoryLimit, sortOrder++, true, false,
				"NodeClientCore.memoryLimitedJobMemoryLimit", "NodeClientCore.memoryLimitedJobMemoryLimitLong",
				new LongCallback() {
					...
					public void set(Long val) throws InvalidConfigValueException, NodeNeedRestartException {
						if (val < FECCodec.MIN_MEMORY_ALLOCATION)
							throw new InvalidConfigValueException(l10n("memoryLimitedJobMemoryLimitMustBeAtLeast",
									"min", SizeUtil.formatSize(FECCodec.MIN_MEMORY_ALLOCATION)));
						memoryLimitedJobRunner.setCapacity(val);
					}

				}, true);
		memoryLimitedJobRunner = new MemoryLimitedJobRunner(nodeConfig.getLong("memoryLimitedJobMemoryLimit"),
				nodeConfig.getInt("memoryLimitedJobThreadLimit"), node.executor,
				PriorityClasses.NUMBER_OF_PRIORITY_CLASSES);
		...
		clientContext = new ClientContext(node.bootID, clientLayerPersister, node.executor, archiveManager,
				persistentTempBucketFactory, tempBucketFactory, persistentTempBucketFactory, healingQueue, uskManager,
				random, node.fastWeakRandom, node.getTicker(), memoryLimitedJobRunner, tempFilenameGenerator,
				persistentFilenameGenerator, tempBucketFactory, persistentRAFFactory,
				tempBucketFactory.getUnderlyingRAFFactory(), persistentDiskChecker, compressor, storeChecker,
				fcpPersistentRoot, cryptoSecretTransient, toadlets, defaultFetchContext, defaultInsertContext, config,
		...
		node.securityLevels
				.addPhysicalThreatLevelListener(new SecurityLevelListener<SecurityLevels.PHYSICAL_THREAT_LEVEL>() {

					@Override
					public void onChange(SecurityLevels.PHYSICAL_THREAT_LEVEL oldLevel,
							SecurityLevels.PHYSICAL_THREAT_LEVEL newLevel) {
						...
						if (clientLayerPersister.hasLoaded()) {
							// May need to change filenames for client.dat* or even create
							// them.
							try {
		...
		this.downloadsDir = node.setupProgramDir(nodeConfig, "downloadsDir", node.userDir().file("downloads").getPath(),
				"NodeClientCore.downloadsDir", "NodeClientCore.downloadsDirLong", l10n("couldNotFindOrCreateDir"),
				(SubConfig) null);

		// Downloads allowed, uploads allowed

		nodeConfig.register("downloadAllowedDirs", new String[] { "all" }, sortOrder++, true, true,
				"NodeClientCore.downloadAllowedDirs", "NodeClientCore.downloadAllowedDirsLong",
		...
		nodeConfig.register("uploadAllowedDirs", new String[] { "all" }, sortOrder++, true, true,
				"NodeClientCore.uploadAllowedDirs", "NodeClientCore.uploadAllowedDirsLong", new StringArrCallback() {
		...
		this.alerts.register(startingUpAlert = new SimpleUserAlert(true, l10n("startingUpTitle"), l10n("startingUp"),
				l10n("startingUpShort"), UserAlert.ERROR));
		this.alerts.register(new SimpleUserAlert(true,
				NodeL10n.getBase().getString("QueueToadlet.persistenceBrokenTitle"),
				NodeL10n.getBase().getString("QueueToadlet.persistenceBroken", new String[] { "TEMPDIR", "DBFILE" },
						new String[] {
								new File(FileUtil.getCanonicalFile(getPersistentTempDir()), File.separator).toString(),
								new File(FileUtil.getCanonicalFile(node.getUserDir()), "client.dat").toString() }),
		...
		nodeConfig.register("alwaysCommit", false, sortOrder++, true, false, "NodeClientCore.alwaysCommit",
				"NodeClientCore.alwaysCommitLong", new BooleanCallback() {
	...
	protected void updatePersistentRAFSpaceLimit() {
		...
		// So we add the RAM limit for the temp factory to the disk limit for the
		// persistent one.
	...
	private void initDiskSpaceLimits(SubConfig nodeConfig, int sortOrder) {
		nodeConfig.register("minDiskFreeLongTerm", "1G", sortOrder++, true, true, "NodeClientCore.minDiskFreeLongTerm",
				"NodeClientCore.minDiskFreeLongTermLong", new LongCallback() {
		...
		nodeConfig.register("minDiskFreeShortTerm", "512M", sortOrder++, true, true,
				"NodeClientCore.minDiskFreeShortTerm", "NodeClientCore.minDiskFreeShortTermLong", new LongCallback() {
		...
		// Do not register the UserAlert yet, since we haven't finished constructing stuff
		// it uses.
	}
	...
	/**
	 * Give ClientLayerPersister a filename and possibly an encryption key. May cause it
	 * to load, but can also be called afterwards to change where to write to.
	...
	private void initStorage(DatabaseKey databaseKey) throws MasterKeysWrongPasswordException {
		clientLayerPersister.setFilesAndLoad(node.nodeDir.dir(), "client.dat", node.wantEncryptedDatabase(),
				node.wantNoPersistentDatabase(), databaseKey, clientContext, requestStarters,
				FCPClientRequest::restartFrom, random);
	}
	...
	private void finishInitStorage() {
		...
		try {
			persistentTempBucketFactory.completedInit(); // Only GC persistent-temp after
															// a successful load.
			success = true;
		}
		finally {
	...
	/**
	 * UID -1 is used internally, so never generate it. It is not however a problem if a
	 * node does use it; it will slow its messages down by them being round-robin'ed in
	 * PeerMessageQueue with messages with no UID, that's all.
	 */
	long makeUID() {
		while (true) {
	...
	@Override
	public KeyBlockStore getKeyBlockStore() {
		return node;
	}
	 ...
	 * @param offersOnly If true, only fetch the key from nodes that have offered it,
	 * using GetOfferedKeys, don't do a normal fetch for it.
	 * @param listener The listener is called if we start a request and fail to fetch, and
	 * also in most cases on success or on not starting one. FIXME it may not always be
	 * called e.g. on fetching the data from the datastore - is this a problem?
	 ...
	 * @param localOnly If true, only check the datastore, don't create a request if
	 * nothing is found.
	 * @param ignoreStore If true, don't check the datastore, create a request
	 * immediately.
	 */
	public void asyncGet(final Key key, boolean offersOnly, final RequestCompletionListener listener,
			boolean canReadClientCache, boolean canWriteClientCache, final boolean realTimeFlag, boolean localOnly,
			boolean ignoreStore) {
		...
		if (!tracker.lockUID(uid, isSSK, false, false, true, realTimeFlag, tag)) {
			Logger.error(this,
					"Could not lock UID just randomly generated: " + uid + " - probably indicates broken PRNG");
		...
		// If another node requested it within the ULPR period at a lower HTL, that may
		// allow
		// us to cache it in the datastore. Find the lowest HTL fetching the key in that
		// period,
		...
		asyncGet(key, offersOnly, uid, new RequestSenderListener() {
			...
			public void onRequestSenderFinished(int status, boolean fromOfferedKey, RequestSender rs) {
				...
				if (status != RequestSender.TIMED_OUT && status != RequestSender.GENERATED_REJECTED_OVERLOAD
					...
					(isSSK ? nodeStats.localSskFetchBytesReceivedAverage : nodeStats.localChkFetchBytesReceivedAverage)
							.report(rs.getTotalReceivedBytes());
					if (status == RequestSender.SUCCESS)
						// See comments above declaration of successful* : We don't report
						// sent bytes here.
				...
				else if (rs.hasForwarded() && ((status == RequestSender.DATA_NOT_FOUND)
						|| (status == RequestSender.RECENTLY_FAILED) || (status == RequestSender.SUCCESS)
						|| (status == RequestSender.ROUTE_NOT_FOUND) || (status == RequestSender.VERIFY_FAILURE)
						|| (status == RequestSender.GET_OFFER_VERIFY_FAILURE))) {
					long rtt = System.currentTimeMillis() - startTime;
					double targetLocation = key.toNormalizedDouble();
					if (!rejectedOverload)
						requestStarters.requestCompleted(isSSK, false, key, realTimeFlag);
					// Count towards RTT even if got a RejectedOverload - but not if timed
					// out.
				...
				if (status == RequestSender.SUCCESS)
					// FIXME how to identify failed to decode and report it back to the
					// client layer??? do we even need to???
					listener.onSucceeded();
				else {
					switch (status) {
	...
	/**
	 * Start an asynchronous fetch of the key in question, which will complete to the
	 * datastore. It will not decode the data because we don't provide a ClientKey. It
	 * will not return anything and will run asynchronously. Caller is responsible for
	 * unlocking the UID.
	 * @param key The key being fetched.
	 * @param offersOnly If true, only fetch the key from nodes that have offered it,
	 * using GetOfferedKeys, don't do a normal fetch for it.
	 * @param uid The UID of the request. This should already be locked, see the tag.
	 * @param tag The RequestTag for the request. In case of an error when starting it we
	 * will unlock it, but in other cases the listener should unlock it.
	 ...
	 * @param canReadClientCache Can this request read the client-cache?
	 * @param canWriteClientCache Can this request write the client-cache?
	 * @param htl The HTL to start the request at. See the caller, this can be modified in
	 * the case of fetching an offered key.
	 * @param realTimeFlag Is this a real-time request? False = this is a bulk request.
	 * @param localOnly If true, only check the datastore, don't create a request if
	 * nothing is found.
	 * @param ignoreStore If true, don't check the datastore, create a request
	...
	void asyncGet(Key key, boolean offersOnly, long uid, RequestSenderListener listener, RequestTag tag,
			boolean canReadClientCache, boolean canWriteClientCache, short htl, boolean realTimeFlag, boolean localOnly,
			boolean ignoreStore) {
		try {
	...
	public ClientKeyBlock realGetKey(ClientKey key, boolean localOnly, boolean ignoreStore, boolean canWriteClientCache,
			boolean realTimeFlag) throws LowLevelGetException {
	 ...
	 * @param localOnly
	 * @param ignoreStore
	 * @param canWriteClientCache Can we write to the client cache? This is a local
	 * request, so we can always read from it, but some clients will want to override to
	 * avoid polluting it.
	 * @return The fetched block.
	 * @throws LowLevelGetException
	 */
	ClientCHKBlock realGetCHK(ClientCHK key, boolean localOnly, boolean ignoreStore, boolean canWriteClientCache,
			boolean realTimeFlag) throws LowLevelGetException {
		...
		try {
			Object o = node.makeRequestSender(key.getNodeCHK(), node.maxHTL(), uid, tag, null, localOnly, ignoreStore,
					false, true, canWriteClientCache, realTimeFlag);
	...
	ClientSSKBlock realGetSSK(ClientSSK key, boolean localOnly, boolean ignoreStore, boolean canWriteClientCache,
			boolean realTimeFlag) throws LowLevelGetException {
		long startTime = System.currentTimeMillis();
		long uid = makeUID();
		...
		try {
			...
			while (true) {
				...
				if (status != RequestSender.TIMED_OUT && status != RequestSender.GENERATED_REJECTED_OVERLOAD
					...
					if (status == RequestSender.SUCCESS)
						// See comments above successfulSskFetchBytesSentAverage : we
						// don't relay the data, so
	...
	/**
	 * Start a local request to insert a block. Note that this is a KeyBlock not a
	 * ClientKeyBlock mainly because of random reinserts.
	 * @param block
	 * @param canWriteClientCache
	 * @throws LowLevelPutException
	 */
	public void realPut(KeyBlock block, boolean canWriteClientCache, boolean forkOnCacheable, boolean preferInsert,
			boolean ignoreLowBackoff, boolean realTimeFlag) throws LowLevelPutException {
	...
	public void realPutCHK(CHKBlock block, boolean canWriteClientCache, boolean forkOnCacheable, boolean preferInsert,
			boolean ignoreLowBackoff, boolean realTimeFlag) throws LowLevelPutException {
		...
		try {
			long startTime = System.currentTimeMillis();
			is = node.makeInsertSender(block.getKey(), node.maxHTL(), uid, tag, null, headers, prb, false,
			...
			while (true) {
				...
				if (is.anyTransfersFailed() && (!hasReceivedRejectedOverload)) {
					hasReceivedRejectedOverload = true; // not strictly true but same
														// effect
					requestStarters.rejectedOverload(false, true, realTimeFlag);
				}
			}
			...
			if (logMINOR)
				Logger.minor(this,
						"Completed " + uid + " overload=" + hasReceivedRejectedOverload + ' ' + is.getStatusString());
			...
			// Get status explicitly, *after* completed(), so that it will be
			// RECEIVE_FAILED if the receive failed.
	...
	public void realPutSSK(SSKBlock block, boolean canWriteClientCache, boolean forkOnCacheable, boolean preferInsert,
			boolean ignoreLowBackoff, boolean realTimeFlag) throws LowLevelPutException {
		SSKInsertSender is;
		long uid = makeUID();
		...
		try {
			long startTime = System.currentTimeMillis();
			// Be consistent: use the client cache to check for collisions as this is a
			// local insert.
			...
			is = node.makeInsertSender(block, node.maxHTL(), uid, tag, null, false, canWriteClientCache, false,
			...
			if (is.hasCollided()) {
				...
				// Store it locally so it can be fetched immediately, and overwrites any
				// locally inserted.
				try {
	 ...
	 * but their transfers are faster. Latency-optimised requests are expected to be
	 * bursty, whereas throughput-optimised (bulk) requests can be constant.
	 */
	public HighLevelSimpleClient makeClient(short prioClass, boolean forceDontIgnoreTooManyPathComponents,
	...
	/**
	 * Returns the link filter exception provider of the node. At the moment this is the
	 * {@link #getToadletContainer() toadlet container}.
	...
	public void queueRandomReinsert(KeyBlock block) {
		NodeSimpleSendableInsert ssi = new NodeSimpleSendableInsert(this, block,
				PriorityClasses.MAXIMUM_PRIORITY_CLASS);
		if (logMINOR)
	...
	public boolean allowDownloadTo(File filename) {
		SecurityLevels.PHYSICAL_THREAT_LEVEL physicalThreatLevel = node.securityLevels.getPhysicalThreatLevel();
	...
	/*
	 * FIXME SECURITY When/if introduce tunneling or similar mechanism for starting
	 * requests at a distance this will need to be reconsidered. See the comments on the
	 * caller in RequestHandler (onAbort() handler).
	 */
	...
	public long checkRecentlyFailed(Key key, boolean realTime) {
		...
		node.peers.closerPeer(null, new HashSet<PeerNode>(), key.toNormalizedDouble(), true, false, -1, null, 2.0, key,
				origHTL, 0, true, realTime, r, false, System.currentTimeMillis(),
