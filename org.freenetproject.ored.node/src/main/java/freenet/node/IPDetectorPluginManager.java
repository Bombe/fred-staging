...
/**
 * Tracks all known IP address detection plugins, and runs them when appropriate. Normally
 * there would only be one, but sometimes there may be more than one.
 */
public class IPDetectorPluginManager implements ForwardPortCallback {

	public class PortForwardAlert extends BaseNodeUserAlert {
		...
		public String getShortText() {
			String prefix = innerGetPriorityClass() == FCPUserAlert.ERROR ? l10n("seriousConnectionProblems")
			...
			if (portsNotForwarded.length == 1) {
				...
				return prefix + l10n("forwardTwoPortsShort" + keySuffix, new String[] { "port1", "port2" },
						new String[] { Integer.toString(Math.abs(portsNotForwarded[0])),
	...
	public class MyUserAlert extends BaseNodeUserAlert {
		...
		public MyUserAlert(String title, String text, boolean suggestPortForward, short code) {
			super(false, title, text, title, null, code, true, NodeL10n.getBase().getString("UserAlert.hide"), false,
					null);
		...
		public HTMLNode getHTMLText() {
			...
			if (suggestPortForward) {
				if (portsNotForwarded.length == 1) {
					NodeL10n.getBase().addL10nSubstitution(div, "IPDetectorPluginManager.suggestForwardPortWithLink",
							new String[] { "link", "port" },
							new HTMLNode[] {
									HTMLNode.link(ExternalLinkToadlet
											.escape("http://wiki.freenetproject.org/FirewallAndRouterIssues")),
					...
					NodeL10n.getBase().addL10nSubstitution(div,
							...
							new HTMLNode[] {
									HTMLNode.link(ExternalLinkToadlet
											.escape("http://wiki.freenetproject.org/FirewallAndRouterIssues")),
									HTMLNode.text(portsNotForwarded[0]), HTMLNode.text(portsNotForwarded[1]) });
				}
			}
			return div;
		}
	...
	IPDetectorPluginManager(Node node, NodeIPDetector detector) {
		...
		noConnectionAlert = new MyUserAlert(l10n("noConnectivityTitle"), l10n("noConnectivity"), true,
				FCPUserAlert.ERROR);
		symmetricAlert = new MyUserAlert(l10n("symmetricTitle"), l10n("symmetric"), true, FCPUserAlert.ERROR);
		portRestrictedAlert = new MyUserAlert(l10n("portRestrictedTitle"), l10n("portRestricted"), true,
				FCPUserAlert.WARNING);
		restrictedAlert = new MyUserAlert(l10n("restrictedTitle"), l10n("restricted"), false, FCPUserAlert.MINOR);
		portForwardAlert = new PortForwardAlert();
	}

	/**
	 * Return all the ports that we have reason to believe are not forwarded. E.g. for the
	 * user-alert, which only shows if what we return is of nonzero length.
	 */
	public int[] getUDPPortsNotForwarded() {
		OpennetManager om = node.getOpennet();
		Status darknetStatus = (node.peers.anyDarknetPeers() ? node.darknetCrypto.getDetectedConnectivityStatus()
		...
		if (om == null || opennetStatus.ordinal() >= AddressTracker.Status.DONT_KNOW.ordinal()) {
			if (darknetStatus.ordinal() >= AddressTracker.Status.DONT_KNOW.ordinal()) {
				...
				return new int[] { (darknetStatus.ordinal() < AddressTracker.Status.MAYBE_NATED.ordinal() ? -1 : 1)
						* node.getDarknetPortNumber() };
			}
		}
		else {
			if (darknetStatus.ordinal() >= AddressTracker.Status.DONT_KNOW.ordinal()) {
				return new int[] { (opennetStatus.ordinal() < AddressTracker.Status.MAYBE_NATED.ordinal() ? -1 : 1)
	...
	public String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase().getString("IPDetectorPluginManager." + key, new String[] { pattern },
				new String[] { value });
	}
	...
	/**
	 * Start the detector plugin manager. This includes running the plugin, if there is
	 * one, and if it is necessary to do so.
	 */
	void start() {
	...
	/**
	 * Start the plugin detection, if necessary. Either way, schedule another attempt in 1
	 * minute's time.
	 */
	...
	/*
	 * When should we run an IP address detection? This is for things like STUN, so there
	 * may conceivably be some exposure or risk, or limited resources, so not all the
	 * time.
	 *
	 * If we don't get a real IP address from a detection, we should not run another one
	 * for 5 minutes. This indicated that we were not on the internet *at all*.
	 *
	 * If we have a directly detected IP, and: - We have no peers older than 30 minutes OR
	 * - We have successfully connected to two different peers with different real
	 * internet addresses to us since startup
	 *
	 * Then we should not run a detection. (However, we don't entirely exclude it because
	 * we may be behind a firewall).
	 *
	 * If we have no peers, and we haven't run a detection in the last 6 hours (don't save
	 * this time over startups), we should run a detection.
	 *
	 * Otherwise, we have peers, and if we have run a detection in the last hour we should
	 * not run another one.
	 *
	 * If we have one or two connected peers, both of which report the same IP address,
	 * and we have other nodes which have been connected recently, and this state has
	 * persisted for 2 minutes, we should run a detection. (To protect against bogus IP
	 * address reports)
	 *
	 ...
	 * detection. (every hour that we are down) (To detect new IP address)
	 */
	...
	/**
	 * Given that we have no peers, should we run the detection plugins? Algorithm: Run
	 * the detection once every 6 hours.
	...
	private boolean shouldDetectWithPeers(long now, PeerNode[] peers, PeerNode[] conns,
		...
		if (realConnections == 0 && realDisconnected > 0) {
			...
			if (detector.oldIPAddress != null && detector.oldIPAddress.isRealInternetAddress(false, false, false)) {
				...
				// Allow 2 minutes to get incoming connections and therefore detect from
				// them.
		...
		// If we have no connections, and have lost several connections recently, we
		// should
		...
		// If it appears to be an SNAT, do a detection at least once to verify that, and
		// to
					...
					if (p.isConnected() && (addr != null) && IPUtil.isValidAddress(peer.getAddress(), false)) {
						...
						if (!internal) {
							...
							if (addressesConnected.size() > 2) {
								// 3 connected addresses, lets assume we have
								// connectivity.
								if (logMINOR)
	...
	public class DetectorRunner implements Runnable {
		...
		public void realRun() {
			...
			try {
				...
				// Node does not know about individual interfaces, so just process the
				// lot.

				// FIXME if we use the interfaces we should simply take the most popular
				...
				if (connectionType == DetectedIP.NO_UDP) {
					...
					synchronized (this) {
						if (noConnectivityAlert == null)
							noConnectivityAlert = toRegister = new SimpleUserAlert(false, l10n("noConnectivityTitle"),
									l10n("noConnectivity"), l10n("noConnectivityShort"), FCPUserAlert.ERROR);
					}
					if (toRegister != null)
	...
	public void portForwardStatus(Map<ForwardPort, ForwardPortStatus> statuses) {
		...
		for (ForwardPort p : currentPorts) {
			...
			if (status.status == ForwardPortStatus.DEFINITE_SUCCESS) {
				...
				Logger.normal(this, "Probably succeeded forwarding " + p.name + " port " + p.portNumber + " for "
						+ p.protocol + " - port forward probably succeeded " + status.reasonString);
			}
			else if (status.status == ForwardPortStatus.MAYBE_SUCCESS) {
				Logger.normal(this,
						"Maybe succeeded forwarding " + p.name + " port " + p.portNumber + " for " + p.protocol
				...
				Logger.error(this, "Probably failed forwarding " + p.name + " port " + p.portNumber + " for "
						+ p.protocol + " - port forward probably failed " + status.reasonString);
			}
