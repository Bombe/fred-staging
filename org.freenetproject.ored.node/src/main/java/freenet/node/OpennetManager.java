...
public class OpennetManager {
	...
	public void start() {
		...
		// ready.
		node.peers.tryReadPeers(node.nodeDir().file("openpeers-" + crypto.portNumber).toString(), crypto, this, true,
				false);
		...
		writeFile();
		// Read old peers
		node.peers.tryReadPeers(node.nodeDir().file("openpeers-old-" + crypto.portNumber).toString(), crypto, this,
				true, true);
		crypto.start();
		if (announcer != null)
			announcer.start();
	...
	public boolean wantPeer(OpennetPeerNode nodeToAddNow, boolean addAtLRU, boolean justChecking,
		...
		if (notMany) {
			if (nodeToAddNow != null) {
				node.peers.addPeer(nodeToAddNow, true, true); // Add to peers outside the
																// OM lock
			}
			return true;
		}
		...
		synchronized (this) {
			...
			if (size == maxPeers && nodeToAddNow == null) {
				...
				if (toDrop == null) {
					...
					// limit.
					if (toDrop.isConnected() && enforcePerTypeGracePeriodLimits(maxPeers, connectionType,
					...
					if (nodeToAddNow != null || size > maxPeers) {
						if (logMINOR)
							Logger.minor(this, "Drop opennet peer: " + toDrop + " (connected=" + toDrop.isConnected()
									+ ") of " + peersLRU.size() + ":" + getSize(distance));
	...
	public void onSuccess(OpennetPeerNode pn) {
		...
		if (!wantPeer(pn, false, false, false, ConnectionType.RECONNECT, distance)) // Start
																					// at
																					// top
																					// as
																					// it
	...
	private boolean innerSendOpennetRef(long xferUID, byte[] padded, PeerNode peer, ByteCounter ctr, AllSentCallback cb)
		...
		raf.setReadOnly();
		PartiallyReceivedBulk prb = new PartiallyReceivedBulk(node.usm, padded.length, Node.PACKET_SIZE, raf, true);
		try {
	...
	public long startSendAnnouncementRequest(long uid, PeerNode peer, byte[] noderef, ByteCounter ctr, double target,
		...
		long xferUID = node.random.nextLong();
		Message msg = DMT.createFNPOpennetAnnounceRequest(uid, xferUID, noderef.length, paddedSize(noderef.length),
				target, htl);
			...
			node.usm.addAsyncFilter(mf, new SlowAsyncMessageFilterCallback() {
				...
				public void onMatched(Message msg) {
					if (msg.getSpec() == DMT.FNPOpennetCompletedAck
						...
						complete(innerWaitForOpennetNoderef(xferUID, paddedLength, realLength, source, isReply, uid,
								false, ctr, node));
					}
				}
