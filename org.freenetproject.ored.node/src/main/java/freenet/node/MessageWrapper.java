...
public class MessageWrapper {
		...
		synchronized (acks) {
			...
			if (acks.contains(0, item.buf.length - 1)) {
				if (!alreadyAcked) {
					...
					if (logMINOR)
						Logger.minor(this,
								"Total round trip time for message " + messageID + " : " + item + " : "
										+ (System.currentTimeMillis() - created) + " in " + resends + " resends"
	...
	/**
	 * Marks the given range of bytes as lost and returns the number of bytes that were
	 * lost. The input range is inclusive.
			...
			synchronized (acks) {
				...
				for (int[] range : acks) {
					...
					Logger.warning(this, "Lost range (" + start + "->" + end + ") is overlapped by acked range ("
							+ range[0] + "->" + range[1] + "). Adding " + toAddStart + "->" + toAddEnd + " to sent");
	...
	/**
	 * Returns a {@code MessageFragment} with a length of {@code maxLength} or less, or
	 * {@code null} if there is nothing to send. Ranges that have been returned by this
	 * function, and not marked as lost, and data that has been acked is never returned.
	...
	public MessageFragment getMessageFragment(int maxLength) {
		...
		synchronized (sent) {
			...
			dataLength = maxLength - 2 // Message id + flags
					- (isShortMessage ? 1 : 2); // Fragment length

			if (isFragmented(dataLength)) {
				dataLength -= (isShortMessage ? 1 : 3); // Message length / fragment
														// offset
			}
			...
			if (logDEBUG)
				Logger.debug(this, "Using range " + start + " to " + (start + dataLength - 1) + " gives " + sent
						+ " on " + messageID);
		}

		boolean isFragmented = !((start == 0) && (dataLength == item.buf.length));
		return new MessageFragment(isShortMessage, isFragmented, start == 0, messageID, dataLength, item.buf.length,
				start, fragmentData, this);
	}
	...
	/**
	 * Returns {@code true} if all the data of this {@code MessageWrapper} has been sent,
