...
public class BandwidthManager {
	...
	public void start() {
		...
		/*
		 * offer upgrade of the connection speed on upgrade, if auto-detected speed is
		 * much higher than the set speed, or even better: if the detected speed increased
		 * significantly since the last offer.
		 */
		node.ticker.queueTimedJob(new Runnable() {
			...
			public void run() {
				try {
					...
					if (!node.config.get("node").getBoolean("connectionSpeedDetection") || bandwidthIndicator == null) {
						return;
					}
	...
	public static void checkOutputBandwidthLimit(int obwLimit) throws InvalidConfigValueException {
		...
		// Fixme: Node outputThrottle.changeNanosAndBucketSize(SECONDS.toNanos(1) /
		// obwLimit, ...
		if (obwLimit > SECONDS.toNanos(1)) {
			throw new InvalidConfigValueException(NodeL10n.getBase().getString("Node.outputBwlimitMustBeLessThan",
	...
	private static InvalidConfigValueException lowBandwidthLimit(int limit) {
		return new InvalidConfigValueException(
				NodeL10n.getBase().getString("Node.bandwidthMinimum", new String[] { "limit", "minimum" },
