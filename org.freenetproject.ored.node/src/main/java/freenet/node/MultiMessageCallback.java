...
/**
 * Groups a set of message sends together so that we get a single sent(boolean) callback
 * after all the messages have been sent, and then later a finished(boolean). None of the
 * callbacks will be called until after you call arm(). Typical usage:
 * <pre>Message m1 = ...;
 * Message m2 = ...;
 * PeerNode pn = ...;
...
public abstract class MultiMessageCallback {
	...
	/**
	 * True if arm() has been called. finish(boolean) and sent(boolean) will only be
	 * called after arming the callback.
	 */
	...
	/**
	 * Enable the callback. The callbacks sent(boolean) and finish(boolean) will only be
	 * called after this method has been called, so you should use this to indicate that
	 * you won't add any more messages.
	 */
	public void arm() {
		boolean success;
		boolean callSent = false;
