...
public class NewPacketFormat implements PacketFormat {
	...
	// FIXME This should be globally allocated according to available memory etc. For
	// links with
	// high bandwidth and high latency, and lots of memory, a much bigger buffer would be
	// helpful.
	...
	/**
	 * The actual buffer of outgoing messages that have not yet been acked. LOCKING:
	 * Protected by sendBufferLock.
	 */
	...
	/**
	 * The first message id that hasn't been acked by the receiver. LOCKING: Protected by
	 * (this).
	 */
	private int messageWindowPtrAcked;

	/**
	 * All messages that have been acked (we remove those which are out of window to limit
	 * space usage). LOCKING: Protected by (this).
	 */
	 ...
	 * have we used in our send buffer, namely startedByPrio? LOCKING: Protected by
	 * sendBufferLock
	 */
	private int sendBufferUsed = 0;

	/**
	 * Lock protecting buffer usage counters, and the buffer itself (startedByPrio). MUST
	 * BE TAKEN LAST. Justification: The outgoing buffer and the buffer usage should be
	 * protected by the same lock, for consistency. The buffer usage and the connection
	 * status must be protected by the same lock, so we don't send packets when we are
	 * disconnected and get race conditions in onDisconnect(). The incoming buffer
	 * estimate could be separated in theory.
	 */
	...
	private NPFPacket tryDecipherPacket(byte[] buf, int offset, int length, SessionKey sessionKey) {
		...
		// The entry for the highest received sequence number is kept in the middle of the
		// list
		int oldHighestReceived = (int) ((0l + keyContext.watchListOffset + (keyContext.seqNumWatchList.length / 2))
				% NUM_SEQNUMS);
		...
		for (int i = 0; i < keyContext.seqNumWatchList.length; i++) {
			...
			if (!Fields.byteArrayEqual(buf, keyContext.seqNumWatchList[index], offset + HMAC_LENGTH, 0,
	...
	private boolean seqNumGreaterThan(long i1, long i2, int serialBits) {
		// halfValue is half the window of possible numbers, so this returns true if the
		// distance from
	...
	boolean maybeSendPacket(boolean ackOnly, SessionKey sessionKey) throws BlockedTooLongException {
		...
		try {
			if (logMINOR) {
				...
				Logger.minor(this,
						"Sending packet " + packet.getSequenceNumber() + " (" + data.length + " bytes) with fragments "
								+ fragments + " and " + packet.getAcks().size() + " acks on " + this);
			}
	...
	NPFPacket createPacket(int maxPacketSize, PeerMessageQueue messageQueue, SessionKey sessionKey, boolean ackOnly)
		...
		if (!ackOnly) {
			...
			while (true) {
				...
				synchronized (sendBufferLock) {
					// Always finish what we have started before considering sending more
					// packets.
					// Anything beyond this is beyond the scope of NPF and is
					// PeerMessageQueue's job.
		...
		if ((!ackOnly) && (!cantSend)) {
			...
			fragments: for (int i = 0; i < startedByPrio.size(); i++) {
				...
				prio: while (true) {
					...
					while ((packet.getLength() + 10) < maxPacketSize) { // Fragment header
																		// is max 9 bytes,
																		// allow min 1
																		// byte data
						...
						if (item == null) {
							if (mustSendKeepalive && packet.noFragments()) {
								...
								// It will be acked, this ensures both sides don't
								// timeout.
								Message msg;
								synchronized (this) {
								...
								// Should we report this on the PeerNode's stats? We'd
								// need to run a job off-thread, so probably not worth it.
							}
							else {
								break prio;
							}
						...
						if (messageID == -1) {
							// CONCURRENCY: This will fail sometimes if we send messages
							// to the same peer from different threads.
							// This doesn't happen at the moment because we use a single
							// PacketSender for all ports and all peers.
							// We might in future split it across multiple threads but
							// it'd be best to keep the same peer on the same thread.
							Logger.error(this,
							...
							if (!wasGeneratedPing) {
								...
								// But odds are the connection is broken and the other
								// side isn't responding...
							}
							break fragments;
						}
						...
						synchronized (sendBufferLock) {
							// CONCURRENCY: This could go over the limit if we allow
							// createPacket() for the same node on two threads in
							// parallel. That's probably a bad idea anyway.
							sendBufferUsed += item.buf.length;
							if (logDEBUG)
								Logger.debug(this, "Added " + item.buf.length + " to remote buffer. Total is now "
										+ sendBufferUsed + " for " + pn.shortToString());
	...
	public List<MessageItem> onDisconnect() {
		...
		synchronized (sendBufferLock) {
			...
			if (sendBufferUsed != 0) {
				Logger.warning(this, "Possible leak in transport code: Buffer size not empty after disconnecting on "
						+ this + " for " + pn + " after removing " + messageSize + " total was " + sendBufferUsed);
				sendBufferUsed = 0;
			}
		}
		return items;
	...
	public long timeNextUrgent(boolean canSend, long now) {
			...
			synchronized (sendBufferLock) {
				for (Map<Integer, MessageWrapper> started : startedByPrio) {
					for (MessageWrapper wrapper : started.values()) {
						...
						// The RTO computation logic should ensure that we don't use
						// horrible amounts of bandwidth for retransmission.
						long d = wrapper.getItem().getDeadline();
						if (d > 0)
							ret = Math.min(ret, d);
						else
							Logger.error(this,
									"Started sending message " + wrapper.getItem() + " but deadline is " + d);
					}
				}
			}
		}
		...
		if (ret > now) {
			// Always wake up after half an RTT, check whether stuff is lost or needs
			// ack'ing.
	...
	public boolean canSend(SessionKey tracker) {
		...
		synchronized (this) {
			...
			canAllocateID = !seqNumGreaterThan(nextMessageID,
					(messageWindowPtrAcked + MSG_WINDOW_SIZE) % NUM_MESSAGE_IDS, 28);
		}
		...
		if (tracker != null && pn != null) {
			...
			if (throttle == null) {
				...
				if (maxPackets <= packets.countSentPackets()) {
					// FIXME some packets will be visible from the outside yet only
					// contain acks.
					// SECURITY/INVISIBILITY: They won't count here, this is bad.
					// However, counting packets in flight, rather than bytes of messages,
					// is the right solution:
					// 1. It's closer to what TCP does.
					// 2. It avoids needing to have an excessively high minimum window
					// size.
					// 3. It allows us to start work on any message even if it's big,
					// while still having reasonably accurate congestion control.
					// This prevents us from getting into a situation where we never use
					// up the full window but can never send big messages either.
					// 4. It's closer to what we used to do (only limit big packets),
					// which seemed to work mostly.
					// 5. It avoids some complicated headaches with PeerMessageQueue. E.g.
					// we need to avoid requeueing.
					// 6. In spite of the issue with acks, it's probably more "invisible"
					// on the whole, in that the number of packets is visible,
			...
			while (msgIt.hasNext()) {
				...
				if (wrapper.ack(range[0], range[1], npf.pn)) {
					...
					if (removed == null && logMINOR) {
						// ack() can return true more than once, it just only calls the
						// callbacks once.
						Logger.minor(this, "Completed message " + wrapper.getMessageID()
								+ " but it is not in the map from " + wrapper);
					}
	...
	private static class PartiallyReceivedBuffer {
		...
		private boolean setMessageLength(int messageLength) {
			...
			if (buffer.length > messageLength) {
				Logger.warning(this,
						"Buffer is larger than set message length! (" + buffer.length + ">" + messageLength + ")");
			}
		...
		private boolean resize(int length) {
			...
			synchronized (npf.receiveBufferSizeLock) {
				...
				if (logDEBUG)
					Logger.debug(this,
							"Added " + (length - buffer.length) + " to buffer. Total is now " + npf.receiveBufferUsed);
			}
