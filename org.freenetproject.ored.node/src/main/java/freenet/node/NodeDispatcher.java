...
public class NodeDispatcher implements Dispatcher, Runnable {
	...
	private void innerHandleDataRequest(Message m, PeerNode source, boolean isSSK) {
		...
		NodeStats.RejectReason rejectReason = nodeStats.shouldRejectRequest(!isSSK, false, isSSK, false, false, source, block != null, false, realTimeFlag, tag);
	...
	private void handleInsertRequest(Message m, PeerNode source, boolean isSSK) {
		...
		NodeStats.RejectReason rejectReason = nodeStats.shouldRejectRequest(!isSSK, true, isSSK, false, false, source, false, preferInsert, realTimeFlag, tag);
		if(rejectReason != null) {
