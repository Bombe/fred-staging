...
public class NodeDispatcher implements Dispatcher, Runnable {
	...
	public boolean handleMessage(Message m) {
		...
		if (spec == DMT.FNPPing) {
			...
			try {
				source.sendAsync(reply, null, pingCounter); // nothing we can do if can't
															// contact source
			}
	...
	private boolean handlePeerLoadStatus(Message m, PeerNode source) {
		NodeStats.PeerLoadStats stat = node.nodeStats.parseLoadStats(source, m);
	...
	private boolean handleGetOfferedKey(Message m, PeerNode source) {
		...
		if (!HMAC.verifyWithSHA256(node.failureTable.offerAuthenticatorKey, key.getFullKey(), authenticator)) {
			...
			try {
				source.sendAsync(DMT.createFNPGetOfferedKeyInvalid(uid, DMT.GET_OFFERED_KEY_REJECTED_BAD_AUTHENTICATOR),
						null, node.failureTable.senderCounter);
			}
			catch (NotConnectedException e) {
				// Too bad.
			}
		...
		try {
			...
			NodeStats.RejectReason reject = nodeStats.shouldRejectRequest(true, false, isSSK, false, true, source,
	...
	private void innerHandleDataRequest(Message m, PeerNode source, boolean isSSK) {
		...
		NodeStats.RejectReason rejectReason = nodeStats.shouldRejectRequest(!isSSK, false, isSSK, false, false, source,
				block != null, false, realTimeFlag, tag);
	...
	private void handleInsertRequest(Message m, PeerNode source, boolean isSSK) {
		...
		NodeStats.RejectReason rejectReason = nodeStats.shouldRejectRequest(!isSSK, true, isSSK, false, false, source,
	...
	private boolean handleAnnounceRequest(Message m, PeerNode source) {
		...
		try {
				...
				cb = new AnnouncementCallback() {
					...
					public void completed() {
						...
						if (acceptedSomewhere)
							Logger.minor(this, "Announcement from " + origin + " completed (" + totalAdded + " added, "
			...
			AnnounceSender sender = new AnnounceSender(target, htl, uid, source, om, node, xferUID, noderefLength,
					paddedLength, cb);
