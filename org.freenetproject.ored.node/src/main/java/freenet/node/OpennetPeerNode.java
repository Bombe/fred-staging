...
public class OpennetPeerNode extends PeerNode {
	...
	// Not persisted across restart, since after restart grace periods don't apply anyway
	// (except disconnection, which is really separate anyway).
	private OpennetManager.ConnectionType opennetNodeAddedReason;

	public OpennetPeerNode(SimpleFieldSet fs, Node node2, NodeCrypto crypto, OpennetManager opennet, boolean fromLocal)
	...
	enum NOT_DROP_REASON {
		...
		DROPPABLE, TOO_NEW_PEER, TOO_LOW_UPTIME, RECONNECT_GRACE_PERIOD
	...
	/**
	 * Is the peer droppable? SIDE EFFECT: If we are now outside the grace period, we
	 * reset peerAddedTime and opennetPeerAddedReason. Note that the caller must check
	 * separately whether the node is TOO OLD and connected.
	 */
	public NOT_DROP_REASON isDroppableWithReason(boolean ignoreDisconnect) {
		...
		if (age < OpennetManager.DROP_MIN_AGE) {
			if (status == PeerManager.PEER_NODE_STATUS_NEVER_CONNECTED) {
				...
				// This prevents various dubious ways of staying connected while not
				// delivering anything useful.
		...
		if (now - node.usm.getStartedTime() < OpennetManager.DROP_STARTUP_DELAY)
			return NOT_DROP_REASON.TOO_LOW_UPTIME; // Give them time to connect after we
													// startup
		if (!ignoreDisconnect) {
			synchronized (this) {
				// This only applies after it has connected, and only if
				// !ignoreDisconnect.
	...
	private boolean shouldDisconnectTooOld() {
		...
		if (timeSinceSentUOM() < SECONDS.toMillis(60)) {
			// Let it finish.
			// 60 seconds extra to ensure it has time to parse the jar and start fetching
			// dependencies.
			return false;
		}
		return true;
	...
	@Override
	/**
	 * Opennet nodes need to know when a peer node was added. We do NOT clear it on
	 * connect, because we use it for determining whether we are in the initial grace
	 * period. However we will reset it after the grace period expires, in
	 * isDroppableWithReason().
	 */
	...
	@Override
	/*
	 * Opennet peers do not export the peer added time. It is only relevant for the grace
	 * period anyway.
	 */
	...
	public OpennetManager.LinkLengthClass linkLengthClass() {
		if (!Location.isValid(getLocation())) {
			...
			return OpennetManager.LinkLengthClass.SHORT; // FIXME add unknown to enum?
															// Would need more complex
															// error handling...
		}
