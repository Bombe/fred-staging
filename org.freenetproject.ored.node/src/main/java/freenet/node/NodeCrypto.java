...
public class NodeCrypto {
	...
	/**
	 * The object which handles our specific UDP port, pulls messages from it, feeds them
	 * to the packet mangler for decryption etc
	 */
	 ...
	 */
	public NodeCrypto(final Node node, final boolean isOpennet, NodeCryptoConfig config, long startupTime,
			boolean enableARKs) throws NodeInitException {
	...
	/**
	 * Export my node reference so that another node can connect to me. Public version,
	 * includes everything apart from private keys.
	 ...
	 * exchanged immediately after connection setup. I.e. strip out everything that is
	 * invariant, or that can safely be exchanged later.
	 * @param forAnonInitiator If true, we are adding a node from an anonymous initiator
	 * noderef exchange. Minimal noderef which we can construct a PeerNode from. Short
	 * lived so no ARK etc. Already signed so dump the signature.
	 */
	SimpleFieldSet exportPublicFieldSet(boolean forSetup, boolean forAnonInitiator, boolean forARK) {
		...
		if ((!forAnonInitiator) && (!forSetup)) {
			...
			if (ips != null) {
				for (Peer ip : ips)
					fs.putAppend("physical.udp", ip.toString()); // Keep; important that
																	// node know all our
																	// IPs
			}
		...
		if (!(forARK || forSetup || forAnonInitiator)) {
			// We *do* need the location on noderefs exchanged via path folding and
			// announcement.
			// This is necessary so we can take the location into account in
			// OpennetManager.wantPeer().
			fs.put("location", node.lm.getLocation());
		}
		fs.putSingle("version", Version.getVersionString()); // Keep, vital that peer know
																// our version. For
																// example, some types may
																// be sent in different
																// formats to different
																// node versions (e.g.
																// Peer).
		if (!forAnonInitiator)
		...
		if (Node.isTestnetEnabled()) {
			fs.put("testnet", true);
			// fs.put("testnetPort", node.testnetHandler.getPort()); // Useful, saves a
			// lot of complexity
		}
		...
		if (!forAnonInitiator) {
			// Anonymous initiator setup type specifies whether the node is opennet or
			// not.
	...
	SimpleFieldSet exportPublicCryptoFieldSet(boolean forSetup, boolean forAnonInitiator) {
		...
		if (!forSetup) {
			// These are invariant. They cannot change on connection setup. They can
			// safely be excluded.
		...
		if (!forAnonInitiator) {
			...
			if (!forSetup) {
				...
				fs.putSingle("ark.pubURI", myARK.getURI().toString(false, false)); // Can
																					// be
																					// changed
																					// on
																					// setup
	...
	/**
	 * The part of our node reference which is exchanged in the connection setup,
	 * compressed.
	...
	/**
	 * The part of our node reference which is exchanged in the connection setup, if we
	 * don't already have the node, compressed.
	 * @see exportSetupFieldSet()
	 */
	...
	/**
	 * Sign data with the node's ECDSA key. The data does not need to be hashed, the
	 * signing code will handle that for us, using an algorithm appropriate for the
	 * keysize.
	 */
	...
	/**
	 * If oneConnectionPerAddress is not set, but there are peers with the same IP for
	 * which it is set, disconnect them.
	 * @param peerNode
	 * @param address
	 */
			...
			if (pn.crypto.config.oneConnectionPerAddress()) {
				if (pn instanceof DarknetPeerNode) {
					if (!(peerNode instanceof DarknetPeerNode)) {
						...
						// Opennet peers with the same IP will NOT cause darknet peers to
						// be dropped, even if one connection per IP is set for darknet,
						// and even if it isn't set for opennet.
					...
					System.out
							.println("Disconnecting permanently from your friend \"" + ((DarknetPeerNode) pn).getName()
	...
	/**
	 * Get the cipher for connection attempts for e.g. seednode connections from nodes we
	 * don't know.
	 */
