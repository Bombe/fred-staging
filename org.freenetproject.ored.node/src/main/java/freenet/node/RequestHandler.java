...
public class RequestHandler implements PrioRunnable, ByteCounter, RequestSenderListener {
	 ...
	 * @param key
	 * @param tag
	 * @param passedInKeyBlock We ALWAYS look up in the datastore before starting a
	 * request. SECURITY: Do not pass messages into handler constructors. See note at top
	 * of NodeDispatcher.
	 * @param realTimeFlag
	 * @param needsPubKey
	 */
	...
	public void run() {
		...
		try {
			realRun();
			// The last thing that realRun() does is register as a request-sender
	...
	private void realRun() throws NotConnectedException {
		...
		if (o == null) { // ran out of htl?
			...
			synchronized (this) {
				...
				// fatally timed out (and we need not return packets which will not be
				// claimed)
	...
	public void onReceivedRejectOverload() {
		try {
			if (!sentRejectedOverload) {
				...
				// Note: This message is only discernible from the terminal messages by
				...
				// If the status changes (e.g. to SUCCESS), there is little need to send
	...
	public void onCHKTransferBegins() {
		...
		try {
			...
			bt = new BlockTransmitter(node.usm, node.getTicker(), source, uid, prb, this, new ReceiverAbortHandler() {
				...
				public boolean onAbort() {
					...
					if (rs != null && rs.uid != RequestHandler.this.uid) {
						if (logMINOR)
							Logger.minor(this,
									"Not cancelling transfer because was coalesced on " + RequestHandler.this);
						// No need to reassign tag since this UID will end immediately;
						// the RequestSender is on a different one.
						return false;
					}
					...
					if (rs != null && rs.isTransferCoalesced()) {
						if (logMINOR)
							Logger.minor(this,
									"Not cancelling transfer because others want the data on " + RequestHandler.this);
					...
					if (node.failureTable.peersWantKey(key, source)) {
						// This may indicate downstream is having trouble communicating
						// with us.
						Logger.error(this,
								"Downstream transfer successful but upstream transfer to " + source.shortToString()
					...
					if (node.clientCore != null && node.clientCore.wantKey(key)) {
						/**
						 * REDFLAG SECURITY JUSTIFICATION: Theoretically if A routes to us
						 * and then we route to B, and Mallory controls both A and B, and
						 * A cancels the transfer, and we don't cancel the transfer from
						 * B, then Mallory knows we want the key. However, to exploit this
						 * he would have to rule out other nodes having asked for the key
						 * i.e. he would have to surround the node, or he would have to
						 * rely on probabilistic attacks (which will give us away much
						 * more quickly).
						 *
						 * Plus, it is (almost?) always going to be safer to keep
						 * transferring data than to start a new request which potentially
						 * exposes us to distant attackers.
						 *
	...
	protected void transferFinished(boolean success) {
		...
		if (success) {
			status = rs.getStatus();
			// Run off-thread because, on the onRequestSenderFinished path, RequestSender
			// won't start to wait for the noderef until we return!
	...
	public void onRequestSenderFinished(int status, boolean fromOfferedKey, RequestSender rs) {
		...
		if (tooLate) {
			...
			// We need to send the RejectedOverload (or whatever) anyway, for two-stage
			// timeout.
	 ...
	 * started the transfer, complete if we have already completed the transfer, or set a
	 * flag so that we will complete when we do.
	...
	private void maybeCompleteTransfer() throws NotConnectedException {
		...
		synchronized (this) {
			if (disconnected)
				...
				// Obviously this node is confused, send a terminal reject to make sure
				// the requestor is not waiting forever.
	...
	private void sendSSK(byte[] headers, final byte[] data, boolean needsPubKey2, DSAPublicKey pubKey)
		...
		// The pubKey will have been set on the SSK key, and the SSKBlock will have been
		// constructed.
		MultiMessageCallback mcb = null;
		mcb = new MultiMessageCallback() {
			@Override
			public void finish(boolean success) {
				sentPayload(data.length); // FIXME report this at the time when that
											// message is acked for more accurate
											// reporting???
				applyByteCounts();
				// Will call unlockHandler.
				// This is okay, it can be called twice safely, and it ensures that even
				// if sent() is not called it will still be unlocked.
				unregisterRequestHandlerWithNode();
			}
			...
			void sent(boolean success) {
				...
				// 1. Unlocking immediately avoids problems with the recipient reusing the
				// slot when he's received the data, therefore us rejecting the request
				// and getting a mandatory backoff, and
				// 2. However, we do want SSK requests from the datastore to be counted
				// towards the total when accepting requests.
				// This is safe however.
				// We have already done the request so there is no outgoing request.
				// A node might be able to get a few more slots in flight by not acking
				// the SSK messages, but it would quickly stall.
				// Furthermore, we would start sending hard rejections when the send queue
				// gets past a certain point.
				// So it is neither beneficial for the node nor a viable DoS.
				// Alternative solution would be to wait until the pubkey and data have
				// been acked before unlocking and sending the headers, but this appears
				// not to be necessary.
				tag.unlockHandler();
			}
		};
	...
	static void sendSSK(byte[] headers, byte[] data, boolean needsPubKey, DSAPublicKey pubKey, final PeerNode source,
			long uid, ByteCounter ctr, boolean realTimeFlag) throws NotConnectedException, WaitedTooLongException,
			PeerRestartedException, SyncSendWaitedTooLongException {
		// SUCCESS requires that BOTH the pubkey AND the data/headers have been received.
		// The pubKey will have been set on the SSK key, and the SSKBlock will have been
		// constructed.
	...
	private void returnLocalData(KeyBlock block) throws NotConnectedException {
		if (key instanceof NodeSSK) {
			...
			PartiallyReceivedBlock prb = new PartiallyReceivedBlock(Node.PACKETS_IN_BLOCK, Node.PACKET_SIZE,
					block.getRawData());
			BlockTransmitter bt = new BlockTransmitter(node.usm, node.getTicker(), source, uid, prb, this,
					BlockTransmitter.NEVER_CASCADE, new BlockTransmitterCompletion() {
						...
						public void blockTransferFinished(boolean success) {
							if (success) {
								...
								// We've fetched it from our datastore, so there won't be
								// a downstream noderef.
	...
	/**
	 * Sends the 'final' packet of a request in such a way that the thread can be freed
	 * (made non-runnable/exit) and the byte counter will still be accurate.
	 */
	private void sendTerminal(Message msg) {
		...
		// receives it, but we won't, so we may reject his requests and get a mandatory
		// backoff.
		tag.unlockHandler();
		try {
	...
	private class TerminalMessageByteCountCollector implements AsyncMessageCallback {
		...
		private void complete() {
			...
			// For byte counting, this relies on the fact that the callback will only be
			// excuted once.
			applyByteCounts();
			unregisterRequestHandlerWithNode();
		}
	...
	/**
	 * Either send an ack, indicating we've finished and aren't interested in opennet, or
	 * wait for a noderef and relay it and wait for a response and relay that, or send our
	 * own noderef and wait for a response and add that.
	 *
	 * One way or another this method must call applyByteCounts;
	 * unregisterRequestHandlerWithNode. This happens asynchronously via ackOpennet() if
	 * we are unable to send a noderef. It happens explicitly otherwise.
	 */
	...
	/**
	 * There is no noderef to pass downstream. If we want a connection, send our noderef
	 * and wait for a reply, otherwise just send an ack.
	 */
	...
	/**
	 * @param om Completion: Will either call ackOpennet(), sending an ack downstream and
	 * then unlocking after this has been sent (asynchronously), or will unlock itself if
	 * we sent a noderef (after we have handled the incoming noderef / ack / timeout).
	 */
	private void finishOpennetInner(OpennetManager om) {
		...
		if (noderef != null && node.random.nextInt(OpennetManager.RESET_PATH_FOLDING_PROB) == 0) {
			...
			if (ref == null || om.alreadyHaveOpennetNode(ref)) {
				...
				// Reset path folding.
				// We need to tell the source of the noderef that we are not going to use
				// it.
	...
	/**
	 * Send our noderef to the request source, wait for a reply, if we get one add it.
	 * Called when either the request wasn't routed, or the node it was routed to didn't
	 * return a noderef.
	 *
	 * Completion: Will ack downstream if necessary (if we didn't send a noderef), and
	 * will in any case call applyByteCounts(); unregisterRequestHandlerWithNode()
	 * asynchronously, either after receiving the noderef, or after sending the ack.
	 *
	 * In all cases we do not interact with dataSource. The caller must have already sent
	 * an ack to dataSource if necessary (but in most cases dataSource has timed out or
	...
	private void finishOpennetNoRelayInner(final OpennetManager om) {
		...
		OpennetManager.waitForOpennetNoderef(true, source, uid, this, new OpennetManager.NoderefCallback() {

			// We have already sent ours, so we don't need to worry about timeouts.

			@Override
			public void gotNoderef(byte[] noderef) {
				// We have sent a noderef. It is not appropriate for the caller to call
				// ackOpennet():
	...
	/**
	 * Called when the node we routed the request to returned a valid noderef, and we
	 * don't want it. So we relay it downstream to somebody who does, and wait to relay
	 * the response back upstream.
	 *
	 * Completion: Will call applyByteCounts(); unregisterRequestHandlerWithNode()
	...
	private void finishOpennetRelay(byte[] noderef, final OpennetManager om) {
		...
		// We do not need to worry about timeouts here, because we have already sent our
		// noderef.
							...
							om.sendOpennetRef(true, uid, dataSource, newNoderef, RequestHandler.this,
									new AllSentCallback() {
										...
										public void allSent(BulkTransmitter bulkTransmitter, boolean anyFailed) {
											// As soon as the originator receives the
											// three blocks, he can reuse the slot.
											tag.finishedWaitingForOpennet(dataSource);
											tag.unlockHandler();
											applyByteCounts();
											// Note that sendOpennetRef() does not wait
											// for an acknowledgement or even for the
											// blocks to have been sent!
											// So this will be called well after
											// gotNoderef() exits.
										}
	...
	public void sentPayload(int x) {
		/*
		 * Do not add payload to sentBytes. sentBytes() is called with the actual sent
		 * bytes, and we do not deduct the alreadyReportedBytes, which are only used for
		 * accounting for the bandwidth throttle.
		 */
		node.sentPayload(x);
		node.nodeStats.requestSentBytes(key instanceof NodeSSK, -x);
		if (logMINOR)
