 ...
 * <li>Success rates for remote requests by HTL; perhaps over some larger amount of time
 * than the past hour.</li>
 * </ul>
 *
...
public class Probe implements ByteCounter {
	...
	/**
	 * To make the timing less obvious when a node responds with a local result instead of
	 * forwarding at HTL = 1, delay for a number of milliseconds, specifically an
	 * exponential distribution with this constant as its mean.
	 */
	...
	/**
	 * Maximum number of probes started locally in the past minute. This is the maximum
	 * conceivable value; the probes should be used with a number of requests per minute
	 * closer to the per-peer limit times the minimum expected number of peers. Around
	 * this value, and certainly above it, remote OVERLOADs may start coming in, which are
	 * not useful. The Metropolis-Hastings correction makes behavior potentially
	 * inconsistent, so keeping an eye on remote OVERLOADs is wise.
	 */
	...
	public Probe(final Node node) {
		...
		nodeConfig.register("probeBuild", true, sortOrder++, true, true, "Node.probeBuildShort", "Node.probeBuildLong",
				new BooleanCallback() {
					@Override
					public Boolean get() {
						return respondBuild;
		...
		nodeConfig.register("probeIdentifier", true, sortOrder++, true, true, "Node.probeRespondIdentifierShort",
				"Node.probeRespondIdentifierLong", new BooleanCallback() {
					@Override
					public Boolean get() {
		...
		nodeConfig.register("probeOverallBulkOutputCapacityUsage", true, sortOrder++, true, true,
				"Node.respondOverallBulkOutputCapacityUsage", "Node.respondOverallBulkOutputCapacityUsageLong",
				new BooleanCallback() {
		...
		nodeConfig.register("identifier", -1, sortOrder++, true, true, "Node.probeIdentifierShort",
				"Node.probeIdentifierLong", new LongCallback() {
					...
					public void set(Long val) {
						probeIdentifier = val;
						// -1 is reserved for picking a random value; don't pick it
						// randomly.
		...
		/*
		 * set() is not used when setting up an option with its default value, so do so
		 * manually to avoid using an identifier of -1.
		 */
		try {
	...
	/**
	 * Processes an incoming probe request; relays results back to source. If the probe
	 * has a positive HTL, routes with MH correction and probabilistically decrements HTL.
	 * If the probe comes to have an HTL of zero: (an incoming HTL of less than one is
	 * discarded.) Returns (as node settings allow) exactly one of:
	 * <ul>
	 ...
	 * @param source node from which the probe request was received. If null, it is
	 * considered to have been sent by the local node.
	...
	private void request(final Message message, final PeerNode source, final Listener listener) {
		...
		if (htl < 1) {
			...
			if (logMINOR) {
				Logger.minor(Probe.class, "Received out-of-bounds HTL of " + htl + " from " + source.getIdentityString()
						+ " (" + source.userToString() + "); interpreting as " + MAX_HTL + ".");
			}
			htl = MAX_HTL;
		}
		boolean availableSlot = true;
		TimerTask task = null;
		// Allocate one of this peer's probe request slots for 60 seconds; send an
		// overload if none are available.
				...
				task = new TimerTask() {
					...
					public void run() {
						synchronized (accepted) {
							counter.decrement();
							/*
							 * Once the counter hits zero, there's no reason to keep it
							 * around as it can just be recreated when this peer sends
							 * another probe request without changing behavior. To do
							 * otherwise would accumulate counters at zero over time.
							 */
							if (counter.value() == 0) {
		...
		/*
		 * Route to a peer, using Metropolis-Hastings correction and ignoring backoff to
		 * get a more uniform endpoint distribution. HTL is decremented before routing so
		 * that it's possible to respond locally without attempting to route first. Send a
		 * local response if HTL is zero now or becomes zero while trying to route. During
		 * routing HTL decrements if a candidate is rejected by the Metropolis-Hastings
		 * correction.
		 */
	...
	/**
	 * Attempts to route the message to a peer. If the maximum number of send attempts is
	 * exceeded, fails with the error CANNOT_FORWARD.
	 * @return True if no further action needed; false if HTL decremented to zero and a
	 * local response is needed.
	 */
	private boolean route(final Type type, final long uid, byte htl, final Listener listener) {
		// Recreate the request so that any sub-messages or unintended fields are not
		// forwarded.
		...
		for (int sendAttempts = 0; sendAttempts < MAX_SEND_ATTEMPTS; sendAttempts++) {
			...
			if (candidate.isConnected()) {
				...
				/*
				 * Candidate's degree is unknown; fall back to random walk by accepting
				 * this candidate regardless of its degree.
				 */
				...
				if (node.random.nextFloat() < acceptProbability) {
					...
					try {
						...
						// Peer no longer connected - sending was not successful. Try
						// again.
					}
					...
					/*
					 * Metropolis-Hastings correction rejected - decrement HTL so that it
					 * can run out depending on relative degrees.
					 */
					htl = probabilisticDecrement(htl);
	 ...
	 */
	private static MessageFilter createResponseFilter(final Type type, final PeerNode candidate, final long uid,
			final byte htl) {
	...
	/**
	 * Depending on node settings, sends a message to source containing either a refusal
	 * or the requested result.
	 */
	private void respond(final Type type, final Listener listener) {
		...
		/*
		 * This adds noise to the results to make information less identifiable. The goal
		 * is making it difficult to determine which value a node actually has; that any
		 * given value could mean a small range of common values. Different result types
		 * have different sigma values such that one sigma contains multiple reasonable
		 * values.
		 */
		switch (type) {
		case BANDWIDTH:
			/*
			 * 5% noise: Reasonable output bandwidth limit is 20 KiB and people are likely
			 * to set limits in increments of 1 KiB. 1 KiB / 20 KiB = 0.05 sigma. 1,024
			 * (2^10) bytes per KiB.
			 */
		...
		case IDENTIFIER:
			/*
			 * 5% noise: Reasonable uptime percentage is at least ~40 hours a week, or
			 * ~20%. This uptime is quantized so only something above a full percentage
			 * point (0.01 * 168 hours = 1.68 hours) of change will be guaranteed (from a
			 * percentage with a decimal component close to zero) to be reflected. 1% /
			 * 20% = 0.05 sigma.
			 *
			 * 7-day uptime with random noise, then quantized. Quantization is to make it
			 * very, very difficult to get useful information out of any given result
			 * because it is included with an identifier,
			 */
		...
		case LINK_LENGTHS:
			...
			int i = 0;
			/*
			 * 1% noise: Link lengths are in the range [0.0, 0.5], and any change is
			 * enough to make the match not exact between locations. Taking as an example
			 * a link length of 0.2. and with the assumption that a change of 0.002 is
			 * enough to make it still useful for statistics but not useful for
			 * identification, 0.002 change / 0.2 link length = 0.01 sigma.
			 */
		...
		case STORE_SIZE:
			/*
			 * 5% noise: Reasonable datastore size is 20 GiB, and size is likely set in,
			 * at most, increments of 1 GiB. 1 GiB / 20 GiB = 0.05 sigma. 1,073,741,824
			 * bytes (2^30) per GiB.
			 */
		...
		case UPTIME_48H:
			/*
			 * 8% noise: Continuing with the assumption that reasonable weekly uptime is
			 * around 40 hours, this allows for 6 hours per day, 12 hours per 48 hours, or
			 * 25%. A half-hour seems a sufficient amount of ambiguity, so 0.5 hours / 48
			 * hours ~= 1%, and 1% / 25% = 0.04 sigma.
			 */
			listener.onUptime((float) randomNoise(100 * node.uptime.getUptime(), 0.04));
			break;
		case UPTIME_7D:
			/*
			 * 2.4% noise: As a 168-hour uptime covers a longer period 1 hour of ambiguity
			 * seems sufficient. 1 hour / 168 hours ~= 0.6%, and 0.6% / 20% = 0.03 sigma.
			 */
	...
	/**
	 * Decrements 20% of the time at HTL 1; otherwise always. This is to protect the
	 * responding node, whereas the anonymity of the node which initiated the request is
	 * not a concern.
	...
	/**
	 * Filter listener which determines the type of result and calls the appropriate probe
	 * listener method.
	 */
	private class ResultListener implements AsyncMessageFilterCallback {
		...
		/**
		 * @param listener to call appropriate methods for events such as matched messages
		 * or timeout.
		 */
		...
		/**
		 * Parses provided message and calls appropriate Probe.Listener method for the
		 * type of result.
	...
	/**
	 * Listener which relays responses to the node specified during construction. Used for
	 * received probe requests. This leads to reconstructing the messages, but removes
	 * potentially harmful sub-messages and also removes the need for duplicate message
	 * sending code elsewhere, If the result includes a trace this would be the place to
	 * add local results to it.
	 */
	private class ResultRelay implements Listener {
		...
		/**
		 * @param source peer from which the request was received and to which send the
		 * response.
