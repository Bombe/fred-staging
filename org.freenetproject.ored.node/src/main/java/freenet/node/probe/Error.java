...
public enum Error {
	...
	TIMEOUT((byte) 2),
	/**
	 * An error occurred, but a node did not recognize the error used. If this occurs
	 * locally, it will be specified along with the unrecognized code.
	 */
	UNKNOWN((byte) 3),
	/**
	 * A remote node did not recognize the requested probe type. For locally started
	 * probes it will not be a ProbeError but a ProtocolError.
	 */
	UNRECOGNIZED_TYPE((byte) 4),
	/**
	 * A node received and understood the request, but failed to forward it to another
	 * node.
	...
	/**
	 * Stable numerical value to represent the enum value. Used to send over the network
	 * instead of .name(). Ordinals are not acceptable because they rely on the number and
	 * ordering of enums.
	 */
	...
	/**
	 * Checks whether valueOf() will throw for the given code. Intended to make things
	 * more concise and faster than try-catch blocks.
