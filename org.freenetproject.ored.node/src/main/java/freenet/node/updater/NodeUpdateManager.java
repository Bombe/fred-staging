...
public class NodeUpdateManager {
	...
	public NodeUpdateManager(Node node, Config config) throws InvalidConfigValueException {
		...
		transitionMainJarFetcher = new LegacyJarFetcher(previousMainJarSSK, transitionMainJarFile, node.clientCore,
				legacyFetcherCallback);

		updaterConfig.register("updateSeednodes", wasEnabledOnStartup, 6, true, true,
		...
		updaterConfig.register("updateInstallers", wasEnabledOnStartup, 6, true, true,
			...
			try {
				...
				for (int i = 0; i < 10; i++) {
					...
					if (FileUtil.renameTo(temp, directory.file(filename))) {
						System.out
								.println("Successfully fetched " + filename + " for version " + Version.buildNumber());
						break;
					}
					else {
						System.out.println(
								"Failed to rename " + temp + " to " + filename + " after fetching it from Freenet.");
						try {
							Thread.sleep(SECONDS.toMillis(1) + node.fastWeakRandom.nextInt(
									(int) SECONDS.toMillis((long) Math.min(Math.pow(2, i), MINUTES.toSeconds(15)))));
						}
				...
				System.err.println("Fetched but failed to write out " + filename
						+ " - please check that the node has permissions to write in " + directory.dir()
	...
	public void start() throws InvalidConfigValueException {
		...
		if (updateInstallers) {
			SimplePuller installerGetter = new SimplePuller(getInstallerNonWindowsURI(), NodeFile.InstallerNonWindows);
			SimplePuller wininstallerGetter = new SimplePuller(getInstallerWindowsURI(), NodeFile.InstallerWindows);
	...
	void broadcastUOMAnnouncesOld() {
		boolean mainJarAvailable = transitionMainJarFetcher == null ? false : transitionMainJarFetcher.fetched();
		Message msg;
		if (!mainJarAvailable)
			return;
	...
	private Message getOldUOMAnnouncement() {
		boolean mainJarAvailable = transitionMainJarFetcher == null ? false : transitionMainJarFetcher.fetched();
		return DMT.createUOMAnnouncement(previousMainJarUSK.toString(), revocationURI.toString(),
				revocationChecker.hasBlown(), mainJarAvailable ? TRANSITION_VERSION : -1,
				revocationChecker.lastSucceededDelta(), revocationChecker.getRevocationDNFCounter(),
				revocationChecker.getBlobSize(), mainJarAvailable ? transitionMainJarFetcher.getBlobSize() : -1,
	...
	private Message getNewUOMAnnouncement(long blobSize) {
		...
		if (blobSize <= 0)
			fetchedVersion = -1;
		return DMT.createUOMAnnouncement(updateURI.toString(), revocationURI.toString(), revocationChecker.hasBlown(),
				fetchedVersion, revocationChecker.lastSucceededDelta(), revocationChecker.getRevocationDNFCounter(),
				revocationChecker.getBlobSize(), blobSize, (int) node.nodeStats.getNodeAveragePingTime(),
	 ...
	 * @throws InvalidConfigValueException If enable=true and we are not running under the
	 * wrapper.
	 */
	void enable(boolean enable) throws InvalidConfigValueException {
		...
		synchronized (this) {
			...
			if (!enable) {
				...
				// }
				// Start it
				mainUpdater = new MainJarUpdater(this, updateURI, Version.buildNumber(), -1, Integer.MAX_VALUE,
						"main-jar-");
	...
	void startPluginUpdater(OfficialPluginDescription plugin) {
		...
		PluginJarUpdater updater = new PluginJarUpdater(this, uri, (int) minVer, -1,
				(plugin.essential ? (int) minVer : Integer.MAX_VALUE), name + "-", name, node.pluginManager,
	...
	public synchronized void addChangelogLinks(long version, HTMLNode node) {
		...
		String developerDetailsUri = getDeveloperChangelogURI().setSuggestedEdition(version).sskForUSK()
				.toASCIIString();
	...
	private boolean isReadyToDeployUpdate(boolean ignoreRevocation) {
		...
		synchronized (this) {
			...
			if (startedFetchingNextMainJar > 0) {
				waitForNextJar = (int) (startedFetchingNextMainJar + WAIT_FOR_SECOND_FETCH_TO_COMPLETE - now);
				if (waitForNextJar > 0) {
					if (logMINOR)
			...
			if (this.fetchedMainJarVersion != this.dependenciesValidForBuild) {
				if (logMINOR)
					Logger.minor(this, "Not deploying because dependencies are older version "
							+ dependenciesValidForBuild + " - new version " + fetchedMainJarVersion + " may not start");
				return false;
			}
	...
	private boolean writeJars(UpdateDeployContext ctx, MainJarDependenciesChecker.MainJarDependencies deps) {
		...
		try {
			...
			failUpdate(e.getMessage());
			node.clientCore.alerts.register(new SimpleUserAlert(false, l10n("updateCatastropheTitle"), e.getMessage(),
					l10n("updateCatastropheTitle"), FCPUserAlert.CRITICAL_ERROR));
			return false;
		}
		catch (UpdaterParserException e) {
			node.clientCore.alerts.register(new SimpleUserAlert(false, l10n("updateFailedTitle"), e.getMessage(),
					l10n("updateFailedShort", "reason", e.getMessage()), FCPUserAlert.CRITICAL_ERROR));
			return false;
		}
	...
	private void failUpdate(String reason) {
		...
		node.clientCore.alerts
				.register(new SimpleUserAlert(true, l10n("updateFailedTitle"), l10n("updateFailed", "reason", reason),
						l10n("updateFailedShort", "reason", reason), FCPUserAlert.CRITICAL_ERROR));
	}
	...
	public void blow(String msg, boolean disabledNotBlown) {
		...
		synchronized (this) {
			if (hasBeenBlown) {
				...
				Logger.error(this, "The key has ALREADY been marked as blown! Message was " + revocationMessage
						+ " new message " + msg);
				return;
			}
			else {
