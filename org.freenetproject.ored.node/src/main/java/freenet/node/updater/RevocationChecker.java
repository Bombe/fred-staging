...
public class RevocationChecker implements ClientGetCallback, RequestClient {
	...
	public RevocationChecker(NodeUpdateManager manager, File blobFile) {
		...
		ctxRevocation.maxSplitfileBlockRetries = -1; // if we find content, try forever to
														// get it; not used because of the
														// above size limits.
	...
	public void start(boolean aggressive) {
		...
		if (blobFile.exists()) {
			...
			try {
				...
				System.err.println(
						"We may have downloaded an old revocation blob before restarting but it cannot be read: " + e);
				e.printStackTrace();
			}
		}
	}
	...
	public boolean start(boolean aggressive, boolean reset) {
		...
		try {
			...
			synchronized (this) {
				...
				if (revocationGetter != null && !(revocationGetter.isCancelled() || revocationGetter.isFinished())) {
					if (logMINOR)
					...
					if (reset) {
						if (logMINOR)
							Logger.minor(this, "Resetting DNF count from " + revocationDNFCounter,
									new Exception("debug"));
					...
					if (revocationGetter != null && logMINOR)
						Logger.minor(this, "revocation fetcher: cancelled=" + revocationGetter.isCancelled()
					...
					cg = revocationGetter = new ClientGetter(this, manager.getRevocationURI(), ctxRevocation,
	...
	void onFailure(FetchException e, ClientGetter state, Bucket blob) {
		...
		if (e.isFatal()) {
			...
			String message = l10n("revocationFetchFailedFatally", new String[] { "detail", "key" },
					new String[] { e.toUserFriendlyString(), manager.getRevocationURI().toASCIIString() });
