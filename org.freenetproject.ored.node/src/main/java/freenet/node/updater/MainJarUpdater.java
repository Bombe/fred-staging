...
public class MainJarUpdater extends NodeUpdater implements MainJarDependenciesChecker.Deployer {
	...
	protected void parseDependencies(Properties props, int build) {
		...
		MainJarDependenciesChecker.MainJarDependencies deps = dependencies.handle(props, build);
		if (deps != null) {
	...
	private class DependencyJarFetcher
		...
		DependencyJarFetcher(File filename, FreenetURI chk, long expectedLength, byte[] expectedHash,
			...
			getter = new ClientGetter(this, chk, myCtx, PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS,
		...
		public void onSuccess(FetchResult result, ClientGetter state) {
			...
			if (!MainJarDependenciesChecker.validFile(tempFile, expectedHash, expectedLength, executable)) {
				...
				System.err.println(
						"Download of " + filename + " for update failed because temp file appears to be corrupted!");
				if (cb != null)
					cb.onFailure(new FetchException(FetchExceptionMode.BUCKET_ERROR,
							"Downloaded jar from Freenet but failed consistency check: " + tempFile + " length "
									+ tempFile.length() + " "));
				tempFile.delete();
				return;
			}
			if (!FileUtil.renameTo(tempFile, filename)) {
				...
				System.err.println(
						"Download of " + filename + " for update failed because cannot rename from " + tempFile);
				if (cb != null)
					cb.onFailure(new FetchException(FetchExceptionMode.BUCKET_ERROR,
		...
		public synchronized HTMLNode renderRow() {
			...
			if (uomFetcher != null)
				...
				row.addChild(QueueToadlet.createProgressCell(false, true, COMPRESS_STATE.WORKING,
						lastProgress.succeedBlocks, lastProgress.failedBlocks, lastProgress.fatallyFailedBlocks,
						lastProgress.minSuccessfulBlocks, lastProgress.totalBlocks, lastProgress.finalizedTotal,
						false));
			return row;
		}
	...
	public void multiFileReplaceReadyToDeploy(final MainJarDependenciesChecker.AtomicDeployer atomicDeployer) {
		if (this.manager.isAutoUpdateAllowed()) {
			...
			node.clientCore.alerts.register(new FCPUserAlert() {
