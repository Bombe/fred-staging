...
public abstract class NodeUpdater implements ClientGetCallback, USKCallback, RequestClient {
	...
	public void onFoundEdition(long l, USK key, ClientContext context, boolean wasMetadata, short codec, byte[] data,
		...
		synchronized (this) {
			...
			Logger.minor(this,
					"Updating availableVersion from " + availableVersion + " to " + found + " and queueing an update");
	...
	public void maybeUpdate() {
		...
		synchronized (this) {
			...
			if (availableVersion > currentVersion) {
				...
				System.err.println(
						"Starting the update process: found the update (" + availableVersion + "), now fetching it.");
			}
			if (logMINOR)
			...
			try {
				if ((cg == null) || cg.isCancelled()) {
					...
					tempBlobFile = File.createTempFile(blobFilenamePrefix + availableVersion + "-", ".fblob.tmp",
							manager.node.clientCore.getPersistentTempDir());
					FreenetURI uri = URI.setSuggestedEdition(availableVersion);
					uri = uri.sskForUSK();
					cg = new ClientGetter(this, uri, ctx, PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS, null,
							new BinaryBlobWriter(new FileBucket(tempBlobFile, false, false, false, false)), null);
					toStart = cg;
				}
				else {
