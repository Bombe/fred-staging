...
/**
 * Fetches the old freenet-ext.jar and freenet-stable-latest.jar. In other words it
 * fetches the transitional versions.
 *
 * @author toad
 */
	...
	public LegacyJarFetcher(FreenetURI uri, File saveTo, NodeClientCore core, LegacyFetchCallback cb) {
		...
		if (blobBucket.size() > 0) {
			...
			try {
				...
				tmp.deleteOnExit(); // To be used sparingly, as it leaks, but safe enough
									// here as it should only happen twice during a normal
									// run.
			}
			catch (IOException e) {
