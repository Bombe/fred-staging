...
public class UpdateDeployContext {
	...
	void rewriteWrapperConf(boolean writtenNewJar)
		...
		// Don't just write it out from properties; we want to keep it as close to what it
		// was as possible.
		...
		// Therefore, on Windows, if we update freenet-ext.jar, it will use freenet.jar
		// and freenet-ext.jar
		// and freenet-ext.jar.new as well. The old freenet-ext.jar will take precedence,
		// and we won't be
		// able to overwrite either of them, so we'll just restart every 5 minutes
		// forever!
		...
		// allow accessing internal modules (required for Java 16, only supported since
		// Java 9)
		...
		if (!writtenPrivateModulesOpens && JVMVersion.supportsModules()) {
			// WoT: Unable to make field private final java.lang.String
			// java.lang.Enum.name accessible
			bw.write("wrapper.java.additional." + count + "=--add-opens=java.base/java.lang=ALL-UNNAMED" + '\n');
			count++;
			// Unable to make public int
			// java.util.Collections$UnmodifiableCollection.size() accessible
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.startsWith("wrapper.java.maxmemory=")) {
					try {
						...
						// There have been some cases where really high limits have caused
						// the JVM to do bad things.
