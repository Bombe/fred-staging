...
public class NodeARKInserter implements ClientPutCallback, RequestClient {
	...
	private void innerUpdate() {
		...
		if (!checkIPUpdated())
			return;
		// We'll broadcast the new physical.udp entry to our connected peers via a
		// differential node reference
		// We'll err on the side of caution and not update our peer to an empty
		// physical.udp entry using a differential node reference
	...
	private void startInserter() {
		...
		// Delete entire ark.* field for now. Changing this and automatically moving to
		// the new may be supported in future.
		fs.removeSubset("ark");
		fs.removeValue("location");
		fs.removeValue("sig");
		// fs.remove("version"); - keep version because of its significance in
		// reconnection
		...
		inserter = new ClientPutter(this, b, uri, null, // Modern ARKs easily fit inside
														// 1KB so should be pure SSKs =>
														// no MIME type; this improves
														// fetchability considerably
				ctx, PriorityClasses.INTERACTIVE_PRIORITY_CLASS, false, null, false, node.clientCore.clientContext,
				null, -1);
	...
	public void onGeneratedURI(FreenetURI uri, BaseClientPutter state) {
		...
		if (l < crypto.myARKNumber) {
			...
			if (crypto.isOpennet)
				...
				node.writeNodeFile();
			// We'll broadcast the new ARK edition to our connected peers via a
			// differential node reference
			SimpleFieldSet fs = new SimpleFieldSet(true);
