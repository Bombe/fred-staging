...
public final class CHKInsertSender extends BaseSender implements PrioRunnable, AnyInsertSender, ByteCounter {
	...
	private class BackgroundTransfer implements PrioRunnable, SlowAsyncMessageFilterCallback {
		...
		/**
		 * Have we received notice of the downstream success or failure of dependant
		 * transfers from that node? Includes timing out.
		 */
		...
		/**
		 * Do we have the InsertReply, RNF or similar completion? If not, there is no
		 * point starting to wait for a timeout.
		 */
		boolean gotInsertReply;

		/**
		 * Have we started the first wait? We start waiting when we have completed the
		 * transfer AND received an InsertReply, RNF or similar.
		 */
		private boolean startedWait;

		/**
		 * Has the background transfer been terminated due to not receiving an
		 * InsertReply, or due to disconnection etc?
		 */
		...
		BackgroundTransfer(final PeerNode pn, PartiallyReceivedBlock prb, InsertTag thisTag) {
			...
			bt = new BlockTransmitter(node.usm, node.getTicker(), pn, uid, prb, CHKInsertSender.this,
					BlockTransmitter.NEVER_CASCADE, new BlockTransmitterCompletion() {
						...
						public void blockTransferFinished(boolean success) {
							...
							// Double-check that the node is still connected. Pointless to
							// wait otherwise.
		...
		/**
		 * Start waiting for an acknowledgement or timeout. Caller must ensure that the
		 * transfer has succeeded and we have received an RNF, InsertReply or other valid
		 * completion. The timeout is relative to that, since up to that point we could
		 * still be routing.
		 */
		private void startWait() {
			...
			// Add ourselves as a listener for the longterm completion message of this
			// transfer, then gracefully exit.
			try {
		...
		private void realRun() {
			...
			// No confirmation that it has finished, and it won't finish immediately on
			// the transfer finishing.
			// So don't try to thisTag.removeRoutingTo(next), just assume it keeps running
			// until the whole insert finishes.
		}
		 ...
		 * @return True if we should wait again, false if we have already received a
		 * notice or timed out.
		 */
		private boolean receivedNotice(boolean success, boolean timeout, boolean kill) {
			...
			synchronized (backgroundTransfers) {
				if (finishedWaiting) {
					if (!(killed || kill))
						Logger.error(this, "Finished waiting already yet receivedNotice(" + success + "," + timeout
								+ "," + kill + ")", new Exception("error"));
					return false;
				}
				if (killed) {
					...
					if (receivedCompletionNotice) {
						// Two stage timeout.
						if (logMINOR)
							Logger.minor(this, "receivedNotice(" + success + "), already had receivedNotice("
									+ completionSucceeded + ")");
						if (timeout) {
							// Fatal timeout.
							finishedWaiting = true;
						...
						receivedCompletionNotice = true;
						if (!timeout) // Any completion mode other than a timeout
										// immediately sets finishedWaiting, because we
										// won't wait any longer.
							finishedWaiting = true;
						else {
							// First timeout but not had second timeout yet.
							// Unlock downstream (below), but will wait here for the peer
							// to fatally timeout.
							// UIDTag will automatically reassign to self when the time
							// comes if we call handlingTimeout() here, and will avoid
							// unnecessarily logging errors.
							// LOCKING: Note that it is safe to call the tag within the
							// lock since we always take the UIDTag lock last.
			...
			if (!noUnlockPeer)
				// Downstream (away from originator), we need to stay locked on the peer
				// until the fatal timeout / the delayed notice.
				// Upstream (towards originator), of course, we can unlockHandler() as
				// soon as all the transfers are finished.
				// LOCKING: Do this outside the lock as pn can do heavy stuff in response
				// (new load management).
		...
		public void onMatched(Message m) {
			...
			// pn cannot be null, because the filters will prevent garbage collection of
			// the nodes
		...
		public boolean shouldTimeout() {
			// AFIACS, this will still let the filter timeout, but not call onMatched()
			// twice.
			return finishedWaiting;
		}

		private MessageFilter getNotificationMessageFilter(boolean longTimeoutAnyway) {
			return MessageFilter.create().setField(DMT.UID, uid).setType(DMT.FNPInsertTransfersCompleted).setSource(pn)
					.setTimeout(longTimeoutAnyway ? TRANSFER_COMPLETION_ACK_TIMEOUT_BULK : transferCompletionTimeout);
		}

		@Override
		public void onTimeout() {
			/*
			 * FIXME: Cascading timeout... if this times out, we don't have any time to
			 * report to the node of origin the timeout notification (anyTimedOut?).
			 */
			// NORMAL priority because it is normally caused by a transfer taking too long
			// downstream, and that doesn't usually indicate a bug.
		...
		/**
		 * Called when we have received an InsertReply, RouteNotFound or other successful
		 * or quasi-successful completion to routing.
		 */
	...
	CHKInsertSender(NodeCHK myKey, long uid, InsertTag tag, byte[] headers, short htl, PeerNode source, Node node,
			PartiallyReceivedBlock prb, boolean fromStore, boolean canWriteClientCache, boolean forkOnCacheable,
	...
	/**
	 * List of nodes we are waiting for either a transfer completion notice or a transfer
	 * completion from. Also used as a sync object for waiting for transfer completion.
	 */
	...
	protected void routeRequests() {
		...
		while (true) {
			...
			if (node.canWriteDatastoreInsert(htl) && (!canWriteStorePrev) && forkOnCacheable
				...
				// FORK! We are now cacheable, and it is quite possible that we have
				// already gone over the ideal sink nodes,
				// in which case if we don't fork we will miss them, and greatly reduce
				// the insert's reachability.
				// So we fork: Create a new UID so we can go over the previous hops again
				// if they happen to be good places to store the data.

				// Existing transfers will keep their existing UIDs, since they copied the
				// UID in the constructor.
				// Both local and remote inserts can be forked here: If it has reached
				// this HTL, it means it's already been routed to some nodes.
			...
			next = node.peers.closerPeer(forkedRequestTag == null ? source : null, nodesRoutedTo, target, true,
					node.isAdvancedModeEnabled(), -1, null, null, htl, ignoreLowBackoff ? Node.LOW_BACKOFF : 0,
					source == null, realTimeFlag, newLoadManagement);
	...
	private void handleRejectedTimeout(Message msg, PeerNode next) {
		...
		// However it is not fatal because we can be confident now that even if the
		// DataInsert
		// is delivered late, it will not be acted on. I.e. we are certain how many
		// requests
		...
		// We have always started the transfer by the time this is called, so we do NOT
		// need to removeRoutingTo().
	...
	private void handleRNF(Message msg, PeerNode next, InsertTag thisTag) {
		...
		// Finished as far as this node is concerned - except for the data transfer, which
	...
	private void handleDataInsertRejected(Message msg, PeerNode next, InsertTag thisTag) {
		...
		if (reason == DMT.DATA_INSERT_REJECTED_VERIFY_FAILED) {
			if (fromStore) {
				// That's odd...
				Logger.error(this,
						"Verify failed on next node " + next + " for DataInsert but we were sending from the store!");
			}
			else {
				try {
					if (!prb.allReceived())
			...
			if (recvFailed) {
				...
				try {
					if (prb.allReceived()) {
						...
						// Only fatal timeouts warrant ERROR's because they indicate
						// something seriously wrong that didn't result in a
						// disconnection, and because they cause disconnections.
						Logger.warning(this, "Received all data but send failed to " + next);
					}
					else {
						if (prb.isAborted()) {
							Logger.normal(this,
									"Send failed: aborted: " + prb.getAbortReason() + ": " + prb.getAbortDescription());
						}
						else
	...
	protected void handleAcceptedRejectedTimeout(final PeerNode next, final UIDTag tag) {
		...
		// This is a WARNING not an ERROR because it's possible that the problem is we
		// simply haven't been able to send the message yet, because we don't use
		// sendSync().
			...
			node.usm.addAsyncFilter(mf, new SlowAsyncMessageFilterCallback() {
				...
				public void onMatched(Message m) {
					if (m.getSpec() == DMT.FNPRejectedLoop || m.getSpec() == DMT.FNPRejectedOverload) {
						...
						// We have moved on, and we don't want inserts to fork
						// unnecessarily.
						// However, we need to send a DataInsertRejected, or two-stage
						// timeout will happen.
						try {
							next.sendAsync(
									DMT.createFNPDataInsertRejected(uid,
									...
									new AsyncMessageCallback() {
										...
										public void sent() {
											// Ignore.
											if (logDEBUG)
												Logger.debug(this, "DataInsertRejected sent after accepted timeout on "
	...
	/**
	 * Finish the insert process. Will set status, wait for underlings to complete, and
	...
	private void finish(int code, PeerNode next) {
		...
		// If there is an InsertReply, it always happens before the transfer completion
		// notice.
		// So we do NOT need to removeRoutingTo().

		synchronized (this) {
			if (allTransfersCompleted)
				return; // Already called. Doesn't prevent race condition resulting in the
						// next bit running but that's not really a problem.
		...
		boolean failedRecv = false; // receiveFailed is protected by backgroundTransfers
									// but status by this
	...
	public void onReceiveFailed() {
		...
		// Do not call finish(), that can only be called on the main thread and it will
		// block.
	}
	...
	private void waitForBackgroundTransferCompletions() {
		try {
			...
			// We must presently be at such a stage that no more background transfers will
			// be added.
	...
	/**
	 * Block until all transfers have reached a final-terminal state (success/failure). On
	 * success this means that a successful 'received-notification' has been received.
	...
	private boolean waitForBackgroundTransfers(BackgroundTransfer[] transfers) {
		...
		while (true) {
			if (System.currentTimeMillis() > deadline) {
				// NORMAL priority because it is normally caused by a transfer taking too
				// long downstream, and that doesn't usually indicate a bug.
				Logger.normal(this,
						"Timed out waiting for background transfers! Probably caused by async filter not getting a timeout notification! DEBUG ME!");
				return false;
			}
			// If we want to be sure to exit as-soon-as the transfers are done, then we
			// must hold the lock while we check.
			synchronized (backgroundTransfers) {
				if (receiveFailed)
					return false;
	...
	protected void onAccepted(PeerNode next) {
		...
		/**
		 * What are we waiting for now??: - FNPRouteNotFound - couldn't exhaust HTL, but
		 * send us the data anyway please - FNPInsertReply - used up all HTL, yay -
		 * FNPRejectOverload - propagating an overload error :( - FNPRejectTimeout - we
		 * took too long to send the DataInsert - FNPDataInsertRejected - the insert was
		 * invalid
		 */

		int searchTimeout = calculateTimeout(htl);
		MessageFilter mfInsertReply = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(searchTimeout).setType(DMT.FNPInsertReply);
		MessageFilter mfRejectedOverload = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(searchTimeout).setType(DMT.FNPRejectedOverload);
		MessageFilter mfRouteNotFound = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(searchTimeout).setType(DMT.FNPRouteNotFound);
		MessageFilter mfDataInsertRejected = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(searchTimeout).setType(DMT.FNPDataInsertRejected);
		MessageFilter mfTimeout = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(searchTimeout).setType(DMT.FNPRejectedTimeout);

		MessageFilter mf = mfInsertReply
				.or(mfRouteNotFound.or(mfDataInsertRejected.or(mfTimeout.or(mfRejectedOverload))));
		...
		// Once the transfer has started, we only unlock the tag after the transfer
		...
		while (true) {
			...
			if (msg == null) {
				...
				Runnable r = new Runnable() {
					...
					public void run() {
						...
						// That will happen in the BackgroundTransfer, which has already
						// started.

						// FIXME factor out
						int searchTimeout = calculateTimeout(htl);
						MessageFilter mfInsertReply = MessageFilter.create().setSource(waitingFor)
								.setField(DMT.UID, uid).setTimeout(searchTimeout).setType(DMT.FNPInsertReply);
						MessageFilter mfRejectedOverload = MessageFilter.create().setSource(waitingFor)
								.setField(DMT.UID, uid).setTimeout(searchTimeout).setType(DMT.FNPRejectedOverload);
						MessageFilter mfRouteNotFound = MessageFilter.create().setSource(waitingFor)
								.setField(DMT.UID, uid).setTimeout(searchTimeout).setType(DMT.FNPRouteNotFound);
						MessageFilter mfDataInsertRejected = MessageFilter.create().setSource(waitingFor)
								.setField(DMT.UID, uid).setTimeout(searchTimeout).setType(DMT.FNPDataInsertRejected);
						MessageFilter mfTimeout = MessageFilter.create().setSource(waitingFor).setField(DMT.UID, uid)
								.setTimeout(searchTimeout).setType(DMT.FNPRejectedTimeout);
						...
						while (true) {
							...
							if (msg == null) {
								...
								Logger.error(this, "Got second (local) timeout on " + CHKInsertSender.this + " from "
										+ waitingFor);
								transfer.onCompleted();
							...
							if (msg.getSpec() == DMT.FNPRejectedOverload) {
								if (handleRejectedOverload(msg, waitingFor, tag)) {
									// Already set the status, and handle... will have
									// unlocked the next node, so no need to call
									// finished().
									transfer.onCompleted();
							...
							if (msg.getSpec() != DMT.FNPInsertReply) {
								...
								// Our task is complete, one node (quite deep), has
								// accepted the insert.
								// The request will not be routed to any other nodes, this
								// is where the data *should* be.
								// We will removeRoutingTo() after the node has sent the
								// transfer completion notice, which never happens before
								// the InsertReply.
								transfer.onCompleted();
								return;
							}
				...
				// Meanwhile, finish() to update allTransfersCompleted and hence allow the
				// CHKInsertHandler to send the message downstream.
				// We have already set the status code, this is necessary in order to
			...
			if (msg.getSpec() == DMT.FNPRejectedOverload) {
				if (handleRejectedOverload(msg, next, thisTag)) {
					...
					// They will complete it (finish()), so we need to wait for a transfer
					// completion.
					// FIXME it might be less confusing and therefore less likely to cause
					// problems
			...
			if (msg.getSpec() == DMT.FNPRouteNotFound) {
				// RNF means that the HTL was not exhausted, but that the data will still
				// be stored.
			...
			if (msg.getSpec() != DMT.FNPInsertReply) {
				...
				// The request will not be routed to any other nodes, this is where the
				// data *should* be.
				// We will removeRoutingTo() after the node has sent the transfer
				// completion notice, which never happens before the InsertReply.
				finish(SUCCESS, next);
				return;
			}
		}
