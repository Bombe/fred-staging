...
/**
 * Detect the IP address of the node. Doesn't return port numbers, doesn't have access to
 * per-port information (NodeCrypto - UdpSocketHandler etc).
 */
public class NodeIPDetector {
	...
	/**
	 * Explicit forced IP address in string form because we want to keep it even if it's
	 * invalid and therefore unused
	 */
	...
	private class MinimumMTU {
		...
		/**
		 * Report a new MTU from an interface or detector. If the minimum MTU has changed,
		 * returns true.
		 */
	...
	/**
	 * What is my IP address? Use all globally available information (everything which
	 * isn't specific to a given port i.e. opennet or darknet) to determine our current IP
	 * addresses. Will include more than one IP in many cases when we are not strictly
	 * multi-homed. For example, if we have a DNS name set, we will usually return an IP
	 * as well.
	 *
	...
	FreenetInetAddress[] detectPrimaryIPAddress(boolean dumpLocalAddresses) {
		...
		if (overrideIPAddress != null) {
			// If the IP is overridden and the override is valid, the override has to be
			// the first element.
	...
	private boolean innerDetect(List<FreenetInetAddress> addresses) {
		...
		// Add the old address only if we have no choice, or if we only have the word of
		// two peers to go on.
	...
	/**
	 * Process a list of DetectedIP's from the IP detector plugin manager. DetectedIP's
	 * can tell us what kind of NAT we are behind as well as our public IP address.
	 */
	...
	/**
	 * Is called by IPAddressDetector to inform NodeIPDetector about the MTU associated to
	 * this interface
	 */
	...
	public int registerConfigs(SubConfig nodeConfig, int sortOrder) {
		...
		nodeConfig.register("ipAddressOverride", "", sortOrder++, true, false, "NodeIPDectector.ipOverride",
				"NodeIPDectector.ipOverrideLong", new StringCallback() {
		...
		if (overrideIPAddressString.length() == 0)
			...
			try {
				...
				// **FIXME** This never happens for this reason with current
				// FreenetInetAddress(String, boolean, boolean) code; perhaps it needs
				// review?
		...
		nodeConfig.register("tempIPAddressHint", "", sortOrder++, true, false, "NodeIPDectector.tempAddressHint",
	...
	public void start() {
		...
		redetectAddress();
		// 60 second delay for inserting ARK to avoid reinserting more than necessary if
		// we don't detect IP on startup.
	...
	public void setMaybeSymmetric() {
		if (ipDetectorManager != null && ipDetectorManager.isEmpty()) {
			if (maybeSymmetricAlert == null) {
				maybeSymmetricAlert = new SimpleUserAlert(true, l10n("maybeSymmetricTitle"), l10n("maybeSymmetric"),
						l10n("maybeSymmetricShort"), FCPUserAlert.ERROR);
			}
