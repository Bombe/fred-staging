...
import freenet.node.math.DecayingKeyspaceAverage;
...
import freenet.support.node.stats.StatsNotAvailableException;
...
/**
 * Node (as opposed to NodeClientCore) level statistics. Includes shouldRejectRequest(),
 * but not limited to stuff required to implement that.
 */
public class NodeStats implements Persistable, BlockTimeCallback {

	public static enum RequestType {

		CHK_REQUEST, SSK_REQUEST, CHK_INSERT, SSK_INSERT, CHK_OFFER_FETCH, SSK_OFFER_FETCH;
	...
	private static class RequestsByLocation {
		...
		/**
		 * Get the request count bins. The total request count is placed at position zero
		 * of the given array.
		 */
	...
	/**
	 * Maximum overall average ping time. If ping is greater than this, we reject all
	 * requests.
	 */
	public static final long DEFAULT_MAX_PING_TIME = MILLISECONDS.toMillis(1500);

	/**
	 * Maximum throttled packet delay. If the throttled packet delay is greater than this,
	 * reject all packets.
	 */
	...
	/**
	 * If the throttled packet delay is less than this, reject no packets; if it's between
	 * the two, reject some packets.
	 */
	...
	/**
	 * How long we're over the nodeAveragePingTime threshold before we alert (in
	 * milliseconds)
	 */
	public static final long MAX_NODE_AVERAGE_PING_TIME_ALERT_DELAY = MINUTES.toMillis(10);

	/**
	 * Accept one request every 10 seconds regardless, to ensure we update the block send
	 * time.
	 */
	...
	NodeStats(Node node, int sortOrder, SubConfig statsConfig, int obwLimit, int ibwLimit, int lastVersion)
		...
		REJECT_STATS_AVERAGERS = new RunningAverage[] { pInstantRejectIncomingCHKRequestBulk,
				pInstantRejectIncomingSSKRequestBulk, pInstantRejectIncomingCHKInsertBulk,
		...
		throttledPacketSendAverage = new BootstrappingDecayingRunningAverage(0, 0, Long.MAX_VALUE, 100, null);
		throttledPacketSendAverageRT = new BootstrappingDecayingRunningAverage(0, 0, Long.MAX_VALUE, 100, null);
		throttledPacketSendAverageBulk = new BootstrappingDecayingRunningAverage(0, 0, Long.MAX_VALUE, 100, null);
		...
		statsConfig.register("threadLimit", defaultThreadLimit, sortOrder++, true, true, "NodeStat.threadLimit",
				"NodeStat.threadLimitLong", new IntCallback() {
					@Override
					public Integer get() {
						return threadLimit;
					}
		...
		// Yes it could be in seconds insteed of multiples of 0.12, but we don't want
		// people to play with it :)
		statsConfig.register("aggressiveGC", aggressiveGCModificator, sortOrder++, true, false, "NodeStat.aggressiveGC",
				"NodeStat.aggressiveGCLong", new IntCallback() {
					@Override
					public Integer get() {
		...
		statsConfig.register("memoryChecker", true, sortOrder++, true, false, "NodeStat.memCheck",
				"NodeStat.memCheckLong", new BooleanCallback() {
					@Override
					public Boolean get() {
		...
		statsConfig.register("ignoreLocalVsRemoteBandwidthLiability", false, sortOrder++, true, false,
				"NodeStat.ignoreLocalVsRemoteBandwidthLiability", "NodeStat.ignoreLocalVsRemoteBandwidthLiabilityLong",
				new BooleanCallback() {
		...
		statsConfig.register("maxPingTime", DEFAULT_MAX_PING_TIME, sortOrder++, true, true, "NodeStat.maxPingTime",
				"NodeStat.maxPingTimeLong", new LongCallback() {
		...
		// This is a *network* level setting, because it affects the rate at which we
		// initiate local
		...
		statsConfig.register("enableNewLoadManagementRT", false, sortOrder++, true, false,
				"Node.enableNewLoadManagementRT", "Node.enableNewLoadManagementRTLong", new BooleanCallback() {
		...
		statsConfig.register("enableNewLoadManagementBulk", false, sortOrder++, true, false,
				"Node.enableNewLoadManagementBulk", "Node.enableNewLoadManagementBulkLong", new BooleanCallback() {
		...
		if (node.lastVersion <= 1455 && (enableNewLoadManagementRT || enableNewLoadManagementBulk)) {
			...
			System.err.println(
					"Turning off NLM when upgrading from pre-1455. The load stats messages aren't being sent at the moment so it won't work anyway.");
			statsConfig.config.store();
		}

		persister = new ConfigurablePersister(this, statsConfig, "nodeThrottleFile", "node-throttle.dat", sortOrder++,
				true, false, "NodeStat.statsPersister", "NodeStat.statsPersisterLong", node.ticker, node.getRunDir());
		...
		localChkFetchBytesReceivedAverage = new TimeDecayingRunningAverage(
				32768 + 2048/* path folding */, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("LocalChkFetchBytesReceivedAverage"), node);
		...
		remoteChkFetchBytesSentAverage = new TimeDecayingRunningAverage(
				32768 + 1024 + 500 + 2048/* path folding */, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("RemoteChkFetchBytesSentAverage"), node);
		remoteSskFetchBytesSentAverage = new TimeDecayingRunningAverage(1024 + 1024 + 500, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("RemoteSskFetchBytesSentAverage"), node);
		remoteChkInsertBytesSentAverage = new TimeDecayingRunningAverage(32768 + 32768 + 1024, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("RemoteChkInsertBytesSentAverage"), node);
		remoteSskInsertBytesSentAverage = new TimeDecayingRunningAverage(1024 + 1024 + 500, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("RemoteSskInsertBytesSentAverage"), node);
		remoteChkFetchBytesReceivedAverage = new TimeDecayingRunningAverage(
				32768 + 1024 + 500 + 2048/* path folding */, 180000, 0.0, 200 * 1024,
		...
		remoteChkInsertBytesReceivedAverage = new TimeDecayingRunningAverage(32768 + 1024 + 500, 180000, 0.0,
				200 * 1024, throttleFS == null ? null : throttleFS.subset("RemoteChkInsertBytesReceivedAverage"), node);
		remoteSskInsertBytesReceivedAverage = new TimeDecayingRunningAverage(1024 + 1024 + 500, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("RemoteSskInsertBytesReceivedAverage"), node);

		successfulChkFetchBytesSentAverage = new TimeDecayingRunningAverage(
				32768 + 1024 + 500 + 2048/* path folding */, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("SuccessfulChkFetchBytesSentAverage"), node);
		successfulSskFetchBytesSentAverage = new TimeDecayingRunningAverage(1024 + 1024 + 500, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("SuccessfulSskFetchBytesSentAverage"), node);
		successfulChkInsertBytesSentAverage = new TimeDecayingRunningAverage(32768 + 32768 + 1024, 180000, 0.0,
				200 * 1024, throttleFS == null ? null : throttleFS.subset("SuccessfulChkInsertBytesSentAverage"), node);
		successfulSskInsertBytesSentAverage = new TimeDecayingRunningAverage(1024 + 1024 + 500, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("SuccessfulSskInsertBytesSentAverage"), node);
		successfulChkOfferReplyBytesSentAverage = new TimeDecayingRunningAverage(32768 + 500, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("successfulChkOfferReplyBytesSentAverage"), node);
		successfulSskOfferReplyBytesSentAverage = new TimeDecayingRunningAverage(3072, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("successfulSskOfferReplyBytesSentAverage"), node);
		...
		successfulSskFetchBytesReceivedAverage = new TimeDecayingRunningAverage(2048 + 500, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("SuccessfulSskFetchBytesReceivedAverage"), node);
		successfulChkInsertBytesReceivedAverage = new TimeDecayingRunningAverage(32768 + 1024 + 500, 180000, 0.0,
				200 * 1024, throttleFS == null ? null : throttleFS.subset("SuccessfulChkInsertBytesReceivedAverage"),
				node);
		...
		successfulChkOfferReplyBytesReceivedAverage = new TimeDecayingRunningAverage(32768 + 500, 180000, 0.0,
				200 * 1024,
				throttleFS == null ? null : throttleFS.subset("successfulChkOfferReplyBytesReceivedAverage"), node);
		successfulSskOfferReplyBytesReceivedAverage = new TimeDecayingRunningAverage(3072, 180000, 0.0, 200 * 1024,
				throttleFS == null ? null : throttleFS.subset("successfulSskOfferReplyBytesReceivedAverage"), node);
	...
	/**
	 * Every 60 seconds, check whether we need to adjust the bandwidth delay time because
	 * of idleness. (If no packets have been sent, the throttledPacketSendAverage should
	 * decrease; if it doesn't, it may go high, and then no requests will be accepted, and
	 * it will stay high forever.
	 */
	static final long CHECK_THROTTLE_TIME = SECONDS.toMillis(60);

	/**
	 * Absolute limit of 4MB queued to any given peer. FIXME make this configurable. Note
	 * that for many MessageItem's, the actual memory usage will be significantly more
	 * than this figure.
	 */
	private static final long MAX_PEER_QUEUE_BYTES = 4 * 1024 * 1024;

	/**
	 * Don't accept requests if it'll take more than 1 minutes to send the current message
	 * queue. On the assumption that most of the message queue is block transfer data.
	 * Note that this only applies to data on the queue before calling
	 * shouldRejectRequest(): we do *not* attempt to include any estimate of how much the
	 * request will add to it. This is important because if we did, the AIMD may not have
	 * reached sufficient speed to transfer it in 60 seconds yet, because it hasn't had
	 * enough data in transit to need to increase its speed.
	 *
	 ...
	 *
	 * A fast peer can have slightly more than half our output limit queued in requests to
	 * run. If they all complete, they will take half the time limit. If they are all
	 * served from the store, this will be shown on the queue time. But the queue time is
	 * estimated based on using at most half the limit, so the time will be slightly over
	 * the overall limit.
	 */
	 ...
	 * prophecy, and it takes a long time to shake it off as the averages gradually
	 * increase. If we accept no requests then everything is overhead! Whereas with a high
	 * minimum overhead the worst case is that more stuff succeeds than expected and we
	 * have a few timeouts (because output bandwidth liability was assuming a lower
	 * overhead than actually happens) - but this should be very rare.
	 */
	...
	// /** Proportion of the various bandwidth limits we can use without triggering
	// slow-down messages */
	 ...
	 */
	static final int BANDWIDTH_LIABILITY_LIMIT_SECONDS_BULK = 120; // FIXME slowdown
																	// (int)(120 /
	 ...
	 */
	static final int BANDWIDTH_LIABILITY_LIMIT_SECONDS_REALTIME = 60; // FIXME slowdown
																		// (int)(60 /
																		// SOFT_REJECT_MAX_BANDWIDTH_USAGE);

	/**
	 * Stats to send to a single peer so it can determine whether we are likely to reject
	 * a request. Includes the various limits, but also, the expected transfers from
	 * requests already accepted from other peers (but NOT from this peer). Note that
	 * requests which are sourceRestarted() or reassignToSelf() are included in "from
	 * other peers", as are local requests.
	 */
	public class PeerLoadStats {
		...
		/**
		 * Maximum transfers out - per-peer limit. If total is over the lower limit, we
		 * will be accepted as long as we are below this limit.
		 */
		public final int maxTransfersOutPeerLimit;

		/**
		 * Maximum transfers out - lower overall limit. If total is over this limit, we
		 * will be accepted as long as the per-peer usage is above the peer limit.
		 */
		public final int maxTransfersOutLowerLimit;

		/**
		 * Maximum transfers out - upper overall limit. Nothing is accepted above this
		 * limit.
		 */
		...
		public String toString() {
			return peer.toString() + ":output:{lower=" + outputBandwidthLowerLimit + ",upper="
					...
					+ inputBandwidthLowerLimit + ",upper=" + inputBandwidthUpperLimit + ",peer="
					+ inputBandwidthPeerLimit + "},requests:" + "in:" + expectedTransfersInCHK + "chk/"
					+ expectedTransfersInSSK + "ssk:out:" + expectedTransfersOutCHK + "chk/" + expectedTransfersOutSSK
					+ "ssk transfers=" + maxTransfersOut + "/" + maxTransfersOutPeerLimit + "/"
					+ maxTransfersOutLowerLimit + "/" + maxTransfersOutUpperLimit;
		}
		...
		public PeerLoadStats(PeerNode peer, int transfersPerInsert, boolean realTimeFlag) {
			...
			// Peer limits are adjusted to deduct any requests already allocated by
			// sourceRestarted() requests.
			// I.e. requests which were sent before the peer restarted, which it doesn't
			// know about or failed for when the bootID changed.
			// We try to complete these quickly, but they can stick around for a while
			// sometimes.

			// This implies that the sourceRestarted() requests are not counted when
			// checking whether the peer is over the limit.
			...
			maxTransfersOutPeerLimit = (int) Math.max(1,
					getPeerLimit(peer, maxTransfersOutUpperLimit - maxTransfersOutLowerLimit, false, transfersPerInsert,
							realTimeFlag, peers,
							runningLocal.expectedTransfersOutCHKSR + runningLocal.expectedTransfersOutSSKSR));
			maxTransfersOut = calculateMaxTransfersOut(peer, realTime, nonOverheadFraction, maxTransfersOutUpperLimit);

			outputBandwidthPeerLimit = getPeerLimit(peer, outputBandwidthUpperLimit - outputBandwidthLowerLimit, false,
					transfersPerInsert, realTimeFlag, peers, runningLocal.calculateSR(ignoreLocalVsRemote, false));
			inputBandwidthPeerLimit = getPeerLimit(peer, inputBandwidthUpperLimit - inputBandwidthLowerLimit, true,
					transfersPerInsert, realTimeFlag, peers, runningLocal.calculateSR(ignoreLocalVsRemote, true));

			this.averageTransfersOutPerInsert = transfersPerInsert;

			RunningRequestsSnapshot runningGlobal = new RunningRequestsSnapshot(node.tracker, ignoreLocalVsRemote,
					transfersPerInsert, realTimeFlag);
	...
	class RunningRequestsSnapshot {
		...
		/**
		 * Create a snapshot of all requests running. Because this isn't for any
		 * particular peer, it includes all requests, even those which are
		 * SourceRestarted.
		 * @param tracker The RequestTracker for the Node.
		 * @param ignoreLocalVsRemote If true, pretend that the request is remote even if
		 * it's local (that is, count imaginary onward transfers etc depending on the
		 * request type).
		 * @param transfersPerInsert Assume that any insert will cause this many outgoing
		 * transfers. This is not predictable, so we use an average.
		 * @param realTimeFlag If true, count real-time requests, if false, count bulk
		 * requests.
		 */
		RunningRequestsSnapshot(RequestTracker tracker, boolean ignoreLocalVsRemote, int transfersPerInsert,
			...
			tracker.countRequests(true, false, false, false, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countCHK, countCHKSR);
			tracker.countRequests(true, true, false, false, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countSSK, countSSKSR);
			tracker.countRequests(true, false, true, false, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countCHK, countCHKSR);
			tracker.countRequests(true, true, true, false, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countSSK, countSSKSR);
			tracker.countRequests(false, false, false, false, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countCHK, countCHKSR);
			tracker.countRequests(false, true, false, false, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countSSK, countSSKSR);
			tracker.countRequests(false, false, true, false, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countCHK, countCHKSR);
			tracker.countRequests(false, true, true, false, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countSSK, countSSKSR);
			tracker.countRequests(false, false, false, true, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countCHK, countCHKSR);
			tracker.countRequests(false, true, false, true, realTimeFlag, transfersPerInsert, ignoreLocalVsRemote,
					countSSK, countSSKSR);
		...
		/**
		 * Create a snapshot of either the requests from a node, or the requests routed to
		 * a node. If we are counting requests from a node, we also fill in the *SR
		 * counters with the counts for requests which have sourceRestarted() i.e. the
		 * requests where the peer has reconnected after a timeout but the requests are
		 * still running. These are only counted in the *SR totals, they are not in the
		 * basic totals. The caller will reduce the limits according to the *SR totals,
		 * and only consider the non-SR requests when deciding whether the peer is over
		 * the limit. The updated limits are sent to the downstream node so that it can
		 * send the right number of requests.
		...
		RunningRequestsSnapshot(RequestTracker tracker, PeerNode source, boolean requestsToNode,
			...
			tracker.countRequests(source, requestsToNode, true, false, false, false, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countCHK, countCHKSR);
			tracker.countRequests(source, requestsToNode, true, true, false, false, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countSSK, countSSKSR);
			tracker.countRequests(source, requestsToNode, true, false, true, false, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countCHK, countCHKSR);
			tracker.countRequests(source, requestsToNode, true, true, true, false, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countSSK, countSSKSR);
			tracker.countRequests(source, requestsToNode, false, false, false, false, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countCHK, countCHKSR);
			tracker.countRequests(source, requestsToNode, false, true, false, false, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countSSK, countSSKSR);
			tracker.countRequests(source, requestsToNode, false, false, true, false, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countCHK, countCHKSR);
			tracker.countRequests(source, requestsToNode, false, true, true, false, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countSSK, countSSKSR);
			tracker.countRequests(source, requestsToNode, false, false, false, true, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countCHK, countCHKSR);
			tracker.countRequests(source, requestsToNode, false, true, false, true, realTimeFlag, transfersPerInsert,
					ignoreLocalVsRemote, countSSK, countSSKSR);
			if (!requestsToNode) {
				this.expectedTransfersInCHKSR = countCHKSR.expectedTransfersIn();
				this.expectedTransfersInSSKSR = countSSKSR.expectedTransfersIn();
		...
		private RunningRequestsSnapshot(int averageTransfersPerInsert) {
			...
			totalRequests = 1; // Yes this is inconsistent, but it's used for a different
								// calculation.
		...
		public void log(PeerNode source) {
			String message = "Running (adjusted): CHK in: " + expectedTransfersInCHK + " out: "
					+ expectedTransfersOutCHK + " SSK in: " + expectedTransfersInSSK + " out: "
					+ expectedTransfersOutSSK + " total=" + totalRequests + (source == null ? "" : (" for " + source))
					+ (realTimeFlag ? " (realtime)" : " (bulk)");
			if (expectedTransfersInCHK < 0 || expectedTransfersOutCHK < 0 || expectedTransfersInSSK < 0
		...
		public double calculate(boolean ignoreLocalVsRemoteBandwidthLiability, boolean input) {
			...
			if (input)
				return this.expectedTransfersInCHK * (32768 + 256) + this.expectedTransfersInSSK * (2048 + 256)
						+ this.expectedTransfersOutCHK * TRANSFER_OUT_IN_OVERHEAD
						+ this.expectedTransfersOutSSK * TRANSFER_OUT_IN_OVERHEAD;
			else
				return this.expectedTransfersOutCHK * (32768 + 256) + this.expectedTransfersOutSSK * (2048 + 256)
		...
		public double calculateSR(boolean ignoreLocalVsRemoteBandwidthLiability, boolean input) {
			...
			if (input)
				return this.expectedTransfersInCHKSR * (32768 + 256) + this.expectedTransfersInSSKSR * (2048 + 256)
						+ this.expectedTransfersOutCHKSR * TRANSFER_OUT_IN_OVERHEAD
						+ this.expectedTransfersOutSSKSR * TRANSFER_OUT_IN_OVERHEAD;
			else
				return this.expectedTransfersOutCHKSR * (32768 + 256) + this.expectedTransfersOutSSKSR * (2048 + 256)
						+ expectedTransfersInCHKSR * TRANSFER_IN_OUT_OVERHEAD
						+ expectedTransfersInSSKSR * TRANSFER_IN_OUT_OVERHEAD;
		}

		/**
		 * @return The number of requests running or -1 if not known (remote doesn't tell
		 * us).
		 */
	...
	/**
	 * Output bytes required for an inbound transfer. Includes e.g. sending the request in
	 * the first place.
	 */
	...
	static class RejectReason {
		...
		/**
		 * If true, rejected because of preemptive bandwidth limiting, i.e. "soft", at
		 * least somewhat predictable, can be retried. If false, hard rejection, should
		 * backoff and not retry.
		 */
	...
	/**
	 * Should a request be accepted by this node, based on its local capacity? This
	 * includes thread limits and ping times, but more importantly, mechanisms based on
	 * predicting worst case bandwidth usage for all running requests, and fairly sharing
	 * that capacity between peers. Currently there is no mechanism for fairness between
	 * request types, this should be implemented on the sender side, and is with new load
	 * management. New load management has caused various changes here but that's probably
	 * sorted out now, i.e. changes involved in new load management will probably be
	 * mainly in PeerNode and RequestSender now.
	 * @param canAcceptAnyway Periodically we ignore the ping time and accept a request
	 * anyway. This is because the ping time partly depends on whether we have accepted
	 * any requests... FIXME this should be reconsidered.
	 * @param isInsert Whether this is an insert.
	 * @param isSSK Whether this is a request/insert for an SSK.
	 * @param isLocal Is this request originated locally? This can affect our estimate of
	 * likely bandwidth usage. Whether it should be used is unclear, since an attacker can
	 * observe bandwidth usage. It is configurable.
	 * @param isOfferReply Is this request actually a GetOfferedKey? This is a non-relayed
	 * fetch of a block which we recently offered via ULPRs.
	 * @param source The node that sent us this request. This should be null on local
	 * requests and non-null on remote requests, but in some parts of the code that
	 * doesn't always hold. It *should* hold here, but that needs more checking before we
	 * can remove isLocal.
	 * @param hasInStore If this is a request, do we have the block in the datastore
	 * already? This affects whether we accept it, which gives a significant performance
	 * gain. Arguably there is a security issue, although timing attacks are pretty easy
	 * anyway, and making requests go further may give attackers more samples...
	 * @param preferInsert If true, prefer inserts to requests (slightly). There is a flag
	 * for this on inserts. The idea is that when inserts are misrouted this causes
	 * long-term problems because the data is stored in the wrong place. New load
	 * management should avoid the need for this.
	 * @param realTimeFlag Is this a realtime request (low latency, low capacity) or a
	 * bulk request (high latency, high capacity)? They are accounted for separately.
	 * @return The reason for rejecting it, or null to accept it.
	 */
	public RejectReason shouldRejectRequest(boolean canAcceptAnyway, boolean isInsert, boolean isSSK, boolean isLocal,
			boolean isOfferReply, PeerNode source, boolean hasInStore, boolean preferInsert, boolean realTimeFlag,
			UIDTag tag) {
		// Serialise shouldRejectRequest.
		// It's not always called on the same thread, and things could be problematic if
		...
		synchronized (serializeShouldRejectRequest) {
			...
			// slowDown(">softThreadLimit", isLocal, isInsert, isSSK, isOfferReply,
			// realTimeFlag, tag);
			...
			// Don't need to decrement because it won't be counted until setAccepted()
			// below.
			...
			// Allow a bit more if the data is in the store and can therefore be served
			// immediately.
			...
			if (hasInStore) {
				...
				if (logMINOR)
					Logger.minor(this,
							"Maybe accepting extra request due to it being in datastore (limit now " + limit + "s)...");
			}
			...
			/**
			 * Requests running for this specific peer (local counts as a peer). Note that
			 * this separately counts requests which have sourceRestarted, which are not
			 * included in the count, and are decremented from the peer limit before it is
			 * used and sent to the peer. This ensures that the peer doesn't use more than
			 * it should after a restart.
			 */
			RunningRequestsSnapshot peerRequestsSnapshot = new RunningRequestsSnapshot(node.tracker, source, false,
					ignoreLocalVsRemoteBandwidthLiability, transfersPerInsert, realTimeFlag);
			if (logMINOR)
			...
			int maxTransfersOutPeerLimit = (int) Math.max(1, getPeerLimit(source,
					...
					peers,
					(peerRequestsSnapshot.expectedTransfersOutCHKSR + peerRequestsSnapshot.expectedTransfersOutSSKSR)));
			...
			String ret = checkBandwidthLiability(getOutputBandwidthUpperLimit(limit, nonOverheadFraction),
					requestsSnapshot, peerRequestsSnapshot, false, limit, source, isLocal, isSSK, isInsert,
					isOfferReply, hasInStore, transfersPerInsert, realTimeFlag, maxOutputTransfers,
					maxTransfersOutPeerLimit, tag);
			if (ret != null) {
				return new RejectReason(ret, true);
			}

			ret = checkBandwidthLiability(getInputBandwidthUpperLimit(limit), requestsSnapshot, peerRequestsSnapshot,
					true, limit, source, isLocal, isSSK, isInsert, isOfferReply, hasInStore, transfersPerInsert,
					realTimeFlag, maxOutputTransfers, maxTransfersOutPeerLimit, tag);
			if (ret != null) {
				return new RejectReason(ret, true);
			}
			...
			ret = checkMaxOutputTransfers(maxOutputTransfers, maxTransfersOutUpperLimit, maxTransfersOutLowerLimit,
					maxTransfersOutPeerLimit, requestsSnapshot, peerRequestsSnapshot, isLocal, realTimeFlag, isInsert,
					isSSK, isOfferReply, tag);
			if (ret != null) {
				return new RejectReason(ret, true);
			}
	...
	// if(logMINOR) Logger.minor(this, "Slow down: "+reason+" insert="+isInsert+"
	// SSK="+isSSK+" realTimeFlag="+realTimeFlag);
	// tag.slowDown();
	// }
	...
	public int calculateMaxTransfersOut(PeerNode peer, boolean realTime, double nonOverheadFraction,
	...
	private double getNonOverheadFraction(long now) {
		...
		if (nonOverheadFraction < MIN_NON_OVERHEAD) {
			// If there's been an auto-update, we may have used a vast amount of bandwidth
			// for it.
			// Also, if things have broken, our overhead might be above our bandwidth
			// limit,
			// especially on a slow node.

			// So impose a minimum of 20% of the bandwidth limit.
			// This will ensure we don't get stuck in any situation where all our
			// bandwidth is overhead,
	...
	private int getMaxTransfersUpperLimit(boolean realTime, double nonOverheadFraction) {
		// FIXME refactor with getOutputBandwidthUpperLimit so we can avoid calling it
		// twice.
		double outputAvailablePerSecond = node.getOutputBandwidthLimit() * nonOverheadFraction;
	...
	/**
	 * Should the request be rejected due to bandwidth liability? Enforces fair sharing
	 * between peers, while allowing peers to opportunistically use a bit more than their
	 * fair share as long as the total is below the lower limit. Used for both
	 * bandwidth-based limiting and transfer-count-based limiting.
	 * @param bandwidthAvailableOutputUpperLimit The overall upper limit, already
	 * calculated.
	...
	private String checkBandwidthLiability(double bandwidthAvailableOutputUpperLimit,
			...
			long limit, PeerNode source, boolean isLocal, boolean isSSK, boolean isInsert, boolean isOfferReply,
		...
		// Calculate the peer limit so the peer gets notified, even if we are going to
		// ignore it.
		...
		double thisAllocation = getPeerLimit(source,
				...
				realTimeFlag, peers, peerRequestsSnapshot.calculateSR(ignoreLocalVsRemoteBandwidthLiability, input));
		...
		// Because we reassignToSelf() in various tricky timeout conditions, it is
		...
		if (bandwidthLiabilityOutput > bandwidthAvailableOutputLowerLimit) {
			...
			// Bandwidth is scarce (we are over the lower limit i.e. more than half our
			// capacity is used).
			// Share available bandwidth fairly between peers.

			if (logMINOR)
				Logger.minor(this,
						"Allocation (" + name + ") for " + source + " is " + thisAllocation + " total usage is "
								+ bandwidthLiabilityOutput + " of lower limit" + bandwidthAvailableOutputLowerLimit
								+ " upper limit is " + bandwidthAvailableOutputUpperLimit + " for " + name);

			double peerUsedBytes = getPeerBandwidthLiability(peerRequestsSnapshot, source, isSSK, transfersPerInsert,
					input);
			if (peerUsedBytes > thisAllocation) {
				rejected(name + " bandwidth liability: fairness between peers", isLocal, isInsert, isSSK, isOfferReply,
						realTimeFlag);
				return name + " bandwidth liability: fairness between peers (peer " + source + " used " + peerUsedBytes
						+ " allowed " + thisAllocation + ")";
				// FIXME slowdown
				// } else {
				// double slowDownLimit = thisAllocation *
				// SOFT_REJECT_MAX_BANDWIDTH_USAGE;
				// // Allow any node to use one request of each type. They'll just have to
				// get backed off if necessary.
				// slowDownLimit = Math.max(slowDownLimit,
				// requestsSnapshot.calculateMinimum(input,
				// ignoreLocalVsRemoteBandwidthLiability, transfersPerInsert));
			...
			// Plenty of bandwidth available, allow one peer to use up to the lower limit
			// (about half the total).

			// slowDown() is unnecessary, we do NOT want to keep the total below the lower
			// limit.
	...
	private String checkMaxOutputTransfers(int maxOutputTransfers, int maxTransfersOutUpperLimit,
			int maxTransfersOutLowerLimit, int maxTransfersOutPeerLimit, RunningRequestsSnapshot requestsSnapshot,
			RunningRequestsSnapshot peerRequestsSnapshot, boolean isLocal, boolean realTime, boolean isInsert,
			boolean isSSK, boolean isOfferReply, UIDTag tag) {
		if (logMINOR)
			Logger.minor(this,
					"Max transfers: congestion control limit " + maxOutputTransfers + " upper "
							+ maxTransfersOutUpperLimit + " lower " + maxTransfersOutLowerLimit + " peer "
		...
		if (peerOutTransfers <= maxTransfersOutPeerLimit) {
			// The total is above the lower limit, but the per-peer is below the peer
			// limit.
	 ...
	 * @return
	 */
	private double getPeerLimit(PeerNode source, double totalGuaranteedBandwidth, boolean input, int transfersPerInsert,
			boolean realTimeFlag, int peers, double sourceRestarted) {
	...
	/**
	 * Calculate the worst-case bytes used by a specific peer. This will be compared to
	 * its peer limit, and if it is higher, the request may be rejected.
	 ...
	 * @param x The threshold. We should reject the request unless a random number is
	...
	private void rejected(String reason, boolean isLocal, boolean isInsert, boolean isSSK, boolean isOfferReply,
			boolean isRealTime) {
		reason += " " + (isRealTime ? " (rt)" : " (bulk)");
		if (logMINOR)
			Logger.minor(this,
					"Rejecting (local=" + isLocal + ") isSSK=" + isSSK + " isInsert=" + isInsert + " : " + reason);
		if (!isLocal)
	...
	private void accepted(boolean isLocal, boolean isInsert, boolean isSSK, boolean isOfferReply,
			boolean realTimeFlag) {
		if (!isLocal && !isOfferReply) {
	...
	private BootstrappingDecayingRunningAverage getRejectedTracker(boolean isRealTime, boolean isSSK,
			boolean isInsert) {
		if (isRealTime) {
			if (isSSK) {
	...
	private void dumpByteCostAverages() {
		Logger.minor(this,
				"Byte cost averages: REMOTE:" + " CHK insert " + remoteChkInsertBytesSentAverage.currentValue() + '/'
						+ remoteChkInsertBytesReceivedAverage.currentValue() + " SSK insert "
						+ remoteSskInsertBytesSentAverage.currentValue() + '/'
						+ remoteSskInsertBytesReceivedAverage.currentValue() + " CHK fetch "
						+ remoteChkFetchBytesSentAverage.currentValue() + '/'
						+ remoteChkFetchBytesReceivedAverage.currentValue() + " SSK fetch "
						+ remoteSskFetchBytesSentAverage.currentValue() + '/'
						+ remoteSskFetchBytesReceivedAverage.currentValue());
		Logger.minor(this, "Byte cost averages: LOCAL:" + " CHK insert " + localChkInsertBytesSentAverage.currentValue()
				+ '/' + localChkInsertBytesReceivedAverage.currentValue() + " SSK insert "
				+ localSskInsertBytesSentAverage.currentValue() + '/'
				+ localSskInsertBytesReceivedAverage.currentValue() + " CHK fetch "
		...
		Logger.minor(this,
				"Byte cost averages: SUCCESSFUL:" + " CHK insert " + successfulChkInsertBytesSentAverage.currentValue()
						+ '/' + successfulChkInsertBytesReceivedAverage.currentValue() + " SSK insert "
						+ successfulSskInsertBytesSentAverage.currentValue() + '/'
						+ successfulSskInsertBytesReceivedAverage.currentValue() + " CHK fetch "
						+ successfulChkFetchBytesSentAverage.currentValue() + '/'
						+ successfulChkFetchBytesReceivedAverage.currentValue() + " SSK fetch "
						+ successfulSskFetchBytesSentAverage.currentValue() + '/'
						+ successfulSskFetchBytesReceivedAverage.currentValue() + " CHK offer reply "
						+ successfulChkOfferReplyBytesSentAverage.currentValue() + '/'
						+ successfulChkOfferReplyBytesReceivedAverage.currentValue() + " SSK offer reply "
	...
	/**
	 * Update peerManagerUserAlertStats if the timer has expired. Only called from
	 * PacketSender so doesn't need sync.
	 */
		...
		if (now > nextPeerManagerUserAlertStatsUpdateTime) {
			...
			if (logDEBUG)
				Logger.debug(this, "mUPMUAS: " + now + ": " + getBwlimitDelayTime() + " >? "
						...
						+ MAX_NODE_AVERAGE_PING_TIME_ALERT_THRESHOLD + " since "
						+ firstNodeAveragePingTimeThresholdBreak + " (" + nodeAveragePingAlertRelevant + ')');
	...
	public SimpleFieldSet persistThrottlesToFieldSet() {
		...
		// These are not really part of the 'throttling' data, but are also running
		// averages which should be persisted
	...
	/**
	 * Gets a copy of the thread list. The result might contain null values: The end of
	 * the list is marked by a null entry in the array.
	 */
	...
	public SimpleFieldSet exportVolatileFieldSet() {
		...
		int numberOfNotConnected = numberOfTooNew + numberOfTooOld + numberOfDisconnected + numberOfNeverConnected
				+ numberOfDisabled + numberOfBursting + numberOfListening + numberOfListenOnly;
	...
	public void fillSuccessRateBox(HTMLNode parent) {
		...
		final RunningAverage[] averages = new RunningAverage[] { globalFetchPSuccess, chkLocalFetchPSuccess,
				chkRemoteFetchPSuccess, sskLocalFetchPSuccess, sskRemoteFetchPSuccess, blockTransferPSuccessBulk,
				blockTransferPSuccessRT, blockTransferPSuccessLocal, blockTransferFailTimeout };
		final String[] names = new String[] { l10n("allRequests"), l10n("localCHKs"), l10n("remoteCHKs"),
				l10n("localSSKs"), l10n("remoteSSKs"), l10n("blockTransfersBulk"), l10n("blockTransfersRT"),
				l10n("blockTransfersLocal"), l10n("transfersTimedOut") };
	...
	// Opennet-related bytes - *not* including bytes sent on requests, those are accounted
	// towards
	// the requests' totals.
	...
	/**
	 * The average number of bytes sent per second for things other than requests,
	 * inserts, and offer replies.
	 */
	public double getSentOverheadPerSecond() {
		long uptime = node.getUptime();
		// actually we’d want to convert uptime to seconds here but this is results in
		// better accuracy.
	...
	public synchronized void failedBlockReceive(boolean normalFetch, boolean timeout, boolean realTimeFlag,
			boolean isLocal) {
		if (normalFetch) {
	...
	public void reportCHKOutcome(long rtt, boolean successful, double location, boolean isRealtime) {
		if (successful) {
			...
			(isRealtime ? unsuccessfulLocalCHKFetchTimeAverageRT : unsuccessfulLocalCHKFetchTimeAverageBulk)
					.report(rtt);
	...
	public void reportSSKOutcome(long rtt, boolean successful, boolean isRealtime) {
		if (successful) {
			...
			(isRealtime ? unsuccessfulLocalSSKFetchTimeAverageRT : unsuccessfulLocalSSKFetchTimeAverageBulk)
					.report(rtt);
		}
	...
	public void fillDetailedTimingsBox(HTMLNode html) {
		...
		row.addChild("td",
				TimeUtil.formatTime((long) unsuccessfulLocalCHKFetchTimeAverageBulk.currentValue(), 2, true));
		row.addChild("td", TimeUtil.formatTime((long) unsuccessfulLocalCHKFetchTimeAverageRT.currentValue(), 2, true));
		row.addChild("td",
				TimeUtil.formatTime((long) unsuccessfulLocalSSKFetchTimeAverageBulk.currentValue(), 2, true));
	...
	private String sanitizeDBJobType(String jobType) {
		int typeBeginIndex = jobType.lastIndexOf('.'); // Only use the actual class name,
	...
	private double cappedDistance(DecayingKeyspaceAverage avgLocation, StoreCallback<?> store) {
		...
		// Cap the reported value at 100%, as the decaying average does not account beyond
		// that anyway.
		if (cachePercent > 1.0) {
			cachePercent = 1.0;
		}
	...
	enum AnnouncementDecision {
		...
		ACCEPT, OVERLOAD, LOOP

	}

	public AnnouncementDecision shouldAcceptAnnouncement(long uid) {
		int outputPerSecond = node.getOutputBandwidthLimit() / 2; // FIXME: Take overhead
																	// into account??? Be
																	// careful, it may
																	// include
																	// announcements and
		...
		if (logMINOR)
			Logger.minor(this,
					"Delay times: realtime: local=" + nlmDelayRTLocal.currentValue() + " remote = "
							+ nlmDelayRTRemote.currentValue() + " bulk: local=" + nlmDelayBulkLocal.currentValue()
							+ " remote=" + nlmDelayBulkRemote.currentValue());
	}

	public void drawNewLoadManagementDelayTimes(HTMLNode content) {
		RequestTracker.WaitingForSlots waitingSlots = node.tracker.countRequestsWaitingForSlots();
		content.addChild("p").addChild("#", l10n("slotsWaiting", new String[] { "local", "remote" },
				new String[] { Integer.toString(waitingSlots.local), Integer.toString(waitingSlots.remote) }));
		...
		synchronized (slotTimeoutsSync) {
			if (fatalTimeoutsInWaitLocal + fatalTimeoutsInWaitRemote + allocatedSlotLocal + allocatedSlotRemote > 0) {
				...
				header.addChild("th", l10n("remoteHeader"));
				row = table.addChild("tr");
				row.addChild("td", this.fix3p3pct.format(((double) fatalTimeoutsInWaitLocal)
						/ ((double) (fatalTimeoutsInWaitLocal + allocatedSlotLocal))));
				row.addChild("td", this.fix3p3pct.format(((double) fatalTimeoutsInWaitRemote)
	...
	private final Runnable noisyRejectStatsUpdater = new Runnable() {
		...
		@Override
		public void run() {
			// SECURITY/TRIVIAL PERFORMANCE TRADEOFF: I don't think we want to run this
			// lazily.
			// If we run it lazily, an attacker could trigger it, given that it's
			// triggered rarely
			// in normal operation. FIXME long term we probably want to get rid of this
			// from the
			// production network, and just surveil a few "special" nodes which volunteer
			// to have
	...
	/**
	 * If positive, the level of fuzz (size of 1 standard deviation for gauss in percent)
	 * to use
	 */
	...
	/**
	 * @return Array of 4 bytes, with the percentage rejections for (bulk only): CHK
	 * request, SSK request, CHK insert, SSK insert. Negative value = insufficient data.
	 * Positive value = percentage rejected. PRECAUTIONS: We update this statistic every
	 * 10 minutes. We don't return a value unless we have at least 200 samples. We add
	 * Gaussian noise. FIXME SECURITY We should remove this eventually.
	 */
	...
	/**
	 * Applies multiplicative Gaussian noise of mean 1.0 and the specified sigma to the
	 * input value.
	...
	public final double randomNoise(final double input, final double sigma) {
		...
		/*
		 * Cap noise to [0.5, 1.5]. Such amounts are very rare (5 sigma at 10%) and serve
		 * only to throw off the statistics by including crazy things like negative values
		 * or impossibly huge limits.
		 */
		if (multiplier < 0.5)
			multiplier = 0.5;
