...
public class InvalidAddressOverrideUserAlert extends BaseNodeUserAlert {
	...
	public HTMLNode getHTMLText() {
		...
		NodeL10n.getBase().addL10nSubstitution(textNode, "InvalidAddressOverrideUserAlert.unknownAddressWithConfigLink",
				new String[] { "link" }, new HTMLNode[] { HTMLNode.link("/config/node") });
		HTMLNode formNode = textNode.addChild("form", new String[] { "action", "method" },
				new String[] { "/config/node", "post" });
		...
		itemNode.addChild("span", "class", "configshortdesc", o.getLocalisedShortDesc()).addChild("input",
				...
				new String[] { "text", sc.getPrefix() + ".ipAddressOverride", o.getValueDisplayString() });
