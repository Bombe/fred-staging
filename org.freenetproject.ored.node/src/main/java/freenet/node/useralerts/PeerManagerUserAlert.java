...
public class PeerManagerUserAlert extends BaseNodeUserAlert {
	...
	/**
	 * How many peers with clock problems can we have without getting alerted about too
	 * many
	 */
	...
	public static final long MAX_OLDEST_NEVER_CONNECTED_PEER_AGE_ALERT_THRESHOLD = DAYS.toMillis(14); // 2
																										// weeks

	public PeerManagerUserAlert(NodeStats n, NodeUpdateManager nodeUpdater) {
		super(false, null, null, null, null, (short) 0, true, NodeL10n.getBase().getString("UserAlert.hide"), false,
				null);
		this.n = n;
		...
		synchronized (this) {
			...
			if (peers == 0 && !isOpennetEnabled) {
				...
				s = l10n("tooHighBwlimitDelayTime", new String[] { "delay", "max" },
						new String[] { Integer.toString(bwlimitDelayTime),
				...
				s = l10n("tooHighPingTime", new String[] { "ping", "max" },
						new String[] { Integer.toString(nodeAveragePingTime),
				...
				s = l10n("connError", "count", Integer.toString(connError));
					...
					&& !darknetAssumeNAT) {
				s = l10n("tooManyDisconnected", new String[] { "count", "max" }, new String[] {
						Integer.toString(disconnDarknetPeers), Integer.toString(MAX_DISCONN_PEER_ALERT_THRESHOLD) });
			}
		...
		synchronized (this) {
			if (isOutdated)
				...
				alertNode.addChild("#", l10n("twoConns"));
					...
					&& (bwlimitDelayTime > NodeStats.MAX_BWLIMIT_DELAY_TIME_ALERT_THRESHOLD)) {
				alertNode.addChild("#",
						l10n("tooHighBwlimitDelayTime", new String[] { "delay", "max" },
								new String[] { Integer.toString(bwlimitDelayTime),
										Long.toString(NodeStats.MAX_BWLIMIT_DELAY_TIME_ALERT_THRESHOLD) }));
			}
			else if (nodeAveragePingAlertRelevant
					&& (nodeAveragePingTime > NodeStats.MAX_NODE_AVERAGE_PING_TIME_ALERT_THRESHOLD)) {
				alertNode.addChild("#",
						l10n("tooHighPingTime", new String[] { "ping", "max" },
								new String[] { Integer.toString(nodeAveragePingTime),
										Long.toString(NodeStats.MAX_NODE_AVERAGE_PING_TIME_ALERT_THRESHOLD) }));
			}
			else if (clockProblem > MIN_CLOCK_PROBLEM_PEER_ALERT_THRESHOLD) {
				...
				alertNode.addChild("#", l10n("connError", "count", Integer.toString(connError)));
					...
					&& !darknetAssumeNAT) {
				alertNode.addChild("#", l10n("tooManyDisconnected", new String[] { "count", "max" }, new String[] {
						Integer.toString(disconnDarknetPeers), Integer.toString(MAX_DISCONN_PEER_ALERT_THRESHOLD) }));
			}
			else if (darknetConns > MAX_DARKNET_CONN_ALERT_THRESHOLD) {
	...
	private boolean calculateIsOutdated() {
		...
		synchronized (this) {
			if (tooNewPeersDarknet >= PeerManager.OUTDATED_MIN_TOO_NEW_DARKNET)
				return true;
			return conns < PeerManager.OUTDATED_MAX_CONNS && tooNewPeersTotal >= PeerManager.OUTDATED_MIN_TOO_NEW_TOTAL;
		}
	}
	...
	public boolean isValid() {
		...
		boolean ret;
		synchronized (this) {
			ret = ((peers == 0 && !isOpennetEnabled) || (conns < 3 && !isOpennetEnabled)
