...
/**
 * Tell the user when there is insufficient disk space for either short term (transient
 * requests, request completion) or long term (starting persistent requests).
 *
 * @author toad
 */
public class DiskSpaceUserAlert implements FCPUserAlert {
	...
	enum Status {
		...
		OK,
		/**
		 * Not enough space to start persistent requests: Space on persistent-temp-* <
		 * long term limit.
		 */
		PERSISTENT,
		/**
		...
		TRANSIENT,
		/**
		 * Not enough space to complete persistent requests: Space on persistent-temp-* <
		 * short term limit.
		 */
	...
	private File getWhere(Status status) {
		// FIXME return the filesystem rather than the directory. Will need java.nio.file
		// (1.7).
