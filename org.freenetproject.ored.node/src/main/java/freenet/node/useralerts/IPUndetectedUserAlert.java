...
public class IPUndetectedUserAlert extends BaseNodeUserAlert {

	public IPUndetectedUserAlert(Node n) {
		super(true, null, null, null, null, (short) 0, true, NodeL10n.getBase().getString("UserAlert.hide"), false,
				null);
		this.node = n;
	}
	...
	public boolean isValid() {
		if (node.isOpennetEnabled())
			return false;
		if (node.peers.countConnectiblePeers() >= 5
				&& (node.getUptime() < MINUTES.toMillis(1) || node.ipDetector.isDetecting()))
			return false;
		return true;
	}
	...
	public HTMLNode getHTMLText() {
		...
		NodeL10n.getBase().addL10nSubstitution(textNode,
				"IPUndetectedUserAlert."
						+ (node.ipDetector.isDetecting() ? "detectingWithConfigLink" : "unknownAddressWithConfigLink"),
				new String[] { "link" }, new HTMLNode[] { HTMLNode.link("/config/" + sc.getPrefix()) });
		...
		if (node.ipDetector.noDetectPlugins()) {
			...
			NodeL10n.getBase().addL10nSubstitution(p, "IPUndetectedUserAlert.loadDetectPlugins",
					new String[] { "plugins", "config", },
		...
		HTMLNode formNode = textNode.addChild("form", new String[] { "action", "method" },
		...
		itemNode.addChild("span", "class", "configshortdesc", o.getLocalisedShortDesc()).addChild("input",
				...
				new String[] { "text", sc.getPrefix() + ".tempIPAddressHint", o.getValueDisplayString() });
	...
	private void addPortForwardSuggestion(HTMLNode textNode) {
		// FIXME we should support any number of ports, UDP or TCP, and pick them up from
		// the node as we do with the forwarding plugin ... that would be a bit of a pain
		// for L10n though ...
