...
/**
 * SSKs require separate logic for inserts and requests, for various reasons: - SSKs can
 * collide. - SSKs have only 1kB of data, so we can pack it into the DataReply, and we
 * don't need to wait for a long data-transfer timeout. - SSKs have pubkeys, which don't
...
public class SSKInsertSender extends BaseSender implements PrioRunnable, AnyInsertSender, ByteCounter {
	...
	SSKInsertSender(SSKBlock block, long uid, InsertTag tag, short htl, PeerNode source, Node node, boolean fromStore,
		...
		// pubKey.fingerprint() is not the same as hash(pubKey.asBytes())). FIXME it
		// should be!
	...
	private void handleNoPubkeyAccepted(PeerNode next, InsertTag thisTag) {
		// FIXME implementing two stage timeout would likely involve forking at this
		// point.
	...
	private MessageFilter makeSearchFilter(PeerNode next, int searchTimeout) {
		 ...
		 * FNPRejectOverload - propagating an overload error :( - FNPDataFound - target
		 * already has the data, and the data is an SVK/SSK/KSK, therefore could be
		 * different to what we are inserting. - FNPDataInsertRejected - the insert was
		 * invalid
		 */
		...
		MessageFilter mfSSKDataFoundHeaders = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(searchTimeout).setType(DMT.FNPSSKDataFoundHeaders);

		return mfRouteNotFound
				.or(mfInsertReply.or(mfRejectedOverload.or(mfDataInsertRejected.or(mfSSKDataFoundHeaders))));
	}
	...
	/**
	 * @return True if fatal and we should try another node, false if just relayed so we
	 * should wait for more responses.
	 */
	...
	private DO handleSSKDataFoundHeaders(Message msg, PeerNode next, InsertTag thisTag) {
		 ...
		 *
		 * We can either accept the old data or the new data. OLD DATA: - KSK-based stuff
		 * is usable. Well, somewhat; a node could spoof KSKs on receiving an insert, (if
		 * it knows them in advance), but it cannot just start inserts to overwrite old
		 * SSKs. - You cannot "update" an SSK. NEW DATA: - KSK-based stuff not usable.
		 * (Some people think this is a good idea!). - Illusion of updatability. (VERY BAD
		 * IMHO, because it's not really updatable... FIXME implement TUKs; would
		 * determine latest version based on version number, and propagate on request with
		 * a certain probability or according to time. However there are good arguments to
		 * do updating at a higher level (e.g. key bottleneck argument), and TUKs should
		 * probably be distinct from SSKs.
		 *
		...
		// Wait for the data
		MessageFilter mfData = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(SSKInsertHandler.DATA_INSERT_TIMEOUT).setType(DMT.FNPSSKDataFoundData);
		Message dataMessage;
		try {
		...
		try {
			...
			// The node will now propagate the new data. There is no need to move to the
			// next node yet.
			return DO.WAIT;
		}
	...
	protected MessageFilter makeAcceptedRejectedFilter(PeerNode next, long acceptedTimeout, UIDTag tag) {
		...
		final long uid = tag.uid;
		/*
		 * Because messages may be re-ordered, it is entirely possible that we get a
		 * non-local RejectedOverload, followed by an Accepted. So we must loop here.
		 */

		MessageFilter mfAccepted = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
				.setTimeout(acceptedTimeout).setType(DMT.FNPSSKAccepted);
	...
	protected void onAccepted(PeerNode next) {
		...
		if (needPubKey) {
			...
			// FIXME doubled the timeout because handling it properly would involve
			// forking.
			MessageFilter mf1 = MessageFilter.create().setSource(next).setField(DMT.UID, uid)
					.setTimeout(ACCEPTED_TIMEOUT * 2).setType(DMT.FNPSSKPubKeyAccepted);
