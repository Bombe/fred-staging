...
public class DefaultThreadDiagnostics implements Runnable, ThreadDiagnostics {
	...
	/**
	 * Calculate the "delta" CPU time for a given thread. This method keeps track of the
	 * previous CPU Time and calculates the difference between that snapshot and the
	 * current CPU Time.
	 *
	 * If there's no previous snapshot of CPU Time for the thread this method will return
	 * 0.
	...
	/**
	 * Gets the job's ID from the thread (PooledExecutor.MyThread) or defaults to the
	 * thread's ID.
	 * @param thread
	...
	private void purgeInactiveThreads(List<NodeThreadInfo> threads) {
		List<Long> activeThreads = threads.stream().map(NodeThreadInfo::getJobId) // job
																					// id
																					// might
																					// be
																					// the
	...
	/**
	 * Get thread (jobs) name at the time of measurement or default to current's thread
	 * name.
	 * @param thread
	...
	public void run() {
		List<NodeThreadInfo> threads = Arrays.stream(nodeStats.getThreads()).filter(Objects::nonNull)
				.filter(thread -> thread.getThreadGroup() != null).filter(thread -> getJobId(thread) != 0)
				.map(thread -> new NodeThreadInfo(thread.getId(), getJobId(thread), getCpuTimeDelta(thread),
						getJobName(thread), thread.getPriority(), thread.getThreadGroup().getName(),
