...
/**
 * Base class for request and insert senders. Mostly concerned with what happens *before
 * and up to* we get the Accepted. After that it hands over to the child class.
 */
public abstract class BaseSender implements ByteCounter {
	...
	/**
	 * The main route requests loop. This must be filled in by the implementation. It will
	 * deal with decrementing the HTL, completing on running out of HTL, RecentlyFailed
	 * for requests, fork on cacheable for inserts, and so on. It will choose a peer and
	 * then chain to innerRouteRequests(), which in turn can call back to routeRequests()
	 * if it needs a new peer.
	 */
	...
	protected void innerRouteRequestsOld(PeerNode next, UIDTag origTag) {
		...
		// Not possible to get an accurate time for sending, guaranteed to be not later
		// than the time of receipt.
		// Why? Because by the time the sent() callback gets called, it may already have
		// been acked, under heavy load.
		...
		try {
			...
			/*
			 * using sendSync could: make ACCEPTED_TIMEOUT more accurate (as it is
			 * measured from the send-time), use a lot of our time that we have to fulfill
			 * this request (simply waiting on the send queue, or longer if the node just
			 * went down), using sendAsync could: make ACCEPTED_TIMEOUT much more likely,
			 * leave many hanging-requests/unclaimedFIFO items, potentially make
			 * overloaded peers MORE overloaded (we make a request and promptly forget
			 * about them).
			 *
			 * Don't use sendAsync().
			 */
	...
	/**
	 * Limit the number of nodes that we route to that reject the request due to looping,
	 * while waiting for a peer. This ensures that if there is a slow node, we don't route
	 * to all the other nodes and DNF, rather than waiting, and possibly timing out, for
	 * the slow node. Note that this does not cause us to stop routing, only to stop
	 * adding more nodes to wait for while waiting. This is particularly an issue if we
	 * have a fast network connected to a slow network.
	 */
	...
	/**
	 * New load management. Give it the peer you want to route to, it might route to a
	 * different one, but it will either get accepted and call onAccepted(), or decide it
	 * needs to reroute and call routeRequests().
	 *
	 * IMPORTANT: Caller must not decrement htl when rerouting if dontDecrementHTLThisTime
	 * is set (but must still reroute); this happens when we discover too that the
	 * proposed peer is no longer suitable. One of the reasons for this is that BaseSender
	 * does not handle RecentlyFailedReturn, which can only happen in the main peer
	 * selection loop. If we set it, we haven't routed to a peer (we might have been
	 * soft-rejected, but that is not serious in NLM).
	 * @param next The peer that we have routed to. We won't necessarily route to this one
	 * in all cases.
	...
	protected void innerRouteRequestsNew(PeerNode next, UIDTag origTag) {
		...
		NodeStats.RequestType type = isSSK ? NodeStats.RequestType.SSK_REQUEST : NodeStats.RequestType.CHK_REQUEST;
		...
		loadWaiterLoop: while (true) {
			...
			expectedAcceptState = next.outputLoadTracker(realTimeFlag).tryRouteTo(origTag,
					PeerNode.RequestLikelyAcceptedState.LIKELY, false);

			if (expectedAcceptState == PeerNode.RequestLikelyAcceptedState.UNKNOWN) {
				// No stats, old style, just go for it.
				// This can happen both when talking to an old node and when we've just
				// connected, but should not be the case for long enough to be a problem.
				if (logMINOR)
				...
				if (expectedAcceptState != null) {
					...
					if (lastNext == next && lastExpectedAcceptState == PeerNode.RequestLikelyAcceptedState.GUARANTEED
							&& (expectedAcceptState == PeerNode.RequestLikelyAcceptedState.GUARANTEED)) {
						// We routed it, thinking it was GUARANTEED.
						// It was rejected, and as far as we know it's still GUARANTEED.
						// :(
						Logger.warning(this,
								"Rejected overload (last time) yet expected state was " + lastExpectedAcceptState
										+ " is now " + expectedAcceptState + " from " + next.shortToString() + " ("
				...
				if (expectedAcceptState == null) {
					...
					if (next != null) {
						if (!waiter.addWaitingFor(next)) {
							...
							return;
							// Will be rerouted.
							// This is essential to avoid adding the same bogus node again
							// and again.
							// This is only an issue with next. Hence the other places we
							// route explicitly so there is no risk as they won't return
							// the same node repeatedly after it is no longer routable.
						}
					}
					...
					if (next.isLowCapacity(realTimeFlag)) {
						if (waiter.waitingForCount() == 1 // if not, already accepted
							...
							// Nodes we were waiting for that then became backed off will
							...
							if (alsoWaitFor != null) {
								...
								// We will not reuse alsoWaitFor if it is disconnected
								// etc.
								if (logMINOR)
									Logger.minor(this, "Waiting for " + next + " and " + alsoWaitFor + " on " + waiter
											+ " because realtime");
								PeerNode matched;
								try {
				...
				if (expectedAcceptState == null && waiter.waitingForCount() <= canWaitFor && canRerouteWhileWaiting) {
					...
					// Nodes we were waiting for that then became backed off will have
				...
				if (expectedAcceptState == null) {
					...
					if (!addedExtraNode) {
						...
						// However after adding it once, we will wait for as long as it
						// takes.
						maxWait = getShortSlotWaiterTimeout();
					}
					HashSet<PeerNode> waitedFor = waiter.waitingForList();
					PeerNode waited;
					// FIXME figure out a way to wake-up mid-wait if
					// origTag.hasSourceRestarted().
					try {
					...
					if (waited == null) {
						...
						if (logMINOR)
							Logger.minor(this, "Sending to " + next + " after waited for "
									+ TimeUtil.formatTime(endTime - startTime) + " realtime=" + realTimeFlag);
						expectedAcceptState = waiter.getAcceptedState();
					}
				...
				// FIXME only report for routing accuracy purposes at this point, not in
				// closerPeer().
			...
			// Not possible to get an accurate time for sending, guaranteed to be not
			// later than the time of receipt.
			// Why? Because by the time the sent() callback gets called, it may already
			// have been acked, under heavy load.
			// So take it from when we first started to try to send the request.
			// See comments below when handling FNPRecentlyFailed for why we need this.
			synchronized (this) {
				timeSentRequest = System.currentTimeMillis();
			...
			try {
				...
				/*
				 * using sendSync could: make ACCEPTED_TIMEOUT more accurate (as it is
				 * measured from the send-time), use a lot of our time that we have to
				 * fulfill this request (simply waiting on the send queue, or longer if
				 * the node just went down), using sendAsync could: make ACCEPTED_TIMEOUT
				 * much more likely, leave many hanging-requests/unclaimedFIFO items,
				 * potentially make overloaded peers MORE overloaded (we make a request
		...
		// This includes the time for the Accepted to come back, so it can take a while
		// sometimes.
	...
	private PeerNode closerPeer(HashSet<PeerNode> exclude, long now, boolean newLoadManagement) {
		return node.peers.closerPeer(sourceForRouting(), exclude, target, true, node.isAdvancedModeEnabled(), -1, null,
				2.0, isInsert() ? null : key, htl, ignoreLowBackoff(), source == null, realTimeFlag, null, false, now,
				newLoadManagement);
	}
	...
	private void logDelta(long delta, int tryCount, boolean waitedForLoadManagement, boolean retriedForLoadManagement) {
		...
		if ((delta > longTimeout) || tryCount > 3)
			Logger.error(this, "Took " + tryCount + " tries in " + TimeUtil.formatTime(delta, 2, true) + " waited="
					+ waitedForLoadManagement + " retried=" + retriedForLoadManagement
					+ (realTimeFlag ? " (realtime)" : " (bulk)") + ((source == null) ? " (local)" : " (remote)"));
		else if ((delta > longTimeout / 5) || tryCount > 1)
			Logger.warning(this, "Took " + tryCount + " tries in " + TimeUtil.formatTime(delta, 2, true) + " waited="
					+ waitedForLoadManagement + " retried=" + retriedForLoadManagement
					+ (realTimeFlag ? " (realtime)" : " (bulk)") + ((source == null) ? " (local)" : " (remote)"));
		else if (logMINOR && (waitedForLoadManagement || retriedForLoadManagement))
			Logger.minor(this, "Took " + tryCount + " tries in " + TimeUtil.formatTime(delta, 2, true) + " waited="
					+ waitedForLoadManagement + " retried=" + retriedForLoadManagement
					+ (realTimeFlag ? " (realtime)" : " (bulk)") + ((source == null) ? " (local)" : " (remote)"));
	...
	private DO waitForAccepted(PeerNode.RequestLikelyAcceptedState expectedAcceptState, PeerNode next, UIDTag origTag) {
		while (true) {
			...
			if (msg.getSpec() == DMT.FNPRejectedOverload) {
				...
				// Could be a previous rejection, the timeout to incur another
				// ACCEPTED_TIMEOUT is minimal...
				continue;
			}
	 ...
	 * @param load The proportion of requests getting timed out, on average, across the
	 * nodes we are waiting for. This is used to decide how long the RecentlyFailed should
	 * be for.
	 */
	...
	protected static enum DO {
		...
		FINISHED, WAIT, NEXT_PEER

	}

	/**
	 * Construct a filter to wait the specified time for accepted or rejected. The actual
	 * messages may vary.
	 * @param next The peer we are waiting for a response from.
	 * @param acceptedTimeout The time to wait.
	 * @param tag Use the UID from this tag. Some requests may change the tag after some
	 * hops, and if e.g. waiting for confirmation after a timeout, we need to use the old
	 * tag.
	 */
