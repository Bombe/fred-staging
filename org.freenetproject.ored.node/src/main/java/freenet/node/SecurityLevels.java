...
/**
 * We have 3 basic security settings. The user chooses these in the first-time wizard, and
 * can reconfigure them at any time. Each impacts on many other config settings, changing
 * their defaults and changing their values when the security level changes, but the user
 * can change those options independantly if they do not change the security level. These
 * options are important, and there are explanations of every option for each setting.
 * They have their own sub-page on the config toadlet. And the security levels are
 * displayed on the homepage as a useralert (instead of the opennet warning).
 *
...
public class SecurityLevels {
	...
	public enum FRIENDS_THREAT_LEVEL {
		...
		HIGH, // Share no/minimal information and take measures to reduce harm if Friends
				// are compromised
	...
	public enum PHYSICAL_THREAT_LEVEL {
		...
		NORMAL, // Encrypt temp files, centralise keys for client cache in master.keys, if
				// that is deleted client cache is unreadable. Later on will include
				// encrypting node.db4o as well, which contains tempfile keys.
		HIGH, // Password master.keys.
		MAXIMUM // Transient encryption for client cache, no persistent downloads support,
				// etc.
	...
	public SecurityLevels(Node node, PersistentConfig config) {
		...
		if (myConfig.getRawOption("networkThreatLevel") != null) {
			...
			// Call all the callbacks so that the config is consistent with the threat
			// level.
	...
	/**
	 * If changing to the new threat level is a potential problem, warn the user, and
	 * include a checkbox for confirmation.
	...
	public HTMLNode getConfirmWarning(NETWORK_THREAT_LEVEL newThreatLevel, String checkboxName) {
		...
		if ((newThreatLevel == NETWORK_THREAT_LEVEL.HIGH && networkThreatLevel != NETWORK_THREAT_LEVEL.MAXIMUM)
			...
			if (node.peers.getDarknetPeers().length == 0) {
				...
				parent.addChild("input", new String[] { "type", "name", "value" },
						new String[] { "checkbox", checkboxName, "off" }, l10n("noFriendsCheckbox"));
				return parent;
			}
				...
				parent.addChild("input", new String[] { "type", "name", "value" },
						new String[] { "checkbox", checkboxName, "off" }, l10n("noConnectedFriendsCheckbox"));
				return parent;
			}
			else if (node.peers.countConnectedDarknetPeers() < 10) {
				parent.addChild("p",
						l10n("fewConnectedFriendsWarning", new String[] { "connected", "added" },
				...
				parent.addChild("input", new String[] { "type", "name", "value" },
						new String[] { "checkbox", checkboxName, "off" }, l10n("fewConnectedFriendsCheckbox"));
				return parent;
			}
		}
			...
			parent.addChild("input", new String[] { "type", "name", "value" },
					new String[] { "checkbox", checkboxName, "off" }, l10n("networkThreatLevelLowCheckbox"));
			return parent;
		...
		if (newThreatLevel == NETWORK_THREAT_LEVEL.MAXIMUM) {
			...
			NodeL10n.getBase().addL10nSubstitution(p, "SecurityLevels.maxSecurityYouNeedFriends",
					new String[] { "bold" }, new HTMLNode[] { HTMLNode.STRONG });
			parent.addChild("input", new String[] { "type", "name", "value" },
					new String[] { "checkbox", checkboxName, "off" }, l10n("maximumNetworkThreatLevelCheckbox"));
			return parent;
		}
		return null;
	}
