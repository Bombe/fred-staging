...
/**
 * Combine the detected IP address with the NodeCrypto's port number and the port numbers
 * we have on connections in the given class to get a list of Peer's.
 *
 * @author toad
 */
public class NodeIPPortDetector {
	...
	/**
	 * Combine the NodeIPDetector's output with any per-port information we may have to
	 * get a definitive (for that port/NodeCrypto) list of IP addresses (still without
	 * port numbers).
	 */
	FreenetInetAddress[] detectPrimaryIPAddress() {
		...
		if (addr.isRealInternetAddress(false, true, false)) {
			// Binding to a real internet address => don't want us to use the others, most
			// likely
	...
	/**
	 * Get our Peer's. This is a list of IP:port's at which we might be contactable. Some
	 * of them will have the same port as the listenPort, but if we are behind a NAT which
	 * rewrites our port number, some of them may not. (If we're behind a symmetric NAT
	 * which rewrites it differently for each connection, we're stuffed, and we tell the
	 * user).
	 */
