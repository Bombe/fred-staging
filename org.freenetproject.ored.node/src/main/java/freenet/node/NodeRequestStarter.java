...
/**
 * Starts requests. Nobody starts a request directly, you have to go through
 * RequestStarter. And you have to provide a RequestStarterClient. We do round robin
 * between clients on the same priority level.
 */
public class NodeRequestStarter implements Runnable, RandomGrabArrayItemExclusionList, RequestStarter {
	...
	public NodeRequestStarter(NodeClientCore node, BaseRequestThrottle throttle, String name,
			RunningAverage averageOutputBytesPerRequest, RunningAverage averageInputBytesPerRequest, boolean isInsert,
			boolean isSSK, boolean realTime) {
		this.core = node;
	...
	void realRun() {
		...
		while (true) {
			...
			if (req != null) {
				...
				// // Firstly, they use threads. This could be a serious problem for
				// faster nodes.
				...
				// Always take the lock on RequestStarter first. AFAICS we don't
				// synchronize on RequestStarter anywhere else.
				// Nested locks here prevent extra latency when there is a race, and
				// therefore allow us to sleep indefinitely
				synchronized (this) {
					req = sched.grabRequest();
					if (req == null) {
						try {
	...
	/**
	 * LOCKING: Caller must avoid locking while calling this function. In particular, if
	 * the RequestStarter lock is held we will get a deadlock.
	 */
	public void wakeUp() {
		synchronized (this) {
			notifyAll();
