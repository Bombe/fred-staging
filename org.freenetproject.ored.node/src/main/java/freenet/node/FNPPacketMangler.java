 ...
 * @author amphibian
 *
 * Handles connection setup and more complex packet decoding (cases where we don't
 * immediately know which peer sent the packet). Connection setup uses JFKi, but with an
 * outer obfuscation layer keyed on the "identity" of both the peer and this node.
...
public class FNPPacketMangler implements OutgoingPacketMangler {
	...
	/**
	 * Objects cached during JFK message exchange: JFK(3,4) with authenticator as key The
	 * messages are cached in hashmaps because the message retrieval from the cache can be
	 * performed in constant time( given the key)
	 */
	...
	/*
	 * The FIFO itself Get a lock on dhContextFIFO before touching it!
	 */
	...
	/**
	 * The max amount of time we will accept to use the current tracker when it should
	 * have been replaced
	 */
	...
	/**
	 * Start up the FNPPacketMangler. By the time this is called, all objects will have
	 * been constructed, but not all will have been started yet.
	 */
	public void start() {
	...
	/**
	 * Decrypt and authenticate packet. Then feed it to USM.checkFilters. Packets
	 * generated should have a PeerNode on them. Note that the buffer can be modified by
	 * this method.
	 */
	public DECODED process(byte[] buf, int offset, int length, Peer peer, PeerNode opn, long now) {
		...
		if (wantAnonAuth && !wantAnonAuthChangeIP) {
			if (checkAnonAuthChangeIP(opn, buf, offset, length, peer, now)) {
				// This can happen when a node is upgraded from a SeedClientPeerNode to an
				// OpennetPeerNode.
				// Logger.error(this, "Last resort match anon-auth against all anon setup
				// peernodes succeeded - this should not happen! (It can happen if they
				// change address)");
				return DECODED.DECODED;
			}
		}
	...
	/**
	 * Might be an anonymous-initiator negotiation packet (i.e. we are the responder).
	 * Anonymous initiator is used for seednode connections, and will in future be used
	 * for other things for example one-side-only invites, password based invites etc.
	 * @param buf The buffer to read bytes from
	 * @param offset The offset at which to start reading
	 * @param length The number of bytes to read
	 * @param peer The Peer to send a reply to
	...
	/**
	 * Might be a reply to an anonymous-initiator negotiation packet (i.e. we are the
	 * initiator). Anonymous initiator is used for seednode connections, and will in
	 * future be used for other things for example one-side-only invites, password based
	 * invites etc.
	 * @param buf The buffer to read bytes from
	 * @param offset The offset at which to start reading
	 * @param length The number of bytes to read
	 ...
	 * processDecryptedAuth()), we know the node that is trying to contact us. But in this
	 * case, we don't know the node yet, and we are doing a special-purpose connection
	 * setup. At the moment the only type supported is for a new node connecting to a
	 * seednode in order to announce. In future, nodes may support other
	 * anonymous-initiator connection types such as when a node (which is certain of its
	 * connectivity) issues one-time invites which allow a new node to connect to it.
	...
	private void processDecryptedAuthAnon(final byte[] payload, final Peer replyTo) {
		...
		/**
		 * Negotiation type. Common to anonymous-initiator auth and normal setup. 2 = JFK.
		 * 3 = JFK, reuse PacketTracker Other types might indicate other DH variants, or
		 * even non-DH-based algorithms such as password based key setup.
		 */
		 ...
		 * purpose of the connection. At the moment it is SETUP_OPENNET_SEEDNODE to
		 * indicate we are connecting to a seednode (which doesn't know us). Invites might
		 * require a different setupType.
		 */
		...
		if (packetType == 0 || packetType == 2) {
			...
			Logger.error(this,
					"Invalid phase " + packetType + " for anonymous-initiator (we are the responder) from " + replyTo);
		}
	...
	private void processDecryptedAuthAnonReply(final byte[] payload, final Peer replyTo, final PeerNode pn) {
		if (logMINOR)
			Logger.minor(this,
					"Processing decrypted auth packet from " + replyTo + " for " + pn + " length " + payload.length);

		/** Protocol version. Should be 1. */
		final int version = payload[0];
		/**
		 * Negotiation type. 2 = JFK. 3 = JFK, reuse PacketTracker Other types might
		 * indicate other DH variants, or even non-DH-based algorithms such as password
		 * based key setup.
		 */
		final int negType = payload[1];
		/** Packet phase. */
		...
		if (packetType == 1 || packetType == 3) {
			...
			Logger.error(this,
					"Invalid phase " + packetType + " for anonymous-initiator (we are the initiator) from " + replyTo);
		}
	...
	private void processDecryptedAuth(final byte[] payload, final PeerNode pn, final Peer replyTo,
		...
		/*
		 * Format: 1 byte - version number (1) 1 byte - negotiation type (0 = simple DH,
		 * will not be supported when implement JFKi || 1 = StS) 1 byte - packet type
		 * (0-3)
		 */
		if (version != 1) {
			Logger.error(this, "Decrypted auth packet but invalid version: " + version);
		...
		if (negType >= 0 && negType < 10) {
			...
			// negType == 10 => Changes the method of ack encoding (from single-ack to
			// cummulative range acks)
			...
			// - use the hash of the pubkey as an identity instead of just random
			// (forgeable) data
			// negType == 8 => use ECDH with P256 instead of DH1024
			// negType == 7 => same as 6, but determine the initial sequence number by
			// hashing the identity
			// instead of negotiating it
			/*
			 * We implement Just Fast Keying key management protocol with active identity
			 * protection for the initiator and no identity protection for the responder
			 * M1: This is a straightforward DiffieHellman exponential. The Initiator
			 * Nonce serves two purposes;it allows the initiator to use the same
			 ...
			 * sessions M2: Responder replies with a signed copy of his own exponential, a
			 * random nonce and an authenticator which provides sufficient defense against
			 * forgeries,replays We slightly deviate JFK here;we do not send any public
			 * key information as specified in the JFK docs M3: Initiator echoes the data
			 * sent by the responder including the authenticator. This helps the responder
			 * verify the authenticity of the returned data. M4: Encrypted message of the
			 * signature on both nonces, both exponentials using the same keys as in the
			 * previous message
			 */
				...
				authHandlingThread.execute(new Runnable() {
					...
					public void run() {
						if (packetType == 0) {
							 ...
							 * exponential. The Initiator Nonce serves two purposes;it
							 * allows the initiator to use the same exponentials during
							 ...
							 * exponential, a random nonce and an authenticator calculated
							 * from a transient hash key private to the responder.
							 */
							 ...
							 * messages are cached by the Responder.Receiving a duplicate
							 * message simply causes the responder to Re-transmit the
							 ...
							 * exponentials using the same keys as in the previous
							 * message. The signature is non-message recovering
							 */
	...
	/*
	 * Initiator Method:Message1 Process Message1 Receive the Initiator nonce and
	 * DiffieHellman Exponential
	 *
	 * @param The packet phase number
	 *
	 * @param The peerNode we are talking to. CAN BE NULL if anonymous initiator, since we
	 * are the responder.
	 *
	 * @param The peer to which we need to send the packet
	 *
	 * @param unknownInitiator If true, we (the responder) don't know the initiator, and
	 * should check for fields which would be skipped in a normal setup where both sides
	 * know the other (indicated with * below).
	 *
	 * @param setupType The type of unknown-initiator setup.
	 *
	 * format : Ni' g^i *IDr'
	 *
	 * See http://www.wisdom.weizmann.ac.il/~reingold/publications/jfk-tissec.pdf Just
	 * Fast Keying: Key Agreement In A Hostile Internet Aiello, Bellovin, Blaze, Canetti,
	 * Ioannidis, Keromytis, Reingold. ACM Transactions on Information and System
	 * Security, Vol 7 No 2, May 2004, Pages 1-30.
	 *
	 */
	private void processJFKMessage1(byte[] payload, int offset, PeerNode pn, Peer replyTo, boolean unknownInitiator,
		...
		if (payload.length < nonceSizeHashed + modulusLength + 3
		...
		if (unknownInitiator) {
			...
			if (!MessageDigest.isEqual(expectedIdentityHash, crypto.identityHash)) {
				Logger.error(this,
						"Invalid unknown-initiator JFK(1), IDr' is " + HexUtil.bytesToHex(expectedIdentityHash)
								+ " should be " + HexUtil.bytesToHex(crypto.identityHash));
				return;
			}
		}
	...
	private void handleNoContextsException(NoContextsException e,
		...
		// Log it immediately.
		Logger.warning(this,
				"No contexts available " + context + " - running out of entropy or severe CPU usage problems?");
	...
	private void logLoudErrorNoContexts() {
		// If this is happening regularly post-startup then it's unlikely that reading the
		// disk will help.
		// FIXME localise this, give a useralert etc.
		// RNG exhaustion shouldn't happen for Windows users at all, and may not happen on
		// Linux depending on the JVM version, so lets leave it for now.
		System.err.println(
	...
	private boolean throttleRekey(PeerNode pn, Peer replyTo) {
		...
		synchronized (throttleRekeysByIP) {
			...
			while (throttleRekeysByIP.size() > REKEY_BY_IP_TABLE_SIZE || ((!throttleRekeysByIP.isEmpty())
	...
	/*
	 * format: Ni',g^i We send IDr' only if unknownInitiator is set.
	 *
	 * @param pn The node to encrypt the message to. Cannot be null, because we are the
	 * initiator and we know the responder in all cases.
	 *
	...
	private void sendJFKMessage1(PeerNode pn, Peer replyTo, boolean unknownInitiator, int setupType, int negType)
		...
		int nonceSizeHashed = HASH_LENGTH;
		byte[] message1 = new byte[nonceSizeHashed + modulusLength
				+ (unknownInitiator ? NodeCrypto.IDENTITY_LENGTH : 0)];
	...
	/*
	 * format: Ni',Nr,g^r Signature[g^r,grpInfo(r)] Hashed JFKAuthenticator :
	 * HMAC{Hkr}[g^r, g^i, Nr, Ni', IPi]
	 *
	 * NB: we don't send IDr nor groupinfo as we know them: even if the responder doesn't
	 * know the initiator, the initiator ALWAYS knows the responder.
	 *
	...
	/*
	 * Assemble what will be the jfk-Authenticator : computed over the Responder
	 * exponentials and the Nonces and used by the responder to verify that the round-trip
	 * has been done
	 *
	 */
	 ...
	 *
	 * @see{sendJFKMessage2} for packet format details. Note that this packet is exactly
	 * the same for known initiator as for unknown initiator.
	 *
	...
	private void processJFKMessage2(byte[] payload, int inputOffset, PeerNode pn, Peer replyTo,
		...
		if (myNi == null) {
			if (shouldLogErrorInHandshake(t1)) {
				Logger.normal(this,
						"We received an unexpected JFK(2) message from " + pn.getPeer() + " (time since added: "
								+ pn.timeSinceAddedOrRestarted() + " time last receive:" + pn.lastReceivedPacketTime()
								+ ')');
			}
			return;
		}
		...
		if (!ECDSA.verify(Curves.P256, pn.peerECDSAPubKey, sig, hisExponential)) {
			...
			if (logDEBUG)
				Logger.debug(this, "Expected signature on " + HexUtil.bytesToHex(hisExponential) + " with "
						+ HexUtil.bytesToHex(pn.peerECDSAPubKeyHash) + " signature " + HexUtil.bytesToHex(sig));
			return;
		}
	...
	/*
	 * Initiator Method:Message3 Process Message3 Send the Initiator nonce,Responder nonce
	 * and DiffieHellman Exponential of the responder and initiator in the
	 * clear.(unVerifiedData) Send the authenticator which allows the responder to verify
	 * that a roundtrip occured Compute the signature of the unVerifiedData and encrypt it
	 * using a shared key which is derived from DHExponentials and the nonces; add a HMAC
	 * to protect it
	 *
	 * Format: Ni, Nr, g^i, g^r Authenticator - HMAC{Hkr}[g^r, g^i, Nr, Ni', IPi]
	 * HMAC{Ka}(cyphertext) IV + E{KE}[S{i}[Ni',Nr,g^i,g^r,idR, bootID, znoderefI],
	 * bootID, znoderefI*]
	 *
	 * * Noderef is sent whether or not unknownInitiator is true, however if it is, it
	 * will be a *full* noderef, otherwise it will exclude the pubkey etc.
	 *
	 * @param payload The buffer containing the decrypted auth packet.
	 *
	 * @param replyTo The peer to which we need to send the packet.
	 *
	 * @param pn The PeerNode we are talking to. CAN BE NULL in the case of anonymous
	...
	private void processJFKMessage3(byte[] payload, int inputOffset, PeerNode pn, Peer replyTo, boolean oldOpennetPeer,
		...
		final int expectedLength = nonceSize * 2 + // Ni, Nr
		...
		if (!HMAC
				.verifyWithSHA256(
						getTransientKey(), assembleJFKAuthenticator(responderExponential, initiatorExponential,
								nonceResponder, nonceInitiatorHashed, replyTo.getAddress().getAddress()),
						authenticator)) {
			if (shouldLogErrorInHandshake(t1)) {
				if (logDEBUG)
		...
		if (message4 != null) {
			...
			if (logDEBUG)
				Logger.debug(this, "No message4 found for " + HexUtil.bytesToHex(authenticator)
						...
						+ Fields.hashCode(initiatorExponential) + " nonceResponder " + Fields.hashCode(nonceResponder)
						+ " nonceInitiator " + Fields.hashCode(nonceInitiatorHashed) + " address "
		...
		if (logDEBUG)
			Logger.debug(this,
					"The shared Master secret is : " + HexUtil.bytesToHex(computedExponential) + " for " + pn);
		...
		/*
		 * Bytes 1-4: Initial sequence number for the initiator Bytes 5-8: Initial
		 * sequence number for the responder Bytes 9-12: Initial message id for the
		 * initiator Bytes 13-16: Initial message id for the responder Note that we are
		 * the responder
		 */
		...
		int theirInitialSeqNum = ((sharedData[0] & 0xFF) << 24) | ((sharedData[1] & 0xFF) << 16)
				| ((sharedData[2] & 0xFF) << 8) | (sharedData[3] & 0xFF);
		int ourInitialSeqNum = ((sharedData[4] & 0xFF) << 24) | ((sharedData[5] & 0xFF) << 16)
				| ((sharedData[6] & 0xFF) << 8) | (sharedData[7] & 0xFF);
		int theirInitialMsgID, ourInitialMsgID;

		theirInitialMsgID = unknownInitiator ? getInitialMessageID(crypto.myIdentity)
				: getInitialMessageID(pn.identity, crypto.myIdentity);
		ourInitialMsgID = unknownInitiator ? getInitialMessageID(crypto.myIdentity)
		...
		pk.blockDecipher(decypheredPayload, decypheredPayloadOffset,
				decypheredPayload.length - decypheredPayloadOffset);
		/*
		 * DecipheredData Format: Signature Node Data (starting with BootID)
		 */
		int sigLength = getSignatureLength(negType);
		byte[] sig = new byte[sigLength];
		...
		if (oldOpennetPeer && pn instanceof OpennetPeerNode /* true */) {
			...
			/*
			 * When an old-opennet-peer connects, add it at the top of the LRU, so that it
			 * isn't immediately dropped when there is no droppable peer to drop. If it
			 * was dropped from the bottom of the LRU list, we would not have added it to
			 * the LRU; so it was somewhere in the middle.
			 */
			if (!opennet.wantPeer(opn, false, false, true, OpennetManager.ConnectionType.RECONNECT)) {
		...
		if ((!dontWant) && !crypto.allowConnection(pn, replyTo.getFreenetAddress())) {
			if (pn instanceof DarknetPeerNode) {
				Logger.error(this,
						"Dropping peer " + pn + " because don't want connection due to others on the same IP address!");
		...
		long newTrackerID = pn.completedHandshake(bootID, hisRef, 0, hisRef.length, outgoingCipher, outgoingKey,
				incommingCipher, incommingKey, replyTo, true, negType, trackerID, false, false, hmacKey, ivCipher,
		...
		if (newTrackerID > 0) {
			...
			sendJFKMessage4(1, negType, 3, nonceInitiatorHashed, nonceResponder, initiatorExponential,
					responderExponential, c, Ke, Ka, authenticator, hisRef, pn, replyTo, unknownInitiator, setupType,
					newTrackerID, newTrackerID == trackerID);

			if (dontWant) {
				node.peers.disconnectAndRemove(pn, true, true, true); // Let it connect
																		// then tell it to
																		// remove it.
			}
			else {
		...
		if ((t2 - t1) > 500) {
			Logger.error(this, "Message3 Processing packet for " + pn.getPeer() + " took "
	...
	/*
	 * Responder Method:Message4 Process Message4
	 *
	 * Format: HMAC{Ka}[cyphertext] IV + E{Ke}[S{R}[Ni', Nr, g^i, g^r, IDi, bootID,
	 * znoderefR, znoderefI], bootID, znoderefR]
	 *
	...
	private boolean processJFKMessage4(byte[] payload, int inputOffset, PeerNode pn, Peer replyTo,
		...
		// If it is already present it indicates duplicate/replayed message4 and we can
		// discard
		...
		if (message4Timestamp != null) {
			Logger.normal(this, "We got a replayed message4 (first handled at "
					+ TimeUtil.formatTime(t1 - Fields.bytesToLong(message4Timestamp)) + ") from - " + pn);
			return true;
		}
		...
		pk.blockDecipher(decypheredPayload, decypheredPayloadOffset,
				decypheredPayload.length - decypheredPayloadOffset);
		/*
		 * DecipheredData Format: Signature-r,s bootID, znoderef
		 */
		...
		byte[] locallyGeneratedText = new byte[nonceSizeHashed + nonceSize + modulusLength * 2 + identity.length
				+ dataLen + pn.jfkMyRef.length];
		...
		if (!ECDSA.verify(Curves.P256, pn.peerECDSAPubKey, sig, locallyGeneratedText)) {
			Logger.error(this, "The ECDSA signature verification has failed!! JFK(4) - " + pn.getPeer() + " length "
					+ locallyGeneratedText.length + " hisRef " + hisRef.length + " hash " + Fields.hashCode(hisRef)
					+ " myRef " + pn.jfkMyRef.length + " hash " + Fields.hashCode(pn.jfkMyRef) + " boot ID " + bootID);
			return true;
		}
		...
		long newTrackerID = pn.completedHandshake(bootID, hisRef, 0, hisRef.length, outgoingCipher, pn.outgoingKey,
				incommingCipher, pn.incommingKey, replyTo, false, negType, trackerID, true, reusedTracker, pn.hmacKey,
	 ...
	 * bootID, znoderefI]
	 *
	 * @param pn The PeerNode to encrypt the message for. Cannot be null as we are the
	 * initiator.
	 *
	 * @param replyTo The Peer to send the packet to.
	 */
	...
	private void sendJFKMessage3(int version, final int negType, int phase, byte[] nonceInitiator,
			byte[] nonceResponder, byte[] hisExponential, byte[] authenticator, final PeerNode pn, final Peer replyTo,
			final boolean unknownInitiator, final int setupType) {
		if (logMINOR)
		...
		offset += HASH_LENGTH;
		/*
		 * Digital Signature of the message with the private key belonging to the
		 * initiator/responder It is assumed to be non-message recovering
		 */
		// save parameters so that we can verify message4
		byte[] toSign = assembleDHParams(nonceInitiatorHashed, nonceResponder, ourExponential, hisExponential,
				pn.getPubKeyHash(), data);
		pn.setJFKBuffer(toSign);
		byte[] sig = crypto.ecdsaSign(toSign);

		byte[] computedExponential = ((ECDHLightContext) ctx)
				.getHMACKey(ECDH.getPublicKey(hisExponential, ecdhCurveToUse));
		...
		byte[] sharedData = computeJFKSharedKey(computedExponential, nonceInitiatorHashed, nonceResponder, "6");
		Arrays.fill(computedExponential, (byte) 0);
		pn.ourInitialSeqNum = ((sharedData[0] & 0xFF) << 24) | ((sharedData[1] & 0xFF) << 16)
				| ((sharedData[2] & 0xFF) << 8) | (sharedData[3] & 0xFF);
		pn.theirInitialSeqNum = ((sharedData[4] & 0xFF) << 24) | ((sharedData[5] & 0xFF) << 16)
				| ((sharedData[6] & 0xFF) << 8) | (sharedData[7] & 0xFF);

		pn.theirInitialMsgID = unknownInitiator ? getInitialMessageID(pn.identity)
				: getInitialMessageID(pn.identity, crypto.myIdentity);
		pn.ourInitialMsgID = unknownInitiator ? getInitialMessageID(pn.identity)
				: getInitialMessageID(crypto.myIdentity, pn.identity);
	...
	/*
	 * Format: HMAC{Ka}(cyphertext) IV, E{Ke}[S{R}[Ni',Nr,g^i,g^r,idI, bootID, znoderefR,
	 * znoderefI],bootID,znoderefR]
	 *
	 * @param replyTo The Peer we are replying to.
	 *
	 * @param pn The PeerNode to encrypt the auth packet to. Cannot be null, because even
	 * in anonymous initiator, we will have created one before calling this method.
	 */
	private void sendJFKMessage4(int version, int negType, int phase, byte[] nonceInitiatorHashed,
			byte[] nonceResponder, byte[] initiatorExponential, byte[] responderExponential, BlockCipher c, byte[] Ke,
			byte[] Ka, byte[] authenticator, byte[] hisRef, PeerNode pn, Peer replyTo, boolean unknownInitiator,
			int setupType, long newTrackerID, boolean sameAsOldTrackerID) {
		if (logMINOR)
		...
		if (logMINOR)
			Logger.minor(this,
					"Message length " + params.length + " myRef: " + myRef.length + " hash " + Fields.hashCode(myRef)
							+ " hisRef: " + hisRef.length + " hash " + Fields.hashCode(hisRef) + " boot ID "
							+ node.bootID);
	...
	private void sendAuthPacket(int version, int negType, int phase, byte[] data, PeerNode pn, Peer replyTo) {
		...
		if (logMINOR) {
			...
			delta = TimeUtil.formatTime(now - last, 2, true) + " ago";
			Logger.minor(this,
					"Sending auth packet for " + String.valueOf(pn.getPeer()) + " (phase=" + phase + ", ver=" + version
							+ ", nt=" + negType + ") (last packet sent " + delta + ") to " + replyTo + " data.length="
	 ...
	 * @param cipher
	 */
	private void sendAnonAuthPacket(int version, int negType, int phase, int setupType, byte[] data, PeerNode pn,
			Peer replyTo, BlockCipher cipher) {
	...
	/**
	 * Send an auth packet (we have constructed the payload, now hash it, pad it, encrypt
	 * it).
	 */
	private void sendAuthPacket(byte[] output, BlockCipher cipher, PeerNode pn, Peer replyTo, boolean anonAuth) {
		...
		if (prePaddingLength < maxPacketSize) {
			...
			paddingLength = 0; // Avoid oversize packets if at all possible, the MTU is an
								// estimate and may be wrong, and fragmented packets are
	...
	/**
	 * Should we log an error for an event that could easily be caused by a handshake
	...
	private ECDHLightContext _genECDHLightContext() {
		...
		if (logDEBUG)
			Logger.debug(this, "ECDSA Signature: " + HexUtil.bytesToHex(ctx.ecdsaSig) + " for "
	...
	/**
	 * Used in processJFK[3|4] That's O^(n) ... but we have only a few elements and we
	 * call it only once a round-trip has been done
	 * @param exponential
	...
	/*
	 * Prepare DH parameters of message2 for them to be signed (useful in message3 to
	 * check the sig)
	 */
	private byte[] assembleDHParams(byte[] nonceInitiator, byte[] nonceResponder, byte[] initiatorExponential,
			byte[] responderExponential, byte[] id, byte[] sa) {
	...
	private byte[] computeJFKSharedKey(byte[] exponential, byte[] nI, byte[] nR, String what) {
		assert ("0".equals(what) || "1".equals(what) || "2".equals(what) || "3".equals(what) || "4".equals(what)
				|| "5".equals(what) || "6".equals(what) || "7".equals(what));
		byte[] number = null;
		try {
	...
	/**
	 * How big can the authenticator cache get before we flush it ? n * 40 bytes (32 for
	 * the authenticator and 8 for the timestamp)
	 *
