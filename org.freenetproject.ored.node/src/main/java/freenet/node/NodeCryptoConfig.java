...
/**
 * Tracks config parameters related to a NodeCrypto. The NodeCrypto may or may not exist.
 * If it exists, parameter changes are passed on to it, if it doesn't, they can be changed
 * trivially.
 *
 * Allows users to set the opennet port number while opennet is disabled, enable opennet
 * on the fly etc.
 *
 * @author toad
 */
public class NodeCryptoConfig {
	...
	/**
	 * If nonzero, 1/dropProbability = probability of UdpSocketHandler dropping a packet
	 * (for debugging purposes; not static as we may need to simulate some nodes with more
	 * loss than others).
	 */
	...
	/**
	 * Whether we should prevent multiple connections to the same IP (taking into account
	 * other NodeCrypto's - this will usually be set for opennet but not for darknet).
	 */
	...
	NodeCryptoConfig(SubConfig config, int sortOrder, boolean isOpennet, SecurityLevels securityLevels)
			throws NodeInitException {
		config.register("listenPort", -1 /* means random */, sortOrder++, true, true,
				isOpennet ? "Node.opennetPort" : "Node.port", isOpennet ? "Node.opennetPortLong" : "Node.portLong",
				new IntCallback() {
					@Override
					public Integer get() {
					...
					public void set(Integer val) throws InvalidConfigValueException {
						...
						synchronized (NodeCryptoConfig.class) {
							...
							// Note that this sort of thing should be the exception rather
							// than the rule!!!!
							if (crypto != null)
		...
		config.register("testingDropPacketsEvery", 0, sortOrder++, true, false, "Node.dropPacketEvery",
				"Node.dropPacketEveryLong", new IntCallback() {
		...
		config.register("oneConnectionPerIP", isOpennet, sortOrder++, true, false,
				...
				(isOpennet ? "OpennetManager" : "Node") + ".oneConnectionPerIPLong", new BooleanCallback() {
		...
		if (isOpennet) {
			securityLevels.addNetworkThreatLevelListener(new SecurityLevelListener<NETWORK_THREAT_LEVEL>() {
				...
				public void onChange(NETWORK_THREAT_LEVEL oldLevel, NETWORK_THREAT_LEVEL newLevel) {
					// Might be useful for nodes on the same NAT etc, so turn it off for
					// LOW. Otherwise is sensible.
					// It's always off on darknet, since we can reasonably expect to know
					// our peers, even if we are paranoid
					// about them!
		...
		config.register("alwaysAllowLocalAddresses", !isOpennet, sortOrder++, true, false,
				"Node.alwaysAllowLocalAddresses", "Node.alwaysAllowLocalAddressesLong", new BooleanCallback() {
		...
		config.register("assumeNATed", true, sortOrder++, true, true, "Node.assumeNATed", "Node.assumeNATedLong",
				new BooleanCallback() {
		...
		securityLevels.addNetworkThreatLevelListener(new SecurityLevelListener<NETWORK_THREAT_LEVEL>() {
			...
			@Override
			public void onChange(NETWORK_THREAT_LEVEL oldLevel, NETWORK_THREAT_LEVEL newLevel) {
				// Might be useful for nodes which are running with a tight bandwidth
				// quota to minimize the overhead,
	...
	class NodeBindtoCallback extends StringCallback {
		...
		public void set(String val) throws InvalidConfigValueException {
			if (val.equals(get()))
				return;
			// FIXME why not? Can't we use freenet.io.NetworkInterface like everywhere
			// else, just adapt it for UDP?
