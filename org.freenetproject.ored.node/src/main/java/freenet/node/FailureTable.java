...
/**
 * Tracks recently DNFed keys, where they were routed to, what the location was at the
 * time, who requested them. Implements Ultra-Lightweight Persistent Requests: Refuse
 * requests for a key for 10 minutes after it's DNFed (UNLESS we find a better route for
 * the request), and when it is found, offer it to those who've asked for it in the last
 * hour. LOCKING: Do not lock PeerNode before FailureTable/FailureTableEntry.
 *
 * @author toad
 */
public class FailureTable {
	...
	/**
	 * BlockOfferList by key. Synchronized on self, as it doesn't interact with the main
	 * FT.
	 */
	 ...
	 * Maximum time for any FailureTable i.e. for this period after a DNF, we will avoid
	 * the node that DNFed.
	 */
	static final long REJECT_TIME = MINUTES.toMillis(10);

	/**
	 * Maximum time for a RecentlyFailed. I.e. until this period expires, we take a
	 * request into account when deciding whether we have recently failed to this peer. If
	 * we get a DNF, we use this figure. If we get a RF, we use what it tells us, which
	 * can be less than this. Most other failures use shorter periods.
	 */
	...
	/**
	 * Called when we route to a node and it fails for some reason, but we continue the
	 * request. Normally the timeout will be the time it took to route to that node and
	 * wait for its response / timeout waiting for its response.
	 * @param key
	 * @param routedTo
	 * @param htl
	 * @param rfTimeout
	...
	public void onFailed(Key key, PeerNode routedTo, short htl, long rfTimeout, long ftTimeout) {
		...
		synchronized (this) {
			...
			// We have to do this inside the lock to prevent race condition with the
			// cleaner causing us to get dropped because isEmpty() before updating.
	...
	/**
	 * When a request finishes with a failure, record who generated the failure so we
	 * don't route to them next time, and also who originated it so we can send the data
	 * back to them if we find them. ORDERING: You should generally call this *before*
	 * calling finish() to avoid problems. LOCKING: NEVER synchronize on PeerNode before
	 * calling any FailureTable method.
	 */
	public void onFinalFailure(Key key, PeerNode routedTo, short htl, short origHTL, long rfTimeout, long ftTimeout,
			PeerNode requestor) {
	...
	// LOCKING: Synchronized on FailureTable because we need to remove self in
	// deleteOffer().
	...
	/**
	 * Called when a data block is found (after it has been stored; there is a good chance
	 * of its being available in the near future). If there are nodes waiting for it, we
	 * will offer it to them. Removes the list of nodes that offered the key too (but this
	 * is a separate operation). LOCKING: Never call when locked PeerNode, and try to
	 * avoid other locks as they might cause a deadlock. Schedule off-thread if necessary.
	 */
	...
	/**
	 * Run onOffer() on a separate thread since it can block for disk I/O, and we don't
	 * want to cause transfer timeouts etc because of slow disk.
	 */
	private final SerialExecutor offerExecutor;

	/**
	 * Called when we get an offer for a key. If this is an SSK, we will only accept it if
	 * we have previously asked for it. If it is a CHK, we will accept it if we want it.
	...
	/**
	 * This method runs on the SerialExecutor. Therefore, any blocking network I/O needs
	 * to be scheduled on a separate thread. However, blocking disk I/O *should happen on
	 * this thread*. We deliberately serialise it, as high latencies can otherwise result.
	 */
	protected void innerOnOffer(Key key, PeerNode peer, byte[] authenticator) {
		...
		/*
		 * Accept (subject to later checks) if we asked for it. Should we accept it if we
		 * were asked for it? This is "bidirectional propagation". It's good because it
		 * makes the whole structure much more reliable; it's bad because it's not
		 * entirely under our control - we didn't choose to route it to the node, the node
		 * routed it to us. Now it's found it before we did...
		 *
		 * Attacks: - Frost spamming etc: Is it easier to offer data to our peers rather
		 * than inserting it? Will it result in it being propagated further? The peer node
		 * would then do the request, rather than this node doing an insert. Is that
		 * beneficial?
		 *
		 * Not relevant with CHKs anyway.
		 *
		 * On the plus side, propagation to nodes that have asked is worthwhile because
		 * reduced polling cost enables more secure messaging systems e.g. outbox
		 * polling... - Social engineering: If a key is unpopular, you can put a different
		 * copy of it on different nodes. You can then use this to trace the requestor -
		 * identify that he is or isn't on the target. You can't do this with a regular
		 * insert because it will often go several nodes even at htl 0. With
		 * subscriptions, you might be able to bypass this - but only if you know no other
		 * nodes in the neighbourhood are subscribed. Easier with SSKs; with CHKs you have
		 * only binary information of whether the person got the key (with social
		 * engineering). Hard to exploit on darknet; if you're that close to the suspect
		 * there are easier ways to get at them e.g. correlation attacks.
		 *
		 * Conclusion: We should accept the request if: - We asked for it from that node.
		 * (Note that a node might both have asked us and been asked). - That node asked
		 * for it, and it's a CHK.
		 */
	...
	/**
	 * We offered a key, a node has responded to the offer. Note that this runs on the
	 * incoming packets thread so should allocate a new thread if it does anything heavy.
	 * Note also that it is responsible for unlocking the UID.
	 ...
	 */
	public void sendOfferedKey(final Key key, final boolean isSSK, final boolean needPubKey, final long uid,
			final PeerNode source, final OfferReplyTag tag, final boolean realTimeFlag) throws NotConnectedException {
	...
	protected void innerSendOfferedKey(Key key, final boolean isSSK, boolean needPubKey, final long uid,
		...
		if (isSSK) {
			...
			PartiallyReceivedBlock prb = new PartiallyReceivedBlock(Node.PACKETS_IN_BLOCK, Node.PACKET_SIZE,
					block.getRawData());
			final BlockTransmitter bt = new BlockTransmitter(node.usm, node.getTicker(), source, uid, prb,
					senderCounter, BlockTransmitter.NEVER_CASCADE, new BlockTransmitterCompletion() {
	...
	class OfferList {
		...
		/**
		 * Keep the last offer - we weren't able to use it e.g. because of
		 * RejectedOverload. Maybe it will be useful again in the future.
		 */
