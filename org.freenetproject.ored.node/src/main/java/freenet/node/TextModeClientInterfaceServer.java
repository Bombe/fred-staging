...
public class TextModeClientInterfaceServer implements Runnable {
	...
	public static TextModeClientInterfaceServer maybeCreate(Node node, NodeClientCore core, Config config)
			throws IOException {
		...
		TMCIConfig.register("directEnabled", false, 1, true, false, "TextModeClientInterfaceServer.enableInputOutput",
		...
		if (direct) {
			HighLevelSimpleClient client = core.makeClient(PriorityClasses.INTERACTIVE_PRIORITY_CLASS, true, false);
			TextModeClientInterface directTMCI = new TextModeClientInterface(node, core, client, core.getDownloadsDir(),
