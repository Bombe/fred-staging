 ...
 *
 * When calling <code>start()</code>, the thread will iterate the first time after
 * <code>getStartupDelay()</code> milliseconds. After each iteration, it will sleep for
 * <code>getSleepTime()</code> milliseconds.
 *
 * @deprecated This class uses {@link TrivialTicker} in a bogus way. See
 * <a href="https://bugs.freenetproject.org/view.php?id=6423">issue 6423</a>.<br>
 * Also, it was not used by fred itself but only by Web Of Trust and Freetalk, while not
 * being part of the fred official plugin API. Thus it should not be contained in
 * fred.<br>
 * If you do continue to need this class for a plugin, please move it to a shared library
 * for plugins.<br>
 * In that case, please use the version of Web Of Trust as it contains fixes for the
 * existing bugs of this version here.
 * @author xor
 */
@Deprecated
public abstract class TransferThread implements PrioRunnable, ClientGetCallback, ClientPutCallback {
	...
	/**
	 * Tells this TransferThread to start it's execution. You have to call this after
	 * constructing an object of an implementing class - it must not be called in the
	...
	/**
	 * Specify the priority of this thread. Priorities to return can be found in class
	 * NativeThread.
	 */
	@Override
		...
		if (mFetches != null)
			synchronized (mFetches) {
				...
				for (ClientGetter fetch : fetches) {
					/*
					 * This calls onFailure which removes the fetch from mFetches on the
					 * same thread, therefore we need to copy to an array
					 */
	...
	/**
	 * Called by the TransferThread after getStartupDelay() milliseconds for the first
	 * time and then after each getSleepTime() milliseconds.
	 */
