...
public class RealNodeProbeTest extends RealNodeRoutingTest {
	...
	static final int OUTPUT_BANDWIDTH_LIMIT = 0; // Can be useful to set this for some
													// tests.
	...
	public static void main(String[] args) throws Exception {
		...
		Listener print = new Listener() {
			...
			public void onIdentifier(long identifier, byte uptimePercentage) {
				System.out.println(
						"Probe got identifier " + identifier + " with uptime percentage " + uptimePercentage + ".");
			}
		...
		final Type types[] = { Type.BANDWIDTH, Type.BUILD, Type.IDENTIFIER, Type.LINK_LENGTHS, Type.LOCATION,
				Type.STORE_SIZE, Type.UPTIME_48H, Type.UPTIME_7D, Type.REJECT_STATS,
		...
		if (console != null)
			...
			r = new BufferedReader(new InputStreamReader(System.in)); // Use the system
																		// locale here.
		while (true) {
			...
			try {
				...
				if (selection == types.length) {
					...
					String[] options = { "probeBandwidth", "probeBuild", "probeIdentifier", "probeLinkLengths",
							"probeLinkLengths", "probeUptime" };
