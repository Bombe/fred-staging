...
public class RealNodeRequestInsertTest extends RealNodeRoutingTest {
	...
	public static void main(String[] args)
			...
			NodeInitException, ReferenceSignatureVerificationException, InterruptedException {
		...
		for (int i = 0; i < NUMBER_OF_NODES; i++) {
			nodes[i] = NodeStarter.createTestNode(DARKNET_PORT_BASE + i, 0, name, DISABLE_PROBABILISTIC_HTLS, MAX_HTL,
	 ...
	 */
	int insertRequestTest() throws CHKEncodeException, InvalidCompressionCodecException, SSKEncodeException,
			IOException, KeyDecodeException {
		...
		// Logger.error(RealNodeRequestInsertTest.class,"Inserting: \""+dataString+"\" to
		// "+node1);
		...
		if (isSSK) {
			...
			block = ((InsertableClientSSK) insertKey).encode(new ArrayBucket(buf), false, false, (short) -1, buf.length,
					random, COMPRESSOR_TYPE.DEFAULT_COMPRESSORDESCRIPTOR);
		}
		else {
		...
		if (block == null) {
			...
			if (Arrays.equals(results, data)) {
				fetchSuccesses++;
				int percentSuccess = 100 * fetchSuccesses / insertAttempts;
				Logger.error(RealNodeRequestInsertTest.class, "Fetch #" + requestNumber + " from node " + node2
						+ " succeeded (" + percentSuccess + "%): " + new String(results));
				System.err.println("Fetch #" + requestNumber + " succeeded (" + percentSuccess + "%): \""
						+ new String(results) + '\"');
