...
public class RealNodeTest {
	...
	/*
	 * Borrowed from mrogers simulation code (February 6, 2008) -- FIXME: May not generate
	 * good networks. Presumably this is because the arrays are always scanned [0..n],
	 * some nodes tend to have *much* higher connections than the degree (the first few),
	...
	static void makeKleinbergNetwork(Node[] nodes, boolean idealLocations, int degree,
		...
		if (idealLocations) {
			// First set the locations up so we don't spend a long time swapping just to
			// stabilise each network.
		...
		while (true) {
			...
			if (countFullyConnected == nodes.length && countReallyConnected == nodes.length && totalBackedOff == 0
				...
				System.err.println("Waiting for nodes to be fully connected: " + countFullyConnected + " / "
						...
						+ totalPartialConnections + " compatible " + totalCompatibleConnections + ") - backed off "
						+ totalBackedOff + " ping min/avg/max " + (int) minPingTime + "/" + (int) avgPingTime + "/"
						+ (int) maxPingTime + " at " + tDelta + 's');
				Logger.normal(RealNodeTest.class,
						"Waiting for nodes to be fully connected: " + countFullyConnected + " / " + nodes.length + " ("
								...
								+ totalPartialConnections + " compatible " + totalCompatibleConnections
								+ ") - backed off " + totalBackedOff + " ping min/avg/max " + (int) minPingTime + "/"
								+ (int) avgPingTime + "/" + (int) maxPingTime + " at " + tDelta + 's');
				Thread.sleep(1000);
			}
		}
	}
