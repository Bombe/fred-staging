...
/**
 * Create a key block with random key and contents. Create a bunch of nodes. Connect them.
 * Request the key from each node. Then insert it to one of them. Expected results: fast
 * propagation via ULPRs of the data to every node.
 *
 * Note that this relies as much on per-node failure tables causing the request to go to
 * every node as it does on ULPRs to propagate the data to every node that it's been
 * requested by.
 *
...
public class RealNodeULPRTest extends RealNodeTest {
	...
	// However, per-node failure tables should ensure the node doesn't make the same
	// mistake twice so visits every node.
	...
	static final boolean ENABLE_ULPRS = true; // This is the point of the test, but it's
												// probably a good idea to be able to do a
	...
	public static void main(String[] args) throws FSParseException, PeerParseException, CHKEncodeException,
			InvalidThresholdException, NodeInitException, ReferenceSignatureVerificationException,
			KeyCollisionException, SSKEncodeException, IOException, InterruptedException, SSKVerifyException,
		...
		// NodeStarter.globalTestInit(testName, false, LogLevel.ERROR,
		// "freenet.node.Location:normal,freenet.node.simulator.RealNodeRoutingTest:normal,freenet.node.Node:MINOR,freenet.io.comm.MessageCore:MINOR,freenet.node.PeerNode:MINOR,freenet.node.DarknetPeerNode:MINOR,freenet.node.FNP:MINOR,freenet.io.xfer.PacketThrottle:MINOR,freenet.node.PeerManager:MINOR",
		// true);
		...
		for (int i = 0; i < NUMBER_OF_NODES; i++) {
			nodes[i] = NodeStarter.createTestNode(DARKNET_PORT_BASE + i, 0, testName, true, MAX_HTL,
					20 /* 5% */, random, executor, 500 * NUMBER_OF_NODES, 1024 * 1024, true, ENABLE_SWAPPING, false,
					ENABLE_ULPRS, ENABLE_PER_NODE_FAILURE_TABLES, true, true, 0, ENABLE_FOAF, false, true, false, null);
		...
		for (int totalCount = 0; totalCount < NUMBER_OF_TESTS; totalCount++) {
			...
			// Only those nodes which have been asked by another node (not directly here)
			// for the key will want it.
			// Further, only those nodes on the actual DNF path will learn from their
			// mistakes: a RejectedLoop doesn't tell you much.
			...
			for (int i = 0; i < nodes.length; i++) {
				...
				try {
					...
					case LowLevelGetException.RECENTLY_FAILED:
						System.err.println(
								"Node " + i % nodes.length + " : recently failed (expected behaviour on later tests)");
						continue;
					default:
			...
			System.err.println(
					"Nodes which were asked for the key by another node: " + visitedCount + " : " + sb.toString());

			// Store the key to ONE node.

			Logger.normal(RealNodeULPRTest.class, "Inserting to node " + (nodes.length - 1));
			long tStart = System.currentTimeMillis();
			nodes[nodes.length - 1].store(block.getBlock(), false, false, true, false); // Write
																						// to
																						// datastore
			...
			while (true) {
				...
				System.err.println("T=" + x + " : " + count + '/' + nodes.length + " have the data on test "
						+ successfulTests + ".");
				Logger.normal(RealNodeULPRTest.class, "T=" + x + " : " + count + '/' + nodes.length
						+ " have the data on test " + successfulTests + ".");
				if (x > 300) {
					System.err.println();
