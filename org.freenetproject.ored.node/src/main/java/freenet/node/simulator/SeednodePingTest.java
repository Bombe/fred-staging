...
public class SeednodePingTest extends RealNodeTest {
	...
	public static void main(String[] args)
			throws FSParseException, IOException, OpennetDisabledException, PeerParseException, InterruptedException,
		...
		try {
			...
			List<SimpleFieldSet> seedNodesAsSFS = Announcer
					.readSeednodes(new File("/tmp/", NodeFile.Seednodes.getFilename()));
			...
			while (System.currentTimeMillis() < deadline) {
				...
				for (SeedServerPeerNode seednode : node.peers.getConnectedSeedServerPeersVector(null)) {
					try {
						...
						if (ping)
							System.out.println(seednode.getIdentityString() + " uptime=" + uptime + " ping=" + ping
									+ " pingTime=" + pingTime + " uptime=" + seednode.getUptime() + " timeDelta="
			...
			for (SeedServerTestPeerNode peer : seedNodes) {
				...
				do {
					...
					if (results.length != 3) {
						System.err.println("Unable to parse line in " + logFile + " : wrong number of fields : "
								+ results.length + " : " + line);
						continue;
					}
				...
				if (firstSample < countSince && successes == 0)
					System.err.println(
							"RESULT:" + peer.getIdentityString() + " NOT CONNECTED IN LAST WEEK! LAST CONNECTED: "
