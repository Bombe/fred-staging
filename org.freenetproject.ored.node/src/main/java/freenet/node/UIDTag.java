...
/**
 * Base class for tags representing a running request. These store enough information to
 * detect whether they are finished; if they are still in the list, this normally
 * represents a bug.
 *
...
public abstract class UIDTag {
	 ...
	 * and there are still nodes in the above two, we will log an error; but if those
	 * nodes are here too, we will reassignTagToSelf and not log an error.
	 */
	 ...
	 * This should be called before we send the actual request message, to avoid us
	 * thinking we have more outgoing capacity than we actually have on a specific peer.
	 * @param peer The peer we are routing to.
	 * @param offeredKey If true, we are fetching an offered key, if false we are routing
	 * a normal request. Fetching an offered key is quite distinct, notably it has much
	 * shorter timeouts.
	 * @return True if we were already routing to (or fetching an offered key from,
	 * depending on offeredKey) the peer.
	 */
	...
	// Note that these don't actually get removed until the request is finished, unless
	// there is a disconnection or similar. But that
	...
	/**
	 * Notify that we are no longer fetching an offered key from a specific node. Must be
	 * called only when we are sure the next node doesn't think we are routing to it any
	 * more. See removeRoutingTo() explanation for more detail. When we are not routing to
	 * any nodes, and not fetching from, and the handler has also been unlocked, the UID
	 * is unlocked.
	...
	/**
	 * Notify that we are no longer routing to a specific node. When we are not routing to
	 * (or fetching offered keys from) any nodes, and the handler side has also been
	 * unlocked, the whole tag is unlocked (note that this is only relevant to incoming
	 * requests; outgoing requests only care about what we are routing to). We should not
	 * call this method until we are reasonably sure that the node in question no longer
	 * thinks we are routing to it. Whereas we unlock the handler as soon as possible,
	 * without waiting for acknowledgement of our completion notice. Be cautious (late) in
	 * what you send, and generous (early) in what you accept! This avoids problems with
	 * the previous node thinking we've finished when we haven't, or us thinking the next
	 * node has finished when it hasn't.
	 ...
	 * local.
	 * @param outwardTransfersPerInsert Expected number of outward transfers for an
	 * insert.
	 * @param forAccept If true, we are deciding whether to accept a request. If false, we
	 * are deciding whether to SEND a request. We need to be more careful for the latter
	 * than the former, to avoid unnecessary rejections and mandatory backoffs.
	 */
	...
	/**
	 * Unlock the handler. That is, the incoming request has finished. This method should
	 * be called before the acknowledgement that the request has finished is sent
	 * downstream. Therefore, we will never be waiting for an acknowledgement from
	 * downstream in order to release the slot it is using, during which time it might
	 * think we are rejecting wrongly.
	 *
	 * Once both the incoming and outgoing requests are unlocked, the whole tag is
	 * unlocked.
	 */
	...
	/**
	 * Mark a peer as handling a timeout. Hence if when the handler is unlocked, this peer
	 * is still marked as routing to (or fetching offered keys from), rather than logging
	 * an error, we will reassign this tag to self, to wait for the fatal timeout.
	 * @param next
	 */
	...
	/**
	 * Set when we are going to tell downstream that the request has timed out, but can't
	 * terminate it yet. We will terminate the request if we have to reroute it, and we
	 * count it towards the peer's limit, but we don't stop messages to the request
	 * source.
	 */
	...
	/**
	 * Should we deduct this request from the source's limit, instead of counting it
	 * towards it? A normal request is counted towards it. A hidden request is deducted
	 * from it. This is used when the source has restarted but also in some other cases.
	 */
	...
	/**
	 * Should we stop the request as soon as is convenient? Normally this happens when the
	 * source is restarted or disconnected.
	 */
	...
	/**
	 * Set a flag indicating the originator should slow down. Only used at the
	 * shouldRejectRequest stage.
	 */
