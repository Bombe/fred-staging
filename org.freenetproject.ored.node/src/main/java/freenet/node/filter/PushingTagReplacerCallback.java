...
/**
 * This TagReplcaerCallback adds pushing support for freesites, and replaces their img's
 * to pushed ones
 */
public class PushingTagReplacerCallback implements TagReplacerCallback {
	...
	/**
	 * Returns the javascript code that initializes the l10n on the client side. It must
	 * be inserted to the page.
	...
	public static String getClientSideLocalizationScript() {
		...
		for (String key : NodeL10n.getBase().getAllNamesWithPrefix("fproxy.push")) {
			l10nBuilder.append(key.substring("fproxy.push".length() + 1) + ": \""
					+ HTMLEncoder.encode(NodeL10n.getBase().getString(key)) + "\",\n");
			isNamePresentAtLeastOnce = true;
		}
		String l10n = isNamePresentAtLeastOnce ? l10nBuilder.substring(0, l10nBuilder.length() - 2)
	...
	public String processTag(ParsedTag pt, URIProcessor uriProcessor) {
		...
		if (ctx.getContainer().isFProxyJavascriptEnabled() && ctx.getContainer().isFProxyWebPushingEnabled()) {
			if (pt.element.toLowerCase().compareTo("img") == 0) {
				...
				return ""
						.concat(/* new XmlAlertElement(ctx).generate() */""
								.concat("<input id=\"requestId\" type=\"hidden\" value=\"" + ctx.getUniqueId()
										+ "\" name=\"requestId\"/>"))
						.concat("<script type=\"text/javascript\" language=\"javascript\">"
								.concat(getClientSideLocalizationScript()).concat("</script>"))
						.concat("</body>");
			}
