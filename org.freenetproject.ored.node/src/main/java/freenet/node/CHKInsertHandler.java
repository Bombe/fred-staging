 ...
 * @author amphibian
 *
 * Handle an incoming insert request. This corresponds to RequestHandler.
 */
public class CHKInsertHandler implements PrioRunnable, ByteCounter {
	...
	CHKInsertHandler(NodeCHK key, short htl, PeerNode source, long id, Node node, long startTime, InsertTag tag,
	...
	private void realRun() {
		...
		try {
			// Using sendSync here will help the next message filter not timeout... wait
			// here or at the message filter.
		...
		if (msg.getSpec() == DMT.FNPDataInsertRejected) {
			try {
				source.sendAsync(DMT.createFNPDataInsertRejected(uid, msg.getShort(DMT.DATA_INSERT_REJECTED_REASON)),
						null, this);
			}
			catch (NotConnectedException e) {
				// Ignore.
		...
		if (htl > 0)
			sender = node.makeInsertSender(key, htl, uid, tag, source, headers, prb, false, false, forkOnCacheable,
		...
		while (true) {
			...
			// Internal error counts as overload. It'd only create a timeout otherwise,
	...
	private MessageFilter makeDataInsertFilter(long timeout) {
		MessageFilter mfDataInsert = MessageFilter.create().setType(DMT.FNPDataInsert).setField(DMT.UID, uid)
				.setSource(source).setTimeout(timeout);
		// DataInsertRejected means the transfer failed upstream so a DataInsert will not
		// be sent.
		MessageFilter mfDataInsertRejected = MessageFilter.create().setType(DMT.FNPDataInsertRejected)
				.setField(DMT.UID, uid).setSource(source).setTimeout(timeout);
		return mfDataInsert.or(mfDataInsertRejected);
	}

	private void handleNoDataInsert() {
		try {
			// Nodes wait until they have the DataInsert before forwarding, so there is
			// absolutely no excuse: There is a local problem here!
			...
			// Yes it's ugly everywhere but since we have a longish connection timeout
			// it's necessary everywhere. :|
			// FIXME review two stage timeout everywhere with some low level networking
			// guru.
			...
			node.usm.addAsyncFilter(mf, new SlowAsyncMessageFilterCallback() {
				...
				public void onMatched(Message m) {
					// Okay, great.
					// Either we got a DataInsert, in which case the transfer was aborted
					// above, or we got a DataInsertRejected, which means it never
					// started.
					// FIXME arguably we should wait until we have the message before
					// sending the transfer cancel in case the message gets lost? Or maybe
					// not?
					// FIXME unlock here rather than in finally block in realRun???
					// Unlocking in the finally block is safe (won't cause rejects),
					// unlocking here might be more accurate ...
				}
				...
				public void onTimeout() {
					...
					// We've waited long enough that we know it's not just a connectivity
					// problem - if it was we'd have disconnected by now.
					source.fatalTimeout();
				}
	...
	private void finish(int code) {
		...
		long transferTimeout = realTimeFlag ? CHKInsertSender.TRANSFER_COMPLETION_ACK_TIMEOUT_REALTIME
		...
		// If we wanted to reduce latency at the cost of security (bug 3338), we'd commit
		// here, or even on the receiver thread.
		...
		if ((sender != null) && (!sentCompletionWasSet)) {
			...
			// If there are downstream senders, our final success report depends on there
			// being no timeouts in the chain.
			while (true) {
				synchronized (sender) {
					if (sender.completed()) {
						break;
		...
		if ((sender == null) && (!sentCompletionWasSet) && (canCommit)) {
			// There are no downstream senders, but we stored the data locally, report
			// successful transfer.
			// Note that this is done even if the verify fails.
			m = DMT.createFNPInsertTransfersCompleted(uid, false /* no timeouts */);
		}

		// Don't commit until after we have received all the downstream transfer
		// completion notifications.
		// We don't want an attacker to see a ULPR notice from the inserter before he sees
		// it from the end of the chain (bug 3338).
		if (block != null) {
			commit(block);
			block = null;
		}
		...
		if (code != CHKInsertSender.TIMED_OUT && code != CHKInsertSender.GENERATED_REJECTED_OVERLOAD
			...
			if (code == CHKInsertSender.SUCCESS) {
				// Report both sent and received because we have both a Handler and a
				// Sender
	...
	private CHKBlock verify() {
		...
		synchronized (this) {
			...
			try {
				...
				Logger.error(this,
						"Verify failed in CHKInsertHandler: " + e + " - headers: " + HexUtil.bytesToHex(headers), e);
	...
	private void commit(CHKBlock block) {
		try {
			node.store(block,
					node.shouldStoreDeep(key, source, sender == null ? new PeerNode[0] : sender.getRoutedTo()), false,
	...
	public class DataReceiver implements PrioRunnable {
		...
		public void run() {
			...
			// Don't log whether the transfer succeeded or failed as the transfer was
			// initiated by the source therefore could be unreliable evidence.
			br.receive(new BlockReceiverCompletion() {
				...
				public void blockReceiveFailed(RetrievalException e) {
					...
					// Cancel the sender
					if (sender != null)
						sender.onReceiveFailed(); // tell it to stop if it hasn't already
													// failed... unless it's sending from
													// store
					runThread.interrupt();
					tag.timedOutToHandlerButContinued(); // sender is finished, or will be
															// very soon; we may however
															// be waiting for the
															// sendAborted downstream.
					...
					try {
						...
						// If they are not connected, that's probably why the receive
						// failed!
						if (logMINOR)
					...
					if (e.getReason() == RetrievalException.SENDER_DISCONNECTED)
						Logger.normal(this, "Failed to retrieve (disconnect): " + e + " for " + CHKInsertHandler.this,
								e);
					else
						// Annoying, but we have stats for this; no need to call attention
						// to it, it's unlikely to be a bug.
						Logger.normal(this,
								"Failed to retrieve (" + e.getReason() + "/"
										+ RetrievalException.getErrString(e.getReason()) + "): " + e + " for "
