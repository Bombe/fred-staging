 ...
 *
 * Note that the older version has a half life on the influence of any given report
 * without taking into account the fact that reports persist and accumulate. :)
 *
 */
public final class TimeDecayingRunningAverage implements RunningAverage, Cloneable {
	...
	public String toString() {
		...
		synchronized (this) {
			return super.toString() + ": currentValue=" + curValue + ", halfLife=" + halfLife + ", lastReportTime="
					+ (now - lastReportTime) + "ms ago, createdTime=" + (now - createdTime) + "ms ago, totalReports="
					+ totalReports + ", started=" + started + ", defaultValue=" + defaultValue + ", min=" + minReport
					+ ", max=" + maxReport;
		}
	}
	 ...
	 * @param callback
	 */
	public TimeDecayingRunningAverage(double defaultValue, long halfLife, double min, double max,
	 ...
	 * @param callback
	 */
	public TimeDecayingRunningAverage(double defaultValue, long halfLife, double min, double max, SimpleFieldSet fs,
			TimeSkewDetectorCallback callback) {
		curValue = defaultValue;
		this.defaultValue = defaultValue;
		started = false;
		...
		this.lastReportTime = -1; // long warm-up may skew results, so lets wait for the
									// first report
		this.minReport = min;
		this.maxReport = max;
		totalReports = 0;
	 ...
	 * @throws IOException
	 */
	public TimeDecayingRunningAverage(double defaultValue, double halfLife, double min, double max, DataInputStream dis,
			TimeSkewDetectorCallback callback) throws IOException {
		int m = dis.readInt();
		if (m != MAGIC)
	...
	public void report(double d) {
		synchronized (this) {
			...
			if (!started) {
				...
				if (uptime < 0) {
					Logger.error(this, "Clock (uptime) went back in time, ignoring report: " + now + " was "
							+ createdTime + " (back " + (-uptime) + "ms)");
					if (timeSkewCallback != null)
						timeSkewCallback.setTimeSkewDetectedUserAlert();
					return;
					// Disable sensitivity hack.
					// Excessive sensitivity at start isn't necessarily a good thing.
					// In particular it makes the average inconsistent - 20 reports of 0
					// at 1s intervals have a *different* effect to 10 reports of 0 at 2s
					// intervals!
					// Also it increases the impact of startup spikes, which then take a
				...
				curValue = curValue * changeFactor /*
													 * close to 1.0 if short interval,
													 * close to 0.0 if long interval
													 */
						+ (1.0 - changeFactor) * d;
				...
				if (logDEBUG)
					Logger.debug(this, "Reported " + d + " on " + this + ": thisInterval=" + thisInterval
							...
							+ ", changeFactor=" + changeFactor + ", oldCurValue=" + oldCurValue + ", currentValue="
							+ currentValue() + ", thisInterval=" + thisInterval + ", thisHalfLife=" + thisHalfLife
