 ...
 * @author robert
 *
 * A filter on BootstrappingDecayingRunningAverage which makes it aware of the circular
 * keyspace.
 */
public final class DecayingKeyspaceAverage implements RunningAverage, Cloneable {
	...
	/**
	 * 'avg' is the normalized average location, note that the the reporting bounds are
	 * (-2.0, 2.0) however.
	 */
	...
	public synchronized void report(double d) {
		if ((d < 0.0) || (d > 1.0))
			// Just because we use non-normalized locations doesn't mean we can accept
			// them.
		...
		/*
		 * To gracefully wrap around the 1.0/0.0 threshold we average over (or under) it,
		 * and simply normalize the result when reporting a currentValue ---example---
		 * d=0.2; //being reported superValue=1.9; //already wrapped once, but at 0.9
		 * thisValue=0.9; //the normalized value of where we are in the keyspace diff =
		 * +0.3; //the diff from the normalized values; Location.change(0.9, 0.2);
		 * avg.report(2.2);//to successfully move the average towards the closest route to
		 * the given value.
		 */
