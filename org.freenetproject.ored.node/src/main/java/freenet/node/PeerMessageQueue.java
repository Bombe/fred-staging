...
/**
 * Queue of messages to send to a node. Ordered first by priority then by time. Will soon
 * be round-robin between different transfers/UIDs/clients too.
 *
...
public class PeerMessageQueue {
	...
	private class PrioQueue {
		...
		/**
		 * If true, do round-robin between UID's, and count the timeout relative to the
		 * last send. Block transfers need this - both realtime and bulk.
		 */
		 ...
		 * iterator failing, and also delete efficiently. Ordered by timeLastSent, NOT by
		 * timeout. Items we have not yet sent are at the beginning with timeLastSent =
		 * -1.
		 */
		DoublyLinkedListImpl<Items> nonEmptyItemsWithID;

		/**
		 * Items which have been sent within the last 10 minutes, so we need to track them
		 * for good round-robin, but which we don't have anything queued on right now.
		 */
		...
		/**
		 * Non-urgent messages. Same order as in Items, so stuff to send first is at the
		 * beginning.
		 */
		...
		/**
		 * Add a new message. For a normal priority level, we just add it to the end of
		 * the list. It will be sent after the messages that are already queued, and its
		 * deadline is effectively the time it was submitted plus the timeout. For a
		 * priority level using round robin between peers, it is the same unless we have
		 * recently sent a message with the same UID. If we have, the timeout is relative
		 * to the last send.
		 */
		public void addLast(MessageItem item) {
		...
		private void addToNonUrgent(MessageItem item) {
			...
			// MessageItem's can be created out of order, so the timestamps may not be
			// consistent.
			// CONCURRENCY: This is not a problem in addNonUrgentMessages() because it is
			// always called from one thread.
			while (true) {
				if (!it.hasPrevious()) {
					it.add(item);
					if (logMINOR)
						...
						if (list == null) {
							...
							if (list.items.isEmpty()) {
								if (list.getParent() == nonEmptyItemsWithID) {
									...
									// Must add it to the list before moving to non-empty
		...
		/**
		 * Add a new message to the beginning i.e. send it as soon as possible (e.g. if we
		 * tried to send it and failed); it is assumed to already be urgent.
		 */
		public void addFirst(MessageItem item) {
		...
		/**
		 * Check that nonEmptyItemsWithID is ordered correctly. LOCKING: Caller must
		 * synchronize on PeerMessageQueue.this.
		 */
		private void checkOrder() {
			if (nonEmptyItemsWithID != null) {
				...
				for (Items items : nonEmptyItemsWithID) {
					...
					if (thisTime < prev)
						Logger.error(this,
								"Inconsistent order in non empty items with ID: prev timeout was " + prev + " for "
										+ prevItems + " but this timeout is " + thisTime + " for " + items,
								new Exception("error"));
					prev = thisTime;
					prevItems = items;
				}
			...
			if (itemsNonUrgent != null) {
				...
				for (MessageItem item : itemsNonUrgent) {
					if (item.submitted < prev)
						Logger.error(this, "Inconsistent order in itemsNonUrgent: prev submitted at " + prev
								+ " but this at " + item.submitted + " prev is " + prevItem + " this is " + item);
					prev = item.submitted;
					prevItem = item;
				}
			}
		 ...
		 * addMessages().
		 * @param t The initial urgent time. What we return must be less than or equal to
		 * this. Convenient for chaining.
		 * @param stopIfBeforeTime If the next urgent time is <= to this time, return
		 * immediately.
		 */
				...
				if (nonEmptyItemsWithID != null) {
					for (Items items : nonEmptyItemsWithID) {
						...
						if (items.timeLastSent > 0) {
							...
							// It is possible that something requeued isn't urgent, so
							// check anyway.
					...
					for (MessageItem item : itemsNonUrgent) {
						...
						if (items != null && items.timeLastSent > 0) {
							...
							if (t <= stopIfBeforeTime)
								return t;
							if (itemsByID == null)
								break; // Only the first one matters, since none have been
										// sent.
						}
					}
				}
		...
		/**
		 * Add the size of messages in this queue to <code>length</code> until length is
		 * larger than <code>maxSize</code>, or all messages have been added.
				...
				if (itemsByID != null) {
					...
					if (tracker != null) {
						...
						if (tracker.items.isEmpty()) {
							if (logDEBUG)
								Logger.debug(this,
										"Moving " + tracker + " to end of empty list in addNonUrgentMessages");
							if (emptyItemsWithID == null)
								emptyItemsWithID = new DoublyLinkedListImpl<Items>();
							if (parent == null) {
								...
								// Normal. Remove it so we can re-add it in the right
								// place.
			...
			while (true) {
				...
				for (int i = 0; i < lists && list != null; i++) {
					if (logMINOR)
						checkOrder();
					if (list.items.isEmpty()) {
						// Should not happen, but check for it anyway since it keeps
						// happening. :(
		 ...
		 * @param now
		 * @param messages
		 * @param addPeerLoadStatsRT Will be set if the caller needs to include a load
		 * stats message for realtime (i.e. a realtime request completes etc).
		 * @param addPeerLoadStatsBulk Will be set if the caller needs to include a load
		 * stats message for bulk (i.e. a bulk request completes etc).
		 * @param incomplete Will be set if there were more messages but they did not fit.
		 * If this is not set, we can try another priority.
		 * @return
		 */
		MessageItem addPriorityMessages(long now, MutableBoolean addPeerLoadStatsRT,
			...
			if (logMINOR) {
				...
				if (nonEmpty + empty < byID) {
					Logger.error(this, "Leaking itemsByID? non empty = " + nonEmpty + " empty = " + empty + " by ID = "
							+ byID + " on " + this);
				}
				else if (logDEBUG)
					Logger.debug(this,
							"Items: non empty " + nonEmpty + " empty " + empty + " by ID " + byID + " on " + this);
			}
			if (roundRobinBetweenUIDs)
				moveToUrgent(now);
		...
		private void clearOldNonUrgent(long now) {
			...
			while (true) {
				...
				if (list.timeLastSent == -1 || now - list.timeLastSent > FORGET_AFTER) {
					// FIXME: Urgh, what a braindead API! remove(Object) on a Map<Long,
					// Items> !?!?!?!
					...
					if (old == null)
						...
						Logger.error(this, "Different list in the items by ID tracker: " + old + " not " + list
								+ " for " + list.id);
					emptyItemsWithID.remove(list);
					removed++;
				}
	...
	PeerMessageQueue() {
		...
		for (int i = 0; i < queuesByPriority.length; i++) {
			if (i == DMT.PRIORITY_BULK_DATA)
				// Bulk: round-robin between UID's (timeout since last sent), long
				// timeout.
				queuesByPriority[i] = new PrioQueue(PacketSender.MAX_COALESCING_DELAY_BULK, true);
			else if (i == DMT.PRIORITY_REALTIME_DATA)
				// Realtime: round-robin between UID's (timeout since last sent), short
				// timeout.
				queuesByPriority[i] = new PrioQueue(PacketSender.MAX_COALESCING_DELAY, true);
			else
				// Everything else: Still round-robin between UID's, but timeout on
				// submitted.
	...
	/**
	 * Queue a <code>MessageItem</code> and return an estimate of the size of this queue.
	 * The value returned is the estimated number of bytes needed for sending the all
	 * messages in this queue. Note that if the returned estimate is higher than 1024, it
	 * might not cover all messages.
	...
	/**
	 * like enqueuePrioritizedMessageItem, but adds it to the front of those in the same
	 * priority.
	 *
	...
	/**
	 * Get the time at which the next message must be sent. If any message is overdue, we
	 * will return a value less than now, which may not be completely accurate.
	 * @param t The current next urgent time. The return value will be no greater than
	 * this.
	 * @param returnIfBefore The current time. If the next urgent time is less than this
	 * we return immediately rather than computing an accurate past value. Set to
	...
	public synchronized long getNextUrgentTime(long t, long returnIfBefore) {
		for (PrioQueue queue : queuesByPriority) {
			...
			if (t <= returnIfBefore)
				return t; // How much in the past doesn't matter, as long as it's in the
							// past.
		}
		return t;
	}
	...
	/**
	 * Returns <code>true</code> if <code>minSize</code> + the length of all messages in
	 * this queue is greater than <code>maxSize</code>.
	 * @param minSize the starting size
	 * @param maxSize the maximum size
	 * @return <code>true</code> if <code>minSize</code> + the length of all messages in
	 * this queue is greater than <code>maxSize</code>
	 */
	...
	/**
	 * Grab a message to send. WARNING: PeerMessageQueue not only removes the message, it
	 * assumes it has been sent for purposes of fairness between UID's. You should try not
	 * to call this function if you are not going to be able to send the message: check in
	 * advance if possible.
	 */
	public synchronized MessageItem grabQueuedMessageItem(int minPriority) {
		...
		if (tryRealtimeFirst) {
			...
			MessageItem ret = queuesByPriority[DMT.PRIORITY_REALTIME_DATA].addPriorityMessages(now, addPeerLoadStatsRT,
			...
			MessageItem ret = queuesByPriority[DMT.PRIORITY_BULK_DATA].addPriorityMessages(now, addPeerLoadStatsRT,
					addPeerLoadStatsBulk);
			if (ret != null)
				return ret;
			if (logMINOR)
