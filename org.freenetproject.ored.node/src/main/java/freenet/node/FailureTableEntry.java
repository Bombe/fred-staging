 ...
 * it; this greatly improves latency and efficiency for polling-based tools. For nodes we
 * have routed to, we keep up to HTL separate entries; for nodes we have received requests
 * from, we keep only one entry.
 *
 * SECURITY: All this could be a security risk if not regularly cleared - which it is, of
 * course: We forget about either kind of node after a fixed period, in
...
class FailureTableEntry implements TimedOutNodesList {
	...
	/** The key */
	final Key key; // FIXME should this be stored compressed somehow e.g. just the routing
					// key?
	...
	/**
	 * Boot ID when they requested it. We don't send it to restarted nodes, as a (weak,
	 * but useful if combined with other measures) protection against seizure.
	 */
	...
	/**
	 * Their locations when we requested it. This may be needed in the future to determine
	 * whether to let a request through that we would otherwise have failed with
	 * RecentlyFailed, because the node we would route it to is closer to the target than
	 * any we've routed to in the past.
	 */
	...
	/**
	 * Timeouts for each node for purposes of RecentlyFailed. We accept what they say,
	 * subject to an upper limit, because we MUST NOT suppress too many requests, as that
	 * could lead to a self-sustaining key blocking.
	 */
	long[] requestedTimeoutsRF;

	/**
	 * Timeouts for each node for purposes of per-node failure tables. We use our own
	 * estimates, based on time elapsed, for most failure modes; a fixed period for DNF
	 * and RecentlyFailed.
	 */
	...
	/**
	 * We remember that a node has asked us for a key for up to an hour; after that, we
	 * won't offer the key, and if we receive an offer from that node, we will reject it
	 */
	...
	public static final WeakReference<? extends PeerNodeUnlocked>[] EMPTY_WEAK_REFERENCE = (WeakReference<? extends PeerNodeUnlocked>[]) new WeakReference<?>[0];
	 ...
	 * @param rfTimeout The time until we can route to the node again, for purposes of
	 * RecentlyFailed.
	 * @param ftTimeout The time until we can route to the node again, for purposes of
	...
	// Dunno if there's a more elegant way of dealing with this which doesn't
	...
	synchronized int addRequestor(PeerNodeUnlocked requestor, long now, short origHTL) {
		...
		// Because weak, these can become null; doesn't matter, but we want to minimise
		// memory usage
		...
		@SuppressWarnings("unchecked")
		WeakReference<? extends PeerNodeUnlocked>[] newRequestorNodes = (WeakReference<? extends PeerNodeUnlocked>[]) new WeakReference<?>[requestorNodes.length
				+ notIncluded - nulls];
	...
	private synchronized int addRequestedFrom(PeerNodeUnlocked requestedFrom, short htl, long now) {
		...
		@SuppressWarnings("unchecked")
		WeakReference<? extends PeerNodeUnlocked>[] newRequestedNodes = (WeakReference<? extends PeerNodeUnlocked>[]) new WeakReference<?>[requestedNodes.length
				+ notIncluded - nulls];
	...
	/**
	 * Offer this key to all the nodes that have requested it, and all the nodes it has
	 * been requested from. Called after a) the data has been stored, and b) this entry
	 * has been removed from the FT
	 */
	public void offer() {
		HashSet<PeerNodeUnlocked> set = new HashSet<PeerNodeUnlocked>();
		final boolean logMINOR = FailureTableEntry.logMINOR;
		if (logMINOR)
			Logger.minor(this, "Sending offers to nodes which requested the key from us: (" + requestorNodes.length
					+ ") for " + key);
		synchronized (this) {
			for (int i = 0; i < requestorNodes.length; i++) {
				WeakReference<? extends PeerNodeUnlocked> ref = requestorNodes[i];
			...
			if (logMINOR)
				Logger.minor(this,
						"Sending offers to nodes which we sent the key to: (" + requestedNodes.length + ") for " + key);
			for (int i = 0; i < requestedNodes.length; i++) {
				WeakReference<? extends PeerNodeUnlocked> ref = requestedNodes[i];
				if (ref == null)
					continue;
	...
	/**
	 * Get the timeout time for the given peer, taking HTL into account. If there was a
	 * timeout at HTL 1, and we are now sending a request at HTL 2, we ignore the timeout.
	 */
	@Override
	...
	public synchronized boolean cleanup() {
		long now = System.currentTimeMillis(); // don't pass in as a pass over the whole
												// FT may take a while. get it in the
												// method.
