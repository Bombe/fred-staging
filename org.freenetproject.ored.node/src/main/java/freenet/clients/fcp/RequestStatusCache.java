...
public class RequestStatusCache {
	...
	synchronized void finishedDownload(String identifier, boolean success, long dataSize, String mimeType,
			FetchExceptionMode failureCode, String failureReasonLong, String failureReasonShort, Bucket dataShadow,
			boolean filtered) {
		...
		status.setFinished(success, dataSize, mimeType, failureCode, failureReasonLong, failureReasonShort, dataShadow,
				filtered);
	}
	...
	synchronized void finishedUpload(String identifier, boolean success, FreenetURI finalURI,
	...
	synchronized void updateDetectedCompatModes(String identifier, InsertContext.CompatibilityMode[] compatModes,
			byte[] splitfileKey, boolean dontCompress) {
		DownloadRequestStatus status = (DownloadRequestStatus) requestsByIdentifier.get(identifier);
		if (status == null)
			return; // Can happen during cancel etc.
	...
	public synchronized void updateStarted(String identifier, boolean started) {
		...
		if (!started)
			// Caller should call with false first, so we only need to unset finished when
	...
	public synchronized CacheFetchResult getShadowBucket(FreenetURI key, boolean noFilter) {
		...
		for (Object o : downloads) {
			...
			if (noFilter && download.filterData)
				continue;
			// FIXME it probably *is* worth the effort to allow this when it is overridden
			// on the fetcher, since the user changed the type???
