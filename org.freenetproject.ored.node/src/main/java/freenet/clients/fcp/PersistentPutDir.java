...
public class PersistentPutDir extends FCPMessage {
	...
	public PersistentPutDir(String identifier, FreenetURI publicURI, FreenetURI privateURI, int verbosity,
			short priorityClass, Persistence persistence, boolean global, String defaultName,
			HashMap<String, Object> manifestElements, String token, boolean started, int maxRetries,
			boolean dontCompress, String compressorDescriptor, boolean wasDiskPut, boolean realTime,
			byte[] splitfileCryptoKey, InsertContext.CompatibilityMode cmode) {
	...
	private SimpleFieldSet generateFieldSet() {
		...
		// FIXME/RESOLVE: The new BaseManifestPutter's container mode does not hold the
		// origin data,
		...
		ManifestElementNew[] elements = BaseManifestPutter.flatten(manifestElements);
		fs.putSingle("DefaultName", defaultName);
		for (int i = 0; i < elements.length; i++) {
			String num = Integer.toString(i);
			ManifestElementNew e = elements[i];
			...
			if (tempURI != null) {
				...
				if (data == null) {
					Logger.error(this, "Bucket already freed: " + e.getData() + " for " + e + " for " + e.getName()
							+ " for " + identifier);
				}
