 ...
 *
 * ClientPutDiskDirMessage < generic fields from ClientPutDirMessage > Filename=<filename>
 * AllowUnreadableFiles=<unless true, any unreadable files cause the whole request to
 * fail> End
 *
 * FIXME this should use the same code as makeDiskDirManifest does for internal directory
 * inserts.
 */
public class ClientPutDiskDirMessage extends ClientPutDirMessage {
	...
	private HashMap<String, Object> makeBucketsByName(File thisdir, String prefix) throws MessageInvalidException {
		...
		if (filelist == null)
			throw new MessageInvalidException(ProtocolErrorMessage.FILE_NOT_FOUND, "No such directory!", identifier,
					global);
		for (int i = 0; i < filelist.length; i++) {
			...
			if (filelist[i].canRead() && filelist[i].exists()) {
				if (filelist[i].isFile()) {
					...
					ret.put(f.getName(), new ManifestElementNew(f.getName(), prefix + f.getName(), bucket,
							DefaultMIMETypes.guessMIMEType(f.getName(), true), f.length()));
				}
				else if (filelist[i].isDirectory()) {
					HashMap<String, Object> subdir = makeBucketsByName(new File(thisdir, filelist[i].getName()),
							prefix + filelist[i].getName() + '/');
