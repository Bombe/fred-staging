...
/**
 * A simple client fetch. This can of course fetch arbitrarily large files, including
 * splitfiles, redirects, etc.
 */
public class ClientGet extends FCPClientRequest
	...
	/**
	 * Bucket returned when the request was completed, if returnType ==
	 * RETURN_TYPE_DIRECT.
	 */
	...
	/**
	 * Length of the found data. Will be updated from ClientGetter in onResume() but we
	 * persist it anyway.
	 */
	private long foundDataLength = -1;

	/**
	 * MIME type of the found data. Will be updated from ClientGetter in onResume() but we
	 * persist it anyway.
	 */
	...
	/**
	 * Current compatibility mode. This is updated over time as the request progresses,
	 * and can be used e.g. to reinsert the file. This is NOT transient, as the
	 * ClientGetter does not retain this information.
	 */
	private CompatibilityAnalyser compatMode;

	/**
	 * Expected hashes of the final data. Will be updated from ClientGetter in onResume()
	 * but we persist it anyway.
	 */
	...
	public enum ReturnType {
		...
		DIRECT((short) 0), NONE((short) 1), DISK((short) 2), CHUNKED((short) 3);
	...
	public ClientGet(PersistentRequestClient globalClient, FreenetURI uri, boolean dsOnly, boolean ignoreDS,
			boolean filterData, int maxSplitfileRetries, int maxNonSplitfileRetries, long maxOutputLength,
			ReturnType returnType, boolean persistRebootOnly, String identifier, int verbosity, short prioClass,
			File returnFilename, String charset, boolean writeToClientCache, boolean realTimeFlag, boolean binaryBlob,
			NodeClientCore core) throws IdentifierCollisionException, NotAllowedException, IOException {
		super(uri, identifier, verbosity, charset, null, globalClient, prioClass,
	...
	public ClientGet(FCPConnectionHandler handler, ClientGetMessage message, NodeClientCore core)
		...
		super(message.uri, message.identifier, message.verbosity, message.charset, handler, message.priorityClass,
				message.persistence, message.realTimeFlag, message.clientToken, message.global);
		...
		if (returnType == ReturnType.DISK) {
			...
			if (!core.allowDownloadTo(targetFile))
				...
				throw new MessageInvalidException(
						ProtocolErrorMessage.DIRECT_DISK_ACCESS_DENIED, "Not allowed to download to " + targetFile
								+ ". You might need to do a " + TestDDARequestMessage.NAME + " first.",
						identifier, global);
		...
		try {
			...
			// This is *not* a FetchException since we don't register it: it's a protocol
			// error.
	...
	private ClientGetter makeGetter(NodeClientCore core, Bucket ret) throws IOException {
		if (binaryBlob && ret == null) {
			ret = core.clientContext.getBucketFactory(persistence == Persistence.FOREVER)
					.makeBucket(fctx.maxOutputLength);
		}

		return new ClientGetter(this, uri, fctx, priorityClass, binaryBlob ? new NullBucket() : ret,
				binaryBlob ? new BinaryBlobWriter(ret) : null, false, initialMetadata, extensionCheck);
	}
	...
	private void trySendDataFoundOrGetFailed(FCPConnectionOutputHandler handler, String listRequestIdentifier) {
		...
		if (succeeded) {
			...
			// FIXME: CompletionTime is set on finish() : we need to give it current time
			// here
	...
	private synchronized AllDataMessage getAllDataMessage() {
		if (returnType != ReturnType.DIRECT)
			return null;
		AllDataMessage msg = new AllDataMessage(returnBucketDirect, identifier, global, startupTime, completionTime,
				foundDataMimeType);
		if (persistence == Persistence.CONNECTION)
			msg.setFreeOnSent();
		return msg;
	...
	private FCPMessage persistentTagMessage() {
		return new PersistentGet(identifier, uri, verbosity, priorityClass, returnType, persistence, targetFile,
				clientToken, client.isGlobalQueue, started, fctx.maxNonSplitfileRetries, binaryBlob,
	...
	private boolean isRealTime() {
		...
		if (lowLevelClient == null) {
			// This can happen but only due to data corruption - old databases on which
			// various bugs have resulted in it getting deleted, and also possibly failed
			// deletions.
	...
	public void onFailure(FetchException e, ClientGetter state) {
		...
		// may be restarted, and the bucket persists on the getter, even if we get rid of
		// it here.
		// freeData(container);
		finish();
		if (client != null)
	...
	public void receive(ClientEvent ce, ClientContext context) {
		...
		if (ce instanceof SplitfileProgressEvent) {
			...
			synchronized (this) {
				progress = progressPending = new SimpleProgressMessage(identifier, global, (SplitfileProgressEvent) ce);
			}
			if (client != null) {
				RequestStatusCache cache = client.getRequestStatusCache();
				if (cache != null) {
	...
	private void innerHandleCompatibilityMode(SplitfileCompatibilityModeEvent ce, ClientContext context) {
		compatMode.merge(ce.minCompatibilityMode, ce.maxCompatibilityMode, ce.splitfileCryptoKey, ce.dontCompress,
				ce.bottomLayer);
		if (client != null) {
			RequestStatusCache cache = client.getRequestStatusCache();
			if (cache != null) {
	 ...
	 * @return The data in a {@link Bucket}, or <code>null</code> if this isn&rsquo;t
	 * applicable
	 */
	...
	synchronized RequestStatus getStatus() {
		...
		if (shadow != null) {
			if (dataSize != shadow.size()) {
				Logger.error(this,
						"Size of downloaded data has changed: " + dataSize + " -> " + shadow.size() + " on " + shadow);
				shadow = null;
			}
			else {
		...
		return new DownloadRequestStatus(identifier, persistence, started, finished, succeeded, total, min, fetched,
				latestSuccess, fatal, failed, latestFailure, totalFinalized, priorityClass, failureCode, mimeType,
				dataSize, target, getCompatibilityMode(), getOverriddenSplitfileCryptoKey(), getURI(),
				failureReasonShort, failureReasonLong, overriddenDataType, shadow, filterData, getDontCompress());
	}
	...
	public void getClientDetail(DataOutputStream dos, ChecksumChecker checker) throws IOException {
		...
		DataOutputStream innerDOS = new DataOutputStream(
	...
	public static ClientRequest restartFrom(DataInputStream dis, RequestIdentifier reqID, ClientContext context,
	...
	private ClientGet(DataInputStream dis, RequestIdentifier reqID, ClientContext context, ChecksumChecker checker)
		...
		try {
			DataInputStream innerDIS = new DataInputStream(
