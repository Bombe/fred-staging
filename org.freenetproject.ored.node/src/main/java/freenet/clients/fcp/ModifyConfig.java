...
public class ModifyConfig extends FCPMessage {
	...
	public void run(FCPConnectionHandler handler, Node node) throws MessageInvalidException {
		if (!handler.hasFullAccess()) {
			throw new MessageInvalidException(ProtocolErrorMessage.ACCESS_DENIED, "ModifyConfig requires full access",
					identifier, false);
		}
				...
				if (s != null) {
					if (!(o.getValueString().equals(s))) {
						...
						try {
							...
							// Bad values silently fail from an FCP perspective, but the
							// FCP client can tell if a change took by comparing
							// ConfigData messages before and after
