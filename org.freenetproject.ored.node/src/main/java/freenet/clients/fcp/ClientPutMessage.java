...
/**
 *
 * ClientPut URI=CHK@ // could as easily be an insertable SSK URI
 * Metadata.ContentType=text/html Identifier=Insert-1 // identifier, as always Verbosity=0
 * // just report when complete MaxRetries=999999 // lots of retries PriorityClass=1 //
 * FProxy priority level
 *
 * UploadFrom=direct // attached directly to this message DataLength=100 // 100kB or
...
public class ClientPutMessage extends DataCarryingMessage {
	...
	/**
	 * The hash of the file you want the node to deal with. it is MANDATORY to do DDA
	 * operations and should be computed like that:
	 *
	 * Base64Encode(SHA256( Handler.connectionIdentifer + ClientPutMessage.identifier +
	 * content))
	 */
	...
	public ClientPutMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		if (priorityString == null) {
			...
			priorityClass = PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS;
		}
		else {
			try {
		...
		// We do *NOT* check that FileHash is valid here for backward compatibility... and
		...
		extraInsertsSingleBlock = fs.getInt("ExtraInsertsSingleBlock", ClientConstants.EXTRA_INSERTS_SINGLE_BLOCK);
		extraInsertsSplitfileHeaderBlock = fs.getInt("ExtraInsertsSplitfileHeaderBlock",
				ClientConstants.EXTRA_INSERTS_SPLITFILE_HEADER);
