 ...
 * It is the inverse of {@link FCPPluginServerMessage} which produces the on-network
 * format of server to client messages.<br>
 *
 * There is a similar class {@link FCPPluginMessage} which serves as a container of FCP
 * plugin messages which are produced and consumed by the actual server and client plugin
 * implementations. Consider this class here as an internal representation of FCP plugin
 * messages used solely for parsing client-to-server messages, while the other one is the
 * external representation used for both server and client messages. Also notice that
 * interface {@link FCPPluginConnection} consumes only objects of type
 * {@link FCPPluginMessage}, not of this class here: As the external representation to
 * both server and client, it does not care whether a message is from the server or the
 * client, and is only interested in the external representation {@link FCPPluginMessage}
 * of the message.<br>
 * <br>
 *
 * ATTENTION: The on-network name of this message is different: It is {@value #NAME}. The
 * class previously had the same name but it was decided to rename it to fix the name
 * clash with the aforementioned external representation class {@link FCPPluginMessage}.
 * To stay backward compatible, it was decided to keep the raw network message name as
 * is.<br>
 * TODO: Would it technically be possible to add a second name to the on-network data so
 * we can get rid of the old name after a transition period?<br>
 * <br>
 *
 * @link FCPPluginConnection FCPPluginConnection gives an overview of how plugin messaging
 * works in general.
 * @link FCPPluginConnectionImpl FCPPluginConnectionImpl gives an overview of the internal
 * code paths which messages take.
 * @author saces
 * @author xor (xor@freenetproject.org)
 *
 * FCPPluginMessage Identifer=me PluginName=plugins.HelloFCP.HelloFCP
 * Param.Itemname1=value1 Param.Itemname2=value2 ...
 *
 * EndMessage or DataLength=datasize Data <datasize> bytes of data
 *
 */
public class FCPPluginClientMessage extends DataCarryingMessage {
	 ...
	 *
	 * ATTENTION: This one is different to the class name. For an explanation, see the
	 * class-level JavaDoc {@link FCPPluginClientMessage}.
	 */
	...
	FCPPluginClientMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		// subset() will return null if the subset is empty. To make server code more
		// robust, we
	...
	protected FCPPluginMessage constructFCPPluginMessage() {
		return FCPPluginMessage.constructRawMessage(null, identifier, plugparams, this.bucket, success, errorCode,
				errorMessage);
	}
	...
	public void run(final FCPConnectionHandler handler, final Node node) throws MessageInvalidException {
		...
		// 1. The new interface FCPPluginConnection. This is only available if the plugin
		// implements
		// the new interface FredPluginFCPMessageHandler.ServerSideFCPMessageHandler
		// 2. The old class PluginTalker. This is available if the plugin implements the
		// old
		// interface FredPluginFCP.
		// We first try code path 1 by doing
		// FCPConnectionHandler.getFCPPluginConnection(): That
		// function will only yield a result if the new interface is implemented.
		// If that fails, we try the old code path of PluginTalker, which will fail if the
		// plugin
		...
		try {
			...
			// Do not send an error yet: Allow plugins which only implement the old
			// interface to
			// keep working.
			// TODO: Once we remove class PluginTalker, we should throw here as we do
			// below.
		}
		...
		if (serverConnection != null) {
			...
			// handler.getFCPPluginConnection() might also throw IOException in the future
			// and we
			// don't want to mix that up with the one whose reason is that the plugin does
			// not
			// support the new interface: In the case of send() throwing, it would
			// indicate that the
			// plugin DOES support the new interface but was unloaded meanwhile. So we can
			// exit the
