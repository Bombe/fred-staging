...
/**
 * client -> node: DDARequest { WantRead=true, WantWrite=true, Dir=/tmp/blah } node ->
 * client: DDAReply { Dir=/tmp/blah, ReadFilename=random1, WriteFilename=random2,
 * ContentToWrite=random3 } client -> node: DDAResponse { Dir=/tmp/blah, ReadContent=blah
...
public class TestDDARequestMessage extends FCPMessage {
	...
	public TestDDARequestMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		if ((wantRead == false) && (wantWrite == false))
			throw new MessageInvalidException(ProtocolErrorMessage.INVALID_MESSAGE, "Both " + WANT_READ + " and "
					+ WANT_WRITE + " are set to false: what's the point of sending a message?", identifier, false);
	}
