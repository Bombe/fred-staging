...
public class SimpleProgressMessage extends FCPMessage {
	...
	public SimpleFieldSet getFieldSet() {
		...
		/*
		 * FIXME: This field has been disabled since it will always be 0 (= "never") even
		 * if parts of the file transfer failed due to temporary reasons such as
		 * "data not found" / "route not found" / etc. This is due to shortcomings in the
		 * underlying event framework. Please re-enable it once the underlying issue is
		 * fixed: https://bugs.freenetproject.org/view.php?id=6526
		 */
		// fs.put("LastFailure", event.latestFailure != null ?
		// event.latestFailure.getTime() : 0);
