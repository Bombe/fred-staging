...
public abstract class FCPMessage {
	...
	/**
	 * Create a message from a SimpleFieldSet, and the message's name, if possible. Useful
	 * for FCPClients
	 */
	...
	/**
	 * Returns an FCP message that delegates the methods {@link #getFieldSet()},
	 * {@link #getName()}, and {@link #run(FCPConnectionHandler, Node)} to the given FCP
	 * message, adding a “ListRequestIdentifier” field to the {@link SimpleFieldSet}
	 * returned by {@link #getFieldSet()}.
	 * @param fcpMessage The FCP message to wrap
	 * @param listRequestIdentifier The list request identifier to add (may be
	 * {@code null} in which case nothing is added)
