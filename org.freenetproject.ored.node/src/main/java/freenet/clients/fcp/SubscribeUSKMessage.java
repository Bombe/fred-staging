 ...
 * SubscribedUSKMessage that his request has been taken into account and whenever a new
 * latest version of the USK is available. There is a flag for whether the node should
 * actively probe for the USK.
 *
 * SubscribeUSK
 * URI=USK@60I8H8HinpgZSOuTSD66AVlIFAy-xsppFr0YCzCar7c,NzdivUGCGOdlgngOGRbbKDNfSCnjI0FXjHLzJM4xkJ4,AQABAAE/index/4
 * DontPoll=true // meaning passively subscribe, don't cause the node to actively probe
 * for it Identifier=identifier End
 */
public class SubscribeUSKMessage extends FCPMessage {
	...
	public SubscribeUSKMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		if (suri == null)
			throw new MessageInvalidException(ProtocolErrorMessage.MISSING_FIELD, "Expected a URI on SubscribeUSK",
					identifier, false);
		FreenetURI uri;
		try {
		...
		if (!dontPoll)
			...
			sparsePoll = false;
		prio = fs.getShort("PriorityClass", PriorityClasses.BULK_SPLITFILE_PRIORITY_CLASS);
