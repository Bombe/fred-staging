...
import freenet.client.request.NodeClientRequest;
...
/**
 * A request process carried out by the node for an FCP client. Examples: ClientGet,
 * ClientPut, MultiGet.
 */
public abstract class FCPClientRequest implements Serializable, ClientRequest {

	/**
	 * ATTENTION: When incrementing this, please skip version 2. Version 2 had already
	 * temporarily been used by a development branch.
	 */
	...
	public FCPClientRequest(FreenetURI uri2, String identifier2, int verbosity2, String charset,
	...
	public FCPClientRequest(FreenetURI uri2, String identifier2, int verbosity2, String charset,
			FCPConnectionHandler handler, short priorityClass2, Persistence persistenceType2, final boolean realTime,
			String clientToken2, boolean global) {
		int hash = super.hashCode();
		if (hash == 0)
			hash = 1;
		hashCode = hash;
	...
	public abstract void sendPendingMessages(FCPConnectionOutputHandler handler, String listRequestIdentifier,
	...
	public enum Persistence {
		...
		CONNECTION,
		/**
		 * Reports to client by name; persists over connection loss. Not saved to disk, so
		 * dies on reboot.
		 */
		REBOOT,
		...
		public static Persistence parseOrThrow(String persistenceString, String identifier, boolean global)
	...
	/**
	 * Is the request persistent? False = we can drop the request if we lose the
	 * connection
	 */
	...
	/**
	 * Returns the time of the request’s last activity, or {@code 0} if there is no known
	 * last activity.
	...
	public long getLastActivity() {
		...
		// But this function is deprecated at the time of its writing and thus will be
		// removed soon,
		// so there is no reason to invest the time to read the code to see whether it
		// will happen.
		if (cr == null) {
			return 0;
		}
	...
	/**
	 * Called after a ModifyPersistentRequest. Sends a PersistentRequestModified message
	 * to clients if any value changed.
	 */
	...
	/**
	 * Called after a RemovePersistentRequest. Send a PersistentRequestRemoved to the
	 * clients. If the request is in the database, delete it.
	 */
	...
	protected FCPClientRequest(DataInputStream dis, RequestIdentifier reqID, ClientContext context)
		...
		if (priorityClass < PriorityClasses.MAXIMUM_PRIORITY_CLASS
				|| priorityClass > PriorityClasses.PAUSED_PRIORITY_CLASS)
		...
		// constructors.
		this.client = ((PersistentRequestRoot) context.persistentRequestChecker).makeClient(global, clientName);
		this.lowLevelClient = client.lowLevelClient(realTime);
	}

	/**
	 * Called just after serializing in the request. Called by the ClientRequester, i.e.
	 * the tree starts there, and we MUST NOT call back to it or we get an infinite
	 * recursion. The main purpose of this method is to give us an opportunity to connect
	 * to the various (transient) system utilities we get from ClientContext, e.g. bucket
	 * factories, the FCP persistent root etc. The base class implementation in
	 * ClientRequest will register the request with an PersistentRequestClient via the new
	...
	public final void onResume(ClientContext context) throws ResumeFailedException {
		client = ((PersistentRequestRoot) context.persistentRequestChecker).makeClient(global, clientName);
		lowLevelClient = client.lowLevelClient(realTime);
		innerResume(context);
		ClientRequester req = getClientRequest();
		if (req != null)
			req.onResume(context); // Can legally be null.
		((PersistentRequestRoot) context.persistentRequestChecker).resume(this, global, clientName);
	}
