...
public class LoadPlugin extends FCPMessage {
	...
	public LoadPlugin(SimpleFieldSet fs) throws MessageInvalidException {
		...
		if (identifier == null)
			throw new MessageInvalidException(ProtocolErrorMessage.MISSING_FIELD, "Must contain an Identifier field",
					null, false);
		pluginURL = fs.get("PluginURL");
		if (pluginURL == null)
			throw new MessageInvalidException(ProtocolErrorMessage.MISSING_FIELD, "Must contain a PluginURL field",
					identifier, false);
		...
		if ((type != null) && (type.trim().length() > 0))
			...
			urlType = null;
		if (urlType != null) {
			if (!(TYPENAME_FILE.equalsIgnoreCase(urlType) || TYPENAME_FREENET.equalsIgnoreCase(urlType)
					|| TYPENAME_OFFICIAL.equalsIgnoreCase(urlType) || TYPENAME_URL.equalsIgnoreCase(urlType)))
		...
		if (officialSource != null) {
			if (officialSource.equalsIgnoreCase("https")) {
				...
				throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
						"Unknown OfficialSource '" + officialSource + "'", identifier, false);
			}
		}
		else {
			force = false;
	...
	public void run(final FCPConnectionHandler handler, final Node node) throws MessageInvalidException {
		if (!handler.hasFullAccess()) {
			throw new MessageInvalidException(ProtocolErrorMessage.ACCESS_DENIED, "LoadPlugin requires full access",
					identifier, false);
		}

		if (!node.pluginManager.isEnabled()) {
			handler.send(new ProtocolErrorMessage(ProtocolErrorMessage.PLUGINS_DISABLED, false, "Plugins disabled",
					identifier, false));
			return;
		}
				...
				if (urlType == null) {
					if (node.pluginManager.isOfficialPlugin(pluginURL) != null) {
						...
						try {
							...
							// FIXME currently i have no idea how to auto detect a proper
							// url,
