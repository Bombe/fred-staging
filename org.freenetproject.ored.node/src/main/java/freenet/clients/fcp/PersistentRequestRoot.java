...
/**
 * Tracks persistent requests by PersistentRequestClient. Not persistent itself. Populated
 * on startup.
 *
 * @author toad
 */
public class PersistentRequestRoot implements PersistentRequestChecker {
	...
	public ClientRequest[] getPersistentRequests() {
		List<FCPClientRequest> requests = new ArrayList<FCPClientRequest>();
	...
	PersistentRequestClient resume(FCPClientRequest FCPClientRequest, boolean global, String clientName) {
		PersistentRequestClient client = makeClient(global, clientName);
		client.resume(FCPClientRequest);
		return client;
	}
