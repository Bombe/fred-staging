...
public abstract class DataCarryingMessage extends BaseDataCarryingMessage {
	...
	/**
	 * If this is a message from the client, then the Bucket was created by createBucket()
	 * and will be a RandomAccessBucket. However if it is a message we are sending to the
	 * client, it may not be. FIXME split up into two classes?
	 */
	...
	/**
	 * Should only be called from code parsing a message sent to us, in which case Bucket
	 * will be a RandomAccessBucket, which it needs to be as it's likely to be inserted.
	 * If we are sending a message to the client, it might not be. FIXME split up into two
	 * classes?
	 */
