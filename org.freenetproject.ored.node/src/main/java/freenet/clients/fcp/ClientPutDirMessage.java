 ...
 *
 * Both share: Identifier=<identifier> Verbosity=<verbosity as ClientPut> MaxRetries=<max
 * retries as ClientPut> PriorityClass=<priority class> URI=<target URI>
 * GetCHKOnly=<GetCHKOnly as ClientPut> DontCompress=<DontCompress as ClientPut>
 * ClientToken=<ClientToken as ClientPut> Persistence=<Persistence as ClientPut>
...
public abstract class ClientPutDirMessage extends BaseDataCarryingMessage {
	...
	public ClientPutDirMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		try {
			...
			// Client is allowed to put a slash at the end if it wants to, but this is
			// discouraged.
