...
public abstract class ClientPutDirMessage extends BaseDataCarryingMessage {
	...
	public ClientPutDirMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		if(priorityString == null) {
			...
			try {
				...
				if(!PriorityClasses.isValidPriorityClass(priorityClass))
					throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD, "Invalid priority class "+priorityClass+" - range is "+ PriorityClasses.PAUSED_PRIORITY_CLASS+" to "+ PriorityClasses.MAXIMUM_PRIORITY_CLASS, identifier, global);
			} catch (NumberFormatException e) {
		...
		extraInsertsSplitfileHeaderBlock = fs.getInt("ExtraInsertsSplitfileHeaderBlock", ClientConstants.EXTRA_INSERTS_SPLITFILE_HEADER);
