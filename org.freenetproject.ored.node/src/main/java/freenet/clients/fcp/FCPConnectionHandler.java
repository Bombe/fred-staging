...
public class FCPConnectionHandler implements Closeable {
	...
	final HashMap<String, FCPClientRequest> requestsByIdentifier;

	/**
	 * {@link FCPPluginConnectionImpl} indexed by the server plugin name (see
	 * {@link PluginManager#getPluginFCPServer(String)}.<br>
	 * <br>
	 *
	 * This also serves the same purpose as the client which is connected to this would
	 * normally be responsible for if it was running as a plugin in the node (as specified
	 * by
	 * {@link PluginRespirator#connectToOtherPlugin(String, ClientSideFCPMessageHandler)}):<br>
	 * It keeps strong references to the {@link FCPPluginConnectionImpl} objects, and
	 * thereby marks them as alive.<br>
	 * This in turn causes the {@link FCPPluginConnectionImpl} objects to stay available
	 * in the {@link FCPPluginConnectionTracker}, which allows server plugins to query
	 * them by their ID.
	 */
	private final TreeMap<String, FCPPluginConnectionImpl> pluginConnectionsByServerName = new TreeMap<String, FCPPluginConnectionImpl>();
	 ...
	 * - {@link ReadWriteLock} can do that faster than a regular Lock. (A
	 * {@link ReentrantReadWriteLock} because thats the only implementation of
	 * {@link ReadWriteLock}.)
	 */
	...
	public FCPConnectionHandler(Socket s, FCPServer server) {
		...
		requestsByIdentifier = new HashMap<String, FCPClientRequest>();
		...
		// The random 16-byte identifier was used before we added the UUID. Luckily, UUIDs
		// are also
		// 16 byetes, so we can re-use the bytes.
		// TODO: When getting rid of the non-UUID connectionIdentifier, use
		// UUID.randomUUID();
	 ...
	 * <br>
	 *
	 * ATTENTION: The function will return immediately before even trying to send the
	 * message, the message will be sent asynchronously.<br>
	 * As a consequence, this function not throwing does not give any guarantee whatsoever
	 * that the message will ever be sent.
	 */
	...
	public void close() {
		...
		synchronized (this) {
			...
			isClosed = true;
			requests = new FCPClientRequest[requestsByIdentifier.size()];
		...
		for (FCPClientRequest req : requests)
	...
	public void setClientName(final String name) {
		...
		// Create foreverClient lazily. Everything that needs it (especially creating
		// ClientGet's etc) runs on a database job.
		if (logMINOR)
	 ...
	 * IdentifierCollisionMessage. Hence, we can run stuff on other threads if we need to,
	 * as long as we send the right messages.
	 */
				...
				try {
					...
					if (!persistent) {
						...
						try {
							...
							outputHandler.queue(new ProtocolErrorMessage(ProtocolErrorMessage.PERSISTENCE_DISABLED,
									false, "Persistence is disabled", id, global));
							return;
						}
					...
					outputHandler
							.queue(new ProtocolErrorMessage(e.protocolCode, false, e.getMessage(), e.ident, e.global));
					return;
				}
			}
		}
			...
			if (success) {
				if (!persistent) {
					try {
						...
						failedMessage = new ProtocolErrorMessage(ProtocolErrorMessage.IO_ERROR, true, e.getMessage(),
								id, message.global);
					}
				}
				else if (message.persistence == Persistence.FOREVER) {
						...
						server.core.clientContext.jobRunner.queue(new PersistentJob() {
							...
							public boolean run(ClientContext context) {
								...
								try {
									...
									outputHandler.queue(new ProtocolErrorMessage(e.protocolCode, false, e.getMessage(),
											e.ident, e.global));
									return false;
								}
								catch (MalformedURLException e) {
									outputHandler.queue(
											new ProtocolErrorMessage(ProtocolErrorMessage.FREENET_URI_PARSE_ERROR, true,
													null, id, message.global));
									return false;
								}
								catch (IOException e) {
									outputHandler.queue(new ProtocolErrorMessage(ProtocolErrorMessage.IO_ERROR, true,
											null, id, message.global));
									return false;
								}
								try {
	...
	public void startClientPutDir(final ClientPutDirMessage message, final HashMap<String, Object> buckets,
		...
		if (success) {
			if (!persistent) {
				...
				// FIXME register non-persistent requests in the constructors also, we
				// already register persistent ones...
			}
			else if (message.persistence == Persistence.FOREVER) {
				try {
					server.core.clientContext.jobRunner.queue(new PersistentJob() {
						...
						public boolean run(ClientContext context) {
							ClientPutDir putter;
							try {
								putter = new ClientPutDir(FCPConnectionHandler.this, message, buckets, wasDiskPut,
										server);
							}
							catch (IdentifierCollisionException e) {
								Logger.normal(this, "Identifier collision on " + this);
								...
								outputHandler.queue(new ProtocolErrorMessage(
	 ...
	 * {@link PluginManager#getPluginFCPServer(String)}). Atomically creates and stores it
	...
	FCPPluginConnection getFCPPluginConnection(String serverPluginName) throws PluginNotFoundException {
		...
		// The suspected typical usage pattern of this function is that the great majority
		// of calls
		// will return an existing FCPPluginConnection. Creating a fresh one will
		// typically only
		// happen at the start of a connection and then it will be re-used a lot.
		// Therefore, it would cost a lot of performance to use synchronized() and we
		// instead use a
		// ReadWriteLock which is optimal for such patterns.
		//
		// The double-checked locking pattern which this induces is necessary due to the
		// fact that a
		// read-lock cannot be upgraded to a write lock.
		// The JavaDoc of ReentrantReadWriteLock specifically recommends this pattern, so
		// it ought
		// to be a safe version of double-checked locking.

		pluginConnectionsByServerName_Lock.readLock().lock();
		try {
			// We use the actual *Impl instead of the interface because the implementation
			// provides
			// isServerDead(), which the interface does not.
			FCPPluginConnectionImpl peekOldConnection = pluginConnectionsByServerName.get(serverPluginName);
		...
		try {
			// Re-check whether there is an existing connection since we had to re-acquire
			// the lock
			// meanwhile.
			...
			if (oldConnection != null) {
				if (!oldConnection.isServerDead()) {
					...
					// oldConnection.isDead() returned true because the WeakReference to
					// the server
					// has been nulled because the plugin was unloaded or reloaded.
					// The connection should be discarded then. We have no ReferenceQueue
					// to discard
					// affected connections from the pluginConnectionsByServerName table,
					// so we
					...
					// NOTICE: Even if there was automatic disposal of nulled references,
					// we still
					// would have to manually remove dead ones here: I have observed that
					// it can
					// take minutes until the JVM flushes a ReferenceQueue. So if we
					// relied upon
					// that only, during those minutes a client would be unable to send
					// messages to
					// a re-loaded server plugin because the continued existence of the
					// dead old
			...
			FCPPluginConnectionImpl newConnection = server.createFCPPluginConnectionForNetworkedFCP(serverPluginName,
					this);
	...
	public void finishedClientRequest(FCPClientRequest get) {
		synchronized (this) {
	...
	/**
	 * That method ought to be called before any DirectDiskAccess operation is performed
	 * by the node
	 * @param filename
	 ...
	 *
	 * FIXME: Maybe we need to enqueue a PS job to delete the created file after something
	 * like ... 5 mins ?
	 */
	protected DDACheckJob enqueueDDACheck(String path, boolean read, boolean write) throws IllegalArgumentException {
		...
		if (read && (readFile != null) && readFile.canWrite()) {
			// We don't want to attempt to write before: in case an IOException is raised,
			// we want to inform the
			// client somehow that the node can't write there... And setting readFile to
			// null means we won't inform
	...
	public FCPClientRequest removeRequestByIdentifier(String identifier, boolean kill) {
		FCPClientRequest req;
		synchronized (this) {
	...
	FCPClientRequest getRebootRequest(boolean global, FCPConnectionHandler handler, String identifier) {
		if (global)
	...
	FCPClientRequest getForeverRequest(boolean global, FCPConnectionHandler handler, String identifier) {
		if (global)
	...
	FCPClientRequest removePersistentRebootRequest(boolean global, String identifier) throws MessageInvalidException {
		PersistentRequestClient client = global ? server.globalRebootClient : getRebootClient();
		FCPClientRequest req = client.getRequest(identifier);
		if (req != null) {
	...
	FCPClientRequest removePersistentForeverRequest(boolean global, String identifier) throws MessageInvalidException {
