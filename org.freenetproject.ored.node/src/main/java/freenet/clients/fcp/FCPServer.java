...
public class FCPServer implements Runnable, DownloadCache {
	...
	/**
	 * Stores {@link FCPPluginConnectionImpl} objects by ID and automatically garbage
	 * collects them so we don't have to bloat this class with that.
	 */
	...
	public FCPServer(String ipToBindTo, String allowedHosts, String allowedHostsFullAccess, int port, Node node,
			NodeClientCore core, boolean isEnabled, boolean assumeDDADownloadAllowed, boolean assumeDDAUploadAllowed,
			boolean neverDropAMessage, int maxMessageQueueLength, PersistentRequestRoot persistentRoot)
	...
	public void maybeStart() {
		...
		if (node.pluginManager.isEnabled()) {
			// We need to start the FCPPluginConnectionTracker no matter whether
			// this.enabled == true:
			// If networked FCP is disabled, plugins might still communicate via
			// non-networked
			// intra-node FCP.
	...
	public static FCPServer maybeCreate(Node node, NodeClientCore core, Config config, PersistentRequestRoot root)
			throws IOException, InvalidConfigValueException {
		SubConfig fcpConfig = config.createSubConfig("fcp");
		short sortOrder = 0;
		fcpConfig.register("enabled", true, sortOrder++, true, false, "FcpServer.isEnabled", "FcpServer.isEnabledLong",
		...
		fcpConfig.register("port", FCPServer.DEFAULT_FCP_PORT /*
																 * anagram of 1984, and
																 * 1000 up from old number
																 */, 2, true, true, "FcpServer.portNumber",
				"FcpServer.portNumberLong", new FCPPortNumberCallback(core), false);
		fcpConfig.register("bindTo", NetworkInterface.DEFAULT_BIND_TO, sortOrder++, true, true, "FcpServer.bindTo",
				"FcpServer.bindToLong", new FCPBindtoCallback(core));
		...
		fcpConfig.register("allowedHostsFullAccess", NetworkInterface.DEFAULT_BIND_TO, sortOrder++, true, true,
				"FcpServer.allowedHostsFullAccess", "FcpServer.allowedHostsFullAccessLong",
		...
		fcpConfig.register("maxMessageQueueLength", 1024, sortOrder++, true, false, "FcpServer.maxMessageQueueLength",
				"FcpServer.maxMessageQueueLengthLong", cb7 = new MaxMessageQueueLengthCallback(), false);
		fcpConfig.register("neverDropAMessage", false, sortOrder++, true, false, "FcpServer.neverDropAMessage",
				"FcpServer.neverDropAMessageLong", cb6 = new NeverDropAMessageCallback());
	...
	/**
	 * <p>
	 * Creates and registers a {@link FCPPluginConnectionImpl} object for a FCP connection
	 * which is attached by network.<br/>
	 * In other words, the actual client application is NOT a plugin running within the
	 * node, it only connected to the node via network.
	 * </p>
	 *
	 * <p>
	 * The object is registered at the backend {@link FCPPluginConnectionTracker} and thus
	 * can be queried from this server by ID via the frontend
	 * {@link #getPluginConnectionByID(UUID)} as long as something else keeps a strong
	 * reference to it.<br/>
	 * Once it becomes weakly reachable, it will be garbage-collected from the backend
	 * {@link FCPPluginConnectionTracker} and {@link #getPluginConnectionByID(UUID)} will
	 * not return it anymore. <br>
	 * In other words, you don't have to take care of registering or unregistering
	 * connections. You only have to take care of keeping a strong reference to them while
	 * they are in use.
	 * </p>
	 *
	 * <p>
	 * ATTENTION: Only for internal use by the frontend function
	 * {@link FCPConnectionHandler#getFCPPluginConnection(String)}.
	 * </p>
	 *
	 * @see FCPPluginConnectionImpl The class JavaDoc of FCPPluginConnectionImpl explains
	 * the code path for both networked and non-networked FCP.
	 */
	final FCPPluginConnectionImpl createFCPPluginConnectionForNetworkedFCP(String serverPluginName,
			FCPConnectionHandler messageHandler) throws PluginNotFoundException {

		FCPPluginConnectionImpl connection = FCPPluginConnectionImpl.constructForNetworkedFCP(pluginConnectionTracker,
	...
	/**
	 * <p>
	 * Creates and registers a {@link FCPPluginConnection} object for FCP connections
	 * between plugins running within the same node.<br/>
	 * In other words, the actual client application is NOT connected to the node by
	 * network, it is a plugin running within the node just like the server.
	 * </p>
	 *
	 * <p>
	 * The object is registered at the backend {@link FCPPluginConnectionTracker} and thus
	 ...
	 * connectToPlugin() which in turn is only to be used by clients, the returned
	 * connection will have a default send direction of {@link SendDirection#ToServer}.
	 *
	 * @see FCPPluginConnectionImpl The class JavaDoc of FCPPluginConnectionImpl explains
	 * the code path for both networked and non-networked FCP.
	 */
	public final FCPPluginConnection createFCPPluginConnectionForIntraNodeFCP(String serverPluginName,
			ClientSideFCPMessageHandler messageHandler) throws PluginNotFoundException {

		FCPPluginConnectionImpl connection = FCPPluginConnectionImpl.constructForIntraNodeFCP(pluginConnectionTracker,
				node.executor, node.pluginManager, serverPluginName, messageHandler);
		// The constructor function already did this for us
		/* pluginConnectionTracker.registerConnection(connection); */
	...
	/**
	 * <p>
	 * <b>The documentation of {@link FCPPluginConnectionTracker#getConnection(UUID)}
	 * applies to this function.</b>
	 * </p>
	 *
	 * ATTENTION: Only for internal use by the frontend function
	 * {@link PluginRespirator#getPluginConnectionByID(UUID)}. Plugins must use that
	 * instead.<br>
	 * <br>
	 *
	 * ATTENTION: Since this function is only to be used by the aforementioned
	 ...
	 *
	 * @see FCPPluginConnectionTracker The JavaDoc of FCPPluginConnectionTracker explains
	 * the general purpose of this mechanism.
	 */
	...
	public void makePersistentGlobalRequestBlocking(final FreenetURI fetchURI, final boolean filterData,
			...
			final boolean realTimeFlag, final File downloadsDir)
			...
			public boolean run(ClientContext context) {
				...
				try {
					makePersistentGlobalRequest(fetchURI, filterData, expectedMimeType, persistenceTypeString,
							returnTypeString, realTimeFlag, downloadsDir);
					return true;
				}
	...
	public boolean modifyGlobalRequestBlocking(final String identifier, final String newToken, final short newPriority)
		...
		FCPClientRequest req = this.globalRebootClient.getRequest(identifier);
		if (req != null) {
			...
			core.clientContext.jobRunner.queue(new PersistentJob() {
				...
				public boolean run(ClientContext context) {
					...
					try {
						FCPClientRequest req = globalForeverClient.getRequest(identifier);
						if (req != null)
	...
	public void makePersistentGlobalRequest(FreenetURI fetchURI, boolean filterData, String expectedMimeType,
			String persistenceTypeString, String returnTypeString, boolean realTimeFlag)
			throws NotAllowedException, IOException {
		makePersistentGlobalRequest(fetchURI, filterData, expectedMimeType, persistenceTypeString, returnTypeString,
				realTimeFlag, core.getDownloadsDir());
	}
	 ...
	 */
	public void makePersistentGlobalRequest(FreenetURI fetchURI, boolean filterData, String expectedMimeType,
			String persistenceTypeString, String returnTypeString, boolean realTimeFlag, File downloadsDir)
			throws NotAllowedException, IOException {
		...
		// public ClientGet(PersistentRequestClient globalClient, FreenetURI uri, boolean
					...
					try {
						...
						while (true) {
							...
							try {
								...
								innerMakePersistentGlobalRequest(fetchURI, filterData, persistence, returnType, id,
										returnFilename, realTimeFlag);
								return;
							}
	...
	private void innerMakePersistentGlobalRequest(FreenetURI fetchURI, boolean filterData, boolean persistRebootOnly,
			ReturnType returnType, String id, File returnFilename, boolean realTimeFlag)
		...
		final ClientGet cg = new ClientGet(persistRebootOnly ? globalRebootClient : globalForeverClient, fetchURI,
				defaultFetchContext.localRequestOnly, defaultFetchContext.ignoreStore, filterData, QUEUE_MAX_RETRIES,
				QUEUE_MAX_RETRIES, QUEUE_MAX_DATA_SIZE, returnType, persistRebootOnly, id, Integer.MAX_VALUE,
				PriorityClasses.BULK_SPLITFILE_PRIORITY_CLASS, returnFilename, null, false, realTimeFlag, false, core);
		cg.register(false);
	...
	public FCPClientRequest getGlobalRequest(String identifier) {
		FCPClientRequest req = globalRebootClient.getRequest(identifier);
		if (req == null)
	...
	/**
	 * Start a request on the global queue. Return after it has started, e.g. it will show
	 * up on the queue page, it will persist after restart etc. Actually it won't persist
	 * until the next commit, but it's close...
	 * @param req The request (insert etc) to start.
	 * @param context The client layer context object.
	 * @throws IdentifierCollisionException If there is already a request with that
	 * identifier.
	 * @throws PersistenceDisabledException If the database is disabled/broken/turned off,
	 * if we are shutting down, if we are waiting for the user to give us the decryption
	 * password etc.
	 */
	public void startBlocking(final FCPClientRequest req, ClientContext context)
	...
	public FetchResult getCompletedRequestBlocking(final FreenetURI key) throws PersistenceDisabledException {
		...
		if (get != null) {
			// FIXME race condition with free() - arrange refcounting for the data to
			// prevent this
