 ...
 * Example:
 *
 * ClientGet IgnoreDS=false // true = ignore the datastore DSOnly=false // true = only
 * check the datastore, don't route (~= htl 0) URI=KSK@sample.txt Identifier=Request
 * Number One Verbosity=0 // no status, just tell us when it's done ReturnType=direct //
 * return all at once over the FCP connection MaxSize=100 // maximum size of returned data
 * MaxTempSize=1000 // maximum size of intermediary data MaxRetries=100 // automatic retry
 * supported as an option PriorityClass=1 // priority class 1 = interactive
 * Persistence=reboot // continue until node is restarted; report progress while client is
 * connected, including if it reconnects after losing connection ClientToken=hello //
 * returned in PersistentGet, a hint to the client, so the client doesn't need to maintain
 * its own state IgnoreUSKDatehints=false // true = don't use USK datehints EndMessage
 */
public class ClientGetMessage extends BaseDataCarryingMessage {
	...
	public ClientGetMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		if (returnType == ReturnType.DIRECT) {
			...
			defaultPriority = PriorityClasses.IMMEDIATE_SPLITFILE_PRIORITY_CLASS;
		}
		else if (returnType == ReturnType.NONE) {
			diskFile = null;
			defaultPriority = PriorityClasses.PREFETCH_PRIORITY_CLASS;
		}
		else if (returnType == ReturnType.DISK) {
			defaultPriority = PriorityClasses.BULK_SPLITFILE_PRIORITY_CLASS;
		...
		if (priorityString == null) {
			...
			try {
				...
				if (!PriorityClasses.isValidPriorityClass(priorityClass))
					throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
							"Invalid priority class " + priorityClass + " - range is "
									+ PriorityClasses.PAUSED_PRIORITY_CLASS + " to "
		...
		if (global && (persistence == Persistence.CONNECTION)) {
			throw new MessageInvalidException(ProtocolErrorMessage.NOT_SUPPORTED, "Global requests must be persistent",
					identifier, global);
		}
