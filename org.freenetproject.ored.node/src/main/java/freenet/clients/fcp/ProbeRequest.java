...
/**
 * FCP Message which is received from a client and requests a network probe of a specific
 * type.
 * <ul>
 ...
 * <li>LINK_LENGTHS - returns link lengths between the endpoint and its connected
 * peers.</li>
 ...
 * </ul>
 * </li>
 * <li>hopsToLive: Optional; approximately how many hops the probe will take before
 * possibly returning a result. Valid values are [1, Probe.MAX_HTL]. If omitted
 * Probe.MAX_HTL is used.</li>
 * </ul>
 */
public class ProbeRequest extends FCPMessage {
	...
	public ProbeRequest(SimpleFieldSet fs) throws MessageInvalidException {
		/*
		 * If not defined in the field set Identifier will be null. As adding a null value
		 * to the field set does not actually add something under the key, it will also be
		 * omitted in the response messages.
		 */
		...
		try {
			...
			// Getting a String from a SimpleFieldSet does not throw - it can at worst
			// return null.
			throw new MessageInvalidException(ProtocolErrorMessage.INVALID_MESSAGE,
