...
/**
 * An FCP client. Identified by its Name which is sent on connection. Tracks persistent
 * requests for either PERSISTENCE_REBOOT or PERSISTENCE_FOREVER.
 */
public class PersistentRequestClient {
	...
	public PersistentRequestClient(String name2, FCPConnectionHandler handler, boolean isGlobalQueue,
		...
		runningPersistentRequests = new ArrayList<FCPClientRequest>();
		completedUnackedRequests = new ArrayList<FCPClientRequest>();
		clientRequestsByIdentifier = new HashMap<String, FCPClientRequest>();
	...
	private final List<FCPClientRequest> runningPersistentRequests;

	/** Completed unacknowledged persistent requests */
	private final List<FCPClientRequest> completedUnackedRequests;

	/** ClientRequest's by identifier */
	private final Map<String, FCPClientRequest> clientRequestsByIdentifier;
	...
	private FCPClientRequestClient lowLevelClient;

	private FCPClientRequestClient lowLevelClientRT;
	...
	/**
	 * Called when a client request has finished, but is persistent. It has not been acked
	 * yet, so it should be moved to the unacked-completed-requests set.
	 */
	...
	/**
	 * Queue any and all pending messages from already completed, unacknowledged,
	 * persistent requests, to be immediately sent. This happens automatically on startup
	 * and hopefully will encourage clients to acknowledge persistent requests!
	 */
	public int queuePendingMessagesOnConnectionRestart(FCPConnectionOutputHandler outputHandler,
		...
		for (i = offset; i < Math.min(reqs.length, offset + max); i++) {
			FCPClientRequest req = (FCPClientRequest) reqs[i];
	...
	public void register(FCPClientRequest cg) throws IdentifierCollisionException {
		assert (cg.persistence == persistence);
		if (logMINOR)
			Logger.minor(this, "Registering " + cg.getIdentifier());
		synchronized (this) {
			String ident = cg.getIdentifier();
			FCPClientRequest old = clientRequestsByIdentifier.get(ident);
			if ((old != null) && (old != cg))
	...
	public boolean removeByIdentifier(String identifier, boolean kill, FCPServer server, ClientContext context) {
		...
		synchronized (this) {
			...
			if (req == null) {
				for (FCPClientRequest r : completedUnackedRequests) {
					if (r.getIdentifier().equals(identifier)) {
						req = r;
						completedUnackedRequests.remove(r);
						Logger.error(this, "Found completed unacked request " + r + " for identifier "
								+ r.getIdentifier() + " but not in clientRequestsByIdentifier!!");
						break;
					}
				}
				if (req == null) {
					for (FCPClientRequest r : runningPersistentRequests) {
						if (r.getIdentifier().equals(identifier)) {
							req = r;
	...
	public void addPersistentRequests(List<FCPClientRequest> v, boolean onlyForever) {
		synchronized (this) {
			for (FCPClientRequest req : runningPersistentRequests) {
				if (req == null) {
					Logger.error(this,
							"Request is null on runningPersistentRequests for " + this + " - database corruption??");
					continue;
				}
	...
	private void addPersistentRequestStatus(List<RequestStatus> status, boolean onlyForever) {
		...
		List<FCPClientRequest> reqs = new ArrayList<FCPClientRequest>();
		addPersistentRequests(reqs, onlyForever);
		for (FCPClientRequest req : reqs) {
			try {
			...
			// FIXME deactivate? Unconditional deactivate depends on callers. Keep-as-is
			// would need merge with addPersistentRequests.
		}
	}
	...
	public synchronized FCPClientRequest getRequest(String identifier) {
		FCPClientRequest req = clientRequestsByIdentifier.get(identifier);
		return req;
	}
	...
	public synchronized void addRequestCompletionCallback(RequestCompletionCallback cb) {
		if (completionCallbacks == null)
			completionCallbacks = new ArrayList<RequestCompletionCallback>(); // it is
																				// transient
																				// so it
																				// might
																				// be null
	...
	public void removeAll(ClientContext context) {
		HashSet<FCPClientRequest> toKill = new HashSet<FCPClientRequest>();
		if (statusCache != null)
			statusCache.clear();
		synchronized (this) {
			for (FCPClientRequest req : runningPersistentRequests) {
			...
			for (FCPClientRequest req : completedUnackedRequests) {
				toKill.add(req);
			}
			completedUnackedRequests.clear();
			for (FCPClientRequest req : clientRequestsByIdentifier.values()) {
				toKill.add(req);
			}
	...
	public ClientGet getCompletedRequest(FreenetURI key) {
		...
		for (int i = 0; i < completedUnackedRequests.size(); i++) {
			FCPClientRequest req = completedUnackedRequests.get(i);
	...
	public FCPClientRequestClient lowLevelClient(boolean realTime) {
		if (realTime)
	...
	public void resume(FCPClientRequest FCPClientRequest) {
		if (FCPClientRequest.hasFinished())
			completedUnackedRequests.add(FCPClientRequest);
		else
			runningPersistentRequests.add(FCPClientRequest);
		String identifier = FCPClientRequest.identifier;
		if (clientRequestsByIdentifier.get(identifier) != null) {
			if (FCPClientRequest != clientRequestsByIdentifier.get(identifier))
				throw new IllegalArgumentException("Adding new client request " + FCPClientRequest
						+ " with same name \"" + identifier + "\" as " + clientRequestsByIdentifier.get(identifier));
			else {
			...
			clientRequestsByIdentifier.put(identifier, FCPClientRequest);
		}
	}
