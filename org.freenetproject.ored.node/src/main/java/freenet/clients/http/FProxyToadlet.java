...
import freenet.bucket.NoFreeBucket;
...
public final class FProxyToadlet extends Toadlet implements RequestClient {
	...
	static final Set<String> prefetchAllowedTypes = new HashSet<>(Arrays.asList("image/png", "image/jpeg", "image/gif",
			"audio/mp3", "audio/ogg", "video/ogg", "application/ogg"));
	 ...
	 */
	public static long MAX_LENGTH_WITH_PROGRESS = (100 * 1024 * 1024) * 11 / 10; // 100MiB
																					// plus
																					// a
																					// bit
																					// due
	...
	public static long MAX_LENGTH_NO_PROGRESS = (2 * 1024 * 1024) * 11 / 10; // 2MiB plus
																				// a bit
																				// due to
																				// buggy
																				// inserts
	...
	public static final short PRIORITY = PriorityClasses.INTERACTIVE_PRIORITY_CLASS;
	static {
		try {
	...
	private void handleDownload(ToadletContext context, Bucket data, BucketFactory bucketFactory, String mimeType,
			String requestedMimeType, String forceString, boolean forceDownload, String basePath, FreenetURI key,
			String extras, String referrer, boolean downloadLink, ToadletContext ctx, NodeClientCore core,
			boolean dontFreeData, String maybeCharset) throws ToadletContextClosedException, IOException {
		if (logMINOR)
			Logger.minor(FProxyToadlet.class,
					"handleDownload(data.size=" + data.size() + ", mimeType=" + mimeType + ", requestedMimeType="
							+ requestedMimeType + ", forceDownload=" + forceDownload + ", basePath=" + basePath
							+ ", key=" + key);
		String extrasNoMime = extras; // extras will not include MIME type to start with -
										// REDFLAG maybe it should be an array
		...
		if ((!force) && (!forceDownload)) {
			// Horrible hack needed for GWT as it relies on document.write() which is not
			// supported in xhtml
			...
			if (isSniffedAsFeed(data) && !(mimeType.startsWith("application/rss+xml"))) {
				...
				NodeL10n.getBase().addL10nSubstitution(option, "FProxyToadlet.openPossRSSAsPlainText",
						...
						new HTMLNode[] { HTMLNode.link(basePath + key.toString() + "?type=text/plain&force="
								+ getForceValue(key, now) + extrasNoMime), HTMLNode.STRONG });
				// FIXME: is this safe? See bug #131
				option = optionList.addChild("li");
				NodeL10n.getBase().addL10nSubstitution(option, "FProxyToadlet.openPossRSSForceDisk",
						new String[] { "link", "bold" },
						new HTMLNode[] { HTMLNode.link(basePath + key.toString() + "?forcedownload" + extras),
								HTMLNode.STRONG });
				...
				if (!(mimeRSS || mimeType.startsWith("text/plain"))) {
					...
					NodeL10n.getBase().addL10nSubstitution(option, "FProxyToadlet.openRSSForce",
							...
							new HTMLNode[] {
									...
									HTMLNode.STRONG, HTMLNode.text(mimeType) });
				}
				option = optionList.addChild("li");
				NodeL10n.getBase()
						.addL10nSubstitution(option, "FProxyToadlet.openRSSAsRSS", new String[] { "link", "bold" },
								new HTMLNode[] { HTMLNode.link(basePath + key.toString()
										+ "?type=application/xml+rss&force=" + getForceValue(key, now) + extrasNoMime),
										HTMLNode.STRONG });
				addDownloadOptions(ctx, optionList, key, mimeType, true, false, core); // FIXME
																						// Or
																						// false,
																						// false?
																						// Or
				...
				if (referrer != null) {
					...
					NodeL10n.getBase().addL10nSubstitution(option, "FProxyToadlet.backToReferrer",
							new String[] { "link" }, new HTMLNode[] { HTMLNode.link(referrer) });
				}
				option = optionList.addChild("li");
				...
				context.sendReplyHeaders(200, "OK", new MultiValueTable<String, String>(), "text/html; charset=utf-8",
						pageBytes.length);
		...
		if (forceDownload) {
			...
			// was application/x-msdownload, but some unix browsers offer to open that in
			// Wine as default!
			// it is important that this type not be understandable, but
			// application/octet-stream doesn't work.
			// see http://onjava.com/pub/a/onjava/excerpt/jebp_3/index3.html
			// Testing on FF3.5.1 shows that application/x-force-download wants to run it
			// in wine,
			...
			/*
			 * Firefox and its derivatives may use the MIME type implied by the filename
			 * extension for plain text, unless a Content-Encoding is specified.
			 *
			 * See https://developer.mozilla.org/en-US/docs/Mozilla/
			 * How_Mozilla_determines_MIME_Types#HTTP
			 */
			...
			if (rangeStr != null) {
				...
				try {
					...
					// FIXME catch IOException here and tell the user there is a problem
					// instead of just closing the connection.
					// Currently there is no way to tell the difference between an IOE
					// caused by the connection to the client and an internal one, we just
					// close the connection in both cases.
					os.close();
					os = null; // If we can't write, we need to throw, so we don't send
								// too little data.
				}
				finally {
					Closer.close(is);
	...
	private static void addDownloadOptions(ToadletContext ctx, HTMLNode optionList, FreenetURI key, String mimeType,
		...
		// Display FProxy option to download to disk if the user isn't at maximum physical
		// threat level
		...
		if (threatLevel != PHYSICAL_THREAT_LEVEL.MAXIMUM && !core.isDownloadDisabled()) {
			...
			NodeL10n.getBase().addL10nSubstitution(optionForm, "FProxyToadlet.downloadInBackgroundToDisk",
					new String[] { "dir", "page" },
					new HTMLNode[] {
							new HTMLNode("input", new String[] { "type", "name", "value", "maxlength", "size" },
							...
							DOWNLOADS_LINK });
			optionForm.addChild("#", " ");
			NodeL10n.getBase().addL10nSubstitution(optionForm, "FProxyToadlet.downloadToDiskWarningNotFiltered",
					new String[] { "bold" }, new HTMLNode[] { HTMLNode.STRONG });
			optionForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit",
					"select-location", NodeL10n.getBase().getString("QueueToadlet.browseToChange") + "..." });
			if (!dontShowFilter) {
				HTMLNode filterControl = optionForm.addChild("div", l10n("filterData"));
				HTMLNode f = filterControl.addChild("input", new String[] { "type", "name", "value" },
		...
		// Display fetch option if not at low physical security or the user has disabled
		// downloading to disk.
	...
	/**
	 * Does the first 512 bytes of the data contain anything that Firefox might regard as
	 * RSS? This is a horrible evil hack; we shouldn't be doing blacklisting, we should be
	 * doing whitelisting. REDFLAG Expect future security issues!
	 * @throws IOException
	 */
	...
	private void innerHandleMethodGET(URI uri, HTTPRequest httprequest, ToadletContext ctx, int recursion)
		...
		final boolean canSendProgress = isBrowser(ua) && !ctx.disableProgressPage()
				&& (accept == null || accept.indexOf("text/html") > -1) && !httprequest.isParameterSet("forcedownload");
		...
		if (ks.equals("/")) {
			if (httprequest.isParameterSet("key")) {
				...
				try {
					...
					sendErrorPage(ctx, 404, l10n("notFoundTitle"),
							NodeL10n.getBase().getString("FProxyToadlet.invalidKeyWithReason",
									new String[] { "reason" }, new String[] { e.toString() }));
					return;
				}
			...
			try {
				...
				/*
				 * This shouldn't happen because all the inputs to the URI constructor
				 * come from getters of existing URIs.
				 */
		...
		else if (ks.startsWith("/darknet/") || ks.equals("/darknet")) { // TODO (pre-build
																		// 1045 url
																		// format) remove
																		// when obsolete
		...
		else if (ks.startsWith("/opennet/") || ks.equals("/opennet")) { // TODO (pre-build
																		// 1045 url
																		// format) remove
																		// when obsolete
		...
		try {
			...
			errorInfobox.addChild("div", "class", "infobox-header", NodeL10n.getBase().getString(
					"FProxyToadlet.invalidKeyWithReason", new String[] { "reason" }, new String[] { e.toString() }));
				...
				public void foundURI(FreenetURI uri, boolean inline) {
					...
					synchronized (this) {
						if (uris.size() < MAX_PREFETCH)
							// FIXME Maybe we should do this randomly, but since it's a
							// DoS protection (in an obscure feature), if so we should do
		...
		String maybeCharset = httprequest.isParameterSet("maybecharset") ? httprequest.getParam("maybecharset", null)
				: null;
		...
		if (fetch != null)
			while (true) {
				...
				if (fr.hasData()) {
					...
					fetch.close(); // Not waiting any more, but still locked the results
									// until sent
					break;
				}
					...
					if (isJsEnabled) {
						...
						headNode.addChild("noscript").addChild("meta", "http-equiv", "Refresh").addAttribute("content",
								"2;URL=" + location);
						// If pushing is disabled, but js is enabled, then we add the
						// original progresspage.js
						...
						// If he disabled it, we just put the http refresh meta, without
						// the noscript
		...
		try {
			...
			if (data == null && fe == null) {
				...
				// If we don't have the data, then check if an FProxyFetchInProgress has.
				// It can happen when one FetchInProgress downloaded an image
				// asynchronously, then loads it. This way a FetchInprogress will have the
				// full image, and no need to block.
				...
				if (needsFetch) {
					// If we don't have the data, then we need to fetch it and block until
					// it is available
			...
			handleDownload(ctx, data, ctx.getBucketFactory(), mimeType, requestedMimeType, forceString,
					httprequest.isParameterSet("forcedownload"), "/", key, "&max-size=" + maxSizeDownload, referer,
					true, ctx, core, fr != null, maybeCharset);
		}
		catch (FetchException e) {
			...
			if (e.newURI != null) {
				if (accept != null && (accept.startsWith("text/css") || accept.startsWith("image/"))
					...
					// If it's an image or a CSS fetch, auto-follow the redirect, up to a
					// limit.
				...
				if (!restricted) {
					option = optionList.addChild("li");
					HTMLNode optionForm = option.addChild("form", new String[] { "action", "method" },
							new String[] { '/' + key.toString(), "get" });
					optionForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden",
							"max-size", String.valueOf(e.expectedSize == -1 ? Long.MAX_VALUE : e.expectedSize * 2) });
					if (requestedMimeType != null)
						optionForm.addChild("input", new String[] { "type", "name", "value" },
				...
				// optionTable.addChild("tr").addChild("td", "colspan", "2").addChild("a",
				...
				if ((e.mode == FetchExceptionMode.NOT_IN_ARCHIVE
					...
					if ((keyUtil = core.node.pluginManager.getPluginInfo("plugins.KeyUtils.KeyUtilsPlugin")) != null) {
						...
						if (keyUtil.getPluginLongVersion() < 5010)
							NodeL10n.getBase().addL10nSubstitution(option, "FProxyToadlet.openWithKeyExplorer",
									new String[] { "link" },
							...
							option = optionList.addChild("li");
							NodeL10n.getBase().addL10nSubstitution(option, "FProxyToadlet.openWithSiteExplorer",
									new String[] { "link" },
						...
						if (keyUtil.getPluginLongVersion() > 4999)
							...
							NodeL10n.getBase().addL10nSubstitution(option, "FProxyToadlet.openWithKeyExplorer",
									new String[] { "link" }, new HTMLNode[] { HTMLNode
											.link("/plugins/plugins.KeyExplorer.KeyExplorer/?key=" + key.toString()) });
					}
				}
				if (filterException != null) {
					if ((mime.equals("application/x-freenet-index"))
						...
						option = optionList.addChild("li");
						NodeL10n.getBase().addL10nSubstitution(option, "FProxyToadlet.openAsThawIndex",
					...
					textMediaType.setParameter("charset", (e.getExpectedMimeType() != null)
							? MediaType.getCharsetRobust(e.getExpectedMimeType()) : null);
					NodeL10n.getBase().addL10nSubstitution(option, "FProxyToadlet.openAsText", new String[] { "link" },
							new HTMLNode[] { HTMLNode.link(getLink(key, textMediaType.toString(), maxSize, null, false,
									maxRetries, overrideSize)) });
					option = optionList.addChild("li");
					...
					if (!(mime.equals("application/octet-stream") || mime.equals("application/x-msdownload")
						...
						option = optionList.addChild("li");
						NodeL10n.getBase()
								.addL10nSubstitution(option, "FProxyToadlet.openForce", new String[] { "link", "mime" },
										new HTMLNode[] {
												HTMLNode.link(getLink(key, mime, maxSize, getForceValue(key, now),
				...
				this.writeHTMLReply(ctx, (e.mode == FetchExceptionMode.NOT_IN_ARCHIVE) ? 404
						: 500 /*
								 * close enough - FIXME probably should depend on status
								 * code
	...
	private String getLink(FreenetURI uri, String requestedMimeType, long maxSize, String force, boolean forceDownload,
			int maxRetries, boolean appendMaxSize) {
	...
	private String sanitizeReferer(ToadletContext ctx) {
		...
		if (referer != null) {
			try {
				...
				HTTPRequest req = new ToadletHTTPRequest(refererURI, "GET");
	...
	public static void maybeCreateFProxyEtc(NodeClientCore core, Node node, Config config, SimpleToadletServer server)
			throws IOException {

		// FIXME how to change these on the fly when the interface language is changed?

		HighLevelSimpleClient client = core.makeClient(PriorityClasses.INTERACTIVE_PRIORITY_CLASS, true, true);
		...
		LocalDownloadDirectoryToadlet localDownloadDirectoryToadlet = new LocalDownloadDirectoryToadlet(core, client,
				"/downloads/");
