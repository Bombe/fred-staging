	...
	public FProxyFetchResult getResult(boolean waitForever) {
		...
		synchronized (this) {
			if (waitForever) {
				...
				/*
				 * Wait for 5 seconds or until something happens. The most common
				 * something other than finishing is a callback because the request has
				 * finished checking the datastore and has been sent to the network, in
				 * which case we want to show the progress bar.
				 */
