...
public class ContentFilterToadlet extends Toadlet implements LinkEnabledCallback {
		...
		try {
			...
			if (request.isPartSet("filter-local")) {
				try {
					...
					responseHeaders.put("Location", LocalFileFilterToadlet.PATH + "?filter-operation=" + filterOperation
							+ "&result-handling=" + resultHandling + "&mime-type=" + mimeType);
	...
	private HTMLNode createContent(PageMaker pageMaker, ToadletContext ctx) {
		...
		filterForm.addChild("input", new String[] { "type", "name", "value" },
				new String[] { "hidden", "filter-operation", FilterOperation.BOTH.toString() });

		// display in browser or save to disk
		filterForm.addChild("input", new String[] { "type", "name", "value", "id" }, new String[] { "radio",
				"result-handling", ResultHandling.DISPLAY.toString(), "result-handling-display" });
		filterForm.addChild("label", new String[] { "for" }, new String[] { "result-handling-display" },
				l10n("displayResultLabel"));
		filterForm.addChild("br");
		filterForm.addChild("input", new String[] { "type", "name", "value", "id" },
				new String[] { "radio", "result-handling", ResultHandling.SAVE.toString(), "result-handling-save" });
		filterForm.addChild("label", new String[] { "for" }, new String[] { "result-handling-save" },
	...
	private void handleFilterRequest(HTTPRequest request, ToadletContext ctx, NodeClientCore core, boolean localFile)
		...
		try {
			...
			try {
				...
				writeBadRequestError(l10n("errorNoFileOrCannotReadTitle"),
						l10n("errorNoFileOrCannotRead", "file", filename), ctx, true);
			}
		}
		catch (BadRequestException e) {
			String invalidPart = e.getInvalidRequestPart();
	...
	private void handleFilter(Bucket data, String mimeType, FilterOperation operation, ResultHandling resultHandling,
			String resultFilename, ToadletContext ctx, NodeClientCore core)
	...
	private FilterStatus applyFilter(Bucket input, Bucket output, String mimeType, FilterOperation operation,
			NodeClientCore core) throws UnsafeContentTypeException, IOException {
	...
	private FilterStatus applyFilter(InputStream input, OutputStream output, String mimeType, FilterOperation operation,
			NodeClientCore core) throws UnsafeContentTypeException, IOException {
		URI fakeUri;
		try {
