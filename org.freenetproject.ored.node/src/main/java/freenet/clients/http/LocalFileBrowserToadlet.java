...
public abstract class LocalFileBrowserToadlet extends Toadlet {
	...
	/**
	 * Last directory from which an action was performed. If accessible, this is used
	 * instead of the fallback default directory.
	 */
	...
	/**
	 * Whether the directory is allowed for the purposes of the specific browser. For
	 * example, do the node settings allow downloading to the given directory?
	 * @param path The path to check permissions for.
	 * @return Whether browsing that directory is allowed. If it is not, the
	 * LocalFileBrowserToadlet will not render a selection button or directory-changing
	 * button for it.
	 */
	...
	/**
	 * Determines the appropriate directory to start out in for the given browser. If a
	 * path is not already specified, the browser will attempt to display this directory.
	 * @return path to directory
	 */
	protected abstract String startingDir();

	/**
	 * Determines the appropriate directory to start out in if browsing for something to
	 * upload.
	 * @return If all directories or no directories are allowed, returns the user's home
	 * directory. Otherwise, returns the first allowed directory.
	 */
	protected String defaultUploadDir() {
		if ((core.getAllowedUploadDirs().length == 1 && core.getAllowedUploadDirs()[0].toString().equals("all"))
			...
			/*
			 * If all directories are allowed, or none are, go for the home directory. If
			 * none are allowed, any directory will result in an error anyway.
			 */
	 ...
	 * Determines the appropriate directory to start out in if browsing for something to
	 * download.
	 * @return If all directories or no directories are allowed, the default downloads
	 * directory is returned. Otherwise, returns the first allowed directory.
	 */
	protected String defaultDownloadDir() {
		if ((core.getAllowedDownloadDirs().length == 1 && core.getAllowedDownloadDirs()[0].toString().equals("all"))
			...
			/*
			 * If all directories are allowed, or none are, go for the default download
			 * directory. If none are allowed, any directory will result in an error
			 * anyway.
			 */
	 ...
	 * @exception ToadletContextClosedException Access is denied: uploading might be
	 * disabled overall. The user might be denied access to this directory, which could be
	 * their home directory.
	 * @exception IOException Something file-related went wrong.
	 * @see <a href=
	 * "freenet/clients/http/Toadlet#findSupportedMethods()">findSupportedMethods</a>
	 * @see "java.net.URI"
	...
	public void handleMethodPOST(URI uri, HTTPRequest request, final ToadletContext ctx)
			throws ToadletContextClosedException, IOException, RedirectException {
		renderPage(persistenceFields(readPOST(request)), request.getPartAsStringFailsafe("path", MAX_POST_SIZE), ctx,
				selectedValue(request));
	}

	/**
	 * Presents a file selection screen, or a something has been selected notes its
	 * directory and redirects to the POST target.
	...
	private void renderPage(Hashtable<String, String> fieldPairs, String path, final ToadletContext ctx,
			String filename) throws ToadletContextClosedException, IOException, RedirectException {
		...
		if (currentPath != null && !allowedDir(currentPath)) {
			...
			pageMaker.getInfobox("infobox-error", "Forbidden", page.content, "access-denied", true).addChild("#",
		...
		if (currentPath != null && currentPath.exists() && currentPath.isDirectory() && currentPath.canRead()) {
			...
			for (File currentFile : files) {
				if (currentFile.isHidden()) {
					...
					// FIXME in advanced mode, show them, and have a different style, and
					// a toggle for whether to include them in a folder insert.
					continue;
				}
	...
	private String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase().getString("LocalFileInsertToadlet." + key, new String[] { pattern },
				new String[] { value });
	}
