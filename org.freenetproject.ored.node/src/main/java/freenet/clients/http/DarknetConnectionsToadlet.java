...
public class DarknetConnectionsToadlet extends ConnectionsToadlet {
	...
	protected class DarknetComparator extends ComparatorByStatus {
		...
		protected int customCompare(PeerNodeStatus firstNode, PeerNodeStatus secondNode, String sortBy) {
			if (sortBy.equals("name")) {
				return ((DarknetPeerNodeStatus) firstNode).getName()
				...
				return ((DarknetPeerNodeStatus) firstNode).getPrivateDarknetCommentNote()
						.compareToIgnoreCase(((DarknetPeerNodeStatus) secondNode).getPrivateDarknetCommentNote());
			}
			else if (sortBy.equals("trust")) {
				return ((DarknetPeerNodeStatus) firstNode).getTrustLevel()
						.compareTo(((DarknetPeerNodeStatus) secondNode).getTrustLevel());
			}
			else if (sortBy.equals("visibility")) {
				int ret = ((DarknetPeerNodeStatus) firstNode).getOurVisibility()
						.compareTo(((DarknetPeerNodeStatus) secondNode).getOurVisibility());
				if (ret != 0)
					return ret;
				return ((DarknetPeerNodeStatus) firstNode).getTheirVisibility()
	...
	protected void drawPrivateNoteColumn(HTMLNode peerRow, PeerNodeStatus peerNodeStatus,
		...
		if (fProxyJavascriptEnabled) {
			peerRow.addChild("td", "class", "peer-private-darknet-comment-note").addChild("input",
					new String[] { "type", "name", "size", "maxlength", "onChange", "value" },
					new String[] { "text", "peerPrivateNote_" + peerNodeStatus.hashCode(), "16", "250",
							"peerNoteChange();", status.getPrivateDarknetCommentNote() });
		}
		else {
			peerRow.addChild("td", "class", "peer-private-darknet-comment-note").addChild("input",
	...
	protected String getPageTitle(String titleCountString) {
		return NodeL10n.getBase().getString("DarknetConnectionsToadlet.fullTitle", new String[] { "counts" },
				new String[] { titleCountString });
	}

	@Override
	protected boolean shouldDrawNoderefBox(boolean advancedModeEnabled) {
		return advancedModeEnabled; // Convenient for advanced users, but normally we will
									// use the "Add a friend" box.
	}
	...
	protected void drawPeerActionSelectBox(HTMLNode peerForm, boolean advancedModeEnabled) {
		peerForm.addChild("input", new String[] { "type", "name", "value" },
				new String[] { "submit", "doSendMessageToPeers", l10n("sendConfidentialMessage") });
		peerForm.addChild("br");

		HTMLNode actionSelect = peerForm.addChild("select", new String[] { "id", "name" },
		...
		HTMLNode changeTrustLevelSelect = peerForm.addChild("select", new String[] { "id", "name" },
				new String[] { "changeTrust", "changeTrust" });
		...
		HTMLNode changeVisibilitySelect = peerForm.addChild("select", new String[] { "id", "name" },
		...
		if (request.isPartSet("doSendMessageToPeers")) {
			...
			for (DarknetPeerNode pn : peerNodes) {
				if (request.isPartSet("peerPrivateNote_" + pn.hashCode())) {
					if (!request.getPartAsStringFailsafe("peerPrivateNote_" + pn.hashCode(), 250)
			...
			FRIEND_VISIBILITY trust = FRIEND_VISIBILITY
					.valueOf(request.getPartAsStringFailsafe("changeVisibility", 10));
			DarknetPeerNode[] peerNodes = node.getDarknetConnections();
			for (DarknetPeerNode pn : peerNodes) {
				if (request.isPartSet("node_" + pn.hashCode())) {
			...
			return;
				...
				|| (request.isPartSet("doAction") && request.getPartAsStringFailsafe("action", 25).equals("remove"))) {
			if (logMINOR)
				...
				if (request.isPartSet("node_" + pn.hashCode())) {
					if ((pn.timeLastConnectionCompleted() < (System.currentTimeMillis()
						...
						content.addChild("p").addChild("#",
								NodeL10n.getBase().getString("DarknetConnectionsToadlet.confirmRemoveNode",
										new String[] { "name" }, new String[] { pn.getName() }));
			...
			// FIXME this is ugly, should probably move both this code and the PeerNode
			// code somewhere.
			DarknetPeerNode[] peerNodes = node.getDarknetConnections();
			for (DarknetPeerNode pn : peerNodes) {
				if (request.isPartSet("node_" + pn.hashCode())) {
					long id = Long.parseLong(request.getPartAsStringFailsafe("id", 32)); // FIXME
																							// handle
