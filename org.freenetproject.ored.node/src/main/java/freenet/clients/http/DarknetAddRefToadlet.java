	...
	public void handleMethodGET(URI uri, final HTTPRequest request, ToadletContext ctx)
		...
		if (installer != null)
			NodeL10n.getBase().addL10nSubstitution(p, "DarknetAddRefToadlet.explainInstallerWindows",
					new String[] { "filename", "get-windows" }, new HTMLNode[] {
			...
			NodeL10n.getBase().addL10nSubstitution(p, "DarknetAddRefToadlet.explainInstallerWindowsNotYet",
					new String[] { "link" },
					new HTMLNode[] { HTMLNode.link("/" + node.nodeUpdater.getInstallerWindowsURI().toString()) });
		...
		if (installer != null)
			...
			NodeL10n.getBase().addL10nSubstitution(p, "DarknetAddRefToadlet.explainInstallerNonWindowsNotYet",
					...
					new HTMLNode[] { HTMLNode.link("/" + node.nodeUpdater.getInstallerNonWindowsURI().toString()),
							HTMLNode.text(shortFilename) });
