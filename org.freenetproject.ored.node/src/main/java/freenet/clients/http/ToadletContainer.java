...
public interface ToadletContainer {
	...
	/**
	 * Register a Toadlet. All requests whose URL starts with the given prefix will be
	 * passed to this toadlet.
	 * @param t the toadlet to register
	 * @param menu the menu category to register a navigation link with with. It is most
	 * likely also a l10n key, though that is irrelevant to this method.
	 ...
	 * previously-registered Toadlet whose urlPrefix also matches. Otherwise, the other
	 * matching Toadlet is used instead.
	 * @param fullAccessOnly Whether or not the navigation link is shown when the http
	 * client does not have full security access. Note that passing false does not prevent
	 * the Toadlet from receiving requests under urlPrefix, so Toadlet authors are advised
	 * to check for full access themselves, possibly returning a 403 error code.
	 */
	public void register(Toadlet t, String menu, String urlPrefix, boolean atFront, boolean fullAccessOnly);

	/**
	 * Registers a Toadlet and optionally adds a navigation link to the menu. All requests
	 * whose URL starts with the given prefix will be given to this Toadlet. If either the
	 * menu or the name parameter is null, then no navigation link is be registered for
	 * the Toadlet and the title, fullOnly, cb, and l10n parameters are ignored.
	 * @param t the toadlet to register
	 * @param menu the menu category to register a navigation link with with. It is most
	 * likely also a l10n key, though that is irrelevant to this method.
	 * @param urlPrefix the prefix that the Toadlet will serve; should be a path like
	 ...
	 * does not have full security access. Note that passing false does not prevent the
	 * Toadlet from receiving requests under urlPrefix, so Toadlet authors are advised to
	 * check for full access themselves, possibly returning a 403 error code.
	 * @param cb A LinkEnabledCalback, allowing fine control of when the navigation link
	 * is visible and when it isn't. Passing null means it is always visible.
	 */
	 ...
	 * @param cb A LinkEnabledCalback, allowing fine control of when the navigation link
	 * is visible and when it isn't. Passing null means it is always visible.
	 * @param l10n A FredPluginL10n instance for translating the name and title
	...
	/**
	 * Was public-gateway mode enabled on startup? (Changing it won't take effect until
	 * restart because of bookmark-related issues). If so, users with full access will
	 * still be able to configure the node etc, but everyone else will not have access to
	 * the download queue or anything else that might conceivably result in a DoS.
	 */
