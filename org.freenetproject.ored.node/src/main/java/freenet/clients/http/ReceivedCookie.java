...
public final class ReceivedCookie extends Cookie {
	 ...
	 *
	 * Does not validate the names or values of the keys, each attribute is validated at
	 * the first call to it's getter method. Therefore, no CPU time is wasted if the
	 * client sends cookies which we do not use.
	 */
	private ReceivedCookie(String myName, Hashtable<String, String> myContent) {
		// We do not validate the input here, we only parse it if someone actually tries
		// to access this cookie.
		notValidatedName = myName;
		content = myContent;

		// We do NOT parse the version even though RFC2965 requires it because Firefox
		// (3.0.14) does not give us a version.
	...
	/**
	 * Parses the value of a "Cookie:" HTTP header and returns a list of received cookies
	 * which it contained. - A single "Cookie:" header is allowed to contain multiple
	 * cookies. Further, a HTTP request can contain multiple "Cookie" keys!.
	 * @param httpHeader The value of a "Cookie:" header (i.e. the prefix "Cookie:" must
	 * not be contained in this parameter!)
	 * @return A list of {@link ReceivedCookie} objects. The validity of their name/value
	 * pairs is not deeply checked, their getName() / getValue() might throw!
	...
	protected static ArrayList<ReceivedCookie> parseHeader(String httpHeader) throws ParseException {
		...
		ArrayList<ReceivedCookie> cookies = new ArrayList<ReceivedCookie>(4); // TODO:
																				// Adjust
																				// to the
																				// usual
																				// amount
		...
		// 1. The value of cookies can be quoted, therefore it is a context-free language
		// and not a regular language - we cannot express it with a regexp!
		// 2. Its very fast :)

		// Set to true if a broken browser (Konqueror) specifies a cookie where the name
		// is NOT the first attribute.
		try {
			for (int i = 0; i < header.length;) {
				// Skip leading whitespace of key, we must do a header.length check
				// because there might be no more key, so we continue;
				...
				// Parse key
				{
					...
					while (Character.isWhitespace(header[keyEndIndex - 1])) // Remove
																			// trailing
																			// whitespace
						--keyEndIndex;
				...
				if (value == null && i < header.length) {
					...
					while (valueEndIndex > valueBeginIndex && Character.isWhitespace(header[valueEndIndex - 1])) // Remove
																													// trailing
																													// whitespace
						--valueEndIndex;
					...
					// Skip whitespace between end of quotation and the semicolon
				...
				// RFC2965: Name MUST be first. Anything key besides the name of the
				// cookie begins with $. The next cookie begins if a key occurs which is
				// not
				// prefixed with $.

				if (currentCookieName == null) { // We have not found the name yet, the
													// first key/value pair must be the
													// name and the value of the cookie.
					if (key.charAt(0) == '$') {
						// We cannot throw because Konqueror (4.2.2) is broken and
						// specifies $version as the first attribute.
						// throw new IllegalArgumentException("Invalid cookie: Name is not
						// the first attribute: " + httpHeader);
					...
					if (key.charAt(0) == '$')
						currentCookieContent.put(key, value);
					else {// We finished parsing of the current cookie, a new one starts
							// here.
							// if(singleCookie)
							// throw new ParseException("Invalid cookie header: Multiple
							// cookies specified but "
							// + " the name of the first cookie was not the first
							// attribute: " + httpHeader, i);

						cookies.add(new ReceivedCookie(currentCookieName, currentCookieContent)); // Store
																									// the
																									// previous
																									// cookie.
		...
		// Store the last cookie (the loop only stores the current cookie when a new one
		// starts).
