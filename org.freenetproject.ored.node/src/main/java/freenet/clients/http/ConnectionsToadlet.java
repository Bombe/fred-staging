...
public abstract class ConnectionsToadlet extends Toadlet {
	...
	protected class ComparatorByStatus implements Comparator<PeerNodeStatus> {
		...
		private int compareLocations(PeerNodeStatus firstNode, PeerNodeStatus secondNode) {
			double diff = firstNode.getLocation() - secondNode.getLocation(); // Can
																				// occasionally
																				// be the
																				// same,
																				// and we
	...
	public enum PeerAdditionReturnCodes {
		...
		OK, WRONG_ENCODING, CANT_PARSE, INTERNAL_ERROR, INVALID_SIGNATURE, TRY_TO_ADD_SELF, ALREADY_IN_REFERENCE
	...
	public void handleMethodGET(URI uri, final HTTPRequest request, ToadletContext ctx)
		...
		int numberOfNotConnected = numberOfTooNew + numberOfTooOld + numberOfNoLoadStats + numberOfDisconnected
		...
		if (node.isAdvancedModeEnabled()) {
			...
			titleCountString = (numberOfNotConnected + numberOfSimpleConnected) > 0
		...
		if (peerNodeStatuses.length > 0) {
			...
			if (advancedMode) {
				...
				overviewList.addChild("li",
						"darknetSizeEstimateSession:\u00a0" + networkSizeEstimateSession + "\u00a0nodes");
				if (nodeUptimeSeconds > HOURS.toSeconds(48)) {
					overviewList.addChild("li",
				...
				overviewList.addChild("li",
						"routingMissDistanceOverall:\u00a0" + fix4.format(routingMissDistanceOverall));
				...
				StatisticsToadlet.drawPeerStatsBox(peerStatsInfobox, advancedMode, numberOfConnected,
						...
						numberOfListenOnly, 0, 0, numberOfRoutingDisabled, numberOfClockProblem, numberOfConnError,
			...
			if (fProxyJavascriptEnabled) {
				...
				contentNode.addChild("script", new String[] { "type", "src" },
			...
			if (peerNodeStatuses.length == 0) {
				NodeL10n.getBase().addL10nSubstitution(peerTableInfoboxContent,
						"DarknetConnectionsToadlet.noPeersWithHomepageLink", new String[] { "link" },
				...
				if (enablePeerActions) {
					if (fProxyJavascriptEnabled) {
						peerTableHeaderRow.addChild("th").addChild("input", new String[] { "type", "onclick" },
				...
				peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "status")).addChild("#",
						l10n("statusTitle"));
				if (hasNameColumn())
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "name")).addChild(
							"span", new String[] { "title", "style" },
							new String[] { l10n("nameClickToMessage"), "border-bottom: 1px dotted; cursor: help;" },
							l10n("nameTitle"));
				if (hasTrustColumn())
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "trust")).addChild(
							"span", new String[] { "title", "style" },
				...
				if (hasVisibilityColumn())
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "trust")).addChild(
							"span", new String[] { "title", "style" },
							new String[] { l10n("visibilityMessage" + (advancedMode ? "Advanced" : "Simple")),
									"border-bottom: 1px dotted; cursor: help;" },
							l10n("visibilityTitle"));
				peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "address")).addChild(
						"span", new String[] { "title", "style" },
						new String[] { l10n("ipAddress"), "border-bottom: 1px dotted; cursor: help;" },
						l10n("ipAddressTitle"));
				peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "version")).addChild("#",
						l10n("versionTitle"));
				if (advancedMode) {
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "location"))
							.addChild("#", l10n("locationTitle"));
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "backoffRT"))
							.addChild("span", new String[] { "title", "style" }, new String[] {
									...
									"border-bottom: 1px dotted; cursor: help;" }, "Backoff (realtime)");
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "backoffBulk"))
							.addChild("span", new String[] { "title", "style" }, new String[] {
									...
									"border-bottom: 1px dotted; cursor: help;" }, "Backoff (bulk)");

					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "overload_p"))
							.addChild("span", new String[] { "title", "style" }, new String[] {
									"Probability of the node rejecting a request due to overload or causing a timeout.",
									"border-bottom: 1px dotted; cursor: help;" }, "Overload Probability");
				}
				peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "idle")).addChild("span",
						new String[] { "title", "style" },
						new String[] { l10n("idleTime"), "border-bottom: 1px dotted; cursor: help;" },
						l10n("idleTimeTitle"));
				if (hasPrivateNoteColumn())
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "privnote"))
							.addChild("span", new String[] { "title", "style" },
									new String[] { l10n("privateNote"), "border-bottom: 1px dotted; cursor: help;" },
									l10n("privateNoteTitle"));

				if (advancedMode) {
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "time_routable"))
							.addChild("#", "%\u00a0Time Routable");
					peerTableHeaderRow.addChild("th")
							.addChild("a", "href", sortString(isReversed, "selection_percentage"))
							.addChild("#", "%\u00a0Selection");
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "total_traffic"))
					...
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "time_delta"))
							.addChild("#", "Time\u00a0Delta");
					peerTableHeaderRow.addChild("th").addChild("a", "href", sortString(isReversed, "uptime"))
				...
				if (endCols != null) {
					for (SimpleColumn col : endCols) {
						...
						header.addChild("span", new String[] { "title", "style" },
								new String[] { NodeL10n.getBase().getString(col.getExplanationKey()),
										"border-bottom: 1px dotted; cursor: help;" },
				...
				// calculate the total selection rate using all peers, not just the peers
			...
			if (advancedMode) {
					...
					for (PeerNodeStatus peerNodeStatus : peerNodeStatuses) {
						...
						if (peersLoc != null) {
							for (double location : peersLoc) {
								...
								// FIXME Fix O(n^2): Use Arrays.binarySearch or use a
								// TreeMap.
								for (i = 0; i < max && locations.get(i) < location; i++)
									;
								// i now points to the proper location (equal, insertion
								// point, or end-of-list)
				...
				// transform complete.... now we have peers listed by foaf's ordered by
				// ascending location
				int trivialCount = 0;
				...
				peerTableInfoboxContent.addChild("b",
				...
				if (!showTrivialFoafConnections) {
					peerTableInfoboxContent.addChild("i",
							l10n("secondDegreeTrivialHiddenCount", "count", Integer.toString(trivialCount)));
					// @todo: add "show these" link
				}
				else {
					peerTableInfoboxContent.addChild("i",
							l10n("secondDegreeNonTrivialCount", "count", Integer.toString(nonTrivialCount)));
					// @todo: add "hide these" link
				}
				HTMLNode foafTable = peerTableInfoboxContent.addChild("table", "class", "darknet_connections"); // @todo:
																												// change
																												// css
																												// class?
				...
				for (int i = 0; i < max; i++) {
					...
					foafRow = foafTable.addChild("tr");
					{
						...
						for (PeerNodeStatus peerNodeStatus : peersWithFriend) {
							String address = ((peerNodeStatus.getPeerAddress() != null)
				...
				if (transitiveCount > 0) {
					peerTableInfoboxContent.addChild("i",
							l10n("secondDegreeAlsoOurs", "count", Integer.toString(transitiveCount)));
				}
			}
			// END FOAF TABLE
	...
	public void handleMethodPOST(URI uri, final HTTPRequest request, ToadletContext ctx)
		...
		if (request.isPartSet("add")) {
			...
			if (urltext.length() > 0) {
				...
				try {
					...
					this.sendErrorPage(ctx, 200, l10n("failedToAddNodeTitle"),
							NodeL10n.getBase().getString("DarknetConnectionsToadlet.cantFetchNoderefURL",
									new String[] { "url" }, new String[] { urltext }),
							!isOpennet());
					return;
				}
				finally {
				...
				// this slightly scary looking regexp chops any extra characters off the
			...
			// FIXME split by lines at this point rather than in addNewNode would be more
			// efficient
			int idx;
			...
			for (int i = 0; i < nodesToAdd.length; i++) {
				// We need to trim then concat 'End' to the node's reference, this way we
				// have a normal reference(the split() removes the 'End'-s!)
			...
			// Header of the table
			detailedStatusBox.addChild(new HTMLNode("tr")).addChildren(new HTMLNode[] {
			...
			for (PeerAdditionReturnCodes returnCode : PeerAdditionReturnCodes.values()) {
				if (results.containsKey(returnCode)) {
					...
					// occasions it happened. If the code is OK, we use green, red
					// elsewhere.
					statusBoxTable
							.addChild(new HTMLNode("tr", "style",
									"color:" + (returnCode == PeerAdditionReturnCodes.OK ? "green" : "red")))
							.addChildren(new HTMLNode[] {
	...
	private PeerAdditionReturnCodes addNewNode(String nodeReference, String privateComment, FRIEND_TRUST trust,
		...
		try {
			...
			fs.setEndMarker("End"); // It's always End ; the regex above doesn't always
									// grok this
		}
		catch (IOException e) {
	...
	protected void handleAltPost(URI uri, HTTPRequest request, ToadletContext ctx, boolean logMINOR)
		...
		handleMethodGET(uri, new ToadletHTTPRequest(uri, "GET"), ctx);
	}
	...
	/**
	 * Should there be a checkbox for each peer, and drawPeerActionSelectBox() be called
	 * directly after drawing the peers list?
	 */
	protected abstract boolean showPeerActionsBox();

	/**
	 * If showPeerActionsBox() is true, this will be called directly after drawing the
	 * peers table. A form has been added, and checkboxes added for each peer. This
	 * function should draw the rest of the form - any additional controls and one or more
	 * submit buttons.
	 */
	 ...
	 * @param showNoderef If true, render the text of the noderef so that it may be
	 * copy-pasted. If false, only show a link to download it.
	 */
	void drawNoderefBox(HTMLNode contentNode, SimpleFieldSet fs, boolean showNoderef) {
		...
		NodeL10n.getBase().addL10nSubstitution(headerReferenceInfobox, "DarknetConnectionsToadlet.myReferenceHeader",
				new String[] { "linkref", "linktext" }, new HTMLNode[] { REF_LINK, REFTEXT_LINK });
	...
	/**
	 * Draw the add a peer box. This comes immediately after the main peers table and
	 * before the noderef box. Implementors may skip it by not doing anything in this
	 * method.
	 */
	...
	protected static void drawAddPeerBox(HTMLNode contentNode, ToadletContext ctx, boolean isOpennet,
		...
		peerAdditionForm.addChild("textarea", new String[] { "id", "name", "rows", "cols" },
				new String[] { "reftext", "ref", "8", "74" });
		peerAdditionForm.addChild("br");
		peerAdditionForm.addChild("#", (l10n("urlReference") + ' '));
		peerAdditionForm.addChild("input", new String[] { "id", "type", "name" },
				new String[] { "refurl", "text", "url" });
		peerAdditionForm.addChild("br");
		peerAdditionForm.addChild("#", (l10n("fileReference") + ' '));
		peerAdditionForm.addChild("input", new String[] { "id", "type", "name" },
				new String[] { "reffile", "file", "reffile" });
		peerAdditionForm.addChild("br");
		if (!isOpennet) {
		...
		peerAdditionForm.addChild("input", new String[] { "type", "name", "value" },
	...
	private void drawRow(HTMLNode peerTable, PeerNodeStatus peerNodeStatus, boolean advancedModeEnabled,
			boolean fProxyJavascriptEnabled, long now, String path, boolean enablePeerActions, SimpleColumn[] endCols,
			boolean drawMessageTypes, double totalSelectionRate, DecimalFormat fix1) {
		...
		HTMLNode peerRow = peerTable.addChild("tr", "class", "darknet_connections_"
				+ (peerSelectionPercentage > PeerNode.SELECTION_PERCENTAGE_WARNING ? "warning" : "normal"));
		...
		/*
		 * Some status names have spaces, but a space separates key and value if not
		 * immediately followed by '='. Changing the names is not an option because they
		 * are exposed over FCP and TMCI.
		 */
		final String key = "ConnectionsToadlet.nodeStatus." + statusString.replace(' ', '_');
		peerRow.addChild("td", "class", "peer-status").addChild("span", "class", peerNodeStatus.getStatusCSSName(),
		...
		if (peerNodeStatus.getStatusValue() != PeerManager.PEER_NODE_STATUS_NEVER_CONNECTED
				&& (peerNodeStatus.isPublicInvalidVersion() || peerNodeStatus.isPublicReverseInvalidVersion())) { // Don't
																													// draw
																													// attention
																													// to
																													// a
		...
		if (!peerNodeStatus.isConnected() && (now - idle) > (2 * 7 * 24 * 60 * 60 * (long) 1000)) { // 2
																									// weeks
		...
		if (advancedModeEnabled) {
			...
			peerRow.addChild("td", "class", "peer-idle" /* FIXME */).addChild("#",
					SizeUtil.formatSize(received) + " / " + SizeUtil.formatSize(sent) + "/"
							+ SizeUtil.formatSize(resent) + " (" + fix1.format(((double) resent) / ((double) sent))
							+ ")");
			...
			if (t == null)
				...
				val = (int) t.getBandwidth() + "B/sec delay " + t.getDelay() + "ms (RTT " + t.getRoundTripTime()
						+ "ms window " + t.getWindowSize() + ')';
	...
	/**
	 * Draw the name column, if there is one. This will be directly after the status
	 * column.
	 */
	 ...
	 */
	protected void sendErrorPage(ToadletContext ctx, int code, String desc, String message, boolean returnToAddFriends)
