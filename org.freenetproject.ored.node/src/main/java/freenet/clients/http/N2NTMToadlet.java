...
public class N2NTMToadlet extends Toadlet {
	...
	/*
	 * File size limit is 1 MiB (1024*1024 bytes) or 5% of maximum Java memory, whichever
	 * is greater.
	 */
		...
		if (request.isPartSet("n2nm-upload") || request.isPartSet(LocalFileBrowserToadlet.selectFile)
			...
			for (DarknetPeerNode pn : peerNodes) {
				if (request.isPartSet("node_" + pn.hashCode())) {
								...
								if (size > 0) {
									...
									if (size > limit) {
										peerTableInfobox.addChild("#",
												l10n("tooLarge", new String[] { "attempt", "limit" },
					...
					peerRow.addChild("td", "class", sendStatusClass).addChild("span", new String[] { "title", "style" },
	...
	public static void createN2NTMSendForm(HTMLNode pageNode, boolean advancedMode, HTMLNode contentNode,
			ToadletContext ctx, HashMap<String, String> peers) throws ToadletContextClosedException, IOException {
		HTMLNode infobox = contentNode.addChild("div", new String[] { "class", "id" },
		...
		messageForm.addChild("textarea", new String[] { "id", "name", "rows", "cols" },
		...
		if (advancedMode) {
			...
			if (ctx.isAllowedFullAccess()) {
				...
				messageForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit",
						"n2nm-browse", NodeL10n.getBase().getString("QueueToadlet.insertFileBrowseButton") + "..." });
				messageForm.addChild("br");
			}
