...
public class WelcomeToadlet extends Toadlet {
	...
	/**
	 * Suffix {@link #path()} with "#" + BOOKMARKS_ANCHOR to deep link to the bookmark
	 * list
	 */
	...
	private void addCategoryToList(BookmarkCategory cat, HTMLNode list, boolean noActiveLinks, ToadletContext ctx) {
		...
		if (items.size() > 0) {
			// FIXME CSS noborder ...
			HTMLNode table = list.addChild("li").addChild("table", new String[] { "border", "style" },
					new String[] { "0", "border: none" });
			for (int i = 0; i < items.size(); i++) {
				...
				if (item.hasAnActivelink() && !noActiveLinks) {
					...
					cell.addChild("div", "style", "height: 36px; width: 108px;")
							...
							.addChild("img", new String[] { "src", "alt", "style", "title" }, new String[] { key,
									"activelink", "height: 36px; width: 108px", item.getDescription() });
				}
				else {
					cell.addChild("#", " ");
				}
				...
				boolean updated = item.hasUpdated(); // We use it twice so copy for thread
														// safety
				String linkClass = updated ? "bookmark-title-updated" : "bookmark-title";
				cell.addChild("a", new String[] { "href", "title", "class" },
						new String[] { '/' + item.getKey(), item.getDescription(), linkClass }, item.getVisibleName());
				...
				if (updated) {
					cell = row.addChild("td", "style", "border: none");
					cell.addChild(node.clientCore.alerts.renderDismissButton(item.getUserAlert(),
							path() + "#" + BOOKMARKS_ANCHOR));
				}
			}
		}
	...
	public boolean showSearchBox() {
		...
		return (node.pluginManager != null && node.pluginManager.isPluginLoaded("plugins.Library.Main"));
	}
	...
	public void addSearchBox(HTMLNode contentNode) {
		...
		if (showSearchBox()) {
			...
			searchForm.addChild("input", new String[] { "type", "size", "name" },
					new String[] { "text", "80", "search" });
			searchForm.addChild("input", new String[] { "type", "name", "value" },
					new String[] { "submit", "find", l10n("searchFreenet") });
			// Search must be in a new window so that the user is able to browse the
			// bookmarks.
			...
			NodeL10n.getBase().addL10nSubstitution(textSpan, "WelcomeToadlet.searchPluginLoading",
	...
	public void handleMethodPOST(URI uri, HTTPRequest request, ToadletContext ctx)
		...
		if (request.getPartAsStringFailsafe("updateconfirm", 32).length() > 0) {
			...
			HTMLNode content = ctx.getPageMaker().getInfobox("infobox-information", l10n("updatingTitle"), contentNode,
					null, true);
			...
			if (node.isUsingWrapper()) {
				...
				ctx.getPageMaker().getInfobox("infobox-error", l10n("threadDumpSubTitle"), contentNode,
						"thread-dump-generation", true).addChild("#", l10n("threadDumpNotUsingWrapper"));
			}
			...
			FCPUserAlert[] alerts = ctx.getAlertManager().getAlerts();
			for (FCPUserAlert alert : alerts) {
			...
			// FIXME If we support it from freesites we need a confirmation page with the
			// formPassword.
			...
			try {
				...
				NodeL10n.getBase().addL10nSubstitution(content, "WelcomeToadlet.keyInsertedSuccessfullyWithKeyAndName",
						new String[] { "link", "name" }, new HTMLNode[] { HTMLNode.link("/" + u), HTMLNode.text(u) });
			}
			...
			try {
				key = URLDecoder.decode(
						new FreenetURI(request.getPartAsStringFailsafe("key", Short.MAX_VALUE)).toURI("/").toString(),
						false);
			}
			catch (Exception e) {
			...
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			HTMLNode content = ctx.getPageMaker().getInfobox("infobox-query", l10n("shutdownConfirmTitle"), contentNode,
			...
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			HTMLNode content = ctx.getPageMaker().getInfobox("infobox-query", l10n("restartConfirmTitle"), contentNode,
			...
			for (FCPUserAlert alert : node.clientCore.alerts.getAlerts()) {
			...
			try {
				int outputBandwidthLimit = Fields
						.parseInt(request.getPartAsStringFailsafe("outputBandwidthLimit", Byte.MAX_VALUE));
			...
			try {
				int inputBandwidthLimit = Fields
						.parseInt(request.getPartAsStringFailsafe("inputBandwidthLimit", Byte.MAX_VALUE));
	...
	public void handleMethodGET(URI uri, HTTPRequest request, ToadletContext ctx)
		...
		if (ctx.isAllowedFullAccess()) {
			...
			if (request.isParameterSet("latestlog")) {
				final File logs = new File(
						node.config.get("logger").getString("dirname") + File.separator + "freenet-latest.log");
				...
				ctx.getPageMaker().getInfobox("infobox-information", l10n("shutdownDone"), contentNode,
						"shutdown-progressing", true).addChild("#", l10n("thanks"));
				...
				if (node.getDarknetConnections().length > 0) {
					...
					if (node.isFProxyJavascriptEnabled()) {
						addForm.addChild("script", new String[] { "type", "src" },
				...
				addForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit",
						"addbookmark", NodeL10n.getBase().getString("BookmarkEditorToadlet.addBookmark") });

				this.writeHTMLReply(ctx, 200, "OK", pageNode.generate());
				return;
			}
			else if (uri.getQuery() != null && uri.getQuery().startsWith("_CHECKED_HTTP_=")) {
				// Redirect requests for escaped URLs using the old destination to
				// ExternalLinkToadlet.
		...
		if (useragent != null) {
			...
			if ((useragent.indexOf("msie") > -1) && (useragent.indexOf("opera") == -1)) {
				ctx.getPageMaker().getInfobox("infobox-alert", l10n("ieWarningTitle"), contentNode,
						"internet-explorer-used", true).addChild("#", l10n("ieWarning"));
			}
		}
		...
		bookmarkBoxHeader.addChild("a", new String[] { "class", "title" },
				new String[] { "bookmarks-header-text",
		...
		if (ctx.isAllowedFullAccess() || !ctx.getContainer().publicGatewayMode()) {
			addCategoryToList(BookmarkManager.MAIN_CATEGORY, bookmarksList,
					(!container.enableActivelinks())
							|| (useragent != null && useragent.contains("khtml") && !useragent.contains("chrome")),
					ctx);
		}
		else {
			addCategoryToList(BookmarkManager.DEFAULT_CATEGORY, bookmarksList,
					(!container.enableActivelinks())
							|| (useragent != null && useragent.contains("khtml") && !useragent.contains("chrome")),
					ctx);
		}
		...
		HTMLNode versionContent = ctx.getPageMaker().getInfobox("infobox-information", l10n("versionHeader"),
				contentNode, "freenet-version", true);
	...
	private void putFetchKeyBox(ToadletContext ctx, HTMLNode contentNode) {
		// Fetch-a-key box
		HTMLNode fetchKeyContent = ctx.getPageMaker().getInfobox("infobox-normal", l10n("fetchKeyLabel"), contentNode,
				"fetch-key", true);
		fetchKeyContent.addAttribute("id", "keyfetchbox");
		HTMLNode fetchKeyForm = fetchKeyContent.addChild("form", new String[] { "method" }, new String[] { "POST" })
				.addChild("div");
		...
		fetchKeyForm.addChild("input", new String[] { "type", "name", "value" },
	...
	static HTMLNode sendRestartingPageInner(ToadletContext ctx) {
		...
		HTMLNode contentNode = page.content;
		ctx.getPageMaker()
				.getInfobox("infobox-information", l10n("restartingTitle"), contentNode, "shutdown-progressing", true)
	...
	/**
	 * Reads and returns the content of <code>logfile</code>. At most
	 * <code>byteLimit</code> bytes will be read. If <code>byteLimit</code> is less than
	 * the size of <code>logfile</code>, the first part of the file will be skipped. If
	 * this leaves a partial line at the beginning of the content to return, that partial
	 * line will also be skipped.
