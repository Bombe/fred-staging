...
public final class PageMaker {
	...
	public enum THEME {
		...
		BOXED("boxed", "Boxed (Top menu)", "", false, false), BOXED_CLASSIC("boxed-classic", "Boxed (Classic menu)", "",
				false, false), BOXED_DROPDOWN("boxed-dropdown", "Boxed (Dropdown menu)", "", false,
						false), BOXED_DYNAMIC("boxed-classic", "Boxed (Dynamic menu)", "", false, false), BOXED_STATIC(
								"boxed-static", "Boxed (Static menu)", "", false,
								false), CLEAN("clean", "Clean", "Mr. Proper", false, false), CLEAN_CLASSIC(
										"clean-classic", "Clean (Classic menu)", "Clean theme with a classic menu.",
										false, false), CLEAN_DROPDOWN("clean-dropdown", "Clean (Dropdown menu)",
												"Clean theme with a dropdown menu.", false,
												false), CLEAN_STATIC("clean-static", "Clean (Static menu)",
														"Clean theme with a static menu.", false,
														false), CLEAN_TOP("clean-top", "Clean (Top menu)",
																"Clean theme with a static top menu.", false,
																false), GRAYANDBLUE("grayandblue",
																		"Gray And Blue (Classic menu)", "", false,
																		false), GRAYANDBLUE_DYNAMIC(
																				"grayandblue-dynamic",
																				"Gray And Blue (Dynamic menu)", "",
																				false, false), GRAYANDBLUE_DROPDOWN(
																						"grayandblue-dropdown",
																						"Gray And Blue (Dropdown menu)",
																						"", false,
																						false), GRAYANDBLUE_STATIC(
																								"grayandblue-static",
																								"Gray And Blue (Static menu)",
																								"", false,
																								false), GRAYANDBLUE_TOP(
																										"grayandblue-top",
																										"Gray And Blue (Top menu)",
																										"", false,
																										false), SKY(
																												"sky",
																												"Sky (Top menu)",
																																						...
																																						false), MINIMALISTIC(
																																								...
																																								true), RABBIT_HOLE(
																																										...
																																										false), WINTERFACEY(
																																												"winterfacey",
																																												"Winterfacey",
																																												"2016th-theme, based on Winterface (Bootstrap)",
																																												true,
																																												false);

		public static final String[] possibleValues = { BOXED.code, BOXED_CLASSIC.code, BOXED_DROPDOWN.code,
				BOXED_DYNAMIC.code, BOXED_STATIC.code, CLEAN.code, CLEAN_CLASSIC.code, CLEAN_DROPDOWN.code,
				CLEAN_STATIC.code, CLEAN_TOP.code, GRAYANDBLUE.code, GRAYANDBLUE_DYNAMIC.code,
				GRAYANDBLUE_DROPDOWN.code, GRAYANDBLUE_STATIC.code, GRAYANDBLUE_TOP.code, SKY.code, SKY_CLASSIC.code,
				SKY_DROPDOWN.code, SKY_DYNAMIC.code, SKY_STATIC.code, MINIMALBLUE.code, MINIMALISTIC.code,
				RABBIT_HOLE.code, WINTERFACEY.code };
		...
		/**
		 * If true, the activelinks will appear on the welcome page, whether the user has
		 * enabled them or not.
		 */
		public final boolean forceActivelinks;

		/**
		 * If true, the "Fetch a key" infobox will appear above the bookmarks infobox on
		 * the welcome page.
		 */
		...
		private THEME(String code, String name, String description, boolean forceActivelinks,
	...
	private static class SubMenu {
		...
		public void removeNavigationLink(String name) {
			...
			navigationLinkL10n.remove(name); // Should this be here? If so, why not remove
												// from navigationLinkCallbacks too
		}
	...
	/**
	 * Remove a navigation link from a sub-menu. Applies globally, do not use this to
	 * customise menus when sending one page!
	 */
	...
	/**
	 * Generates an FProxy template page with optional navigation bar suitable for adding
	 * content to.
	 * @param title Title of the page.
	 * @param renderNavigationLinks Whether to render navigation links.
	 * @param ctx ToadletContext to use to render the page.
	 * @return A template PageNode.
	 * @deprecated Use {@link #getPageNode(String, ToadletContext, RenderParameters)}
	 * instead
	 */
	@Deprecated
	...
	public PageNode getPageNode(String title, boolean renderNavigationLinks, boolean renderStatus, ToadletContext ctx) {
		return getPageNode(title, ctx, new RenderParameters().renderNavigationLinks(renderNavigationLinks)
	 ...
	 * Generates an FProxy template page suitable for adding content to.
	 * @param title Title of the page.
	 * @param ctx ToadletContext to use to render the page. Can be null, e.g. if the HTML
	 * is not being generated as part of a toadlet request, for example if it's using the
	 * old FredPluginHTTP interface.
	 * @param renderParameters Parameters for inclusion or omission of certain page
	 * elements
	 * @return A template PageNode.
	 */
	public PageNode getPageNode(String title, ToadletContext ctx, RenderParameters renderParameters) {
		...
		// To make something only rendered when javascript is on, then add the jsonly
		// class to it
		headNode.addChild("noscript").addChild("style", " .jsonly {display:none;}");
		if (override != null)
			headNode.addChild(getOverrideContent());
		else
			headNode.addChild("link", new String[] { "rel", "href", "type", "title" }, new String[] { "stylesheet",
					"/static/themes/" + theme.code + "/theme.css", "text/css", theme.code });
		...
		if (sendAllThemes) {
			for (THEME t : THEME.values()) {
				...
				headNode.addChild("link", new String[] { "rel", "href", "type", "media", "title" },
						new String[] { "alternate stylesheet", "/static/themes/" + themeName + "/theme.css", "text/css",
								"screen", themeName });
			}
		}
		...
		if (renderParameters.isRenderStatus() && fullAccess) {
			final HTMLNode statusBarDiv = pageDiv.addChild("div", "id", "statusbar-container").addChild("div", "id",
					"statusbar");
			...
			if (node.clientCore != null && ctx != null && renderParameters.isRenderModeSwitch()) {
				...
				Map<String, List<String>> parameters = ToadletHTTPRequest.parseUriParameters(uri, true);
				...
				switchMode.addChild("a", "href", "?" + ToadletHTTPRequest.createQueryString(parameters, false),
			...
			if (node != null && node.clientCore != null) {
				...
				progressBar.addChild("div", new String[] { "class", "title" },
						new String[] { "progress_fraction_finalized",
								NodeL10n.getBase().getString("StatusBar.connectedPeers", new String[] { "X", "Y" },
	...
	/**
	 * Filters a given string so that it will be a valid CSS identifier. It replaces all
	 * characters that are not a dash, underscore, or alphanumeric with an underscore. If
	 * the first character is a dash and the second character is not a letter or
	 * underscore, replaces the second character with an underscore. This filter is overly
	 * strict as it does not allow non-ASCII characters or escapes. If the given string is
	 * below two characters in length, it appends underscores until it is not.
	...
	/**
	 * Bundles parameters that are used to create the page node. The default for the
	 * render parameters is to include all optional render tasks. Individual tasks may be
	 * enabled or disabled by calling the appropriate methods which returns a new
	 * {@link RenderParameters} object as {@link RenderParameters} are immutable.
	 *
	...
	public static class RenderParameters {
		 ...
		 * @param renderNavigationLinks {@code true} to include navigation links in the
		 * page
		 * @param renderStatus {@code true} to include the status bar in the page
		 * @param renderModeSwitch {@code true} to include the mode switch in the status
		 * bar
		 */
		 ...
		 * @return {@code true} if the navigation links should be included in the page,
		...
		/**
		 * Returns a new {@link RenderParameters} object that renders the navigation links
		 * according to the given parameter.
		 * @param renderNavigationLinks {@code true} to render the navigation links,
		 * {@code false} otherwise
		 ...
		 * @return {@code true} if the status bar should be included in the page,
		 * {@code false} otherwise
		 */
		...
		/**
		 * Returns a new {@link RenderParameters} object that renders the status bar
		 * according to the given parameter.
		 ...
		 * @return {@code true} if the mode switch should be included in the page,
		 * {@code false} otherwise
		 */
		...
		/**
		 * Returns a new {@link RenderParameters} object that renders the mode switch
		 * according to the given parameter.
