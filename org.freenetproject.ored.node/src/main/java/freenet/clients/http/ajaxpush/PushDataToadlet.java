...
/**
 * A toadlet that provides the current data of pushed elements. It requires the requestId
 * and the elementId parameters.
 */
public class PushDataToadlet extends Toadlet {
	...
	public void handleMethodGET(URI uri, HTTPRequest req, ToadletContext ctx)
		...
		elementId = elementId.replace(" ", "+");// This is needed, because BASE64 has '+',
												// but it is a HTML escape for ' '
		if (logMINOR) {
			Logger.minor(this, "Getting data for element:" + elementId);
		}
		BaseUpdateableElement node = ((SimpleToadletServer) ctx.getContainer()).pushDataManager
				.getRenderedElement(requestId, elementId);
		if (logMINOR) {
			Logger.minor(this, "Data got element:" + node.generateChildren());
		}
		writeHTMLReply(ctx, 200, "OK",
				UpdaterConstants.SUCCESS + ":" + Base64.encodeStandard(node.getUpdaterType().getBytes("UTF-8")) + ":"
						+ Base64.encodeStandard(node.generateChildren().getBytes("UTF-8")));
	}
