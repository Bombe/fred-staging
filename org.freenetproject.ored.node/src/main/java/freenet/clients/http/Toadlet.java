 ...
 * continuations, but that hasn't been implemented, and modern servlets do support
 * continuations anyway. Also it was supposed to be simpler, which may not be true any
 * more. Many plugins wrap their own API around this! FIXME consider using servlets.
 *
 * Important API complexity: The methods for handling the actual requests are synthetic:
 *
 * Methods are handled via handleMethodGET/POST/whatever ( URI uri, HTTPRequest request,
 * ToadletContext ctx ) Typically this throws IOException and
 * ToadletContextClosedException.
 */
public abstract class Toadlet {

	/**
	 * Handle a GET request. Other methods are accessed via handleMethodPOST etc, invoked
	 * via reflection. But all toadlets are expected to support GET.
	 * @param uri The URI being fetched.
	 * @param request The original HTTPRequest, convenient for e.g. fetching ?blah=blah
	 * parameters.
	 * @param ctx The request context. Mainly used for sending a reply; this identifies
	 * which request we are replying to. Also gives access to lots of important objects
	 * e.g. PageMaker.
	 */
	...
	/**
	 * Primary purpose of this function is being overridden in your Toadlet
	 * implementations - for the following purpose:
	 *
	 * When displaying this Toadlet, the web interface should show the menu from which it
	 * was selected as opened, and mark the appropriate entry as selected in the menu.
	 * This function may return the Toadlet whose menu shall be opened and whose entry
	 * shall be marked as selected in the menu.
	 *
	 * It is necessary to have this function instead of just marking
	 * <code>Toadlet.this</code> as selected: Some Toadlets won't be added to a menu. They
	 * will be only accessible through other Toadlets. For example a Toadlet for deleting
	 * a single download might only be accessible through the Toadlet which shows all
	 * downloads. For still being able to figure out the menu entry through which those
	 * so-called invisible Toadlets where accessed, this function is necessary.
	 * @param context Can be used to decide the return value, for example to check session
	 * cookies using {@link SessionManager}.
	 * @return The result of {@link #showAsToadlet()}, which is <code>this</code> by
	 * default.<br>
	 * This behavior is for backwards compatibility with existing code which overrides
	 * that function.<br>
	 * <br>
	 *
	...
	/**
	 * @deprecated Use {@link #showAsToadlet(ToadletContext)} instead. Internally fred
	 * will always call that function, which calls this function by default, so old code
	 * which only overrides this function still works. TODO: When removing this deprecated
	 * function, change {@link #showAsToadlet(ToadletContext)} to return <code>this</code>
	 * by default, as already specified in its JavaDoc.
	 * @return <code>this</code>
	 */
	@Deprecated
	public Toadlet showAsToadlet() {
		// DO NOT CHANGE THIS ANYMORE: Otherwise showAsToadlet(ToadletContext) will not
		// follow the
	...
	/**
	 * Override to return true if the toadlet should handle POSTs that don't have the
	 * correct form password. Otherwise they will be rejected and not passed to the
	 * toadlet.
	 */
	...
	/**
	 * Which methods are supported by this Toadlet. Should return a string containing the
	 * methods supported, separated by commas For example: "GET, PUT" (in which case both
	 * 'handleMethodGET()' and 'handleMethodPUT()' must be implemented).
	 *
	 * IMPORTANT: This will discover inherited methods because of getMethod() below. If
	 * you do not want to expose a method implemented by a parent class, then *OVERRIDE
	 * THIS METHOD*.
	 */
	...
	/**
	 * Client calls from the above messages to run a Freenet request. This method may
	 * block (or suspend).
	 * @param maxSize Maximum length of returned content.
	 * @param clientContext Client context object. This should be the same for any group
	 * of related requests, but different for any two unrelated requests. Request
	 * selection round-robin's over these, within any priority and retry count class, and
	 * above the level of individual block fetches.
	 */
	...
	/**
	 * Write an HTTP response, e.g. a page, an image, an error message, with no special
	 * headers.
	 ...
	 */
	protected void writeReply(ToadletContext ctx, int code, String mimeType, String desc, byte[] data, int offset,
	 ...
	 * @param mimeType The MIME type of the data we are returning.
	 * @param desc The HTTP response description for the code.
	 * @param data The Bucket which contains the reply data. This function assumes
	 * ownership of the Bucket, calling free() on it when done. If this behavior is
	 * undesired, callers can wrap their Bucket in a NoFreeBucket.
	 *
	 * @see freenet.bucket.NoFreeBucket
	 */
	...
	/**
	 * Write an HTTP response, e.g. a page, an image, an error message, possibly with
	 * custom headers, for example, we may want to send a redirect, or a file with a
	 * specified filename.
	...
	protected void writeReply(ToadletContext context, int code, String mimeType, String desc,
			MultiValueTable<String, String> headers, Bucket data) throws ToadletContextClosedException, IOException {
		context.sendReplyHeaders(code, desc, headers, mimeType, data.size());
		context.writeData(data);
	}

	/**
	 * Write a text-based HTTP response, e.g. a page or an error message, with no special
	 * headers.
	 * @param ctx The specific request to reply to.
	 * @param code The HTTP reply code to use.
	 * @param mimeType The MIME type of the data we are returning.
	 * @param desc The HTTP response description for the code.
	 * @param reply The reply data, as a String (so only use this for text-based replies,
	 * e.g. HTML, plain text etc).
	 */
	protected void writeReply(ToadletContext ctx, int code, String mimeType, String desc, String reply)
			throws ToadletContextClosedException, IOException {
	...
	/**
	 * Write an HTTP response as HTML, possibly with custom headers, for example, we may
	 * want to send a redirect, or a file with a specified filename.
	 * @param ctx The specific request to reply to.
	 * @param code The HTTP reply code to use.
	 * @param desc The HTTP response description for the code.
	 * @param headers The additional HTTP headers to send.
	 * @param reply The HTML page, as a String.
	 */
	protected void writeHTMLReply(ToadletContext ctx, int code, String desc, MultiValueTable<String, String> headers,
	 ...
	 */
	protected void writeHTMLReply(ToadletContext ctx, int code, String desc, MultiValueTable<String, String> headers,
			String reply, boolean forceDisableJavascript) throws ToadletContextClosedException, IOException {
		writeReply(ctx, code, "text/html; charset=utf-8", desc, headers, reply, forceDisableJavascript);
	}

	/**
	 * Write an HTTP response as plain text, possibly with custom headers, for example, we
	 * may want to send a redirect, or a file with a specified filename.
	 * @param ctx The specific request to reply to.
	 * @param code The HTTP reply code to use.
	 * @param desc The HTTP response description for the code.
	 * @param headers The additional HTTP headers to send.
	 * @param reply The text of the page, as a String.
	 */
	protected void writeTextReply(ToadletContext ctx, int code, String desc, MultiValueTable<String, String> headers,
			String reply) throws ToadletContextClosedException, IOException {
		writeReply(ctx, code, "text/plain; charset=utf-8", desc, headers, reply, true);
	}

	protected void writeReply(ToadletContext context, int code, String mimeType, String desc,
			MultiValueTable<String, String> headers, String reply) throws ToadletContextClosedException, IOException {
		writeReply(context, code, mimeType, desc, headers, reply, false);
	}

	protected void writeReply(ToadletContext context, int code, String mimeType, String desc,
			MultiValueTable<String, String> headers, String reply, boolean forceDisableJavascript)
			throws ToadletContextClosedException, IOException {
	...
	/**
	 * Write a generated HTTP response, e.g. a page, an image, an error message, possibly
	 * with custom headers, for example, we may want to send a redirect, or a file with a
	 * specified filename. This should not be used for fproxy content i.e. content
	 ...
	 *
	 * This will write rudimentary HTML, but typically browsers will follow the Location
	 * header. TODO Refactor with writeTemporaryRedirect.
	 * @param ctx
	 * @param msg
	 * @param location
	...
	static void writePermanentRedirect(ToadletContext ctx, String msg, String location)
		...
		String redirDoc = "<html><head><title>" + msg + "</title></head><body><h1>"
				+ l10n("permRedirectWithReason", "reason", msg) + "</h1><a href=\"" + HTMLEncoder.encode(location)
	 ...
	 *
	 * This will write rudimentary HTML, but typically browsers will follow the Location
	 * header. TODO Refactor with writePermanentRedirect.
	 * @param ctx
	...
	protected void writeTemporaryRedirect(ToadletContext ctx, String msg, String location)
		...
		if (msg == null)
			...
			msg = HTMLEncoder.encode(msg);
		String redirDoc = "<html><head><title>" + msg + "</title></head><body><h1>"
				+ l10n("tempRedirectWithReason", "reason", msg) + "</h1><a href=\"" + HTMLEncoder.encode(location)
				+ "\">" + l10n("clickHere") + "</a></body></html>";
		byte[] buf;
		try {
			buf = redirDoc.getBytes("UTF-8");
	 ...
	 * @throws ToadletContextClosedException See
	 * {@link #sendErrorPage(ToadletContext, int, String, String)}
	 */
	...
	/**
	 * Get the client impl. DO NOT call the blocking methods on it!! Just use it for
	 * configuration etc.
	 */
