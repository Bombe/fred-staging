...
/**
 * ToadletContext implementation, including all the icky HTTP parsing etc. An actual
 * ToadletContext object represents a request, after we have parsed the headers. It
 * provides methods to send replies.
 *
 * @author root
 *
 */
public class ToadletContextImpl implements ToadletContext {
	...
	private static final Class<?> HANDLE_PARAMETERS[] = new Class<?>[] { URI.class, HTTPRequest.class,
			ToadletContext.class };

	/*
	 * methods listed here are *not* configurable with AllowData annotation
	 */
	...
	private ArrayList<ReceivedCookie> cookies; // Null until the first time the user
												// queries us for a ReceivedCookie.

	private ArrayList<Cookie> replyCookies; // Null until the first time the user sets a
											// Cookie.
	...
	private static String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase().getString("ToadletContextImpl." + key, new String[] { pattern },
				new String[] { value });
	}

	/**
	 * Send an error message. Caller provides the HTTP code, reason string, and a message,
	 * which will become the title and the h1'ed contents of the error page.
	 */
	 ...
	 * @param httpReason The HTTP reason string for the HTTP status code. Do not make
	 * stuff up, use the official reason string, or some browsers may break.
	...
	public void sendReplyHeaders(int code, String desc, MultiValueTable<String, String> mvt, String mimeType,
	...
	public void sendReplyHeadersStatic(int replyCode, String replyDescription, MultiValueTable<String, String> mvt,
			String mimeType, long contentLength, Date mTime) throws ToadletContextClosedException, IOException {
		if (mTime == null)
	...
	@Override
	public void sendReplyHeadersFProxy(int replyCode, String replyDescription, MultiValueTable<String, String> mvt,
			String mimeType, long contentLength) throws ToadletContextClosedException, IOException {
	...
	private void sendReplyHeaders(int replyCode, String replyDescription, MultiValueTable<String, String> mvt,
			String mimeType, long contentLength, Date mTime, boolean isOutlinkConfirmationPage, boolean allowFrames,
		...
		if (replyCookies != null) {
			...
			// We do NOT use "set-cookie2" even though we should according though RFC2965
			// - Firefox 3.0.14 ignores it for me!
	...
	public ReceivedCookie getCookie(URI domain, URI path, String name) throws ParseException {
		...
		// RFC2965: Two cookies are equal if name and domain are equal with
		// case-insensitive comparison and path is equal with case-sensitive comparison.
		// getName() / getDomain() returns lowercase and getPath() returns the original
		// path.

		// UNFORTUNATELY firefox will ONLY give us the name and the value of the cookie,
		...
		for (ReceivedCookie cookie : cookies) {
			try {
				...
				// }
				//
				// if(cookie.getPath().toString().equals(stringPath) &&
	...
	static void sendReplyHeaders(OutputStream sockOutputStream, int replyCode, String replyDescription,
			MultiValueTable<String, String> mvt, String mimeType, long contentLength, Date mTime, boolean disconnect,
			boolean allowScripts, boolean allowFrames) throws IOException {
		...
		String cacheControl;
		if (allowCaching) {
			// use an expiry time of 30 day from now, about the frequency of Freenet
			// releases
	...
	private static String generateCSP(boolean allowScripts, boolean allowFrames) {
		...
		sb.append(allowScripts ? "'self' 'unsafe-inline'; options inline-script" : generateRestrictedScriptSrc());
		sb.append("; frame-src ");
		sb.append(allowFrames ? "'self'" : "'none'");
		sb.append("; object-src 'none'");
		// Always send unsafe-inline for CSS. This is safe given it can't use external
		// stuff anyway.
	...
	private static String generateRestrictedScriptSrc() {
		...
		String[] allowedScriptHashes = new String[] { "sha256-dLiB4u0FXPxUpecGDu2olWWLKRY+mS4gBZh2u4jdAtk=" // freenet/clients/http/staticfiles/js/m3u-player.js
		};
	...
	/**
	 * Fix key case to be conformant to HTTP expectations. Note that HTTP is case
	 * insensitive on header names, but we may as well send something as close to the spec
	 * as possible in case of broken clients...
	 */
	...
	public static void handle(Socket sock, ToadletContainer container, PageMaker pageMaker,
		...
		try {
			...
			while (true) {
				...
				String firstLine = lis.readLine(32768, 128, false); // ISO-8859-1 or
																	// US-ASCII, _not_
																	// UTF-8
				if (firstLine == null) {
					sock.close();
				...
				try {
					...
					if (logMINOR)
						Logger.minor(ToadletContextImpl.class,
								"URI: " + uri + " path " + uri.getPath() + " host " + uri.getHost() + " frag "
										+ uri.getFragment() + " port " + uri.getPort() + " query " + uri.getQuery()
				...
				while (true) {
					String line = lis.readLine(32768, 128, false); // ISO-8859 or
																	// US-ASCII, not UTF-8
					if (line == null) {
						sock.close();
						return;
				...
				/*
				 * copy the data into a bucket now, before we go into the redirect loop
				 */
				...
				if (slen != null) {
					...
					if (allowPost && ((!container.publicGatewayMode()) || ctx.isAllowedFullAccess())) {
						...
						if (method.equals("POST")) {
							...
							sendError(sock.getOutputStream(), 403, "Forbidden",
									"Content not allowed in this configuration", true, null);
						}
						ctx.close();
						return;
					}
				...
				try {
					...
					while (redirect) {
						...
						// if the Toadlet does not support the method, we don't need to
						// parse the data
						// also due this pre check a 'NoSuchMethodException' should never
						// appear
						...
						ToadletHTTPRequest req = new ToadletHTTPRequest(uri, data, ctx, method);

						// require form password if it's a POST, unless the toadlet
						// requests otherwise
			...
			try {
				sendError(sock.getOutputStream(), 400, "Bad Request",
						l10n("parseErrorWithError", "error", e.getMessage()), true, null);
			}
			...
			try {
				String msg = "<html><head><title>" + NodeL10n.getBase().getString("Toadlet.internalErrorTitle")
						+ "</title></head><body><h1>"
						+ NodeL10n.getBase().getString("Toadlet.internalErrorPleaseReport") + "</h1><pre>";
				StringWriter sw = new StringWriter();
				PrintWriter pw = new PrintWriter(sw);
	...
	private static void callToadletMethod(Toadlet t, String method, URI uri, ToadletHTTPRequest req,
