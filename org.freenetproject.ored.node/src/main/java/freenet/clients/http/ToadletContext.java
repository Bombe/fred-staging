...
/**
 * Object represents context for a single request. Is used as a token, when the Toadlet
 * wants to e.g. write a reply.
 */
public interface ToadletContext {
	 ...
	 * @param length The length of the reply. as a whole.
	 */
	void sendReplyHeaders(int code, String desc, MultiValueTable<String, String> mvt, String mimeType, long length)
	 ...
	 * @param forceDisableJavascript Disable javascript even if it is enabled for the web
	...
	/**
	 * @deprecated Write reply headers for either generated content (web interface pages)
	 * or static content. Callers should use either sendReplyHeaders() or
	 ...
	 */
	void sendReplyHeadersStatic(int code, String desc, MultiValueTable<String, String> mvt, String mimeType,
			long length, Date mTime) throws ToadletContextClosedException, IOException;

	/**
	 * Write reply headers for content downloaded from Freenet. Progress bars etc are not
	 * content downloaded from Freenet, so are rendered using sendReplyHeaders(). For
	 * content downloaded from Freenet we send headers which absolutely forbid Javascript,
	 * even if it somehow got through the content filter.
	 * @param code HTTP code.
	 * @param desc HTTP code description.
	 * @param mvt Any extra headers. Can be null.
	 * @param mimeType The MIME type of the reply.
	 * @param length The length of the reply.
	 */
	void sendReplyHeadersFProxy(int code, String desc, MultiValueTable<String, String> mvt, String mimeType,
	...
	/**
	 * Force a disconnection after handling this request. Used only when a throwable was
	 * thrown and we don't know what the state of the connection is. FIXME we could handle
	 * this better by remembering whether headers have been sent, how long the attached
	 * data should be, how much data has been sent etc.
	 */
	 ...
	 * @param data The Bucket which contains the data. This function assumes ownership of
	 * the Bucket, calling free() on it when done. If this behavior is undesired, callers
	...
	/**
	 * Check a request for the form password, and send an error to the client if the
	...
	/**
	 * Check a request for the form password. Some Toadlet's will want to e.g. send a
	 * confirmation page using the submitted data if the form password isn't present.
	 * @throws IOException
	 */
	 ...
	 *
	 * If it is false, an error page is sent to the client, and false is returned. You can
	 * then abort processing of the request.
	 ...
	 * {@link Toadlet#sendUnauthorizedPage(ToadletContext)}
	 */
	...
	/**
	 * Add a form node to an HTMLNode under construction. This will have the correct
	 * enctype and formPassword set already, so all the caller needs to do is add its
	 * specific fields.
	 * @param parentNode The parent HTMLNode.
	 * @param target Where the form should be POSTed to.
	 * @param id HTML name for the form for stylesheet/script access. Will be added as
	 * both id and name.
	...
	/**
	 * Is this Toadlet allowed full access to the node, including the ability to
	 * reconfigure it, restart it etc?
	 */
