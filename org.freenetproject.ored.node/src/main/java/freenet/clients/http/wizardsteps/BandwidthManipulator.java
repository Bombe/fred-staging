...
/**
 * Utility class used by bandwidth steps to detect and set bandwidth, and set the wizard
 * completion flag.
 */
public abstract class BandwidthManipulator {
	 ...
	 * @throws freenet.config.InvalidConfigValueException If the value is negative, a
	 * number cannot be parsed from it, or the value is too low to be usable.
	...
	protected HTMLNode parseErrorBox(HTMLNode parent, PageHelper helper, String message) {
		HTMLNode infoBox = helper.getInfobox("infobox-warning", WizardL10n.l10n("bandwidthErrorSettingTitle"), parent,
				null, false);
	 ...
	 * @return Upstream and downstream bandwidth in bytes per second. If a limit is set to
	 * -1, it is unavailable or nonsensically low. In case of error, both values are set
	 * to: -2 if the upstream bandwidth setting has already been configured. -3 if the
	 * UPnP plugin is not loaded or done starting up.
	 */
	protected BandwidthLimit detectBandwidthLimits() {
		...
		Logger.normal(this, "bandwidthIndicator reports downstream " + downstreamBits + " bits/s and upstream "
				+ upstreamBits + " bits/s.");
	...
	protected void setWizardComplete() {
		...
		try {
			...
			// TODO: Is there anything that can reasonably be done about this? What kind
