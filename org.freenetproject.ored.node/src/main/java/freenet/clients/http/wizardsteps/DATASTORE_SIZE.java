...
/**
 * Allows the user to select datastore size, considering available storage space when
 * offering options.
 */
public class DATASTORE_SIZE implements Step {
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		...
		below.addChild("input", new String[] { "type", "name", "value" },
	...
	public String postStep(HTTPRequest request) {
		// drop down options may be 6 chars or less, but formatted ones e.g. old value if
		// re-running can be more
	...
	private long maxDatastoreSize() {
		...
		// There are 3 types of keys. We want the number of { SSK, CHK, pubkey } i.e. the
		// number of slots in each store.
		slots /= 3;
		// We return the total size, so we don't need to worry about cache vs store or
		// even client cache.
		// One key of all 3 types combined uses Node.sizePerKey bytes on disk. So we get a
		// size.
	...
	private long canAutoconfigureDatastoreSize() {
		...
		if (freeSpace <= 0) {
			...
			if (freeSpace > 50 * oneGiB) {
				...
				shortSize = Math.max(10 * oneGiB, Math.min(freeSpace / 10, Math.min(diskIoMax, bloomFilter128MiBMax)));
			}
