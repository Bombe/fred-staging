...
/**
 * This step allows the user to choose between security levels. If opennet is disabled,
 * only high and maximum are shown. If opennet is enabled, only low and normal are shown.
 */
public class SECURITY_NETWORK implements Step {
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		...
		if (request.isParameterSet("confirm")) {
			...
			SecurityLevels.NETWORK_THREAT_LEVEL newThreatLevel = SecurityLevels
					.parseNetworkThreatLevel(networkThreatLevel);
			...
			if (newThreatLevel == SecurityLevels.NETWORK_THREAT_LEVEL.MAXIMUM) {
				...
				NodeL10n.getBase().addL10nSubstitution(p, "SecurityLevels.maxSecurityYouNeedFriends",
						new String[] { "bold" }, new HTMLNode[] { HTMLNode.STRONG });
				formNode.addChild("p").addChild("input", new String[] { "type", "name", "value" },
				...
				HTMLNode checkbox = formNode.addChild("p").addChild("input", new String[] { "type", "name", "value" },
						new String[] { "checkbox", "security-levels.networkThreatLevel.confirm", "off" });
				NodeL10n.getBase().addL10nSubstitution(checkbox,
						"FirstTimeWizardToadlet.highNetworkThreatLevelCheckbox", new String[] { "bold", "addAFriend" },
						new HTMLNode[] { HTMLNode.STRONG,
								new HTMLNode("#", NodeL10n.getBase().getString("FProxyToadlet.addFriendTitle")), });
			}
			formNode.addChild("input", new String[] { "type", "name", "value" },
	...
	/**
	 * Adds to the given parent node description and a radio button for the selected
	 * security level.
	...
	private void securityLevelChoice(HTMLNode parent, SecurityLevels.NETWORK_THREAT_LEVEL level) {
		HTMLNode input = parent.addChild("p").addChild("input", new String[] { "type", "name", "value", "id" },
				new String[] { "radio", "security-levels.networkThreatLevel", level.name(),
						"security-levels.networkThreatLevel" + level.name() });
		input.addChild("label", new String[] { "for" },
	...
	public String postStep(HTTPRequest request) {
		...
		/*
		 * If the user didn't select a network security level before clicking continue or
		 * the selected security level could not be determined, redirect to the same page.
		 */
		...
		if (request.isPartSet("return-from-confirm")) {
			...
			if (isInPreset) {
				...
				return FirstTimeWizardToadlet
						.getPreviousStep(FirstTimeWizardToadlet.WIZARD_STEP.SECURITY_NETWORK, persistFields.preset)
						.name();
			}
		...
		if ((newThreatLevel == SecurityLevels.NETWORK_THREAT_LEVEL.MAXIMUM
			...
			// Make the user aware of the effects of high or maximum network threat if
			// selected.
			...
			if ((!request.isPartSet("security-levels.networkThreatLevel.confirm"))
				...
				return redirectTo.toString();
					...
					&& request.isPartSet("security-levels.networkThreatLevel.tryConfirm")) {
				// If the user did not check the box and clicked next, redisplay the
				// prompt.
				displayConfirmationBox(redirectTo, networkThreatLevel);
				return redirectTo.toString();
			}
		}
		// The user selected low or normal security, or confirmed high or maximum. Set the
		// configuration
