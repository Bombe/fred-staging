...
/**
 * Allows the user to set bandwidth limits with an emphasis on limiting to certain
...
public class BANDWIDTH_RATE extends BandwidthManipulator implements Step {
	...
	public BANDWIDTH_RATE(NodeClientCore core, Config config) {
		...
		limits = new BandwidthLimit[] {
				// FIXME feedback on typical real world ratios on slow connections would
				// be helpful.

				// // Dial-up
				// // 57.6/33.6; call it 4KB/sec each way
				// new BandwidthLimit(4*KiB, 4*KiB, "bandwidthConnectionDialUp"),
				// // 128kbps symmetrical = 16KB/sec each way, take half so 8KB/sec each
				// way
				...
				// 6Mbps/256kbps - 6Mbps is common in parts of china, as well as being the
				// real value in lots of DSL areas
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		...
		HTMLNode infoBox = helper.getInfobox("infobox-normal", WizardL10n.l10n("bandwidthLimitRateTitle"), formNode,
				null, false);
		...
		customForm.addChild("td").addChild("input", new String[] { "type", "name" },
		...
		infoBox.addChild("input", new String[] { "type", "name", "value" },
		...
		if (!limitSelected.isEmpty()) {
			...
			if (x != -1) {
				...
				if (!preset.isEmpty()) {
					...
					// This should not happen, as there are no units to confound the
					// parser.
	 ...
	 * @return a space-separated string of the messages from any exceptions thrown when
	 * setting limits. If both are successful, an empty string.
	 */
	...
	/**
	 * Adds a row to the table for the given limit. Adds download limit, upload limit, and
	 * selection button.
	 ...
	 * @param recommended Whether to mark the limit with (Recommended) next to the select
	 * button.
	 */
	private void addLimitRow(HTMLNode table, PageHelper helper, BandwidthLimit limit, boolean recommended,
		...
		HTMLNode radio = buttonCell.addChild("input", new String[] { "type", "name", "value" },
