...
public class SECURITY_PHYSICAL implements Step {
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		...
		HTMLNode infoboxContent = helper.getInfobox("infobox-normal", WizardL10n.l10nSec("physicalThreatLevelShort"),
		...
		NodeL10n.getBase().addL10nSubstitution(swapWarning, "SecurityLevels.physicalThreatLevelTruecrypt",
				new String[] { "bold", "truecrypt" }, new HTMLNode[] { HTMLNode.STRONG,
						HTMLNode.linkInNewWindow(ExternalLinkToadlet.escape("http://www.truecrypt.org/")) });
		OperatingSystem os = FileUtil.detectedOS;
		div.addChild("p", NodeL10n.getBase().getString("SecurityLevels.physicalThreatLevelSwapfile", "operatingSystem",
		...
		for (SecurityLevels.PHYSICAL_THREAT_LEVEL level : SecurityLevels.PHYSICAL_THREAT_LEVEL.values()) {
			...
			input.addChild("label", new String[] { "for" }, new String[] { controlName + level.name() }).addChild("b",
					WizardL10n.l10nSec("physicalThreatLevel.name." + level));
			input.addChild("#", ": ");
			...
			if (level == SecurityLevels.PHYSICAL_THREAT_LEVEL.HIGH
				...
				p.addChild("input", new String[] { "id", "type", "name" },
	...
	private boolean errorHandler(HTTPRequest request, PageHelper helper) {
		...
		SecurityLevels.PHYSICAL_THREAT_LEVEL newThreatLevel = SecurityLevels
				.parsePhysicalThreatLevel(physicalThreatLevel);
		...
		if (error.equals("pass")) {
			...
			HTMLNode content = helper.getInfobox("infobox-error", WizardL10n.l10nSec(infoboxTitleKey), contentNode,
					null, true);
			...
			SecurityLevelsToadlet.sendCantDeleteMasterKeysFileInner(helper, core.node.getMasterPasswordFile().getPath(),
					newThreatLevel.name());
			return true;
		}
	...
	public String postStep(HTTPRequest request) throws IOException {
		...
		/*
		 * If the user didn't select a network security level before clicking continue,
		 * the selected security level could not be determined, clicked back from a
		 * password error page, redirect to the main page.
		 */
		...
		if (newThreatLevel == SecurityLevels.PHYSICAL_THREAT_LEVEL.MAXIMUM) {
			try {
				...
				return FirstTimeWizardToadlet.WIZARD_STEP.SECURITY_PHYSICAL + "&error=delete&newThreatLevel="
						+ newThreatLevel.name();
			}
		}
	 ...
	 * @param newThreatLevel the user-selected threat level, to be used in creating the
	 * form.
	...
	private String promptPassword(SecurityLevels.PHYSICAL_THREAT_LEVEL newThreatLevel, PASSWORD_PROMPT type) {
		...
		StringBuilder destination = new StringBuilder(
				FirstTimeWizardToadlet.WIZARD_STEP.SECURITY_PHYSICAL + "&error=pass&newThreatLevel=")
						.append(newThreatLevel.name()).append("&type=").append(type.name());
	...
	public void setThreatLevel(SecurityLevels.PHYSICAL_THREAT_LEVEL newThreatLevel,
		...
		try {
			...
			System.err.println("Failed starting up database while switching physical security level to "
					+ newThreatLevel + " from " + oldThreatLevel + " : " + core.node.getMasterPasswordFile()
					+ " is too " + e.sizeToString());
		}
	}
