...
public class BANDWIDTH_MONTHLY extends BandwidthManipulator implements Step {
	...
	/*
	 * Bandwidth used if both the upload and download limit are at the minimum. In GB.
	 * Assumes 24/7 uptime.
	 */
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		...
		if (request.isParameterSet("parseError")) {
			...
			HTMLNode errorBox = parseErrorBox(contentNode, helper,
					WizardL10n.l10n("bandwidthMonthlyLow", new String[] { "requested", "minimum", "useMinimum" },
							new String[] { parseTarget, String.valueOf(Math.round(minCap)),
									WizardL10n.l10n("bandwidthMonthlyUseMinimum") }));

			HTMLNode minimumForm = helper.addFormChild(errorBox, ".", "use-minimum");
			minimumForm.addChild("input", new String[] { "type", "name", "value" },
					new String[] { "hidden", "capTo", String.valueOf(minCap) });
			minimumForm.addChild("input", new String[] { "type", "value" },
		...
		// TODO: The user can always set a custom limit. At least one limit should be
		...
		for (long cap : caps) {
			...
			selectForm.addChild("input", new String[] { "type", "name", "value" },
					new String[] { "hidden", "capTo", String.valueOf(cap) });
			selectForm.addChild("input", new String[] { "type", "value" },
		...
		customForm.addChild("td").addChild("input", new String[] { "type", "value" },
				new String[] { "submit", WizardL10n.l10n("bandwidthSelect") });

		// Back / next buttons
		HTMLNode backForm = helper.addFormChild(infoBox, ".", "backForm");
		backForm.addChild("input", new String[] { "type", "name", "value" },
	...
	public String postStep(HTTPRequest request) {
		...
		StringBuilder target = new StringBuilder(FirstTimeWizardToadlet.WIZARD_STEP.BANDWIDTH_MONTHLY.name())
				.append("&parseTarget=");
		try {
		...
		/*
		 * Fraction of total limit used for download. Asymptotically from 0.5 at the
		 * minimum cap to 0.8.
		 *
		 * FIXME: Why do we do this? It does not actually work, since download cannot be
		 * larger than upload for any long amount of time.
		 *
		 * This 50/50 split is consistent with the assumption in the definition of minCap
		 * that the upload and download limits are equal.
		 */
		double bytesPerSecond = bytesPerMonth / secondsPerMonth;
		double minBytesPerSecond = Node.getMinimumBandwidth();
		double bwinc = bytesPerSecond - 2 * minBytesPerSecond; // min for up and min for
																// down
