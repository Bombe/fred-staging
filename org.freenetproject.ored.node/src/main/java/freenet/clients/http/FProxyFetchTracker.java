...
public class FProxyFetchTracker implements Runnable {
	...
	public FProxyFetchWaiter makeFetcher(FreenetURI key, long maxSize, FetchContext fctx,
		...
		/*
		 * LOCKING: Call getWaiter() inside the fetchers lock, since we will purge old
		 * fetchers inside that lock, hence avoid a race condition. FetchInProgress lock
		 * is always taken last.
		 */
	...
	/**
	 * Gets an FProxyFetchInProgress identified by the URI and the maxsize. If no such
	 * FetchInProgress exists, then returns null.
