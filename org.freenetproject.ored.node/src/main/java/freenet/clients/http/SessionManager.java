...
/**
 * A basic session manager for cookie-based HTTP session. It allows its parent web
 * interface to associate a "UserID" (a string) with a session ID.
 *
 * Formal definition of a SessionManager: A 1:1 mapping of SessionID to UserID. Queries by
 * SessionID and UserID run in O(1). The Session ID primary key consists of: Cookie path +
 * Cookie name + random "actual" session ID The user ID is received from the client
 * application.
 *
 * Therefore, when multiple client applications want to store sessions, each one is
 * supposed to create its own SessionManager because user IDs might overlap.
 *
 ...
 *
 * Paths are used when client applications do NOT share the same path on the server. For
 * example "/Chat" would cause the browser to only send back the cookie if the user is
 * browsing "/Chat/", not for "/". BUT usually we want the menu contents of client
 * applications to be in the logged-in state even if the user is NOT browsing the client
 * application web interface right now, therefore the "/" path must be used in most cases.
 * If client application cookies shall be received from all paths on the server, the
 * client application should use the constructor which requires a cookie namespace.
 *
 * The usage of a namespace gurantees that Sessions of different client applications do
 * not overlap.
 *
...
public final class SessionManager {
	...
	/**
	 * Constructs a new session manager for use with the given cookie path. Cookies are
	 * only sent back if the user is browsing the domain within that path.
	...
	public SessionManager(URI myCookiePath) {
		...
		// FIXME: The new constructor was written at 2010-11-15. Uncomment the following
		// safety check after we gave plugins some time to migrate
	 ...
	 * Must not be empty. Must be latin letters and numbers only.
	 */
	...
	public static final class Session {
		 ...
		 * @return {@code true} if this session contains an attribute with the given name,
		...
		/**
		 * Returns the value of the attribute with the given name. If there is no
		 * attribute with the given name, {@code null} is returned.
		...
		/**
		 * Removes the attribute with the given name. Nothing will happen if there is no
	...
	/**
	 * Returns the cookie path as specified in the constructor. Returns "/" if the
	 * constructor which only requires a namespace was used.
	 */
	...
	/**
	 * Returns the namespace as specified in the constructor. Returns an empty string if
	 * the constructor which requires a cookie path only was used.
	 */
	 ...
	 *
	 * If a session for the given user ID already exists, it is deleted. It is not re-used
	 * to ensure that parallel logins with the same user account from different computers
	 * do not work.
	...
	public synchronized Session createSession(String userID, ToadletContext context) {
		// We must synchronize around the fetching of the time and mSessionsByID.push()
		// because mSessionsByID is no sorting data structure: It's a plain
		// LRUMap so to ensure that it stays sorted the operation "getTime(); push();"
		// must be atomic.
	...
	/**
	 * Returns true if the given {@link ToadletContext} contains a session cookie for a
	 * valid (existing and not expired) session.
	 *
	 * In opposite to {@link getSessionUserID}, this function does NOT extend the validity
	 * of the session. Therefore, this function can be considered as a way of peeking for
	 * a session, to decide which Toadlet links should be visible.
	 */
	 ...
	 * {@link ToadletContext}, checks if it contains a valid (existing and not expired)
	 * session and if yes, returns the {@link Session}.
	 *
	 * If the session was valid, then its validity is extended by
	 * {@link MAX_SESSION_IDLE_TIME}.
	 *
	 * If the session did not exist or is not valid anymore, <code>null</code> is
	 * returned.
	 */
	 ...
	 * Retrieves the session ID from the session cookie in the given
	 * {@link ToadletContext}, checks if it contains a valid (existing and not expired)
	 * session and if yes, deletes the session.
	 * @return True if the session was deleted, false if there was no session cookie or no
	 * session.
	 */
	...
	/**
	 * @return Returns the session ID stored in the cookies of the HTTP headers of the
	 * given {@link ToadletContext}. Returns null if there is no session ID stored.
	 */
	...
	/**
	 * Stores a session cookie for the given session in the given {@link ToadletContext}'s
	 * HTTP headers.
	 * @param session
	 * @param context
	 */
	...
	/**
	 * Garbage-collects any expired sessions. Must be called before client-inteface
	 * functions do anything which relies on the existence a session, that is: creating
	 * sessions, using sessions or checking whether sessions exist.
	 *
	 * FIXME: Before putting the session manager into fred, write a thread which
	 * periodically garbage collects old sessions - currently, sessions will only be
	 * garbage collected if any client continues using the SessiomManager
	...
	/**
	 * Debug function which checks whether the sessions by user ID table does not contain
	 * any sessions which do not exist anymore;
	 */
