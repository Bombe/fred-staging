...
public class RssSniffer {
	...
	/**
	 * Look for any of the following strings as top-level XML tags: &lt;rss &lt;feed
	 * &lt;rdf:RDF
	 *
	 * If they start at the beginning of the file, or are preceded by one or more &lt;! or
	 * &lt;? tags, then firefox will read it as RSS. In which case we must force it to be
	 * downloaded to disk.
	 */
	public static boolean isSniffedAsFeed(byte[] prefix) {
		...
		return startsWithString(prefix, "<rss", tlt) || startsWithString(prefix, "<feed", tlt)
				|| startsWithString(prefix, "<rdf:RDF", tlt);
	}

	/**
	 * Finds the smallest index of a top-level XML tag in the data. A tag is any sequence
	 * that starts with '<'. A top-level tag is any subsequence that is not a comment,
	 * processing instruction or doctype (e.g. "<?" or "<!") and is not embedded in such a
	 * tag. Returns -1 if no such tag exists in the data.
	 */
	private static int indexOfTopLevelTag(byte[] data) {
		...
		while ((i = indexOf(data, (byte) '<', i)) != -1) {
			...
			// Found a comment, processing instruction or doctype; proceed to the end of
			// the tag.
	...
	/**
	 * Checks whether the given data starts with the characters in the key value, when
	 * starting at fromIndex in the data array.
	 */
