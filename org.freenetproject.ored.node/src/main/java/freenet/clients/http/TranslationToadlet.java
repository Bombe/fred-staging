...
public class TranslationToadlet extends Toadlet {
	...
	public void handleMethodGET(URI uri, HTTPRequest request, ToadletContext ctx)
		...
		if (request.isParameterSet("getOverrideTranlationFile")) {
			...
			footer.addChild("%", "&nbsp;&nbsp;");
			footer.addChild("a", "href", TOADLET_URL + "?translate=" + key + (showEverything ? "" : "&toTranslateOnly"))
			...
			contentRow.addChild("td", "class", "translation-new").addChild("textarea",
					new String[] { "name", "rows", "cols" }, new String[] { "trans", "20", "80" },
					this.base.getString(key));

			contentRow.addChild("input", new String[] { "type", "name", "value" },
			...
			HTMLNode content = ctx.getPageMaker().getInfobox("infobox-warning", l10n("removeOverrideWarningTitle"),
					contentNode, "translation-override", true);
			content.addChild("p").addChild("#", NodeL10n.getBase().getString("TranslationToadlet.confirmRemoveOverride",
					new String[] { "key", "value" }, new String[] { key, this.base.getString(key) }));
		...
		final HTMLNode translatingForForm = ctx.addFormChild(translatingForBox, TOADLET_URL, "ChooseWhatToTranslate")
	...
	private HTMLNode _setOrRemoveOverride(String key, boolean isOverriden, boolean showEverything) {
		...
		if (value == null) {
			...
			translationField
					.addChild("a", "href",
							TranslationToadlet.TOADLET_URL + "?translate=" + key
			...
			if (isOverriden)
				translationField
						.addChild("a", "href",
								TranslationToadlet.TOADLET_URL + "?remove=" + key
										+ (showEverything ? "" : "&toTranslateOnly"))
	...
	private String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase().getString("TranslationToadlet." + key, new String[] { pattern },
				new String[] { value });
	}
