...
public class StatisticsToadlet extends Toadlet {
	...
	public void handleMethodGET(URI uri, HTTPRequest request, ToadletContext ctx)
		...
		synchronized (this) {
			...
			if (node.isUsingWrapper()) {
				...
				threadDumpForm.addChild("input", new String[] { "type", "name", "value" },
			...
			if (nodeConfig.config.get("logger").getBoolean("enabled"))
				logsList.addChild("li").addChild("a", new String[] { "href", "target" },
						new String[] { "/?latestlog", "_blank" }, l10n("getLogs"));
			logsList.addChild("li").addChild("a", "href", TranslationToadlet.TOADLET_URL + "?getOverrideTranlationFile")
			...
			if (advancedMode) {
				// store size box
				// HTMLNode storeSizeInfobox = nextTableCell.addChild("div", "class",
				// "infobox");
			...
			if (advancedMode || numberOfConnected + numberOfRoutingBackedOff > 0) {
				...
				if (advancedMode) {
					...
					drawOverviewBox(overviewInfobox, nodeUptimeSeconds,
							node.clientCore.bandwidthStatsPutter.getLatestUptimeData().totalUptime, now, swaps,
							noSwaps);
				}
				...
				drawPeerStatsBox(peerStatsInfobox, advancedMode, numberOfConnected, numberOfRoutingBackedOff,
						...
						numberOfBursting, numberOfListening, numberOfListenOnly, numberOfSeedServers,
						numberOfSeedClients, numberOfRoutingDisabled, numberOfClockProblem, numberOfConnError,
						numberOfDisconnecting, numberOfNoLoadStats, node);
			...
			if (advancedMode) {
				...
				HTMLNode peerCircleTable = peerCircleInfobox.addChild("div", "class", "infobox-content")
						.addChild("table");
				...
				HTMLNode nodeCircleTable = nodeCircleInfobox.addChild("div", "class", "infobox-content")
						.addChild("table");
				...
				if (incomingRequestsCount > 0) {
					...
					HTMLNode nodeSpecialisationTable = nodeSpecialisationInfobox
							.addChild("div", "class", "infobox-content").addChild("table");
					addSpecialisation(nodeSpecialisationTable, myLocation, incomingRequestLocation);
				}

				nextTableCell = overviewTableRow.addChild("td");
				int[] outgoingLocalRequestCountArray = new int[1];
				int[] outgoingLocalRequestLocation = stats
						.getOutgoingLocalRequestLocation(outgoingLocalRequestCountArray);
				...
				HTMLNode nodeSpecialisationTable = nodeSpecialisationInfobox.addChild("div", "class", "infobox-content")
						.addChild("table");
	...
	private void drawClientRequestersBox(HTMLNode box) {
		...
		for (ClientRequester request : requests) {
			...
			FreenetURI uri = request.getURI(); // getURI() sometimes returns null, eg for
												// ClientPutters
	...
	private void drawStoreSizeBox(HTMLNode storeSizeInfobox, double loc, long nodeUptimeSeconds) {
		...
		for (Map.Entry<DataStoreInstanceType, DataStoreStats> entry : storeStats.entrySet()) {
			...
			try {
				...
				// FIXME this is not necessarily the same as the datastore's uptime if
				// we've switched.
	...
	private void drawUnclaimedFIFOMessageCountsBox(HTMLNode unclaimedFIFOMessageCountsInfobox) {
		...
		HTMLNode unclaimedFIFOMessageCountsInfoboxContent = unclaimedFIFOMessageCountsInfobox.addChild("div", "class",
				"infobox-content");
		...
		Arrays.sort(unclaimedFIFOMessageCountsArray, new Comparator<STMessageCount>() {
			...
			public int compare(STMessageCount firstCount, STMessageCount secondCount) {
				return secondCount.messageCount - firstCount.messageCount; // sort in
																			// descending
																			// order
			}
		});
		for (STMessageCount messageCountItem : unclaimedFIFOMessageCountsArray) {
			...
			unclaimedFIFOMessageCountsList.addChild("li", "" + messageCountItem.messageName + ":\u00a0"
					+ thisMessageCount + "\u00a0(" + fix3p1pct.format(thisMessagePercentOfTotal) + ')');
		}
		unclaimedFIFOMessageCountsList.addChild("li", "Unclaimed Messages Considered:\u00a0" + totalCount);

	}

	private void drawSwapStatsBox(HTMLNode locationSwapInfobox, double location, long nodeUptimeSeconds, double swaps,
			double noSwaps) {
	...
	protected static void drawPeerStatsBox(HTMLNode peerStatsInfobox, boolean advancedModeEnabled,
			int numberOfConnected, int numberOfRoutingBackedOff, int numberOfTooNew, int numberOfTooOld,
			int numberOfDisconnected, int numberOfNeverConnected, int numberOfDisabled, int numberOfBursting,
			int numberOfListening, int numberOfListenOnly, int numberOfSeedServers, int numberOfSeedClients,
			int numberOfRoutingDisabled, int numberOfClockProblem, int numberOfConnError, int numberOfDisconnecting,
			int numberOfNoLoadStats, Node node) {
		...
		if (numberOfConnected > 0) {
			...
			peerStatsConnectedListItem.addChild(
					"span", new String[] { "class", "title", "style" }, new String[] { "peer_connected",
		...
		if (numberOfDisconnected > 0) {
			...
			peerStatsDisconnectedListItem.addChild(
					"span", new String[] { "class", "title", "style" }, new String[] { "peer_disconnected",
		...
		if (numberOfNeverConnected > 0) {
			...
			peerStatsNeverConnectedListItem.addChild(
					"span", new String[] { "class", "title", "style" }, new String[] { "peer_never_connected",
		...
		if (numberOfListening > 0) {
			...
			peerStatsListeningListItem.addChild(
					"span", new String[] { "class", "title", "style" }, new String[] { "peer_listening",
		...
		if (numberOfListenOnly > 0) {
			...
			peerStatsListenOnlyListItem.addChild(
					"span", new String[] { "class", "title", "style" }, new String[] { "peer_listen_only",
		...
		if (numberOfClockProblem > 0) {
			...
			peerStatsRoutingDisabledListItem.addChild(
					"span", new String[] { "class", "title", "style" }, new String[] { "peer_clock_problem",
		...
		if (numberOfConnError > 0) {
			HTMLNode peerStatsRoutingDisabledListItem = peerStatsList.addChild("li").addChild("span");
			peerStatsRoutingDisabledListItem.addChild(
					"span", new String[] { "class", "title", "style" }, new String[] { "peer_routing_disabled",
		...
		if (numberOfDisconnecting > 0) {
			HTMLNode peerStatsListenOnlyListItem = peerStatsList.addChild("li").addChild("span");
			peerStatsListenOnlyListItem.addChild(
					"span", new String[] { "class", "title", "style" }, new String[] { "peer_disconnecting",
		...
		if (numberOfSeedServers > 0) {
			...
			peerStatsSeedServersListItem.addChild("span", new String[] { "class", "title", "style" }, new String[] {
					"peer_listening" /* FIXME */, l10nDark("seedServers"), "border-bottom: 1px dotted; cursor: help;" },
		...
		if (numberOfSeedClients > 0) {
			...
			peerStatsSeedClientsListItem.addChild("span", new String[] { "class", "title", "style" }, new String[] {
					"peer_listening" /* FIXME */, l10nDark("seedClients"), "border-bottom: 1px dotted; cursor: help;" },
		...
		if (numberOfNoLoadStats > 0) {
			...
			peerStatsNoLoadStatsListItem.addChild(
					"span", new String[] { "class", "title", "style" }, new String[] { "peer_no_load_stats",
							l10nDark("noLoadStats"), "border-bottom: 1px dotted; cursor: help;" },
					l10nDark("noLoadStatsShort"));
			peerStatsNoLoadStatsListItem.addChild("span", ":\u00a0" + numberOfNoLoadStats);
		}
		OpennetManager om = node.getOpennet();
		if (om != null) {
			peerStatsList.addChild("li",
	...
	private static String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase().getString("StatisticsToadlet." + key, new String[] { pattern },
				new String[] { value });
	}
	...
	private void drawActivityBox(HTMLNode activityInfobox, boolean advancedModeEnabled) {
		...
		if (advancedModeEnabled && activityList != null) {
			...
			activityList.addChild("li",
					"BackgroundFetcherByUSKSize:\u00a0" + node.clientCore.uskManager.getBackgroundFetcherByUSKSize());
			activityList.addChild("li", "temporaryBackgroundFetchersLRUSize:\u00a0"
					+ node.clientCore.uskManager.getTemporaryBackgroundFetchersLRU());
			activityList.addChild("li", "outputBandwidthLiabilityUsage:\u00a0"
	...
	static void drawBandwidth(HTMLNode activityList, Node node, long nodeUptimeSeconds, boolean isAdvancedModeEnabled) {
		...
		if (delta > 0) {
			...
			activityList.addChild("li", l10n("inputRate", new String[] { "rate", "max" }, new String[] {
					SizeUtil.formatSize(input_rate, true), SizeUtil.formatSize(inputBandwidthLimit, true) }));
			activityList.addChild("li", l10n("outputRate", new String[] { "rate", "max" }, new String[] {
					SizeUtil.formatSize(output_rate, true), SizeUtil.formatSize(outputBandwidthLimit, true) }));
		}
		activityList.addChild("li", l10n("totalInputSession", new String[] { "total", "rate" },
				new String[] { SizeUtil.formatSize(total[1], true), SizeUtil.formatSize(total_input_rate, true) }));
		activityList.addChild("li", l10n("totalOutputSession", new String[] { "total", "rate" },
				new String[] { SizeUtil.formatSize(total[0], true), SizeUtil.formatSize(total_output_rate, true) }));
		activityList.addChild("li",
				l10n("payloadOutput", new String[] { "total", "rate", "percent" },
						new String[] { SizeUtil.formatSize(totalPayload, true),
								SizeUtil.formatSize(total_payload_rate, true), Integer.toString(percent) }));
		activityList.addChild("li", l10n("totalInput", new String[] { "total" },
				new String[] { SizeUtil.formatSize(overall_total_in, true) }));
		activityList.addChild("li", l10n("totalOutput", new String[] { "total" },
		...
		if (isAdvancedModeEnabled) {
			...
			long totalBytesSentRemaining = total[0] - (totalPayload + totalBytesSentCHKRequests
					+ totalBytesSentSSKRequests + totalBytesSentCHKInserts + totalBytesSentSSKInserts
					+ totalBytesSentOfferedKeys + totalBytesSendOffers + totalBytesSentSwapOutput + totalBytesSentAuth
					+ totalBytesSentAckOnly + totalBytesSentResends + totalBytesSentUOM + totalBytesSentAnnounce
					+ totalBytesSentRoutingStatus + totalBytesSentNetworkColoring + totalBytesSentPing
					+ totalBytesSentProbeRequest + totalBytesSentRouted + totalBytesSentDisconn + totalBytesSentInitial
					+ totalBytesSentChangedIP + totalBytesSentNodeToNode + totalBytesSentAllocationNotices
					+ totalBytesSentFOAF);
			activityList.addChild("li",
					l10n("requestOutput", new String[] { "chk", "ssk" },
							new String[] { SizeUtil.formatSize(totalBytesSentCHKRequests, true),
									SizeUtil.formatSize(totalBytesSentSSKRequests, true) }));
			activityList.addChild("li",
					l10n("insertOutput", new String[] { "chk", "ssk" },
							new String[] { SizeUtil.formatSize(totalBytesSentCHKInserts, true),
									SizeUtil.formatSize(totalBytesSentSSKInserts, true) }));
			activityList.addChild("li",
					l10n("offeredKeyOutput", new String[] { "total", "offered" },
							new String[] { SizeUtil.formatSize(totalBytesSentOfferedKeys, true),
			...
			activityList.addChild("li",
					l10n("ackOnlyBytes", "total", SizeUtil.formatSize(totalBytesSentAckOnly, true)));
			activityList.addChild("li",
					l10n("resendBytes", new String[] { "total", "percent" },
			...
			activityList.addChild("li",
					l10n("announceBytes", new String[] { "total", "payload" },
			...
			activityList.addChild("li",
					l10n("unaccountedBytes", new String[] { "total", "percent" },
			...
			activityList.addChild("li",
					l10n("totalOverhead", new String[] { "rate", "percent" },
	...
	static HTMLNode drawActivity(HTMLNode activityInfoboxContent, Node node) {
		...
		if ((numTransferringRequests == 0) && (numCHKRequests == 0) && (numSSKRequests == 0) && (numCHKInserts == 0)
				&& (numSSKInserts == 0) && (numTransferringRequestHandlers == 0) && (numCHKOfferReplys == 0)
				&& (numSSKOfferReplys == 0)) {
			...
			if (numCHKInserts > 0 || numSSKInserts > 0) {
				activityList.addChild("li", NodeL10n.getBase().getString("StatisticsToadlet.activityInserts",
						...
						new String[] { Integer.toString(numCHKInserts), Integer.toString(numSSKInserts),
								Integer.toString(numLocalCHKInserts) + "/" + Integer.toString(numLocalSSKInserts) }));
			}
			if (numCHKRequests > 0 || numSSKRequests > 0) {
				activityList.addChild("li", NodeL10n.getBase().getString("StatisticsToadlet.activityRequests",
						new String[] { "CHKhandlers", "SSKhandlers", "local" },
						new String[] { Integer.toString(numCHKRequests), Integer.toString(numSSKRequests),
								Integer.toString(numLocalCHKRequests) + "/" + Integer.toString(numLocalSSKRequests) }));
			}
			if (numTransferringRequests > 0 || numTransferringRequestHandlers > 0) {
				activityList.addChild("li",
						NodeL10n.getBase().getString("StatisticsToadlet.transferringRequests",
			...
			if (numCHKOfferReplys > 0 || numSSKOfferReplys > 0) {
				activityList.addChild("li", NodeL10n.getBase().getString("StatisticsToadlet.offerReplys",
						new String[] { "chk", "ssk" },
						new String[] { Integer.toString(numCHKOfferReplys), Integer.toString(numSSKOfferReplys) }));
			}
			activityList.addChild("li",
					NodeL10n.getBase().getString("StatisticsToadlet.runningBlockTransfers",
							new String[] { "sends", "receives" },
							new String[] { Integer.toString(BlockTransmitter.getRunningSends()),
									Integer.toString(BlockReceiver.getRunningReceives()) }));
			return activityList;
		}
	}

	private void drawOverviewBox(HTMLNode overviewInfobox, long nodeUptimeSeconds, long nodeUptimeTotal, long now,
		...
		if ((numberOfRemotePeerLocationsSeenInSwaps > 0.0) && ((swaps > 0.0) || (noSwaps > 0.0))) {
			overviewList.addChild("li", "avrConnPeersPerNode:\u00a0"
					+ fix6p6.format(numberOfRemotePeerLocationsSeenInSwaps / (swaps + noSwaps)) + "\u00a0peers");
		}
		overviewList.addChild("li",
				"nodeUptimeSession:\u00a0" + TimeUtil.formatTime(MILLISECONDS.convert(nodeUptimeSeconds, SECONDS)));
		...
		overviewList.addChild("li",
				"pInstantRejectRequestBulk:\u00a0" + fix3p1pct.format(stats.pRejectIncomingInstantlyCHKRequestBulk())
						+ " (CHK) " + fix3p1pct.format(stats.pRejectIncomingInstantlySSKRequestBulk()) + " (SSK)");
		overviewList.addChild("li",
				"pInstantRejectInsertBulk:\u00a0" + fix3p1pct.format(stats.pRejectIncomingInstantlyCHKInsertBulk())
						+ " (CHK) " + fix3p1pct.format(stats.pRejectIncomingInstantlySSKInsertBulk()) + " (SSK)");
		overviewList.addChild("li",
				"pInstantRejectRequestRT:\u00a0" + fix3p1pct.format(stats.pRejectIncomingInstantlyCHKRequestRT())
						+ " (CHK) " + fix3p1pct.format(stats.pRejectIncomingInstantlySSKRequestRT()) + " (SSK)");
		overviewList.addChild("li",
				"pInstantRejectInsertRT:\u00a0" + fix3p1pct.format(stats.pRejectIncomingInstantlyCHKInsertRT())
						+ " (CHK) " + fix3p1pct.format(stats.pRejectIncomingInstantlySSKInsertRT()) + " (SSK)");
		overviewList.addChild("li", "unclaimedFIFOSize:\u00a0" + node.getUnclaimedFIFOSize());
		overviewList.addChild("li",
				"RAMBucketPoolSize:\u00a0" + SizeUtil.formatSize(core.tempBucketFactory.getRamUsed()) + " / "
		...
		if (decoded != null) {
			overviewList.addChild("li", "packetsDecoded:\u00a0"
					+ fix3p1pct.format(((double) decoded[0]) / ((double) decoded[1])) + "\u00a0(" + decoded[1] + ")");
		}
	...
	// FIXME this should probably be moved to nodestats so it can be used by FCP??? would
	// have to make ThreadBunch public :<
	private void getThreadNames(HTMLNode threadUsageList) {
		...
		for (ThreadBunch bunch : bunches) {
			...
			threadUsageList.addChild("li", "" + bunch.name + ":\u00a0" + Integer.toString(bunch.count) + "\u00a0("
	...
	private void addNodeCircle(HTMLNode circleTable, double myLocation) {
		...
		HTMLNode nodeCircleInfoboxContent = nodeCircleTableCell.addChild("div", new String[] { "style", "class" },
				new String[] {
						"position: relative; height: " + ((PEER_CIRCLE_RADIUS + PEER_CIRCLE_ADDITIONAL_FREE_SPACE) * 2)
								+ "px; width: " + ((PEER_CIRCLE_RADIUS + PEER_CIRCLE_ADDITIONAL_FREE_SPACE) * 2) + "px",
						"peercircle" });
		nodeCircleInfoboxContent.addChild("span", new String[] { "style", "class" },
		...
		nodeCircleInfoboxContent.addChild("span", new String[] { "style", "class" },
				new String[] { "position: absolute; top: " + PEER_CIRCLE_RADIUS + "px; left: "
		...
		for (int i = 0; i < locations.length; i++) {
			...
			nodeCircleInfoboxContent.addChild("span", new String[] { "style", "class" }, new String[] {
					generatePeerCircleStyleString(location.doubleValue(), false, strength), "connected" }, "x");
		}
		nodeCircleInfoboxContent.addChild("span", new String[] { "style", "class" },
				new String[] { generatePeerCircleStyleString(myLocation, true, 1.0), "me" }, "x");
		//
		int histogramDiv = simpleHistogramDivisor(histogram); // Prevent division by 0 on
																// empty histograms.
		...
		for (int i = 0; i < HISTOGRAM_LENGTH; i++) {
			...
			nodeHistogramGraphCell.addChild("%", "<div class=\"histogramConnected\" style=\"height: "
	...
	private void addSpecialisation(HTMLNode table, double peerLocation, int[] incomingRequestLocation) {
		...
		for (int i = 0; i < incomingRequestLocation.length; i++) {
			...
			nodeHistogramGraphCell.addChild("div", new String[] { "class", "style" },
					new String[] { "histogramConnected",
							"height: " + fix3pctUS.format(((double) incomingRequestLocation[i]) / histogramDiv)
									+ "; width: 100%;" },
					"\u00a0");
		}
	}

	private void addCombinedSpecialisation(HTMLNode table, double peerLocation, int[] locallyOriginatingRequests,
		...
		for (int i = 0; i < locallyOriginatingRequests.length; i++) {
			...
			nodeHistogramGraphCell.addChild("div", new String[] { "class", "style" },
					new String[] { "histogramConnected",
							"height: " + fix3pctUS.format(((double) locallyOriginatingRequests[i]) / histogramDiv)
									+ "; width: 100%;" },
					"\u00a0");
			nodeHistogramGraphCell.addChild("div", new String[] { "class", "style" },
					new String[] { "histogramDisconnected",
							"height: " + fix3pctUS.format(((double) remotelyOriginatingRequests[i]) / histogramDiv)
									+ "; width: 100%;" },
					"\u00a0");
		}
	}
	...
	private void addFOAFLinkLengthHistogram(HTMLNode circleTable, PeerNodeStatus[] peerNodeStatuses) {
		...
		for (int i = 0; i < HISTOGRAM_LENGTH; i++) {
			...
			peerHistogramGraphCell.addChild("div", new String[] { "class", "style" }, new String[] {
					"histogramConnected", "height: " + fix3pctUS.format(histogramFraction) + "; width: 100%;" },
					"\u00a0");
			peerHistogramGraphCell.addChild("div", new String[] { "class", "style" }, new String[] {
					"histogramDisconnected", "height: " + fix3pctUS.format(cumulativeFraction) + "; width: 100%;" },
					"\u00a0");
	...
	private void addPeerCircle(HTMLNode circleTable, PeerNodeStatus[] peerNodeStatuses, double myLocation) {
		...
		HTMLNode peerCircleInfoboxContent = peerCircleTableCell.addChild("div", new String[] { "style", "class" },
				new String[] {
						"position: relative; height: " + ((PEER_CIRCLE_RADIUS + PEER_CIRCLE_ADDITIONAL_FREE_SPACE) * 2)
								+ "px; width: " + ((PEER_CIRCLE_RADIUS + PEER_CIRCLE_ADDITIONAL_FREE_SPACE) * 2) + "px",
						"peercircle" });
		peerCircleInfoboxContent.addChild("span", new String[] { "style", "class" },
				new String[] { generatePeerCircleStyleString(0, false, 1.0), "mark" }, "|");
		peerCircleInfoboxContent.addChild("span", new String[] { "style", "class" },
				new String[] { generatePeerCircleStyleString(0.125, false, 1.0), "mark" }, "+");
		peerCircleInfoboxContent.addChild("span", new String[] { "style", "class" },
		...
		for (int peerIndex = 0; peerIndex < peerCount; peerIndex++) {
			...
			if (foafLocations != null && peerNodeStatus.isRoutable()) {
				for (double foafLocation : foafLocations) {
					...
					peerCircleInfoboxContent.addChild("span", new String[] { "style", "class" },
							new String[] { generatePeerCircleStyleString(foafLocation, false, 0.9), "disconnected" },
							".");
				}
			}
		...
		for (int i = 0; i < HISTOGRAM_LENGTH; i++) {
			...
			peerHistogramGraphCell.addChild("div", new String[] { "class", "style" }, new String[] {
					"histogramDisconnected", "height: " + fix3pctUS.format(histogramPercent) + "; width: 100%;" },
					"\u00a0");
		}
	}
	...
	private String generatePeerCircleStyleString(double peerLocation, boolean offsetMe, double strength) {
		...
		double x = PEER_CIRCLE_ADDITIONAL_FREE_SPACE + PEER_CIRCLE_RADIUS
				+ Math.sin(peerLocation) * (PEER_CIRCLE_RADIUS - offset);
		double y = PEER_CIRCLE_RADIUS - Math.cos(peerLocation) * (PEER_CIRCLE_RADIUS - offset); // no
