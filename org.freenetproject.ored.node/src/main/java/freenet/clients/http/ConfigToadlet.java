...
public class ConfigToadlet extends Toadlet implements LinkEnabledCallback {
	 ...
	 */
	private class NeedRestartUserAlert extends BaseNodeUserAlert {
		...
		public HTMLNode getHTMLText() {
			...
			if (node.isUsingWrapper()) {
				...
				HTMLNode restartForm = alertNode
	...
	public ConfigToadlet(String directoryBrowserPath, HighLevelSimpleClient client, Config conf, SubConfig subConfig,
	...
	public void handleMethodPOST(URI uri, HTTPRequest request, ToadletContext ctx)
		...
		if (request.isPartSet("confirm-reset-to-defaults")) {
			...
			HTMLNode content = ctx.getPageMaker().getInfobox("infobox-warning", l10n("confirmResetTitle"), contentNode,
					"reset-confirm", true);
			...
			for (String part : request.getParts()) {
				if (part.startsWith(subconfig)) {
					formNode.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", part,
							request.getPartAsStringFailsafe(part, MAX_PARAM_VALUE_SIZE) });
				}
			}

			formNode.addChild("input", new String[] { "type", "name", "value" },
					new String[] { "submit", "reset-to-defaults", NodeL10n.getBase().getString("Toadlet.yes") });

			formNode.addChild("input", new String[] { "type", "name", "value" },
					new String[] { "submit", "decline-default-reset", NodeL10n.getBase().getString("Toadlet.no") });
		...
		for (String key : request.getParts()) {
			...
			if (key.startsWith("select-directory.")) {
				paramsBuilder.append("select-for=")
						.append(URLEncoder.encode(key.substring("select-directory.".length()), true)).append('&');
				directorySelector = true;
			}
			else {
				paramsBuilder.append(URLEncoder.encode(key, true)).append('=').append(URLEncoder.encode(value, true))
						.append('&');
			}
		}
		...
		if (directorySelector) {
			...
			// download directory.
			headers.put("Location", directoryBrowserPath + params + "path=" + core.getDownloadsDir().getAbsolutePath());
		...
		if (errbuf.length() == 0) {
			...
			HTMLNode content = ctx.getPageMaker()
					.getInfobox("infobox-error", l10n("appliedFailureTitle"), contentNode, "configuration-error", true)
	...
	public void handleMethodGET(URI uri, HTTPRequest req, ToadletContext ctx)
		...
		/*
		 * Special case: present an option for the wrapper's maximum memory under Core
		 * configuration, provided the maximum memory property is defined. (the wrapper is
		 * being used)
		 */
			...
			if (curValue != null) {
				...
				item.addChild("span", new String[] { "class", "title", "style" },
						new String[] { "configshortdesc",
								NodeL10n.getBase().getString("ConfigToadlet.defaultIs", new String[] { "default" },
										new String[] { defaultValue }),
								"cursor: help;" })
						.addChild(new ToadletL10nAdapter(NodeL10n.getBase())
								.getHTMLNode("WrapperConfig." + configName + ".short"));
				item.addChild("span", "class", "config").addChild("input",
						new String[] { "type", "class", "name", "value" },
						new String[] { "text", "config", configName, curValue });
				item.addChild("span", "class", "configlongdesc").addChild(new ToadletL10nAdapter(NodeL10n.getBase())
						.getHTMLNode("WrapperConfig." + configName + ".long"));
			}
		}
		...
		for (Option<?> o : subConfig.getOptions()) {
			if (!((!advancedModeEnabled) && o.isExpert())) {
				...
				// instance like
				// normal.
				HTMLNode shortDesc = new ToadletConfigOptionAdapter<>(o).getShortDescNode(plugin);
				HTMLNode longDesc = new ToadletConfigOptionAdapter<>(o).getLongDescNode(plugin);
				...
				configItemNode.addChild("a", new String[] { "name", "id" }, new String[] { configName, configName })
						.addChild("span", new String[] { "class", "title", "style" },
								new String[] { "configshortdesc",
				...
				case DROP_DOWN:
					configItemValueNode.addChild(
							addComboBox(value, (EnumerableOptionCallback) callback, fullName, callback.isReadOnly()));
					break;
				case BOOLEAN:
					configItemValueNode
							.addChild(addBooleanComboBox(Boolean.valueOf(value), fullName, callback.isReadOnly()));
					break;
				case DIRECTORY:
					configItemValueNode.addChild(addTextBox(value, fullName, o, false));
					configItemValueNode.addChild("input", new String[] { "type", "name", "value" },
	 ...
	 * @return An input of type "text" and class "config" containing the current value of
	 * the option.
	 */
	public static HTMLNode addTextBox(String value, String fullName, Option<?> o, boolean disabled) {
		...
		if (disabled) {
			result = new HTMLNode("input", new String[] { "type", "class", "disabled", "alt", "name", "value" }, //
					new String[] { "text", "config", "disabled", o.getShortDesc(), fullName, value });
		}
		else {
			result = new HTMLNode("input", new String[] { "type", "class", "alt", "name", "value" }, //
	...
	/**
	 * Generates a drop-down combobox for the given enumerable option suitable for adding
	 * to an existing form. Its first element is the "select" element, so any Javascript
	 * attributes can be added to the output.
	 ...
	 * @return An HTMLNode of a "select" with "option" children for each of the possible
	 * values. If the value specified in value is one of the options, it will be selected.
	 */
	...
	/**
	 * Generates a drop-down combobox for a true/false option suitable for adding to an
	 * existing form. Its first element is the "select" element, so any Javascript
	 * attributes can be added to the output.
	 * @param value The current value of the option. This will be selected.
	 * @param fullName The full name of the option, used to name the drop-down.
	 * @param disabled Whether the drop-down should be disabled.
	 * @return An HTMLNode of a "select" with an "option" child for localized "true" and
	 * "false", with the current value selected.
	 */
