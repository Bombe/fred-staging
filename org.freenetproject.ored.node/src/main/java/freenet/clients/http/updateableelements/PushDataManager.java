...
/**
 * A manager class that manages all the pushing. All it's public method must be
...
public class PushDataManager {
	...
	/**
	 * The Cleaner only runs when needed. If this field is true, then the Cleaner is
	 * scheduled to run
	 */
		...
		if (elements.containsKey(id))
			for (String reqId : elements.get(id)) {
				...
				for (Map.Entry<String, List<UpdateEvent>> entry : awaitingNotifications.entrySet()) {
					// for (List<UpdateEvent> notificationList :
					// awaitingNotifications.values()) {
	...
	public synchronized void elementRendered(String requestUniqueId, BaseUpdateableElement element) {
		...
		// If the Cleaner isn't running, then we schedule it to clear this request if
		// failing
	...
	/**
	 * Fails a request and copies all notifications directed to it to another request. It
	 * is invoked when a leadership change occurs.
	...
	/**
	 * Waits and return the next notification. Calling this method setup the notification
	 * list.
	...
	public synchronized UpdateEvent getNextNotification(String requestId) {
		...
		while (awaitingNotifications.get(requestId) != null && awaitingNotifications.get(requestId).size() == 0 || // No
																													// notifications
				(awaitingNotifications.get(requestId) != null && awaitingNotifications.get(requestId).size() != 0
						&& isFirstKeepaliveReceived
								.containsKey(awaitingNotifications.get(requestId).get(0).requestId) == false)) { // Not
																													// asked
																													// us
																													// yet
			try {
		...
		if (logMINOR) {
			Logger.minor(this, "Getting notification, notification:" + awaitingNotifications.get(requestId).get(0)
					+ ",remaining:" + (awaitingNotifications.get(requestId).size() - 1));
		}
