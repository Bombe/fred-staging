...
/**
 * Toadlet displaying information on the node's connectivity status. Eventually this will
 * include all information gathered by the node on its connectivity from plugins, local IP
 * detection, packet monitoring etc. For the moment it's just a dump of the
 * AddressTracker.
 *
 * @author toad
 */
public class ConnectivityToadlet extends Toadlet {
	...
	public void handleMethodGET(URI uri, final HTTPRequest request, ToadletContext ctx)
		...
		HTMLNode summaryContent = pageMaker.getInfobox("#",
				NodeL10n.getBase().getString("ConnectivityToadlet.summaryTitle"), contentNode, "connectivity-summary",
				true);
		...
		if (ctx.isAdvancedModeEnabled()) {
			...
			for (UdpSocketHandler handler : handlers) {
				// Peers
				AddressTracker tracker = handler.getAddressTracker();
				HTMLNode portsContent = pageMaker.getInfobox("#", NodeL10n.getBase().getString(
						"ConnectivityToadlet.byPortTitle", new String[] { "port", "status", "tunnelLength" },
						new String[] { handler.getTitle(), AddressTracker.statusString(tracker.getPortForwardStatus()),
				...
				for (PeerAddressTrackerItem item : items) {
					...
					// Initiator: local/remote FIXME something more graphical e.g. colored
					// cells
					row.addChild("td", item.packetsReceived() == 0 ? noreply : (item.weSentFirst() ? local : remote));
				...
				// IPs
				portsContent = pageMaker.getInfobox("#", NodeL10n.getBase().getString("ConnectivityToadlet.byIPTitle",
