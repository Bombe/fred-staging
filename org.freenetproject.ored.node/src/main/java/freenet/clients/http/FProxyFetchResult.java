...
/**
 * The result of fproxy waiting for a fetch: It can either be the final data, or it can be
 * the progress of the fetch so far. This is a snapshot, so should be consistent but could
 * be out of date: if the snapshot says it is in progress, it might actually be finished.
 * Note that close() must be called when fproxy has finished with the data (and is the
 * only method that actually calls back to the freenet.clients.http.FProxyFetchInProgress
 * ).
 *
...
public class FProxyFetchResult {
	...
	/**
	 * Constructor when we are not returning the data, because it is still running or it
	 * failed
	 */
	FProxyFetchResult(FProxyFetchInProgress parent, String mimeType, long size, long timeStarted, boolean goneToNetwork,
			int totalBlocks, int requiredBlocks, int fetchedBlocks, int failedBlocks, int fatallyFailedBlocks,
