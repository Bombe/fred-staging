...
public class FProxyFetchInProgress implements ClientEventListener, ClientGetCallback {
	...
	public enum REFILTER_POLICY {
		...
		RE_FILTER, // Re-run the filter over data that has already been filtered. Probably
					// requires allocating a new temp file.
		ACCEPT_OLD, // Accept the old filtered data. Only as safe as the filter when the
					// data was originally downloaded.
		RE_FETCH // Fetch the data again. Unnecessary in most cases, avoids any
					// possibility of filter artefacts.
	...
	/**
	 * Any request which is waiting for a progress screen or data. We may want to wake
	 * requests separately in future.
	 */
	...
	private final List<FProxyFetchListener> listener = Collections
			.synchronizedList(new ArrayList<FProxyFetchListener>());
	...
	public FProxyFetchInProgress(FProxyFetchTracker tracker, FreenetURI key, long maxSize2, long identifier,
			ClientContext context, FetchContext fctx, RequestClient rc, REFILTER_POLICY refilter) {
	...
	synchronized FProxyFetchResult innerGetResult(boolean hasWaited) {
		...
		if (data != null)
			...
			res = new FProxyFetchResult(this, mimeType, size, timeStarted, goneToNetwork, totalBlocks, requiredBlocks,
					fetchedBlocks, failedBlocks, fatallyFailedBlocks, finalizedBlocks, failed, getETA(), hasWaited);
		}
		results.add(res);
	...
	private boolean checkCache(ClientContext context) {
		// Fproxy uses lookupInstant() with mustCopy = false. I.e. it can reuse stuff
		// unsafely. If the user frees it it's their fault.
		if (bogusUSK(context))
			return false;
		...
		if ((!fctx.filterData) && (!result.alreadyFiltered)) {
			if (fctx.overrideMIME == null || fctx.overrideMIME.equals(result.getMimeType())) {
				// Works as-is.
				// Any time we re-use old content we need to remove the tracker because it
	...
	/**
	 * If the key is a USK and a) we are requested to do an exhaustive search, or b) there
	 * is a later version, then we can't use the download queue as a cache.
	...
	private boolean shouldAcceptCachedFilteredData(FetchContext fctx, CacheFetchResult result) {
		...
		if (fctx.overrideMIME == null) {
			...
			if (fctx.overrideMIME.equals(finalMIME))
				...
				return true;
			// FIXME we could make this work in a few more cases... it doesn't matter much
			// though as usually people don't override the MIME type!
		}
		return false;
	}
	...
	/**
	 * Caller should take the lock on FProxyToadlet.fetchers, then call this function, if
	 * it returns true then finish the cancel outside the lock.
	 */
	...
	/**
	 * Finish the cancel process, freeing the data if necessary. The fetch must have been
	 * removed from the tracker already, so it won't be reused.
	 */
	...
	public synchronized boolean notFinishedOrFatallyFinished() {
		...
		if (failed != null && (System.currentTimeMillis() - timeFailed < 1000 || fetched < 2)) // Once
																								// for
																								// javascript
																								// and
																								// once
