...
public class ToadletConfigOptionAdapter<T> {
	...
	public HTMLNode getShortDescNode(FredPluginConfigurable plugin) {
		return (plugin == null) ? new ToadletL10nAdapter(NodeL10n.getBase())
				.getHTMLNode(this.option.getShortDesc(), new String[] { "default" } , new String[] { this.option.getDefault() }) : new HTMLNode("#",
				plugin.getString(this.option.getShortDesc()));
	}
	...
	public HTMLNode getLongDescNode(FredPluginConfigurable plugin) {
		return (plugin == null) ? new ToadletL10nAdapter(NodeL10n.getBase())
				.getHTMLNode(this.option.getLongDesc(), new String[] { "default" } , new String[] { this.option.getDefault() }) : new HTMLNode("#",
				plugin.getString(this.option.getLongDesc()));
	}
