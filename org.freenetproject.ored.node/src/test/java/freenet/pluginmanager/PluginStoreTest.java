...
public class PluginStoreTest extends TestCase {
	...
	/**
	 * These are not allowed in SFS except base64 encoded when the always allow base64
	 * flag is on. So they ARE allowed in PluginStore's.
	 */
	...
	private char randomChar(Random r) {
		...
		if (mode == 0)
			...
			while (true) {
				...
				if (Character.isDefined(ch) && (!Character.isHighSurrogate(ch)) && (!Character.isISOControl(ch))
