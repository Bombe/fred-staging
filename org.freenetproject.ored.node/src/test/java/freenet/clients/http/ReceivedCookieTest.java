...
public class ReceivedCookieTest extends CookieTest {
	...
	static final String validEncodedCookie = " SessionID = \"abCd12345\" ;" + " $Version = 1 ;"
			+ " $Path = \"/Freetalk\";" + " $Discard; " + " $Expires = \"Sun, 25 Oct 2030 15:09:37 GMT\"; " + " $blah;";
	...
	public void testParseHeader() throws ParseException {
		// The tests for getPath(), getName() etc will be executed using the parsed
		// mCookie and therefore also test parseHeader() for valid values,
		...
		cookie = ReceivedCookie.parseHeader(" SessionID = \"abCd12345\" ;" + " $blah;").get(0);
		assertEquals(VALID_NAME.toLowerCase(), cookie.getName());
		assertEquals(VALID_VALUE, cookie.getValue());
