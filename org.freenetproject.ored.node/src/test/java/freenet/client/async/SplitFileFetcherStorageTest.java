...
public class SplitFileFetcherStorageTest extends TestCase {
	...
	static class TestSplitfile {
		...
		private TestSplitfile(Bucket data, Metadata m, byte[][] originalDataBlocks, byte[][] originalCheckBlocks,
		...
		static TestSplitfile constructSingleSegment(long size, int checkBlocks, String mime, boolean persistent)
			...
			Metadata m = new Metadata(SplitfileAlgorithm.ONION_STANDARD, dataKeys, checkKeys, dataBlocks, checkBlocks,
					0, cm, size, null, null, size, false, null, null, size, size, dataBlocks, dataBlocks + checkBlocks,
					false, COMPATIBILITY_MODE, cryptoAlgorithm, cryptoKey, true, 0);
			...
			metaBucket.free();
			copyBucket.free();
			return new TestSplitfile(data, m1, originalDataBlocks, originalCheckBlocks, dataKeys, checkKeys, cryptoKey,
					cryptoAlgorithm, null, null, persistent);
		}

		/**
		 * Create a multi-segment test splitfile. The main complication with multi-segment
		 * is that we can't choose the number of blocks in each segment arbitrarily; that
		 * depends on the metadata format; the caller must ensure that the number are
		 * consistent.
		 * @param size
		 * @param segmentDataBlockCount The actual number of data blocks in each segment.
		 * Must be consistent with the other parameters; this cannot be chosen freely due
		 * to the metadata format.
		 * @param segmentCheckBlockCount The actual number of check blocks in each
		 * segment. Must be consistent with the other parameters; this cannot be chosen
		 ...
		 * @param deductBlocksFromSegments The number of segments from which a single
		 * block has been deducted. This is used when the number of data blocks isn't an
		 * exact multiple of the number of segments.
		 * @param topCompatibilityMode The "short" value of the definitive compatibility
		 * mode used to create the splitfile. This must again be consistent with the rest,
		 * as it is sometimes used in decoding.
		 * @param mime
		 * @return
		 * @throws IOException
		...
		static TestSplitfile constructMultipleSegments(long size, int[] segmentDataBlockCount,
				int[] segmentCheckBlockCount, int segmentSize, int checkSegmentSize, int deductBlocksFromSegments,
				CompatibilityMode topCompatibilityMode, String mime, boolean persistent)
				throws IOException, CHKEncodeException, MetadataUnresolvedException, MetadataParseException {
			...
			Metadata m = new Metadata(SplitfileAlgorithm.ONION_STANDARD, dataKeys, checkKeys, segmentSize,
					checkSegmentSize, deductBlocksFromSegments, cm, size, null, null, size, false, null, null, size,
					size, dataBlocks, dataBlocks + checkBlocks, false, topCompatibilityMode, cryptoAlgorithm, cryptoKey,
			...
			copyBucket.free();
			return new TestSplitfile(data, m1, originalDataBlocks, originalCheckBlocks, dataKeys, checkKeys, cryptoKey,
					cryptoAlgorithm, segmentDataBlockCount, segmentCheckBlockCount, persistent);
		}
		...
		public SplitFileFetcherStorage createStorage(final StorageCallback cb, FetchContext ctx)
			...
			return new SplitFileFetcherStorage(metadata, cb, NO_DECOMPRESSORS, metadata.getClientMetadata(), false,
					(short) COMPATIBILITY_MODE.ordinal(), ctx, false, salt, URI, URI, true, new byte[0], random, bf, f,
					jobRunner, ticker, memoryLimitedJobRunner, new CRCChecksumChecker(), persistent, null, null,
					fetchingKeys);
		}
	...
	public void testSingleSegment() throws CHKEncodeException, IOException, FetchException, MetadataParseException,
			MetadataUnresolvedException {
		// 2 data blocks.
		// testSingleSegment(1, 2, BLOCK_SIZE);
		// We don't test this case because it just copies the data block to the check
		// blocks.
	...
	private void testSingleSegment(int dataBlocks, int checkBlocks, long size) throws CHKEncodeException, IOException,
			FetchException, MetadataParseException, MetadataUnresolvedException {
	...
	private void testRandomMixture(TestSplitfile test)
		...
		for (int i = 0; i < total; i++)
			segment.onNonFatalFailure(i); // We want healing on all blocks that aren't
											// found.
	...
	public void testMultiSegment() throws CHKEncodeException, IOException, MetadataUnresolvedException,
		...
		// We have to be consistent with the format, but we can in fact play with the
		// segment sizes
		// to some degree.
	...
	private void testMultiSegment(long size, int[] segmentDataBlockCount, int[] segmentCheckBlockCount, int segmentSize,
		...
		TestSplitfile test = TestSplitfile.constructMultipleSegments(size, segmentDataBlockCount,
				segmentCheckBlockCount, segmentSize, checkSegmentSize, deductBlocksFromSegments, topCompatibilityMode,
				null, false);
	...
	public void testChooseKeyOneTry() throws CHKEncodeException, IOException, MetadataUnresolvedException,
			MetadataParseException, FetchException {
	...
	private void innerChooseKeyTest(int dataBlocks, int checkBlocks, SplitFileFetcherSegmentStorage storage,
			boolean[] tried, TestSplitfile test, boolean cooldown) {
	...
	public void testChooseKeyThreeTries() throws CHKEncodeException, IOException, MetadataUnresolvedException,
			MetadataParseException, FetchException {
		int dataBlocks = 3, checkBlocks = 3;
		TestSplitfile test = TestSplitfile.constructSingleSegment(dataBlocks * BLOCK_SIZE, checkBlocks, null, false);
		StorageCallback cb = test.createStorageCallback();
	...
	public void testChooseKeyCooldown() throws CHKEncodeException, IOException, MetadataUnresolvedException,
			MetadataParseException, FetchException, InterruptedException {
		int dataBlocks = 3, checkBlocks = 3;
	...
	public void testPersistenceReload() throws CHKEncodeException, IOException, MetadataUnresolvedException,
			MetadataParseException, FetchException, StorageFormatException {
		int dataBlocks = 2;
		int checkBlocks = 3;
		long size = 32768 * 2 - 1;
		assertTrue(dataBlocks * (long) BLOCK_SIZE >= size);
	...
	public void testPersistenceReloadThenFetch() throws IOException, StorageFormatException, CHKEncodeException,
	...
	public void testPersistenceReloadThenChooseKey() throws IOException, StorageFormatException, CHKEncodeException,
			MetadataUnresolvedException, MetadataParseException, FetchException {
		int dataBlocks = 2;
		int checkBlocks = 3;
		long size = 32768 * 2 - 1;
	...
	public void testPersistenceReloadBetweenChooseKey() throws IOException, StorageFormatException, CHKEncodeException,
			MetadataUnresolvedException, MetadataParseException, FetchException {
		int dataBlocks = 2;
		int checkBlocks = 3;
		long size = 32768 * 2 - 1;
		...
		for (int i = 0; i < 3; i++) {
			...
			try {
				...
				if (i != 2)
					throw e; // Already failed on the final iteration, otherwise is an
								// error.
				return;
			}
		}
	...
	public void testPersistenceReloadBetweenFetches() throws IOException, StorageFormatException, CHKEncodeException,
			MetadataUnresolvedException, MetadataParseException, FetchException {
		int dataBlocks = 2;
		int checkBlocks = 3;
		long size = 32768 * 2 - 1;
