 ...
 * <p>
 * This class implements Yarrow-160, a cryptraphically secure PRNG developed by John
 * Kelsey, Bruce Schneier, and Neils Ferguson. It was designed to follow the specification
 * (www.counterpane.com/labs) given in the paper by the same authors, with the following
 * exceptions:
 * </p>
 * <ul>
 * <li>Instead of 3DES as the output cipher, Rijndael was chosen. It was my belief that an
 * AES candidate should be selected. Twofish was an alternate choice, but the AES
 * implementation does not allow easy selection of a faster key-schedule, so twofish's
 * severely impaired performance.</li>
 * <li>h prime, described as a 'size adaptor' was not used, since its function is only to
 * constrain the size of a byte array, our own key generation routine was used instead
 * (See {@link freenet.crypt.Util#makeKey freenet.crypt.Util.makeKey})</li>
 * <li>Our own entropy estimation routines are used, as they use a third-order delta
 * calculation that is quite conservative. Still, its used along side the global
 * multiplier and program- supplied guesses, as suggested.</li>
 * </ul>
 *
...
public class Yarrow extends RandomSource implements PersistentRandomSource {
	...
	Yarrow(File seed, String digest, String cipher, boolean updateSeed, boolean canBlock, boolean reseedOnStartup) {
		...
		if (updateSeed && !(seed.toString()).equals("/dev/urandom")) // Dont try to update
																		// the seedfile if
																		// we know that it
																		// wont be
																		// possible
		...
		if (reseedOnStartup) {
			...
			/**
			 * If we don't reseed at this point, we will be predictable, because the
			 * startup entropy won't cause a reseed.
			 */
			fast_pool_reseed();
			slow_pool_reseed();
		}
	...
	static final int bitTable[][] = { { 0, 0x0 }, { 1, 0x1 }, { 1, 0x3 }, { 1, 0x7 }, { 1, 0xf }, { 1, 0x1f },
			{ 1, 0x3f }, { 1, 0x7f }, { 1, 0xff }, { 2, 0x1ff }, { 2, 0x3ff }, { 2, 0x7ff }, { 2, 0xfff },
			{ 2, 0x1fff }, { 2, 0x3fff }, { 2, 0x7fff }, { 2, 0xffff }, { 3, 0x1ffff }, { 3, 0x3ffff }, { 3, 0x7ffff },
			{ 3, 0xfffff }, { 3, 0x1fffff }, { 3, 0x3fffff }, { 3, 0x7fffff }, { 3, 0xffffff }, { 4, 0x1ffffff },
			{ 4, 0x3ffffff }, { 4, 0x7ffffff }, { 4, 0xfffffff }, { 4, 0x1fffffff }, { 4, 0x3fffffff },
			{ 4, 0x7fffffff }, { 4, 0xffffffff } };
	...
	private int acceptEntropy(EntropySource source, long data, int entropyGuess, double bias) {
		return accept_entropy(data, source,
	...
	private int accept_entropy(long data, EntropySource source, int actualEntropy) {
		...
		byte[] b = new byte[] { (byte) data, (byte) (data >> 8), (byte) (data >> 16), (byte) (data >> 24),
	...
	private int estimateEntropy(EntropySource source, long newVal) {
		...
		/*
		 * delta is now minimum absolute delta. Round down by 1 bit on general principles,
	...
	/**
	 * If entropy estimation is supported, this method will block until the specified
	 * number of bits of entropy are available. If estimation isn't supported, this method
	...
	/**
	 * If the RandomSource has any resources it wants to close, it can do so when this
	 * method is called
	 */
	@Override
	public void close() {
	...
	public static void main(String[] args) throws Exception {
		...
		if ((args.length == 0) || args[0].equalsIgnoreCase("latency")) {
			...
			for (int i = 0; i < 100; i++)
				r.nextBytes(b);
			System.out.println((double) (System.currentTimeMillis() - start) / (100 * b.length) * 1024 + " ms/k");
