 ...
 * authenticating. FIXME This probably needs an internal buffer. Shouldn't be too
 * inefficient provided that any short writes are buffered before they reach here though.
 *
 * @author toad
 */
public class AEADOutputStream extends FilterOutputStream {
	...
	/**
	 * Create an encrypting, authenticating OutputStream. Will write the nonce to the
	 * stream.
	 * @param os The underlying OutputStream.
	 * @param key The encryption key.
	 * @param nonce The nonce. This serves the function of an IV. As a nonce, this MUST be
	 * unique. We will write it to the stream so the other side can pick it up, like an
	 * IV. Should generally be generated from a SecureRandom. The top bit must be 0, i.e.
	 * nonce[0] &= 0x7F.
	 * @param mainCipher The BlockCipher for encrypting data. E.g. AES; not a block mode.
	 * This will be used for encrypting a fairly large amount of data so could be any of
	 * the 3 BC AES impl's.
	 * @param hashCipher The BlockCipher for the final hash. E.g. AES, not a block mode.
	 * This will not be used very much so should be e.g. an AESLightEngine.
	 */
	public AEADOutputStream(OutputStream os, byte[] key, byte[] nonce, BlockCipher hashCipher, BlockCipher mainCipher)
			throws IOException {
		super(os);
		os.write(nonce);
