...
public class ECDSA {
	...
	public enum Curves {
		...
		P256("secp256r1", "SHA256withECDSA", 91, 72), P384("secp384r1", "SHA384withECDSA", 120, 104), P521("secp521r1",
				"SHA512withECDSA", 158, 139);
		...
		static private KeyPair selftest(KeyPairGenerator kg, KeyFactory kf, int modulusSize)
			...
			/*
			 * if(!Arrays.equals(pk2.getEncoded(), pkey)) throw new
			 * Error("Pubkey encoding mismatch");
			 */
			return key;
		}

		static private void selftest_sign(KeyPair key, Signature sig) throws SignatureException, InvalidKeyException {
		...
		private Curves(String name, String defaultHashAlgorithm, int modulusSize, int maxSigSize) {
			...
			try {
				...
				try {
					...
					/* we don't care why we fail, just fallback */
					Logger.warning(this,
							"default Signature provider (" + (sig != null ? sig.getProvider() : null)
	...
	public byte[] sign(byte[]... data) {
		...
		try {
			while (true) {
				// FIXME: we hardcode bouncycastle here because right now that's the only
				// that works
	...
	/**
	 * Sign data and return a fixed size signature. The data does not need to be hashed,
	 * the signing code will handle that for us, using an algorithm appropriate for the
	 * keysize.
	 * @return A zero padded DER signature (maxSigSize). Space Inefficient but
	 * constant-size.
	 */
	...
	/*
	 * Calculates the actual signature length based on the encoded length of the DER
	 * sequence contained in the signature. If decoding fails, a SignatureException is
	 * thrown.
	 */
	...
	public static boolean verify(Curves curve, ECPublicKey key, byte[] signature, int sigoffset, int siglen,
			byte[]... data) {
	...
	/**
	 * Returns an SFS containing: - the private key - the public key - the name of the
	 * curve in use
	 *
