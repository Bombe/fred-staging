 ...
 * <a href="http://tools.ietf.org/html/draft-irtf-cfrg-ocb-00">The OCB
 ...
 * Open-Source Software Implementations of OCB</a> (Jan 9, 2013) &mdash; &ldquo;License
 * 1&rdquo; <br>
 ...
 * <p/>
 * This is a non-binding summary of a legal document (the link above). The parameters of
 * the license are specified in the license document and that document is controlling.
 * </blockquote>
 */
@Deprecated
public class OCBBlockCipher_v149 implements AEADBlockCipher {
	...
	public void init(boolean forEncryption, CipherParameters parameters) throws IllegalArgumentException {
		 ...
		 * (http://www.ietf.org/mail-archive/web/cfrg/current/msg03433.html) debating
		 * including (macSize % 128) in the nonce, in which case this test becomes
		 * simpler:
		 */
		// if (N.length > 15)
		// {
		...
		if (N.length > 16 || (N.length == 16 && (N[0] & 0x80) != 0)) {
			/*
			 * NOTE: We don't just ignore bit 128 because it would hide from the caller
			 * the fact that two nonces differing only in bit 128 are not different.
			 */
		 ...
		 * TODO There's currently a thread on CFRG
		 * (http://www.ietf.org/mail-archive/web/cfrg/current/msg03433.html) debating
		 * including (macSize % 128) in the nonce, in which case this code becomes
		 * simpler:
		 */
