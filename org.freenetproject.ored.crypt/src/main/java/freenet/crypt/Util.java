...
public class Util {
	...
	/**
	 * Fill byte array with random data. randomBytes(random, buf) is same as
	...
	/**
	 * Fill specified range of byte array with random data. Optimised version for Random
	 * with fast nextInt(). Must be same as randomBytesSlowNextInt(buf, from, len).
	...
	public static BlockCipher getCipherByName(String name, int keySize) {
		// throws UnsupportedCipherException {
		try {
			return (BlockCipher) Loader.getInstance("freenet.crypt.ciphers." + name, new Class<?>[] { Integer.class },
