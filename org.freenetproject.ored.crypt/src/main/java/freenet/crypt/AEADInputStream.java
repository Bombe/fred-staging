...
public class AEADInputStream extends FilterInputStream {
	...
	/**
	 * Create a decrypting, authenticating InputStream. IMPORTANT: We only authenticate
	 * when closing the stream, so do NOT use Closer.close() etc and swallow IOException's
	 * on close(), as that's what we will throw if authentication fails. We will read the
	 * nonce from the stream; it functions similarly to an IV.
