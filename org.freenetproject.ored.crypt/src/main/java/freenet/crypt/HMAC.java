...
/**
 * Implements the HMAC Keyed Message Authentication function, as described in the draft
 * FIPS standard.
 */
public enum HMAC {
	...
	public static byte[] mac(HMAC hash, byte[] key, byte[] data) {
		if (key.length != hash.digestSize)
			throw new IllegalArgumentException(
					"Wrong keysize! We're not doing key stretching " + key.length + " expected " + hash.digestSize);
