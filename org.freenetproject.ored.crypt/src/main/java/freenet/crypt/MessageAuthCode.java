...
public final class MessageAuthCode {
	...
	/**
	 * Creates an instance of MessageAuthCode that will use the specified algorithm and
	 * key. Must not be used on algorithms that require an IV, as a specified key but a
	 * random IV is probably not useful for an HMAC.
	 * @param type The MAC algorithm to use
	...
	/**
	 * Creates an instance of MessageAuthCode that will use the specified algorithm and
	 * key which is converted from a byte[] to a SecretKey. Must not be used on algorithms
	 * that require an IV, as a specified key but a random IV is probably not useful for
	 * an HMAC.
	 * @param type The MAC algorithm to use
	 * @param cryptoKey The key to use
	 * @throws InvalidKeyException
	...
	/**
	 * Creates an instance of MessageAuthCode that will use the specified algorithm and
	 * key which is converted from a ByteBuffer to a SecretKey. If that algorithms
	 * requires an IV it will generate one.
	 * @param type The MAC algorithm to use
	 * @param cryptoKey The key to use
	 * @throws InvalidKeyException
	 */
	...
	/**
	 * Adds the remaining bytes from a ByteBuffer to the buffer of bytes to be used for
	 * MAC generation. The bytes read from the ByteBuffer will be from input.position() to
	 * input.remaining(). Upon return, the ByteBuffer's .position() will be equal to
	 * .remaining() and .remaining() will stay unchanged.
	...
	/**
	 * Adds the specified portion of the byte[] passed in to the buffer of bytes to be
	 ...
	 * The buffer is then cleared after the MAC has been generated.
	 * @return The Message Authentication Code. Will have a backing array and array offset
	 * 0, so you can call array() on it if you really must.
	 */
	 ...
	 * processing the input to ensure that no extra data is included. Once the MAC has
	...
	public final static boolean verify(byte[] mac1, byte[] mac2) {
		/*
		 * An April 2015 patch prevented null input from throwing. JVMs without that patch
		 * will throw, so the change is included here for consistent behavior.
		 *
	...
	/**
	 * Verifies that the two MAC addresses passed are equivalent. The two ByteBuffer's
	 * will both be emptied.
	 * @param mac1 First MAC to be verified
	 * @param mac2 Second MAC to be verified
	 * @return Returns true if the MACs match, otherwise false.
	 */
	...
	/**
	 * Generates the MAC of the byte arrays provided and checks to see if that MAC is the
	 * same as the one passed in. The buffer is cleared before processing the input to
	 * ensure that no extra data is included. Once the MAC has been generated, the buffer
	 * is cleared again.
	...
	/**
	 * Changes the current iv to the provided iv. Only works with algorithms that support
	 * IVs.
