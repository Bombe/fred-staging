...
/**
 * KeyGenUtils offers a set of methods to easily generate Keys and KeyPairs for specific
 * algorithms as well as for generating IVs and nonces. It will also take keys stored in
 * byte arrays and put them in SecretKey or KeyPair instances.
 *
 * @author unixninja92
 *
 */
public final class KeyGenUtils {
	...
	/**
	 * Generates a public/private key pair formatted for the algorithm specified and
	 * stores the keys in a KeyPair. Can not handle DSA keys.
	...
	/**
	 * Converts a specified key for a specified algorithm to a PublicKey. Can not handle
	 * DSA keys.
	...
	/**
	 * Converts a specified key for a specified algorithm to a PublicKey which is then
	 * stored in a KeyPair. The private key of the KeyPair is null. Can not handle DSA
	 * keys.
	 * @param type The type of key being passed in
	 * @param pub Public key as byte[]
	...
	/**
	 * Converts the specified key into a SecretKey for the specified algorithm. Checks the
	 * length of the key to make sure it is correct. HMAC does not have a set key length,
	 * so any key is acceptable when using a key of this type.
	...
	/**
	 * Converts an iv in a specified portion of a byte[] and places it in a
	 * IvParameterSpec.
	...
	/**
	 * Derives a ByteBuffer that is 512 bits (32 bytes) long from the given key using the
	 * provided class name and kdfString
	...
	/**
	 * Derives a ByteBuffer of the specified length from the given key using the provided
	 ...
	 * @throws InvalidKeyException
	 */
	private static ByteBuffer deriveBytesTruncated(SecretKey kdfKey, Class<?> c, String kdfString, int len)
			throws InvalidKeyException {
	...
	/**
	 * Derives a SecretKey of the specified type from the given key using the provided
	 * class name and kdfString
	 * @param kdfKey The key to derive from
	 * @param c Class name to use in derivation
	 * @param kdfString String to use in derivation
	 ...
	 * @throws InvalidKeyException
	 */
	public static SecretKey deriveSecretKey(SecretKey kdfKey, Class<?> c, String kdfString, KeyType type)
			throws InvalidKeyException {
		return getSecretKey(type, deriveBytesTruncated(kdfKey, c, kdfString, type.keySize >> 3));
	}

	/**
	 * Derives a IvParameterSpec of the specified type from the given key using the
	 * provided class name and kdfString
	 * @param kdfKey The key to derive from
	 * @param c Class name to use in derivation
	 * @param kdfString String to use in derivation
	 ...
	 * @throws InvalidKeyException
	 */
	public static IvParameterSpec deriveIvParameterSpec(SecretKey kdfKey, Class<?> c, String kdfString, KeyType ivType)
			throws InvalidKeyException {
