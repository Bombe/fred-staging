...
public class ECDH {
	...
	public enum Curves {
		...
		P256("secp256r1", 91, 32), P384("secp384r1", 120, 48), P521("secp521r1", 158, 66);
		...
		static private void selftest_genSecret(KeyPair key, KeyAgreement ka) throws InvalidKeyException {
		...
		private Curves(String name, int modulusSize, int derivedSecretSize) {
			...
			try {
				...
				try {
					...
					/* we don't care why we fail, just fallback */
					Logger.warning(this,
							"default KeyAgreement provider (" + (ka != null ? ka.getProvider() : null)
	...
	/**
	 * Initialize the key pair generators, which in turn will create the global
	 * SecureRandom, which may block waiting for entropy from /dev/random on unix-like
	 * systems. So this should be called on startup during the "may block for entropy"
	 * stage. Note that because this can block, we still have to do lazy initialisation:
	 * We do NOT want to have it blocking *at time of loading the classes*, as this will
	 * likely appear as the node completely failing to load. Running this after fproxy has
	 * started, with a warning timer, allows us to tell the user what is going on if it
	 * takes a while.
	 */
	...
	public byte[] getPublicKeyNetworkFormat() {
		...
		if (ret.length == curve.modulusSize) {
			...
			throw new IllegalStateException(
					"Encoded public key too long: should be " + curve.modulusSize + " bytes but is " + ret.length);
		}
		else {
