 ...
 * algorithm, key, and also an iv if the algorithm requires one. Note that the input and
 * the output are the same length, i.e. these are either stream ciphers or non-padded
 * block ciphers (where it must be called with whole blocks). For a stream cipher, once a
 * CryptByteBuffer is initialised, processed bytes will be treated as a single stream,
 * i.e. repeatedly encrypting the same data will give different results.
 *
 * @author unixninja92
 *
 * Suggested CryptByteBufferType to use: ChaCha128
 */
@SuppressWarnings("deprecation") // Suppresses warnings about RijndaelPCFB being
									// deprecated
public final class CryptByteBuffer implements Serializable {
	 ...
	 * algorithm requires an iv, it will either use the one passed in, or if that is null,
	...
	/**
	 * Creates an instance of CryptByteBuffer that will be able to encrypt and decrypt
	 * sets of bytes using the specified algorithm type with the given key and iv. The iv
	 * will be extracted from the passed in byte[] starting at the offset using the length
	 * provided by type.ivSize
	 * @param type The symmetric algorithm, mode, and key and block size to use
	 * @param key The key that will be used for encryption
	...
	/**
	 * Encrypts the specified section of provided byte[] into a new array returned as a
	 * ByteBuffer. Does not modify the original array. If you are using a RijndaelECB alg
	 * then len must equal the block size.
	...
	/**
	 * Encrypts the provided byte[]. If you are using a RijndaelECB alg then the length of
	 * input must equal the block size.
	...
	/**
	 * Encrypts the provided ByteBuffer, returning a new ByteBuffer. Only reads the bytes
	 * that are actually readable, i.e. from position to limit, so equivalent to get()ing
	 * into a buffer, encrypting that and returning. If you are using a RijndaelECB alg
	 * then the length of input must equal the block size.
	 * @param input The byte[] to be encrypted
	 * @return A new ByteBuffer containing the ciphertext. It will have a backing array
	 * and its arrayOffset() will be 0, its position will be 0 and its capacity will be
	 * the length of the input data.
	 */
	public ByteBuffer encryptCopy(ByteBuffer input) {
		if (input.hasArray())
			return ByteBuffer
					.wrap(encryptCopy(input.array(), input.arrayOffset() + input.position(), input.remaining()));
		else {
	...
	/**
	 * Get bytes from one ByteBuffer and encrypt them and put them into the other
	 * ByteBuffer.
	 */
	public void encrypt(ByteBuffer input, ByteBuffer output) {
		if (input.hasArray() && output.hasArray()) {
			int moved = Math.min(input.remaining(), output.remaining());
			encrypt(input.array(), input.arrayOffset() + input.position(), moved, output.array(),
	...
	// FIXME
	/*
	 * BitSet based operations commented out. If you need them, wait until we are using
	 * java 7, or implement your own toByteArray(). Please don't steal it from OpenJDK
	 * because we are GPL2+ and link ASL2 code, therefore it is illegal for us to use
	 * GPL2-only code such as OpenJDK. Please test very, VERY carefully, as it's essential
	 * that the representation not change when we do switch to using java 7's
	 * BitSet.toByteArray(). The javadocs give a precise definition so you can test it
	 * with unit tests.
	 */
	// /**
	...
	/**
	 * Decrypts the specified section of provided byte[] into an array which is returned
	 * as a ByteBuffer. Does not modify the original array. If you are using a RijndaelECB
	 ...
	 * @return Returns the decrypted plaintext, a newly allocated byte array of the same
	...
	/**
	 * Decrypts the provided byte[]. If you are using a RijndaelECB alg then the length of
	 * input must equal the block size.
	...
	/**
	 * Decrypts the provided ByteBuffer, returning a new ByteBuffer. Only reads the bytes
	 * that are actually readable, i.e. from position to limit, so equivalent to get()ing
	 * into a buffer, decrypting that and returning. If you are using a RijndaelECB alg
	 * then the length of input must equal the block size.
	 * @param input The buffer to be decrypted
	 * @return A new ByteBuffer containing the plaintext. It will have a backing array and
	 * its arrayOffset() will be 0, its position will be 0 and its capacity will be the
	 * length of the input data.
	 */
	public ByteBuffer decryptCopy(ByteBuffer input) {
		if (input.hasArray())
			return ByteBuffer
					.wrap(decryptCopy(input.array(), input.arrayOffset() + input.position(), input.remaining()));
		else
	...
	public void decrypt(ByteBuffer input, ByteBuffer output) {
		if (input.hasArray() && output.hasArray()) {
			int moved = Math.min(input.remaining(), output.remaining());
			decrypt(input.array(), input.arrayOffset() + input.position(), moved, output.array(),
					output.arrayOffset() + output.position());
			input.position(input.position() + moved);
			output.position(output.position() + moved);
		}
