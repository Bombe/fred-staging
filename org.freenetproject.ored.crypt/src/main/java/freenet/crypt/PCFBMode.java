...
/**
 * Control mechanism for the Periodic Cipher Feed Back mode. This is a CFB variant used
 * apparently by a number of programs, including PGP. Thanks to Hal for suggesting it.
 *
 * http://www.streamsec.com/pcfb1.pdf
 *
 * NOTE: This is identical to CFB if block size = key size. As of Freenet 0.7, we use it
 * with block size = key size. Which is not recommended, but is as safe as CFB. We will
 * get rid of this eventually, and move to 128-bit block size (i.e. standard AES) with a
 * more standard mode (e.g. CTR or CBC).
 *
 * @author Scott
 */
public class PCFBMode {
	...
	/**
	 * Create the PCFB with no IV. The caller must either: a) Call reset() with a proper
	 * IV, or b) Accept the initial IV of all zero's. (We will still encrypt this before
	 * using it). If the key is random and never reused, for instance if it is a one time
	 * key or derived from a hash, b) may be acceptable. It is used in some parts of
	 * Freenet. However, it is very bad practice cryptographically, and we should get rid
	 * of it. NOTE THAT IV:KEY PAIRS *MUST* BE UNIQUE! If two instances use the same key
	 * with the same empty IV, the bad guys will be able to XOR the two ciphertexts to get
	 * the XOR of the plaintext. If they can deduce the register's value they may even be
	 * able to decrypt the next 32 bytes, however after that they should hopefully be
	 * stumped - but it will certainly make more sophisticated attacks easier. FIXME
	 * CRYPTO !!!
	...
	protected PCFBMode(BlockCipher c, byte[] iv, int offset) {
		...
		// registerPointer is already set to the end by this(c), so we will refillBuffer()
		// immediately.
	}
	...
	/**
	 * Writes the initialization vector to the stream. Though the IV is transmitted in the
	 * clear, this gives the attacker no additional information because the
	 * registerPointer is set so that the encrypted buffer is empty. This causes an
	 * immediate encryption of the IV, thus invalidating any information that the attacker
	 * had.
	 */
	...
	/**
	 * Deciphers one byte of data, by XOR'ing the ciphertext byte with one byte from the
	 * encrypted buffer. Then places the received byte in the feedback register. If no
	 * bytes are available in the encrypted buffer, the feedback register is encrypted,
	 * providing block_size/8 new bytes for decryption
	 */
	...
	/**
	 * Enciphers one byte of data, by XOR'ing the plaintext byte with one byte from the
	 * encrypted buffer. Then places the enciphered byte in the feedback register. If no
	 * bytes are available in the encrypted buffer, the feedback register is encrypted,
	 * providing block_size/8 new bytes for encryption
	 */
