...
public abstract class StoreCallback<T extends StorableBlock> {
	...
	/**
	 * Length of a routing key. Routing key is what we use to search for a block. Also
	 * fixed.
	 */
	...
	/**
	 * Called when first connecting to a FreenetStore. If the FreenetStore is a wrapper,
	 * it can be called more than once, but the last call will determine which store we
	 * use.
	 */
	...
	/**
	 * Construct a StorableBlock from the data, headers, and optionally routing key or
	 * full key. IMPORTANT: Using the full key or routing key is OPTIONAL, and if we don't
	 * use them, WE DON'T CHECK THEM EITHER! Caller MUST check that the key is the one
	 * expected.
	 * @throws KeyVerifyException
	 */
	public abstract T construct(byte[] data, byte[] headers, byte[] routingKey, byte[] fullKey,
			boolean canReadClientCache, boolean canReadSlashdotCache, BlockMetadata meta, StoreDSAPublicKey knownPubKey)
			throws KeyVerifyException;
	...
	/**
	 * @return The total size in memory of a block with its key and routing key and data
	 * and headers.
	 */
