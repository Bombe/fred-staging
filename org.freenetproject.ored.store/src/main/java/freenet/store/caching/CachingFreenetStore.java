...
public class CachingFreenetStore<T extends StorableBlock> extends ProxyFreenetStore<T> {
	...
	public T fetch(byte[] routingKey, byte[] fullKey, boolean dontPromote, boolean canReadClientCache,
		...
		return backDatastore.fetch(routingKey, fullKey, dontPromote, canReadClientCache, canReadSlashdotCache,
		...
		try {
			if (!shuttingDown) {
				...
				if (!collisionPossible || overwrite) {
					...
					// Is probablyInStore()? If so, remove it from blocksByRoutingKey, and
					// set a flag so we don't call put()
	 ...
	 * @return The number of bytes written to disk if we successfully wrote a block, 0 if
	 * we wrote a block but can't remove it because it changed while we were writing it,
	 * and -1 if there were no blocks to write because the cache is empty.
	 */
	long pushLeastRecentlyBlock() {
		...
		try {
			...
			/**
			 * it might have changed if there was a put() with overwrite=true. If it has
			 * changed, return 0 , i.e. don't remove it
			 */
