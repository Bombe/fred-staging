...
/**
 * Tracks the memory used by a bunch of CachingFreenetStore's, and writes blocks to disk
 * when full or after 5 minutes. One major objective here is we should not do disk I/O
 * inside a lock, all methods should be non-blocking, even if it means the caller needs to
 * do a blocking disk write.
 *
...
public class CachingFreenetStoreTracker {
	...
	/**
	 * Is a write job queued for some point in the next period? There should only be one
	 * such job queued. However if we then run out of memory we will run a job
	 * immediately.
	 */
	private boolean queuedJob;

	/**
	 * Is a write job running right now? This prevents us from running multiple
	 * pushAllCachingStores() in parallel and thus wasting memory, even if we run out of
	 * memory and so have to run a job straight away.
	 */
	...
	/**
	 * register a CachingFreenetStore to be called when we get full or to flush all after
	 * a setted period.
	 */
	...
	/**
	 * If we are close to the limit, we will schedule an off-thread job to flush ALL the
	 * caches. Even if we are not, we schedule one after period. If we are at the limit,
	 * we will return false, and the caller should write directly to the underlying store.
	 */
	public synchronized boolean add(long sizeBlock) {
		/**
		 * Here have a lower threshold, say 90% of maxSize, when it will start a write
		 * job, but still accept the data.
		 */
		...
		if (this.size + sizeBlock > this.maxSize) {
			...
			this.size += sizeBlock;
			if (!justStartedPush) {
				// Write everything to disk after the maximum delay (period), unless there
				// is already
