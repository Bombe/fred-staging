...
public class RAMFreenetStore<T extends StorableBlock> implements FreenetStore<T> {
	...
	@Override
	public synchronized T fetch(byte[] routingKey, byte[] fullKey, boolean dontPromote, boolean canReadClientCache,
		...
		try {
			T ret = callback.construct(block.data, block.header, routingKey, block.fullKey, canReadClientCache,
	...
	@Override
	public synchronized void put(T block, byte[] data, byte[] header, boolean overwrite, boolean isOldBlock)
		...
		if (oldBlock != null) {
			if (callback.collisionPossible()) {
				boolean equals = Arrays.equals(oldBlock.data, data) && Arrays.equals(oldBlock.header, header)
	...
	public synchronized void setMaxKeys(long maxStoreKeys, boolean shrinkNow) throws IOException {
		...
		// Always shrink now regardless of parameter as we will shrink on the next put()
		// anyway.
		while (blocksByRoutingKey.size() > maxKeys) {
			blocksByRoutingKey.popKey();
		}
	...
	public void migrateTo(StoreCallback<T> target, boolean canReadClientCache) throws IOException {
		...
		while (keys.hasMoreElements()) {
			...
			T ret;
			try {
				ret = callback.construct(block.data, block.header, routingKey, block.fullKey, canReadClientCache, false,
						null, null);
			}
			catch (KeyVerifyException e) {
