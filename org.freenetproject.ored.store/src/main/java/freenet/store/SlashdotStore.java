 ...
 * security levels), or just to cache data fetched through ULPRs (on higher security
 * levels). - Strict LRU. - Size limit. - Strictly enforced time limit. - Blocks are
 * encrypted, and kept in temp files.
 *
...
public class SlashdotStore<T extends StorableBlock> implements FreenetStore<T> {
	...
	// PURGING OLD DATA:
	// Every X period? I don't think it matters if we're a few minutes out, and it's
	...
	public SlashdotStore(StoreCallback<T> callback, int maxKeys, long maxLifetime, long purgePeriod, Ticker ticker,
	...
	public T fetch(byte[] routingKey, byte[] fullKey, boolean dontPromote, boolean canReadClientCache,
		...
		try {
			T ret = callback.construct(data, header, routingKey, fk, canReadClientCache, canReadSlashdotCache, null,
					null);
			synchronized (this) {
				hits++;
				if (!dontPromote) {
	...
	/**
	 * @param isOldBlock Ignored, we don't distinguish between stuff that should be cached
	 * and stuff that shouldn't be cached; really it's all in the latter category anyway
	 * here!
	 */
	@Override
